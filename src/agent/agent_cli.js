...
class AgentCLI {
    ...
    setInterval(() => {
      ...
      for (const agent of Object.keys(this.agents)) {
        ...
        if (agent_stats) {
          ...
          dbg.log0(
            `agent_stats_titles - ${agent}: ` + agent_stats_keys.join(', '),
          );
          dbg.log0(
            `agent_stats_values - ${agent}: ` +
              agent_stats_keys.map(key => agent_stats[key]).join(', '),
          );
        }
      }
    }, 60000).unref();
    ...
    return self.agent_conf
      ...
      .then(function (mount_points) {
        ...
        if (self.params.cleanup) {
          return P.all(
            _.map(self.params.all_storage_paths, storage_path_info => {
    ...
    return os_utils
      ...
      .then(added_mount_points => {
        ...
        added_paths = _.filter(
          added_paths,
          added_path =>
            !_.includes(self.params.ignore_drives || [], added_path.drive_id),
        );
        if (_.isEmpty(added_paths)) return P.resolve();
    ...
    return P.all(
      _.map(paths_to_work_on, function (storage_path_info) {
        ...
        return P.resolve()
          ...
          .then(() =>
            P.resolve(self.hide_storage_folder(storage_path)).catch(err => {
  ...
  create_node_helper(current_node_path_info) {
    ...
    return P.fcall(function () {
      ...
      if (os.type().indexOf('Windows') >= 0) {
        node_name =
          node_name + '-' + current_node_path_info.drive_id.replace(':', '');
      ...
      return fs_utils
        ...
        .then(() =>
          fs_utils.create_path(node_path, fs_utils.PRIVATE_DIR_PERMISSIONS),
        )
        .then(function () {
  ...
  create(number_of_nodes, paths_to_work_on) {
    ...
    return P.all(
      _.map(_.drop(storage_paths_to_add, 1), function (current_storage_path) {
        return fs.promises
...
function list_hosts() {
  _.each(hosts, (host, host_name) =>
    console.log(host.running ? '<RUNNING>' : '<STOPPED>', 'host', host_name),
  );
}
