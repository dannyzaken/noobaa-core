...
const FUNC_NODE_PATH = path.resolve(
    __dirname,
    '..',
    '..',
    '..',
...
class FuncNode {
    constructor(params) {
        ...
        this.functions_loading_path = path.join(
            this.storage_path,
            'functions_loading',
        );
                        ...
                        if (msg.error) {
                            return resolve({
                                error: {
                                    message:
                                        msg.error.message ||
                                        'Unknown error from func process',
        ...
        return this.loading_serial.surround(async () => {
            ...
            try {
                try {
                    ...
                    const func_json_buf = await fs.promises.readFile(
                        func_json_path,
                        'utf8',
                    );
                    ...
                    func = await this._write_func_into_dir(
                        code_dir,
                        name,
                        version,
                        code_sha256,
    ...
    async _write_func_into_dir(
        ...
        const loading_dir = path.join(
            this.functions_loading_path,
        ...
        if (
            code_sha256 !== func.config.code_sha256 ||
        ...
        const zipfile = await zip_utils.unzip_from_buffer(
            func[RPC_BUFFERS].zipfile,
        );
        ...
        await fs.promises.writeFile(func_json_path, JSON.stringify(func));
        await P.retry({
            attempts: 3,
            delay_ms: 500,
            func: async () => {
                try {
                    ...
                    dbg.error(
                        'Got error when trying to place new function, will retry',
                        e,
                    );
                    throw e;
                }
