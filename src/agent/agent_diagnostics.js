...
function collect_agent_diagnostics(storage_path) {
    ...
    return base_diagnostics
        ...
        .then(function () {
            diag_log = fs
                .createWriteStream(
                    path.join(TMP_WORK_DIR, 'agent_diagnostics.log'),
                )
                .on('error', err =>
                    console.error('ERROR in agent_diagnostics.log', err),
                );
            return base_diagnostics.collect_basic_diagnostics().catch(err => {
                diag_log.write(
                    `failed collecting basic diagnostics. ${err.message}`,
                );
            });
        })
        .then(function () {
            if (fs.existsSync(process.cwd() + '/logs')) {
                ...
                return fs_utils
                    ...
                    .catch(err => {
                        diag_log.write(
                            `failed collecting log files. ${err.message}`,
                        );
                    });
            }
        })
        .then(function () {
            if (fs.existsSync('/var/log/noobaalocalservice.log')) {
                return fs_utils
                    .file_copy('/var/log/noobaalocalservice.log', TMP_WORK_DIR)
                    .catch(err => {
                        diag_log.write(
                            `failed collecting noobaalocalservice.log. ${err.message}`,
                        );
                    });
            }
        })
        .then(function () {
            const file = fs.createWriteStream(
                TMP_WORK_DIR + '/ls_noobaa_storage.out',
            );
            return fs_utils
                .read_dir_recursive({
                    root: storage_path,
                ...
                .catch(err => {
                    diag_log.write(
                        `failed reading ${storage_path} dir. ${err.message}`,
                    );
                });
        })
