...
class BlockStoreS3 extends BlockStoreBase {
        ...
        if (cloud_utils.is_aws_endpoint(endpoint)) {
            ...
            this.s3cloud = new AWS.S3({
                ...
                s3DisableBodySigning:
                    cloud_utils.disable_s3_compatible_bodysigning(endpoint),
                httpOptions: {
    ...
    async init() {
        try {
            if (this.cloud_info.aws_sts_arn) {
                this.s3cloud = await cloud_utils.createSTSS3Client(
                    this.cloud_info,
                    this.additionalS3Params,
                );
            }
            const res = await this.s3cloud
                .getObject({
    ...
    async _write_block(block_md, data, options) {
        try {
            ...
            const encoded_md =
                this.disable_metadata ? '' : this._encode_block_md(block_md);
            ...
            await this.s3cloud
                .putObject({
                    ...
                    Body: data,
                    Metadata:
                        this.disable_metadata ? undefined : (
    ...
    async test_store_validity() {
        ...
        try {
            ...
            if (err.code !== 'NoSuchKey') {
                if (err.code === 'NoSuchBucket') {
                    dbg.error(
                        `s3 bucket ${this.cloud_info.target_bucket} not found`,
                        err,
                    );
                    throw new RpcError(
                        'STORAGE_NOT_EXIST',
                    ...
                    dbg.error(
                        `access denied to the s3 bucket ${this.cloud_info.target_bucket}`,
                        err,
                    );
                    throw new RpcError(
                        'AUTH_FAILED',
                ...
                dbg.warn(
                    `unexpected error (code=${err.code}) from deleteObject during test. ignoring..`,
                );
            }
        }
    }
    ...
    async _delete_past_versions(key) {
        if (
            this.cloud_info.endpoint_type !== 'AWS' &&
            this.cloud_info.endpoint_type !== 'AWSSTS'
        ) {
            return;
        }
        ...
        while (is_truncated) {
            ...
            if (delete_list.length) {
                dbg.log1(
                    'BlockStoreS3._delete_past_versions: target_bucket',
                    this.cloud_info.target_bucket,
                    'delete_list',
                    delete_list,
                );
                await this.s3cloud
    ...
    async cleanup_target_path() {
        ...
        try {
            ...
            while (!done) {
                const list_res = await this.s3cloud
                    .listObjectVersions({
                    ...
                    .promise();
                const del_objs = list_res.Versions.map(ver => ({
                    Key: ver.Key,
        ...
        await P.map_with_concurrency(10, block_ids, async block_id => {
            try {
                ...
                const res = await this.s3cloud
                    ...
                    .promise();
                const noobaablockmd =
                    res.Metadata.noobaablockmd || res.Metadata.noobaa_block_md;
