    ...
    return P.timeout(
      ...
      (async () => {
        try {
          ...
          const encoded_md = Buffer.from(JSON.stringify(block_md)).toString(
            'base64',
          );
          ...
          const usage =
            data_length ?
              {
                size:
                  (block_md.is_preallocated ? 0 : data_length) +
                  encoded_md.length,
    ...
    return P.timeout(
      ...
      (async () => {
        try {
          ...
          const block_id = block_md.id;
          const block_dir = get_block_internal_dir(block_id);
          const blob = azure_storage.BlobServiceClient.fromConnectionString(
      ...
      (async () => {
        try {
          ...
          const write_params = {
            ...
            Metadata:
              disable_metadata ? undefined : { noobaablockmd: encoded_md },
          };
    ...
    return P.timeout(
      ...
      (async () => {
        try {
          ...
          const noobaablockmd =
            data.Metadata.noobaablockmd || data.Metadata.noobaa_block_md;
      ...
      await Promise.all(
        updates.map(async ([address, update]) => {
          try {
            ...
            await rpc_client.block_store.update_store_usage(io_stats, {
              address,
            });
          } catch (err) {
            dbg.error(
