...
const {
    get_block_internal_dir,
} = require('../../agent/block_store_services/block_store_base');
...
const block_store_info_cache = new LRUCache({
    ...
    load: async ({ rpc_client, options }) =>
        rpc_client.block_store.get_block_store_info({}, options),
});
class BlockStoreClient {
    ...
    write_block(rpc_client, params, options) {
        ...
        switch (block_md.node_type) {
            case 'BLOCK_STORE_S3':
                return this._delegate_write_block_s3(
                    rpc_client,
                    params,
                    options,
                );
            case 'BLOCK_STORE_AZURE':
                return this._delegate_write_block_azure(
                    rpc_client,
                    params,
                    options,
                );
            case 'BLOCK_STORE_GOOGLE':
                return this._delegate_write_block_google(
                    rpc_client,
                    params,
                    options,
                );
    ...
    read_block(rpc_client, params, options) {
        ...
        switch (block_md.node_type) {
            case 'BLOCK_STORE_S3':
                return this._delegate_read_block_s3(
                    rpc_client,
                    params,
                    options,
                );
            case 'BLOCK_STORE_AZURE':
                return this._delegate_read_block_azure(
                    rpc_client,
                    params,
                    options,
                );
            case 'BLOCK_STORE_GOOGLE':
                return this._delegate_read_block_google(
                    rpc_client,
                    params,
                    options,
                );
    ...
    async _delegate_write_block_google(rpc_client, params, options) {
        ...
        return P.timeout(
            ...
            (async () => {
                try {
                    bs_info = await block_store_info_cache.get_with_cache({
                        options,
                        rpc_client,
                    });
                    if (!bs_info) {
                        throw new Error("couldn't resolve cloud credentials");
                    }
                    ...
                    const encoded_md = Buffer.from(
                        JSON.stringify(block_md),
                    ).toString('base64');
                    ...
                    const usage =
                        data_length ?
                            {
                                size:
                                    (block_md.is_preallocated ? 0 : (
                                        data_length
                                    )) + encoded_md.length,
                                count: block_md.is_preallocated ? 0 : 1,
                            }
                        :   { size: 0, count: 0 };
                    this._update_usage_stats(
                        rpc_client,
                        usage,
                        options.address,
                        'WRITE',
                    ...
                    dbg.error(
                        'Google write operation failed for block:',
                        util.inspect(block_md, { depth: 4 }),
                        err,
                    );
                    if (err.code === 403) {
                        ...
                        throw new RpcError(
                            'AUTH_FAILED',
                            `access denied to the google cloud bucket for block ${block_md.id}. got error ${err}`,
                        );
                    } else if (err.code === 404) {
                        dbg.error(
                            'got 404 error when trying to write block. checking if bucket exists',
                        );
                        try {
                            ...
                            if (bucket_err.code === 404) {
                                block_store_info_cache.invalidate_key(
                                    options.address,
                                );
                                throw new RpcError(
                                    'STORAGE_NOT_EXIST',
                                    `google cloud bucket not found for block ${block_md.id}. got error ${err}`,
                                );
                            }
                        }
                    }
        ...
        return P.timeout(
            ...
            (async () => {
                try {
                    ...
                    this._update_usage_stats(
                        rpc_client,
                        { size: params.block_md.size, count: 1 },
                        options.address,
                        'READ',
                    );
                    return {
                        [RPC_BUFFERS]: { data },
                        block_md: JSON.parse(
                            Buffer.from(block_md_b64, 'base64').toString(),
                        ),
                    };
                } catch (err) {
                    dbg.error(
                        'Google read operation failed for block: ',
                        util.inspect(block_md, { depth: 4 }),
                        err,
                    );
                    if (err.code === 403) {
                ...
                try {
                    ...
                    const blob =
                        azure_storage.BlobServiceClient.fromConnectionString(
                            azure_params.connection_string,
                        );
                    ...
                    const blob_client = azure_storage.get_blob_client(
                        container_client,
                        block_key,
                    );
                    dbg.log1(
                        'block_store_client._delegate_write_block_azure uploading block_key:',
                        block_key,
                        ' container: ',
                        container,
                        'data_length: ',
                    ...
                    await blob_client.upload(data, data.length, {
                        metadata: {
                            noobaablockmd: encoded_md,
                        },
                    });
                    dbg.log1(
                        'block_store_client._delegate_write_block_azure uploaded succefully block_key:',
                        block_key,
                        ' container: ',
                        container,
                    );
                    ...
                    dbg.error(
                        'Azure write operation failed for block:',
                        util.inspect(block_md, { depth: 4 }),
                        err,
                    );
                    if (
                        err.details &&
                        err.details.code === 'ContainerNotFound'
                    ) {
                        block_store_info_cache.invalidate_key(options.address);
                        throw new RpcError(
                            'STORAGE_NOT_EXIST',
                            `azure container not found for block ${block_md.id}. got error ${err}`,
                        );
                    } else if (
                        err.details &&
                        err.details.code === 'AuthenticationFailed'
                    ) {
                        block_store_info_cache.invalidate_key(options.address);
                        throw new RpcError(
                            'AUTH_FAILED',
                            `access denied to the azure container for block ${block_md.id}. got error ${err}`,
                        );
                    }
                    throw err;
                }
        ...
        return P.timeout(
            ...
            (async () => {
                try {
                    ...
                    dbg.log1(
                        'block_store_client._delegate_read_block_azure starting download block_key:',
                        block_key,
                        ' container: ',
                        container,
                    );
                    ...
                    dbg.log1(
                        'block_store_client._delegate_read_block_azure downloaded succefully block_key:',
                        block_key,
                        ' container: ',
                        container,
                    );
                    ...
                    const noobaablockmd =
                        response.metadata.noobaablockmd ||
                        response.metadata.noobaa_block_md;
                    const store_block_md = JSON.parse(
                        Buffer.from(noobaablockmd, 'base64').toString(),
                    );
                    this._update_usage_stats(
                        rpc_client,
                        { size: params.block_md.size, count: 1 },
                    ...
                    return {
                        [RPC_BUFFERS]: {
                            data: await buffer_utils.read_stream_join(
                    ...
                    dbg.error(
                        'Azure read operation failed for block:',
                        util.inspect(block_md, { depth: 4 }),
                        err,
                    );
                    if (
        ...
        return P.timeout(
            ...
            (async () => {
                try {
                    ...
                    const encoded_md =
                        disable_metadata ? '' : (
                            Buffer.from(JSON.stringify(block_md)).toString(
                                'base64',
                            )
                        );
                    const write_params = {
                        ...
                        Metadata:
                            disable_metadata ? undefined : (
                                { noobaablockmd: encoded_md }
                            ),
                    };
                    s3_params.httpOptions = {
                        agent: http_utils.get_unsecured_agent(
                            s3_params.endpoint,
                        ),
                    };
                    if (bs_info.connection_params.aws_sts_arn) {
                        const creds = await cloud_utils.generate_aws_sts_creds(
                            s3_params,
                            '_delegate_write_block_s3_session',
                        );
                    ...
                    dbg.error(
                        'S3 write operation failed for block:',
                        util.inspect(block_md, { depth: 4 }),
                        err,
                    );
                    if (err.code === 'NoSuchBucket') {
                        ...
                        throw new RpcError(
                            'STORAGE_NOT_EXIST',
                            `s3 bucket not found for block ${block_md.id}. got error ${err}`,
                        );
                    } else if (err.code === 'AccessDenied') {
                        block_store_info_cache.invalidate_key(options.address);
                        throw new RpcError(
                            'AUTH_FAILED',
                            `access denied to the s3 bucket for block ${block_md.id}. got error ${err}`,
                        );
                    }
                    throw err;
                }
            ...
            (async () => {
                try {
                    ...
                    if (bs_info.connection_params.aws_sts_arn) {
                        const creds = await cloud_utils.generate_aws_sts_creds(
                            s3_params,
                            '_delegate_read_block_s3_session',
                        );
                        s3_params.accessKeyId = creds.accessKeyId;
                        s3_params.secretAccessKey = creds.secretAccessKey;
                        s3_params.sessionToken = creds.sessionToken;
                    ...
                    const noobaablockmd =
                        data.Metadata.noobaablockmd ||
                        data.Metadata.noobaa_block_md;
                    const store_block_md =
                        disable_metadata ? block_md : (
                            JSON.parse(
                                Buffer.from(noobaablockmd, 'base64').toString(),
                            )
                        );
                    ...
                    dbg.error(
                        'S3 read operation failed for block:',
                        util.inspect(block_md, { depth: 4 }),
                        err,
                    );
                    if (err.code === 'NoSuchBucket') {
    ...
    _update_usage_stats(rpc_client, usage, address, read_or_write) {
        ...
        if (read_or_write === 'WRITE') {
            ...
            throw new Error(
                '_update_usage_stats should except READ or WRITE only',
            );
        }
    }
            ...
            await Promise.all(
                updates.map(async ([address, update]) => {
                    try {
                        ...
                        dbg.log0(
                            `Sending io_stats ${JSON.stringify(io_stats)} to block address ${address}`,
                        );
                        await rpc_client.block_store.update_store_usage(
                            io_stats,
                            { address },
                        );
                    } catch (err) {
                        dbg.error(
                            `Failed sending io_stats update to block address ${address}`,
                            err,
                        );
                        this._update_usage_stats(
                            update.rpc_client,
                            _.omit(update, 'rpc_client'),
                            address,
                            'BOTH',
                        );
                    }
