...
class BlockStoreFs extends BlockStoreBase {
  ...
  async init() {
    ...
    for (let i = 0; i < num_dirs; ++i) {
      const dir_str =
        string_utils.left_pad_zeros(i.toString(16), num_digits) + '.blocks';
    ...
    return P.map_with_concurrency(10, dir_list, dir =>
      ...
      .then(() => fs.promises.stat(this.usage_path).catch(ignore_not_found))
      .then(stat => {
        if (stat) {
          return fs.promises
  ...
  async _delete_block(block_id) {
    ...
    await Promise.all([
      block_stat &&
        nb_native().fs.unlink(fs_context, block_path).catch(ignore_not_found),
      meta_stat &&
        nb_native().fs.unlink(fs_context, meta_path).catch(ignore_not_found),
    ]);

    if (this._usage && block_stat) {
      const usage = {
        size: -(
          block_stat.size + (meta_stat && meta_stat.size ? meta_stat.size : 0)
        ),
        count: -1,
      };
      this._update_usage(usage);
