...
class BlockStoreAzure extends BlockStoreBase {
    constructor(options) {
        ...
        this.blob = azure_storage.BlobServiceClient.fromConnectionString(
            this.cloud_info.azure.connection_string,
        );
    ...
    async _read_block(block_md) {
        ...
        dbg.log1(
            'block_store_azure._read_block downloading: ',
            block_key,
            block_md,
        );
        ...
        try {
            ...
            dbg.log1(
                'block_store_azure._read_block download response: ',
                response,
            );
            return {
                ...
                block_md: this._decode_block_md(
            ...
            dbg.error(
                'BlockStoreAzure _read_block failed:',
                this.container_name,
                block_key,
                err,
                err.details && err.details.code,
            );
            if (err.details && err.details.code === 'ContainerNotFound') {
                throw new RpcError(
                    'STORAGE_NOT_EXIST',
                    `azure container ${this.container_name} not found. got error ${err}`,
                );
            } else if (
                err.details &&
                ...
                throw new RpcError(
                    'AUTH_FAILED',
                    `access denied to the azure container ${this.container_name}. got error ${err}`,
                );
            }
            throw err;
        }
    ...
    async _read_block_md(block_md) {
        ...
        const store_block_md = this._decode_block_md(
            block_info.metadata.noobaablockmd ||
                block_info.metadata.noobaa_block_md,
        );
        const store_md5 = block_info.contentMD5;
        dbg.log1(
            'block_store_azure._read_block_md store_block_md: ',
            store_block_md,
            ' store_md5: ',
            store_md5,
        );
    ...
    async _write_block(block_md, data, options) {
        ...
        dbg.log1(
            'block_store_azure._write_block upload: data.length: ',
            data.length,
            ' md: ',
            block_md,
        );
        ...
        try {
            ...
            const usage = {
                size:
                    (block_md.is_preallocated ? 0 : data.length) +
                    encoded_md.length,
            ...
            dbg.error(
                'BlockStoreAzure _write_block failed:',
                this.container_name,
                block_key,
                err.details && err.details.code,
                err,
    ...
    async cleanup_target_path() {
        ...
        try {
            ...
            while (!done) {
                ...
                const iterator = this.container_client
                    .listBlobsFlat({
                        prefix: this.base_path,
                    })
                    .byPage({
                        continuationToken: prev_continuation_token,
                        maxPageSize: 1000,
                    });
                const list_res = (await iterator.next()).value;
                dbg.log1(
                    'block_store_azure.cleanup_target_path list_res: ',
                    list_res,
                );

                if (list_res.segment.blobItems.length !== 0) {
                    await P.map_with_concurrency(
                        10,
                        list_res.segment.blobItems,
                        async entry => {
                            try {
                                dbg.log1(
                                    'block_store_azure.cleanup_target_path deleting blob: ',
                                    entry.name,
                                );

                                await this.container_client.deleteBlob(
                                    entry.name,
                                );
                                dbg.log1(
                                    'block_store_azure.cleanup_target_path deleted blob succefully: ',
                                    entry.name,
                                );
                            } catch (err) {
                                dbg.warn(
                                    'BlockStoreAzure _delete_blocks failed for block',
                                    this.container_name,
                                    entry.name,
                                    err,
                                );
                ...
                dbg.log1(
                    'block_store_azure.cleanup_target_path deleted blob: list_res.segment.blobItems: ',
                    list_res.segment.blobItems,
                );
                dbg.log1(
                    'block_store_azure.cleanup_target_path deleted blob: list_res.continuationToken: ',
                    list_res.continuationToken,
                );
                ...
                if (
                    !continuation_token ||
                    list_res.segment.blobItems.length === 0
                ) {
                    done = true;
                }
            }
        ...
        dbg.log0(
            `completed cleanup of ${total} objects with perfix ${this.base_path}`,
        );
    }
    ...
    _delete_blocks(block_ids) {
        ...
        return P.map_with_concurrency(10, block_ids, async block_id => {
            ...
            try {
                ...
                dbg.log1(
                    'block_store_azure._delete_blocks info_arg: ',
                    info_arg,
                    'block_key: ',
                    block_key,
                );
                ...
                dbg.log1(
                    'block_store_azure._delete_blocks deleted blob: block_key: ',
                    block_key,
                );

                const data_size = Number(info_arg.contentLength);
                const noobaablockmd =
                    info_arg.metadata.noobaablockmd ||
                    info_arg.metadata.noobaa_block_md;
                dbg.log1(
                    'block_store_azure._delete_blocks info: data_size: ',
                    data_size,
                    'noobaablockmd: ',
                    noobaablockmd,
                );
                ...
                dbg.log1(
                    'block_store_azure._delete_blocks err.details.code: ',
                    err.details && err.details.code,
                    err,
                );
    ...
    async test_store_validity() {
        ...
        try {
            ...
            dbg.log1(
                'block_store_azure.test_store_validity: err.details.code',
                err.details && err.details.code,
                'err: ',
                err,
            );
            if (err.details && err.details.code !== 'BlobNotFound') {
                dbg.error(
                    'in _test_cloud_service - deleteBlob failed:',
                    err,
                ...
                if (err.details && err.details.code === 'ContainerNotFound') {
                    throw new RpcError(
                        'STORAGE_NOT_EXIST',
                        `s3 bucket ${this.cloud_info.target_bucket} not found. got error ${err}`,
                    );
                } else if (
                    err.details &&
                    err.details.code === 'AuthenticationFailed'
                ...
                dbg.warn(
                    `unexpected error (code=${err.details && err.details.code}) from deleteBlob during test. ignoring..`,
                );
            }
        }
    ...
    _handle_delegator_error(err, usage, op_type) {
        dbg.log1(
            'block_store_azure._handle_delegator_error: err',
            err,
            'usage: ',
            usage,
            'op_type: ',
        ...
        dbg.error(
            'BlockStoreAzure operation failed:',
            this.container_name,
            err.details && err.details.code,
            err,
        );
    ...
    async _read_usage() {
        ...
        try {
            ...
            if (usage_data) {
                ...
                dbg.log0(
                    'BlockStoreAzure init: found usage data in',
                    this.usage_path,
                    'usage_data = ',
                    this._usage,
                );
            ...
            dbg.log1(
                'block_store_azure._read_usage: err.details.code',
                err.details && err.details.code,
                'err: ',
                err,
            );
