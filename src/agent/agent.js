...
const BlockStoreFs =
    require('./block_store_services/block_store_fs').BlockStoreFs;
const BlockStoreS3 =
    require('./block_store_services/block_store_s3').BlockStoreS3;
const BlockStoreGoogle =
    require('./block_store_services/block_store_google').BlockStoreGoogle;
const BlockStoreMongo =
    require('./block_store_services/block_store_mongo').BlockStoreMongo;
const BlockStoreMem =
    require('./block_store_services/block_store_mem').BlockStoreMem;
const BlockStoreAzure =
    require('./block_store_services/block_store_azure').BlockStoreAzure;
...
class Agent {
    ...
    constructor(params) {
        ...
        this.rpc =
            params.rpc ||
            (params.routing_hint ?
                api.new_rpc_from_base_address(
                    params.address,
                    params.routing_hint,
                )
            :   api.new_rpc_from_routing(
                    api.new_router_from_base_address(params.address),
                ));
        ...
        this.agent_conf =
            params.agent_conf || new json_utils.JsonObjectWrapper();
            ...
            if (params.cloud_info) {
                ...
                if (
                    ...
                    const connection_string =
                        cloud_utils.get_azure_new_connection_string({
                            endpoint: params.cloud_info.endpoint,
                            access_key:
                                params.cloud_info.access_keys.access_key,
                            secret_key:
                                params.cloud_info.access_keys.secret_key,
                        });
                    ...
                    this.block_store = new BlockStoreGoogle(
                        block_store_options,
                    );
                }
    ...
    _update_servers_list(new_list) {
        ...
        if (
            _.isUndefined(
                new_list.find(
                    srv =>
                        srv.address.toLowerCase() ===
                        this.base_address.toLowerCase(),
                ),
            )
        ) {
            new_list.push({
    ...
    _handle_server_change(suggested) {
        ...
        if (suggested) {
            ...
            const current_server = _.remove(
                this.servers,
                srv => srv.address === suggested,
            );
            if (current_server[0]) {
        ...
        while (!done) {
            if (this.shutdown) {
                ...
                try {
                    ...
                    dbg.log0(
                        `_do_heartbeat called. sending HB to ${this.master_address}`,
                    );

                    if (this.connect_attempts > MASTER_MAX_CONNECT_ATTEMPTS) {
                        dbg.error(
                            'too many failure to connect, switching servers',
                        );
                        await this._handle_server_change();
                        throw new Error(
                            'server change after too many attempts',
                        );
                    }
                    ...
                    dbg.log0(
                        'heartbeat successful. connected to server. got reply',
                        req.reply,
                    );
                    ...
                    if (err.rpc_code === 'DUPLICATE') {
                        ...
                        if (this.cloud_info || this.mongo_info) {
                            ...
                            throw new Error(
                                'found duplicated cloud or mongo node',
                            );
                        } else {
                    ...
                    if (err.rpc_code === 'NODE_NOT_FOUND') {
                        ...
                        if (this.cloud_info || this.mongo_info) {
                            ...
                            throw new Error(
                                'node not found cloud or mongo node',
                            );
                        } else {
    ...
    _start_stop_server() {
        ...
        if (addr_url.protocol === 'n2n:') {
            ...
            addr_url.protocol === 'ws:' ||
            addr_url.protocol === 'http:'
        ) {
            ...
            const tls_server = this.rpc.create_tcp_server(
                addr_url.port,
                this.ssl_context,
            );
            tls_server.on('close', () => {
    ...
    _authenticate_agent_api(req) {
        ...
        if (
            ...
            dbg.error(
                `agent api not allowed for method ${req.method_api.fullname}`,
                'connection',
                req.connection?.connid,
                req.connection?.url,
                'server',
                this._server_connection?.connid,
            );
            throw new RpcError(
                'FORBIDDEN',
                `agent api not allowed for method ${req.method_api.fullname}`,
        ...
        if (req_protocol !== allowed_protocol) {
            ...
            setTimeout(() => req.connection.close(), 1000);
            dbg.error(
                `agent api not allowed for protocol ${req_protocol}`,
                'connection',
                req.connection?.connid,
                req.connection?.url,
                'allowed protocol',
    ...
    async _update_rpc_config_internal(params) {
        ...
        if (
            params.rpc_address &&
            params.rpc_address !== params.old_rpc_address
        ) {
            dbg.log0(
                'new rpc_address',
                params.rpc_address,
                'old',
                params.old_rpc_address,
            );
        ...
        if (
            params.base_address &&
            params.base_address.toLowerCase() !==
                this.base_address.toLowerCase()
        ) {
            dbg.log0(
                'new base_address',
            ...
            try {
                ...
                throw new Error(
                    `failed ping to new base_address ${params.base_address}`,
                );
            }
            ...
            P.timeout(MASTER_RESPONSE_TIMEOUT, this.client.node.ping())
                ...
                .catch(err => {
                    if (err instanceof P.TimeoutError) {
                        dbg.error(
                            'node_server did not respond to ping. closing connection',
                            err,
                        );
                        this._server_connection.close();
                        this._test_connection_timeout = null;
    ...
    get_agent_info_and_update_masters(req) {
        ...
        dbg.log0_throttled(
            'Recieved potential servers list',
            req.rpc_params.addresses,
        );
        ...
        const reply = {
            ...
            io_stats:
                this.block_store && this.block_store.get_and_reset_io_stats(),
        };
        ...
        return P.resolve()
            .then(
                () =>
                    ...
                    os_utils
                        ...
                        .catch(err =>
                            dbg.warn('failed to get detailed os info', err),
                        ),
            )
            ...
            .then(storage_info => {
                if (storage_info) {
                    ...
                    if (storage_info.used < MIN_USED_STORAGE) {
                        dbg.log0(
                            `used storage is under 200 KB (${storage_info.used}), treat as 0`,
                        );
                        storage_info.used = 0;
                    }
            ...
            .then(drives => {
                ...
                const used_drives = _.filter(drives, drive => {
                    ...
                    if (
                        this.storage_path_mount === drive.mount ||
                        !this.storage_path_mount
                    ) {
    ...
    update_create_node_token(req) {
        const dbg = this.dbg;
        dbg.log0(
            'update_create_node_token: received new token',
            req.rpc_params,
        );
        return this.create_node_token_wrapper.write(
            req.rpc_params.create_node_token,
        );
    }
    ...
    test_network_perf(req) {
        const dbg = this.dbg;
        const data =
            req.rpc_params[RPC_BUFFERS] && req.rpc_params[RPC_BUFFERS].data;
            ...
            return this.client.agent
                ...
                .then(io_req => {
                    ...
                    if (
                        ...
                        throw new Error(
                            'test_network_perf_to_peer: response_length mismatch',
                        );
                    }
    ...
    collect_diagnostics(req) {
        ...
        return P.resolve()
            ...
            .catch(err => {
                ...
                throw new Error(
                    'Agent Collect Diag Error on collecting diagnostics',
                );
            })
            .then(() => {
                ...
                return fs.promises
                    ...
                    .catch(err => {
                        ...
                        throw new Error(
                            'Agent Collect Diag Error on reading packages diag file',
                        );
                    });
            })
            .catch(err => {
    ...
    async send_message_and_exit(message_code, exit_code) {
        ...
        if (process.send) {
            dbg.log0(
                `sending message ${message_code} to parent process (agent_wrapper)`,
            );
            ...
            dbg.log0(
                `no messages channel to parent process. exit with code ${exit_code}`,
            );
            process.exit(exit_code);
        }
    }
