...
class NooBaaCoreReport extends BasePrometheusReport {
        ...
        if (this.enabled) {
            ...
            for (const m of NOOBAA_CORE_METRICS) {
                if (!m.type && !this.prom_client[m.type]) {
                    dbg.warn(
                        `noobaa_core_report - Metric ${m.name} has an unknown type`,
                    );
                    continue;
                }
    ...
    set_unhealthy_cloud_types(types) {
        ...
        this._metrics.unhealthy_cloud_types.set(
            { type: 'AWS' },
            types.unhealthy_pool_target.amazon_unhealthy,
        );
        this._metrics.unhealthy_cloud_types.set(
            { type: 'Azure' },
            types.unhealthy_pool_target.azure_unhealthy,
        );
        this._metrics.unhealthy_cloud_types.set(
            { type: 'GCP' },
            types.unhealthy_pool_target.gcp_unhealthy,
        );
        this._metrics.unhealthy_cloud_types.set(
            { type: 'Kubernetes' },
            types.unhealthy_pool_target.kubernetes_unhealthy,
        );
        // TODO: Fill this up when we will know how to recognize
        // this._metrics.unhealthy_cloud_types.set({ type: 'Ceph' }, types.unhealthy_pool_target.ceph_unhealthy);
        this._metrics.unhealthy_cloud_types.set(
            { type: 'S3_Compatible' },
            types.unhealthy_pool_target.s3_comp_unhealthy,
        );
    }
    ...
    set_bucket_class_capacity_usage(usage_info) {
        ...
        for (const [bucket_class, value] of Object.entries(usage_info)) {
            this._metrics.bucket_class_capacity_usage.set(
                { bucket_class },
                value,
            );
        }
    ...
    set_accounts_io_usage(accounts_info) {
        ...
        accounts_info.accounts.forEach(account_info => {
            const { account, read_count, write_count, read_write_bytes } =
                account_info;
            this._metrics.accounts_usage_logical.set(
                { account },
                read_write_bytes,
            );
            this._metrics.accounts_usage_read_count.set(
                { account },
                read_count,
            );
            this._metrics.accounts_usage_write_count.set(
                { account },
                write_count,
            );
        });
        ...
        for (const bin of sizes) {
            if (!_.isEmpty(bin.bins)) {
                for (const cur of bin.bins) {
                    this._metrics.object_histo.set(
                        { size: cur.label, avg: cur.avg },
                        cur.count,
                        new Date(),
                    );
                }
    ...
    set_system_info(info) {
        ...
        this._metrics.system_info.set(
            { system_name: info.name, system_address: info.address },
            Date.now(),
        );
    }
    ...
    set_system_links(links) {
        ...
        this._metrics.system_links.set(
            {
                resources: links.resources,
                buckets: links.buckets,
    ...
    set_bucket_status(buckets_info) {
        ...
        buckets_info.forEach(bucket_info => {
            ...
            if (bucket_info.tagging && bucket_info.tagging.length) {
                const tagging = bucket_info.tagging.map(
                    tag => `{ ${tag.key} : ${tag.value} }`,
                );
                this._metrics.bucket_tagging.set(
                    { ...bucket_labels, tagging },
                    Date.now(),
                );
            }
            this._metrics.bucket_status.set(
                bucket_labels,
                Number(bucket_info.is_healthy),
            );
            this._metrics.bucket_size_quota.set(
                { bucket_name: bucket_info.bucket_name },
                bucket_info.quota_size_precent,
            );
            this._metrics.bucket_quantity_quota.set(
                { bucket_name: bucket_info.bucket_name },
                bucket_info.quota_quantity_percent,
            );
            this._metrics.bucket_capacity.set(
                { bucket_name: bucket_info.bucket_name },
                bucket_info.capacity_precent,
            );
            this._metrics.bucket_used_bytes.set(
                { bucket_name: bucket_info.bucket_name },
                bucket_info.bucket_used_bytes,
            );
        });
    }
    ...
    set_namespace_bucket_status(buckets_info) {
        ...
        buckets_info.forEach(bucket_info => {
            ...
            if (bucket_info.tagging && bucket_info.tagging.length) {
                ...
                this._metrics.namespace_bucket_tagging.set(
                    { ...bucket_labels, tagging },
                    Date.now(),
                );
            }
            this._metrics.namespace_bucket_status.set(
                bucket_labels,
                Number(bucket_info.is_healthy),
            );
        });
    ...
    set_resource_status(resources_info) {
        ...
        resources_info.forEach(resource_info => {
            this._metrics.resource_status.set(
                { resource_name: resource_info.resource_name },
                Number(resource_info.is_healthy),
            );
        });
    }
    ...
    set_namespace_resource_status(namespace_resources_info) {
        ...
        namespace_resources_info.forEach(namespace_resource_info => {
            this._metrics.namespace_resource_status.set(
                {
                    namespace_resource_name:
                        namespace_resource_info.namespace_resource_name,
                },
    ...
    set_replication_status(repl_info) {
        ...
        delete this._metrics.replication_status.hashMap[
            String(repl_info.replication_id)
        ];
        this._metrics.replication_status.set(
            _.omit(repl_info, [
                'last_cycle_writes_size',
                'last_cycle_writes_num',
                'last_cycle_error_writes_size',
                'last_cycle_error_writes_num',
            ]),
            Date.now(),
        );

        delete this._metrics.replication_last_cycle_writes_size.hashMap[
            String(repl_info.replication_id)
        ];
        this._metrics.replication_last_cycle_writes_size.set(
            { replication_id },
            repl_info.last_cycle_writes_size,
        );

        delete this._metrics.replication_last_cycle_writes_num.hashMap[
            String(repl_info.replication_id)
        ];
        this._metrics.replication_last_cycle_writes_num.set(
            { replication_id },
            repl_info.last_cycle_writes_num,
        );

        delete this._metrics.replication_last_cycle_error_writes_size.hashMap[
            String(repl_info.replication_id)
        ];
        this._metrics.replication_last_cycle_error_writes_size.set(
            { replication_id },
            repl_info.last_cycle_error_writes_size,
        );

        delete this._metrics.replication_last_cycle_error_writes_num.hashMap[
            String(repl_info.replication_id)
        ];
        this._metrics.replication_last_cycle_error_writes_num.set(
            { replication_id },
            repl_info.last_cycle_error_writes_num,
        );
    }
}
