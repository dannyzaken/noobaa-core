...
class MasterKeysManager {
  ...
  _resolve_master_key(m_key) {
    ...
    const m_of_mkey_id =
      (m_key.master_key_id && m_key.master_key_id._id) || m_key.master_key_id;
  ...
  decrypt_value_with_master_key_id(value, _id) {
    ...
    if (m_key.cipher_type !== 'aes-256-gcm') {
      deciphered_value = Buffer.concat([deciphered_value, decipher.final()]);
    }
      ...
      if (pool.cloud_pool_info && pool.cloud_pool_info.access_keys) {
        if (
          !this.is_m_key_disabled(
            pool.cloud_pool_info.access_keys.account_id.master_key_id._id,
          )
        ) {
