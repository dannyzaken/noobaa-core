...
class MasterKeysManager {
    constructor() {
        ...
        this.secret_keys_cache = new LRUCache({
            ...
            load: params => {
                ...
                if (!decipher) {
                    const m_key = this.get_master_key_by_id(
                ...
                return new SensitiveString(
                    decipher
                        .update(Buffer.from(params.encrypted_value, 'base64'))
                        .toString(),
                );
            },
        });
    ...
    load_root_key() {
        ...
        const { NOOBAA_ROOT_SECRET } = process.env;
        if (!NOOBAA_ROOT_SECRET && !this.active_root_key) {
            throw new Error('NON_EXISTING_ROOT_KEY');
        }
    ...
    async load_root_keys_from_mount() {
        ...
        const active_root_key_path = path.join(
            config.ROOT_KEY_MOUNT,
            'active_root_key',
        );
        const active_key_stat = await fs.promises.stat(active_root_key_path);
        dbg.log1(
            `load_root_keys_from_mount: Root keys was last updated at: ${this.last_load_time}. ` +
        ...
        if (
            dir_stat.ctime < this.last_load_time &&
            active_key_stat.ctime < this.last_load_time
        ) {
            return;
        }
        this.last_load_time = new Date();
        const root_keys = await fs.promises.readdir(config.ROOT_KEY_MOUNT);
        const active_root_key_id = await fs.promises.readFile(
            active_root_key_path,
            'utf8',
        );
        for (const key_id of root_keys) {
            ...
            if (key_id === 'active_root_key' || key_id.startsWith('..')) {
                continue;
            }
            const current_key_path = path.join(config.ROOT_KEY_MOUNT, key_id);
            const key_cipher = await fs.promises.readFile(
                current_key_path,
                'utf8',
            );
            const r_key = this._add_to_resolved_keys(
                key_id,
                key_cipher,
                key_id !== active_root_key_id,
            );
            this.root_keys_by_id[key_id] = r_key;
        }
        this.active_root_key = active_root_key_id;
        dbg.log0(
            `load_root_keys_from_mount: Root keys was updated at: ${this.last_load_time}. ` +
    ...
    new_master_key(options) {
        const { description, master_key_id, root_key_id, cipher_type } =
            options;
        const _id = db_client.new_object_id();
        const master_id =
            (master_key_id && db_client.parse_object_id(master_key_id)) ||
            undefined;
        ...
        encrypted_m_key.cipher_key = this.encrypt_buffer_with_master_key_id(
            m_key.cipher_key,
            root_key_id || master_key_id,
        );
    ...
    get_master_key_by_id(_id) {
        ...
        return (
            this.resolved_master_keys_by_id[_id.toString()] ||
    ...
    _resolve_master_key(m_key) {
        ...
        if (
            !NOOBAA_ROOT_SECRET &&
            this.is_root_key(m_key.master_key_id) &&
            !m_key.root_key_id
        ) {
        ...
        const m_of_mkey_id =
            (m_key.master_key_id && m_key.master_key_id._id) ||
            m_key.master_key_id;
        const m_of_mkey = this.get_master_key_by_id(
            m_key.root_key_id || m_of_mkey_id || ROOT_KEY,
        );
        if (!m_of_mkey) throw new Error('NO_SUCH_KEY');

        const iv = m_of_mkey.cipher_iv || Buffer.alloc(16);
        const decipher = crypto.createDecipheriv(
            m_of_mkey.cipher_type,
            m_of_mkey.cipher_key,
            iv,
        );
        // cipher_key is Buffer and after load system - cipher_key is binary.
        const data =
            Buffer.isBuffer(m_key.cipher_key) ?
                m_key.cipher_key
            :   Buffer.from(m_key.cipher_key.buffer, 'base64');
        let cipher_key = decipher.update(data);
        if (m_key.cipher_type !== 'aes-256-gcm') {
            cipher_key = Buffer.concat([cipher_key, decipher.final()]);
        }
        ...
        if (!Buffer.isBuffer(decrypted_master_key.cipher_iv)) {
            ...
            decrypted_master_key.cipher_iv = Buffer.from(
                decrypted_master_key.cipher_iv.buffer,
                'base64',
            );
        }
        this.resolved_master_keys_by_id[m_key._id.toString()] =
    ...
    _reencrypt_master_key(m_key_id, new_father_m_key_id) {
        ...
        const reencrypted_mkey = this.encrypt_buffer_with_master_key_id(
            decrypted_mkey.cipher_key,
            new_father_m_key_id,
        );
    ...
    _reencrypt_master_key_by_current_root(m_key_id, new_root_key_id) {
        const decrypted_mkey = this.get_master_key_by_id(m_key_id); // returns the decrypted m_key
        if (!decrypted_mkey) throw new Error('NO_SUCH_KEY');
        const encrypted_key = this.encrypt_buffer_with_master_key_id(
            decrypted_mkey.cipher_key,
            new_root_key_id,
        );
    ...
    encrypt_buffer_with_master_key_id(value, _id) {
        ...
        if (m_key.cipher_type !== 'aes-256-gcm') {
            ciphered_value = Buffer.concat([ciphered_value, cipher.final()]);
        }
    ...
    decrypt_value_with_master_key_id(value, _id) {
        ...
        if (m_key.cipher_type !== 'aes-256-gcm') {
            deciphered_value = Buffer.concat([
                deciphered_value,
                decipher.final(),
            ]);
        }
    ...
    encrypt_sensitive_string_with_master_key_id(value, _id) {
        ...
        if (this.is_m_key_disabled(m_key._id)) {
            console.log(
                'encrypt_sensitive_string_with_master_key_id: master key is disabled, ignoring encrypting...',
            );
            return value;
        }
        const cipher = crypto.createCipheriv(
        ...
        if (m_key.cipher_type !== 'aes-256-gcm') {
            updated_value = Buffer.concat([updated_value, cipher.final()]);
        }
    ...
    is_m_key_disabled(id) {
        const db_m_key =
            this.is_root_key(id) ?
                this.get_root_key()
            :   this.master_keys_by_id[id.toString()];
        const db_r_key = this.root_keys_by_id[id.toString()];
        if (!db_m_key && !db_r_key) {
            throw new Error(
                `is_m_key_disabled: master/root key id ${id} was not found`,
            );
        }
        return db_m_key ?
                db_m_key.disabled === true
            :   db_r_key.disabled === true;
    }

    set_m_key_disabled_val(_id, val) {
        if (!_id) {
            throw new Error(
                `set_m_key_disabled_val: master key id ${_id} was not found`,
            );
        }
        const m_key = this.get_master_key_by_id(_id);
        if (!m_key) throw new Error('NO_SUCH_KEY');
    ...
    async decrypt_all_accounts_secret_keys({
        accounts,
        pools,
        namespace_resources,
    }) {
            ...
            if (account.master_key_id && account.master_key_id._id) {
                ...
                if (account.access_keys) {
                    for (const keys of account.access_keys) {
                        ...
                        keys.secret_key =
                            await this.secret_keys_cache.get_with_cache(
                                {
                ...
                if (account.sync_credentials_cache) {
                    for (const keys of account.sync_credentials_cache) {
                        ...
                        if (keys.azure_log_access_keys) {
                            keys.azure_log_access_keys.azure_client_secret =
                                await this.secret_keys_cache.get_with_cache(
                                    {
                                        encrypted_value:
                                            keys.azure_log_access_keys.azure_client_secret.unwrap(),
                                        decipher: crypto.createDecipheriv(
                                            m_key.cipher_type,
                                            m_key.cipher_key,
                                            m_key.cipher_iv,
        ...
        for (const pool of pools) {
            if (pool.cloud_pool_info && pool.cloud_pool_info.access_keys) {
                if (
                    !this.is_m_key_disabled(
                        pool.cloud_pool_info.access_keys.account_id
                            .master_key_id._id,
                    )
                ) {
                    pool.cloud_pool_info.access_keys.secret_key =
                        await this.secret_keys_cache.get_with_cache(
                            {
                                encrypted_value:
                                    pool.cloud_pool_info.access_keys.secret_key.unwrap(),
                                undefined,
                                master_key_id:
                                    pool.cloud_pool_info.access_keys.account_id
                                        .master_key_id._id,
        ...
        for (const ns_resource of namespace_resources) {
            if (ns_resource.connection) {
                if (
                    !this.is_m_key_disabled(
                        ns_resource.account.master_key_id._id,
                    )
                ) {
                    ns_resource.connection.secret_key =
                        await this.secret_keys_cache.get_with_cache(
                            {
                                encrypted_value:
                                    ns_resource.connection.secret_key.unwrap(),
                                undefined,
                                master_key_id:
                                    ns_resource.account.master_key_id._id,
                            },
                    ...
                    if (ns_resource.connection.azure_log_access_keys) {
                        ns_resource.connection.azure_log_access_keys.azure_client_secret =
                            await this.secret_keys_cache.get_with_cache(
                                {
                                    encrypted_value:
                                        ns_resource.connection.azure_log_access_keys.azure_client_secret.unwrap(),
                                    undefined,
                                    master_key_id:
                                        ns_resource.account.master_key_id._id,
                                },
