...
function create_tier(req) {
    ...
    const pools = attached_pools.map(
        pool_name => req.system.pools_by_name[pool_name],
    );
    ...
    const mirrors = _convert_pools_to_data_placement_structure(
        policy_pool_ids,
        req.rpc_params.data_placement,
    );
    ...
    if (req.rpc_params.data_placement) {
        tier.data_placement = req.rpc_params.data_placement;
    }
    changes.insert.tiers = [tier];

    dbg.log0('Creating new tier', tier);
    return system_store.make_changes(changes).then(function () {
        req.load_auth();
...
function read_tier(req) {
    ...
    return Promise.all([
        nodes_client
            .instance()
            .aggregate_nodes_by_pool(_.compact(pool_names), req.system._id),
        nodes_client
            .instance()
            .aggregate_data_free_by_tier([String(tier._id)], req.system._id),
    ]).then(function (res) {
        const nodes_aggregate_pool = res[0];
        const available_to_upload = res[1];
        return get_tier_info(tier, nodes_aggregate_pool, {
            mirror_storage: available_to_upload[String(tier._id)],
        });
    });
}
...
function update_tier(req) {
    ...
    throw_on_invalid_pools_for_tier(
        attached_pools.map(pool_name => req.system.pools_by_name[pool_name]),
    );
    ...
    if (req.rpc_params.data_placement) {
        ...
        if (req.rpc_params.attached_pools) {
            ...
            _.forEach(tier.mirrors, mirror_object => {
                old_pool_names = _.concat(
                    old_pool_names,
                    _.map(
                        (mirror_object && mirror_object.spread_pools) || [],
                        pool => pool.name,
                    ),
                );
            });
    ...
    return system_store.make_changes(changes).then(res => {
        const bucket = find_bucket_by_tier(req);
        if (bucket) {
            if (req.rpc_params.data_placement) {
                //Placement policy changes
                const desc_string = [];
                const policy_type_change =
                    (
                        String(tier.data_placement) ===
                        String(req.rpc_params.data_placement)
                    ) ?
                        'No changes'
                    :   `Changed to ${req.rpc_params.data_placement} from ${tier.data_placement}`;
                const removed_pools = _.difference(
                    old_pool_names,
                    req.rpc_params.attached_pools || [],
                );
                const added_pools = _.difference(
                    req.rpc_params.attached_pools || [],
                    old_pool_names,
                );
                desc_string.push(
                    `Bucket policy was changed by: ${req.account && req.account.email.unwrap()}`,
                );
                desc_string.push(`Policy type: ${policy_type_change}`);
                if (removed_pools.length) {
                    desc_string.push(
                        `Removed resources: ${removed_pools.join(', ')}`,
                    );
                }
                if (added_pools.length) {
                    desc_string.push(
                        `Added resources: ${added_pools.join(', ')}`,
                    );
                }
            ...
            if (req.rpc_params.chunk_coder_config && chunk_config_changed) {
                const new_cc_type =
                    req.rpc_params.chunk_coder_config.parity_frags ?
                        'Erasure Coding'
                    :   'Replication';
                const old_cc_type =
                    tier.chunk_config.chunk_coder_config.parity_frags ?
                        'Erasure Coding'
                    :   'Replication';
                let desc_string;
...
function create_policy(req) {
    ...
    return system_store
        .make_changes({ insert: { tieringpolicies: [policy] } })
        .then(() => {
            req.load_auth();
...
function update_policy(req) {
    ...
    return system_store
        .make_changes({ update: { tieringpolicies: [updates] } })
        .then(() => {
            req.load_auth();
            const created_policy = find_policy_by_name(req);
            return get_tiering_policy_info(created_policy);
...
function _check_validity_of_configuration({ req, bucket_class_buckets }) {
    ...
    if (
        !_.every(
            bucket_class_buckets,
            bucket => tiers.length >= bucket.tiering.tiers.length,
        )
    ) {
        throw new RpcError('BAD_REQUEST', 'cannot remove tiers');
    }
    try {
        const pools = [];
        tiers.forEach(tier => {
            if (
                tier.data_placement === 'MIRROR' &&
                tier.attached_pools.length < 2
            ) {
                throw new RpcError(
                    'BAD_REQUEST',
                    'cannot mirror on less than 2 pools',
                );
            }
            throw_on_invalid_pools_for_tier(
                (tier.attached_pools || []).map(
                    pool_name => req.system.pools_by_name[pool_name],
                ),
            );
            if (_.some(tier.attached_pools, pool => _.includes(pools, pool))) {
                throw new RpcError(
                    'BAD_REQUEST',
                    "can't use pool for more than one tier",
                );
            } else {
    ...
    try {
        ...
        for (const bucket of bucket_class_buckets) {
            ...
            for (const [index, tier] of tiers.entries()) {
                ...
                if (tier.data_placement) {
                    new_tier.data_placement = tier.data_placement;
                }
                if (old_tier) {
                    const old_tier_full =
                        req.system.tiers_by_name[old_tier.unwrap()];
            ...
            new_policy._id =
                req.system.tiering_policies_by_name[
                    old_policy.name.unwrap()
                ]._id;
            if (added_tier) {
...
function _calc_revert_policy(policy_name, tiering) {
    ...
    return {
        ...
        tiers: tiering.tiers.map(t => ({
            ...
            attached_pools: _.map(
                _.flatMap(t.tier.mirrors, 'spread_pools'),
                pool => pool.name,
            ),
        })),
    };
...
function _is_change_in_tier(old_tier, new_tier) {
    ...
    for (const [mirror_index, mirror] of old_tier.mirrors.entries()) {
        for (const [pool_index, pool] of mirror.spread_pools.entries()) {
            if (
                String(pool) !==
                String(new_tier.mirrors[mirror_index].spread_pools[pool_index])
            ) {
                return true;
            }
        }
...
function update_chunk_config_for_bucket(req) {
    // please remove when CCC is per tier and not per policy
    const bucket =
        req.system.buckets_by_name &&
        req.system.buckets_by_name[req.rpc_params.bucket_name];
    if (!bucket || bucket.deleting) {
        dbg.error('BUCKET NOT FOUND', req.rpc_params.bucket_name);
        throw new RpcError(
            'NO_SUCH_BUCKET',
            'No such bucket: ' + req.rpc_params.bucket_name,
        );
    }
...
function add_tier_to_bucket(req) {
    ...
    const policy_pool_ids = _.map(
        tier_params.attached_pools,
        pool_name => req.system.pools_by_name[pool_name]._id,
    );
    const mirrors = _convert_pools_to_data_placement_structure(
        policy_pool_ids,
    ...
    const chunk_config = chunk_config_utils.resolve_chunk_config(
        req.rpc_params.chunk_coder_config || tier0_ccc,
        req.account,
        req.system,
    );
    ...
    if (order < 0 || order > policy.tiers.length) {
        throw new RpcError(
            'ILLEGAL_ORDER',
            `tier order ${order} is not allowed`,
        );
    }
...
function read_policy(req) {
    ...
    return Promise.all([
        nodes_client
            .instance()
            .aggregate_nodes_by_pool(pool_names, req.system._id),
        nodes_client
            .instance()
            .aggregate_hosts_by_pool(pool_names, req.system._id),
    ]).then(function (res) {
        const nodes_aggregate_pool = res[0];
        const hosts_aggregate_pool = res[1];
        return get_tiering_policy_info(
            policy,
...
function find_bucket_by_tier(req) {
    const tier = find_tier_by_name(req);
    const policy = _.find(system_store.data.tieringpolicies, p =>
        _.find(p.tiers, t => String(t.tier._id) === String(tier._id)),
    );
...
function find_tier_by_name(req, tier_name) {
    ...
    const tier =
        req.system.tiers_by_name && req.system.tiers_by_name[name.unwrap()];
    if (!tier) {
...
function find_policy_by_name(req) {
    ...
    const policy =
        req.system.tiering_policies_by_name &&
        req.system.tiering_policies_by_name[name.unwrap()];
    if (!policy) {
        throw new RpcError(
            'NO_SUCH_TIERING_POLICY',
            'No such tiering policy: ' + name,
        );
    }
    return policy;
}
...
function get_tier_extra_info(
    ...
    tier.mirrors.forEach(mirror_object => {
        _.compact(
            (mirror_object.spread_pools || []).map(pool => {
                info.has_any_pool_configured = true;
                const spread_pool = system_store.data.pools.find(
                    pool_rec => String(pool_rec._id) === String(pool._id),
                );
                tiering_pools_used_agg.push(
                    _.get(spread_pool, 'storage_stats.blocks_size') || 0,
                );
                return _.extend(
                    { pool_id: pool._id },
                    tiering_pools_status[tier._id].pools[pool._id],
                );
            }),
        ).forEach(pool_status => {
            info.any_rebuilds =
                ...
                _.get(
                    nodes_aggregate_pool,
                    `groups.${pool_status.pool_id}.data_activities.length`,
                );
            const pool_num_nodes =
                _.get(
                    nodes_aggregate_pool,
                    `groups.${pool_status.pool_id}.storage_nodes.count`,
                ) || 0;
            const pool_num_hosts =
                _.get(
                    hosts_aggregate_pool,
                    `groups.${pool_status.pool_id}.nodes.online`,
                ) || 0;
            ...
            info.has_internal_or_cloud_pool =
                info.has_internal_or_cloud_pool ||
                pool_status.resource_type !== 'HOSTS';
            info.use_internal =
                info.use_internal || pool_status.resource_type === 'INTERNAL';
        });
        if (
            info.has_valid_pool ||
            (info.num_valid_nodes || 0) >= required_valid_nodes
        ) {
            info.mirrors_with_valid_pool += 1;
        }
        if (
            info.num_nodes_in_mirror_group >= required_valid_nodes ||
            info.has_internal_or_cloud_pool
        ) {
            info.mirrors_with_enough_nodes += 1;
        }

        const existing_minimum =
            _.isUndefined(info.num_of_nodes) ?
                Number.MAX_SAFE_INTEGER
            :   info.num_of_nodes;
        const existing_hosts_minimum =
            _.isUndefined(info.num_of_hosts) ?
                Number.MAX_SAFE_INTEGER
            :   info.num_of_hosts;
        const num_valid_nodes_for_minimum =
            _.isUndefined(info.num_valid_nodes) ?
                Number.MAX_SAFE_INTEGER
            :   info.num_valid_nodes;
        const potential_new_minimum =
            info.has_internal_or_cloud_pool ?
                Number.MAX_SAFE_INTEGER
            :   num_valid_nodes_for_minimum;
        info.num_of_nodes =
            info.use_internal ? 0 : (
                Math.min(existing_minimum, potential_new_minimum)
            );
        const potential_new_hosts_minimum =
            info.has_internal_or_cloud_pool ?
                Number.MAX_SAFE_INTEGER
            :   info.num_valid_hosts;
        info.num_of_hosts =
            info.use_internal ? 0 : (
                Math.min(existing_hosts_minimum, potential_new_hosts_minimum)
            );
    });
    ...
    info.used_of_pools_in_tier = size_utils.json_to_bigint(
...
function get_tier_info(tier, nodes_aggregate_pool, tiering_tier_status) {
    ...
    _.forEach(tier.mirrors, mirror => {
        ...
        const spread_storage = size_utils.reduce_storage(
            size_utils.reduce_sum,
            pools_storage,
        );
    ...
    const data = _.pick(
        _.defaults(
            size_utils.reduce_storage(
                size_utils.reduce_minimum,
                tiering_tier_status && tiering_tier_status.mirrors_storage,
            ),
            {
                free: 0,
            },
...
function get_tiering_policy_info(
    tiering_policy,
    tiering_pools_status,
    nodes_aggregate_pool,
    hosts_aggregate_pool,
    ...
    info.tiers = _.map(tiering_policy.tiers, t => {
        let mode;
        if (tiers_storage) {
            const extra_info = get_tier_extra_info(
                t.tier,
                tiering_pools_status,
                nodes_aggregate_pool,
                hosts_aggregate_pool,
            );
            const tier_info = get_tier_info(
                t.tier,
                nodes_aggregate_pool,
    ...
    if (tiers_storage) {
        info.storage = size_utils.reduce_storage(
            size_utils.reduce_sum,
            tiers_storage,
        );
    }
    if (tiers_data) {
        info.data = size_utils.reduce_storage(
            size_utils.reduce_sum,
            tiers_data,
        );
    }
...
function calc_tier_policy_status(tier, tier_info, extra_info) {
    ...
    const tier_free = size_utils.json_to_bigint(
        _.get(tier_info, 'storage.free') || 0,
    );
    const tier_used = size_utils.json_to_bigint(
        _.get(tier_info, 'storage.used') || 0,
    );
    const tier_used_other = size_utils.json_to_bigint(
        _.get(tier_info, 'storage.used_other') || 0,
    );
    ...
    if (tier.mirrors.length > 0) {
        if (extra_info.mirrors_with_valid_pool === tier.mirrors.length) {
            has_enough_healthy_nodes_for_tier = true;
        }
        if (extra_info.mirrors_with_enough_nodes === tier.mirrors.length) {
            has_enough_total_nodes_for_tier = true;
        }
    }
...
function get_associated_tiering_policies(tier) {
    const associated_tiering_policies = _.filter(
        tier.system.tiering_policies_by_name,
        policy =>
            _.find(policy.tiers, t => String(t.tier._id) === String(tier._id)),
    );

    return _.map(
        associated_tiering_policies,
        tiering_policies => tiering_policies._id,
    );
}
...
function get_associated_buckets(policy) {
    ...
    const associated_buckets = _.filter(
        policy.system.buckets_by_name,
        bucket =>
            bucket.tiering && String(bucket.tiering._id) === String(policy._id),
    );
...
function throw_on_invalid_pools_for_tier(pools) {
    const deleting_pools = pools.filter(
        pool => pool.hosts_pool_info && pool.hosts_pool_info.host_count === 0,
    );
    if (deleting_pools.length > 0) {
        throw new Error(
            `Invalid attached pools, ${deleting_pools.join(', ')} are in deleting state`,
        );
    }
}
...
function check_tiering_policy_exists(req, policy_name) {
    const name =
        policy_name instanceof SensitiveString ?
            policy_name.unwrap()
        :   policy_name;
    if (
        req.system.tiering_policies_by_name &&
        req.system.tiering_policies_by_name[name]
    ) {
        throw new RpcError(`Policy ${policy_name} already exists`);
    }
}

function check_tier_exists(req, tier_name) {
    const name =
        tier_name instanceof SensitiveString ? tier_name.unwrap() : tier_name;
    if (req.system.tiers_by_name && req.system.tiers_by_name[name]) {
        throw new RpcError(`Tier ${tier_name} already exists`);
    }
}
