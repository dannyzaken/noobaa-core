...
function _check_validity_of_configuration({ req, bucket_class_buckets }) {
  ...
  try {
    const pools = [];
    tiers.forEach(tier => {
      if (tier.data_placement === 'MIRROR' && tier.attached_pools.length < 2) {
        throw new RpcError('BAD_REQUEST', 'cannot mirror on less than 2 pools');
      }
      throw_on_invalid_pools_for_tier(
...
async function update_bucket_class(req) {
  ...
  try {
    ...
    for (const bucket of bucket_class_buckets) {
      ...
      new_policy._id =
        req.system.tiering_policies_by_name[old_policy.name.unwrap()]._id;
      if (added_tier) {
...
function get_tier_extra_info(
  ...
  tier.mirrors.forEach(mirror_object => {
    ...
    info.num_of_nodes =
      info.use_internal ? 0 : Math.min(existing_minimum, potential_new_minimum);
...
function get_tier_info(tier, nodes_aggregate_pool, tiering_tier_status) {
  ...
  _.forEach(tier.mirrors, mirror => {
    const pools_storage = _.map(mirror.spread_pools, pool =>
      _.defaults(
        _.get(nodes_aggregate_pool, ['groups', String(pool._id), 'storage']),
        {
          used: 0,
          total: 0,
          free: 0,
...
function check_tiering_policy_exists(req, policy_name) {
  const name =
    policy_name instanceof SensitiveString ? policy_name.unwrap() : policy_name;
  if (
    req.system.tiering_policies_by_name &&
