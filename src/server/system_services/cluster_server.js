        ...
        Dispatcher.instance().activity({
          ...
          server: {
            hostname:
              ...
              _.get(
                system_store.data.cluster_by_server[req.rpc_params.secret],
    ...
    P.resolve(
      ...
      () =>
        MongoCtrl.add_new_mongos(
          cutil.extract_servers_ip(cutil.get_topology().config_servers),
        ),
          ...
          if (hosts_excluding_current.length > 0) {
            res_host =
              hosts_excluding_current[
                Math.floor(Math.random() * hosts_excluding_current.length)
              ];
          }
        }
      });
    ...
    .then(() =>
      P.map(target_servers, function (server) {
        return server_rpc.client.cluster_internal
          ...
          .then(res_data => {
            const data =
              (res_data[RPC_BUFFERS] && res_data[RPC_BUFFERS].data) || '';
            if (!data) {
              dbg.warn(
            ...
            const server_hostname =
              (server.heartbeat && server.heartbeat.health.os_info.hostname) ||
              'unknown';
  ...
  if (config_changes) {
    return P.resolve()
      .then(() => {
        ...
        return MongoCtrl.add_member_to_replica_set(name, IPs, is_config);
      })
      .then(() => {
