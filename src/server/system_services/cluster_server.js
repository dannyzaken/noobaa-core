...
async function new_cluster_info(params) {
    ...
    const address =
        (params && params.address) || os_utils.get_local_ipv4_ips()[0];
    const cluster = {
...
async function add_member_to_cluster(req) {
    ...
    dbg.log0(
        'Recieved add member to cluster req',
        req.rpc_params,
        'current topology',
    ...
    try {
        ...
        add_member_to_cluster_invoke(req, my_address).finally(() => {
    ...
    return _validate_member_request(req)
        ...
        .then(version_check_res => {
            if (version_check_res.result !== 'OKAY') {
                throw new Error(
                    `Verify join version check returned ${version_check_res}`,
                );
            }
        })
        .then(() =>
            server_rpc.client.cluster_internal.verify_join_conditions(
                {
                    secret: req.rpc_params.secret,
                },
                {
                    address: server_rpc.get_base_address(
                        req.rpc_params.address,
                    ),
                    timeout: 60000, //60s
                },
            ),
        ...
        .then(response => {
            ...
            if (response.result !== 'OKAY') {
                throw new Error(
                    `Verify join conditions check returned ${response.result}`,
                );
            }
            dbg.log0(
                'validating succeeded, got this caller_address',
                response.caller_address,
            );
            my_address =
                response.caller_address || os_utils.get_local_ipv4_ips()[0];
            if (!is_clusterized && response.caller_address) {
                ...
                const server_idx = _.findIndex(
                    topology.shards[shard_idx].servers,
                    server => net_utils.is_localhost(server.address),
                );
                if (server_idx === -1) {
                    dbg.warn(
                        'db does not contain internal ip of master server',
                    );
                } else {
                    ...
                    return system_store
                        .make_changes({
                            update: {
                                clusters: [
                                    {
                                        _id: system_store.get_local_cluster_info()
                                            ._id,
                        ...
                        .catch(err =>
                            dbg.warn(
                                "Failed to update adding-server's ip",
                                err,
                            ),
                        );
                }
    ...
    return (
        Promise.resolve()
            .then(() => {
                ...
                if (!is_clusterized) {
                    dbg.log0(
                        'Current server is first on cluster and has single mongo running, updating',
                    );
            ...
            .then(() => {
                ...
                return Promise.all([
                    fs.promises.readFile(
                        config.MONGO_DEFAULTS.ROOT_CA_PATH,
                        'utf8',
                    ),
                    fs.promises.readFile(
                        config.MONGO_DEFAULTS.SERVER_CERT_PATH,
                        'utf8',
                    ),
                    fs.promises.readFile(
                        config.MONGO_DEFAULTS.CLIENT_CERT_PATH,
                        'utf8',
                    ),
                ]);
            })
            ...
            .then(res => {
                Dispatcher.instance().activity({
                    ...
                    server: {
                        hostname:
                            req.rpc_params.new_hostname ||
                            _.get(
                                system_store.data.cluster_by_server[
                                    req.rpc_params.secret
                                ],
            ...
            .catch(function (err) {
                console.error(
                    'Failed adding members to cluster',
                    req.rpc_params,
                    'with',
                    err,
                );
            ...
            .catch(err => {
                console.error(
                    `Caught err in add_member_to_cluster_invoke ${err}`,
                );
            })
            .then(() => {
                // do nothing.
    ...
    return P.resolve()
        ...
        .then(os_info => {
            ...
            if (req.connection && req.connection.url) {
                caller_address =
                    req.connection.url.hostname.includes('ffff') ?
                        req.connection.url.hostname.replace(/^.*:/, '')
                    :   req.connection.url.hostname;
            } else {
                dbg.error(
                    'No connection on request for verify_join_conditions. Got:',
                    req,
                );
                throw new Error(
                    'No connection on request for verify_join_conditions',
                );
            }
            return _verify_join_preconditons(req)
                .catch(err => {
                    dbg.error(
                        'verify_join_conditions: HAD ERROR',
                        err,
                        err.message,
                    );
                    if (_.includes(VERIFY_RESPONSE, err.message)) {
                        return err.message;
                    }
                    if (err.message === 'CONNECTION_TIMEOUT') {
                        return 'CONNECTION_TIMEOUT_NEW';
                    }
                    throw err;
                })
                .then(result => ({
    ...
    return P.resolve(
        ...
        .catch(err => {
            if (err.rpc_code === 'NO_SUCH_RPC_SERVICE') {
                dbg.warn(
                    '_check_candidate_version got NO_SUCH_RPC_SERVICE from a server with an old version',
                );
...
async function verify_candidate_join_conditions(req) {
    try {
        dbg.log0(
            'Got verify_candidate_join_conditions for server secret:',
            req.rpc_params.secret,
            'address:',
            req.rpc_params.address,
        );
        ...
        if (req.rpc_params.secret === system_store.get_server_secret()) {
            dbg.error(
                'lol trying to add self to cluster - self secret received:',
                req.rpc_params.secret,
            );
            return {
                result: 'ADDING_SELF',
        ...
        const nc_res = await os_utils.exec(
            `echo -n | nc -w5 ${req.rpc_params.address} ${config.MONGO_DEFAULTS.SHARD_SRV_PORT} 2>&1`,
            {
                ignore_rc: true,
                return_stdout: true,
            },
        ...
        if (nc_res.includes('Connection timed out')) {
            dbg.warn(
                `Could not reach ${req.rpc_params.address}:${config.MONGO_DEFAULTS.SHARD_SRV_PORT}, might be due to a FW blocking`,
            );
            return {
        ...
        const verify_res =
            await server_rpc.client.cluster_internal.verify_join_conditions(
                {
                    secret: req.rpc_params.secret,
                },
                {
        ...
        if (
            err.rpc_code === 'RPC_CONNECT_TIMEOUT' ||
...
function join_to_cluster(req) {
    dbg.log0(
        'Got join_to_cluster request with topology',
    ...
    return (
        P.resolve()
            .then(() =>
                _verify_join_preconditons(req).catch(err => {
            ...
            .then(verify_res => {
                if (verify_res !== 'OKAY') {
                    throw new Error(
                        `Verify join preconditions failed with result ${verify_res}`,
                    );
                }
            ...
            .then(() => {
                ...
                return Promise.all([
                    fs.promises.writeFile(
                        config.MONGO_DEFAULTS.ROOT_CA_PATH,
                        req.rpc_params.ssl_certs.root_ca,
                    ),
                    fs.promises.writeFile(
                        config.MONGO_DEFAULTS.SERVER_CERT_PATH,
                        req.rpc_params.ssl_certs.server_cert,
                    ),
                    fs.promises.writeFile(
                        config.MONGO_DEFAULTS.CLIENT_CERT_PATH,
                        req.rpc_params.ssl_certs.client_cert,
                    ),
                ]);
            })
            ...
            .then(() => {
                ...
                if (req.rpc_params.role === 'SHARD') {
                    ...
                    if (req.rpc_params.location) {
                        dbg.log0(
                            'setting location tag to ',
                            req.rpc_params.new_hostname,
                        );
                        cluster_info.location = req.rpc_params.location;
                    }
                    return P.resolve(_update_cluster_info(cluster_info)).then(
                        () =>
                            _add_new_shard_on_server(
                                req.rpc_params.shard,
                                req.rpc_params.ip,
                                {
                                    first_shard: false,
                                    remote_server: true,
            ...
            .then(function () {
                const topology_to_send = _.omit(
                    cutil.get_topology(),
                    'dns_servers',
                    'timezone',
                );
                dbg.log0(
                    'Added member, publishing updated topology',
                    cutil.pretty_topology(topology_to_send),
                );
            ...
            .then(() =>
                cutil.send_master_update(false, req.rpc_params.master_ip),
            )
...
function verify_new_ip(req) {
    ...
    return server_rpc.client.cluster_internal
        .get_secret(
            {},
            {
                address,
    ...
    return (
        _validate_member_request(
            ...
            .then(version_check_res => {
                if (version_check_res.result !== 'OKAY') {
                    throw new Error(
                        `Verify member version check returned ${version_check_res}`,
                    );
                }
            })
            .then(() => {
                ...
                for (let i = 0; i < info.shards.length; ++i) {
                    server_idx = _.findIndex(
                        info.shards[i].servers,
                        server =>
                            server.secret === req.rpc_params.target_secret,
                    );
                    if (server_idx !== -1) {
                        shard_index = i;
                        break;
                ...
                if (shard_index === -1 || server_idx === -1) {
                    throw new Error(
                        `could not find address:${req.rpc_params.secret} in any shard`,
                    );
                }
                ...
                const new_rs_params = {
                    ...
                    IPs: cutil.extract_servers_ip(new_shard.servers),
            ...
            .then(function () {
                dbg.log0(
                    'Edited member',
                    req.rpc_params.old_address,
                    'of cluster. New topology',
                    cutil.pretty_topology(cutil.get_topology()),
                );
            ...
            .catch(function (err) {
                console.error(
                    'Failed edit of member to cluster',
                    req.rpc_params,
                    'with',
                    err,
                );
...
function news_config_servers(req) {
    dbg.log0(
        'Recieved news: news_config_servers',
        cutil.pretty_topology(req.rpc_params),
    );
    ...
    return P.resolve(
        ...
        P.resolve(
            ...
            () =>
                MongoCtrl.add_new_mongos(
                    cutil.extract_servers_ip(
...
function news_replicaset_servers(req) {
    dbg.log0(
        'Recieved news: news_replicaset_servers',
        cutil.pretty_topology(req.rpc_params),
    );
    //Verify we recieved news on the cluster we are joined to
    cutil.verify_cluster_id(req.rpc_params.cluster_id);
    dbg.log0(
        'replica set params - IPs:',
        req.rpc_params.IPs,
        'name:',
        req.rpc_params.name,
    );
    return P.resolve(
        _update_rs_if_needed(req.rpc_params.IPs, req.rpc_params.name, false),
    ).then(() => {
        // do nothing.
    });
}
...
function news_updated_topology(req) {
    dbg.log0(
        'Recieved news: news_updated_topology',
        cutil.pretty_topology(req.rpc_params),
    );
    const params = {
        is_clusterized: true,
    ...
    if (req.rpc_params.cluster_member_edit) {
        ...
        if (
            String(current_clustering.owner_address) ===
            String(req.rpc_params.cluster_member_edit.old_address)
        ) {
            params.owner_address =
                req.rpc_params.cluster_member_edit.new_address;
        }
    }

    dbg.log0(
        'updating topology to the new published topology:',
        req.rpc_params.new_topology,
    );
...
function redirect_to_cluster_master(req) {
    ...
    if (!current_clustering) {
        const address =
            system_store.data.systems[0].base_address ||
            os_utils.get_local_ipv4_ips()[0];
        return address;
    }
    if (!current_clustering.is_clusterized) {
        const address =
            system_store.data.systems[0].base_address ||
            current_clustering.owner_address;
        return address;
    }
    return P.fcall(function () {
        ...
        if (topology && topology.shards) {
            _.forEach(topology.shards, shard => {
                if (
                    String(shard.shardname) === String(topology.owner_shardname)
                ) {
                    const hosts_excluding_current = _.difference(
                        shard.servers,
                        [
                            {
                    ...
                    if (hosts_excluding_current.length > 0) {
                        res_host =
                            hosts_excluding_current[
                                Math.floor(
                                    Math.random() *
                                        hosts_excluding_current.length,
                                )
                            ];
                    }
                }
    ...
    return P.fcall(function () {
        if (debug_params.target_secret) {
            const cluster_server =
                system_store.data.cluster_by_server[debug_params.target_secret];
            if (!cluster_server) {
                throw new RpcError(
                    'CLUSTER_SERVER_NOT_FOUND',
        ...
        return P.map_one_by_one(target_servers, function (server) {
            return server_rpc.client.cluster_internal.apply_set_debug_level(
                debug_params,
                {
        ...
        .then(() => {
            if (
                !debug_params.target_secret &&
                req.system.debug_level !== debug_params.level
            ) {
                Dispatcher.instance().activity(
                    _.defaults(audit_activity, {
...
function apply_set_debug_level(req) {
    ...
    if (req.rpc_params.target_secret) {
        const cluster_server =
            system_store.data.cluster_by_server[req.rpc_params.target_secret];
        if (!cluster_server) {
            throw new RpcError(
                'CLUSTER_SERVER_NOT_FOUND',
        ...
        if (cluster_server.debug_level === req.rpc_params.level) {
            dbg.log0(
                'requested to set debug level to the same as current level. skipping..',
                req.rpc_params,
            );
            return;
        }
    ...
    return _set_debug_level_internal(req, req.rpc_params.level)
        .then(() => {
            if (req.rpc_params.level > 0) {
                //If level was set, remove it after 10m
...
function _set_debug_level_internal(req, level) {
    dbg.log0(
        'Recieved _set_debug_level_internal req',
        req.rpc_params,
        'With Level',
        level,
    );
...
function diagnose_system(req) {
    ...
    return fs_utils
        .create_fresh_path(`${TMP_WORK_DIR}`)
        .then(() =>
            P.map(target_servers, function (server) {
                return server_rpc.client.cluster_internal
                    .collect_server_diagnostics(
                        {},
                        {
                            address: server_rpc.get_base_address(
                                server.owner_address,
                            ),
                            auth_token: req.auth_token,
                        },
                    )
                    .then(res_data => {
                        const data =
                            (res_data[RPC_BUFFERS] &&
                                res_data[RPC_BUFFERS].data) ||
                            '';
                        if (!data) {
                            dbg.warn(
                                'diagnose_system: no diagnostics data from ',
                                server.owner_address,
                            );
                        }
                        const server_hostname =
                            (server.heartbeat &&
                                server.heartbeat.health.os_info.hostname) ||
                            'unknown';
                        // Should never exist since above we delete the root folder
                        return fs_utils
                            .create_fresh_path(
                                `${TMP_WORK_DIR}/${server_hostname}_${server.owner_secret}`,
                            )
                            .then(() =>
                                fs.promises.writeFile(
        ...
        .then(() =>
            os_utils.exec(`find ${TMP_WORK_DIR} -maxdepth 1 -type f -delete`),
        )
...
function collect_server_diagnostics(req) {
    ...
    return P.resolve()
        ...
        .then(os_info => {
            ...
            const out_path =
                '/public/' + os_info.hostname + '_srv_diagnostics.tgz';
        ...
        .then(out_path => {
            ...
            return fs.promises
                ...
                .catch(err => {
                    ...
                    throw new Error(
                        'Server Collect Diag Error on reading packges diag file',
                    );
                });
        })
        .then(res => {
...
function read_server_time(req) {
    ...
    return server_rpc.client.cluster_internal.apply_read_server_time(
        req.rpc_params,
        {
...
function update_server_conf(req) {
    ...
    return P.resolve()
        .then(() => {
            audit_server.hostname = _.get(
                cluster_server,
                'heartbeat.health.os_info.hostname',
            );
            audit_server.secret = cluster_server.owner_secret;
            if (
                req.rpc_params.hostname &&
                req.rpc_params.hostname !== audit_server.hostname
            ) {
                //hostname supplied and actually changed
                ...
                if (!os_utils.is_valid_hostname(req.rpc_params.hostname)) {
                    throw new Error(
                        `Invalid hostname: ${req.rpc_params.hostname}. See RFC 1123`,
                    );
                }
                return server_rpc.client.cluster_internal
                    .set_hostname_internal(
                        ...
                        {
                            address: server_rpc.get_base_address(
                                cluster_server.owner_address,
                            ),
                            timeout: 60000, //60s
                        },
                    )
                    .then(() =>
                        cluster_hb.do_heartbeat({ skip_server_monitor: true }),
                    ) //We call for HB since the hostname changed
                    .then(() => cluster_server);
            }
            return cluster_server;
        })
        .then(() => {
            if (req.rpc_params.location !== cluster_server.location) {
                //location supplied and actually changed
                const new_name =
                    req.rpc_params.location ? req.rpc_params.location : "''";
...
function _validate_member_request(req) {
    if (!os_utils.is_supervised_env()) {
        console.warn(
            'Environment is not a supervised one, currently not allowing clustering operations',
        );
        throw new Error(
            'Environment is not a supervised one, currently not allowing clustering operations',
        );
    ...
    if (req.rpc_params.address && !net.isIPv4(req.rpc_params.address)) {
        throw new Error(
            'Adding new members to cluster is allowed by using IP only',
        );
    }

    if (
        req.rpc_params.new_hostname &&
        !os_utils.is_valid_hostname(req.rpc_params.new_hostname)
    ) {
        throw new Error(
            `Invalid hostname: ${req.rpc_params.new_hostname}. See RFC 1123`,
        );
    }
    ...
    return os_utils
        .exec(
            `echo -n | nc -w5 ${req.rpc_params.address} ${config.MONGO_DEFAULTS.SHARD_SRV_PORT} 2>&1`,
            { ignore_rc: true, return_stdout: true },
        )
        .then(response => {
...
function _verify_join_preconditons(req) {
    ...
    return P.resolve().then(() => {
        ...
        return os_utils
            .exec(
                `echo -n | nc -w5 ${caller_address} ${config.MONGO_DEFAULTS.SHARD_SRV_PORT} 2>&1`,
                { ignore_rc: true, return_stdout: true },
            )
            .then(response => {
                if (response.includes('Connection timed out')) {
            ...
            .then(() => {
                ...
                if (system) {
                    ...
                    if (
                        cutil.get_topology().shards.length !== 1 ||
                    ...
                    return MDStore.instance()
                        .had_any_objects_in_system(system._id)
                        .then(has_objects => {
                            if (has_objects) {
            ...
            .then(() => {
                ...
                dbg.log0(
                    '_verify_join_preconditons okay. server has no system',
                );
                return 'OKAY';
            })
            .catch(err => {
...
function _add_new_shard_on_server(shardname, ip, params) {
    ...
    dbg.log0(
        'Adding shard, new topology',
        cutil.pretty_topology(current_topology),
    );

    //Actually add a new mongo shard instance
    return P.resolve(
        MongoCtrl.add_new_shard_server(shardname, params.first_shard),
    )
        .then(function () {
            dbg.log0(
                'Checking current config servers set, currently contains',
                current_topology.config_servers.length,
                'servers',
            );
            if (current_topology.config_servers.length === 3) {
                //Currently stay with a repset of 3 for config
                dbg.log0(
                    'Adding a new mongos without chanding the config array',
                );
                //We already have a config replica set of 3, simply set up a mongos instance
                return MongoCtrl.add_new_mongos(
                    cutil.extract_servers_ip(current_topology.config_servers),
                );
            } else {
                // < 3 since we don't add once we reach 3, add this server as config as well
                dbg.log0(
                    'Adding a new config server',
                    ip,
                    'to the config array',
                ...
                return _add_new_config_on_server(
                    cutil.extract_servers_ip(config_updates.config_servers),
                    params,
                );
            }
        })
        ...
        .then(function () {
            if (!params.first_shard) {
                ...
                dbg.log0(
                    'Writing back old topology, for owner_secret reference',
                    cutil.pretty_topology(current_topology),
                );
...
function _initiate_replica_set(shardname) {
    ...
    return P.resolve(() => _update_cluster_info(new_topology))
        .then(() =>
            MongoCtrl.add_replica_set_member(
                shardname,
                /*first_server=*/ true,
                new_topology.shards[shard_idx].servers,
            ),
        )
        .then(() => {
    ...
    return P.resolve()
        ...
        .then(new_clustering => {
            ...
            const update = _.defaults(
                _.pick(params, _.keys(current_clustering)),
                current_clustering,
            );
            update.owner_secret = system_store.get_server_secret(); //Keep original owner_secret
            update.owner_address =
                params.owner_address || current_clustering.owner_address;
            update._id = current_clustering._id;
            dbg.log0(
                'Updating local cluster info for owner',
                update.owner_secret,
                'previous cluster info',
                cutil.pretty_topology(current_clustering),
                'new cluster info',
                cutil.pretty_topology(update),
            );
            ...
            return system_store
                ...
                .catch(err => {
                    console.error(
                        'failed on local cluster info update with',
                        err.message,
                    );
                    throw err;
                });
    ...
    return P.resolve(
        ...
        .then(dns_config => {
            ...
            if (dns_config.dns_servers.length) {
                dbg.log0(
                    `_add_new_server_to_replica_set: using existing DNS servers configuration: `,
            ...
            dbg.log0(
                'inserting topology for new server to clusters collection:',
                new_topology,
            );
            return system_store.make_changes({
                insert: {
...
function _add_new_config_on_server(cfg_array, params) {
    ...
    return P.resolve(MongoCtrl.add_new_config())
        ...
        .then(function () {
            dbg.log0(
                'Updating config replica set, initiate_replica_set=',
                params.first_shard ? 'true' : 'false',
            );
            if (params.first_shard) {
...
function _update_rs_if_needed(IPs, name, is_config) {
    ...
    if (config_changes) {
        return P.resolve()
            .then(() => {
                dbg.log0(
                    'Current server is master for config RS, Updating to',
                    IPs,
                );
                return MongoCtrl.add_member_to_replica_set(
                    name,
...
function check_cluster_status() {
    ...
    return P.map(other_servers, server =>
        P.timeout(
            30000,
            server_rpc.client.cluster_server.ping(
                {},
                {
                    address: server_rpc.get_base_address(server.owner_address),
                },
            ...
            .catch(err => {
                dbg.warn(
                    `error while pinging server ${server.owner_secret}: `,
                    err.stack || err,
                );
                return {
                    secret: server.owner_secret,
