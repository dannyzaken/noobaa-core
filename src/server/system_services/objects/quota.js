...
class Quota {
    ...
    constructor(quota_config) {
        this.size =
            quota_config && quota_config.size ?
                quota_config.size
            :   { value: 0 };
        this.size[RAW_VALUE_FIELD_NAME] = size_utils
            .size_unit_to_bigint(this.size.value, this.size.unit)
            .toString();
        this.quantity =
            quota_config && quota_config.quantity ?
                quota_config.quantity
            :   { value: 0 };
        this.quantity[RAW_VALUE_FIELD_NAME] = BigInt(
            this.quantity.value,
        ).toString();
    }
    ...
    is_empty_quota() {
        return (
            this.get_quota_by_size() === '0' &&
            this.get_quota_by_quantity() === '0'
        );
    }
    ...
    is_valid_quota() {
        return !(
            this.get_quota_by_size() < 0 || this.get_quota_by_quantity() < 0
        );
    }
    ...
    get_bucket_quota_usages_percent(bucket) {
        return {
            size_used_percent:
                this.get_bucket_size_quota_usages_percent(bucket),
            quantity_used_percent:
                this.get_bucket_quantity_quota_usages_percent(bucket),
        };
    }
    ...
    get_bucket_size_quota_usages_percent(bucket) {
        ...
        if (quota_size > 0) {
            size_used_percent = objects_size
                .multiply(100)
                .divide(quota_size)
                .valueOf();
        }
    ...
    get_bucket_quantity_quota_usages_percent(bucket) {
        ...
        const objects_count =
            bucket.storage_stats &&
            size_utils.json_to_bigint(bucket.storage_stats.objects_count);
        const quantity_quota = size_utils.json_to_bigint(
            this.get_quota_by_quantity(),
        );
        if (quantity_quota > 0) {
            quantity_used_percent = objects_count
                .multiply(100)
                .divide(quantity_quota)
                .valueOf();
        }
    ...
    is_quota_exceeded(bucket) {
        ...
        const { size_used_percent, quantity_used_percent } =
            this.get_bucket_quota_usages_percent(bucket);
        exceeded.is_quota_exceeded =
            size_used_percent >= 100 || quantity_used_percent >= 100;
        exceeded.is_quota_low =
            !exceeded.is_quota_exceeded &&
            (size_used_percent >= config.QUOTA_LOW_THRESHOLD ||
    ...
    get_available_size_for_upload(total_size, used) {
        ...
        if (this.get_quota_by_size() > 0) {
            const quota_size_free = size_utils
                .json_to_bigint(this.get_quota_by_size())
    ...
    get_available_quantity_for_upload(used) {
        ...
        if (this.get_quota_by_quantity() > 0) {
            const quota_quantity_free = size_utils
                .json_to_bigint(this.get_quota_by_quantity())
                .minus(size_utils.json_to_bigint(used));
            available_quantity_for_upload =
                size_utils.bigint_to_json(quota_quantity_free);
        }
    ...
    add_quota_alerts(sysid, bucket, collection) {
        const { size_used_percent, quantity_used_percent } =
            this.get_bucket_quota_usages_percent(bucket);
        const size_alert = Quota.get_quota_alert(
            sysid,
            bucket,
            size_used_percent,
            this.get_quota_by_size(),
            'size',
        );
        if (size_alert !== null) {
            collection.push(size_alert);
        }
        const quantity_alert = Quota.get_quota_alert(
            sysid,
            bucket,
            quantity_used_percent,
            this.get_quota_by_quantity(),
