...
async function create_account(req) {
  ...
  if (req.rpc_params.s3_access) {
    if (req.rpc_params.new_system_parameters) {
      ...
      if (!resource) {
        throw new RpcError('BAD_REQUEST', "default resource doesn't exist");
      }
      if (
      ...
      if (bucket_claim_owner) {
        ...
        if (creator_roles.includes('operator')) {
          ...
          account.bucket_claim_owner =
            req.system.buckets_by_name[bucket_claim_owner.unwrap()]._id;
        } else {
          dbg.warn(
...
function update_account_s3_access(req) {
  ...
  if (req.rpc_params.s3_access) {
    ...
    if (req.rpc_params.nsfs_account_config) {
      if (
        _.isUndefined(req.rpc_params.nsfs_account_config.distinguished_name) &&
  ...
  return system_store
    ...
    .then(() => {
      ...
      if (req.rpc_params.s3_access) {
        ...
        if (
          account.allow_bucket_creation !== req.rpc_params.allow_bucket_creation
        ) {
      ...
      if (removed_buckets.length) {
        _.forEach(removed_buckets, bucket_name => {
          ...
          bucket_server.check_for_lambda_permission_issue(req, bucket, [
            account,
          ]);
        });
      }
...
function delete_account(req) {
  ...
  const roles_to_delete = system_store.data.roles
    .filter(role => String(role.account._id) === String(account_to_delete._id))
    .map(role => role._id);
      ...
      account =>
        ...
        (req.rpc_params.filter &&
          ...
          req.rpc_params.filter.fs_identity.uid ===
            (account.nsfs_account_config && account.nsfs_account_config.uid) &&
          req.rpc_params.filter.fs_identity.gid ===
            (account.nsfs_account_config && account.nsfs_account_config.gid)),
    )
  ...
  if (req.rpc_params.azure_log_access_keys) {
    info.azure_log_access_keys = {
      ...
      azure_logs_analytics_workspace_id:
        req.rpc_params.azure_log_access_keys.azure_logs_analytics_workspace_id,
    };
  }
...
async function _check_azure_connection_internal(params) {
  ...
  try {
    ...
    if (
      ...
      err.authenticationerrordetail.indexOf('Request date header too old') > -1
    ) {
  ...
  if (params.azure_log_access_keys) {
    try {
      ...
      dbg.warn(
        ...
        _.omit(params, ['secret', 'azure_log_access_keys.azure_client_secret']),
        ` error: ${err}`,
      );
