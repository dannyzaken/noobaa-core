...
async function create_account(req) {
    const account = {
        _id:
            req.rpc_params.new_system_parameters ?
                system_store.parse_system_store_id(
                    req.rpc_params.new_system_parameters.account_id,
                )
    ...
    if (
        account.name.unwrap() === 'demo' &&
        account.email.unwrap() === 'demo@noobaa.com'
    ) {
        ...
        if (!access_keys.length) {
            throw new RpcError(
                'FORBIDDEN',
                'cannot create account without access_keys',
            );
        }
    ...
    const sys_id =
        req.rpc_params.new_system_parameters ?
            system_store.parse_system_store_id(
                req.rpc_params.new_system_parameters.new_system_id,
            )
        :   req.system._id;
    ...
    if (req.rpc_params.s3_access) {
        if (req.rpc_params.new_system_parameters) {
            account.default_resource = system_store.parse_system_store_id(
                req.rpc_params.new_system_parameters.default_resource,
            );
            account.allow_bucket_creation = true;
        } else {
            const resource =
                req.rpc_params.default_resource ?
            ...
            if (!resource) {
                throw new RpcError(
                    'BAD_REQUEST',
                    "default resource doesn't exist",
                );
            }
            if (
                resource.nsfs_config &&
                resource.nsfs_config.fs_root_path &&
            ...
            account.allow_bucket_creation =
                _.isUndefined(req.rpc_params.allow_bucket_creation) ? true : (
            ...
            if (bucket_claim_owner) {
                ...
                if (creator_roles.includes('operator')) {
                    // Not allowed to create claim owner outside of the operator
                    account.bucket_claim_owner =
                        req.system.buckets_by_name[
                            bucket_claim_owner.unwrap()
                        ]._id;
                } else {
                    dbg.warn(
                        'None operator user was trying to set a bucket-claim-owner for account',
                        req.account,
                    );
                }
            }
    ...
    if (!account_roles.includes('operator')) {
        Dispatcher.instance().activity({
            ...
            account: account._id,
            desc:
                `${account.email.unwrap()} was created ` +
                (req.account ? `by ${req.account.email.unwrap()}` : ``),
        });
    }
    ...
    if (req.rpc_params.role_config) {
        validate_assume_role_policy(
            req.rpc_params.role_config.assume_role_policy,
        );
...
function create_external_user_account(req) {
    ...
    const default_resource =
        cloudResource[0] || hostPools[0] || internalStorage[0];
...
function read_account(req) {
    ...
    return get_account_info(account, is_self || is_operator);
}
...
function read_account_by_access_key(req) {
    ...
    const account = _.find(
        ...
        acc =>
            acc.access_keys &&
            acc.access_keys[0].access_key.unwrap() === access_key.unwrap(),
    );

    if (!account) {
        throw new RpcError(
            'NO_SUCH_ACCOUNT',
            'No such account with credentials: ' + access_key,
        );
    }
...
async function generate_account_keys(req) {
    ...
    if (!account) {
        throw new RpcError(
            'NO_SUCH_ACCOUNT',
            'No such account email: ' + req.rpc_params.email,
        );
    }
...
async function update_account_keys(req) {
    ...
    if (
        !_.isUndefined(
            system_store.get_account_by_access_key(access_keys.access_key),
        )
    ) {
        throw new RpcError(
            'ACCESS_KEY_DUPLICATION',
            'Duplicate access key is found, each access_key must be unique',
        );
    }
...
function update_account_s3_access(req) {
    const account = _.cloneDeep(
        system_store.get_account_by_email(req.rpc_params.email),
    );
    if (!account) {
        throw new RpcError(
            'NO_SUCH_ACCOUNT',
    ...
    if (req.rpc_params.s3_access) {
        if (!req.rpc_params.default_resource) {
            const pools = _.filter(
                req.system.pools_by_name,
                p => !_.get(p, 'mongo_pool_info'),
            ); // find none-internal pools
            if (pools.length) {
                // has resources which is not internal - must supply resource
                throw new RpcError(
                    'BAD_REQUEST',
                    'Enabling S3 requires providing default_resource',
                );
            }
        }
        ...
        if (req.rpc_params.default_resource) {
            ...
            update.default_resource = Object.values(
                req.system.pools_by_name,
            )[0]._id;
        }
        ...
        if (req.rpc_params.nsfs_account_config) {
            if (
                _.isUndefined(
                    req.rpc_params.nsfs_account_config.distinguished_name,
                ) &&
                _.isUndefined(req.rpc_params.nsfs_account_config.uid) &&
                _.isUndefined(req.rpc_params.nsfs_account_config.gid) &&
                !req.rpc_params.nsfs_account_config.new_buckets_path &&
    ...
    return system_store
        ...
        .then(() => {
            ...
            desc_string.push(
                `${account.email.unwrap()} S3 access was updated by ${req.account && req.account.email.unwrap()}`,
            );
            if (req.rpc_params.s3_access) {
                if (original_pool !== req.rpc_params.default_resource) {
                    desc_string.push(
                        `default pool changed to`,
                        req.rpc_params.default_resource ?
                            req.rpc_params.default_resource
                        :   `None`,
                    );
            ...
            if (removed_buckets.length) {
                _.forEach(removed_buckets, bucket_name => {
                    ...
                    bucket_server.check_for_lambda_permission_issue(
                        req,
                        bucket,
                        [account],
                    );
...
function update_account(req) {
    ...
    if (!account) {
        throw new RpcError(
            'NO_SUCH_ACCOUNT',
            'No such account email: ' + params.email,
        );
    }
    if (!is_support_or_admin_or_me(req.system, req.account, account)) {
        throw new RpcError('UNAUTHORIZED', 'Cannot update account');
    ...
    if (
        ...
        !_.every(
            params.ips,
            ip_range => net.isIP(ip_range.start) && net.isIP(ip_range.end),
        )
    ) {
    ...
    const updates = {
        ...
        next_password_change:
            params.must_change_password === true ? new Date() : undefined,
        allowed_ips:
            !_.isUndefined(params.ips) && params.ips !== null ?
                params.ips
            :   undefined,
        preferences:
            _.isUndefined(params.preferences) ? undefined : params.preferences,
    };
    ...
    const removals = {
        next_password_change:
            params.must_change_password === false ? true : undefined,
    ...
    if (!_.isUndefined(params.ips) && account.allowed_ips !== params.ips) {
        if (params.ips === null) {
            ...
            event_desc +=
                `Allowed IPs were changed to ` +
                params.ips
                    .map(ip_range => ip_range.start + ' - ' + ip_range.end)
                    .join(', ');
        }
    }
    ...
    return system_store
        ...
        .then(() =>
            Dispatcher.instance().activity({
                ...
                account: account._id,
                desc:
                    `${account.email.unwrap()} was updated by ${req.account && req.account.email.unwrap()}` +
...
async function reset_password(req) {
    ...
    const account =
        system_store.data.accounts_by_email[req.rpc_params.email.unwrap()];
    if (!account) {
        throw new RpcError(
            'NO_SUCH_ACCOUNT',
            'No such account email: ' + req.rpc_params.email,
    ...
    if (!account.has_login) {
        throw new RpcError(
            'FORBIDDEN',
            'Cannot change non management password',
        );
    }
...
function delete_account(req) {
    const account_to_delete = system_store.get_account_by_email(
        req.rpc_params.email,
    );
...
function delete_account_by_property(req) {
    let roles_to_delete = [];
    const accounts_to_delete = system_store
        .get_accounts_by_nsfs_account_config(req.rpc_params.nsfs_account_config)
        .map(account_to_delete => {
            _verify_can_delete_account(req, account_to_delete);
            roles_to_delete = roles_to_delete.concat(
                system_store.data.roles
                    .filter(
                        role =>
                            String(role.account._id) ===
                            String(account_to_delete._id),
                    )
                    .map(role => role._id),
            );
    ...
    if (!accounts_to_delete.length) {
        throw new RpcError(
            'NO_SUCH_ACCOUNT',
            'No account with provided uid/gid: ' +
                req.rpc_params.nsfs_account_config.uid +
                '/' +
                req.rpc_params.nsfs_account_config.gid,
        );
    }
        ...
        .filter(
            account =>
                ...
                (req.rpc_params.filter &&
                    ...
                    _.isEqual(
                        req.rpc_params.filter.fs_identity.distinguished_name,
                        account.nsfs_account_config &&
                            account.nsfs_account_config.distinguished_name,
                    ) &&
                    req.rpc_params.filter.fs_identity.uid ===
                        (account.nsfs_account_config &&
                            account.nsfs_account_config.uid) &&
                    req.rpc_params.filter.fs_identity.gid ===
                        (account.nsfs_account_config &&
                            account.nsfs_account_config.gid)),
        )
...
function accounts_status(req) {
    const any_non_support_account = _.find(
        system_store.data.accounts,
        function (account) {
...
async function add_external_connection(req) {
    ...
    const res = await server_rpc.client.account.check_external_connection(
        req.rpc_params,
        {
    ...
    if (req.rpc_params.azure_log_access_keys) {
        info.azure_log_access_keys = {
            azure_tenant_id:
                req.rpc_params.azure_log_access_keys.azure_tenant_id,
            azure_client_id:
                req.rpc_params.azure_log_access_keys.azure_client_id,
            azure_client_secret:
                system_store.master_key_manager.encrypt_sensitive_string_with_master_key_id(
                    req.rpc_params.azure_log_access_keys.azure_client_secret,
                    req.account.master_key_id._id,
                ),
            azure_logs_analytics_workspace_id:
                req.rpc_params.azure_log_access_keys
                    .azure_logs_analytics_workspace_id,
        };
    }
    ...
    info.auth_method =
        req.rpc_params.auth_method ||
        config.DEFAULT_S3_AUTH_METHOD[info.endpoint_type] ||
        undefined;
    info = _.omitBy(info, _.isUndefined);
    if (
        (info.endpoint_type === 'AWS' ||
            info.endpoint_type === 'AWSSTS' ||
            info.endpoint_type === 'S3_COMPATIBLE') &&
        !info.endpoint.startsWith('http://') &&
        !info.endpoint.startsWith('https://')
    ) {
...
async function update_external_connection(req) {
    ...
    try {
        ...
        dbg.error(
            'update_external_connection: _check_external_connection_internal had error',
            error,
        );
        check_failed = true;
    }
    ...
    if (check_failed) {
        throw new RpcError(
            'INVALID_CREDENTIALS',
            `Credentials are not valid ${name}`,
        );
    }
    ...
    if (azure_log_access_keys) {
        const encrypted_azure_client_secret =
            system_store.master_key_manager.encrypt_sensitive_string_with_master_key_id(
        ...
        const azure_creds_obj = {
            ...
            azure_logs_analytics_workspace_id:
                azure_log_access_keys.azure_logs_analytics_workspace_id,
        };
        acc_update_set_obj['sync_credentials_cache.$.azure_log_access_keys'] =
            azure_creds_obj;
        ns_resource_update_map_obj['connection.azure_log_access_keys'] =
            azure_creds_obj;
    }
    ...
    const pools_updates = system_store.data.pools
        .filter(
            pool =>
                ...
                pool.cloud_pool_info.access_keys.account_id._id.toString() ===
                    req.account._id.toString() &&
                pool.cloud_pool_info.access_keys.access_key.unwrap() ===
                    connection.access_key.unwrap(),
        )
        .map(pool => ({
            _id: pool._id,
    ...
    const ns_resources_updates = system_store.data.namespace_resources
        .filter(
            ns_resource =>
                ...
                ns_resource.connection.endpoint_type ===
                    connection.endpoint_type &&
                ns_resource.connection.endpoint === connection.endpoint &&
                ns_resource.account._id.toString() ===
                    req.account._id.toString() &&
                ns_resource.connection.access_key.unwrap() ===
                    connection.access_key.unwrap(),
        )
        .map(ns_resource => ({
            _id: ns_resource._id,
...
async function check_external_connection(req) {
    ...
    if (connection && !req.rpc_params.ignore_name_already_exist) {
        throw new RpcError(
            ...
            'Connection name already exists: ' + req.rpc_params.name,
        );
    }
...
async function check_azure_connection(params) {
    try {
        ...
        dbg.warn(
            `check_azure_connection: params`,
            _.omit(params, 'secret'),
            ` error: ${err}`,
        );
        return {
            status: err.message,
...
async function _check_azure_connection_internal(params) {
    ...
    try {
        ...
        dbg.warn(
            `got error on BlobServiceClient.fromConnectionString with params`,
            _.omit(params, 'secret'),
            ` error: ${err}`,
        );
    ...
    try {
        ...
        dbg.warn(
            `got error on listContainers with params`,
            _.omit(params, 'secret'),
            ` error: ${err}`,
        );
        if (
            err.code === 'AuthenticationFailed' &&
            err.authenticationerrordetail &&
            err.authenticationerrordetail.indexOf(
    ...
    try {
        ...
        dbg.warn(
            `got error on getServiceProperties with params`,
            _.omit(params, 'secret'),
            ` error: ${err}`,
        );
    ...
    if (!service_properties.blobAnalyticsLogging) {
        dbg.warn(
            `Error - connection for Premium account with params`,
            _.omit(params, 'secret'),
        );
    ...
    if (params.azure_log_access_keys) {
        try {
            ...
            const logs_query_client = new LogsQueryClient(
            ...
            dbg.warn(
                'got error on LogsQueryClient init with params',
                _.omit(params, [
                    'secret',
                    'azure_log_access_keys.azure_client_secret',
                ]),
...
async function check_aws_sts_connection(params) {
    const creds = await cloud_utils.generate_aws_sts_creds(
        params,
        'check_aws_sts_sessions',
    );
...
async function check_aws_connection(params) {
    ...
    const s3_params = {
        ...
        requestHandler: noobaa_s3_client.get_requestHandler_with_suitable_agent(
            params.endpoint,
        ),
        region: params.region || config.DEFAULT_REGION,
    };
    const s3 = noobaa_s3_client.get_s3_client_v3_params(s3_params);
    const timeoutError = Object.assign(new Error('Operation timeout'), {
    ...
    try {
        ...
        dbg.warn(
            `got error on listBuckets with params`,
            _.omit(params, 'secret'),
...
function check_net_storage_connection(params) {
    ...
    return P.timeout(
        ...
        err => {
            dbg.warn(
                `got error on dir with params`,
                _.omit(params, 'secret'),
                ` error: ${err}`,
                err.message,
            );
            const status =
                net_storage_error_mapping[err.code] || 'UNKNOWN_FAILURE';
            return {
                status,
                error: {
                    code: err.code || '',
...
function delete_external_connection(req) {
    ...
    const connection_to_delete = cloud_utils.find_cloud_connection(
        account,
    ...
    if (
        _.find(
            ...
            pool =>
                pool.cloud_pool_info &&
                pool.cloud_pool_info.endpoint_type ===
                    connection_to_delete.endpoint_type &&
                pool.cloud_pool_info.endpoint ===
                    connection_to_delete.endpoint &&
                pool.cloud_pool_info.access_keys.account_id._id ===
                    account._id &&
                pool.cloud_pool_info.access_keys.access_key.unwrap() ===
                    connection_to_delete.access_key.unwrap(),
        )
    ) {
        throw new RpcError(
            'IN_USE',
            'Cannot delete connection as it is being used for a cloud pool',
        );
    }
...
function get_account_info(account, include_connection_cache) {
    ...
    info.systems = _.compact(
        _.map(account.roles_by_system, function (roles, system_id) {
...
function ensure_support_account() {
    return system_store
        .refresh()
        .then(function () {
            const existing_support_account = _.find(
                system_store.data.accounts,
                function (account) {
...
function bcrypt_password(password) {
    return P.resolve().then(() => password && bcrypt.hash(password, 10));
}
...
function is_support_or_admin_or_me(system, account, target_account) {
    return (
        account.is_support ||
        (target_account &&
            String(target_account._id) === String(account._id)) ||
        (system &&
            account.roles_by_system[system._id].some(
...
function validate_create_account_params(req) {
    // find none-internal pools
    const has_non_internal_resources =
        req.system && req.system.pools_by_name ?
            Object.values(req.system.pools_by_name).some(
                p => !p.mongo_pool_info,
            )
        :   false;
    ...
    if (req.rpc_params.name.unwrap() !== req.rpc_params.name.unwrap().trim()) {
        throw new RpcError(
            'BAD_REQUEST',
            'system name must not contain leading or trailing spaces',
        );
    }
    ...
    if (req.rpc_params.s3_access) {
        if (!req.rpc_params.new_system_parameters) {
            if (req.system.pools_by_name === 0) {
                throw new RpcError(
                    "No resources in the system - Can't create accounts",
                );
            }
            ...
            if (
                ...
                //default resource needed only if new bucket can be created
                if (has_non_internal_resources) {
                    // has resources which is not internal - must supply resource
                    throw new RpcError(
                        'BAD_REQUEST',
                        'Enabling S3 requires providing default_resource',
    ...
    if (req.rpc_params.has_login) {
        ...
        const { default_resource } =
            req.rpc_params.new_system_parameters || req.rpc_params;
        const allow_bucket_creation =
            req.rpc_params.new_system_parameters ?
                true
            :   req.rpc_params.allow_bucket_creation;
        ...
        if (
            ...
            throw new RpcError(
                'BAD_REQUEST',
                'Accounts with login access must have full s3 access permissions',
            );
        }
...
async function verify_authorized_account(req) {
    ...
    return bcrypt.compare(
        req.rpc_params.verification_password.unwrap(),
        req.account.password.unwrap(),
    );
}
...
function _verify_can_delete_account(req, account_to_delete) {
    ...
    if (
        !is_support_or_admin_or_me(req.system, req.account, account_to_delete)
    ) {
