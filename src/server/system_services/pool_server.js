...
const HistoryDataStore =
    require('../analytic_services/history_data_store').HistoryDataStore;
const IoStatsStore =
...
function default_create_pool_controller(system, pool) {
    return pool.hosts_pool_info.is_managed ?
            new pool_ctrls.ManagedStatefulSetPoolController(
                system.name,
                pool.name,
            )
        :   new pool_ctrls.UnmanagedStatefulSetPoolController(
                system.name,
                pool.name,
            );
}
...
function set_pool_controller_factory(pool_controller_factory) {
    if (pool_controller_factory === null) {
        ...
        throw new TypeError(
            'Invalid pool_controller_factory, must be a function or null',
        );
    }
}
...
function check_deletion_ownership(req, resource_owner_id) {
    if (config.RESTRICT_RESOURCE_DELETION) {
        const requester_is_sys_owner =
            String(req.account._id) === String(req.system.owner._id);
        if (
            !requester_is_sys_owner &&
            String(resource_owner_id) !== String(req.account._id)
        ) {
            dbg.error(
                'check_deletion_ownership: requester (',
                req.account._id,
                ') is not the owner (',
                resource_owner_id,
                ') of the resource',
...
function new_namespace_resource_defaults(
    name,
    system_id,
    account_id,
    connection,
...
async function create_hosts_pool(req) {
    ...
    const PV_SIZE_GB = 20;
    const GB = 1024 ** 3;
    const conf_volume_size =
        rpc_params.host_config && rpc_params.host_config.volume_size;
    if (conf_volume_size && conf_volume_size < MIN_PV_SIZE_GB * GB) {
        dbg.error(
            `create_hosts_pool: insufficient PV size, minimal PV size is ${MIN_PV_SIZE_GB}GB`,
        );
        throw new RpcError(
            'BAD_REQUEST',
            `insufficient PV size, minimal PV size is ${MIN_PV_SIZE_GB}GB`,
        );
    }
    ...
    Dispatcher.instance().activity({
        ...
        actor: account._id,
        pool: pool._id,
        desc: `A ${hosts_pool_info.is_managed ? 'managed' : 'unmanaged'} pool ${
            rpc_params.name
        } with ${
            hosts_pool_info.host_count
        } nodes was created by ${account.email.unwrap()}`,
    });

    try {
        const routing_hint =
            hosts_pool_info.is_managed ? 'INTERNAL' : 'EXTERNAL';
        const agent_install_string = await get_agent_install_conf(
            system,
            pool,
            account,
            routing_hint,
        ...
        console.error(
            `create_hosts_pool: Could not deploy underlaying agents, got: ${err.message}`,
        );
        throw err;
    }
}
...
async function get_hosts_pool_agent_config(req) {
    ...
    if (!pool.hosts_pool_info) {
        throw new RpcError(
            'INVALID_POOL_TYPE',
            `pool ${pool.name} not a hosts pool`,
        );
    }
    const agent_install_string = await get_agent_install_conf(
        req.system,
...
async function get_agent_install_conf(system, pool, account, routing_hint) {
    let cfg = system_store.data.agent_configs.find(
        conf => conf.pool === pool._id,
    );
    if (!cfg) {
        // create new configuration with the required settings
        dbg.log0(
            `creating new installation string for pool: ${pool.name} (${pool._id})`,
        );
    ...
    const addr = addr_utils.get_base_address(system.system_address, {
        hint: routing_hint,
    });
...
async function create_namespace_resource(req) {
    ...
    if (req.rpc_params.nsfs_config) {
        namespace_resource = new_namespace_resource_defaults(
            name,
            req.system._id,
            req.account._id,
            undefined,
            req.rpc_params.nsfs_config,
            req.rpc_params.access_mode,
        );
        const already_used_by = system_store.data.namespace_resources.find(
            cur_nsr =>
                cur_nsr.nsfs_config &&
                cur_nsr.nsfs_config.fs_root_path ===
                    namespace_resource.nsfs_config.fs_root_path,
        );
        if (already_used_by) {
            dbg.error(
                `fs root path ${already_used_by.nsfs_config.fs_root_path} already exported by ${already_used_by.name}`,
            );
        ...
        if (connection.azure_log_access_keys) {
            azure_log_access_keys = {
                azure_client_id:
                    connection.azure_log_access_keys.azure_client_id,
                azure_client_secret:
                    system_store.master_key_manager.encrypt_sensitive_string_with_master_key_id(
                        connection.azure_log_access_keys.azure_client_secret,
                        req.account.master_key_id._id,
                    ),
                azure_tenant_id:
                    connection.azure_log_access_keys.azure_tenant_id,
                azure_logs_analytics_workspace_id:
                    connection.azure_log_access_keys
                        .azure_logs_analytics_workspace_id,
            };
        }
        ...
        if (
            !cloud_buckets.find(
                bucket_name =>
                    bucket_name.name.unwrap() === req.rpc_params.target_bucket,
            )
        ) {
            dbg.error('This endpoint target bucket does not exist');
            throw new RpcError('INVALID_TARGET', "Target bucket doesn't exist");
        }
        const already_used_by = cloud_utils
            .get_used_cloud_targets(
                [namespace_resource.connection.endpoint_type],
            ...
            .find(
                candidate_target =>
                    candidate_target.endpoint ===
                        namespace_resource.connection.endpoint &&
                    candidate_target.target_name ===
                        namespace_resource.connection.target_bucket,
            );
        if (already_used_by) {
            dbg.error(
                `This endpoint is already being used by a ${already_used_by.usage_type}: ${already_used_by.source_name}`,
            );
            throw new RpcError('IN_USE', 'Target already in use');
        }
    }
...
async function create_cloud_pool(req) {
    ...
    if (
        !cloud_buckets.find(
            bucket_name =>
                bucket_name.name.unwrap() === cloud_info.target_bucket,
        )
    ) {
        dbg.error('This endpoint target bucket does not exist');
        throw new RpcError('INVALID_TARGET', "Target bucket doesn't exist");
    ...
    const already_used_by = cloud_utils
        ...
        .find(
            candidate_target =>
                candidate_target.endpoint === cloud_info.endpoint &&
    ...
    if (req.rpc_params.storage_limit < config.ENOUGH_ROOM_IN_TIER_THRESHOLD) {
        throw new RpcError(
            ...
            'new storage limit is smaller than the minimum allowed',
        );
    }
...
async function update_cloud_pool(req) {
    ...
    if (!pool.cloud_pool_info) {
        throw new RpcError(
            'INVALID_POOL_TYPE',
            `pool ${pool.name} is not a cloud pool`,
        );
    }
    ...
    if (new_available_capacity !== 'undefined') {
        if (new_available_capacity < 0) {
            throw new RpcError(
                'INVALID_AVAIL_CAPACITY',
                'Available capacity must be a positive quantity',
            );
        }
        updates.$set['cloud_pool_info.available_capacity'] =
    ...
    if (new_storage_limit) {
        ...
        const storage_minimum = Math.max(
            pool_info.storage.used,
            config.ENOUGH_ROOM_IN_TIER_THRESHOLD,
        );
        if (new_storage_limit < storage_minimum) {
            throw new RpcError(
                'INVALID_STORAGE_LIMIT',
                'new storage limit is smaller than size already used by pool or the minimum allowed',
            );
        }
        dbg.log0(
            `update_cloud_pool: updating ${pool.name} storage limit to ${new_storage_limit}, current usage is ${storage_minimum}`,
        );
        updates.$set['cloud_pool_info.storage_limit'] = new_storage_limit;
    } else if (
        new_storage_limit === null &&
        pool.cloud_pool_info.storage_limit
    ) {
...
function create_mongo_pool(req) {
    ...
    if (config.DB_TYPE !== 'mongodb') {
        ...
        throw new Error(
            `Cannot create mongo pool with DB_TYPE=${config.DB_TYPE}`,
        );
    }
...
async function read_pool(req) {
    const pool = find_pool_by_name(req);
    const nodes_aggregate_pool = await nodes_client
        .instance()
        .aggregate_nodes_by_pool([pool.name], req.system._id);
    const hosts_aggregate_pool = await nodes_client
        .instance()
        .aggregate_hosts_by_pool([pool.name], req.system._id);
...
function read_namespace_resource(req) {
    ...
    return P.resolve().then(() =>
        get_namespace_resource_info(namespace_resource),
    );
}
...
async function scale_hosts_pool(req) {
    ...
    if (!pool.hosts_pool_info) {
        throw new RpcError(
            'INVALID_POOL_TYPE',
            `pool ${pool.name} is not a hosts pool`,
        );
    }
    ...
    if (pool.mode === 'DELETING') {
        throw new RpcError(
            'INVALID_POOL_STATE',
            `pool ${pool.name} is being deleted`,
        );
    }
    ...
    if (host_count < current_host_count) {
        throw new RpcError(
            'INVALID_POOL_STATE',
            `can't scale down ${pool.name} hosts count to less than current connected hosts`,
        );
    }
    ...
    Dispatcher.instance().activity({
        ...
        pool: pool._id,
        desc: `Pool ${pool.name} was scaled ${
            host_count > current_configured_count ? 'up' : 'down'
        } from ${current_configured_count} to ${
            host_count
        } by  ${req.account.email.unwrap()}`,
    });

    // Update the host pool info.
    dbg.log0(
        `scale_hosts_pool: updating ${pool.name} host count from ${current_configured_count} to ${host_count}`,
    );
    return system_store.make_changes({
        update: {
            pools: [
...
async function update_hosts_pool(req) {
    ...
    // Update the host pool info.
    dbg.log0(
        `update_hosts_pool: updating ${pool.name} host count from ${configured_host_count} to ${host_count}`,
    );
    return system_store.make_changes({
        update: {
            pools: [
...
async function delete_hosts_pool(req, pool) {
    ...
    const current_host_count = await get_current_hosts_count(
        pool.name,
        req.system._id,
    );
    if (current_host_count > 0) {
        ...
        dbg.log0(
            `delete_hosts_pool: removing pool ${pool.name} from the database`,
        );
        await system_store.make_changes({
            remove: {
                pools: [pool._id],
        ...
        const related_funcs = await func_store
            .instance()
            .list_funcs_by_pool(req.system._id, pool._id);
        for (const func of related_funcs) {
            ...
            await func_store
                .instance()
                .update_func(func._id, { pools: new_pools_arr });
        }
    }
}

async function get_current_hosts_count(pool_name, system_id) {
    const hosts_aggregate = await nodes_client
        .instance()
...
function delete_resource_pool(req, pool) {
    ...
    return P.resolve()
        .then(() => {
            ...
            return nodes_client
                .instance()
                .list_nodes_by_pool(pool.name, req.system._id);
        })
        .then(function (pool_nodes) {
            if (!pool_nodes || pool_nodes.total_count === 0) {
                ...
                dbg.log0(
                    `resource_pool ${pool_name} does not have any nodes in nodes_monitor. delete agent and pool from hosted_agents`,
                );
            ...
            return P.map_one_by_one(pool_nodes.nodes, node =>
                nodes_client
                    .instance()
                    .delete_node_by_name(req.system._id, node.name),
            ).then(function () {
...
async function get_namespace_stats_by_cloud_service(
    system,
    start_date,
    end_date,
) {
    const ns_stats = _.keyBy(
        await IoStatsStore.instance().get_all_namespace_resources_stats({
            system,
            start_date,
            end_date,
        }),
...
async function get_cloud_services_stats(req) {
    ...
    const [cloud_stats, namespace_stats] = await P.all([
        ...
        get_namespace_stats_by_cloud_service(
            req.system._id,
            start_date,
            end_date,
        ),
    ...
    for (const acc of system_store.data.accounts) {
        if (acc.sync_credentials_cache) {
            acc.sync_credentials_cache.forEach(conn => {
                all_services[conn.endpoint_type] = all_services[
                    conn.endpoint_type
                ] || {
...
function get_pool_history(req) {
    ...
    return HistoryDataStore.instance()
        .get_pool_history()
        .then(history_records =>
            history_records.map(history_record => ({
                timestamp: history_record.time_stamp.getTime(),
                pool_list: history_record.system_snapshot.pools
                    .filter(
                        pool =>
                            !pool.mongo_info &&
                            (!pool_list || pool_list.includes(pool.name)),
                    )
                    .map(pool => {
...
function get_associated_buckets_int(pool) {
    const associated_buckets = _.filter(
        pool.system.buckets_by_name,
        function (bucket) {
...
function has_associated_buckets_int(pool, { exclude_deleting_buckets } = {}) {
    const associated_bucket = _.find(
        pool.system.buckets_by_name,
        function (bucket) {
            if (bucket.deleting && exclude_deleting_buckets) return false;
            return _.find(
                bucket.tiering && bucket.tiering.tiers,
                function (tier_and_order) {
                    return _.find(
                        tier_and_order.tier.mirrors,
                        function (mirror) {
                            return _.find(
                                mirror.spread_pools,
                                function (spread_pool) {
                                    return (
                                        String(pool._id) ===
                                        String(spread_pool._id)
                                    );
                                },
                            );
                        },
...
async function assign_pool_to_region(req) {
    ...
    if (pool.resource_type === 'INTERNAL') {
        throw new RpcError(
            'NO_REGION_ON_INTERNAL',
            "Can't set region for internal resource",
        );
    }
    ...
    Dispatcher.instance().activity({
        event:
            pool.resource_type === 'CLOUD' ?
                'resource.cloud_assign_region'
            :   'resource.pool_assign_region',
        level: 'info',
...
function find_namespace_resource_by_name(req) {
    ...
    if (!namespace_resource) {
        throw new RpcError(
            'NO_SUCH_NAMESPACE_RESOURCE',
            'No such namespace resource: ' + name,
        );
    }
...
function get_pool_info(pool, nodes_aggregate_pool, hosts_aggregate_pool) {
    ...
    const info = {
        ...
        storage: _.defaults(
            size_utils.to_bigint_storage(p_nodes.storage),
            POOL_STORAGE_DEFAULTS,
        ),
        region: pool.region,
    ...
    if (_is_cloud_pool(pool)) {
        ...
        info.storage_nodes = _.defaults(
            {},
            p_nodes.storage_nodes,
            POOL_NODES_INFO_DEFAULTS,
        );
        info.s3_nodes = _.defaults(
            {},
            p_nodes.s3_nodes,
            POOL_NODES_INFO_DEFAULTS,
        );
        info.hosts = _.mapValues(
            POOL_HOSTS_INFO_DEFAULTS,
            (val, key) => p_hosts.nodes[key] || val,
        );
...
function calc_namespace_resource_mode(namespace_resource) {
    ...
    if (!map_issues_and_monitoring_report.has(namespace_resource_id)) {
        map_issues_and_monitoring_report.set(namespace_resource_id, {
            last_monitoring: undefined,
            issues: [],
        });
    }
    const issues_report = map_issues_and_monitoring_report.get(
        namespace_resource_id,
    ).issues;
    const errors_count = _.reduce(
        issues_report,
        (acc, issue) => {
            ...
            const err_type =
                map_err_to_type_count[issue.error_code] || 'io_errors';
            acc[err_type] += 1;
            return acc;
        },
    ...
    const mode =
        (errors_count.storage_not_exist && 'STORAGE_NOT_EXIST') ||
        (errors_count.auth_failed && 'AUTH_FAILED') ||
        (errors_count.io_errors > config.NS_MAX_ALLOWED_IO_ERRORS &&
            'IO_ERRORS') ||
        'OPTIMAL';
...
function get_namespace_resource_operator_info(req) {
    ...
    const namespace_resource = find_namespace_resource_by_name(req);
    if (
        !namespace_resource.namespace_store ||
...
function get_namespace_resource_extended_info(namespace_resource) {
    const connection_info = namespace_resource.connection && {
        ...
        cp_code: namespace_resource.connection.cp_code || undefined,
        azure_log_access_keys:
            namespace_resource.connection.azure_log_access_keys,
        target_bucket: namespace_resource.connection.target_bucket,
...
function calc_hosts_pool_mode(pool_info, storage_by_mode, s3_by_mode) {
    ...
    const data_activities =
        pool_info.data_activities ? pool_info.data_activities.activities : [];
    ...
    const storage_issues_ratio =
        ((storage_count - storage_optimal) / storage_count) * 100;
    const hosts_initializing = hosts.by_mode.INITIALIZING || 0;
    const hosts_migrating =
        (hosts.by_mode.INITIALIZING || 0) +
        (hosts.by_mode.DECOMMISSIONING || 0) +
        (hosts.by_mode.MIGRATING || 0);
    ...
    const { free, total, reserved, used_other } = _.assignWith(
        {},
        storage,
        (__, size) => size_utils.json_to_bigint(size),
    );
    const potential_for_noobaa = total.subtract(reserved).subtract(used_other);
    const free_ratio =
        potential_for_noobaa.greater(0) ?
            free.multiply(100).divide(potential_for_noobaa)
        :   size_utils.BigInteger.zero;
    const activity_count = data_activities.reduce(
    ...
    return (
        ...
        (storage_issues_ratio >= 90 &&
            s3_issues_ratio >= 90 &&
            'MOST_NODES_ISSUES') ||
        (storage_issues_ratio >= 50 &&
            s3_issues_ratio >= 50 &&
            'MANY_NODES_ISSUES') ||
    ...
    const associated_buckets = _.filter(ns.system.buckets_by_name, bucket => {
        ...
        return (
            _.find(
                ...
                read_resource =>
                    String(ns._id) === String(read_resource.resource._id),
            ) ||
            String(ns._id) ===
                String(bucket.namespace.write_resource.resource._id)
        );
    });
...
function get_internal_mongo_pool(system) {
    return system.pools_by_name[
        `${config.INTERNAL_STORAGE_POOL_NAME}-${system._id}`
    ];
}
...
async function get_optimal_non_mongo_pool_id() {
    for (const pool of system_store.data.pools) {
        ...
        const aggr_nodes = await nodes_client
            .instance()
            .aggregate_nodes_by_pool([pool.name], pool.system._id);
        const aggr_hosts = await nodes_client
            .instance()
            .aggregate_hosts_by_pool([pool.name], pool.system._id);
...
function update_issues_report(req) {
    const { namespace_resource_id, error_code, time, monitoring } =
        req.rpc_params;
    ...
    if (!ns_resource) {
        dbg.log0(
            'update_issues_report: can not find namespace_resource, ignoring update of issues report',
        );
        return;
    }
    ...
    const cur_issues_report = map_issues_and_monitoring_report.get(
        namespace_resource_id,
    ).issues;
    ...
    if (monitoring) {
        map_issues_and_monitoring_report.get(
            namespace_resource_id,
        ).last_monitoring = time;
    }
...
exports.get_namespace_resource_extended_info =
    get_namespace_resource_extended_info;
exports.get_namespace_resource_operator_info =
