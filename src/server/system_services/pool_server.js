    ...
    if (connection.azure_log_access_keys) {
      azure_log_access_keys = {
        ...
        azure_logs_analytics_workspace_id:
          connection.azure_log_access_keys.azure_logs_analytics_workspace_id,
      };
    }
...
async function create_cloud_pool(req) {
  ...
  if (
    !cloud_buckets.find(
      bucket_name => bucket_name.name.unwrap() === cloud_info.target_bucket,
    )
  ) {
    dbg.error('This endpoint target bucket does not exist');
    throw new RpcError('INVALID_TARGET', "Target bucket doesn't exist");
...
async function get_cloud_services_stats(req) {
  ...
  const [cloud_stats, namespace_stats] = await P.all([
    nodes_client
      .instance()
      .get_nodes_stats_by_cloud_service(req.system._id, start_date, end_date),
    ...
    .then(history_records =>
      history_records.map(history_record => ({
        ...
        pool_list: history_record.system_snapshot.pools
          .filter(
            pool =>
              !pool.mongo_info && (!pool_list || pool_list.includes(pool.name)),
          )
          .map(pool => {
  ...
  const associated_buckets = _.filter(ns.system.buckets_by_name, bucket => {
    ...
    return (
      _.find(
        ...
        read_resource => String(ns._id) === String(read_resource.resource._id),
      ) ||
...
function update_last_monitoring(req) {
  ...
  map_issues_and_monitoring_report.get(namespace_resource_id).last_monitoring =
    last_monitoring;
