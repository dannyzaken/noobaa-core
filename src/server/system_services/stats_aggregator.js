    ...
    sys_stats.systems = await P.all(
      _.map(system_store.data.systems, async system => {
        ...
        const has_dns_name = system.system_address.some(
          addr => addr.api === 'mgmt' && !net_utils.is_ip(addr.hostnames),
        );
      ...
      _.compact(
        _.map(system_store.data.accounts, async account => {
          ...
          const read_bytes = size_utils.json_to_bigint(
            account_usage_info[0].read_bytes || size_utils.BigInteger.zero,
          );
          const write_bytes = size_utils.json_to_bigint(
            account_usage_info[0].write_bytes || size_utils.BigInteger.zero,
          );
    ...
    for (const bucket of system_store.data.buckets) {
      ...
      for (const [key, value] of Object.entries(pools)) {
        ...
        if (pool.cloud_pool_info) {
          type =
            supported_cloud_types.includes(pool.cloud_pool_info.endpoint_type) ?
  ...
  try {
    sys_stats.systems = await P.all(
      _.map(system_store.data.systems, async system => {
        ...
        const nodes_aggregate_pool_with_cloud_no_mongo = await nodes_client
          .instance()
    ...
    for (const bucket of system_store.data.buckets) {
      ...
      objects_non_namespace_buckets_sys.chunks_capacity =
        objects_non_namespace_buckets_sys.chunks_capacity.plus(
          (bucket.storage_stats &&
            size_utils.json_to_bigint(bucket.storage_stats.chunks_capacity)) ||
            0,
        );
      ...
      const bucket_project =
        (bucket_info.bucket_claim && bucket_info.bucket_claim.namespace) ||
        'Cluster-wide';
      ...
      const bucket_class =
        (bucket_info.bucket_claim && bucket_info.bucket_claim.bucket_class) ||
        'Cluster-wide';
      const existing_bucket_class =
        usage_by_bucket_class[bucket_class] || size_utils.BigInteger.zero;
  ...
  return Promise.all([
    ...
    .then(([nodes_results, hosts_results]) => {
      ...
      for (const node of nodes_results.nodes) {
        ...
        nodes_histo.histo_free.add_value(node.storage.free / SCALE_BYTES_TO_GB);
...
function get_pool_stats(req) {
  return P.resolve()
    .then(() =>
      nodes_client.instance().aggregate_nodes_by_pool(null, req.system._id),
    )
...
async function get_namespace_resource_stats(req) {
  ...
  await P.all(
    _.map(system_store.data.namespace_resources, async nsr => {
      const nsr_info = await server_rpc.client.pool.read_namespace_resource(
        { name: nsr.name },
        {
          auth_token: req.auth_token,
        },
...
function _set_op_stats(op_name, stats) {
  ...
  if (op_stats[op_name]) {
    ...
    const old_sum_time =
      op_stats[op_name].avg_time_milisec * op_stats[op_name].count;
    ...
    const min_time_milisec = Math.min(
      op_stats[op_name].min_time_milisec,
      stats.min_time,
    );
    const max_time_milisec = Math.max(
      op_stats[op_name].max_time_milisec,
      stats.max_time,
    );
    ...
    const avg_time_milisec = Math.floor(
      (old_sum_time + stats.sum_time) / (count - error_count),
    );
    op_stats[op_name] = {
      min_time_milisec,
      max_time_milisec,
...
function _set_fs_workers_stats(fsworker_name, stats) {
  if (fs_workers_stats[fsworker_name]) {
    ...
    const error_count =
      fs_workers_stats[fsworker_name].error_count + stats.error_count;
    const old_sum_time =
      fs_workers_stats[fsworker_name].avg_time_microsec *
      fs_workers_stats[fsworker_name].count;
    //Min time and Max time are not being counted in the namespace_fs if it was error
    const min_time_microsec = Math.min(
      fs_workers_stats[fsworker_name].min_time_microsec,
      stats.min_time,
    );
    const max_time_microsec = Math.max(
      fs_workers_stats[fsworker_name].max_time_microsec,
      stats.max_time,
    );
    // At this point, as we populate only when there is at least one successful fsworker, there must be old_sum_time
    const avg_time_microsec = Math.floor(
      (old_sum_time + stats.sum_time) / (count - error_count),
    );
