  ...
  const reply = {
    ...
    bucket_info: await P.map_props({
      ...
      nodes_aggregate_pool:
        bucket.tiering &&
        nodes_client.instance().aggregate_nodes_by_pool(pool_names, system._id),
      hosts_aggregate_pool:
        bucket.tiering &&
        nodes_client.instance().aggregate_hosts_by_pool(null, system._id),
      ...
      unused_refresh_tiering_alloc:
        bucket.tiering && node_allocator.refresh_tiering_alloc(bucket.tiering),
  ...
  if (bucket.namespace) {
    reply.namespace = {
        ...
        bucket.namespace.write_resource ?
          {
            resource: pool_server.get_namespace_resource_extended_info(
              system.namespace_resources_by_name[
                bucket.namespace.write_resource.resource.name
              ],
            ),
      ...
      read_resources: _.map(bucket.namespace.read_resources, rs => ({
        resource: pool_server.get_namespace_resource_extended_info(rs.resource),
        path: rs.path,
      })),
...
function get_bucket_changes_quota(
  ...
  if (quota.is_empty_quota()) {
    ...
    if (!quota.is_valid_quota()) {
      throw new RpcError('BAD_REQUEST', 'quota config values must be positive');
    }
...
async function list_buckets(req) {
  const buckets_by_name = _.filter(
    ...
    async bucket =>
      (await req.has_s3_bucket_permission(bucket, 's3:ListBucket', req)) &&
      !bucket.deleting,
  );
  return {
...
async function get_cloud_buckets(req) {
  try {
    ...
    if (connection.endpoint_type === 'AZURE') {
      ...
      const used_cloud_buckets = cloud_utils.get_used_cloud_targets(
        ['AWS', 'AWSSTS', 'AWS_STS', 'S3_COMPATIBLE', 'FLASHBLADE', 'IBM_COS'],
        system_store.data.buckets,
        system_store.data.pools,
        system_store.data.namespace_resources,
      );
      ...
      const same_nsfs_path =
        ...
        Object.values(req.system.buckets_by_name).filter(
          buck =>
            ...
            path.join(
              buck.namespace.write_resource.resource.nsfs_config.fs_root_path,
      ...
      bucket.namespace ?
        {
          write_resource:
            ...
            : undefined,
          read_resources: _.map(bucket.namespace.read_resources, rs => ({
            resource: pool_server.get_namespace_resource_info(rs.resource).name,
            path: rs.path,
          })),
          should_create_underlying_storage:
            bucket.namespace.should_create_underlying_storage,
...
function _calc_metrics({
  ...
  const available_quantity_for_upload = quota.get_available_quantity_for_upload(
    objects_aggregate.count,
  );
...
async function get_bucket_replication(req) {
  ...
  const bucket_names_replication = _.map(replication, rule => {
    const named_rule = {
      ...rule,
      destination_bucket: system_store.data.get_by_id(rule.destination_bucket)
        .name,
    };
        ...
        if (_.isEqual(db_rules._id, dst_bucket.replication_policy_id)) {
          const matching_rule = db_rules.rules.find(
            db_rule =>
              ...
              (!db_rule.filter ||
                db_rule.filter.prefix.toString().startsWith(prefix) ||
...
function normalize_replication(req) {
  ...
  if (req.rpc_params.replication_policy.log_replication_info?.endpoint_type) {
    if (
      req.rpc_params.replication_policy.log_replication_info.endpoint_type ===
      'AZURE'
    ) {
