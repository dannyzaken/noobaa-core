...
const BucketStatsStore =
    require('../analytic_services/bucket_stats_store').BucketStatsStore;
...
function new_bucket_defaults(
    ...
    return {
        ...
        lambda_triggers: [],
        versioning:
            config.WORM_ENABLED && lock_enabled ? 'ENABLED' : 'DISABLED',
...
function auto_setup_tier2(
    ...
    const init_tier_order = tiering_policy.tiers.find(
        tier => String(tier.tier) === String(initial_tier._id),
    ).order;
    ...
    return bucket_semaphore.surround_key(
        ...
        async () => {
            ...
            if (req.rpc_params.tiering) {
                tiering_policy = resolve_tiering_policy(
                    req,
                    req.rpc_params.tiering,
                );
            } else if (
                req.system.namespace_resources_by_name &&
                req.system.namespace_resources_by_name[
                    req.account.default_resource.name
                ]
            ) {
                dbg.log0('creating bucket on default namespace resource');
                if (
                    !req.account.nsfs_account_config ||
                    !req.account.nsfs_account_config.new_buckets_path
                ) {
                ...
                _.isUndefined(req.rpc_params.namespace) ||
                ...
                tier_server.check_tiering_policy_exists(
                    req,
                    bucket_with_suffix,
                );
            ...
            const bucket = new_bucket_defaults(
                req.rpc_params.name,
                req.system._id,
                tiering_policy && tiering_policy._id,
                req.account._id,
                req.rpc_params.tag,
            ...
            if (req.rpc_params.namespace) {
                const read_resources = _.compact(
                    req.rpc_params.namespace.read_resources.map(nsr => {
                        const res =
                            req.system.namespace_resources_by_name &&
                            req.system.namespace_resources_by_name[
                                nsr.resource
                ...
                const wr_obj =
                    ...
                    req.system.namespace_resources_by_name &&
                    req.system.namespace_resources_by_name[
                        req.rpc_params.namespace.write_resource.resource
                    ];
                const write_resource = wr_obj && {
                    resource: wr_obj._id,
                    path: req.rpc_params.namespace.write_resource.path,
                };
                if (!write_resource) {
                    dbg.log0(
                        'write resource was not provided, will create a readonly namespace bucket',
                    );
                }
                if (
                ...
                const ordered_read_resources =
                    write_resource ?
                        [write_resource].concat(
                            read_resources.filter(
                                rr => rr.resource !== write_resource.resource,
                            ),
                        )
                    :   read_resources;
            ...
            if (req.rpc_params.bucket_claim || req.rpc_params.namespace) {
                try {
                    ...
                    dbg.error(
                        'create_bucket: get_partial_stats failed with',
                        error,
                    );
                }
            }
...
async function put_bucket_logging(req) {
    ...
    const log_bucket =
        ...
        req.system.buckets_by_name[log_bucket_name.unwrap()];
    if (!log_bucket) {
    ...
    if (
        log_bucket.owner_account._id.toString() !==
        bucket.owner_account._id.toString()
    ) {
        dbg.error('TARGET BUCKET NOT OWNED BY USER', log_bucket);
        throw new RpcError(
            'INVALID_TARGET_BUCKET',
            'The target bucket for logging is not owned by you',
        );
    }
        ...
        reply.namespace = {
            write_resource:
                bucket.namespace.write_resource ?
                    {
                        resource:
                            pool_server.get_namespace_resource_extended_info(
                                system.namespace_resources_by_name[
                                    bucket.namespace.write_resource.resource
                                        .name
                                ],
                            ),
            ...
            should_create_underlying_storage:
                bucket.namespace.should_create_underlying_storage,
        };
    }
    return reply;
}
...
function get_bucket_changes_versioning(
    ...
    if (
        bucket.namespace &&
        !bucket.namespace.write_resource.resource.nsfs_config
    ) {
        throw new RpcError(
            'BAD_REQUEST',
            'Cannot set versioning on non nsfs namespace buckets',
        );
    }
...
function get_bucket_changes_namespace(
    ...
    const read_resources = _.compact(
        update_request.namespace.read_resources.map(nsr => {
            const res =
                req.system.namespace_resources_by_name &&
                req.system.namespace_resources_by_name[nsr.resource];
            return res && { resource: res._id, path: nsr.path };
        }),
    );
    if (
    ...
    const wr_obj =
        req.system.namespace_resources_by_name &&
        req.system.namespace_resources_by_name[
            update_request.namespace.write_resource.resource
        ];
    const write_resource = wr_obj && {
        resource: wr_obj._id,
        path: update_request.namespace.write_resource.path,
    };
    ...
    const ordered_read_resources = [write_resource].concat(
        read_resources.filter(rr => rr.resource !== write_resource.resource),
    );
...
function get_bucket_changes_quota(
    ...
    if (quota.is_empty_quota()) {
        ...
        if (!quota.is_valid_quota()) {
            throw new RpcError(
                'BAD_REQUEST',
                'quota config values must be positive',
            );
        }

        quota.add_quota_alerts(
...
async function delete_bucket_and_objects(req) {
    ...
    if (bucket.replication_policy_id) {
        ...
        await replication_store
            .instance()
            .delete_replication_by_id(bucket.replication_policy_id);
    }
                    ...
                    ..._.compact(
                        _.map(tiering_policy.tiers, tier_and_order => {
                            const associated_tiering_policies = tier_server
                                .get_associated_tiering_policies(
                                    tier_and_order.tier,
                                )
                                .filter(
                                    policy_id =>
                                        String(policy_id) !==
                                        String(tiering_policy._id),
                                );
...
async function delete_bucket_lifecycle(req) {
    ...
    try {
        ...
        const desc_string = [];
        desc_string.push(
            `lifecycle configuration rules were removed for bucket ${bucket.name.unwrap()} by ${req.account && req.account.email.unwrap()}`,
        );
        ...
        dbg.error(
            'Error deleting lifecycle configuration rules',
            err,
            err.stack,
        );
        throw err;
...
async function get_bucket_throughput_usage(req) {
    ...
    const report = await usage_aggregator.get_bandwith_over_time({
        buckets:
            ...
            buckets.map(
                bucket_name =>
                    req.system.buckets_by_name[bucket_name.unwrap()]._id,
            ),
        resolution,
        since,
        till,
...
async function set_bucket_lifecycle_configuration_rules(req) {
    ...
    try {
        ...
        const desc_string = [];
        desc_string.push(
            `${bucket.name.unwrap()} was updated with lifecycle configuration rules by ${req.account && req.account.email.unwrap()}`,
        );
        ...
        dbg.error(
            'Error setting lifecycle configuration rules',
            err,
            err.stack,
        );
        throw err;
...
async function get_cloud_buckets(req) {
    try {
        ...
        if (connection.endpoint_type === 'AZURE') {
            ...
            return P.timeout(
                EXTERNAL_BUCKET_LIST_TO,
                (async function () {
                    const result = [];
                    for await (const response of blob_svc
                        .listContainers()
            ...
            // else if AWS(s3-compatible/aws/sts-aws)/Flashblade/IBM_COS
            let access_key;
            let secret_key;
            if (connection.aws_sts_arn) {
                const creds = await cloud_utils.generate_aws_sts_creds(
                    connection,
                    'get_cloud_buckets_session',
                );
        ...
        if (err instanceof P.TimeoutError) {
            dbg.log0(
                'failed reading (t/o) external buckets list',
                req.rpc_params,
            );
        } else {
            ...
            buckets: [
                {
                    $find: {
                        _id: bucket._id,
                        'lambda_triggers._id': trigger._id,
                    },
                    $set: _.mapKeys(
                        updates,
...
function validate_nsfs_bucket(req) {
    ...
    if (
        ...
        const write_resource =
            req.system.namespace_resources_by_name[
                req.rpc_params.namespace.write_resource.resource
            ];
        const bucket_nsfs_config = write_resource && write_resource.nsfs_config;
        if (bucket_nsfs_config) {
            const bucket_path = path.join(
                bucket_nsfs_config.fs_root_path,
                req.rpc_params.namespace.write_resource.path || '',
            );

            const same_nsfs_path =
                req.system.buckets_by_name &&
                Object.values(req.system.buckets_by_name).filter(
                    buck =>
                        buck.namespace &&
                        buck.namespace.write_resource.resource.nsfs_config &&
                        path.join(
                            buck.namespace.write_resource.resource.nsfs_config
                                .fs_root_path,
                            buck.namespace.write_resource.path || '',
                        ) === bucket_path,
                );
...
function validate_trigger_update(bucket, validated_trigger) {
    dbg.log0(
        'validate_trigger_update: Checking new trigger is legal:',
        validated_trigger,
    );
...
function _inject_usage_to_cloud_bucket(target_name, endpoint, usage_list) {
    ...
    const using_target = usage_list.find(
        candidate_target =>
...
function get_bucket_info({
    ...
    const tiering =
        bucket.tiering &&
        tier_server.get_tiering_policy_info(
            bucket.tiering,
    ...
    const info = {
        name: bucket.name,
        owner_account:
            bucket.owner_account && bucket.owner_account.email ?
                {
            ...
            bucket.namespace ?
                {
                    write_resource:
                        bucket.namespace.write_resource ?
                            {
                                resource:
                                    pool_server.get_namespace_resource_info(
                                        bucket.namespace.write_resource
                                            .resource,
                                    ).name,
                                path: bucket.namespace.write_resource.path,
                    ...
                    read_resources: _.map(
                        ...
                        rs => ({
                            resource: pool_server.get_namespace_resource_info(
                                rs.resource,
                            ).name,
                            path: rs.path,
                        }),
        ...
        object_lock_configuration:
            config.WORM_ENABLED ? bucket.object_lock_configuration : undefined,
        tagging: bucket.tagging,
    ...
    _.mapKeys(
        _.get(bucket, 'storage_stats.pools') || {},
        function (storage, pool_id) {
    ...
    ignore_quota = true;
    if (info.tiering) {
        info.tiering.mode = calc_bucket_mode(
            tiering.tiers,
            metrics,
            ignore_quota,
        );
    ...
    if (bucket_stats) {
        ...
        const size_by_content_type = _.keyBy(
            content_type_stats,
            'content_type',
        );
...
function _calc_metrics({
    ...
    _.each(bucket.tiering && bucket.tiering.tiers, tier_and_order => {
        ...
        const configured_failure_tolerance =
            ccc.parity_frags || ccc.replicas - 1;
        risky_tolerance = configured_failure_tolerance < 0;

        const mirrors_with_valid_pool = tier_extra_info.mirrors_with_valid_pool;
        const mirrors_with_enough_nodes =
            tier_extra_info.mirrors_with_enough_nodes;
        ...
        if (tier_and_order.tier.mirrors.length > 0) {
            if (
                mirrors_with_valid_pool === tier_and_order.tier.mirrors.length
            ) {
                has_enough_healthy_nodes_for_tiering = true;
            }
            if (
                mirrors_with_enough_nodes === tier_and_order.tier.mirrors.length
            ) {
    ...
    const quota = new Quota(bucket.quota);
    const available_size_for_upload = quota.get_available_size_for_upload(
        actual_free,
        objects_aggregate.size,
    );
    const available_quantity_for_upload =
        quota.get_available_quantity_for_upload(objects_aggregate.count);
    ...
    const tier_with_issues = _.filter(
        info.tiering && info.tiering.tiers,
...
function calc_namespace_bucket_mode(namespace_dict) {
    ...
    const rr_modes = _.reduce(
        rrs,
        (acc, rr) => {
            const resource_mode = pool_server.calc_namespace_resource_mode(
                rr.resource,
            );
    ...
    const mode =
        (rr_modes.auth_failed + rr_modes.storage_not_exist === rrs.length &&
            'NO_RESOURCES') ||
        ((rr_modes.auth_failed || rr_modes.storage_not_exist) &&
...
function calc_bucket_mode(tiers, metrics, ignore_quota, bucket_namespace) {
    ...
    if (bucket_namespace) {
        ...
        if (
            !bucket_namespace.caching ||
            (bucket_namespace.caching && bucket_mode !== 'OPTIMAL')
        ) {
            return bucket_mode;
        }
    }
            ...
            buckets: [
                {
                    _id: bucket._id,
                    object_lock_configuration:
                        req.rpc_params.object_lock_configuration,
                },
            ],
        },
    });
...
function validate_non_nsfs_bucket_creation(req) {
    ...
    dbg.log0(
        'validate_non_nsfs_bucket_create_allowed:',
        nsfs_account_config,
        req.rpc_params.namespace,
    );
    ...
    dbg.log0(
        'validate_non_nsfs_bucket_create_allowed: namespace_bucket_config',
        nsr && nsr.nsfs_config,
    );
...
async function put_bucket_replication(req) {
    ...
    const replication_id =
        bucket_replication_id ?
            await replication_store
                .instance()
                .update_replication(replication_rules, bucket_replication_id)
        :   await replication_store
                .instance()
                .insert_replication(replication_rules);
    ...
    await system_store.make_changes({
        update: {
            buckets: [
                { _id: bucket._id, replication_policy_id: replication_id },
            ],
        },
    });
}
...
async function get_bucket_replication(req) {
    ...
    const replication = await replication_store
        .instance()
        .get_replication_by_id(replication_id);
    const bucket_names_replication = _.map(replication, rule => {
        const named_rule = {
            ...rule,
            destination_bucket: system_store.data.get_by_id(
...
async function validate_replication(req) {
    ...
    if (
        replication_rules.length > config.BUCKET_REPLICATION_MAX_RULES ||
        replication_rules.length < 1
    ) {
        throw new RpcError(
            'INVALID_REPLICATION_POLICY',
            'Number of rules is invalid',
        );
    }
    ...
    const db_repl_rules = await replication_store
        .instance()
    ...
    for (const rule of replication_rules) {
        ...
        const dst_bucket =
            req.system.buckets_by_name &&
            req.system.buckets_by_name[destination_bucket.unwrap()];
        // rule's destination bucket must exist and not equal to the replicated bucket
        if (
            req.rpc_params.name.unwrap() === destination_bucket.unwrap() ||
            !dst_bucket
        ) {
            throw new RpcError(
                'INVALID_REPLICATION_POLICY',
                `Rule ${rule_id} destination bucket is invalid`,
            );
        }
        ...
        if (
            destination_bucket.unwrap() !== req.rpc_params.name.unwrap() &&
            src_bucket
        ) {
                ...
                if (_.isEqual(db_rules._id, dst_bucket.replication_policy_id)) {
                    const matching_rule = db_rules.rules.find(
                        db_rule =>
                            _.isEqual(
                                src_bucket._id,
                                db_rule.destination_bucket,
                            ) &&
                            (!db_rule.filter ||
                                db_rule.filter.prefix
                                    .toString()
                                    .startsWith(prefix) ||
                                prefix
                                    .toString()
                                    .startsWith(
                                        db_rule.filter.prefix.toString(),
                                    )),
                    );
                    if (matching_rule) {
                        throw new RpcError(
        ...
        pref_by_dst_bucket[destination_bucket] =
            pref_by_dst_bucket[destination_bucket] || [];
        pref_by_dst_bucket[destination_bucket].push(
            (filter && filter.prefix) || '',
        );
    }

    // all rule_ids must be different
    if (new Set(rule_ids).size < replication_rules.length) {
        throw new RpcError(
            'INVALID_REPLICATION_POLICY',
    ...
    if (
        Object.keys(pref_by_dst_bucket).length >
        config.BUCKET_REPLICATION_MAX_DST_BUCKETS
    ) {
        throw new RpcError(
            'INVALID_REPLICATION_POLICY',
            'Number of unique destination buckets is invalid',
        );
    }
...
function normalize_replication(req) {
    ...
    if (req.rpc_params.replication_policy.log_replication_info?.endpoint_type) {
        if (
            req.rpc_params.replication_policy.log_replication_info
                .endpoint_type === 'AZURE'
        ) {
            replication_info_dict = {
                azure_log_replication_info: _.omit(
...
exports.set_bucket_lifecycle_configuration_rules =
    set_bucket_lifecycle_configuration_rules;
exports.get_bucket_lifecycle_configuration_rules =
    get_bucket_lifecycle_configuration_rules;
