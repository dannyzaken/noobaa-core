...
class ReplicationStore {
    ...
    static instance() {
        if (!ReplicationStore._instance) {
            ReplicationStore._instance = new ReplicationStore();
        }
    ...
    async get_replication_by_id(replication_id) {
        ...
        const repl = await this._replicationconfigs.findOne({
            _id: db_client.instance().parse_object_id(replication_id),
            deleted: null,
        });
    ...
    async update_replication_status_by_id(_id, rule_id, status) {
        ...
        const ans = await this._replicationconfigs.findOneAndUpdate(
            find,
            update,
            options,
        );
    ...
    async update_log_replication_status_by_id(_id, last_cycle_end) {
        ...
        const update = {
            $set: {
                'log_replication_info.status.last_cycle_end': last_cycle_end,
            },
        };
        const options = { returnOriginal: false };
        dbg.log1(
            'update_log_replication_status_by_id: ',
            find,
            update,
            options,
        );
    ...
    async update_log_replication_marker_by_id(_id, rule_id, log_marker) {
        dbg.log1(
            'update_log_replication_marker_by_id: ',
            _id,
            rule_id,
            log_marker,
        );
    ...
    async find_rules_updated_longest_time_ago() {
        ...
        const replications = await this._replicationconfigs.find({
            deleted: null,
        });
        ...
        const reduced_replications = _.map(replications, repl => ({
            ...
            rule: _.minBy(
                repl.rules,
                rule =>
                    (rule.rule_status && rule.rule_status.last_cycle_end) || 0,
            ), //least_recently_replicated_rule
        }));
