    ...
    while (!update_done) {
        try {
            ...
            if (
                ...
                system_store.data.systems.length
            ) {
...
function new_system_changes(req, name, owner_account_id) {
    ...
    const bucket_with_suffix =
        default_bucket_name + '#' + Date.now().toString(36);
    ...
    if (config.DEFAULT_POOL_TYPE === 'INTERNAL') {
        ...
        const mongo_pool = pool_server.new_pool_defaults(
            pool_name,
            system._id,
            'INTERNAL',
            'BLOCK_STORE_MONGO',
        ...
        const fs_pool = pool_server.new_pool_defaults(
            pool_name,
            system._id,
            'HOSTS',
            'BLOCK_STORE_FS',
        ...
        throw new Error(
            `Unsupported config.DEFAULT_POOL_TYPE=${config.DEFAULT_POOL_TYPE}`,
        );
    }
    ...
    const default_chunk_config =
        config.CHUNK_CODER_EC_IS_DEFAULT ? ec_chunk_config : (
    ...
    bucket_server.auto_setup_tier2(req, tier, policy, changes, true);
...
async function create_system(req) {
    dbg.log0(
        'create_system: got create_system with params:',
        util.inspect(req.rpc_params, { depth: null }),
    );
    ...
    let system_id;
    const { name, email, password, must_change_password } = req.rpc_params;
    ...
    try {
        ...
        const { token: operator_token } =
            await server_rpc.client.account.create_account(
                {
                    name,
...
async function _get_cluster_info() {
    const cluster_info = await cluster_server.new_cluster_info({
        address: 'localhost',
    });
    if (cluster_info) {
        ...
        if (dns_config.dns_servers.length) {
            dbg.log0(
                `create_system: DNS servers were already configured in first install to`,
...
async function _configure_system_address(system_id, account_id) {
    const system_address =
        process.env.CONTAINER_PLATFORM === 'KUBERNETES' ?
...
async function read_system(req) {
    ...
    const {
        ...
        nodes_aggregate_pool_no_cloud_and_mongo: nodes_client
            .instance()
        ...
        nodes_aggregate_pool_with_cloud_and_mongo: nodes_client
            .instance()
            .aggregate_nodes_by_pool(
                null,
                system._id,
                /*skip_cloud_nodes=*/ false,
                /*skip_mongo_nodes=*/ false,
            ),

        // nodes - count, online count, allocated/used storage aggregate by pool
        nodes_aggregate_pool_with_cloud_no_mongo: nodes_client
            .instance()
            .aggregate_nodes_by_pool(
                null,
                system._id,
        ...
        accounts: P.fcall(() =>
            server_rpc.client.account.list_accounts(
                {},
                {
        ...
        buckets_stats: BucketStatsStore.instance().get_all_buckets_stats({
            system: system._id,
        }),
    ...
    const debug_time =
        system.debug_mode ?
            Math.max(
                0,
                config.DEBUG_MODE_PERIOD - (Date.now() - system.debug_mode),
            )
        :   undefined;
    ...
    const system_cap =
        system.freemium_cap.cap_terabytes ?
            system.freemium_cap.cap_terabytes
    ...
    const stats_by_bucket = _.keyBy(buckets_stats, stats =>
        _.get(system_store.data.get_by_id(stats._id), 'name'),
    );

    const base_address = addr_utils.get_base_address(system.system_address);
    const dns_name =
        net.isIP(base_address.hostname) === 0 ?
            base_address.hostname
        :   undefined;
    ...
    return {
        ...
        buckets: _.filter(system.buckets_by_name, bucket =>
        ...
        namespace_resources: _.map(system.namespace_resources_by_name, ns =>
            pool_server.get_namespace_resource_info(ns),
        ),
        pools: _.filter(
            system.pools_by_name,
            pool =>
                !_.get(pool, 'cloud_pool_info.pending_delete') &&
                !_.get(pool, 'mongo_pool_info.pending_delete'),
        ).map(pool =>
            pool_server.get_pool_info(
                pool,
                nodes_aggregate_pool_with_cloud_and_mongo,
                hosts_aggregate_pool,
            ),
        ),
        tiers: _.map(system.tiers_by_name, tier =>
            tier_server.get_tier_info(
                tier,
                nodes_aggregate_pool_with_cloud_and_mongo,
        ...
        nodes_storage: size_utils.to_bigint_storage(
            _.defaults(
                {
                    used: objects_sys.size,
                },
                nodes_aggregate_pool_no_cloud_and_mongo.storage,
                SYS_STORAGE_DEFAULTS,
            ),
        ),
        nodes: _.defaults(
            {},
            nodes_aggregate_pool_no_cloud_and_mongo.nodes,
            SYS_NODES_INFO_DEFAULTS,
        ),
        hosts: _.omit(
            _.defaults({}, hosts_aggregate_pool.nodes, SYS_NODES_INFO_DEFAULTS),
            'storage_count',
        ),
        owner: account_server.get_account_info(
            system_store.data.get_by_id(system._id).owner,
        ),
        ...
        defaults: {
            tiers: {
                ...
                failure_tolerance_threshold:
                    config.CHUNK_CODER_EC_TOLERANCE_THRESHOLD,
            },
        },
...
function set_maintenance_mode(req) {
    ...
    let audit_desc = '';
    const send_event =
        req.rpc_params.duration ?
            'dbg.maintenance_mode'
        :   'dbg.maintenance_mode_stopped';
...
async function update_n2n_config(req) {
    ...
    if (update.tcp_permanent_passive) {
        if (
            update.tcp_permanent_passive.min >= update.tcp_permanent_passive.max
        ) {
            throw new Error(
                'Min port range cannot be equal or higher then max',
            );
        }
    }
...
function _init_system(sysid) {
    dbg.log0(
        'init system - calling init_cluster and collecting first system stats',
    );
...
async function get_join_cluster_yaml(req) {
    ...
    if (ep_max_count < ep_min_count) {
        throw new RpcError(
            'BAD_REQUEST',
            'endpoints.max_count cannot be lower then endpoints.min_count',
        );
    }

    const operator_account = system_store.data.accounts.find(
        account =>
        ...
        stringData: {
            ...
            ...Object.fromEntries(
                ['mgmt', 'bg', 'md', 'hosted_agents'].map(api_name => [
                    `${api_name}_addr`,
                    addr_utils
                        .get_base_address(req.system.system_address, {
                            api: api_name,
                            hint: 'EXTERNAL',
                            protocol: 'wss',
                            secure: true,
    ...
    return yaml_utils.stringify([joinSecret, noobaa]);
}
...
async function update_endpoint_group(req) {
    ...
    const exists = (cluster.endpoint_groups || []).some(
        group => group.name === group_name,
    );

    if (exists) {
        const group = cluster.endpoint_groups.find(
            grp => grp.name === group_name,
        );
        if (!_.isUndefined(is_remote)) {
            if (group.is_remote !== is_remote) {
                // We do not throw in order to not fail the noobaa operator.
                dbg.warn(
                    'update_endpoint_group: Conflicted is_remote value of ',
                    is_remote,
                    ' for group: ',
                    group,
                    ' - aborting request',
        ...
        if (
            group.region !== region ||
            !_.isEqual(group.endpoint_range, endpoint_range)
        ) {
            dbg.log0(
                'updating endpoint group. old values: ',
                group,
                ' new values:',
                { name: group_name, is_remote, region, endpoint_range },
            );
        ...
        dbg.log0('adding new endpoint group:', {
            name: group_name,
            is_remote,
            region,
            endpoint_range,
    ...
    if (!account_roles.includes('operator') && !is_remote) {
        try {
            ...
            dbg.error(
                'update_endpoint_group: Could not update noobaa CRD, got',
                err,
            );
        }
    }
...
async function get_endpoints_history(req) {
    ...
    const since =
        Math.ceil(req.rpc_params.since / monitor_interval) * monitor_interval;
    const till =
        Math.floor(
            Math.min(req.rpc_params.till || Infinity, Date.now()) /
                monitor_interval,
        ) * monitor_interval;
    ...
    const [group_reports, bandwidth_reports] = await Promise.all([
        EndpointStatsStore.instance.get_endpoint_group_reports({
            since,
            till,
            groups,
        }),
        EndpointStatsStore.instance.get_bandwidth_reports({
            since,
            till,
    ...
    for (let start_time = since; start_time < till; start_time += stepInMili) {
        ...
        for (; i < group_reports.length; ++i) {
            ...
            const memory_usage =
                _.sumBy(
                    endpoints,
                    ep_info => ep_info.memory.used / ep_info.memory.total,
                ) / endpoints.length;
            acc.count += 1;
        ...
        for (const acc of by_groups.values()) {
            ...
            bin.memory_usage +=
                acc.memory_usage / desired_report_count / by_groups.size;
...
function _list_s3_addresses(system) {
    ...
    try {
        return system.system_address
            .filter(
                addr =>
                    addr.service === 's3' && addr.api === 's3' && addr.secure,
            )
            .sort((addr1, addr2) => {
            ...
            .map(addr => {
                ...
                const url = url_utils.construct_url({
                    protocol: 'https',
                    hostname,
                    port,
                });
...
async function _get_endpoint_groups() {
    ...
    const till =
        Math.floor(Date.now() / config.ENDPOINT_MONITOR_INTERVAL) *
        config.ENDPOINT_MONITOR_INTERVAL;
    const reports =
        await EndpointStatsStore.instance.get_endpoint_group_reports({
    ...
    return endpoint_groups.map(group => {
        const { end_time = -1, endpoints = [] } =
            reports_by_group[group.name] || {};
...
async function rotate_master_key(req) {
    ...
    if (!entity_info) {
        throw new Error(`rotate_master_key: entity doesn't exist ${entity}`);
    }
    ...
    if (!old_master_key || old_master_key.disabled === true) {
        throw new Error(
            `rotate_master_key: can not rotate master key, current master key is ${old_master_key}`,
        );
    }
    ...
    const mastr_key_id =
        (
            system_store.master_key_manager.is_root_key(
                old_master_key.master_key_id,
            )
        ) ?
            root_key_id
    ...
    const new_master_key = system_store.master_key_manager.new_master_key(
        master_key_base_props,
    );
    ...
    const update_type =
        (entity_type === 'ACCOUNT' && { accounts: change }) ||
    ...
    if (entity_type === 'SYSTEM') {
        await P.all(
            _.map(system_store.data.buckets, async function (bucket) {
                const reencrypted = mkm._reencrypt_master_key(
                    bucket.master_key_id._id,
                    new_master_key._id,
                );
                await upsert_master_key({
                    ...
                    update: {
                        ...
                        master_key_id: new_master_key._id,
                    },
                });
            }),
        );
        await P.all(
            _.map(system_store.data.accounts, async function (account) {
    ...
    if (entity_type === 'ACCOUNT') {
        const new_s3_secret_keys_list = _.map(
            entity_info.access_keys,
            function (creds) {
                creds.secret_key =
                    mkm.encrypt_sensitive_string_with_master_key_id(
                        creds.secret_key,
                        new_master_key._id,
                    );
                return creds;
        ...
        const new_sync_creds_list = _.map(
            entity_info.sync_credentials_cache,
            function (creds) {
                creds.secret_key =
                    mkm.encrypt_sensitive_string_with_master_key_id(
                        creds.secret_key,
        ...
        const pools_ns_resource_updates = get_pools_and_ns_resources_changes(
            new_sync_creds_list,
            entity_info._id,
        );
        await update_account_secrets(
            entity_info._id,
            new_s3_secret_keys_list,
            new_sync_creds_list,
            pools_ns_resource_updates,
        );
...
async function _disable_master_key(entity, entity_type) {
    const entity_info = get_entity_info(entity, entity_type);
    if (!entity_info) {
        throw new Error(`_disable_master_key: entity doesn't exist ${entity}`);
    }

    if (
        !entity_info.master_key_id ||
        entity_info.master_key_id.disabled === true
    ) {
        throw new Error(
            `_disable_master_key: can not disable master key, current master key is: ${util.inspect(entity_info)}`,
        );
    }

    await upsert_master_key({
        _id: entity_info.master_key_id._id,
        update: { disabled: true },
    });
    system_store.master_key_manager.set_m_key_disabled_val(
        entity_info.master_key_id._id,
        true,
    );
...
async function enable_master_key(req) {
    ...
    const entity_info = get_entity_info(entity, entity_type);
    if (!entity_info) {
        throw new Error(`_enable_master_key: entity doesn't exist ${entity}`);
    }

    if (
        !entity_info.master_key_id ||
        entity_info.master_key_id.disabled === false
    ) {
        throw new Error(
            `_enable_master_key: can not enable master key, current master key is: ${entity_info.master_key_id}`,
        );
    }
...
async function update_account_secrets(
    account_id,
    access_keys,
...
function get_entity_info(entity, entity_type) {
    return (
        (entity_type === 'ACCOUNT' &&
            system_store.data.accounts.find(
                acc => acc.email.unwrap() === entity.unwrap(),
            )) ||
        (entity_type === 'BUCKET' &&
            system_store.data.buckets.find(
                bkt => bkt.name.unwrap() === entity.unwrap(),
            )) ||
        (entity_type === 'SYSTEM' &&
            system_store.data.systems.find(
                sys => sys.name === entity.unwrap(),
            )) ||
        undefined
    );
}
...
async function upgrade_master_keys() {
    ...
    _.map(system_store.data.accounts, account => {
        ...
        // encrypt sync creds
        const encrypted_sync_creds = _.map(
        ...
        const pool_and_ns_resources_updates =
            get_pools_and_ns_resources_changes(
                encrypted_sync_creds,
                account._id,
            );
        ...
        namespace_resources_updates.push(
            ...pool_and_ns_resources_updates.namespace_resources,
        );
