...
function new_system_changes(req, name, owner_account_id) {
  ...
  const default_chunk_config =
    config.CHUNK_CODER_EC_IS_DEFAULT ? ec_chunk_config : regular_chunk_config;
  const tier_mirrors = [
    {
      _id: system_store.new_system_store_id(),
...
async function read_system(req) {
  ...
  const dns_name =
    net.isIP(base_address.hostname) === 0 ? base_address.hostname : undefined;
...
async function update_endpoint_group(req) {
  ...
  if (exists) {
    ...
    if (
      ...
      dbg.log0('updating endpoint group. old values: ', group, ' new values:', {
        name: group_name,
        is_remote,
        region,
        endpoint_range,
...
async function get_endpoints_history(req) {
  ...
  const till =
    Math.floor(
      Math.min(req.rpc_params.till || Infinity, Date.now()) / monitor_interval,
    ) * monitor_interval;
...
function _list_s3_addresses(system) {
  ...
  return system.system_address
    .filter(addr => addr.service === 's3' && addr.api === 's3' && addr.secure)
    .sort((addr1, addr2) => {
...
async function rotate_master_key(req) {
  ...
  const mastr_key_id =
    system_store.master_key_manager.is_root_key(old_master_key.master_key_id) ?
      root_key_id
  ...
  if (entity_type === 'ACCOUNT') {
    const new_s3_secret_keys_list = _.map(
      ...
      function (creds) {
        creds.secret_key = mkm.encrypt_sensitive_string_with_master_key_id(
          creds.secret_key,
          new_master_key._id,
        );
        return creds;
...
function get_entity_info(entity, entity_type) {
  return (
    ...
    (entity_type === 'SYSTEM' &&
      system_store.data.systems.find(sys => sys.name === entity.unwrap())) ||
    undefined
  );
}
...
async function upgrade_master_keys() {
  ...
  _.map(system_store.data.accounts, account => {
    ...
    const pool_and_ns_resources_updates = get_pools_and_ns_resources_changes(
