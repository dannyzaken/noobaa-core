...
class NodesMonitor extends EventEmitter {
  ...
  async sync_storage_to_store() {
    ...
    for (const item of items) {
      ...
      const available_capacity =
        pool && pool.cloud_pool_info && pool.cloud_pool_info.available_capacity;
  ...
  _add_new_node(conn, system_id, agent_config, pool_name) {
    ...
    const pool =
      ...
      _.filter(
        ...
        p => !_.get(p, 'mongo_pool_info') && !_.get(p, 'cloud_pool_info'),
  ...
  _handle_issues(item) {
    ...
    if (
      _.some(item.node.issues_report, issue => issue.reason === 'TAMPERING') &&
      !_.some(
  ...
  _update_node_service(item) {
    ...
    const service_enabled_not_changed =
      (item.node.enabled && should_enable) ||
      (!item.node.enabled && !should_enable);
  ...
  _get_item_issues_reasons(item) {
    try {
      ...
      if (item.storage_full) {
        reasons.push(`storage is full (${new Date(item.storage_full)})`);
      }
      ...
      if (item.node.deleted) {
        reasons.push(`node in deleted state (${new Date(item.node.deleted)})`);
      }
  ...
  _update_data_activity_progress(item, now) {
    ...
    if (act.stage && !_.isEmpty(act.stage.size)) {
      act.stage.size.remaining =
        Math.max(0, act.stage.size.total - act.stage.size.completed) || 0;
  ...
  _filter_hosts(query) {
    ...
    for (const host of hosts) {
      ...
      if (item.node.node_type !== 'BLOCK_STORE_FS' && !query.include_all) {
        continue;
      }
      if (
        query.hosts &&
  ...
  _sort_nodes_list(list, options) {
    ...
    if (options.sort === 'name') {
      list.sort(
        js_utils.sort_compare_by(item => String(item.node.name), options.order),
      );
      ...
      list.sort(
        js_utils.sort_compare_by(item => Boolean(item.online), options.order),
      );
    } else if (options.sort === 'trusted') {
      list.sort(
        js_utils.sort_compare_by(item => Boolean(item.trusted), options.order),
      );
      ...
      list.sort(
        js_utils.sort_compare_by(item => item.accessibility, options.order),
      );
    } else if (options.sort === 'connectivity') {
      list.sort(
        js_utils.sort_compare_by(item => item.connectivity, options.order),
      );
  ...
  async _get_nodes_stats_by_service(
    ...
    for (const stat of deleted_stats) {
      ...
      if (!pool) {
        const deleted_pool = await system_store.data.get_by_id_include_deleted(
          node.pool,
          'pools',
        );
  ...
  aggregate_hosts(query, group_by) {
    ...
    if (group_by) {
      if (group_by === 'pool') {
        const pool_groups = _.groupBy(list, item => String(item.node.pool));
  ...
  _is_host_hideable(host_nodes) {
    ...
    if (
      _.some(
        ...
        node_stage =>
          node_stage === STAGE_OFFLINE_GRACE || node_stage === STAGE_REBUILDING,
      )
    ) {
      return false;
    }
  ...
  get_test_nodes(req) {
    ...
    return _.map(list_res.nodes, node => _.pick(node, 'name', 'rpc_address'));
  }
  ...
  allocate_nodes(params) {
    ...
    const partition_avg_disk_write = _.partition(
      list,
      item =>
        !Number.isNaN(item.avg_disk_write) && _.isNumber(item.avg_disk_write),
    );
    const nodes_with_avg_disk_write = partition_avg_disk_write[0];
    const nodes_without_avg_disk_write = partition_avg_disk_write[1];
    if (
      nodes_with_avg_disk_write.length >= config.NODE_ALLOCATOR_NUM_CLUSTERS
    ) {
      // TODO:
...
function progress_by_time(time, now) {
  ...
  return Math.min(1, Math.max(0, (now - time.start) / (time.end - time.start)));
}
