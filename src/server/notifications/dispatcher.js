...
const ActivityLogStore =
  require('../analytic_services/activity_log_store').ActivityLogStore;
...
class Dispatcher {
    ...
    return ActivityLogStore.instance()
      ...
      .then(logs =>
        P.map(logs, function (log_item) {
          ...
          return P.resolve(self._resolve_activity_item(log_item, l)).then(
            () => l,
          );
        }),
      )
    ...
    return P.resolve()
      ...
      .then(should_alert => {
        ...
        if (should_alert) {
          ...
          return AlertsLogStore.instance()
            .create({
              system: sysid,
              severity: sev,
  ...
  update_alerts_state(req) {
    ...
    return AlertsLogStore.instance().update_alerts_state(
      req.system._id,
      query,
      state,
    );
  ...
  read_alerts(req) {
    ...
    return AlertsLogStore.instance()
      .read_alerts(req.system._id, query, skip, limit)
      .then(alerts =>
        P.map(alerts, function (alert_item) {
          return {
  ...
  _resolve_activity_item(log_item, l) {
    return P.resolve()
      .then(() =>
        nodes_client
          ...
          .populate_nodes(
            ...
            'node',
            'node',
            NODE_POPULATE_FIELDS,
          ),
      )
      .then(() =>
        MDStore.instance().populate_objects(
          log_item,
          'obj',
          OBJECT_POPULATE_FIELDS,
        ),
      )
      .then(async () => {
        if (log_item.node) {
        ...
        return P.resolve(
          log_item.node &&
            !l.node.linkable &&
            nodes_store.get_hidden_by_id(log_item.node),
        );
      })
      .then(async node => {
        if (node) {
        ...
        return P.resolve(
          log_item.tier &&
            system_store.data.get_by_id_include_deleted(log_item.tier, 'tiers'),
        );
      })
      .then(tier => {
        if (tier) {
        ...
        return P.resolve(
          log_item.bucket &&
            system_store.data.get_by_id_include_deleted(
              log_item.bucket,
              'buckets',
            ),
        );
      ...
      .then(bucket => {
        ...
        return P.resolve(
          log_item.pool &&
            system_store.data.get_by_id_include_deleted(log_item.pool, 'pools'),
        );
      })
      .then(pool => {
        if (pool) {
      ...
      .then(account => {
        ...
        return P.resolve(
          log_item.func &&
            func_store.get_by_id_include_deleted(log_item.func, 'funcs'),
        );
      })
      .then(func => {
        if (func) {
