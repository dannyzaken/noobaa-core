...
async function load_system_store(req) {
  await system_store.load(req && req.rpc_params && req.rpc_params.since);
}
...
function update_mongo_connection_string(req) {
  ...
  dotenv.load();
  dbg.log0(
    'Recieved update mongo string. will update mongo url from',
    old_url,
    ' to ',
...
function update_master_change(req) {
  ...
  if (req.rpc_params.master_address) {
    ...
    if (old_master_address.indexOf(new_master_address) === -1) {
      dbg.log0(
        `master changed from ${old_master_address} to ${new_master_address}. updating server_rpc`,
      );
