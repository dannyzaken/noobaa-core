...
class MDStore {
    ...
    async populate_objects(docs, doc_path, fields) {
        return db_client
            .instance()
            .populate(docs, doc_path, this._objects, fields);
    }
    ...
    async remove_object_move_latest(old_latest_obj, new_latest_obj) {
        ...
        bulk.find({ _id: old_latest_obj._id, deleted: null }).updateOne({
            $set: { deleted: new Date(), version_past: true },
        });
        bulk.find({ _id: new_latest_obj._id, deleted: null }).updateOne({
            $unset: { version_past: true },
        });
        const res = await bulk.execute();
        if (!res.ok || res.nMatched !== 2 || res.nModified !== 2) {
            dbg.error(
                'remove_object_move_latest: partial bulk update',
                _.clone(res),
                old_latest_obj,
                new_latest_obj,
            );
    ...
    async insert_object_delete_marker(obj) {
        const delete_marker = await this._delete_marker_from_obj(
            obj,
            obj.version_enabled,
        );
    ...
    async insert_object_delete_marker_move_latest(obj, version_enabled) {
        ...
        const bulk = this._objects.initializeOrderedBulkOp();
        bulk.find({ _id: obj._id, deleted: null }).updateOne({
            $set: { version_past: true },
        });
        ...
        if (
            ...
            dbg.error(
                'insert_object_delete_marker_move_latest: partial bulk update',
                _.clone(res),
                obj,
                delete_marker,
            );
    ...
    async complete_object_upload_latest_mark_remove_current({
        ...
        const bulk = this._objects.initializeOrderedBulkOp();
        bulk.find({ _id: unmark_obj._id, deleted: null }).updateOne({
            $set: { version_past: true },
        });
        bulk.find({ _id: put_obj._id, deleted: null }).updateOne({
            $set: set_updates,
            $unset: unset_updates,
        });
        const res = await bulk.execute();
        if (!res.ok || res.nMatched !== 2 || res.nModified !== 2) {
            dbg.error(
                'complete_object_upload_latest_mark_remove_current: partial bulk update',
                _.clone(res),
                unmark_obj,
                put_obj,
                set_updates,
    ...
    async complete_object_upload_latest_mark_remove_current_and_delete({
        ...
        const bulk = this._objects.initializeOrderedBulkOp();
        if (delete_obj) {
            bulk.find({ _id: delete_obj._id, deleted: null }).updateOne({
        ...
        if (
            !res.ok ||
            res.nMatched !== number_of_queries ||
            res.nModified !== number_of_queries
        ) {
            dbg.error(
                'complete_object_upload_latest_mark_remove_current_and_delete: partial bulk update',
                _.clone(res),
                unmark_obj,
                put_obj,
                set_updates,
    ...
    async insert_object_delete_marker_move_latest_with_delete(obj, latest_obj) {
        ...
        let num_updates = 0;
        if (latest_obj) {
            bulk.find({ _id: latest_obj._id, deleted: null }).updateOne({
                $set: { version_past: true },
            });
            bulk.find({ _id: obj._id, deleted: null }).updateOne({
                $set: { deleted: new Date() },
        ...
        if (
            !res.ok ||
            res.nMatched !== num_updates ||
            res.nModified !== num_updates ||
            res.nInserted !== 1
        ) {
            dbg.error(
                'insert_object_delete_marker_move_latest_with_delete: partial bulk update',
                _.clone(res),
                latest_obj,
                obj,
                delete_marker,
    ...
    async find_objects({
        ...
        const query = compact({
            ...
            create_time:
                max_create_time ?
                    {
                        $lt: new Date(
                            moment.unix(max_create_time).toISOString(),
                        ),
                        $exists: true,
                    }
                :   undefined,
        ...
        const [objects, non_paginated, completed, uploading] =
            await Promise.all([
    ...
    async list_objects({ bucket_id, delimiter, prefix, key_marker, limit }) {
    ...
    _build_list_key_query_from_prefix(prefix) {
        ...
        return prefix ?
                { key_query: new RegExp('^' + _.escapeRegExp(prefix)) }
            :   {};
    }

    _build_list_key_query_from_markers(
        prefix,
        delimiter,
        key_marker,
        upload_started_marker,
        ...
        if (!key_marker.startsWith(prefix)) {
            throw new Error(
                `BAD KEY MARKER ${key_marker} FOR PREFIX ${prefix}`,
            );
        }
    ...
    _get_unique_latest_keys(keys) {
        return _.compact(
            _.map(keys, key => (key.obj ? !key.obj.version_past && key : key)),
        );
    }
    ...
    async _aggregate_objects_internal(query) {
        ...
        _.forEach(res, r => {
            r._id.reduce((o, s, i, arr) => {
                o[s] = i < arr.length - 1 ? o[s] || {} : (o[s] = r.value);
                return o[s];
            }, buckets);
        });
        return buckets;
    ...
    update_multipart_by_id(multipart_id, set_updates) {
        return this._multiparts
            ...
            .then(res =>
                db_client.instance().check_update_one(res, 'multipart'),
            );
    }
    ...
    find_multipart_by_id(multipart_id) {
        return this._multiparts
            ...
            .then(obj =>
                db_client.instance().check_entity_not_deleted(obj, 'multipart'),
            );
    }
    ...
    async find_parts_unreferenced_chunk_ids(chunk_ids) {
        return this._parts
            ...
            .then(parts => {
                const referenced_chunks_ids = db_client
                    .instance()
                    .uniq_ids(parts, 'chunk');
                const unreferenced_chunks_ids = db_client
                    .instance()
                    .obj_ids_difference(chunk_ids, referenced_chunks_ids);
                dbg.log0(
                    'find_object_parts_unreferenced_chunk_ids:',
                    'chunk_ids',
                    chunk_ids.length,
                    'referenced_chunks_ids',
                    referenced_chunks_ids.length,
                    'unreferenced_chunks_ids',
                    unreferenced_chunks_ids.length,
                );
    ...
    async load_parts_objects_for_chunks(chunks) {
        ...
        const parts = await this._parts.find({
            chunk: {
                $in: db_client.instance().uniq_ids(chunks, '_id'),
                $exists: true,
            },
            deleted: null,
        });
        const objects = await this._objects.find({
            _id: {
                $in: db_client.instance().uniq_ids(parts, 'obj'),
                $exists: true,
            },
        });
        ...
        for (const chunk of chunks) {
            ...
            chunk.objects = _.uniq(
                _.compact(
                    _.map(
                        chunk.parts,
                        part => objects_by_id[part.obj.toHexString()],
                    ),
                ),
            );
        }
    ...
    update_parts_in_bulk(parts_updates) {
        ...
        for (const update of parts_updates) {
            bulk.find({ _id: update._id }).updateOne(
                compact_updates(update.set_updates, update.unset_updates),
            );
        }
    ...
    populate_chunks(docs, doc_path, fields) {
        return db_client
            .instance()
            .populate(docs, doc_path, this._chunks, fields);
    }
    ...
    async delete_all_chunks_in_system() {
        assert(
            config.test_mode,
            'This function should be called only in test mode!',
        );
        const delete_date = new Date();
        return this._chunks.updateMany(
            {},
    ...
    get_dedup_index_size() {
        return this._chunks.stats().then(res => res.indexSizes.dedup_key_1);
    }
    ...
    get_aprox_dedup_keys_number() {
        ...
        ]).then(([total_count, sample_count]) =>
            sample_count ? sample_count : total_count,
        );
    }
    ...
    iterate_indexed_chunks(limit, marker) {
        return this._chunks
            ...
            .then(chunks => ({
                chunk_ids: db_client.instance().uniq_ids(chunks, '_id'),
                marker:
                    chunks.length ? chunks[chunks.length - 1].dedup_key : null,
            }));
    }
    ...
    async load_blocks_for_chunks(chunks, sorter) {
        ...
        for (const chunk of chunks) {
            const blocks_by_frag = _.groupBy(
                blocks_by_chunk[chunk._id.toHexString()],
                'frag',
            );
            for (const frag of chunk.frags) {
                const frag_blocks =
                    blocks_by_frag[frag._id.toHexString()] || [];
