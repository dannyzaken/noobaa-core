...
class MDStore {
  ...
  async remove_object_move_latest(old_latest_obj, new_latest_obj) {
    ...
    bulk
      .find({ _id: old_latest_obj._id, deleted: null })
      .updateOne({ $set: { deleted: new Date(), version_past: true } });
    bulk
      .find({ _id: new_latest_obj._id, deleted: null })
  ...
  async _delete_marker_from_obj(obj, version_enabled) {
    ...
    const delete_marker = {
      ...
      key: obj.key,
      content_type:
        obj.content_type || mime.getType(obj.key) || 'application/octet-stream',
      delete_marker: true,
  ...
  async complete_object_upload_latest_mark_remove_current({
    ...
    const bulk = this._objects.initializeOrderedBulkOp();
    bulk
      .find({ _id: unmark_obj._id, deleted: null })
      .updateOne({ $set: { version_past: true } });
    bulk
      .find({ _id: put_obj._id, deleted: null })
  ...
  async complete_object_upload_latest_mark_remove_current_and_delete({
    ...
    if (delete_obj) {
      bulk
        .find({ _id: delete_obj._id, deleted: null })
  ...
  async insert_object_delete_marker_move_latest_with_delete(obj, latest_obj) {
    ...
    if (latest_obj) {
      bulk
        .find({ _id: latest_obj._id, deleted: null })
        .updateOne({ $set: { version_past: true } });
      bulk
        .find({ _id: obj._id, deleted: null })
        .updateOne({ $set: { deleted: new Date() } });
  ...
  async load_parts_objects_for_chunks(chunks) {
    ...
    for (const chunk of chunks) {
      ...
      chunk.objects = _.uniq(
        _.compact(
          _.map(chunk.parts, part => objects_by_id[part.obj.toHexString()]),
        ),
      );
    }
  }
  ...
  iterate_indexed_chunks(limit, marker) {
    return this._chunks
      .find(
        {
          dedup_key:
            marker ? { $lt: marker, $exists: true } : { $exists: true },
        },
        {
          projection: {
            _id: 1,
