...
async function read_parts_mapping(parts, location_info) {
    ...
    const sorter =
        location_info ?
            _block_sorter_local(location_info)
        :   _block_sorter_basic;
...
async function update_chunks_on_read(chunks, location_info) {
    ...
    try {
        ...
        for (const chunk of chunks) {
            if (
                (!chunk.tier._id ||
                    !_.isEqual(chunk.tier._id, selected_tier._id)) &&
                map_server.enough_room_in_tier(selected_tier, bucket)
            ) {
                dbg.log0(
                    'Chunk with low tier will be sent for rebuilding',
                    chunk._id,
                );
...
function _block_sorter_local(location_info) {
    ...
    function sort_func(block1, block2) {
        ...
        if (
            String(block2.pool) === pool_id &&
            String(block1.pool) !== pool_id
        ) {
            return 1;
        }
        if (
        ...
        if (region) {
            if (
                block2.pool.region === region &&
                block1.pool.region !== region
            ) {
                return 1;
            }
            if (
