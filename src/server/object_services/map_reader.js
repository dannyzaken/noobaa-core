...
async function read_node_mapping(node_ids, skip, limit) {
  const chunk_ids = await MDStore.instance().find_blocks_chunks_by_node_ids(
    node_ids,
    skip,
    limit,
  );
  const parts = await MDStore.instance().find_parts_by_chunk_ids(chunk_ids);
  const chunks = await read_parts_mapping(
    _.uniqBy(parts, part => part.chunk.toHexString()),
  );
  return chunks;
}
...
async function read_parts_mapping(parts, location_info) {
  const chunks_db = await MDStore.instance().find_chunks_by_ids(
    db_client.instance().uniq_ids(parts, 'chunk'),
  );
  const sorter =
    location_info ? _block_sorter_local(location_info) : _block_sorter_basic;
  await MDStore.instance().load_blocks_for_chunks(chunks_db, sorter);
  const chunks_db_by_id = _.keyBy(chunks_db, '_id');
  const chunks = parts.map(part => {
    const chunk = new ChunkDB({
      ...chunks_db_by_id[part.chunk.toHexString()],
      parts: [part],
    });
    return chunk;
  });
...
async function update_chunks_on_read(chunks, location_info) {
  ...
  try {
    ...
    for (const chunk of chunks) {
      if (
        (!chunk.tier._id || !_.isEqual(chunk.tier._id, selected_tier._id)) &&
        ...
        const mirror = await map_server.select_mirror_for_write(
          chunk.tier,
          bucket.tiering,
          location_info,
        );
        if (
          ...
          dbg.log2(
            'Chunk with following mapping will be sent for rebuilding',
            chunk,
          );
          chunks_to_scrub.push(chunk);
        }
    ...
    if (chunks_to_scrub.length) {
      ...
      dbg.log1(
        "Chunks wasn't found in local pool/upper tier - the following will be rebuilt:",
        util.inspect(chunks_to_scrub),
      );
...
function _block_sorter_local(location_info) {
  ...
  function sort_func(block1, block2) {
    ...
    if (node2.readable && !node1.readable) return 1;
    if (node1.readable && !node2.readable) return -1;
    if (String(node2._id) === node_id && String(node1._id) !== node_id) {
      return 1;
    }
    if (String(node1._id) === node_id && String(node2._id) !== node_id) {
      return -1;
    }
    if (node2.host_id === host_id && node1.host_id !== host_id) return 1;
    if (node1.host_id === host_id && node2.host_id !== host_id) return -1;
    if (String(block2.pool) === pool_id && String(block1.pool) !== pool_id) {
      return 1;
    }
    if (String(block1.pool) === pool_id && String(block2.pool) !== pool_id) {
      return -1;
    }
    if (region) {
      if (block2.pool.region === region && block1.pool.region !== region) {
        return 1;
      }
      if (block1.pool.region === region && block2.pool.region !== region) {
        return -1;
      }
    }
    return node2.heartbeat - node1.heartbeat;
