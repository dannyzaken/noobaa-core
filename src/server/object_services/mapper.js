...
function select_mirror_for_write(tier, tiering, tiering_status, location_info) {
  ...
  for (const mirror of tier.mirrors) {
    ...
    const is_local_pool_valid =
      local_pool &&
      tier_status.pools[local_pool._id.toHexString()].valid_for_allocation;
...
function map_chunk(chunk, tier, tiering, tiering_status, location_info) {
  ...
  const mirror_for_write =
    is_new_chunk ?
      select_mirror_for_write(tier, tiering, tiering_status, location_info)
    : undefined;
    ...
    function map_frag_in_mirror(mirror) {
      ...
      for (const block of accessible_blocks) {
        ...
        const block_pool_in_mirror = mirror.spread_pools.find(
          pool => pool._id.toHexString() === block.pool_id.toHexString(),
        );
      ...
      if (used_replicas === replicas) {
        ...
        const pools =
          used_replicas && !used_redundant_blocks && is_regular_pools_valid ?
            regular_pools
          : pick_pools();
...
function _block_sort_newer_first(block1, block2) {
  return (
    block2._id.getTimestamp().getTime() - block1._id.getTimestamp().getTime()
  );
}
