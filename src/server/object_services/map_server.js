...
class GetMapping {
      ...
      Object.values(this.chunks_per_bucket).map(async chunks => {
        ...
        const dedup_keys = _.compact(
          _.map(
            chunks,
            chunk =>
              chunk.digest_b64 && Buffer.from(chunk.digest_b64, 'base64'),
          ),
        );
        if (!dedup_keys.length) return;
        dbg.log0('GetMapping.find_dups: found keys', dedup_keys.length);
        const dup_chunks_db = await MDStore.instance().find_chunks_by_dedup_key(
          bucket,
          dedup_keys,
        );
  ...
  async allocate_chunk(chunk) {
    const avoid_blocks = _.flatMap(chunk.frags, frag =>
      frag.blocks.filter(block => block.node.node_type === 'BLOCK_STORE_FS'),
    );
    ...
    for (const frag of chunk.frags) {
      ...
      frag.allocations.sort(
        (alloc1, alloc2) => alloc2.locality_level - alloc1.locality_level,
      );
    }
...
class PutMapping {
  ...
  add_new_block(block_md, frag, chunk) {
    ...
    if (
      block_id_time <
      now - (config.MD_GRACE_IN_MILLISECONDS - config.MD_AGGREGATOR_INTERVAL)
    ) {
      dbg.error(
