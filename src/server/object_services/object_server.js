...
async function create_object_upload(req) {
  ...
  const tier =
    req.bucket.tiering && (await map_server.select_tier_for_write(req.bucket));
  if (
    tier &&
    tier.storage_class &&
...
async function complete_object_upload(req) {
  ...
  if (req.rpc_params.size !== map_res.size) {
    if (req.rpc_params.size >= 0) {
      throw new RpcError(
        'BAD_SIZE',
        `size on complete object (${req.rpc_params.size}) differs from parts (${
          map_res.size
        })`,
      );
    }
  ...
  if (req.rpc_params.etag) {
    ...
    set_updates.etag = Buffer.from(req.rpc_params.md5_b64, 'base64').toString(
      'hex',
    );
  }
...
async function list_object_versions(req) {
  ...
  if (key_marker && req.rpc_params.version_id_marker) {
    ...
    if (req.rpc_params.version_id_marker === 'null') {
      const null_version = await MDStore.instance().find_object_null_version(
        req.bucket._id,
        key_marker,
      );
      version_seq_marker =
...
async function list_uploads(req) {
  ...
  return {
    ...
    next_upload_id_marker:
      (state.is_truncated && String(state.upload_started_marker)) || undefined,
  };
}
...
async function list_objects_admin(req) {
  ...
  if (!objects_info.length) {
    if (key) {
      ...
      const has_uploads = await MDStore.instance().has_any_uploads_for_bucket(
        req.bucket._id,
      );
      if (has_uploads) {
      ...
      const has_objects = await MDStore.instance().has_any_objects_for_bucket(
        req.bucket._id,
...
async function add_endpoint_report(req) {
  ...
  const bandwidth = (params.bandwidth || []).map(record => {
    const account = system_store.data.accounts.find(acc => {
      const access_key =
        acc.access_keys && acc.access_keys[0] && acc.access_keys[0].access_key;
      ...
      return P.resolve()
        .then(() => {
          if (params.node_id) {
            return nodes_client
              .instance()
              .read_node_by_id(req.system && req.system._id, params.node_id);
          }
        })
        .then(node => {
          const endpoint_name =
...
function get_object_info(md, options = {}) {
  ...
  return {
    ...
    cache_last_valid_time:
      md.cache_last_valid_time ? md.cache_last_valid_time.getTime() : undefined,
    upload_started:
      md.upload_started ?
...
async function _delete_object_version(req) {
  ...
  if (bucket_versioning === 'ENABLED' || bucket_versioning === 'SUSPENDED') {
    ...
    if (obj.version_past) {
      ...
      const prev_version = await MDStore.instance().find_object_prev_version(
        req.bucket._id,
        req.rpc_params.key,
      );
      if (prev_version) {
