...
async function delete_chunks_if_unreferenced(chunk_ids) {
  ...
  const unreferenced_chunk_ids =
    await MDStore.instance().find_parts_unreferenced_chunk_ids(chunk_ids);
  if (unreferenced_chunk_ids.length) {
    const chunks_db = await MDStore.instance().find_chunks_by_ids(
      unreferenced_chunk_ids,
    );
...
async function delete_chunks(chunks) {
  ...
  const blocks = get_all_chunks_blocks(chunks).filter(
    block => !block.to_db().deleted,
  );
...
async function delete_blocks(blocks) {
  ...
  try {
    ...
    await MDStore.instance().delete_blocks_by_ids(
      db_client.instance().uniq_ids(blocks, '_id'),
    );
...
async function delete_parts_by_chunks(chunks) {
  if (!chunks || !chunks.length) return;
  const parts = get_all_chunk_parts(chunks).filter(
    part => !part.to_db().deleted,
  );
...
async function delete_blocks_from_nodes(blocks) {
  ...
  try {
    const blocks_by_node = _.values(
      _.groupBy(blocks, block => String(block.node._id)),
    );
    const succeeded_block_ids = await Promise.all(
      blocks_by_node.map(delete_blocks_from_node),
    );
    const block_ids = _.flatten(succeeded_block_ids).map(block_id =>
      db_client.instance().parse_object_id(block_id),
    );
    ...
    await MDStore.instance().update_blocks_by_ids(block_ids, {
      reclaimed: new Date(),
    });
  } catch (err) {
    dbg.warn(
      'delete_blocks_from_nodes: Failed to mark blocks as reclaimed',
      err,
    );
  }
}
...
async function delete_blocks_from_node(blocks) {
  ...
  if (!node.rpc_address) {
    dbg.warn(
      'delete_blocks_from_node: no address for node',
      node._id,
      node.name,
      'block_ids',
      block_ids.length,
  ...
  try {
    const res = await server_rpc.client.block_store.delete_blocks(
      { block_ids },
      {
    ...
    if (res.succeeded_block_ids && res.succeeded_block_ids.length) {
      dbg.log0(
        ...
        node.rpc_address,
        'succeeded_block_ids',
        res.succeeded_block_ids.length,
      );
    }
    return res.succeeded_block_ids || [];
  } catch (err) {
    dbg.warn(
      'delete_blocks_from_node: ERROR node',
      node._id,
      node.rpc_address,
      'block_ids',
      block_ids.length,
