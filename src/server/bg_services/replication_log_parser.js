 ...
 * }>} Candidates
 */
async function get_log_candidates(
    source_bucket_id,
    rule_id,
    replication_config,
    candidates_limit,
    ...
    const endpoint_type =
        source_bucket.namespace.write_resource.resource.connection
            .endpoint_type;
    if (endpoint_type === 'AWS') {
        return get_aws_log_candidates(
            source_bucket_id,
            rule_id,
            replication_config,
            candidates_limit,
        ...
        return get_azure_log_candidates(
            source_bucket_id,
            rule_id,
            replication_config,
            candidates_limit,
        ...
        throw new Error(
            `REPLICATION_LOG_PARSER: Unsupported endpoint type ${endpoint_type}`,
        );
    }
}

async function get_aws_log_candidates(
    source_bucket_id,
    rule_id,
    replication_config,
    candidates_limit,
    ...
    const aws_log_replication_info =
        replication_config.log_replication_info.aws_log_replication_info;
    const obj_prefix_filter = _get_obj_prefix_filter_for_rule(
        rule_id,
        replication_config,
    );
    const { logs_bucket, prefix } = aws_log_replication_info.logs_location;
    const s3 = _get_source_bucket_aws_connection(
        source_bucket_id,
        aws_log_replication_info,
    );
    let log_object_continuation_token =
        _get_log_object_continuation_token_for_rule(
            rule_id,
            replication_config,
        );
    ...
    do {
        const next_log_entry = await aws_get_next_log_entry(
            s3,
            logs_bucket,
            prefix,
        ...
        const next_log_data = await _aws_get_next_log(
            s3,
            logs_bucket,
            next_log_entry.Contents[0].Key,
        );
        aws_parse_log_object(
            logs,
            next_log_data,
            sync_deletions,
            obj_prefix_filter,
        ...
        logs.length < candidates_limit &&
        logs_retrieved_count !== 0 &&
        log_object_continuation_token
    );
        ...
        items: create_candidates(logs),
            ...
            if (log_object_continuation_token) {
                await replication_store.update_log_replication_marker_by_id(
                    replication_config._id,
                    rule_id,
                    { continuation_token: log_object_continuation_token },
                );
            }
        },
    };
...
async function get_azure_log_candidates(
    source_bucket_id,
    rule_id,
    replication_config,
    candidates_limit,
    ...
    const obj_prefix_filter =
        _get_obj_prefix_filter_for_rule(rule_id, replication_config) || '';
    const { logs_query_client, monitor_workspace_id } =
        _get_source_bucket_azure_connection(source_bucket_id);
    let candidates;

    // Our "continuation token" is the timestamp of the the last log retrieval
    let continuation_token = _get_log_object_continuation_token_for_rule(
        rule_id,
        replication_config,
    );
    ...
    const kusto_query = `set truncationmaxsize=${config.AZURE_QUERY_TRUNCATION_MAX_SIZE_IN_BITS};
    StorageBlobLogs
    ...
    if (query_result.status === LogsQueryResultStatus.Success) {
        ...
        if (tables_from_result && tables_from_result[0].rows.length > 0) {
            ...
            continuation_token = (
                result_rows[result_rows.length - 1][0].getTime() + 1
            ).toString();
            ...
            dbg.log1(
                'get_azure_log_candidates: No new Azure logs found to process',
            );
        }
        if (tables_from_result.length === 0) {
            dbg.log1(
                `get_azure_log_candidates: No results for Azure replication query '${kusto_query}'`,
            );
            return;
        }
    } else {
        dbg.error(
            `get_azure_log_candidates: Error processing the Azure replication query '${kusto_query}' - ${query_result.partialError}`,
        );
        if (query_result.partialTables.length > 0) {
            console.warn(
                `get_azure_log_candidates: The Azure replication query has also returned partial data in the following table(s) - ${query_result.partialTables}`,
            );
        }
    }
...
function create_candidates(logs) {
    ...
    for (const log of logs) {
        ...
        if (
            logs_for_key_time_map.has(log_time) &&
            logs_for_key_time_map.get(log_time).action !== log.action
        ) {
...
async function aws_get_next_log_entry(
    s3,
    logs_bucket,
    logs_prefix,
    continuation_token,
    ...
    try {
        ...
        dbg.log1(
            'log_parser aws_get_next_log_entry: finished successfully ',
            res,
        );
        return res;
    } catch (err) {
...
function aws_parse_log_object(
    ...
    for (const line of log_array) {
        if (line !== '') {
            ...
            if (log.operation) {
                if (
                    obj_prefix_filter === undefined ||
                    log.key?.startsWith(obj_prefix_filter)
                ) {
                    if (
                        log.operation.includes('PUT.OBJECT') ||
                        log.operation.includes('POST.OBJECT')
                    ) {
                        logs.push({
                            key: log.key,
                            action: 'copy',
                        ...
                        dbg.log2(
                            'aws_parse_log_object:: key',
                            log.key,
                            'contain copy (PUT or POST) entry',
                        );
                    }
                    if (
                        log.operation.includes('DELETE.OBJECT') &&
                        sync_deletions &&
                        log.http_status === 204
                    ) {
                        logs.push({
...
function _get_source_bucket_azure_connection(source_bucket_id) {
    const source_bucket = system_store.data.get_by_id(source_bucket_id);
    const extended_ns_info = pool_server.get_namespace_resource_extended_info(
        source_bucket.namespace.write_resource.resource,
    );
    ...
    return {
        logs_query_client: new LogsQueryClient(azure_token_credential),
        monitor_workspace_id: azure_logs_analytics_workspace_id,
    };
}

function _get_source_bucket_aws_connection(
    source_bucket_id,
    aws_log_replication_info,
) {
    const source_bucket = system_store.data.get_by_id(source_bucket_id);
    ...
    const agent =
        s3_resource_connection_info.endpoint_type === 'AWS' ?
            http_utils.get_default_agent(s3_resource_connection_info.endpoint)
        :   http_utils.get_unsecured_agent(
                s3_resource_connection_info.endpoint,
            );
    ...
    try {
        ...
        Object.keys(log_object_structure).forEach(field => {
            ...
            if (val === undefined) {
                ...
                result[field] = parse_potentially_empty_log_value(
                    val,
                    log_object_structure[field],
                );
            }
        });
        ...
        dbg.error(
            '_parse_aws_log_entry: failed to parse log entry - unexpected structure',
            err,
        );
        return {};
    }
...
function _get_log_object_continuation_token_for_rule(
    rule_id,
    replication_config,
) {
    dbg.log1(
        '_get_log_object_continuation_token_for_rule:: rule_id',
        rule_id,
        'replication_config',
        replication_config,
    );
    const replication_rule = replication_config.rules.find(
...
function _get_obj_prefix_filter_for_rule(rule_id, replication_config) {
    dbg.log1(
        '_get_obj_prefix_filter_for_rule: ',
        rule_id,
        'replication_config: ',
        replication_config,
    );
