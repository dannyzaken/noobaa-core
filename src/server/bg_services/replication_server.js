...
async function copy_objects_mixed_types(req) {
    ...
    dbg.log1(
        'replication_server copy_objects_mixed_types: params:',
        src_bucket_name,
        dst_bucket_name,
        keys_diff_map,
    );
    ...
    await P.map_with_concurrency(100, keys, async key => {
        //The concurrency can only be on the keys as the order of the versions matters
        if (keys_diff_map[key].length === 1) {
            ...
            try {
                ...
                copy_res.size_of_objects +=
                    keys_diff_map[key][0].ContentLength ||
                    keys_diff_map[key][0].Size;
            } catch (err) {
                dbg.error(
                    'replication_server copy_objects_mixed_types: got error:',
                    err,
                );
            }
        } else {
            for (let i = keys_diff_map[key].length - 1; i >= 0; i--) {
                // We need to replicate the oldest first
    ...
    dbg.log1(
        'replication_server copy_objects_mixed_types: finished successfully',
    );
    return copy_res;
}
