...
function background_worker() {
    ...
    return P.resolve()
        .then(() => {
            if (
                this.last_check &&
                Date.now() - this.last_check <
                    config.DEDUP_INDEXER_CHECK_INDEX_CYCLE
            ) {
                return;
            }
            ...
            return Promise.all([
                ...
                dbg.log0(
                    'DEDUP_INDEXER: dedup_index_size',
                    dedup_index_size,
                    'dedup_aprox_keys',
                    dedup_aprox_keys,
                );
        ...
        .then(() => {
            ...
            if (this.dedup_keys_count < config.DEDUP_INDEXER_LOW_KEYS_NUMBER) {
                dbg.log0(
                    'DEDUP_INDEXER: the index keys number is smaller than',
                    config.DEDUP_INDEXER_LOW_KEYS_NUMBER,
                    ' - nothing to do',
                );
            ...
            return MDStore.instance().iterate_indexed_chunks(
                batch_size,
                this.marker,
            );
        })
        .then(res => {
            ...
            dbg.log1(
                'DEDUP_INDEXER:',
                'WORKING ON',
                res.chunk_ids.length,
                'CHUNKS',
            );
            const ignore_back_time =
                Date.now() - config.DEDUP_INDEXER_IGNORE_BACK_TIME;
            chunk_ids = res.chunk_ids.filter(
                id => id.getTimestamp().getTime() < ignore_back_time,
            );
        ...
        .then(parts_by_chunk_id => {
            if (parts_by_chunk_id) {
                chunk_ids = chunk_ids.filter(
                    chunk_id =>
                        !parts_by_chunk_id[chunk_id] ||
                        parts_by_chunk_id[chunk_id].length <= 1,
                );
            }
            if (!chunk_ids.length) return;
