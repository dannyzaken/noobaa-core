...
const nodes_store =
    require('../node_services/nodes_store').NodesStore.instance();
...
async function background_worker() {
    ...
    if (this.last_check && now - this.last_check < config.DB_CLEANER_CYCLE) {
    ...
    if (from_time < last_date_to_remove) {
        dbg.log0(
            'DB_CLEANER: waiting for md_aggregator to advance to later than',
            new Date(last_date_to_remove),
        );
...
async function clean_md_store(last_date_to_remove) {
    const total_objects_count =
        await MDStore.instance().estimated_total_objects();
    ...
    dbg.log0(
        'DB_CLEANER: checking md-store for documents deleted before',
        new Date(last_date_to_remove),
    );
    const objects_to_remove = await MDStore.instance().find_deleted_objects(
        last_date_to_remove,
        config.DB_CLEANER_DOCS_LIMIT,
    );
    dbg.log2('DB_CLEANER: list objects:', objects_to_remove);
    if (objects_to_remove.length) {
        await P.map_with_concurrency(10, objects_to_remove, obj =>
            db_delete_object_parts(obj),
        );
        await MDStore.instance().db_delete_objects(objects_to_remove);
    }
    const blocks_to_remove = await MDStore.instance().find_deleted_blocks(
        last_date_to_remove,
        config.DB_CLEANER_DOCS_LIMIT,
    );
    dbg.log2('DB_CLEANER: list blocks:', blocks_to_remove);
    if (blocks_to_remove.length) {
        await MDStore.instance().db_delete_blocks(blocks_to_remove);
    }
    const chunks_to_remove = await MDStore.instance().find_deleted_chunks(
        last_date_to_remove,
        config.DB_CLEANER_DOCS_LIMIT,
    );
    const filtered_chunks = chunks_to_remove.filter(
        async chunk =>
            !(await MDStore.instance().has_any_blocks_for_chunk(chunk)) &&
            !(await MDStore.instance().has_any_parts_for_chunk(chunk)),
    );
    dbg.log2(
        'DB_CLEANER: list chunks with no blocks and no parts to be removed from DB',
        filtered_chunks,
    );
    if (filtered_chunks.length) {
        await MDStore.instance().db_delete_chunks(filtered_chunks);
    }
    dbg.log0(
        `DB_CLEANER: removed ${objects_to_remove.length + blocks_to_remove.length + filtered_chunks.length} documents from md-store`,
    );
}
...
async function clean_nodes_store(last_date_to_remove) {
    ...
    dbg.log0(
        'DB_CLEANER: checking nodes-store for nodes deleted before',
        new Date(last_date_to_remove),
    );
    const query = {
        deleted: {
    ...
    const nodes = await nodes_store.find_nodes(
        query,
        config.DB_CLEANER_DOCS_LIMIT,
        { _id: 1, deleted: 1 },
    );
    ...
    dbg.log2(
        'DB_CLEANER: list nodes with no agents to be removed from DB',
        filtered_nodes,
    );
    if (filtered_nodes.length) {
        await nodes_store.db_delete_nodes(filtered_nodes);
    }
    dbg.log0(
        `DB_CLEANER: removed ${filtered_nodes.length} documents from nodes-store`,
    );
}

async function clean_system_store(last_date_to_remove) {
    const total_accounts_count =
        await system_store.count_total_docs('accounts');
    ...
    if (
        total_accounts_count < config.DB_CLEANER_MAX_TOTAL_DOCS &&
    ...
    dbg.log0(
        'DB_CLEANER: checking system_store for documents deleted before',
        new Date(last_date_to_remove),
    );
    const [accounts, buckets, pools] = await P.all([
        system_store.find_deleted_docs(
            'accounts',
            last_date_to_remove,
            config.DB_CLEANER_DOCS_LIMIT,
        ),
    ...
    const filtered_buckets = buckets.filter(
        async bucket =>
            !(await MDStore.instance().has_any_objects_for_bucket_including_deleted(
                bucket,
            )),
    );
    const filtered_pools = pools.filter(
        async pool => !(await nodes_store.has_any_nodes_for_pool(pool)),
    );
    ...
    dbg.log0(
        `DB_CLEANER: removed ${accounts.length + filtered_buckets.length + filtered_pools.length} documents from system-store`,
    );
}
