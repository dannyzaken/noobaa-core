...
class BucketsReclaimer {
    ...
    async run_batch() {
        ...
        dbg.log0(
            'bucket_reclaimer: starting batch work on buckets: ',
            deleting_buckets.map(b => b.name).join(', '),
        );
        await P.all(
            deleting_buckets.map(async bucket => {
                try {
                    dbg.log0(
                        `emptying bucket ${bucket.name}. deleting next ${config.BUCKET_RECLAIMER_BATCH_SIZE} objects`,
                    );
                    const { is_empty } =
                        await this.client.object.delete_multiple_objects_unordered(
                            {
                                bucket: bucket.name,
                    ...
                    if (is_empty) {
                        dbg.log0(
                            `bucket ${bucket.name} is empty. calling delete_bucket`,
                        );
                    ...
                    dbg.error(
                        `got error when trying to empty and delete bucket ${bucket.name} :`,
                        err,
                    );
                    has_errors = true;
                }
    ...
    _can_run() {
        if (!system_store.is_finished_initial_load) {
            dbg.log0(
                'BucketsReclaimer: system_store did not finish initial load',
            );
            return false;
        }
    ...
    _add_bucket_to_delete_lists(bucket) {
        ...
        const deleting_buckets = this.buckets_to_delete.map(bucket_id =>
            String(bucket_id),
        );
        ...
        if (associated_buckets.length === 0) {
            ...
            this.tiers_to_delete.push(
                ...tiering_policy.tiers.map(t => t.tier._id),
            );
        }
    }
