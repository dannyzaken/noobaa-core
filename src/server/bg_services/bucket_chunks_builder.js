...
class BucketChunksBuilder {
  constructor({
    ...
    dbg.log0(
      `Created new BucketChunksBuilder. start marker [${this.marker}]. failure marker [${this.failure_marker}]`,
    );
  }
  ...
  async run_batch(buckets) {
    ...
    if (this.marker) {
      dbg.log1(
        'Bucket chunks builder: looking for chunks to mirror. starting from',
        this.marker,
      );
    } else {
      dbg.log0(
        'Bucket chunks builder: marker not set. starting from beginning',
      );
    }

    try {
      const { marker, chunk_ids } = await this.iterate_chunks_func(
        this.marker,
        this.end_marker,
        buckets,
      );
