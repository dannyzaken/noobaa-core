...
class ReplicationScanner {
    ...
    constructor({ name, client }) {
        this.name = name;
        this.client = client;
        this.scanner_semaphore = new Semaphore(
            config.REPLICATION_SEMAPHORE_CAP,
            {
    ...
    _can_run() {
        if (!system_store.is_finished_initial_load) {
            dbg.log0(
                'replication_scanner: system_store did not finish initial load',
            );
            return false;
        }
    ...
    async scan() {
        ...
        const least_recently_replicated_rules =
            await replication_store.find_rules_updated_longest_time_ago();

        await P.all(
            _.map(
                least_recently_replicated_rules,
                async replication_id_and_rule => {
                    ...
                    if (!src_bucket || !dst_bucket) {
                        dbg.error(
                            'replication_scanner: can not find src_bucket or dst_bucket object',
                            src_bucket,
                            dst_bucket,
                        );
                        return;
                    ...
                    const cur_src_cont_token =
                        (rule.rule_status && rule.rule_status.src_cont_token) ||
                        '';
                    const cur_dst_cont_token =
                        (rule.rule_status && rule.rule_status.dst_cont_token) ||
                        '';
                    ...
                    dbg.log1(
                        `scan:: cur_src_cont_token: ${cur_src_cont_token},cur_dst_cont_token: ${cur_dst_cont_token}`,
                    );
                    const {
                        keys_diff_map,
                    ...
                    if (Object.keys(keys_diff_map).length) {
                        ...
                        dbg.log0(
                            'replication_scanner: scan copy_res:',
                            copy_res,
                        );
                    }

                    await replication_store.update_replication_status_by_id(
                        replication_id,
                        rule.rule_id,
                        {
                            ...status,
                    ...
                    if (Object.keys(keys_diff_map).length) {
                        const replication_status =
                            replication_utils.get_rule_status(
                                rule.rule_id,
                                src_cont_token,
                                keys_diff_map,
                                copy_res,
