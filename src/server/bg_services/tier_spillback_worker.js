...
class TieringSpillbackWorker {
    ...
    _can_run() {
        if (!system_store.is_finished_initial_load) {
            dbg.log0(
                'TieringSpillbackWorker: system_store did not finish initial load',
            );
            return false;
        }
    ...
    async run_batch() {
        ...
        const wait =
            await this._spillback_need_to_move_chunks(spillback_buckets);
        console.log(
            `TieringSpillbackWorker: will wait ${wait} ms till next run`,
        );
        return wait;
    }
    ...
    async _spillback_need_to_move_chunks(buckets) {
        for (const bucket of buckets) {
            ...
            const spillback_tier = _.find(
                bucket.tiering.tiers,
                t => t.spillover,
            ).tier;
            const storage = await nodes_client
                .instance()
            ...
            const tier_storage_free = size_utils.json_to_bigint(
                storage[String(selected_tier._id)][0].free,
            );
            if (
                tier_storage_free.greater(
                    new size_utils.BigInteger(config.TIER_SPILLBACK_MIN_FREE),
                )
            ) {
                const chunk_ids =
