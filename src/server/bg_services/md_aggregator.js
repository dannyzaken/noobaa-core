...
function find_minimal_range({ target_now, system_store, global_last_update }) {
    ...
    _.forEach(get_buckets(system_store), bucket => {
        ...
        const last_update =
            global_last_update > bucket_last_update ? global_last_update : (
                bucket_last_update
            );
        if (last_update > target_now) {
            dbg.error(
                'find_next_range: time skew detected for bucket',
                bucket.name,
                'last_update',
                last_update,
                'target_now',
    ...
    _.forEach(system_store.data.pools, pool => {
        const pool_last_update =
            _.get(pool, 'storage_stats.last_update') || config.NOOBAA_EPOCH;
        const last_update =
            global_last_update > pool_last_update ? global_last_update : (
                pool_last_update
            );
        if (last_update > target_now) {
            dbg.error(
                'find_next_range: time skew detected for pool',
                pool.name,
                'last_update',
                last_update,
                'target_now',
...
function find_next_range({
    ...
    _.forEach(get_buckets(system_store), bucket => {
        const bucket_last_update =
            _.get(bucket, 'storage_stats.last_update') || config.NOOBAA_EPOCH;
        const last_update =
            bucket_last_update > global_last_update ? bucket_last_update : (
                global_last_update
            );
        dbg.log1(
            'find_next_range: bucket',
            bucket.name,
            'last_update*',
            last_update - from_time,
        );
    });
    _.forEach(system_store.data.pools, pool => {
        const pool_last_update =
            _.get(pool, 'storage_stats.last_update') || config.NOOBAA_EPOCH;
        const last_update =
            pool_last_update > global_last_update ? pool_last_update : (
                global_last_update
            );
        dbg.log1(
            'find_next_range: pool',
            pool.name,
            'last_update*',
            last_update - from_time,
        );
    ...
    if (should_reset_all) {
        dbg.error(
            'find_next_range: reset all storage_stats due to time skews ',
            'from_time',
            from_time,
            'till_time*',
            till_time - from_time,
...
function range_md_aggregator({ md_store, system_store, range }) {
    const from_time = range.from_time;
    const till_time = range.till_time;
    let more_updates = false;

    const filtered_buckets = _.filter(
        get_buckets(system_store),
        bucket => bucket.storage_stats.last_update <= from_time,
    );
    const filtered_pools = _.filter(
        system_store.data.pools,
        pool => pool.storage_stats.last_update <= from_time,
    );
    if (
        filtered_buckets.length > config.MD_AGGREGATOR_BATCH ||
        filtered_pools.length > config.MD_AGGREGATOR_BATCH
    ) {
        more_updates = true;
    }
    ...
    return Promise.all([
        ...
        ([
            ...
            dbg.log3(
                ...
                till_time,
                'existing_objects_aggregate',
                util.inspect(existing_objects_aggregate, true, null, true),
                'deleted_objects_aggregate',
                util.inspect(deleted_objects_aggregate, true, null, true),
                'existing_blocks_aggregate',
                util.inspect(existing_blocks_aggregate, true, null, true),
                'deleted_blocks_aggregate',
                util.inspect(deleted_blocks_aggregate, true, null, true),
            );
            ...
            const buckets_updates = _.map(buckets, bucket => {
                ...
                if (
                    _.isEqual(
                        _.omit(bucket.storage_stats, 'last_update'),
                        new_storage_stats,
                    )
                ) {
            ...
            const pools_updates = _.map(pools, pool => {
                ...
                if (
                    _.isEqual(
                        _.omit(pool.storage_stats, 'last_update'),
                        new_storage_stats,
                    )
                ) {
                    dont_change_last_update = true;
...
function calculate_new_pool({
    ...
    const new_storage_stats = {
        blocks_size:
            (pool.storage_stats && pool.storage_stats.blocks_size) || 0,
    };
    dbg.log3('Pool storage stats before deltas:', new_storage_stats);
    const bigint_ex_blocks_agg = size_utils.json_to_bigint(
        (existing_blocks_aggregate.pools[pool._id] &&
            existing_blocks_aggregate.pools[pool._id].size) ||
            0,
    );
    const bigint_de_blocks_agg = size_utils.json_to_bigint(
        (deleted_blocks_aggregate.pools[pool._id] &&
    ...
    new_storage_stats.blocks_size = size_utils
        .json_to_bigint(new_storage_stats.blocks_size)
...
function calculate_new_bucket({
    ...
    const new_storage_stats = {
        chunks_capacity:
            (bucket.storage_stats && bucket.storage_stats.chunks_capacity) || 0,
        objects_size:
        ...
        stats_by_content_type:
            (bucket.storage_stats &&
                bucket.storage_stats.stats_by_content_type) ||
            [],
        blocks_size:
            (bucket.storage_stats && bucket.storage_stats.blocks_size) || 0,
        pools: (bucket.storage_stats && bucket.storage_stats.pools) || {},
    };

    dbg.log3('Bucket storage stats before deltas:', new_storage_stats);
    const bigint_ex_chunks_agg = size_utils.json_to_bigint(
        (existing_chunks_aggregate[bucket._id] &&
            existing_chunks_aggregate[bucket._id].compress_size) ||
            0,
    );
    const bigint_de_chunks_agg = size_utils.json_to_bigint(
        (deleted_chunks_aggregate[bucket._id] &&
            deleted_chunks_aggregate[bucket._id].compress_size) ||
            0,
    );
    const bigint_ex_blocks_agg = size_utils.json_to_bigint(
        (existing_blocks_aggregate.buckets[bucket._id] &&
            existing_blocks_aggregate.buckets[bucket._id].size) ||
            0,
    );
    const bigint_de_blocks_agg = size_utils.json_to_bigint(
        (deleted_blocks_aggregate.buckets[bucket._id] &&
            deleted_blocks_aggregate.buckets[bucket._id].size) ||
            0,
    );
    const ex_pools =
        (existing_blocks_aggregate.buckets[bucket._id] &&
    ...
    const de_pools =
        ...
        {};
    const bigint_ex_obj_agg = size_utils.json_to_bigint(
        (existing_objects_aggregate[bucket._id] &&
            existing_objects_aggregate[bucket._id].size) ||
            0,
    );
    const bigint_de_obj_agg = size_utils.json_to_bigint(
        (deleted_objects_aggregate[bucket._id] &&
            deleted_objects_aggregate[bucket._id].size) ||
            0,
    );
    ...
    const delta_chunk_compress_size =
        bigint_ex_chunks_agg.minus(bigint_de_chunks_agg);
    const delta_block_size = bigint_ex_blocks_agg.minus(bigint_de_blocks_agg);

    const delta_buckets_pool_size = _.mergeWith(
        ex_pools,
        de_pools,
        (ex_value, de_value) => ({
            size: size_utils
                .json_to_bigint((ex_value && ex_value.size) || 0)
                .minus(
                    size_utils.json_to_bigint((de_value && de_value.size) || 0),
                ),
        }),
    );

    const delta_object_size = bigint_ex_obj_agg.minus(bigint_de_obj_agg);
    const delta_object_count =
        ((existing_objects_aggregate[bucket._id] &&
            existing_objects_aggregate[bucket._id].count) ||
            0) -
        ((deleted_objects_aggregate[bucket._id] &&
            deleted_objects_aggregate[bucket._id].count) ||
            0);

    // If we won't always update the checkpoint, on no changes
    // We will reduce all of the chunks from last checkpoint (which can be a lot)
    new_storage_stats.chunks_capacity = size_utils
        .json_to_bigint(new_storage_stats.chunks_capacity)
    ...
    new_storage_stats.blocks_size = size_utils
        ...
        .plus(delta_block_size)
        .toJSON();
    new_storage_stats.pools = _.mergeWith(
        new_storage_stats.pools,
        delta_buckets_pool_size,
        (ex_value, de_value) => ({
            blocks_size: size_utils
                .json_to_bigint((ex_value && ex_value.blocks_size) || 0)
                .plus(
                    size_utils.json_to_bigint((de_value && de_value.size) || 0),
                )
                .toJSON(),
    ...
    new_storage_stats.pools = _.pickBy(
        new_storage_stats.pools,
        pool => (pool && pool.blocks_size) || 0,
    );
    new_storage_stats.objects_size = size_utils
        .json_to_bigint(new_storage_stats.objects_size)
        .plus(delta_object_size)
        .toJSON();
    new_storage_stats.objects_count += delta_object_count;
    new_storage_stats.objects_hist = build_objects_hist(
        bucket,
        existing_objects_aggregate,
        deleted_objects_aggregate,
    );
...
function aggregate_by_content_type({
    ...
    const ex_by_content_type =
        existing_objects_aggregate[bucket._id] &&
        existing_objects_aggregate[bucket._id].content_type;
    const de_by_content_type =
        deleted_objects_aggregate[bucket._id] &&
        deleted_objects_aggregate[bucket._id].content_type;

    let stats_by_content_type_obj = _.keyBy(
        new_storage_stats.stats_by_content_type,
        'content_type',
    );

    if (ex_by_content_type || de_by_content_type) {
        // convert current stats to bigint
        stats_by_content_type_obj = _.mapValues(
            stats_by_content_type_obj,
            val => ({
    ...
    if (ex_by_content_type || de_by_content_type) {
        new_storage_stats.stats_by_content_type = _.map(
            stats_by_content_type_obj,
            (bigint_val, content_type) => ({
                content_type,
...
function build_objects_hist(bucket, existing_agg, deleted_agg) {
    ...
    const current_objects_hist =
        (bucket.storage_stats && bucket.storage_stats.objects_hist) || [];

    // get the latest additions\deletions in an array form
    const existing_size_hist = get_hist_array_from_aggregate(
        existing_agg[bucket._id],
        'size',
    );
    const deleted_size_hist = get_hist_array_from_aggregate(
        deleted_agg[bucket._id],
        'size',
    );
    const existing_count_hist = get_hist_array_from_aggregate(
        existing_agg[bucket._id],
        'count',
    );
    const deleted_count_hist = get_hist_array_from_aggregate(
        deleted_agg[bucket._id],
        'count',
    );
    ...
    if (
        deleted_size_hist.length !== deleted_count_hist.length ||
        existing_size_hist.length !== existing_count_hist.length
    ) {
        dbg.error(
            'size histogram and count histogram have different lengths',
            'deleted_size_hist.length =',
            deleted_size_hist.length,
            'deleted_count_hist.length =',
            deleted_count_hist.length,
            'existing_size_hist.length =',
            existing_size_hist.length,
            'existing_count_hist.length =',
            existing_count_hist.length,
        );
    }
    ...
    for (let i = 0; i < num_bins; i++) {
        const bin = {
            label:
                (current_objects_hist[i] && current_objects_hist[i].label) ||
                get_hist_label(i),
            aggregated_sum: get_new_bin(
                existing_size_hist[i] || 0,
                deleted_size_hist[i] || 0,
                (current_objects_hist[i] &&
                    current_objects_hist[i].aggregated_sum) ||
                    0,
            ),
            count: get_new_bin(
...
function get_new_bin(existing, deleted, current) {
    ...
    const delta_size_bin = bigint_existing_size_bin.minus(
        bigint_deleted_size_bin,
    );
    const new_bin = size_utils
        .json_to_bigint(current)
        .plus(delta_size_bin)
        .toJSON();
    return new_bin;
}
