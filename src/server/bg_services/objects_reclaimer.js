...
class ObjectsReclaimer {
    ...
    async run_batch() {
        ...
        const unreclaimed_objects =
            await MDStore.instance().find_unreclaimed_objects(
                config.OBJECT_RECLAIMER_BATCH_SIZE,
            );
        ...
        dbg.log0(
            'object_reclaimer: starting batch work on objects: ',
            unreclaimed_objects.map(o => o.key).join(', '),
        );
        const reclaimed_objects_ids = [];
        await P.all(
            unreclaimed_objects.map(async obj => {
                try {
                    await map_deleter.delete_object_mappings(obj, true);
                    reclaimed_objects_ids.push(obj._id);
                } catch (err) {
                    dbg.error(
                        `got error when trying to delete object ${obj.key} mappings :`,
                        err,
                    );
                    has_errors = true;
                }
        ...
        await MDStore.instance().update_objects_by_ids(reclaimed_objects_ids, {
            reclaimed: new Date(),
        });
    ...
    _can_run() {
        if (!system_store.is_finished_initial_load) {
            dbg.log0(
                'ObjectsReclaimer: system_store did not finish initial load',
            );
            return false;
        }
