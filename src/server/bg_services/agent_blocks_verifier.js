...
class AgentBlocksVerifier {
    ...
    run_batch() {
        if (!system_store.is_finished_initial_load) {
            dbg.log0(
                'AGENT_BLOCKS_VERIFIER: system_store did not finish initial load',
            );
            return;
        }
    ...
    async populate_agent_blocks_verifier_blocks(blocks) {
        ...
        const populated_blocks =
            await this.populate_and_prepare_for_blocks(blocks);
        if (!populated_blocks || !populated_blocks.length) return;
        const blocks_with_alive_nodes = populated_blocks.filter(
            block => block.node && block.node.rpc_address && block.node.online,
        );
    ...
    verify_blocks_on_agents(blocks_to_verify) {
        return P.resolve()
            .then(() => {
                const verify_blocks_group_by_nodes = _.groupBy(
                    blocks_to_verify,
                    'address',
                );
                return P.map(
                    Object.keys(verify_blocks_group_by_nodes),
                    address => {
                        const verify_blocks =
                            verify_blocks_group_by_nodes[address];
