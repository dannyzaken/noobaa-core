...
class NamespaceMonitor {
    ...
    _can_run() {
        if (!system_store.is_finished_initial_load) {
            dbg.log0(
                'namespace_monitor: system_store did not finish initial load',
            );
            return false;
        }
    ...
    async test_namespace_resources_validity() {
        await P.map_with_concurrency(
            ...
            async nsr => {
                ...
                const endpoint_type =
                    nsr.nsfs_config ? 'NSFS' : nsr.connection.endpoint_type;

                try {
                    if (endpoint_type === 'NSFS') {
                        await this.test_nsfs_resource(nsr);
                    } else if (
                        ['AWS', 'AWSSTS', 'S3_COMPATIBLE', 'IBM_COS'].includes(
                            endpoint_type,
                        )
                    ) {
                        ...
                        dbg.error(
                            'namespace_monitor: invalid endpoint type',
                            endpoint_type,
                        );
                    }
                    this.update_last_monitoring(
                        nsr._id,
                        nsr.name,
                        endpoint_type,
                    );
                    ...
                    dbg.log1(
                        `test_namespace_resource_validity: namespace resource ${nsr.name} has error as expected`,
                    );
                }
            },
        );
    ...
    update_last_monitoring(nsr_id, nsr_name, endpoint_type) {
        dbg.log0(
            `update_last_monitoring: monitoring namespace ${nsr_name} type ${endpoint_type}, ${nsr_id} finished successfully..`,
        );
    ...
    async test_s3_resource(nsr) {
        ...
        if (!conn) {
            const { endpoint, access_key, secret_key, auth_method, region } =
                nsr.connection;
    ...
    async test_blob_resource(nsr) {
        ...
        try {
            ...
            if (
                err.code === 'InsufficientAccountPermissions' &&
                nsr.is_readonly_namespace()
            ) {
                return;
            }
    ...
    async test_gcs_resource(nsr) {
        ...
        if (!conn) {
            const { project_id, private_key, client_email } = JSON.parse(
                nsr.connection.secret_key.unwrap(),
            );
        ...
        try {
            ...
            if (
                err.errors[0].reason === 'UserProjectAccessDenied' &&
                nsr.is_readonly_namespace()
            ) {
                return;
            }
    ...
    async test_nsfs_resource(nsr) {
        dbg.log1(
            'test_nsfs_resource: (name, namespace_store, nsfs_config):',
            nsr.name,
            nsr.namespace_store,
            nsr.nsfs_config,
        );
        try {
            ...
            await nb_native().fs.readdir(
                fs_context,
                nsr.nsfs_config.fs_root_path,
            );
            ...
            if (stat.nlink === 0) {
                dbg.log1(
                    'test_nsfs_resource: the number of links is 0',
                    nsr.nsfs_config,
                );
                throw Object.assign(new Error('FS root path has no links'), {
                    code: 'ENOENT',
                });
            }
        } catch (err) {
