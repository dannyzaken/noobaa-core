...
const endpoint_stats_collector =
    require('../../sdk/endpoint_stats_collector').instance();
...
class SemaphoreMonitor {
    ...
    constructor({ name, object_io, report_sample_sizes }) {
        ...
        this.report_sample_sizes =
            report_sample_sizes || semaphore_report_sample_sizes();
    }
    ...
    run_semaphore_monitor() {
        try {
            if (config.ENABLE_OBJECT_IO_SEMAPHORE_MONITOR) {
                this.sample_object_io_semaphore();
            }
            Object.keys(nsfs_semaphores).forEach(s =>
                this.sample_nsfs_semaphore(s),
            );
        } catch (err) {
            dbg.error('semaphore_monitor:', err, err.stack);
        }
    ...
    _can_run() {
        if (
            !this.report_sample_sizes ||
            this.report_sample_sizes.length === 0
        ) {
            dbg.log0(
                'semaphore_monitor: report_sample_sizes do not have valid size',
                this.report_sample_sizes,
            );
            return false;
        }
    ...
    sample_object_io_semaphore() {
        ...
        try {
            ...
            endpoint_stats_collector.update_semaphore_state(
                semaphore_report,
                'object_io',
                this.report_sample_sizes,
            );
    ...
    sample_nsfs_semaphore(name) {
        ...
        const buffers_pool_sem =
            multi_buffer_pool.get_buffers_pool(buffer_size).sem;
        if (!buffers_pool_sem) {
            dbg.log0(
                'semaphore_monitor: buffers_pool_sem is invalid',
                this.object_io,
            );
            return;
        }
        try {
            ...
            dbg.error(
                'Could not submit nsfs endpoint monitor report, got:',
                err,
            );
        }
    }
...
function semaphore_report_sample_sizes() {
    ...
    for (const interval of config.SEMAPHORE_METRICS_AVERAGE_INTERVALS) {
        ...
        const sample_size =
            (interval_num * 60 * 1000) / config.SEMAPHORE_MONITOR_DELAY;
