    ...
    for (const bucket of buckets) {
      ...
      const tier_storage_free = size_utils.json_to_bigint(
        size_utils.reduce_minimum(
          'free',
          tiering_status[String(selected_tier._id)].mirrors_storage.map(
      ...
      const time =
        valid && report ? Math.floor((now - report.timestamp) / 1000 / 60) : 60;
      bucket.TTF =
        valid && report && report.write_bytes ?
          tier_storage_free.divide(
            size_utils.json_to_bigint(report.write_bytes).divide(time),
