...
function update_host_address(address) {
    ...
    _.each(current_clustering.shards, function (shard, i) {
        ...
        if (ind !== -1) {
            current_clustering.shards[i].servers[ind].address =
                url.parse(address).hostname;
        }
    });

    dbg.log0(
        'clustering info after host update is',
        util.inspect(current_clustering, {
            depth: 6,
        }),
    );
    ...
    return system_store
        ...
        .catch(err => {
            ...
            throw new Error(
                `Failed updating host address in clustering info ${err}, ${err.stack}`,
            );
        });
}
...
function verify_cluster_id(cluster_id) {
    if (get_topology().cluster_id !== cluster_id) {
        dbg.error(
            'ClusterID mismatch: has',
            get_topology().cluster_id,
            ' recieved:',
            cluster_id,
        );
...
function get_cluster_info() {
    ...
    _.each(system_store.data.clusters, cinfo => {
        ...
        const server_info = {
            ...
            addresses:
                net_utils.is_localhost(cinfo.owner_address) ?
                    os_utils.get_local_ipv4_ips()
                :   [cinfo.owner_address].concat(os_utils.get_local_ipv4_ips()),
            status: is_connected,
            memory: memory,
            storage: storage,
            cpus: cpus,
...
function send_master_update(is_master, master_address) {
    ...
    return Promise.all([
        server_rpc.client.system
            ...
            .catch(err =>
                dbg.error('got error on set_webserver_master_state.', err),
            ),
        hosted_agents_promise.catch(err =>
            dbg.error('got error on hosted_agents_promise.', err),
        ),
        update_master_promise.catch(err =>
            dbg.error('got error on update_master_promise.', err),
        ),
    ]).then(() => {
        // do nothing.
    });
