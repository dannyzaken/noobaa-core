...
MongoCtrl.prototype.add_replica_set_member = function (
    name,
    first_server,
    servers,
) {
    ...
    return self
        ...
        .then(() => {
            if (first_server) {
                self._init_replica_set_from_shell(
                    cutil.extract_servers_ip(servers)[0],
                );
            }
        })
        .then(() =>
...
MongoCtrl.prototype.initiate_replica_set = function (
    set,
    members,
    is_config_set,
) {
...
MongoCtrl.prototype.add_member_to_replica_set = function (
    set,
    members,
    is_config_set,
) {
...
MongoCtrl.prototype.add_member_shard = function (name, ip) {
    ...
    return mongo_client
        .instance()
        .add_shard(ip, config.MONGO_DEFAULTS.SHARD_SRV_PORT, name);
};
        ...
        .then(mongo_res => {
            ...
            _.forEach(mongo_res.members, member => {
                if (member.stateStr === 'PRIMARY') {
                    res_address = member.name.substring(
                        0,
                        member.name.indexOf(':'),
                    );
                }
...
MongoCtrl.prototype.add_mongo_monitor_program = function () {
    ...
    program_obj.command =
        '/usr/local/bin/node src/server/mongo_services/mongo_monitor.js';
    dbg.log0('adding mongo_monitor program:', program_obj);
    return SupervisorCtl.add_program(program_obj).then(() =>
        SupervisorCtl.apply_changes(),
    );
};
...
MongoCtrl.prototype.update_dotenv = function (name, IPs) {
    ...
    const servers_str = IPs.map(
        ip => ip + ':' + config.MONGO_DEFAULTS.SHARD_SRV_PORT,
    ).join(',');
    const url =
        'mongodb://' +
        user_name +
...
MongoCtrl.prototype._add_replica_set_member_program = async function (
    name,
    first_server,
) {
    if (!name) {
    ...
    const program_obj = {};
    const dbpath =
        config.MONGO_DEFAULTS.COMMON_PATH +
        '/' +
        name +
        (first_server ? '' : 'rs');
    ...
    try {
        ...
        dbg.error(
            `could not get stats for ${config.MONGO_DEFAULTS.COMMON_PATH}. mongod uid and gid are unkown`,
        );
    }
    program_obj.name = 'mongo_wrapper';
    program_obj.command =
        '/root/node_modules/noobaa-core/src/deploy/NVA_build/mongo_wrapper.sh' +
        ' mongod' +
        ' --replSet ' +
        name +
        ' --port ' +
    ...
    if (first_server) {
        //If shard1 (this means this is the first server which will be the base of the cluster)
        ...
        dbg.log0(
            'adding server to an existing cluster. cleaning dbpath:',
            dbpath,
        );
...
MongoCtrl.prototype._add_new_shard_program = function (name, first_shard) {
    ...
    if (first_shard) {
        //If shard1 (this means this is the first servers which will be the base of the cluster)
        //use the original server`s data (i.e. dbpath/shard1)
        return SupervisorCtl.add_program(program_obj);
    } else {
        return fs_utils
            .create_fresh_path(dbpath)
...
MongoCtrl.prototype._init_replica_set_from_shell = function (ip) {
    const host = ip + ':' + config.MONGO_DEFAULTS.SHARD_SRV_PORT;
    const mongo_shell_command =
        `mongo nbcore --port ${config.MONGO_DEFAULTS.SHARD_SRV_PORT} --ssl` +
...
MongoCtrl.prototype._remove_single_mongo_program = function () {
    return P.resolve().then(() =>
        SupervisorCtl.remove_program('mongo_wrapper'),
    );
};
