...
class SupervisorCtrl {
    ...
    init() {
        ...
        return fs.promises
            .stat(config.CLUSTERING_PATHS.SUPER_FILE)
            .catch(function (err) {
                console.warn('Error on reading supervisor file', err);
                throw err;
            })
            .then(() =>
                fs.promises.readFile(config.CLUSTERING_PATHS.SUPER_FILE),
            )
    ...
    async update_program(program) {
        await this.init();
        const prog_index = this._programs.findIndex(
    ...
    async update_services_autostart(services, value) {
        if (value !== true && value !== false) {
            throw new Error('autostart supports true/false only');
        }
