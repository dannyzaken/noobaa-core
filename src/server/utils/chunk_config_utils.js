...
function new_chunk_code_config_defaults(chunk_coder_config) {
    ...
    if (ccc.parity_frags || config.CHUNK_CODER_EC_IS_DEFAULT) {
        ...
        ccc.parity_frags =
            ccc.parity_frags || config.CHUNK_CODER_EC_PARITY_FRAGS;
...
function resolve_chunk_config(chunk_coder_config, account, system) {
    ...
    const chunk_coder_config_full =
        new_chunk_code_config_defaults(chunk_coder_config);
