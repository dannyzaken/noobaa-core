...
class BucketDiff {
    ...
    async get_buckets_diff(params) {
        ...
        while (keep_listing_second_bucket) {
            ({
                ...
                prefix,
                max_keys,
                new_second_bucket_cont_token,
            ));

            const keys_diff_response = await this.get_keys_diff(
                first_bucket_contents_left,
                second_bucket_contents_left,
                new_second_bucket_cont_token,
            );

            first_bucket_contents_left = keys_diff_response.keys_contents_left;
            keep_listing_second_bucket =
                keys_diff_response.keep_listing_second_bucket;
            diff.keys_diff_map = {
                ...diff.keys_diff_map,
                ...keys_diff_response.keys_diff_map,
            };

            const first_bucket_key_array = Object.keys(
                first_bucket_contents_left,
            );
            const second_bucket_key_array = Object.keys(
                second_bucket_contents_left,
            );

            if (
                first_bucket_key_array.length !== 0 &&
                second_bucket_key_array.length !== 0
            ) {
                const first_bucket_key_in_last_pos =
                    first_bucket_key_array[first_bucket_key_array.length - 1];
                const second_bucket_key_in_last_pos =
                    second_bucket_key_array[second_bucket_key_array.length - 1];
                if (
                    first_bucket_key_in_last_pos >=
                    second_bucket_key_in_last_pos
                ) {
        ...
        diff.second_bucket_cont_token =
            first_bucket_cont_token && second_bucket_cont_token ?
                new_second_bucket_cont_token
            :   '';
    ...
    _object_grouped_by_key_and_omitted(list) {
        ...
        if (this.for_deletion && !this.version) {
            throw new Error(
                'Invalid buckets: versioning not enabled for delete replications',
            );
        }
        ...
        if (list.IsTruncated && this.version) {
            ...
            if (Object.keys(grouped_by_key).length > 1) {
                grouped_by_key = _.omit(
                    grouped_by_key,
                    list[field][last_key_pos].Key,
                );
            }
        }
    ...
    _get_next_key_marker(list_objects_response, list) {
        if (this.version) {
            return list_objects_response.IsTruncated ?
                    Object.keys(list)[Object.keys(list).length - 1]
                :   '';
        }
    ...
    async get_objects(bucket, prefix, max_keys, curr_bucket_cont_token) {
        dbg.log2(
            'BucketDiff get_objects::',
            bucket,
            prefix,
            max_keys,
            this.version,
            curr_bucket_cont_token,
        );
        const list_objects_response = await this._list_objects(
            bucket,
            prefix,
            max_keys,
            curr_bucket_cont_token,
        );
        if (!list_objects_response) {
            return { bucket_contents_left: {}, bucket_cont_token: '' };
        }
        dbg.log2(
            'BucketDiff get_objects:: bucket_response',
            list_objects_response,
        );
        const bucket_contents_left = this._object_grouped_by_key_and_omitted(
            list_objects_response,
        );
        const bucket_cont_token = this._get_next_key_marker(
            list_objects_response,
            bucket_contents_left,
        );
        dbg.log2(
            'BucketDiff get_objects:: bucket',
            bucket,
            'bucket_contents_left',
            bucket_contents_left,
        );
    ...
    async get_keys_diff(
        ...
        const stop_compare = this._process_keys_out_of_range(
            ans,
            second_bucket_keys,
        );
        if (stop_compare) return ans;

        await this._process_keys_in_range(
            ans,
            second_bucket_keys,
            second_bucket_cont_token,
        );
    ...
    _process_keys_out_of_range(ans, second_bucket_keys) {
        ...
        if (
            !Object.keys(second_bucket_key_array).length ||
            first_bucket_key_in_last_pos < second_bucket_key_array[0]
        ) {
            ans.keys_diff_map = {
                ...ans.keys_diff_map,
    ...
    async _process_keys_in_range(
        ...
        for (const cur_first_bucket_key of first_bucket_key_array) {
            ...
            const first_bucket_curr_obj =
                ans.keys_contents_left[cur_first_bucket_key];

            // case 2:
            if (cur_first_bucket_key < second_bucket_key_array[0]) {
                this._populate_diff_map_and_omit_contents_left(
                    ans,
            ...
            const second_bucket_curr_obj =
                second_bucket_keys[cur_first_bucket_key];

            if (!this.for_deletion && second_bucket_curr_obj) {
                // for replication
                ...
                let etag_pos_on_first_bucket = this._get_etag_pos(
                    0,
                ...
                if (etag_pos_on_first_bucket.length === 0) {
                    ({ should_continue, etag_pos_on_first_bucket } =
                        this._etag_not_in_first_bucket(
                            ans,
                            etag_pos_on_first_bucket,
                            cur_first_bucket_key,
                            first_bucket_curr_obj,
                            second_bucket_curr_obj,
                        ));
                ...
                if (etag_pos_on_first_bucket.length >= 1) {
                    ...
                    if (pos > 0) {
                        ...
                        if (same_md) {
                            const first_bucket_diff =
                                first_bucket_curr_obj.slice(0, pos);
                            ans.keys_diff_map[cur_first_bucket_key] =
                                first_bucket_diff;
                        } else {
                            this._populate_diff_map_and_omit_contents_left(
                                ans,
                        ...
                        dbg.log1(
                            'The same file with the same ETag found in both buckets on the latest versions',
                            second_bucket_curr_obj,
                        );
                        const same_md = await this._is_same_user_metadata(
                            0,
                ...
                ans.keys_contents_left = _.omit(
                    ans.keys_contents_left,
                    cur_first_bucket_key,
                );
                continue;
            } else if (this.for_deletion && second_bucket_curr_obj) {
                // for deletion
                const latest_delete_marker_first =
                    this._get_latest_delete_marker(first_bucket_curr_obj);
                const latest_delete_marker_second =
                    this._get_latest_delete_marker(second_bucket_curr_obj);
                ...
                if (
                    latest_delete_marker_first &&
                    !latest_delete_marker_second
                ) {
                    this._populate_diff_map_and_omit_contents_left(
                        ans,
                        cur_first_bucket_key,
                ...
                //This is when there is no such key name on the second bucket in this range.
                this._populate_diff_map_and_omit_contents_left(
                    ans,
                    cur_first_bucket_key,
                    first_bucket_curr_obj,
    ...
    _etag_not_in_first_bucket(
        ...
        if (this.version) {
            ...
            while (
                etag_pos_on_first_bucket.length === 0 &&
                pos < second_bucket_curr_obj.length
            ) {
                pos += 1;
                etag_pos_on_first_bucket = this._get_etag_pos(
                    pos,
                    second_bucket_curr_obj,
                    first_bucket_curr_obj,
                );
            ...
            if (
                ...
                this._check_last_modified_for_replication(
                    ans,
                    cur_first_bucket_key,
                    first_bucket_curr_obj,
                    second_bucket_curr_obj,
            ...
            // in non version flow - if the second bucket key etag is not the same as the first bucket key etag - this is a diff.
     ...
     */
    _populate_diff_map_and_omit_contents_left(
        ans,
        cur_bucket_key,
        bucket_curr_obj,
    ) {
     ...
     * @param {any[]} second_bucket_curr_obj
     */
    _check_last_modified_for_replication(
        ans,
        cur_first_bucket_key,
        first_bucket_curr_obj,
        second_bucket_curr_obj,
        ...
        if (
            this.for_replication &&
            first_bucket_curr_obj[0].LastModified <
                second_bucket_curr_obj[0].LastModified
        ) {
            ans.keys_contents_left = _.omit(
                ans.keys_contents_left,
                cur_first_bucket_key,
    ...
    async _is_same_user_metadata(
        ...
        const [
            first_bucket_curr_obj_metadata,
            second_bucket_curr_obj_metadata,
        ] = await Promise.all([
            replication_utils.get_object_md(
                this.first_bucket,
                cur_first_bucket_key,
                this.s3,
                first_bucket_obj_version_id,
            ),
            replication_utils.get_object_md(
                this.second_bucket,
                cur_first_bucket_key,
                this.s3,
                second_bucket_obj_version_id,
            ),
        ]);

        const first_bucket_curr_obj_user_metadata =
            first_bucket_curr_obj_metadata?.Metadata;
        const second_bucket_curr_obj_user_metadata =
            second_bucket_curr_obj_metadata?.Metadata;
        return _.isEqual(
            first_bucket_curr_obj_user_metadata,
            second_bucket_curr_obj_user_metadata,
        );
    }
