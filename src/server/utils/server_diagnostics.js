...
const operations = [
    async () => {
        try {
            ...
            return diag_log(
                'collect_supervisor_logs failed with error: ' + err,
            );
        }
    },
    ...
    async () => {
        try {
            ...
            return diag_log(
                'collecting noobaa_deploy.log failed with error: ' + err,
            );
        }
    },
    ...
    async () => {
        try {
            ...
            return diag_log(
                'collecting client_noobaa.log failed with error: ' + err,
            );
        }
    },
    ...
    async () => {
        try {
            ...
            return diag_log(
                'collecting /log/nbfedump directory failed with error: ' + err,
            );
        }
    },
    ...
    async () => {
        try {
            ...
            return diag_log(
                'collecting chkconfig.out failed with error: ' + err,
            );
        }
    },
    ...
    async () => {
        try {
            ...
            return diag_log(
                'collecting /etc/noobaa files failed with error: ' + err,
            );
        }
    },
    ...
    async () => {
        try {
            ...
            await fs.promises.writeFile(
                path.join(
                    TMP_WORK_DIR,
                    'mongo_db_system_collections_dump.json',
                ),
            ...
            return diag_log(
                'get_system_collections_dump failed with error: ' + err,
            );
        }
    },
    ...
    async req => {
        try {
            ...
            await fs.promises.writeFile(
                path.join(TMP_WORK_DIR, 'hosts_list.json'),
                JSON.stringify(hosts_list, null, 2),
            );
...
async function collect_server_diagnostics(req) {
    try {
        ...
        console.error(
            'Error in collecting server diagnostics (should never happen)',
            err,
        );
        ...
        if (process.platform === 'linux') {
            ...
            return os_utils
                .exec(
                    'GZIP=-1 tar -czf ' +
                        TMP_WORK_DIR +
                        '/supervisor_log.tgz /log/supervisor/* ',
                    {
                        ignore_rc: false,
                        return_stdout: false,
                        timeout: LONG_EXEC_TIMEOUT,
                ...
                .catch(function (err) {
                    ...
                    throw new Error(
                        'Error in collecting supervisor logs ' + err,
                    );
                });
        } else {
            console.log(
                'Skipping supervisor logs collection on non linux server',
            );
        }
    });
}
...
function collect_statistics(req) {
    return P.resolve()
        ...
        .then(function (restats) {
            if (stats_aggregator) {
                ...
                return fs.promises.writeFile(
                    TMP_WORK_DIR + '/phone_home_stats.out',
                    stats_data,
                );
            }
        })
        .catch(function (err) {
            console.error(
                'Failed to collect phone_home_stats',
                err.stack || err,
            );
        });
}
