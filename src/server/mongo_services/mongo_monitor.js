...
async function get_mongo_rs_status_from_shell() {
    const rs_status_str = await os_utils.exec(
        'mongors nbcore --quiet --port 27000 --eval "JSON.stringify(rs.status())" | grep -v " W NETWORK"',
        {
            ignore_rc: true,
            return_stdout: true,
        },
...
async function test_mongo_status() {
    ...
    try {
        const master = rs_status.members.find(
            member => member.stateStr === 'PRIMARY',
        );
        if (!master) {
            dbg.warn(
                'PRIMARY member not found. rs_status =',
        ...
        if (previous_master) {
            dbg.log2(
                `previous_master=${previous_master}  new_master=${master.name}`,
            );
            // check if master is chaged:
            if (master.name !== previous_master) {
                previous_master = master.name;
                dbg.log0(
                    `MASTER CHANGED to ${master.name} restarting the following services: bg_workers, hosted_agents, s3rver, webserver`,
                );
                await os_utils.exec(
                    'supervisorctl restart bg_workers hosted_agents s3rver webserver',
                );
            }
        } else {
