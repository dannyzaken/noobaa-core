        ...
        .surround(() =>
            P.resolve()
                ...
                .catch(err => {
                    if (err.code === 'ENOENT') {
                        console.log(
                            'license_info: no file yet, generating ...',
                        );
                    } else {
                        console.log(
                            'license_info: error reading and parsing file, re-generating ...',
                            err.stack || err,
                        );
                    }
                    // fork and run main() in separate process
                    return os_utils.fork(
                        __filename,
                        [
                            '--rpms',
                            '--dir',
...
function main() {
    ...
    return P.resolve()
        ...
        .then(
            () =>
                (_.isArray(argv.dir) &&
                    P.map(argv.dir, dir => scanner.scan_dir(dir))) ||
                (_.isString(argv.dir) && scanner.scan_dir(argv.dir)),
        )
        .then(() => {
            console.log(
                `completed scan, found ${info.licenses.length} licenses`,
            );
            if (argv.sort) {
            ...
            const text =
                argv.csv ? format_csv(info) : JSON.stringify(info, null, '  ');
            if (!argv.out) {
        ...
        lines.push(
            [
                ...
                (l.path.includes('/node_modules/') &&
                    'Downloadable - Distributed as source') ||
                    'Downloadable - Distributed as binary',
                // (Q10) Compiled Together? (specify if Company's Products compiled together with the Open Source Material)
                (l.path.includes('/native/') && 'Y') || 'N',
            ]
                .map(x => `"${x || ''}"`)
                .join(',') + '\n',
        );
