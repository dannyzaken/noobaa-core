...
const {
    TieringSpillbackWorker,
} = require('./bg_services/tier_spillback_worker');
const cluster_master = require('./bg_services/cluster_master');
const AgentBlocksVerifier =
    require('./bg_services/agent_blocks_verifier').AgentBlocksVerifier;
const AgentBlocksReclaimer =
    require('./bg_services/agent_blocks_reclaimer').AgentBlocksReclaimer;
...
const {
    LogReplicationScanner,
} = require('./bg_services/log_replication_scanner');
...
const register_bg_worker = (worker, run_batch_function) =>
...
function run_master_workers() {
    ...
    if (
        process.env.NOOBAA_DISABLE_AGGREGATOR !== 'true' &&
        config.MD_AGGREGATOR_ENABLED
    ) {
        register_bg_worker(
            {
                name: 'md_aggregator',
