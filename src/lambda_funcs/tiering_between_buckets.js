...
module.exports.handler = async function (event, context, callback) {
    try {
        ...
        const s3 = new AWS.S3();
        const move_list = await find_objects_to_move(
            s3,
            source_bucket,
            move_objects_older_than_seconds,
        );
...
async function find_objects_to_move(
    s3,
    source_bucket,
    move_objects_older_than_seconds,
) {
    const all_objects = await list_all(s3, source_bucket);
    const sorted_objects = all_objects.sort(
    ...
    for (const obj of sorted_objects) {
        if (
            ...
            move_objects_older_than_seconds * 1000
        ) {
            break;
        }
        move_list.push(obj);
    ...
    while (move_list.length) {
        ...
        try {
            ...
            await s3
                ...
                .promise();
            await s3
                .deleteObject({ Bucket: source_bucket, Key: obj.Key })
                .promise();
