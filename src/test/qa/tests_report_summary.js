...
function arg_to_days(arg, days_or_weeks) {
    ...
    if (Number.isNaN(days)) {
        error(`Invalid number of ${days_or_weeks}: ${argv.weeks}`);
    }
...
class ReportsSummarizer {
    constructor(params) {
        ...
        this.connection_string =
            'mongodb://summarizer:T3st1ng1sFun!@ds139841.mlab.com:39841/test_reports'; // gitleaks:allow
    }
    ...
    async init() {
        ...
        debug(
            `reports collection has ${await this._reports.countDocuments()} documents`,
        );
    }
    ...
    async summarize() {
        console.log(
            `summarizing reports between the dates ${this.since.toDateString()} to ${this.till.toDateString()}`,
        );
    ...
    async get_daily_reports() {
        const docs = await this._reports
            .find(
                {
                    date: _.omitBy(
                        {
        ...
        const reports_by_day = _.groupBy(
            docs,
            doc => `${moment.utc(doc.date).startOf('day')}_${doc.suite_name}`,
        );
        ...
        const aggregated_docs = _.map(reports_by_day, reports =>
            reports.reduce((reduced_doc, current_doc) => {
        ...
        debug(
            `found ${docs.length} documents. reduced to ${aggregated_docs.length}`,
        );
    ...
    summarize_suite(reports, suite_name) {
        ...
        const date_line_Array = [
            'Date',
            ...reports.map(report => report.date.toDateString()),
        ];
        ...
        if (this.normalize) {
            ...
            const final_lines = all_cases.map(test_case => {
                const case_results = reports.map(report => {
                    const { passed_cases, failed_cases /*, didnt_run  */ } =
                        report.results;
            ...
            const passed_lines = all_cases.map(test_case => {
                const case_results = reports.map(
                    report => report.results.passed_cases[test_case] || 0,
                );
                return [
                    `Passed_${test_case}`,
                    ...case_results,
            ...
            const failed_lines = all_cases.map(test_case => {
                const case_results = reports.map(
                    report => report.results.failed_cases[test_case] || 0,
                );
                return [
                    `Failed_${test_case}`,
                    ...case_results,
            ...
            const didnt_run_lines = all_cases.map(test_case => {
                const case_results = reports.map(
                    report => report.results.didnt_run[test_case] || 0,
                );
                return [
                    `didnt_run_${test_case}`,
                    ...case_results,
                    _.sum(case_results),
                ].join(SEPARATOR);
            });
