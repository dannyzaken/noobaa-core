...
mocha.describe('UPLOAD TESTS:', function () {
  ...
  mocha.it(
    'Upload multiple file of size:' + file_size + ' MB in different threads',
    function () {
      const promises = [];
      console.info(
        '> Uploading ' + num_of_files + ' files to Noobaa in differnet threads',
      );
      ...
      return P.all(promises)
        ...
        .then(reply => {
          ...
          assert.equal(
            num_of_created,
            num_of_files,
            'Not all the files were created!, only ' + num_of_created,
          );
          console.info(
            '> Found ' + num_of_created + ' new files in NooBaa as should',
          );
        });
      // .catch((err) => {
      //   console.error(err);
