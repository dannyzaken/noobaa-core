...
mocha.describe('UPLOAD TESTS:', function () {
    ...
    [basic_size, 10 * basic_size, 100 * basic_size].forEach(
        function (file_size) {
            const file_name = basic_name + index;
            index += 1;

            mocha.it(
                'Upload single file of size:' + file_size + ' MB in one thread',
                async function () {
                    console.info(
                        '> Uploading file: ' +
                            file_name +
                            ' to Noobaa with size of:' +
                            file_size +
                            ' MB',
                    );
                    await os_utils.exec(
                    ...
                    const reply = await os_utils.exec(
                        'node ' +
                            process.cwd() +
                            '/src/tools/s3cat.js --endpoint ' +
                            end_point +
                    ...
                    assert.include(
                        reply,
                        file_name,
                        "file wasn't found in NooBaa!",
                    );
                    console.info(
                        '> File: ' + file_name + ' was found on Noobaa',
                    );
                    assert.include(
                        reply,
                        file_name,
    ...
    mocha.it(
        'Upload multiple file of size:' +
            file_size +
            ' MB in different threads',
        function () {
            const promises = [];
            console.info(
                '> Uploading ' +
            ...
            return P.all(promises)
                ...
                .then(reply => {
                    ...
                    assert.equal(
                        num_of_created,
                        num_of_files,
                        'Not all the files were created!, only ' +
                            num_of_created,
                    );
                    console.info(
                        '> Found ' +
                            num_of_created +
                            ' new files in NooBaa as should',
                    );
                });
            // .catch((err) => {
            //   console.error(err);
    ...
    mocha.it(
        'Upload one big file ' + file_size + ' using multi part',
        async function () {
            console.info(
                '> Uploading file: ' +
                    file_name +
