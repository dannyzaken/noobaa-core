  ...
  ({
        ...
        tier._id,
        {
          pools: _.fromPairs(
            _.map(pools_by_tier_id[tier._id], pool => [
              pool._id,
              { valid_for_allocation: true, num_nodes: config.NODES_MIN_COUNT },
            ]),
          ),
    ...
    mocha.describe('rebuild', function () {
      ...
      mocha.it('should rebuild missing EC fragment', function () {
        ...
        assert.strictEqual(mapping.deletions, undefined);
        if (parity_frags) {
          const allocs_per_frag = _.groupBy(mapping.allocations, ({ frag }) =>
            _frag_index(frag),
          );
    ...
    mocha.describe('local replication', function () {
      mocha.it('should replicate to local pool', function () {
        ...
        const should_rebuild = mapper.should_rebuild_chunk_to_local_mirror(
          mapping,
          location_info,
        );
        if (data_placement === 'MIRROR' && first_pools.length > 1) {
          ...
          assert.strictEqual(
            ((mapping.blocks_in_use && mapping.blocks_in_use.length) || 0) +
              ((mapping.allocations && mapping.allocations.length) || 0),
            total_blocks,
          );
          assert_allocations_in_tier(mapping.allocations, first_tier);
        }
      ...
      mocha.it(
        ...
        function () {
          ...
          assert.strictEqual(
            mapper.should_rebuild_chunk_to_local_mirror(mapping, location_info),
            false,
          );
        },
      );
        ...
        function () {
          ...
          const chunk = make_chunk({
            ...
            blocks:
              first_pools.length > 1 ?
                make_blocks({
                  pools: first_pools.slice(0, first_pools.length - 1),
                })
          ...
          const chunk = make_chunk({
            ...
            blocks:
              (
                ...
                make_blocks({
                  pools: first_pools.slice(first_pools.length - 1),
                })
              : [],
          });
          const selected_tier = mapper.select_tier_for_write(
        ...
        mocha.describe(tiering_test, function () {
          ...
          _.forEach(chunk_tests, (map_result, chunk_test) => {
            mocha.it(chunk_test, function () {
              const expected_tier =
                map_result === SECOND ? second_tier : first_tier;
              const unexpected_tier =
                map_result === SECOND ? first_tier : second_tier;
              const chunk = make_chunk({
                _id: 1,
                frags,
                chunk_coder_config,
