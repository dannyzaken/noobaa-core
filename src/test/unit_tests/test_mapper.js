...
coretest.describe_mapper_test_case(
    ...
    ({
        ...
        const first_pools = _.times(num_pools, i => ({
            _id: new mongodb.ObjectId(),
            name: 'first_pool' + i,
        }));
        const second_pools = _.times(num_pools, i => ({
            _id: new mongodb.ObjectId(),
            name: 'second_pool' + i,
        }));
        const external_pools = _.times(num_pools, i => ({
            _id: new mongodb.ObjectId(),
            name: 'external_pool' + i,
        }));
        const pool_by_id = _.keyBy(
            _.concat(first_pools, second_pools, external_pools),
            '_id',
        );
        const first_mirrors =
        ...
        const pools_by_tier_id = _.fromPairs(
            _.map(tiering.tiers, ({ tier }) => [
                tier._id,
                _.flatMap(tier.mirrors, 'spread_pools'),
            ]),
        );
        ...
        const default_tiering_status = _.fromPairs(
            _.map(tiering.tiers, ({ tier }) => [
                tier._id,
                {
                    pools: _.fromPairs(
                        _.map(pools_by_tier_id[tier._id], pool => [
                            pool._id,
                            {
        ...
        const no_space_tiering_status = _.fromPairs(
            _.map(tiering.tiers, ({ tier }) => [
                tier._id,
                {
                    pools: _.fromPairs(
        ...
        const first_mirror_no_space_tiering_status = _.fromPairs(
            _.map(tiering.tiers, ({ tier, order }) => [
                tier._id,
                {
                    pools: _.fromPairs(
                        _.map(pools_by_tier_id[tier._id], pool => [
                    ...
                    mirrors_storage: tier.mirrors.map(mirror =>
                        order === 1 ? FULL_STORAGE : ZERO_STORAGE,
                    ),
                },
            ]),
        );
        const first_mirror_not_valid_tiering_status = _.fromPairs(
            _.map(tiering.tiers, ({ tier, order }) => [
                tier._id,
                {
                    pools: _.fromPairs(
        ...
        const first_tiering_status = _.fromPairs(
            _.map(tiering.tiers, ({ tier, order }) => [
                tier._id,
                {
                    pools: _.fromPairs(
                    ...
                    mirrors_storage: tier.mirrors.map(mirror =>
                        order === 1 ? ZERO_STORAGE : FULL_STORAGE,
                    ),
                },
            ]),
        );
        ...
        mocha.describe('allocations', function () {
            mocha.it('should allocate from first_tier', function () {
                ...
                const selected_tier = mapper.select_tier_for_write(
                    tiering,
                    default_tiering_status,
                );
                const mapping = mapper.map_chunk(
                    chunk,
                    selected_tier,
                    tiering,
                    default_tiering_status,
                ...
                assert.strictEqual(
                    mapping.allocations.length,
                    replicas * total_frags,
                );
            ...
            mocha.it(
                'should allocate from first_tier even if it has no space',
                function () {
                    const chunk = make_chunk({
                        frags,
                        chunk_coder_config,
                    ...
                    const selected_tier = mapper.select_tier_for_write(
                        tiering,
                        first_mirror_no_space_tiering_status,
                    );
                    const mapping = mapper.map_chunk(
                        chunk,
                        selected_tier,
            ...
            mocha.it(
                'should allocate from second_tier when first is not valid',
                function () {
                    const chunk = make_chunk({
                        frags,
                        chunk_coder_config,
                    ...
                    const selected_tier = mapper.select_tier_for_write(
                        tiering,
                        first_mirror_not_valid_tiering_status,
                    );
                    const mapping = mapper.map_chunk(
                        chunk,
                        selected_tier,
        ...
        mocha.describe('deletions', function () {
            ...
            mocha.it(
                'should remove blocks from pools not in the tier',
                function () {
                    const external_blocks = make_blocks({
                        pool: external_pools[0],
                    });
                    const chunk = make_chunk({
                        _id: 1,
                        frags,
                    ...
                    mapping.deletions.forEach(block =>
                        assert(external_blocks.includes(block)),
                    );
                },
            );

            mocha.it('should delete inaccessible block', function () {
                const inaccessible_blocks = make_blocks({
                    count: 1,
                    readable: false,
                });
                const chunk = make_chunk({
                ...
                mapping.deletions.forEach(block =>
                    assert(inaccessible_blocks.includes(block)),
                );
            });
        });
        ...
        mocha.describe('rebuild', function () {
            ...
            mocha.it(
                'should first allocate missing and only then delete inaccessible block',
                function () {
                    const chunk = make_chunk({
                        _id: 1,
                        frags,
                    ...
                    if (total_blocks === total_frags) {
                        ...
                        chunk.blocks = _.concat(
                            chunk.blocks,
                            make_blocks({ allocations: mapping.allocations }),
                        );
                        assert.strictEqual(
                            chunk.blocks.length,
                            total_blocks + 1,
                        );
                        const selected_tier2 = mapper.select_tier_for_write(
                            tiering,
                            default_tiering_status,
                        );
                        const mapping2 = mapper.map_chunk(
                            chunk,
                            selected_tier2,
                            tiering,
                            default_tiering_status,
            ...
            mocha.it('should rebuild missing EC fragment', function () {
                ...
                const chunk = make_chunk({
                    ...
                    chunk_coder_config,
                    blocks: make_blocks({
                        count: data_frags * total_replicas,
                        frags: avail_frags,
                    }),
                });
                const selected_tier = mapper.select_tier_for_write(
                ...
                assert.strictEqual(mapping.deletions, undefined);
                if (parity_frags) {
                    const allocs_per_frag = _.groupBy(
                        mapping.allocations,
                        ({ frag }) => _frag_index(frag),
                    );
                    assert.deepStrictEqual(
                        Object.keys(allocs_per_frag).sort(),
                        missing_frags.map(_frag_index).sort(),
                        `Object.keys(allocs_per_frag).sort() === missing_frags.map(_frag_index).sort()`,
                    );
                    assert.deepStrictEqual(
                        _.uniq(mapping.missing_frags).map(_frag_index).sort(),
                        missing_frags.map(_frag_index).sort(),
        ...
        mocha.describe('local replication', function () {
            mocha.it('should replicate to local pool', function () {
                ...
                const chunk = make_chunk({
                    ...
                    chunk_coder_config,
                    blocks:
                        first_pools.length > 1 ?
                            make_blocks({ pools: first_pools.slice(1) })
                        :   [],
                });
                const selected_tier = mapper.select_tier_for_write(
                    tiering,
                ...
                if (data_placement === 'MIRROR' && first_pools.length > 1) {
                    ...
                    assert.strictEqual(
                        ((mapping.blocks_in_use &&
                            mapping.blocks_in_use.length) ||
                            0) +
                            ((mapping.allocations &&
                                mapping.allocations.length) ||
                                0),
            ...
            mocha.it(
                'should replicate to pool with the same region',
                function () {
                    const location_info = {
                        region: 'REGION-X',
                    };
            ...
            mocha.it(
                'should replicate to pool with the same region even if first pool is full',
                function () {
                    const location_info = {
                        region: 'REGION-X',
                    };
            ...
            mocha.it(
                'should not replicate to pool with the different region',
                function () {
                    const location_info = {
                        region: 'REGION-Y',
                    };
            ...
            mocha.it(
                'should not replicate to local pool if local already has blocks',
                function () {
                    const location_info = {
                        pool_id: String(first_pools[0]._id),
                    };
                    const chunk = make_chunk({
                        ...
                        blocks:
                            first_pools.length > 1 ?
                                make_blocks({
                                    pools: first_pools.slice(
                                        0,
                                        first_pools.length - 1,
                                    ),
                                })
                            :   make_blocks({ pool: first_pools[0] }),
                    });
                    const selected_tier = mapper.select_tier_for_write(
                        tiering,
                        default_tiering_status,
            ...
            mocha.it(
                'should not replicate to local pool if no allocation needed',
                function () {
                    const location_info = {
                        pool_id: String(first_pools[0]._id),
                    };
            ...
            mocha.it(
                'should not replicate to local pool if pool is down',
                function () {
                    const location_info = {
                        pool_id: String(first_pools[0]._id),
                    };
                    const chunk = make_chunk({
                        ...
                        blocks:
                            (
                                first_pools.length > 1 // will put blocks only on last pool - should allocate only to the empty pool, or none - pool[0] doesn't suppose to appear
                            ) ?
                                make_blocks({
                                    pools: first_pools.slice(
                                        first_pools.length - 1,
                ...
                mocha.describe(tiering_test, function () {
                    ...
                    _.forEach(chunk_tests, (map_result, chunk_test) => {
                        mocha.it(chunk_test, function () {
                            const expected_tier =
                                map_result === SECOND ? second_tier : (
                                    first_tier
                                );
                            const unexpected_tier =
                                map_result === SECOND ? first_tier : (
                                    second_tier
                                );
                            const chunk = make_chunk({
                                _id: 1,
        ...
        function make_block(params = {}, i = 0) {
            ...
            const pool_i =
                Math.floor(i / frags_to_use.length) % pools_to_use.length;
        ...
        function assert_allocations_in_tier(allocations, tier) {
            ...
            _.forEach(allocations, ({ pools }) =>
                _.forEach(pools, pool =>
                    assert(
                        _.includes(tier_pools, pool),
                        `assert_allocations_in_tier expected tier ${tier.name} found pool ${pool.name}`,
                    ),
                ),
            );
        }
        ...
        function assert_deletions_in_tier(deletions, tier) {
            ...
            _.forEach(deletions, block =>
                assert(
                    _.includes(tier_pools, block.pool),
                    `assert_deletions_in_tier expected tier ${tier.name} found pool ${pool_by_id[block.pool].name}`,
                ),
            );
        }
    },
