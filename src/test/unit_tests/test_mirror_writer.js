...
function get_mirror_writer({
    params,
    chunks_builder,
    retry_chunks_builder,
} = {}) {
...
mocha.describe('mirror_writer', function () {
    mocha.describe('run_batch', function () {
        mocha.it(
            'should return empty delay when nothing to do',
            async function () {
                const mw = get_mirror_writer({
                    chunks_builder: empty_chunks_builder,
                });
        ...
        mocha.it(
            'should return batch delay after successful run',
            async function () {
                const mw = get_mirror_writer({
                    chunks_builder: successful_chunks_builder,
                });
                const delay = await mw.run_batch();
        ...
        mocha.it(
            'should return error delay after unsuccessful run',
            async function () {
                const mw = get_mirror_writer({
                    chunks_builder: unsuccessful_chunks_builder,
                });
                const delay = await mw.run_batch();
        ...
        mocha.it(
            'should return error delay after bucket_chunks_builder throws',
            async function () {
                const mw = get_mirror_writer({
                    chunks_builder: throwing_chunks_builder,
                });
                const delay = await mw.run_batch();
                assert.strictEqual(delay, config.MIRROR_WRITER_ERROR_DELAY);
            },
        ...
        mocha.it(
            'should get a retry scanner with the failed range after failure',
            async function () {
                const mw = get_mirror_writer();
                const get_builder = sinon.stub();
                get_builder
                    .onFirstCall()
                    .returns(unsuccessful_chunks_builder)
                    .onSecondCall()
                    .returns(successful_chunks_builder);
                ...
                assert(
                    get_builder.secondCall.calledWithExactly({
        ...
        mocha.it(
            'should not get retry scanner if no errors',
            async function () {
                const mw = get_mirror_writer();
                const get_builder = sinon.stub();
        ...
        mocha.it(
            'should store only start marker when there are no errors',
            async function () {
                const mw = get_mirror_writer({
                    chunks_builder: successful_chunks_builder,
                });
                ...
                await mw.run_batch();
                assert(
                    mw._update_markers_in_system_store.calledWithExactly({
                        start_marker: test_chunks[test_chunks.length - 1],
                    }),
                );
            },
        );
