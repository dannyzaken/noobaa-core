...
mocha.describe('bucketspace namespace_fs - versioning', function () {
  ...
  const tmp_fs_root = path.join(
    TMP_PATH,
    'test_bucket_namespace_fs_versioning',
  );
  ...
  mocha.describe('delete object version id - versioning enabled', function () {
    ...
    const full_delete_path_null =
      tmp_fs_root + '/' + delete_object_test_bucket_null;
    const full_delete_path_dm =
      tmp_fs_root + '/' + delete_object_test_bucket_dm;
        ...
        async function () {
          ...
          mocha.it(
            ...
            async function () {
              ...
              const cur_version_id2 = await stat_and_get_version_id(
                full_delete_path,
                key1,
              );
          ...
          mocha.it(
            ...
            async function () {
              const cur_version_id = await stat_and_get_version_id(
                full_delete_path,
                key1,
              );
              await account_with_access.deleteObject({
              ...
              await fs_utils.file_must_not_exist(
                path.join(full_delete_path, key1 + '_' + cur_version_id),
              );
              const max_version1 = await find_max_version_past(
                full_delete_path,
                key1,
          ...
          mocha.it(
            ...
            async function () {
              ...
              const latest_dm_version_id1 = await find_max_version_past(
                full_delete_path,
                key1,
                '',
              );
              ...
              const latest_dm_version_id2 = await find_max_version_past(
                full_delete_path,
                key1,
                '',
              );
              assert.equal(latest_dm_version_id1, latest_dm_version_id2);
              const version_deleted = await version_file_must_not_exists(
                full_delete_path,
                key1,
                '',
                upload_res_arr[1].VersionId,
          ...
          mocha.it(
            ...
            async function () {
              ...
              const second_max_version1 = await find_max_version_past(
                full_delete_path_dm,
                key1,
                '',
                [max_version],
              ...
              const new_latest_ver_id = await stat_and_get_version_id(
                full_delete_path_dm,
                key1,
              );
          ...
          mocha.it(
            ...
            async function () {
              ...
              const ltst_version_id2 = await stat_and_get_version_id(
                full_delete_path_dm,
                key1,
              );
  ...
  mocha.describe(
    ...
    function () {
      ...
      const full_delete_path =
        tmp_fs_root + '/' + delete_object_test_bucket_reg;
      const full_delete_path_null =
        tmp_fs_root + '/' + delete_object_test_bucket_null;
      const full_delete_path_dm =
        tmp_fs_root + '/' + delete_object_test_bucket_dm;
        ...
        async function () {
          ...
          mocha.it(
            ...
            async function () {
              ...
              const put_res_null = await account_with_access.putObject({
                Bucket: delete_object_test_bucket_null,
                Key: key1,
                Body: body1,
              });
              ...
              const put_res_regular = await account_with_access.putObject({
                Bucket: delete_object_test_bucket_null,
                Key: key1,
                Body: body1,
              });
    ...
    mocha.it(
      ...
      async function () {
        ...
        for (let i = 0; i < 300; i++) {
          ...
          if (i % 2 === 1) {
            arr.push({ Key: key2, VersionId: put_res2[i].VersionId });
          }
        }
        const delete_res = await account_with_access.deleteObjects({
          Bucket: delete_multi_object_test_bucket,
    ...
    mocha.it(
      ...
      async function () {
        ...
        for (let i = 0; i < 100; i++) {
          ...
          if (i % 2 === 0) {
            assert.equal(delete_res.Deleted[i].DeleteMarker, undefined);
          }
        }
        const versions_dir = path.join(full_multi_delete_path, '.versions');
        const versions = await nb_native().fs.readdir(
...
async function find_max_version_past(full_path, key, dir, skip_list) {
  ...
  try {
    ...
    for (const entry of versions) {
      if (
        skip_list ? !skip_list.includes(entry.name.slice(key.length + 1)) : true
      ) {
...
function get_version_id_by_stat(stat) {
  return (
    'mtime-' + stat.mtimeNsBigint.toString(36) + '-ino-' + stat.ino.toString(36)
  );
}
...
async function generate_nsfs_account(options = {}) {
  const { uid, gid, new_buckets_path, nsfs_only, admin, default_resource } =
    options;
  if (admin) {
