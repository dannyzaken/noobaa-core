...
const {
    ...
    generate_nsfs_account,
    get_new_buckets_path_by_test_env,
    invalid_nsfs_root_permissions,
    generate_s3_client,
    get_coretest_path,
...
const new_bucket_path_param = get_new_buckets_path_by_test_env(
    tmp_fs_root,
    '/',
);
...
mocha.describe('bucketspace namespace_fs - versioning', function () {
    ...
    const suspended_versions_path = path.join(
        suspended_full_path,
        '.versions/',
    );
    let s3_uid5;
    ...
    const suspended_dir1_versions_path = path.join(
        suspended_full_path,
        dir1,
        '.versions/',
    );
    ...
    mocha.before(async function () {
        ...
        // create accounts
        let res = await generate_nsfs_account(
            rpc_client,
            EMAIL,
            new_bucket_path_param,
            { admin: true },
        );
        s3_admin = generate_s3_client(
            res.access_key,
            res.secret_key,
            CORETEST_ENDPOINT,
        );
    ...
    mocha.describe('put/get versioning', function () {
        mocha.it(
            'put object - versioning disabled - to be enabled',
            async function () {
                await s3_uid6.putObject({
                    Bucket: bucket_name,
                    Key: disabled_key,
                    Body: body1,
                });
        ...
        mocha.it(
            'put object - versioning disabled - to be suspended',
            async function () {
                await s3_uid6.putObject({
                    Bucket: suspended_bucket_name,
                    Key: disabled_key,
        ...
        mocha.it(
            'set bucket versioning - Enabled - should fail - no permissions',
            async function () {
                try {
                    await s3_uid5.putBucketVersioning({
                        Bucket: bucket_name,
                    ...
                    assert.fail(
                        `put bucket versioning succeeded for account without permissions`,
                    );
                } catch (err) {
        ...
        mocha.it(
            'set bucket versioning - Enabled - admin - should fail - no permissions',
            async function () {
                ...
                try {
                    await s3_admin.putBucketVersioning({
                        Bucket: bucket_name,
                        VersioningConfiguration: {
                            MFADelete: 'Disabled',
                            Status: 'Enabled',
        ...
        mocha.it('set bucket versioning - Enabled', async function () {
            await s3_uid6.putBucketVersioning({
                Bucket: bucket_name,
                VersioningConfiguration: {
                    MFADelete: 'Disabled',
                    Status: 'Enabled',
            ...
            const res = await s3_uid6.getBucketVersioning({
                Bucket: bucket_name,
            });
        ...
        mocha.it(
            'set bucket versioning - Suspended - should fail - no permissions',
            async function () {
                try {
                    await s3_uid5.putBucketVersioning({
                        Bucket: suspended_bucket_name,
        ...
        mocha.it(
            'set bucket versioning - Suspended - admin - should fail - no permissions',
            async function () {
    ...
    mocha.describe('versioning enabled', function () {
        mocha.describe('put object', function () {
            mocha.it(
                'put object 1st time - no existing objects in bucket',
                async function () {
                    const res = await s3_uid6.putObject({
                        Bucket: bucket_name,
                        Key: key1,
                        Body: body1,
                    });
                    const comp_res = await compare_version_ids(
                        full_path,
                        key1,
                        res.VersionId,
                    );
            ...
            mocha.it(
                'put object 2nd time - versioning enabled',
                async function () {
                    const prev_version_id = await stat_and_get_version_id(
                        full_path,
                        key1,
                    );
                    const res = await s3_uid6.putObject({
                    ...
                    assert.ok(comp_res);
                    const exist = await version_file_exists(
                        full_path,
                        key1,
                        '',
                        prev_version_id,
            ...
            mocha.it(
                'put object 2nd time - versioning enabled - disabled key',
                async function () {
                    const prev_version_id = await stat_and_get_version_id(
                        full_path,
                        disabled_key,
            ...
            mocha.it(
                'put object 3rd time - versioning enabled - disabled key',
                async function () {
                    const prev_version_id = await stat_and_get_version_id(
                        full_path,
                        disabled_key,
                    ...
                    assert.ok(comp_res);
                    let exist = await version_file_exists(
                        full_path,
                        disabled_key,
                        '',
                        prev_version_id,
                    ...
                    assert.ok(exist);
                    exist = await version_file_exists(
                        full_path,
                        disabled_key,
                        '',
                        'null',
            ...
            mocha.it(
                'put object 1st time - versioning enabled - nested',
                async function () {
                    const res = await s3_uid6.putObject({
                        Bucket: bucket_name,
                        Key: nested_key1,
            ...
            mocha.it(
                'put object 2nd time - versioning enabled - nested',
                async function () {
                    const prev_version_id = await stat_and_get_version_id(
                        full_path,
                        nested_key1,
        ...
        mocha.describe('mpu object', function () {
            mocha.it(
                'mpu object 1st time - versioning enabled',
                async function () {
                    const mpu_res = await s3_uid6.createMultipartUpload({
                        Bucket: bucket_name,
                        Key: mpu_key1,
                    });
            ...
            mocha.it(
                'mpu object 2nd time - versioning enabled',
                async function () {
                    const prev_version_id = await stat_and_get_version_id(
                        full_path,
                        mpu_key1,
    ...
    mocha.describe(
        'put object - versioning suspended - simple cases',
        function () {
            ...
            mocha.before(
                'set bucket versioning - Suspended',
                async function () {
                    await s3_uid6.putBucketVersioning({
                        Bucket: suspended_bucket_name,
                        VersioningConfiguration: {
            ...
            mocha.describe('put object', function () {
                mocha.it(
                    'put object 1st time - versioning suspended',
                    async function () {
                        const res = await s3_uid6.putObject({
                            Bucket: suspended_bucket_name,
                            Key: key2,
                        ...
                        assert.ok(comp_res);
                        const exist = await fs_utils.file_not_exists(
                            suspended_versions_path,
                        );
                        assert.ok(exist);
                    },
                );
                ...
                mocha.it(
                    'put object 2nd time - versioning suspended',
                    async function () {
                        const res = await s3_uid6.putObject({
                            Bucket: suspended_bucket_name,
                            Key: key2,
                ...
                mocha.it(
                    'put object 2nd time - versioning suspended - disabled key',
                    async function () {
                        const res = await s3_uid6.putObject({
                            Bucket: suspended_bucket_name,
                            Key: disabled_key,
                ...
                mocha.it(
                    'put object 1st time - versioning suspended - nested',
                    async function () {
                        const res = await s3_uid6.putObject({
                            Bucket: suspended_bucket_name,
                            Key: nested_key1,
                ...
                mocha.it(
                    'put object 2nd time - versioning suspended - nested',
                    async function () {
                        const res = await s3_uid6.putObject({
                            Bucket: suspended_bucket_name,
                            Key: nested_key1,
            ...
            mocha.describe('mpu object', function () {
                mocha.it(
                    'mpu object 1st time - versioning suspended',
                    async function () {
                        const mpu_res = await s3_uid6.createMultipartUpload({
                            Bucket: suspended_bucket_name,
                            Key: mpu_key1,
                ...
                mocha.it(
                    'mpu object 2nd time - versioning suspended',
                    async function () {
                        const prev_version_id = await stat_and_get_version_id(
                            suspended_full_path,
                            mpu_key1,
                        ...
                        assert.ok(comp_res);
                        const exist = await version_file_must_not_exists(
                            suspended_full_path,
                            mpu_key1,
                            '',
                            prev_version_id,
    ...
    mocha.describe(
        'put object - versioning suspended - should have only one null version ID',
        function () {
            mocha.before(
                'put object 2nd time - versioning enabled',
                async function () {
            ...
            mocha.it(
                'put object 3rd time - versioning suspended',
                async function () {
                    await s3_uid6.putBucketVersioning({
                        Bucket: suspended_bucket_name,
                        VersioningConfiguration: {
                    ...
                    exist = await version_file_must_not_exists(
                        suspended_full_path,
                        disabled_key2,
                        '',
                        'null',
    ...
    mocha.describe(
        'put object - versioning suspended - unique version ID should move to .version/ directory',
        function () {
            const key2 = 'soda.txt';
            ...
            mocha.it(
                ...
                async function () {
                    ...
                    const comp_prev =
                        check_enable_version_format(prev_version_id);
                    assert.ok(comp_prev);
                    const res = await s3_uid6.putObject({
                        Bucket: suspended_bucket_name,
                        Key: key2,
    ...
    mocha.describe(
        'copy object - versioning enabled - nsfs copy fallback flow',
        function () {
            mocha.it(
                'copy object - target bucket versioning enabled - 1st',
                async function () {
                    const res = await s3_uid6.copyObject({
                        Bucket: bucket_name,
                        Key: copied_key1,
            ...
            mocha.it(
                'copy object - target bucket versioning enabled - 2nd',
                async function () {
                    const prev_version_id = await stat_and_get_version_id(
                        full_path,
                        copied_key1,
            ...
            mocha.it(
                'copy object latest - source bucket versioning enabled',
                async function () {
                    ...
                    const body_comp_res = await get_obj_and_compare_data(
                        s3_uid6,
                        bucket_name,
                        key,
                        body2,
            ...
            mocha.it(
                'copy object latest & versionId - source bucket versioning enabled - version does not exist',
                async function () {
                    const key = 'copied_key3.txt';
                    const non_exist_version = 'mtime-dsbfjb-ino-sjfhjsa';
                    try {
                        await s3_uid6.copyObject({
                            Bucket: bucket_name,
                            Key: key,
                            CopySource: `${bucket_name}/${key1}?versionId=${non_exist_version}`,
                        });
                        assert.fail(
                            'copy non existing version should have failed',
                        );
                    } catch (err) {
            ...
            mocha.it(
                'copy object latest & versionId - source bucket versioning enabled',
                async function () {
                    const key = 'copied_key3.txt';
                    const res = await s3_uid6.copyObject({
                        Bucket: bucket_name,
            ...
            mocha.it(
                'copy object version id - source bucket versioning enabled',
                async function () {
            ...
            mocha.it(
                'copy object version null - version is in .versions/',
                async function () {
                    const res = await s3_uid6.copyObject({
                        Bucket: bucket_name,
                        Key: copied_key5,
            ...
            mocha.it(
                'copy object version null - no version null - should fail',
                async function () {
                    try {
                        await s3_uid6.copyObject({
                            Bucket: bucket_name,
            ...
            mocha.it(
                'copy object - version does not exist - should fail',
                async function () {
                    try {
                        await s3_uid6.copyObject({
                            Bucket: bucket_name,
            ...
            mocha.it(
                'copy object version null from disabled bucket - version is in parent',
                async function () {
            ...
            mocha.it(
                'copy object version null from enabled bucket - version is in parent',
                async function () {
            ...
            mocha.it(
                'delete object latest - non existing version',
                async function () {
                    const non_exist_version = 'mtime-dsad-ino-sdfasd';
                    const res = await s3_uid6.deleteObject({
                        Bucket: bucket_name,
                        Key: disabled_key,
                        VersionId: non_exist_version,
                    });
            ...
            mocha.it(
                'delete object latest - create dm & move latest -> .versions/',
                async function () {
                    const prev_version_id = await stat_and_get_version_id(
                        full_path,
                        disabled_key,
                    );
                    const max_version1 = await find_max_version_past(
                        full_path,
                        disabled_key,
                        '',
                    );
                    ...
                    await fs_utils.file_must_not_exist(
                        path.join(full_path, disabled_key),
                    );
                    const exist = await version_file_exists(
                        full_path,
                        disabled_key,
                    ...
                    assert.ok(exist);
                    const max_version2 = await find_max_version_past(
                        full_path,
                        disabled_key,
                        '',
                    );
                    assert.notEqual(max_version2, max_version1);
                    const is_dm = await is_delete_marker(
                        full_path,
                        '',
                        disabled_key,
                        max_version2,
            ...
            mocha.it(
                'delete object - create dm & move latest -> .versions/ - 1st',
                async function () {
                    const prev_version_id = await stat_and_get_version_id(
                        full_path,
                        key1,
            ...
            mocha.it(
                'delete object - create dm & move latest -> .versions/ - 2nd time',
                async function () {
                    const max_version1 = await find_max_version_past(
                        full_path,
                        key1,
    ...
    mocha.describe(
        'copy object (latest version) - versioning suspended - nsfs copy fallback flow',
        function () {
            const is_enabled = false;
            ...
            mocha.it(
                'copy object - target bucket versioning suspended - 1st',
                async function () {
                    const res = await s3_uid6.copyObject({
                        Bucket: suspended_bucket_name,
                        Key: copied_key,
            ...
            mocha.it(
                'copy object - target bucket versioning suspended - 2nd',
                async function () {
                    const prev_version_id = await stat_and_get_version_id(
                        suspended_full_path,
                        copied_key,
            ...
            mocha.it(
                'copy object latest - source bucket versioning suspended',
                async function () {
    ...
    mocha.describe(
        'copy object by version id - versioning suspended - nsfs copy fallback flow',
        function () {
            const is_enabled = false;
            ...
            mocha.before(
                'set bucket versioning - Enabled and then Suspended',
                async function () {
                    await s3_uid6.putObject({
                        Bucket: suspended_bucket_name,
                        Key: key_to_copy3,
                    ...
                    const res_put = await s3_uid6.putObject({
                        Bucket: suspended_bucket_name,
                        Key: key_to_copy,
                        Body: body_of_copied_key,
                    });
                    version_id_to_copy = res_put.VersionId;
                    const res_put_latest = await s3_uid6.putObject({
                        Bucket: suspended_bucket_name,
                        Key: key_to_copy,
                        Body: body_of_copied_key_latest,
                    });
            ...
            mocha.it(
                'copy object latest & versionId - source bucket versioning suspended',
                async function () {
            ...
            mocha.it(
                'copy object version id - source bucket versioning suspended',
                async function () {
            ...
            mocha.it(
                'copy object version null - versioning suspended - no version null - should fail',
                async function () {
            ...
            mocha.it(
                'copy object version null - version is in .versions/ - versioning suspended',
                async function () {
    ...
    mocha.describe('object tagging', function () {
        ...
        mocha.it('put object tagging - no versionId', async function () {
            await s3_uid6.putObjectTagging({
                Bucket: suspended_bucket_name,
                Key: tagging_key,
                Tagging: tag_set1,
            });
            const res = await s3_uid6.getObjectTagging({
                Bucket: suspended_bucket_name,
                Key: tagging_key,
            });
        ...
        mocha.it('put object tagging - specific versionId', async function () {
            ...
            const version_res = await s3_uid6.getObjectTagging({
                Bucket: suspended_bucket_name,
                Key: tagging_key,
            });
        ...
        mocha.it('delete object tagging - no versionId', async function () {
            await s3_uid6.deleteObjectTagging({
                Bucket: suspended_bucket_name,
                Key: tagging_key,
            });
            const res = await s3_uid6.getObjectTagging({
        ...
        mocha.it(
            'delete object tagging - specific versionId',
            async function () {
                await s3_uid6.deleteObjectTagging({
                    Bucket: suspended_bucket_name,
                    Key: tagging_key,
    ...
    mocha.describe('delete object latest - versioning suspended', function () {
        ...
        mocha.it(
            ...
            async function () {
                ...
                const max_version_before_delete = await find_max_version_past(
                    suspended_full_path,
                    key_to_delete,
                    '',
                );
                ...
                let exist = await fs_utils.file_not_exists(
                    path.join(suspended_full_path, key_to_delete),
                );
                assert.ok(exist);
                exist = await version_file_exists(
                    suspended_full_path,
                ...
                assert.ok(exist);
                const max_version_after_delete = await find_max_version_past(
                    suspended_full_path,
                    key_to_delete,
                    '',
                );
                assert.notEqual(
                    max_version_after_delete,
                    max_version_before_delete,
                );
                const is_dm = await is_delete_marker(
                    suspended_full_path,
                    '',
        ...
        mocha.it(
            'delete object - create dm & remove latest',
            async function () {
                const prev_version_id = await stat_and_get_version_id(
                    suspended_full_path,
                    key_to_delete2,
                ...
                let exist = await fs_utils.file_not_exists(
                    path.join(suspended_full_path, key_to_delete2),
                );
                assert.ok(exist);
                exist = await version_file_exists(
                    suspended_full_path,
        ...
        mocha.it(
            'delete object when the latest is a dm with a unique version ID - should create another dm with null version ID',
            async function () {
                await s3_uid6.putBucketVersioning({
                    Bucket: suspended_bucket_name,
                    VersioningConfiguration: {
                ...
                const prev_dm = await s3_uid6.deleteObject({
                    Bucket: suspended_bucket_name,
                    Key: key_to_delete3,
                });
                assert.equal(prev_dm.DeleteMarker, true);
                const comp_prev = check_enable_version_format(
                    prev_dm.VersionId,
                );
                assert.ok(comp_prev);
                ...
                assert.ok(comp_cur);
                const latest_dm_version = await find_max_version_past(
                    suspended_full_path,
                    key_to_delete3,
                    '',
                );
                ...
                const version_path = path.join(
                    suspended_full_path,
                    '.versions',
                    key_to_delete3 + '_' + latest_dm_version,
                );
                const version_info = await stat_and_get_all(version_path, '');
                assert.equal(
                    version_info.xattr[XATTR_PREV_VERSION_ID],
                    prev_dm.VersionId,
                );
                assert.equal(
                    version_info.xattr[XATTR_VERSION_ID],
                    NULL_VERSION_ID,
                );
            },
        );
    ...
    mocha.describe(
        'delete object version id - versioning enabled',
        function () {
            const delete_object_test_bucket_reg =
                'delete-object-test-bucket-reg';
            const delete_object_test_bucket_null =
                'delete-object-test-bucket-null';
            const delete_object_test_bucket_dm = 'delete-object-test-bucket-dm';

            const full_delete_path = path.join(
                tmp_fs_root,
                delete_object_test_bucket_reg,
            );
            const full_delete_path_null = path.join(
                tmp_fs_root,
                delete_object_test_bucket_null,
            );
            const full_delete_path_dm = path.join(
                tmp_fs_root,
                delete_object_test_bucket_dm,
            );

            let account_with_access;
            mocha.describe('delete object - versioning enabled', function () {
                mocha.describe(
                    'delete object - regular version - versioning enabled',
                    async function () {
                        mocha.before(async function () {
                            const res = await generate_nsfs_account(
                                rpc_client,
                                EMAIL,
                                new_bucket_path_param,
                                { default_resource: nsr },
                            );
                            account_with_access = generate_s3_client(
                                res.access_key,
                                res.secret_key,
                                CORETEST_ENDPOINT,
                            );
                            await account_with_access.createBucket({
                                Bucket: delete_object_test_bucket_reg,
                            });
                            await put_allow_all_bucket_policy(
                                account_with_access,
                                delete_object_test_bucket_reg,
                            );
                            await account_with_access.createBucket({
                                Bucket: delete_object_test_bucket_null,
                            });
                            await put_allow_all_bucket_policy(
                                account_with_access,
                                delete_object_test_bucket_null,
                            ...
                            await account_with_access.createBucket({
                                Bucket: delete_object_test_bucket_dm,
                            });
                            await put_allow_all_bucket_policy(
                                account_with_access,
                                delete_object_test_bucket_dm,
                        ...
                        mocha.it(
                            'delete version id - fake id - nothing to remove',
                            async function () {
                                const max_version1 =
                                    await find_max_version_past(
                                        full_path,
                                        key1,
                                        '',
                                    );
                                await account_with_access.deleteObject({
                                    Bucket: delete_object_test_bucket_reg,
                                    Key: key1,
                                    VersionId: 'mtime-123-ino-123',
                                });
                                const max_version2 =
                                    await find_max_version_past(
                                        full_path,
                        ...
                        mocha.it(
                            'delete object version id - latest - second latest is null version',
                            async function () {
                                const upload_res_arr =
                                    await upload_object_versions(
                                        account_with_access,
                                        delete_object_test_bucket_reg,
                                        key1,
                                        ['null', 'regular'],
                                    );
                                const cur_version_id1 =
                                    await stat_and_get_version_id(
                                        full_delete_path,
                                        key1,
                                    );
                                ...
                                const delete_res =
                                    await account_with_access.deleteObject({
                                        Bucket: delete_object_test_bucket_reg,
                                        Key: key1,
                                        VersionId: upload_res_arr[1].VersionId,
                                    });
                                assert.equal(
                                    delete_res.VersionId,
                                    cur_version_id1,
                                );

                                const cur_version_id2 =
                                    await stat_and_get_version_id(
                                        full_delete_path,
                                        key1,
                                    );
                        ...
                        mocha.it(
                            'delete object version id - latest - second latest is delete marker version ',
                            async function () {
                                const upload_res_arr =
                                    await upload_object_versions(
                                        account_with_access,
                                ...
                                assert.equal(
                                    upload_res_arr[2].VersionId,
                                    cur_version_id1,
                                );
                                const cur_ver_info = await stat_and_get_all(
                                    full_delete_path,
                                    key1,
                                );
                                assert.equal(
                                    cur_ver_info.xattr[XATTR_PREV_VERSION_ID],
                                    max_version0,
                                );
                                const is_dm = await is_delete_marker(
                                    full_delete_path,
                                ...
                                const delete_res =
                                    await account_with_access.deleteObject({
                                        Bucket: delete_object_test_bucket_reg,
                                        Key: key1,
                                        VersionId: upload_res_arr[2].VersionId,
                                    });
                                assert.equal(
                                    delete_res.VersionId,
                                    cur_version_id1,
                        ...
                        mocha.it(
                            'delete object version id - in .versions/',
                            async function () {
                                const put_res =
                                    await account_with_access.putObject({
                                        Bucket: delete_object_test_bucket_reg,
                                        Key: key1,
                                        Body: body1,
                                    });
                                ...
                                const exist =
                                    await version_file_must_not_exists(
                                        full_delete_path,
                                        key1,
                                        '',
                                        put_res.VersionId,
                        ...
                        mocha.it(
                            'delete object version id - latest - no second latest',
                            async function () {
                                const cur_version_id =
                                    await stat_and_get_version_id(
                                        full_delete_path,
                                        key1,
                                    );
                                await account_with_access.deleteObject({
                                    Bucket: delete_object_test_bucket_reg,
                                    Key: key1,
                                    VersionId: cur_version_id,
                                });
                                await fs_utils.file_must_not_exist(
                                    path.join(
                                        full_delete_path,
                                        key1 + '_' + cur_version_id,
                                    ),
                                );
                                const max_version1 =
                                    await find_max_version_past(
                        ...
                        mocha.it(
                            "delete object version id - in .versions/ 2 - latest exist and it's a regular version",
                            async function () {
                                const upload_res_arr =
                                    await upload_object_versions(
                                        account_with_access,
                        ...
                        mocha.it(
                            "delete object version id - in .versions/ 3 - latest exist and it's a delete marker",
                            async function () {
                                const upload_res_arr =
                                    await upload_object_versions(
                                        account_with_access,
                                        delete_object_test_bucket_reg,
                                        key1,
                                        ['regular', 'regular', 'delete_marker'],
                                    );

                                await fs_utils.file_must_not_exist(
                                    path.join(full_delete_path, key1),
                                );
                                const latest_dm_version_id1 =
                                    await find_max_version_past(
                                        full_delete_path,
                                        key1,
                                        '',
                                ...
                                assert.equal(
                                    latest_dm_version_id1,
                                    latest_dm_version_id2,
                                );
                                const version_deleted =
                                    await version_file_must_not_exists(
                                        full_delete_path,
                                        key1,
                                        '',
                        ...
                        mocha.it(
                            'delete object version id - latest - second latest is regular version ',
                            async function () {
                                const upload_res_arr =
                                    await upload_object_versions(
                                        account_with_access,
                        ...
                        mocha.it(
                            'delete object version null - latest, no second latest',
                            async function () {
                                const upload_res_arr =
                                    await upload_object_versions(
                                        account_with_access,
                                ...
                                await fs_utils.file_must_not_exist(
                                    path.join(full_delete_path_null, key1),
                                );
                                const max_version1 =
                                    await find_max_version_past(
                                        full_delete_path_null,
                        ...
                        mocha.it(
                            'delete object version null - version is in .versions/',
                            async function () {
                                const upload_res_arr =
                                    await upload_object_versions(
                                        account_with_access,
                        ...
                        mocha.it(
                            'delete object version delete marker - latest - second latest is a null version',
                            async function () {
                                await upload_object_versions(
                                    account_with_access,
                                    delete_object_test_bucket_dm,
                                ...
                                await fs_utils.file_must_not_exist(
                                    path.join(full_delete_path_dm, key1),
                                );
                                const max_version = await find_max_version_past(
                                    full_delete_path_dm,
                                    key1,
                                    '',
                                );
                                const second_max_version1 =
                                    await find_max_version_past(
                                        full_delete_path_dm,
                                        key1,
                                        '',
                                ...
                                assert.equal(max_version1, undefined);
                                await fs_utils.file_must_exist(
                                    path.join(full_delete_path_dm, key1),
                                );
                                await version_file_must_not_exists(
                                    full_delete_path_dm,
                        ...
                        mocha.it(
                            'delete object version delete marker - non latest',
                            async function () {
                                await upload_object_versions(
                                    account_with_access,
                                    delete_object_test_bucket_dm,
                        ...
                        mocha.it(
                            'delete object version delete marker - latest - second latest is a delete marker',
                            async function () {
                                await upload_object_versions(
                                    account_with_access,
                                    delete_object_test_bucket_dm,
                        ...
                        mocha.it(
                            'delete object version delete marker - latest - second latest is a regular version',
                            async function () {
                                const put_res = await upload_object_versions(
                                    account_with_access,
                                    delete_object_test_bucket_dm,
                                    key1,
                                    ['regular', 'regular', 'delete_marker'],
                        ...
                        mocha.it(
                            'delete object version delete marker - latest - no second latest',
                            async function () {
                                const put_res = await upload_object_versions(
                                    account_with_access,
                                    delete_object_test_bucket_dm,
                        ...
                        mocha.it(
                            'delete object version delete marker - in .versions/ - latest exist',
                            async function () {
                                const put_res = await upload_object_versions(
                                    account_with_access,
                                    delete_object_test_bucket_dm,
                        ...
                        mocha.it(
                            'delete object version delete marker - in .versions/ - latest is a delete marker',
                            async function () {
                                const put_res = await upload_object_versions(
                                    account_with_access,
                                    delete_object_test_bucket_dm,
                                ...
                                const latest_dm1 = await find_max_version_past(
                                    full_delete_path_dm,
                                    key1,
                                    '',
                                );
                                ...
                                await account_with_access.deleteObject({
                                    Bucket: delete_object_test_bucket_dm,
                                    Key: key1,
                                    VersionId: put_res[1].VersionId,
                                });

                                await fs_utils.file_must_not_exist(
                                    path.join(full_delete_path_dm, key1),
                                );
                                const latest_dm2 = await find_max_version_past(
                                    full_delete_path_dm,
                                    key1,
                                    '',
                                );
    ...
    mocha.describe(
        'delete object version id - versioning suspended',
        function () {
            const delete_object_test_bucket_reg =
                'delete-object-suspended-test-bucket-reg';
            const delete_object_test_bucket_null =
                'delete-object-suspended-test-bucket-null';
            const delete_object_test_bucket_dm =
                'delete-object-test-suspended-bucket-dm';
            ...
            mocha.describe(
                'delete object - regular version - versioning suspended',
                async function () {
                    mocha.it(
                        'delete version id - fake id - nothing to remove - bucket will be suspended',
                        async function () {
                            const max_version1 = await find_max_version_past(
                                full_delete_path,
                                key1,
                    ...
                    mocha.it(
                        'delete object version id - latest - second latest is null version - versioning suspended',
                        async function () {
                            const upload_res_arr = await upload_object_versions(
                                account_with_access,
                                delete_object_test_bucket_reg,
                                key1,
                                ['null', 'regular'],
                            ...
                            const exist = await fs_utils.file_not_exists(
                                path.join(
                                    full_delete_path,
                                    key1 + '_' + upload_res_arr[1].VersionId,
                                ),
                            );
                            assert.ok(exist);
                            const max_version1 = await find_max_version_past(
                    ...
                    mocha.it(
                        'delete object version id - latest - second latest is delete marker version - versioning suspended',
                        async function () {
                            const upload_res_arr = await upload_object_versions(
                                account_with_access,
                                delete_object_test_bucket_reg,
                            ...
                            const max_version0 = await find_max_version_past(
                                full_delete_path,
                                key1,
                                '',
                            );
                    ...
                    mocha.it(
                        'delete object version id - in .versions/ - versioning suspended',
                        async function () {
                    ...
                    mocha.it(
                        'delete object version id - latest - no second latest - versioning suspended',
                        async function () {
                            const cur_version_id =
                                await stat_and_get_version_id(
                                    full_delete_path,
                    ...
                    mocha.it(
                        "delete object version id - in .versions/ 2 - latest exist and it's a regular version - versioning suspended",
                        async function () {
                            const upload_res_arr = await upload_object_versions(
                                account_with_access,
                                delete_object_test_bucket_reg,
                    ...
                    mocha.it(
                        "delete object version id - in .versions/ 3 - latest exist and it's a delete marker - versioning suspended",
                        async function () {
                            const upload_res_arr = await upload_object_versions(
                                account_with_access,
                                delete_object_test_bucket_reg,
                            ...
                            exist = await fs_utils.file_not_exists(
                                path.join(full_delete_path, key1),
                            );
                            assert.ok(exist);
                            const latest_dm_version_id2 =
                    ...
                    mocha.it(
                        'delete object version id - latest - second latest is regular version - versioning suspended',
                        async function () {
                            const upload_res_arr = await upload_object_versions(
                                account_with_access,
                                delete_object_test_bucket_reg,
            ...
            mocha.describe(
                'delete object - null version - versioning suspended',
                async function () {
                    mocha.it(
                        'delete object version null - latest, no second latest - versioning suspended',
                        async function () {
                            const upload_res_arr = await upload_object_versions(
                                account_with_access,
                                delete_object_test_bucket_null,
                    ...
                    mocha.it(
                        'delete object version null - version is in .versions/ - versioning suspended',
                        async function () {
            ...
            mocha.describe(
                'delete object - delete marker version - versioning suspended',
                async function () {
                    mocha.it(
                        'delete object version delete marker - latest - second latest is a null version - versioning suspended',
                        async function () {
                            await upload_object_versions(
                                account_with_access,
                                delete_object_test_bucket_dm,
                            ...
                            assert.equal(max_version1, undefined);
                            exist = await fs_utils.file_exists(
                                path.join(full_delete_path_dm, key1),
                            );
                            assert.ok(exist);
                            exist = await version_file_must_not_exists(
                    ...
                    mocha.it(
                        'delete object version delete marker - non latest - versioning suspended',
                        async function () {
                            await upload_object_versions(
                                account_with_access,
                                delete_object_test_bucket_dm,
                                key1,
                                ['regular', 'delete_marker', 'delete_marker'],
                            );
                            await s3_uid6.putBucketVersioning({
                                Bucket: delete_object_test_bucket_dm,
                                VersioningConfiguration: {
                    ...
                    mocha.it(
                        'delete object version delete marker - latest - second latest is a delete marker - versioning suspended',
                        async function () {
                            await upload_object_versions(
                                account_with_access,
                                delete_object_test_bucket_dm,
                    ...
                    mocha.it(
                        'delete object version delete marker - latest - second latest is a regular version - versioning suspended',
                        async function () {
                            const put_res = await upload_object_versions(
                                account_with_access,
                                delete_object_test_bucket_dm,
                    ...
                    mocha.it(
                        'delete object version delete marker - latest - no second latest  - versioning suspended',
                        async function () {
                            const put_res = await upload_object_versions(
                                account_with_access,
                                delete_object_test_bucket_dm,
                    ...
                    mocha.it(
                        ...
                        async function () {
                            ...
                            let exist = await fs_utils.file_exists(
                                path.join(full_delete_path_dm, key1),
                            );
                            assert.ok(exist);
                            const ltst_version_id2 =
    ...
    mocha.describe('delete multiple objects - versioning enabled', function () {
        const delete_multi_object_test_bucket =
            'delete-multi-object-test-bucket';
        const full_multi_delete_path = path.join(
            tmp_fs_root,
        ...
        mocha.before(async function () {
            ...
            await account_with_access.createBucket({
                Bucket: delete_multi_object_test_bucket,
            });
            await put_allow_all_bucket_policy(
                account_with_access,
                delete_multi_object_test_bucket,
        ...
        mocha.it(
            'delete multiple objects - no version id - versioning disabled',
            async function () {
                const self = this; // eslint-disable-line no-invalid-this
                self.timeout(80000);
                const keys = [];
                for (let i = 0; i < 50; i++) {
                    const random_key = (Math.random() + 1)
                        .toString(36)
                        .substring(7);
                ...
                const objects = await nb_native().fs.readdir(
                    DEFAULT_FS_CONFIG,
                    full_multi_delete_path,
                );
        ...
        mocha.it('delete multiple objects - no version id', async function () {
            ...
            for (let i = 0; i < 300; i++) {
                versions_type_arr.push(
                    i % 2 === 0 ? 'regular' : 'delete_marker',
                );
            }
            await upload_object_versions(
                account_with_access,
            ...
            const versions = await nb_native().fs.readdir(
                DEFAULT_FS_CONFIG,
                versions_dir,
            );
        ...
        mocha.it(
            'delete multiple objects - delete only delete markers',
            async function () {
                const self = this; // eslint-disable-line no-invalid-this
                self.timeout(60000);
                ...
                await fs_utils.file_must_exist(
                    path.join(full_multi_delete_path, key1),
                );
                const latest_stat = await stat_and_get_all(
                    full_multi_delete_path,
                    key1,
                );
                assert.equal(
                    latest_stat.xattr[XATTR_VERSION_ID],
                    put_res[298].VersionId,
                );
                await delete_object_versions(full_multi_delete_path, key1);
            },
        );
        ...
        mocha.it(
            'delete multiple objects - delete only regular versions key1, delete delete markers key2',
            async function () {
                const self = this; // eslint-disable-line no-invalid-this
                self.timeout(150000);
                const key2 = 'key2';
                ...
                const put_res2 = await upload_object_versions(
                    account_with_access,
                    delete_multi_object_test_bucket,
                    key2,
                    versions_type_arr,
                ...
                for (let i = 0; i < 300; i++) {
                    ...
                    if (i % 2 === 1) {
                        arr.push({
                            Key: key2,
                            VersionId: put_res2[i].VersionId,
                        });
                    }
                }
                const delete_res = await account_with_access.deleteObjects({
                ...
                await fs_utils.file_must_exist(
                    path.join(full_multi_delete_path, key2),
                );
                const latest_dm_version = await find_max_version_past(
                    full_multi_delete_path,
                    key1,
        ...
        mocha.it(
            'delete multiple objects - delete regular versions & delete markers - new latest is dm',
            async function () {
                const self = this; // eslint-disable-line no-invalid-this
                self.timeout(150000);
                const versions_type_arr = [];
                ...
                for (let i = 0; i < 100; i++) {
                    ...
                    if (i % 2 === 0) {
                        assert.equal(
                            delete_res.Deleted[i].DeleteMarker,
                            undefined,
                        );
                    }
                }
        ...
        mocha.it(
            'delete multiple objects - delete regular versions & delete markers - new latest is regular version',
            async function () {
                const self = this; // eslint-disable-line no-invalid-this
                self.timeout(150000);
                const versions_type_arr = [];
        ...
        mocha.it(
            'delete multiple objects - delete keys & regular versions & delete markers ',
            async function () {
                const self = this; // eslint-disable-line no-invalid-this
                self.timeout(150000);
                const versions_type_arr = [];
                ...
                for (let i = 0; i < 1; i++) {
                    assert.notEqual(
                        delete_res.Deleted[i].DeleteMarkerVersionId,
                        undefined,
                    );
                    assert.equal(delete_res.Deleted[i].DeleteMarker, true);
                }
        ...
        mocha.it(
            'delete multiple objects - delete regular versions & delete markers & latest & keys- ',
            async function () {
                const self = this; // eslint-disable-line no-invalid-this
                self.timeout(150000);
                const versions_type_arr = [];
    ...
    mocha.describe(
        'delete multiple objects - versioning suspended',
        function () {
            const delete_multi_object_test_bucket =
                'delete-multi-object-test-bucket-suspended';
            const full_multi_delete_path = path.join(
                tmp_fs_root,
                delete_multi_object_test_bucket,
            );
            ...
            mocha.it(
                'delete multiple objects - no version id - versioning disabled - will be suspended',
                async function () {
                    const self = this; // eslint-disable-line no-invalid-this
                    self.timeout(80000);
                    const keys = [];
            ...
            mocha.it(
                'delete multiple objects - no version id - versioning suspended',
                async function () {
                    const self = this; // eslint-disable-line no-invalid-this
                    self.timeout(60000);
                    const versions_type_arr = ['null'];
            ...
            mocha.it(
                'delete multiple objects - delete only delete markers - versioning suspended',
                async function () {
                    const self = this; // eslint-disable-line no-invalid-this
                    self.timeout(60000);
                    const versions_type_arr = [];
                    ...
                    const exist = await fs_utils.file_exists(
                        path.join(full_multi_delete_path, key1),
                    );
                    assert.ok(exist);
                    const latest_stat = await stat_and_get_all(
            ...
            mocha.it(
                'delete multiple objects - delete only regular versions key1, delete delete markers key2 - versioning suspended',
                async function () {
                    const self = this; // eslint-disable-line no-invalid-this
                    self.timeout(60000);
                    const key2 = 'key2';
                    ...
                    const version_info = await stat_and_get_all(
                        version_path,
                        '',
                    );
                    assert.equal(
                        version_info.xattr[XATTR_DELETE_MARKER],
                        'true',
                    );
                    assert.equal(
                        version_info.xattr[XATTR_VERSION_ID],
            ...
            mocha.it(
                'delete multiple objects - delete regular versions & delete markers - new latest is dm - versioning suspended',
                async function () {
                    const self = this; // eslint-disable-line no-invalid-this
                    self.timeout(60000);
                    const versions_type_arr = [];
            ...
            mocha.it(
                'delete multiple objects - delete regular versions & delete markers - new latest is regular version - versioning suspended',
                async function () {
                    const self = this; // eslint-disable-line no-invalid-this
                    self.timeout(60000);
                    const versions_type_arr = [];
            ...
            mocha.it(
                'delete multiple objects - delete keys & regular versions & delete markers - versioning suspended',
                async function () {
                    const self = this; // eslint-disable-line no-invalid-this
                    self.timeout(60000);
                    const versions_type_arr = [];
                    ...
                    assert.equal(
                        delete_res.Deleted[0].DeleteMarkerVersionId,
                        'null',
                    );
                    for (let i = 1; i < 11; i++) {
                        if (i % 2 === 0) {
            ...
            mocha.it(
                'delete multiple objects - delete regular versions & delete markers & latest & keys- - versioning suspended',
                async function () {
                    const self = this; // eslint-disable-line no-invalid-this
                    self.timeout(60000);
                    const versions_type_arr = [];
                    ...
                    assert.equal(
                        delete_res.Deleted[10].DeleteMarkerVersionId,
                        NULL_VERSION_ID,
                    );
    ...
    mocha.describe('list object version - check null version', function () {
        const list_object_versions_test_bucket =
            'list-object-versions-test-bucket';
        ...
        mocha.before(async function () {
            ...
            await put_allow_all_bucket_policy(
                account_with_access,
                list_object_versions_test_bucket,
            );
        });

        mocha.it(
            'list object versions - only null versions - versioning disabled',
            async function () {
                await account_with_access.putObject({
                    Bucket: list_object_versions_test_bucket,
                    Key: key_to_list1,
                ...
                const list_object_versions_res =
                    await account_with_access.listObjectVersions({
                        Bucket: list_object_versions_test_bucket,
                    });
                //bike.txt before car.txt (a-z sort)
                assert.equal(
                    list_object_versions_res.Versions[0].Key,
                    key_to_list2,
                );
                assert.equal(
                    list_object_versions_res.Versions[0].VersionId,
                    NULL_VERSION_ID,
                );
                assert.equal(
                    list_object_versions_res.Versions[1].Key,
                    key_to_list1,
                );
                assert.equal(
                    list_object_versions_res.Versions[1].VersionId,
                    NULL_VERSION_ID,
                );
            },
        );
        ...
        mocha.it(
            'list object versions - no null version id - with versioning enabled',
            async function () {
                const versions_type_arr = [];
                for (let i = 0; i < 3; i++) {
                ...
                assert.equal(
                    list_object_versions_res.Versions[2].Key,
                    key_to_list3,
                );
                assert.equal(
                    list_object_versions_res.Versions[2].VersionId,
                    put_res[0].VersionId,
                );
            },
        );
        ...
        mocha.it(
            'list object versions - latest version is null version id',
            async function () {
                await account_with_access.putBucketVersioning({
                    Bucket: list_object_versions_test_bucket,
                    VersioningConfiguration: {
                        MFADelete: 'Disabled',
                        Status: 'Suspended',
        ...
        mocha.it(
            'list object versions -  oldest version is null version id',
            async function () {
                await account_with_access.putBucketVersioning({
                    Bucket: list_object_versions_test_bucket,
                    VersioningConfiguration: {
                ...
                for (let i = 0; i < 3; i++) {
                    assert.equal(
                        list_object_versions_res.Versions[i].Key,
                        key_to_list2,
                    );
                    assert.notEqual(
                        list_object_versions_res.Versions[i].VersionId,
                        NULL_VERSION_ID,
                    );
                }
                // oldest version is null (the order is from latest to oldest), hence null is last
                assert.equal(
                    list_object_versions_res.Versions[3].Key,
                    key_to_list2,
                );
                assert.equal(
                    list_object_versions_res.Versions[3].VersionId,
                    NULL_VERSION_ID,
                );
            },
        );
...
mocha.describe('bucketspace namespace_fs - versioning', function () {
    mocha.describe('List-objects', function () {
        ...
        mocha.before(async function () {
            ...
            const ls_new_buckets_path = get_new_buckets_path_by_test_env(
                tmp_fs_root2,
                '/',
            );
            if (is_nc_coretest) {
            ...
            accounts.push(res.email);
            await s3_client.putBucketVersioning({
                Bucket: bucket_name,
                VersioningConfiguration: {
                    MFADelete: 'Disabled',
                    Status: 'Enabled',
        ...
        mocha.it(
            'list objects - should return only latest object',
            async function () {
                const res = await s3_client.listObjects({
                    Bucket: bucket_name,
                });
                res.Contents.forEach(val => {
                    if (val.Key.includes('version') === true) {
                        assert.fail(
                            'Not Expected: list objects returned contents fo .version dir',
                        );
                    }
                });
            },
    ...
    mocha.describe('Get/Head object', function () {
        ...
        const tmp_fs_root3 = path.join(
            TMP_PATH,
            'test_namespace_fs_get_objects',
        );
        ...
        const disabled_full_path = path.join(
            tmp_fs_root3,
            disabled_bucket_path,
        );
        ...
        mocha.before(async function () {
            ...
            await fs_utils.create_fresh_path(disabled_full_path, 0o770);
            await fs_utils.file_must_exist(disabled_full_path);
            const new_buckets_path3 = get_new_buckets_path_by_test_env(
                tmp_fs_root3,
                '/',
            );
            if (is_nc_coretest) {
            ...
            await create_object(
                `${disabled_full_path}/${dis_version_key}`,
                dis_version_body,
                'null',
            );
            // create a file after when versioning not enabled
            await create_object(
                `${full_path}/${dis_version_key}`,
                dis_version_body,
                'null',
            );
            await s3_client.putBucketVersioning({
            ...
            file_pointer = await create_object(
                `${full_path}/${en_version_key}`,
                en_version_body,
                versionID_1,
                true,
            );
        ...
        mocha.it(
            'get object, versioning not enabled - should return latest object',
            async function () {
        ...
        mocha.it(
            'get object, versioning not enabled, version id specified - should return NoSuchKey',
            async function () {
                try {
                    await s3_client.getObject({
                        Bucket: disabled_bucket_name,
                        Key: dis_version_key,
                        VersionId: versionID_1,
                    });
        ...
        mocha.it(
            'get object, file created before the version is enabled - should return latest object',
            async function () {
                const res = await s3_client.getObject({
                    Bucket: bucket_name,
                    Key: dis_version_key,
        ...
        mocha.it(
            'get object, file created before the version is enabled, version id specified - should return ENOENT',
            async function () {
                try {
                    await s3_client.getObject({
                        Bucket: bucket_name,
        ...
        mocha.it(
            'get object, with version enabled, no version id specified - should return latest object',
            async function () {
                const res = await s3_client.getObject({
                    Bucket: bucket_name,
                    Key: en_version_key,
        ...
        mocha.it(
            'get object, with version enabled, with version id specified and matching with main file - should return latest object',
            async function () {
                const res = await s3_client.getObject({
                    Bucket: bucket_name,
                    Key: en_version_key,
        ...
        mocha.it(
            'get object, with version enabled, with version id specified and not matching with main file - should return $VersionId object',
            async function () {
                await create_object(
                    `${version_dir_path}/${key_version}`,
                    en_version_body_v1,
                    versionID_2,
                );
                const res = await s3_client.getObject({
        ...
        mocha.it(
            'get object, with version enabled, with version id specified and versioned object not present - should return NoSuchKey',
            async function () {
                try {
                    await s3_client.getObject({
                        Bucket: bucket_name,
                        Key: en_version_key,
                        VersionId: versionID_3,
                    });
                    assert.fail('Should fail');
                } catch (err) {
                    assert.equal(err.Code, 'NoSuchKey');
        ...
        mocha.it(
            'get object, with version enabled, with wrong version id specified - should return Bad Request',
            async function () {
                try {
                    await s3_client.getObject({
                        Bucket: bucket_name,
                        Key: en_version_key,
                        VersionId: 'ctime-12-ino-12a',
                    });
                    assert.fail('Should fail');
                } catch (err) {
        ...
        mocha.it(
            'head object, version not enabled - should return latest object md',
            async function () {
                try {
                    await s3_client.headObject({
                        Bucket: disabled_bucket_name,
                        Key: dis_version_key,
                    });
        ...
        mocha.it(
            'head object, version not enabled, version specified - should return object NotFound',
            async function () {
                try {
                    await s3_client.headObject({
                        Bucket: disabled_bucket_name,
        ...
        mocha.it(
            'head object, file created before the version is enabled - should return latest object md',
            async function () {
                try {
                    await s3_client.headObject({
                        Bucket: bucket_name,
        ...
        mocha.it(
            'head object, file created before the version is enabled, version specified - should return NotFound',
            async function () {
                try {
                    await s3_client.headObject({
                        Bucket: bucket_name,
        ...
        mocha.it(
            'head object, with version enabled, no version id specified - should return latest object md',
            async function () {
                try {
                    await s3_client.headObject({
                        Bucket: bucket_name,
        ...
        mocha.it(
            'head object, with version enabled, with version id specified and matching with main file - should return latest object md',
            async function () {
                try {
                    await s3_client.headObject({
                        Bucket: bucket_name,
        ...
        mocha.it(
            'head object, with version enabled, with version id specified and not matching with main file - should return $VersionId object md',
            async function () {
                await create_object(
                    `${version_dir_path}/${key_version}`,
                    en_version_body_v1,
        ...
        mocha.it(
            'head object, with version enabled, versioned object not created - should return NotFound',
            async function () {
                try {
                    await s3_client.headObject({
                        Bucket: bucket_name,
        ...
        mocha.it(
            'head object, with version enabled, wrong version id specified - should return NotFound',
            async function () {
                try {
                    await s3_client.headObject({
                        Bucket: bucket_name,
        ...
        mocha.it(
            'Put object when version disbled and do put on the same object when version enabled - get object should return versioned object',
            async function () {
                let res = await s3_client.getObject({
                    Bucket: disabled_bucket_name,
                    Key: dis_version_key,
                });
                ...
                await create_object(
                    `${version_obj_path}/${version_key}`,
                    version_body,
                    'null',
                );
                await create_object(
                    `${disabled_full_path}/${dis_version_key}`,
                    version_body_new_version,
                    'mtime-jhdfbkjsd-ino-bnsdf7f',
                );
                res = await s3_client.getObject({
                    Bucket: disabled_bucket_name,
                    Key: dis_version_key,
                    VersionId: 'null',
                });
        ...
        mocha.it(
            'get object, with version enabled, delete marker placed on latest object - should return NoSuchKey',
            async function () {
                const xattr_delete_marker = { [XATTR_DELETE_MARKER]: 'true' };
                await file_pointer.replacexattr(
                    DEFAULT_FS_CONFIG,
                    xattr_delete_marker,
                );
                try {
        ...
        mocha.it(
            'get object, with version enabled, delete marker placed on latest object, version id specified - should return the version object',
            async function () {
                const xattr_delete_marker = { [XATTR_DELETE_MARKER]: 'true' };
                await file_pointer.replacexattr(
                    DEFAULT_FS_CONFIG,
        ...
        mocha.it(
            'head object, with version enabled, delete marked xattr placed on latest object - should return ENOENT',
            async function () {
                const xattr_delete_marker = { [XATTR_DELETE_MARKER]: 'true' };
                await file_pointer.replacexattr(
                    DEFAULT_FS_CONFIG,
        ...
        mocha.it(
            'head object, with version enabled, delete marked xattr placed on latest object, version id specified - should return the version object',
            async function () {
                const xattr_delete_marker = { [XATTR_DELETE_MARKER]: 'true' };
                await file_pointer.replacexattr(
                    DEFAULT_FS_CONFIG,
...
async function upload_object_versions(
    s3_client,
    bucket,
    key,
    object_types_arr,
    ...
    const res = [];
    const versioning_status = await s3_client.getBucketVersioning({
        Bucket: bucket,
    });
    for (const obj_type of object_types_arr) {
        if (obj_type === 'regular' || obj_type === 'null') {
            ...
            const put_res = await s3_client.putObject({
                Bucket: bucket,
                Key: key,
                Body: random_body,
            });
            ...
            const delete_res = await s3_client.deleteObject({
                Bucket: bucket,
                Key: key,
            });
            res.push(delete_res);
...
async function find_max_version_past(full_path, key, dir, skip_list) {
    ...
    try {
        ...
        let max_path;
        const versions = (
            await nb_native().fs.readdir(DEFAULT_FS_CONFIG, versions_dir)
        ).filter(entry => {
            const index =
                entry.name.endsWith('_null') ?
                    entry.name.lastIndexOf('_null')
        ...
        for (const entry of versions) {
            if (
                skip_list ?
                    !skip_list.includes(entry.name.slice(key.length + 1))
                :   true
            ) {
                const version_str = entry.name.slice(key.length + 1);
                const { mtimeNsBigint } =
                    _extract_version_info_from_xattr(version_str) ||
...
function _extract_version_info_from_xattr(version_id_str) {
    ...
    if (arr.length < 2) {
        throw new Error('Invalid version_id_string, cannot extact attributes');
    }
    return {
        mtimeNsBigint: size_utils.string_to_bigint(arr[0], 36),
        ino: parseInt(arr[1], 36),
    };
}
...
async function compare_version_ids(
    full_path,
    key,
    put_result_version_id,
    prev_version_id,
    is_enabled = true,
) {
    const key_path = path.join(full_path, key);
    const stat = await nb_native().fs.stat(DEFAULT_FS_CONFIG, key_path);
    const new_version_id =
        is_enabled ? get_version_id_by_stat(stat) : NULL_VERSION_ID;
...
function get_version_id_by_stat(stat) {
    return (
        'mtime-' +
        stat.mtimeNsBigint.toString(36) +
        '-ino-' +
        stat.ino.toString(36)
    );
}
...
mocha.describe('List-objects', function () {
    ...
    mocha.before(async function () {
        ...
        await fs_utils.create_fresh_path(dir1_version_dir, 0o770);
        await fs_utils.file_must_exist(dir1_version_dir);
        const new_bucket_path4 = get_new_buckets_path_by_test_env(
            tmp_fs_root4,
            '/',
        );
        if (is_nc_coretest) {
        ...
        await create_object(
            `${full_path_version_dir}/${version_key_1}`,
            version_body,
            'mtime-crh3783sxk3k-ino-guty',
        );
        await create_object(
            `${full_path_version_dir}/${version_key_2}`,
            version_body,
            'mtime-crkfjum9883k-ino-guu7',
        );
        await create_object(
            `${full_path_version_dir}/${version_key_3}`,
            version_body,
            'mtime-crkfjx1hui2o-ino-guu9',
        );
        file_pointer = await create_object(
            `${full_path}/${dir_key}`,
            version_body,
        ...
        await create_object(
            `${dir1_version_dir}/${dir_version_key_1}`,
            version_body,
            'mtime-crkfjknr7xmo-ino-guu4',
        );
        await create_object(
            `${dir1_version_dir}/${dir_version_key_2}`,
            version_body,
            'mtime-crkfjr98uiv4-ino-guu6',
        );
    });
    ...
    mocha.it(
        'list object versions - should return all versions of all the object',
        async function () {
            const res = await s3_client.listObjectVersions({
                Bucket: bucket_name,
            });
            let count = 0;
    ...
    mocha.it(
        'list object versions - should by pass the dir cache and load the objects from the disk',
        async function () {
            // Creation of version under HIDDER_VERSION_PATH will trigger load() call
            await create_object(
                `${full_path_version_dir}/${version_key_4}`,
                version_body,
                'null',
            );
            let res = await s3_client.listObjectVersions({
                Bucket: bucket_name,
            });
            let count = 0;
            res.Versions.forEach(val => {
            ...
            await fs_utils.file_delete(
                path.join(full_path_version_dir, version_key_4),
            );
    ...
    mocha.it(
        'list object versions - should return all the versions of the requested object',
        async function () {
            const res = await s3_client.listObjectVersions({
                Bucket: bucket_name,
                KeyMarker: key,
    ...
    mocha.it(
        'list object versions - should return only max_key number of elements',
        async function () {
            const res = await s3_client.listObjectVersions({
                Bucket: bucket_name,
                MaxKeys: 2,
    ...
    mocha.it(
        'list object versions - should return only max keys of the versions of the requested object and nextKeyMarker should point to the next version',
        async function () {
            const res = await s3_client.listObjectVersions({
                Bucket: bucket_name,
                KeyMarker: key,
    ...
    mocha.it(
        'list object versions - should return only max keys of the versions of the requested object from the version id marker',
        async function () {
            const res = await s3_client.listObjectVersions({
                Bucket: bucket_name,
                KeyMarker: key,
    ...
    mocha.it(
        'list object versions - should fail because of missing key_marker',
        async function () {
            try {
                await s3_client.listObjectVersions({
                    Bucket: bucket_name,
                    MaxKeys: 2,
                    VersionIdMarker: version_key_3,
                });
                assert.fail(
                    `list object versions passed though key marker is missing`,
                );
            } catch (err) {
    ...
    mocha.it(
        'list objects - deleted object should not be listed',
        async function () {
            const xattr_delete_marker = { [XATTR_DELETE_MARKER]: 'true' };
    ...
    mocha.it(
        'list object versions - All versions of the object should be listed',
        async function () {
            const xattr_delete_marker = { [XATTR_DELETE_MARKER]: 'true' };
            file_pointer.replacexattr(DEFAULT_FS_CONFIG, xattr_delete_marker);
            const res = await s3_client.listObjectVersions({
    ...
    mocha.it(
        'list object versions - Check whether the deleted object is the latest and should be present under delete markers as well',
        async function () {
            const xattr_delete_marker = { [XATTR_DELETE_MARKER]: 'true' };
            file_pointer.replacexattr(DEFAULT_FS_CONFIG, xattr_delete_marker);
            const res = await s3_client.listObjectVersions({
    ...
    mocha.it(
        'list object versions - should not list .versions folder',
        async function () {
            const res = await s3_client.listObjectVersions({
                Bucket: bucket_name,
                Delimiter: '/',
    ...
    mocha.it(
        'list object versions - should show isLatest flag only for latest objects',
        async function () {
            const res = await s3_client.listObjectVersions({
                Bucket: bucket_name,
            });
    ...
    mocha.it(
        'list object versions - should show version-id in suspention mode',
        async function () {
            await s3_client.putBucketVersioning({
                Bucket: bucket_name,
                VersioningConfiguration: {
...
async function create_object(object_path, data, version_id, return_fd) {
    const target_file = await nb_native().fs.open(
        DEFAULT_FS_CONFIG,
        object_path,
        'w+',
    );
