...
mocha.describe('system_store', function () {
    ...
    mocha.it(
        'Check make_changes updates new created systems',
        async function () {
            ...
            assert.deepStrictEqual(
                data1.systems.length + LOOP_CYCLES,
                data2.systems.length,
            );
        },
    );

    mocha.it(
        'Check make_changes returns no diff when not changing last_update',
        async function () {
    ...
    mocha.it(
        'Check make_changes returns diff when changing last_update',
        async function () {
            const system_id = system_store.new_system_store_id();
            const orig_name = `JenTheMajesticSlothSystemStoreLoop3`;
            await system_store.load();
