...
const parquet_small_base64 = `UEFSMRUEFWQVZEwVFBUAEgAAAQAAADABAAAAMQEAAAAyAQAAADMBAAAANAEAAAA1AQAAADYBAAAA
...
async function simulate_async_loop(writable) {
  ...
  assert(
    writable.emit('error', new Error('I am an error event in an async loop.')),
  );
}
...
mocha.describe('s3select', function () {
  ...
  if (nb_native.select_parquet) {
    mocha.it('parquet - select column', async function () {
      ...
      const select_args = {
        ...
        input_serialization_format: {},
        records_header_buf:
          s3select_utils.S3SelectStream.records_message_headers,
  ...
  mocha.it('double pipe close', async function () {
    ...
    assert.strictEqual(
      ...
      'wrong downstream output.',
    );
    assert(
      readable.emit('error', new Error('readale error')),
      'failed to emit error in upstream.',
    );
