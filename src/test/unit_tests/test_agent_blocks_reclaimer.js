...
const AgentBlocksReclaimer =
    require('../../server/bg_services/agent_blocks_reclaimer').AgentBlocksReclaimer;
...
class ReclaimerMock extends AgentBlocksReclaimer {
    ...
    iterate_all_blocks(marker, limit, deleted_only) {
        ...
        assert(
            limit === config.AGENT_BLOCKS_RECLAIMER_BATCH_SIZE,
            'Wrong reclaimer config limit',
        );
        assert(
            deleted_only === true,
            'Reclaimer should check only deleted and non reclaimed blocks',
        );
    ...
    update_blocks_by_ids(block_ids, set_updates, unset_updates) {
        ...
        assert(
            set_updates &&
                _.difference(Object.keys(set_updates), ['reclaimed']).length ===
                    0,
            'Reclaimer should only send set_updates with reclaimed',
        );
        assert(
            _.every(block_ids, block_id => this.is_valid_md_id(block_id)),
            'Block_ids not valid ObjectId',
        );
            ...
            .then(() =>
                this.nodes.filter(
                    node =>
                        !node.deleted &&
                        _.includes(
                            ids.map(node_id => String(node_id)),
                            String(node._id),
                        ),
                ),
            )
    ...
    delete_blocks_from_nodes(blocks) {
        this.reclaimed_blocks = _.concat(
            this.reclaimed_blocks,
            blocks.map(block => String(block._id)),
        );
        ...
        return P.resolve().then(() => {
            blocks.forEach(block_rec => {
                const block = this.blocks.find(
                    mock_block =>
                        String(mock_block._id) === String(block_rec._id),
                );
...
mocha.describe('not mocked agent_blocks_reclaimer', function () {
    ...
    const generator = crypto.createCipheriv(
        'aes-128-gcm',
        seed,
        Buffer.alloc(12),
    );
    ...
    mocha.it(
        'should reclaim deleted blocks with alive nodes',
        async function () {
            ...
            let obj_id;
            let blocks_uploaded = [];
            const agent_blocks_reclaimer = new AgentBlocksReclaimer(
                self.test.title,
            );
            ...
            try {
                ...
                const chunk_ids = await MDStore.instance().find_parts_chunk_ids(
                    { _id: db_client.instance().parse_object_id(id) },
                );
                let blocks =
                    await MDStore.instance().find_blocks_of_chunks(chunk_ids);
                ...
                if (!all_reclaimed) {
                    throw new Error('NOT ALL BLOCKS WERE RECLAIMED');
                }
                try {
                ...
                if (
                    err.message === 'verify_read_data BLOCKS WERE NOT DELETED'
                ) {
                    console.error(
                        'verify_read_mappings EXPECTED ERROR of deleted blocks:',
                        err,
                    );
                    return;
                }
    ...
    function verify_read_mappings(key, size) {
        ...
        return rpc_client.object
            .read_object_mapping_admin({ bucket, key })
            .then(({ chunks }) => {
                let pos = 0;
                chunks.forEach(chunk => {
                    ...
                    _.forEach(blocks_per_node, (b, node_name) =>
...
mocha.describe('mocked agent_blocks_reclaimer', function () {
    mocha.it('should mark reclaimed on deleted nodes', async function () {
        ...
        const reclaimer_mock = new ReclaimerMock(
            blocks,
            nodes,
            self.test.title,
        );
        try {
            ...
            assert(
                reclaimer_mock.blocks[0].reclaimed,
                'Block was not reclaimed',
            );
            assert(
                reclaimer_mock.reclaimed_blocks.length === 0,
                'Reclaimer sent delete to deleted node',
            );
        } catch (err) {
            console.error(err, err.stack);
            throw err;
    ...
    mocha.it('should not mark reclaimed on offline nodes', async function () {
        ...
        try {
            ...
            assert(
                reclaimer_mock.reclaimed_blocks.length === 1,
                'Reclaimer did not try to delete on offline node',
            );
        } catch (err) {
            console.error(err, err.stack);
            throw err;
    ...
    mocha.it('should mark reclaimed on non existing nodes', async function () {
        ...
        try {
            ...
            assert(
                reclaimer_mock.reclaimed_blocks.length === 0,
                'Reclaimer sent delete to non existing node',
            );
        } catch (err) {
            console.error(err, err.stack);
            throw err;
    ...
    mocha.it(
        'should not mark reclaimed on failure to delete',
        async function () {
            const self = this; // eslint-disable-line no-invalid-this
            const nodes = [
                {
            ...
            try {
                ...
                assert(
                    reclaimer_mock.blocks[0].reclaimed &&
                        !reclaimer_mock.blocks[0].fail_to_delete,
                    'Block was not reclaimed',
                );
                assert(
                    !reclaimer_mock.blocks[1].reclaimed &&
                        reclaimer_mock.blocks[1].fail_to_delete,
                    'Block was reclaimed on failure to delete',
                );
                assert(
                    reclaimer_mock.reclaimed_blocks.length === 2,
                    'Reclaimer did not sent delete to nodes',
                );
            } catch (err) {
                console.error(err, err.stack);
                throw err;
