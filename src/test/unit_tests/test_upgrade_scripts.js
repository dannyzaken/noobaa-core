...
mocha.describe('test upgrade scripts', async function () {
    ...
    mocha.it('test upgrade bucket policy to version 5.14.0', async function () {
        ...
        const bucket = system_store.data.buckets.find(
            bucket_obj => bucket_obj.name.unwrap() === BKT,
        );
        ...
        await upgrade_bucket_policy.run({
            dbg,
            system_store,
            system_server: null,
        });
        const res = await s3.getBucketPolicy({
            // should work - bucket policy should fit current schema
            Bucket: BKT,
        });
        const new_policy = JSON.parse(res.Policy);

        assert.strictEqual(
            new_policy.Statement.length,
            old_policy.statement.length,
        );
        assert.strictEqual(new_policy.Version, old_policy.version);
        assert.strictEqual(
            new_policy.Statement[0].Sid,
            old_policy.statement[0].sid,
        );
        ...
        assert.strictEqual(
            new_policy.Statement[0].Resource[0],
            old_policy.statement[0].resource[0],
        );
    });
