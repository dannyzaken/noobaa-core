...
const {
    ...
    generate_s3_client,
    exec_manage_cli,
    generate_anon_s3_client,
    generate_nsfs_account,
} = require('../system_tests/test_utils');
const nc_mkm =
    require('../../manage_nsfs/nc_master_key_manager').get_instance();
...
const {
    ...
    PASSWORD,
    SYSTEM,
    get_admin_mock_account_details,
    NC_CORETEST_CONFIG_FS,
} = coretest;
coretest.setup({});
...
mocha.describe('bucket operations - namespace_fs', function () {
    ...
    mocha.before(async function () {
        ...
        if (is_nc_coretest) {
            await create_fs_user_by_platform(
                no_permissions_dn,
                'new_password',
                123123123,
                123123123,
    ...
    mocha.it('export dir as a bucket', async function () {
        ...
        if (is_nc_coretest) {
            ...
            await test_utils.set_path_permissions_and_owner(
                src_bucket_path,
                { uid, gid },
                0o700,
            );
    ...
    mocha.it(
        'export other dir as bucket - and update bucket path to original bucket path',
        async function () {
            const obj_nsr = { resource: nsr, path: bucket_path };
            ...
            try {
                ...
                assert.fail(
                    `can not update nsfs bucket for using path of existing exported bucket:`,
                );
            } catch (err) {
    ...
    mocha.it('create account 1 with uid, gid - wrong uid', async function () {
        ...
        s3_creds.credentials = {
            ...
            secretAccessKey:
                account_wrong_uid.access_keys[0].secret_key.unwrap(),
        };
        s3_creds.endpoint = coretest.get_http_address();
    ...
    mocha.it('list buckets with wrong uid, gid', async function () {
        ...
        await rpc_client.bucket.put_bucket_policy({
            name: first_bucket,
            policy: s3_policy.policy,
        });

        const res = await s3_wrong_uid.listBuckets({});
        console.log(inspect(res));
        const list_ok = bucket_in_list(
            [first_bucket],
            [bucket_name],
            res.Buckets,
        );
    ...
    mocha.it('update account', async function () {
        ...
        const arr = [
            {
                nsfs_account_config: { uid: 26041993 },
                default_resource,
                should_fail: false,
            },
            {
                nsfs_account_config: {
                    new_buckets_path: get_new_buckets_path_by_test_env(
                        tmp_fs_root,
                        'dummy_dir1/',
                    ),
                },
            ...
            },
            {
                ...
                error_code:
                    is_nc_coretest ?
                        ManageCLIError.MissingUpdateProperty.code
                    :   'FORBIDDEN',
            },
            {
                nsfs_account_config: { uid: 26041992 },
                default_resource,
                should_fail: false,
            },
        ];
        for (const item of arr) {
            await update_account_nsfs_config(
                email,
                item.default_resource,
                item.nsfs_account_config,
                item.should_fail,
                item.error_code,
            );
        }
    ...
    mocha.it('list namespace resources after creation', async function () {
        ...
        const res = await rpc_client.pool.read_namespace_resource({
            name: nsr,
        });
    ...
    mocha.it('create account 2 uid, gid', async function () {
        ...
        s3_creds.credentials = {
            ...
            secretAccessKey:
                account_correct_uid.access_keys[0].secret_key.unwrap(),
        };
        s3_creds.endpoint = coretest.get_http_address();
    ...
    // s3 workflow
    mocha.it(
        'create account with namespace resource as default pool but without nsfs_account_config',
        async function () {
            // not supported in NC
            if (is_nc_coretest) this.skip(); // eslint-disable-line no-invalid-this
            try {
                ...
                assert.strictEqual(
                    err.message,
                    'Invalid account configuration - must specify nsfs_account_config when default resource is a namespace resource',
                );
            }
        },
    );
    mocha.it(
        'create s3 bucket with correct uid and gid - should fail - existing directory',
        async function () {
            const account_s3_correct_uid1 =
            ...
            s3_creds.credentials = {
                accessKeyId:
                    account_s3_correct_uid1.access_keys[0].access_key.unwrap(),
                secretAccessKey:
                    account_s3_correct_uid1.access_keys[0].secret_key.unwrap(),
            };
            s3_creds.endpoint = coretest.get_http_address();
    ...
    mocha.it('create s3 bucket with correct uid and gid', async function () {
        ...
        s3_creds.credentials = {
            accessKeyId:
                account_s3_correct_uid.access_keys[0].access_key.unwrap(),
            secretAccessKey:
                account_s3_correct_uid.access_keys[0].secret_key.unwrap(),
        };
        s3_creds.endpoint = coretest.get_http_address();
        s3_correct_uid_default_nsr = new S3(s3_creds);
        const res = await s3_correct_uid_default_nsr.createBucket({
        ...
        await fs_utils.file_must_exist(
            path.join(s3_new_buckets_path, bucket_name + '-s3'),
        );
    });

    mocha.it('get bucket acl - rpc bucket', async function () {
        const res = await s3_correct_uid_default_nsr.getBucketAcl({
            Bucket: first_bucket,
        });
        const bucket_info = await rpc_client.bucket.read_bucket({
            name: first_bucket,
        });
    ...
    mocha.it(
        'create s3 bucket with correct distinguished name',
        async function () {
            const account_s3_correct_dn =
                await rpc_client.account.create_account({
                    ...new_account_params,
            ...
            s3_creds.credentials = {
                accessKeyId:
                    account_s3_correct_dn.access_keys[0].access_key.unwrap(),
                secretAccessKey:
                    account_s3_correct_dn.access_keys[0].secret_key.unwrap(),
            };
            s3_creds.endpoint = coretest.get_http_address();
            s3_correct_dn_default_nsr = new S3(s3_creds);
            const res = await s3_correct_dn_default_nsr.createBucket({
                Bucket: bucket_name + '-s3-2',
            });
            console.log(inspect(res));
            await fs_utils.file_must_exist(
                path.join(s3_new_buckets_path, bucket_name + '-s3-2'),
            );
        },
    );
    mocha.it(
        'create s3 bucket - uid and gid no permissions - should fail',
        async function () {
            ...
            const account_s3_incorrect_uid =
                await rpc_client.account.create_account(incorrect_params);
            s3_creds.credentials = {
                accessKeyId:
                    account_s3_incorrect_uid.access_keys[0].access_key.unwrap(),
                secretAccessKey:
                    account_s3_incorrect_uid.access_keys[0].secret_key.unwrap(),
            };
            s3_creds.endpoint = coretest.get_http_address();
            ...
            try {
                ...
                assert.fail(
                    'unpreviliged account could create bucket on nsfs: ',
                );
            } catch (err) {
            ...
            await fs_utils.file_must_not_exist(
                path.join(s3_new_buckets_path, bucket_name + '-s3-should-fail'),
            );
            await fs.promises.chmod(tmp_fs_root, 0o777);
        },
    );
    mocha.it(
        'create s3 bucket - dn without permissions - should fail',
        async function () {
            const incorrect_params = {
                ...new_account_params,
            ...
            s3_creds.credentials = {
                accessKeyId:
                    account_no_perm_dn.access_keys[0].access_key.unwrap(),
                secretAccessKey:
                    account_no_perm_dn.access_keys[0].secret_key.unwrap(),
            };
            s3_creds.endpoint = coretest.get_http_address();
            const s3_client_no_perm_dn = new S3(s3_creds);
            await fs.promises.chmod(tmp_fs_root, 0o770);
            try {
                await s3_client_no_perm_dn.createBucket({
                    Bucket: bucket_name + '-s3-should-fail',
                });
    ...
    mocha.it('update account dn', async function () {
        ...
        const arr = [
            {
                nsfs_account_config: { distinguished_name: 'bla' },
                default_resource,
                should_fail: is_nc_coretest,
                error_code: ManageCLIError.InvalidAccountDistinguishedName.code,
            },
            {
                nsfs_account_config: {
                    new_buckets_path: get_new_buckets_path_by_test_env(
            ...
            {
                nsfs_account_config: { distinguished_name: 'root' },
                default_resource,
                should_fail: false,
            },
            {
    ...
    mocha.it('delete bucket with dn', async function () {
        ...
        await fs_utils.file_must_not_exist(
            path.join(s3_new_buckets_path, bucket),
        );
        const res = await s3_correct_dn_default_nsr.listBuckets({});
        console.log(inspect(res));
        const list_ok = bucket_in_list(
    ...
    mocha.it('put object object with wrong uid gid', async function () {
        ...
        const s3_client_wrong_uid = generate_s3_client(
            access_key.unwrap(),
            secret_key.unwrap(),
            CORETEST_ENDPOINT,
        );
        ...
        const key = 'ob1.txt';
        try {
            const res = await s3_client_wrong_uid.putObject({
                Bucket: bucket,
                Key: key,
                Body: generic_obj_body,
            });
            console.log(inspect(res));
            assert.fail(
                'unpreviliged account could upload object on nsfs bucket',
            );
        } catch (err) {
            assert.strictEqual(err.Code, 'AccessDenied');
        }
    ...
    mocha.it(
        'putObject - regular object - head/get object - key/ - should fail with 404',
        async function () {
            const { access_key, secret_key } =
                account_correct_uid.access_keys[0];
            ...
            try {
                ...
                assert.fail(
                    'head operation successfull although it should have failed',
                );
            } catch (err) {
            ...
            try {
                ...
                assert.fail(
                    'get operation successfull although it should have failed',
                );
            } catch (err) {
                assert.deepStrictEqual(err.Code, 'NoSuchKey');
            }
            const list_res = await s3_client.listObjects({
                Bucket: bucket,
                Prefix: invalid_dir_key,
                Delimiter: '/',
            });
    ...
    mocha.it(
        'PUT file directly to FS/head object - xattr decoded invalid URI',
        async function () {
            const { access_key, secret_key } =
                account_correct_uid.access_keys[0];
            const s3_client = generate_s3_client(
            ...
            const get_res = await s3_client.getObject({
                Bucket: bucket,
                Key: key,
            });
    ...
    mocha.it(
        'PUT file directly to FS/head object - xattr encoded invalid URI',
        async function () {
            const { access_key, secret_key } =
                account_correct_uid.access_keys[0];
            const s3_client = generate_s3_client(
    ...
    mocha.it('list parts with wrong uid gid', async function () {
        ...
        try {
            // list_multiparts
            const res = await s3_wrong_uid.listParts({
                Bucket: bucket_name + '-s3',
                Key: 'ob1.txt',
                UploadId: res1.UploadId,
            });
            console.log(inspect(res));
            assert.fail(
                'unpreviliged account could not list object parts on nsfs bucket',
            );
        } catch (err) {
            assert.strictEqual(err.Code, 'AccessDenied');
        }
    ...
    mocha.it('put object with correct uid gid', async function () {
        ...
        const res = await s3_correct_uid.putObject({
            Bucket: bucket_name + '-s3',
            Key: 'ob1.txt',
            Body: generic_obj_body,
        });
    ...
    mocha.it(
        'delete bucket without uid, gid - bucket is not empty',
        async function () {
            // account without uid and gid not supported in NC
            if (is_nc_coretest) this.skip(); // eslint-disable-line no-invalid-this
            try {
                const res = await s3_owner.deleteBucket({
                    Bucket: bucket_name + '-s3',
                });
                console.log(inspect(res));
                assert.fail(
                    'unpreviliged account could delete bucket on nsfs: ',
                );
            } catch (err) {
                assert.strictEqual(err.Code, 'AccessDenied');
            }
    ...
    mocha.it('copy objects with wrong uid gid', async function () {
        try {
            const res = await s3_wrong_uid.copyObject({
                Bucket: bucket_name + '-s3',
                Key: 'ob2.txt',
                CopySource: bucket_name + '-s3/ob1.txt',
            });
            console.log(inspect(res));
            assert.fail(
                'unpreviliged account could copy objects on nsfs bucket',
            );
        } catch (err) {
            assert.strictEqual(err.Code, 'AccessDenied');
        }
    ...
    mocha.it(
        ...
        async function () {
            ...
            await s3_correct_uid.deleteObject({
                Bucket: bucket_name + '-s3',
                Key: key,
            });
        },
    ...
    mocha.it(
        'copy objects with correct uid gid - non server side copy - fallback',
        async function () {
            const key = 'ob22.txt';
            ...
            assert.deepEqual(
                _.omit(stat1.xattr, 'user.new_xattr'),
                stat_source.xattr,
            );
        },
    );
    ...
    mocha.it('list objects with wrong uid gid', async function () {
        try {
            const res = await s3_wrong_uid.listObjects({
                Bucket: bucket_name + '-s3',
            });
            console.log(inspect(res));
            assert.fail(
                'unpreviliged account could list objects on nsfs bucket',
            );
        } catch (err) {
            assert.strictEqual(err.Code, 'AccessDenied');
        }
    ...
    mocha.it(
        'delete bucket with uid, gid - bucket is not empty',
        async function () {
            try {
                const res = await s3_correct_uid_default_nsr.deleteBucket({
                    Bucket: bucket_name + '-s3',
                });
                console.log(inspect(res));
                assert.fail(
                    'should fail deletion od bucket, bucket is not empty',
                );
            } catch (err) {
    ...
    mocha.it(
        'delete object without uid, gid - bucket is empty',
        async function () {
            try {
                ...
                console.log(inspect(res));
                assert.fail(
                    'unpreviliged account could delete object on nsfs bucket ',
                );
            } catch (err) {
                assert.strictEqual(err.Code, 'AccessDenied');
            }
    ...
    mocha.it('delete object with uid, gid', async function () {
        const res = await s3_correct_uid_default_nsr.deleteObject({
            Bucket: bucket_name + '-s3',
            Key: 'ob1.txt',
        });
        console.log(inspect(res));
    ...
    mocha.it(
        'delete multiple non existing objects without failing',
        async function () {
            ...
            const res = await s3_correct_uid_default_nsr.deleteObjects({
                Bucket: bucket_name + '-s3',
                Delete: {
    ...
    mocha.it(
        'delete bucket without uid, gid - bucket is empty',
        async function () {
    ...
    mocha.it(
        'delete account by uid, gid - account has buckets - should fail',
        async function () {
            // deletion of account that owns bucket currently allowd in NC - remove when supported
            if (is_nc_coretest) this.skip(); // eslint-disable-line no-invalid-this
            await s3_correct_uid_default_nsr.createBucket({
                Bucket: 'bucket-to-delete123',
            });
            try {
                ...
                assert.fail(
                    `delete account succeeded for account that has buckets`,
                );
            } catch (err) {
            ...
            await s3_correct_uid_default_nsr.deleteBucket({
                Bucket: 'bucket-to-delete123',
            });
        },
    );
    ...
    mocha.it(
        'delete bucket with uid, gid - bucket is empty',
        async function () {
            const res = await s3_correct_uid_default_nsr.deleteBucket({
                Bucket: bucket_name + '-s3',
            });
    ...
    mocha.it('delete account by uid, gid', async function () {
        this.timeout(600000); // eslint-disable-line no-invalid-this
        const read_account_resp1 = await rpc_client.account.read_account({
            email: 'account_wrong_uid0@noobaa.com',
        });
        ...
        for (let i = 0; i < 2; i++) {
            try {
                const deleted_account_exist =
                    await rpc_client.account.read_account({
                        email: `account_wrong_uid${i}@noobaa.com`,
                    });
                assert.fail(
                    `found account: ${deleted_account_exist} - account should be deleted`,
                );
            } catch (err) {
                if (is_nc_coretest) {
                    assert.equal(
                        JSON.parse(err.stdout).error.code,
                        ManageCLIError.NoSuchAccountName.code,
                    );
                } else {
        ...
        const list_account_resp2 = (await rpc_client.account.list_accounts({}))
            .accounts;
    ...
    mocha.it('delete account by dn', async function () {
        ...
        // check that both accounts deleted
        const accounts_expected_to_be_deleted = [
            `${no_permissions_dn}1@noobaa.com`,
            `${no_permissions_dn}@noobaa.io`,
        ];
        for (let i = 0; i < accounts_expected_to_be_deleted.length; i++) {
            try {
                const deleted_account_exist =
                    await rpc_client.account.read_account({
                        email: accounts_expected_to_be_deleted[i],
                    });
                assert.fail(
                    `found account: ${deleted_account_exist} - account should be deleted`,
                );
    ...
    mocha.it('delete account by uid, gid - no such account', async function () {
        ...
        if (is_nc_coretest) this.skip(); // eslint-disable-line no-invalid-this
        const list_account_resp1 = (await rpc_client.account.list_accounts({}))
            .accounts;
        assert.ok(list_account_resp1.length > 0);
        try {
            await rpc_client.account.delete_account_by_property({
                nsfs_account_config: { uid: 26041993, gid: 26041993 },
            });
...
mocha.describe('list objects - namespace_fs', async function () {
    ...
    mocha.before(async function () {
        ...
        const obj_nsr = {
            resource: namespace_resource_name,
            path: bucket_path,
        };
        // give read and write permission to owner
        if (is_nc_coretest) {
        ...
        for (const account_name of Object.keys(accounts)) {
            ...
            const account = await generate_nsfs_account(
                rpc_client,
                EMAIL,
                ls_new_buckets_path,
                {
                    ...
                    gid,
                    account_name,
                    default_resource: namespace_resource_name,
                },
            );
            const s3_client = generate_s3_client(
                account.access_key,
    ...
    mocha.it(
        'list buckets - uid & gid mismatch - should fail',
        async function () {
    ...
    mocha.it(
        'put object 1 account_with_access - uid & gid mismatch - should fail',
        async function () {
            try {
                const { s3_client } = accounts.account1;
                await s3_client.putObject({
                ...
                assert.fail(
                    `put object succeeded for account without permissions`,
                );
            } catch (err) {
    ...
    mocha.it(
        'list objects - no access to bucket - uid & gid mismatch - should fail',
        async function () {
            try {
                const { s3_client } = accounts.account3;
                await s3_client.listObjects({ Bucket: bucket_name });
                assert.fail(
                    `list objects succeeded for account without permissions`,
                );
            } catch (err) {
                assert.equal(err.Code, 'AccessDenied');
            }
    ...
    mocha.it(
        'list objects s3_b_name - uid & gid mismatch - should fail',
        async function () {
            try {
                const { s3_client } = accounts.account3;
    ...
    mocha.it('list buckets - uid & gid mismatch - account1', async function () {
        ...
        assert.equal(
            res.Buckets.filter(
                bucket =>
                    bucket.Name === s3_b_name || bucket.Name === first_bucket,
            ).length,
            2,
        );
    });
    ...
    mocha.it(
        'list buckets - uid & gid match - account with permission',
        async function () {
            const { s3_client } = accounts.account4;
            const res = await s3_client.listBuckets({});
...
mocha.describe('nsfs account configurations', function () {
    ...
    const regular_bucket_name = [
        'regular-bucket',
        'regular-bucket1',
        'regular-bucket2',
    ];
    const regular_bucket_fail = [
        'regular-bucket-fail',
        'regular-bucket-fail1',
        'regular-bucket-fail2',
    ];
    ...
    mocha.before(async () =>
        fs_utils.create_fresh_path(tmp_fs_root1 + bucket_path, 0o770),
    );
    mocha.after(async () => {
        for (const bucket_name of [
            bucket_name1,
            data_bucket,
            non_nsfs_bucket2,
        ]) {
    ...
    // default pool
    mocha.it(
        's3 put bucket allowed non nsfs buckets - default pool',
        async function () {
    ...
    // default nsr - nsfs
    mocha.it(
        's3 put bucket allowed non nsfs buckets - default nsr - nsfs',
        async function () {
    ...
    mocha.it(
        's3 put bucket not allowed non nsfs buckets - default nsr - nsfs',
        async function () {
            ...
            const list_ok = bucket_in_list(
                [bucket_name1, regular_bucket_name[2]],
                [non_nsfs_bucket1, non_nsfs_bucket2],
                res.Buckets,
            );
            assert.ok(list_ok);
        },
    ...
    mocha.it(
        's3 put bucket not allowed non nsfs buckets - default pool',
        async function () {
            try {
                const s3_account = accounts.account_nsfs_only1;
                await s3_account.createBucket({
                    Bucket: regular_bucket_fail[0],
                });
    ...
    mocha.it(
        's3 put bucket allowed non nsfs buckets - default nsr - s3 compatible',
        async function () {
            try {
                const s3_account = accounts.account2;
                await s3_account.createBucket({
                    Bucket: regular_bucket_fail[1],
                });
            } catch (err) {
    ...
    mocha.it(
        's3 put bucket not allowed non nsfs buckets - default nsr - s3 compatible',
        async function () {
            try {
                const s3_account = accounts.account_nsfs_only2;
                await s3_account.createBucket({
                    Bucket: regular_bucket_fail[2],
                });
                assert.fail('account should not be allowed to create bucket');
            } catch (err) {
                assert.ok(err.Code === 'AccessDenied');
    ...
    mocha.it(
        's3 object bucket using nsfs_only=false account - regular-bucket',
        async function () {
            const s3_account = accounts.account1;
            // Give account access to the required buckets
            await Promise.all(
    ...
    mocha.it(
        's3 put object using nsfs_only=true account - first.bucket',
        async function () {
            const s3_account = accounts.account_nsfs_only1;
            // Give account access to the required buckets
            await Promise.all(
    ...
    mocha.it(
        's3 put object using nsfs_only=false account - second.bucket(s3 compatible namespace bucket)',
        async function () {
            const s3_account = accounts.account1;
            // Give account access to the required buckets
            await Promise.all(
    ...
    mocha.it(
        's3 put object using nsfs_only=true account - second.bucket(s3 compatible namespace bucket)',
        async function () {
            const s3_account = accounts.account_nsfs_only1;
            // Give account access to the required buckets
            await Promise.all(
    ...
    mocha.it(
        's3 put object allowed non nsfs buckets - nsfs bucket',
        async function () {
            const s3_account = accounts.account1;
            // Give account access to the required buckets
            await Promise.all(
...
mocha.describe('list buckets - namespace_fs', async function () {
    ...
    mocha.before(async function () {
        ...
        CORETEST_ENDPOINT = coretest.get_http_address();
        for (const account_name of Object.keys(accounts)) {
            const { create_bucket_via, bucket, account_info } =
    ...
    mocha.it(
        'list buckets - each account can list only its own bucket - no bucket policies applied ',
        async function () {
    ...
    mocha.it(
        'account1 - all accounts are allowed to list bucket1',
        async function () {
            // allow all accounts to list bucket1
            const public_bucket = accounts.account1.bucket;
            const bucket_policy = generate_s3_policy('*', public_bucket, [
                's3:ListBucket',
            ]);
            await rpc_client.bucket.put_bucket_policy({
                name: public_bucket,
    ...
    mocha.it(
        'account2 - set allow only account1 list bucket2, account1/account2 can list bucket2 but account3 cant',
        async function () {
            ...
            const account1_principal =
                is_nc_coretest ? account_name : `${account_name}@noobaa.com`;
            const bucket_policy = generate_s3_policy(
                account1_principal,
                bucket2,
                ['s3:ListBucket'],
            );
...
mocha.describe('s3 whitelist flow', async function () {
    ...
    mocha.before(async function () {
        ...
        await rpc_client.pool.create_namespace_resource({
            name: nsr,
            nsfs_config: { fs_root_path: fs_path },
        });
    ...
    mocha.after(async function () {
        ...
        await exec_manage_cli(TYPES.IP_WHITELIST, '', {
            ips: JSON.stringify([]),
        });
    });

    mocha.it(
        'whitelist ips empty - s3 request should succeed',
        async function () {
    ...
    mocha.it(
        'server ip is in whitelist ips - s3 request should succeed',
        async function () {
    ...
    mocha.it(
        'server ip is not in whitelist ips - s3 request should fail',
        async function () {
            ...
            try {
                ...
                assert.fail(
                    'test should have failed with AccessDenied - server ip is not in whitelist ips',
                );
            } catch (err) {
                assert.equal(err.Code, 'AccessDenied');
            }
...
mocha.describe('Namespace s3_bucket_policy', function () {
    ...
    if (is_nc_coretest) {
        ...
        account_config_path =
            NC_CORETEST_CONFIG_FS.get_account_or_user_path_by_name(
    ...
    mocha.it(
        'Missing anonymous account - user should not be able to list bucket objects',
        async function () {
            // Skipping because only NC NSFS will have anonymous account.
            if (!is_nc_coretest) this.skip(); // eslint-disable-line no-invalid-this
            await s3_client.createBucket({ Bucket: bucket_name });
            await fs_utils.file_must_exist(
                path.join(s3_new_ns_buckets_path, bucket_name),
            );
            ...
            await assert_throws_async(
                s3_anon_client.listObjects({ Bucket: bucket_name }),
    ...
    mocha.it(
        'Namesapce - anonymous user should be able to list bucket objects',
        async function () {
            // Skipping because only NC NSFS will have anonymous account.
            if (!is_nc_coretest) this.skip(); // eslint-disable-line no-invalid-this
            await fs_utils.file_must_exist(
    ...
    mocha.it(
        'anonymous user should not be able to list bucket objects when there is no policy',
        async function () {
            // Skipping because only NC NSFS will have anonymous account.
            if (!is_nc_coretest) this.skip(); // eslint-disable-line no-invalid-this
    ...
    mocha.it(
        "anonymous user should not be able to list bucket objects when policy doesn't allow",
        async function () {
            // Skipping because only NC NSFS will have anonymous account.
            if (!is_nc_coretest) this.skip(); // eslint-disable-line no-invalid-this
    ...
    mocha.it(
        'anonymous user should not be able to getObject when not explicitly allowed',
        async function () {
            // Skipping because only NC NSFS will have anonymous account.
            if (!is_nc_coretest) this.skip(); // eslint-disable-line no-invalid-this
    ...
    mocha.it(
        'anonymous user should not be able to read_object_md when explicitly allowed to access only specific key',
        async function () {
            // Skipping because only NC NSFS will have anonymous account.
            if (!is_nc_coretest) this.skip(); // eslint-disable-line no-invalid-this
    ...
    mocha.it(
        'anonymous user should not be able to putObject when not explicitly allowed',
        async function () {
            // Skipping because only NC NSFS will have anonymous account.
            if (!is_nc_coretest) this.skip(); // eslint-disable-line no-invalid-this
            // Ensure that the bucket has no policy
    ...
    mocha.it(
        'anonymous user should be able to putBucketPolicy when explicitly allowed',
        async function () {
            // Skipping because only NC NSFS will have anonymous account.
            if (!is_nc_coretest) this.skip(); // eslint-disable-line no-invalid-this
    ...
    mocha.it(
        'anonymous user should be able to putBucketPolicy when explicitly allowed to access only specific key',
        async function () {
            // Skipping because only NC NSFS will have anonymous account.
            if (!is_nc_coretest) this.skip(); // eslint-disable-line no-invalid-this
            const anon_read_policy_2 = {
            ...
            await assert_throws_async(
                s3_anon_client.putBucketPolicy({
                    Bucket: bucket_name,
                    Policy: JSON.stringify(anon_read_policy_2),
                }),
            );
    ...
    mocha.it(
        'Namesapce - distinguished_name - Anonymous user should be able to list bucket objects',
        async function () {
            // Skipping because only NC NSFS will have anonymous account.
            if (!is_nc_coretest) this.skip(); // eslint-disable-line no-invalid-this
            await fs_utils.file_must_exist(
            ...
            await delete_anonymous_account(
                accounts_dir_path,
                account_config_path,
            );
...
async function assert_throws_async(
    promise,
...
async function put_and_delete_objects(
    s3_account,
    bucket,
    key,
    body,
...
function bucket_in_list(
    ...
    const doesnt_exist_checker = not_exist_buckets.every(
        v => !bucket_names.includes(v),
    );
...
async function update_account_nsfs_config(
    email,
    default_resource,
    new_nsfs_account_config,
    should_fail,
    error_code,
) {
    try {
        ...
        if (should_fail) {
            assert.fail(
                `update_account_nsfs_config - action should fail but it didn't`,
            );
        }
    } catch (err) {
        if (should_fail) {
...
async function add_anonymous_account(
    nsfs_account_config,
    accounts_dir_path,
    account_config_path,
) {
    ...
    const name_exists = await native_fs_utils.is_path_exists(
        DEFAULT_FS_CONFIG,
        account_config_path,
    );
    if (name_exists) {
...
async function delete_anonymous_account(
    accounts_dir_path,
    account_config_path,
) {
    const name_exists = await native_fs_utils.is_path_exists(
