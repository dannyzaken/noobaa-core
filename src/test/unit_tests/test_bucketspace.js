...
const { rpc_client, EMAIL, PASSWORD, SYSTEM, get_admin_account_details } =
  coretest;
coretest.setup({});
...
mocha.describe('bucket operations - namespace_fs', function () {
  ...
  const public_new_buckets_dir =
    process.env.NC_CORETEST ? '/public_new_buckets_dir/' : '/';
  const public_new_buckets_path = path.join(
    tmp_fs_root,
    public_new_buckets_dir,
  );
  ...
  mocha.it('Init S3 owner connection', async function () {
    const admin_keys = (await rpc_client.account.read_account({ email: EMAIL }))
      .access_keys;
    ...
    async function () {
      ...
      try {
        await rpc_client.account.delete_account_by_property({
          nsfs_account_config: { uid: process.getuid(), gid: process.getgid() },
        });
        assert.fail(`delete account succeeded for account that has buckets`);
      } catch (err) {
        assert.equal(err.rpc_code, 'FORBIDDEN');
  ...
  mocha.it('delete account by uid, gid', async function () {
    ...
    for (let i = 0; i < 2; i++) {
      try {
        const deleted_account_exist = await rpc_client.account.read_account({
...
mocha.describe('list objects - namespace_fs', async function () {
  ...
  mocha.it('list buckets - uid & gid mismatch - account1', async function () {
    ...
    assert.equal(
      res.Buckets.filter(
        bucket => bucket.Name === s3_b_name || bucket.Name === first_bucket,
      ).length,
      2,
    );
  });
...
mocha.describe('list buckets - namespace_fs', async function () {
  ...
  mocha.before(async function () {
    ...
    if (invalid_nsfs_root_permissions() || !process.env.NC_CORETEST) {
  ...
  mocha.it(
    ...
    async function () {
      ...
      const account1_principal =
        process.env.NC_CORETEST ? account_name : `${account_name}@noobaa.com`;
      const bucket_policy = generate_s3_policy(account1_principal, bucket2, [
        's3:ListBucket',
      ]);
      await rpc_client.bucket.put_bucket_policy({
        name: bucket2,
