...
mocha.describe('manage_nsfs cli', function () {
  ...
  mocha.before(async () => {
    await P.all(
      _.map(
        [
          CONFIG_SUBDIRS.ACCOUNTS,
          CONFIG_SUBDIRS.BUCKETS,
          CONFIG_SUBDIRS.ACCESS_KEYS,
        ],
        async dir => fs_utils.create_fresh_path(`${config_root}/${dir}`),
      ),
    );
  ...
  mocha.describe('cli bucket flow ', async function () {
    ...
    const invalid_bucket_policy = generate_s3_policy('invalid_account', name, [
      's3:*',
    ]).policy;
    ...
    mocha.it(
      ...
      async function () {
        ...
        try {
          await fs_utils.create_fresh_path(bucket_path);
          await fs_utils.file_must_exist(bucket_path);
          const account = await read_config_file(
            config_root,
            CONFIG_SUBDIRS.ACCOUNTS,
            account_name,
          );
    ...
    mocha.it('cli bucket create - bucket_with_policy', async function () {
      ...
      const bucket = await read_config_file(
        config_root,
        CONFIG_SUBDIRS.BUCKETS,
        bucket_with_policy,
      );
  ...
  mocha.describe('cli account flow', async function () {
    ...
    mocha.it('cli account create', async function () {
      ...
      const account_symlink = await read_config_file(
        config_root,
        CONFIG_SUBDIRS.ACCESS_KEYS,
        access_key,
        true,
    ...
    mocha.it('cli account status', async function () {
      ...
      const access_key_account_status = await exec_manage_cli(type, action, {
        config_root,
...
async function read_config_file(
  ...
  const config_path = path.join(
    config_root,
    schema_dir,
    config_file_name + (is_symlink ? '.symlink' : '.json'),
  );
  ...
  if (config_data.access_keys) {
    const encrypted_secret_key =
      config_data.access_keys[0].encrypted_secret_key;
    config_data.access_keys[0].secret_key = await nc_mkm.decrypt(
      encrypted_secret_key,
      config_data.master_key_id,
    );
...
async function write_config_file(
  config_root,
  schema_dir,
  config_file_name,
  data,
...
function assert_bucket(bucket, bucket_options) {
  ...
  assert.strictEqual(
    bucket.fs_backend,
    bucket_options.fs_backend === '' ? undefined : bucket_options.fs_backend,
  );
...
function assert_account(account, account_options, skip_secrets) {
  ...
  assert.equal(
    ...
    account_options.fs_backend === '' ? undefined : account_options.fs_backend,
  );
  return true;
}
