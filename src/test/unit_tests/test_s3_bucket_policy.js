...
coretest.setup({
    pools_to_create: process.env.NC_CORETEST ? undefined : [POOL_LIST[1]],
});
...
mocha.describe('s3_bucket_policy', function () {
    mocha.before(setup);
    mocha.it(
        "should fail setting bucket policy when user doesn't exist",
        async function () {
    ...
    mocha.it(
        "should fail setting bucket policy when resource doesn't exist",
        async function () {
    ...
    mocha.it(
        'should fail setting bucket policy when action is illeagel',
        async function () {
    ...
    mocha.it(
        'should only read bucket policy when have permission to',
        async function () {
            const policy = {
                Statement: [
                    {
            ...
            const res_a = await s3_a.getBucketPolicy({
                // should work - user a has get_bucket_policy permission
                Bucket: BKT,
            });
            console.log('Policy set', res_a);
            await assert_throws_async(
                s3_b.getBucketPolicy({
                    // should fail - user b has no permissions
                    Bucket: BKT,
                }),
            );
        },
    ...
    mocha.it(
        'should put/get bucket policy - principal by account name',
        async function () {
            ...
            console.log(
                'Policy set (principal by name)',
                res_get_bucket_policy.Policy,
            );
        },
    );

    mocha.it(
        'should put object to permitted account (bucket policy - principal by account name)',
        async function () {
            if (!is_nc_coretest) this.skip(); // eslint-disable-line no-invalid-this
            const policy = {
                Statement: [
    ...
    mocha.it(
        'should put/get bucket policy - principal by account ID',
        async function () {
            if (!is_nc_coretest) this.skip(); // eslint-disable-line no-invalid-this
            const policy = {
                Statement: [
    ...
    mocha.it(
        'should put object to permitted account (bucket policy - principal by account ID',
        async function () {
            if (!is_nc_coretest) this.skip(); // eslint-disable-line no-invalid-this
            const policy = {
                Statement: [
    ...
    mocha.it(
        'should be able to set bucket policy when none set',
        async function () {
            const self = this; // eslint-disable-line no-invalid-this
            self.timeout(15000);
            const policy = {
            ...
            await s3_owner.deleteBucketPolicy({
                // should work - owner can always delete the buckets policy
                Bucket: BKT,
            });
            await s3_owner.putBucketPolicy({
                // s3_owner can set the policy
                Bucket: BKT,
                Policy: JSON.stringify(policy),
            });
        },
    ...
    mocha.it(
        'should be able to put and list files when bucket policy permits',
        async function () {
            const self = this; // eslint-disable-line no-invalid-this
            self.timeout(15000);
            const policy = {
            ...
            await s3_a.listObjects({
                // should succeed - user a has can list
                Bucket: BKT,
            });
            await assert_throws_async(
                s3_b.listObjects({
                    // should fail - user b can't
                    Bucket: BKT,
                }),
            );
        },
    ...
    mocha.it(
        'should be able to deny write some file but not other',
        async function () {
            const self = this; // eslint-disable-line no-invalid-this
            self.timeout(15000);
    ...
    mocha.it(
        'should be able to support write * and ? in resource',
        async function () {
            const self = this; // eslint-disable-line no-invalid-this
            self.timeout(15000);
                ...
                Statement: [
                    {
                        ...
                        Action: ['s3:GetObject', 's3:PutObject'],
                        Resource: [
                            `arn:aws:s3:::${BKT}/user_?_files/j?st_*.txt`,
                        ],
                    },
                    {
                        Sid: 'id-2',
    ...
    mocha.it(
        'should be able to put versionning when bucket policy permits',
        async function () {
            const self = this; // eslint-disable-line no-invalid-this
            self.timeout(15000);
            ...
            await s3_a.deleteObject({
                // delete the file versions
                Bucket: BKT,
                Key: new_key,
                VersionId:
                    process.env.NC_CORETEST ? res.VersionId : 'nbver-' + seq,
            });
            await s3_a.deleteObject({
                Bucket: BKT,
                Key: new_key,
                VersionId:
                    process.env.NC_CORETEST ?
                        first_version_etag
                    :   'nbver-' + (seq - 1),
            });
        },
    ...
    mocha.it('should deny bucket owner access', async function () {
        ...
        await s3_b.putBucketPolicy({
            // should work - owner can update the buckets policy unless explicitly denied
            Bucket: BKT_B,
            Policy: JSON.stringify(policy),
        });
        ...
        await assert_throws_async(
            s3_b.listObjects({
                // should fail - bucket owner cwas explicitly denied
                Bucket: BKT_B,
            }),
        );
    });
    ...
    mocha.it(
        'should allow acces after adding anonymous access',
        async function () {
            await s3_owner.putBucketPolicy({
                // should work - system owner can always update the buckets policy
                Bucket: BKT,
    ...
    mocha.it(
        'should set and delete bucket policy when system owner',
        async function () {
            ...
            await s3_owner.putBucketPolicy({
                // should work - owner can always update the buckets policy
                Bucket: BKT,
                Policy: JSON.stringify(policy),
            });
            await s3_owner.deleteBucketPolicy({
    ...
    mocha.it(
        'anonymous user should be able to list bucket objects',
        async function () {
            await s3_owner.putBucketPolicy({
                Bucket: BKT,
                Policy: JSON.stringify(anon_access_policy),
    ...
    mocha.it(
        '[RPC TEST]: anonymous user should not be able to read_object_md when not explicitly allowed',
        async function () {
            ...
            await assert_throws_async(
                anon_rpc_client.object.read_object_md({
                    bucket: BKT,
                    key: KEY,
                }),
    ...
    mocha.it(
        '[RPC TEST]: anonymous user should be able to read_object_md when explicitly allowed',
        async function () {
            // MD ops not implemented on NC - skipping
            if (process.env.NC_CORETEST) this.skip(); // eslint-disable-line no-invalid-this
    ...
    mocha.it(
        '[RPC TEST]: anonymous user should be able to read_object_md when explicitly allowed to access only specific key',
        async function () {
            // MD ops not implemented on NC - skipping
            if (process.env.NC_CORETEST) this.skip(); // eslint-disable-line no-invalid-this
    ...
    mocha.it(
        '[RPC TEST]: anonymous user should not be able to read_object_mapping when not explicitly allowed',
        async function () {
            // MD ops not implemented on NC - skipping
            if (process.env.NC_CORETEST) this.skip(); // eslint-disable-line no-invalid-this
            // Ensure that the bucket has no policy
            ...
            await assert_throws_async(
                anon_rpc_client.object.read_object_mapping({
                    bucket: BKT,
                    key: KEY,
                    obj_id: MDStore.instance().make_md_id(
                        cross_test_store.obj.obj_id,
                    ),
                }),
                'requesting account is not authorized to read the object',
            );
    ...
    mocha.it(
        '[RPC TEST]: anonymous user should be able to read_object_mapping when explicitly allowed',
        async function () {
            // MD ops not implemented on NC - skipping
            if (process.env.NC_CORETEST) this.skip(); // eslint-disable-line no-invalid-this
            const anon_read_policy = {
    ...
    mocha.it(
        '[RPC TEST]: anonymous user should be able to read_object_mapping when explicitly allowed to access only specific key',
        async function () {
            // MD ops not implemented on NC - skipping
            if (process.env.NC_CORETEST) this.skip(); // eslint-disable-line no-invalid-this
            const anon_read_policy_2 = {
    ...
    mocha.it(
        'should be able to deny based on server side encryption',
        async function () {
    ...
    mocha.it(
        'should be able to deny unencrypted object uploads',
        async function () {
            // SSE not implemented on NC - skipping
            if (process.env.NC_CORETEST) this.skip(); // eslint-disable-line no-invalid-this
            const self = this; // eslint-disable-line no-invalid-this
    ...
    mocha.it(
        'should be able to add StringLike and StringEqualsIgnoreCase condition statements, ',
        async function () {
            // SSE not implemented on NC - skipping
            if (process.env.NC_CORETEST) this.skip(); // eslint-disable-line no-invalid-this
            const self = this; // eslint-disable-line no-invalid-this
                ...
                Statement: [
                    {
                        ...
                        Condition: {
                            StringLike: {
                                's3:x-amz-server-side-encryption': 'AES*',
                            },
                        },
                    },
                ],
        ...
        const auth_put_policy = {
            ...
            Statement: [
                ...
                },
                {
                    ...
                    Condition: {
                        StringEquals: {
                            [`s3:ExistingObjectTag/${allow_tag.key}`]:
                                allow_tag.value,
                        },
                    },
                },
    ...
    mocha.it(
        'get_bucket_policy should return correct condition values',
        async function () {
            const self = this; // eslint-disable-line no-invalid-this
            self.timeout(15000);
            ...
            assert.strictEqual(
                Object.keys(actualEncryptionCondition)[0],
                'StringNotEquals',
            );
            assert.strictEqual(
                Object.keys(actualEncryptionCondition.StringNotEquals)[0],
                's3:x-amz-server-side-encryption',
            );
            assert.strictEqual(
                Object.values(actualEncryptionCondition.StringNotEquals)[0],
                'AES256',
            );
            const actualObjectTagCondition = policy.Statement[2].Condition;
            assert.strictEqual(
                Object.keys(actualObjectTagCondition)[0],
                'StringEquals',
            );
            assert.strictEqual(
                Object.keys(actualObjectTagCondition.StringEquals)[0],
                's3:ExistingObjectTag/key',
            );
            assert.strictEqual(
                actualObjectTagCondition.StringEquals[
                    's3:ExistingObjectTag/key'
                ],
                'value',
            );
    ...
    mocha.describe(
        'should only one of Argument or NotArgument',
        async function () {
            mocha.it(
                'should not allow both Action and NotAction',
                async function () {
                    const self = this; // eslint-disable-line no-invalid-this
                    self.timeout(15000);
                    const s3_policy = {
                    ...
                    try {
                        ...
                        assert.fail(
                            'Test was suppose to fail on ' +
                                S3Error.MalformedPolicy.code,
                        );
                    } catch (err) {
            ...
            mocha.it(
                'should not allow both Principal and NotPrincipal',
                async function () {
                    const self = this; // eslint-disable-line no-invalid-this
                    self.timeout(15000);
                    const s3_policy = {
            ...
            mocha.it(
                'should not allow both Resource and NotResource',
                async function () {
                    const self = this; // eslint-disable-line no-invalid-this
                    self.timeout(15000);
                    const s3_policy = {
            ...
            mocha.it(
                'should not allow policy without Principal or notPrincipal',
                async function () {
                    const self = this; // eslint-disable-line no-invalid-this
                    self.timeout(15000);
                    const s3_policy = {
            ...
            mocha.it(
                'should not allow policy without Resource or notResource',
                async function () {
                    const self = this; // eslint-disable-line no-invalid-this
                    self.timeout(15000);
                    const s3_policy = {
            ...
            mocha.it(
                'should not allow policy without action or notAction',
                async function () {
                    const self = this; // eslint-disable-line no-invalid-this
                    self.timeout(15000);
                    const s3_policy = {
    ...
    mocha.describe('get-bucket-policy status should work', async function () {
        mocha.it(
            'get-bucket-policy status should return true for public policy',
            async function () {
                const self = this; // eslint-disable-line no-invalid-this
                self.timeout(15000);
                const s3_policy = {
                ...
                const res = await s3_owner.getBucketPolicyStatus({
                    Bucket: BKT,
                });
        ...
        mocha.it(
            'get-bucket-policy status should return false for non "*" principal',
            async function () {
                const self = this; // eslint-disable-line no-invalid-this
                self.timeout(15000);
                const s3_policy = {
        ...
        mocha.it(
            'get-bucket-policy status should return false for Deny actions',
            async function () {
                const self = this; // eslint-disable-line no-invalid-this
                self.timeout(15000);
                const s3_policy = {
        ...
        mocha.it(
            'get-bucket-policy status should work for pricipal as a string',
            async function () {
                const self = this; // eslint-disable-line no-invalid-this
                self.timeout(15000);
                const s3_policy = {
        ...
        mocha.it(
            'get-bucket-policy principal should return true if at least one statement is public with principal * ',
            async function () {
                const self = this; // eslint-disable-line no-invalid-this
                self.timeout(15000);
                const s3_policy = {
