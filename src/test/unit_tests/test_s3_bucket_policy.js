...
async function setup() {
  ...
  const user_a_keys = (await rpc_client.account.create_account(account))
    .access_keys;
  account.name = user_b;
  account.email = user_b;
  const user_b_keys = (await rpc_client.account.create_account(account))
    .access_keys;
...
mocha.describe('s3_bucket_policy', function () {
  ...
  mocha.it(
    ...
    async function () {
      ...
      await s3_a.deleteObject({
        ...
        Key: new_key,
        VersionId:
          process.env.NC_CORETEST ? first_version_etag : 'nbver-' + (seq - 1),
      });
    },
  );
        ...
        Statement: [
          {
            ...
            Resource: [`arn:aws:s3:::${BKT}/${KEY}`],
            Condition: {
              StringLike: { 's3:x-amz-server-side-encryption': 'AES*' },
            },
          },
        ],
      };
  ...
  mocha.it(
    ...
    async function () {
      ...
      const encryption_policy = {
        ...
        Statement: [
          ...
          },
          {
            ...
            Resource: [`arn:aws:s3:::${BKT}/${KEY}`],
            Condition: {
              StringEquals: { 's3:ExistingObjectTag/key': 'value' },
            },
          },
        ],
      };
      ...
      assert.strictEqual(
        actualObjectTagCondition.StringEquals['s3:ExistingObjectTag/key'],
        'value',
      );
    },
  );
    ...
    async function () {
      mocha.it('should not allow both Action and NotAction', async function () {
        ...
        try {
          ...
          assert.fail(
            'Test was suppose to fail on ' + S3Error.MalformedPolicy.code,
          );
        } catch (err) {
