...
mocha.describe('nsfs nc health', function () {
    ...
    const config_root_invalid = path.join(
        tmp_fs_path,
        'config_root_nsfs_health_invalid',
    );
    let Health;
    ...
    mocha.describe('nsfs nc health cli validations', function () {
        mocha.it(
            'https_port flag type validation - should fail',
            async function () {
                try {
                    await exec_manage_cli(
                        TYPES.DIAGNOSE,
                        DIAGNOSE_ACTIONS.HEALTH,
                        { http_port: '' },
                    );
        ...
        mocha.it(
            'all_account_details flag type validation - should fail',
            async function () {
                try {
                    await exec_manage_cli(
                        TYPES.DIAGNOSE,
                        DIAGNOSE_ACTIONS.HEALTH,
                        { all_account_details: 'bla' },
                    );
        ...
        mocha.it(
            'all_bucket_details flag type validation - should fail',
            async function () {
                try {
                    await exec_manage_cli(
                        TYPES.DIAGNOSE,
                        DIAGNOSE_ACTIONS.HEALTH,
                        { all_bucket_details: 7000 },
                    );
        ...
        mocha.it('all_bucket_details flag type validation', async function () {
            ...
            assert.ok(
                parsed_res.checks.buckets_status.invalid_buckets.length >= 0,
            );
            assert.ok(
                parsed_res.checks.buckets_status.valid_buckets.length >= 0,
            );
        ...
        mocha.it('all_account_details flag type validation', async function () {
            ...
            assert.ok(
                parsed_res.checks.accounts_status.invalid_accounts.length >= 0,
            );
            assert.ok(
                parsed_res.checks.accounts_status.valid_accounts.length >= 0,
            );
    ...
    mocha.describe('health check', async function () {
        ...
        const account_inaccessible_options = {
            name: 'account_inaccessible',
            uid: 999,
            gid: 999,
            new_buckets_path: bucket_storage_path,
        };
        const account_inaccessible_dn_options = {
            name: 'account_inaccessible_dn',
            user: 'inaccessible_dn',
            new_buckets_path: bucket_storage_path,
        };
        const invalid_account_dn_options = {
            name: 'invalid_account_dn',
            user: 'invalid_account_dn',
            new_buckets_path: bucket_storage_path,
        };
        const fs_users = {
            other_user: {
                distinguished_name:
                    account_inaccessible_dn_options.nsfs_account_config
                        .distinguished_name,
                uid: 1312,
                gid: 1312,
            },
        ...
        mocha.before(async () => {
            ...
            await exec_manage_cli(TYPES.ACCOUNT, ACTIONS.ADD, {
                config_root,
                ...account1_options,
            });
            ...
            const get_service_memory_usage = sinon.stub(
                Health,
        ...
        mocha.after(async () => {
            ...
            await exec_manage_cli(TYPES.BUCKET, ACTIONS.DELETE, {
                config_root,
                name: bucket1_options.name,
            });
            await exec_manage_cli(TYPES.ACCOUNT, ACTIONS.DELETE, {
                config_root,
                name: account1_options.name,
            });
            for (const user of Object.values(fs_users)) {
        ...
        mocha.it('Health all condition is success', async function () {
            ...
            get_service_state
                .onFirstCall()
                .returns(
                    Promise.resolve({ service_status: 'active', pid: 100 }),
                )
                .onSecondCall()
                .returns(
                    Promise.resolve({ service_status: 'active', pid: 200 }),
                );
            const get_endpoint_response = sinon.stub(
                Health,
                'get_endpoint_response',
            );
            get_endpoint_response.onFirstCall().returns(
                Promise.resolve({
                    response: {
            ...
            assert.strictEqual(
                health_status.checks.buckets_status.invalid_buckets.length,
                0,
            );
            assert.strictEqual(
                health_status.checks.accounts_status.valid_accounts.length,
                1,
            );
            assert.strictEqual(
                health_status.checks.accounts_status.valid_accounts[0].name,
                'account1',
            );
            assert.strictEqual(
                health_status.checks.buckets_status.valid_buckets.length,
                1,
            );
            assert.strictEqual(
                health_status.checks.buckets_status.valid_buckets[0].name,
                'bucket1',
            );
        });
        ...
        mocha.it('NooBaa service is inactive', async function () {
            ...
            get_service_state
                .onFirstCall()
                .returns(
                    Promise.resolve({ service_status: 'inactive', pid: 0 }),
                )
                .onSecondCall()
                .returns(
                    Promise.resolve({ service_status: 'active', pid: 200 }),
            ...
            assert.strictEqual(
                health_status.error.error_code,
                'NOOBAA_SERVICE_FAILED',
            );
        });

        mocha.it(
            'NooBaa endpoint return error response is inactive',
            async function () {
                Health.get_service_state.restore();
                Health.get_endpoint_response.restore();
                const get_service_state = sinon.stub(
                    Health,
                    'get_service_state',
                );
                get_service_state
        ...
        mocha.it('NSFS account with invalid storage path', async function () {
            Health.get_service_state.restore();
            Health.get_endpoint_response.restore();
            const account_invalid_options = {
                name: 'account_invalid',
                new_buckets_path: new_buckets_path + '/invalid',
            };
            await exec_manage_cli(TYPES.ACCOUNT, ACTIONS.ADD, {
                config_root,
                ...account_invalid_options,
            });
            const get_service_state = sinon.stub(Health, 'get_service_state');
            get_service_state
                .onFirstCall()
                .returns(
                    Promise.resolve({ service_status: 'active', pid: 1000 }),
                )
                .onSecondCall()
                .returns(
                    Promise.resolve({ service_status: 'active', pid: 2000 }),
                );
            const get_endpoint_response = sinon.stub(
                Health,
                'get_endpoint_response',
            ...
            assert.strictEqual(health_status.status, 'OK');
            assert.strictEqual(
                health_status.checks.accounts_status.invalid_accounts.length,
                1,
            );
            assert.strictEqual(
                health_status.checks.accounts_status.invalid_accounts[0].name,
                'account_invalid',
            );
            await exec_manage_cli(TYPES.ACCOUNT, ACTIONS.DELETE, {
                config_root,
                name: account_invalid_options.name,
            });
        });
        ...
        mocha.it('NSFS bucket with invalid storage path', async function () {
            ...
            const bucket_invalid = {
                name: 'bucket_invalid',
                path: new_buckets_path + '/bucket1/invalid',
            };
            ...
            assert.strictEqual(
                health_status.checks.buckets_status.invalid_buckets[0].name,
                'bucket_invalid',
            );
            await exec_manage_cli(TYPES.BUCKET, ACTIONS.DELETE, {
                config_root,
                name: bucket_invalid.name,
            });
        });
        ...
        mocha.it('NSFS invalid bucket schema json', async function () {
            Health.get_service_state.restore();
            Health.get_endpoint_response.restore();
            const bucket_invalid_schema = {
                name: 'bucket_invalid_schema',
            ...
            assert.strictEqual(
                health_status.checks.buckets_status.invalid_buckets[0].name,
                'bucket_invalid_schema',
            );
            await exec_manage_cli(TYPES.BUCKET, ACTIONS.DELETE, {
                config_root,
                name: bucket_invalid_schema.name,
            });
        });
        ...
        mocha.it('NSFS invalid account schema json', async function () {
            Health.get_service_state.restore();
            Health.get_endpoint_response.restore();
            const account_invalid_schema = {
                _id: '9',
                name: 'account_invalid_schema',
                path: new_buckets_path,
                bla: 5,
            };
            await test_utils.write_manual_config_file(
                TYPES.ACCOUNT,
                config_fs,
                account_invalid_schema,
                'invalid',
            );
            const get_service_state = sinon.stub(Health, 'get_service_state');
            get_service_state
            ...
            assert.strictEqual(
                health_status.checks.accounts_status.invalid_accounts[0].name,
                account_invalid_schema.name,
            );
            await exec_manage_cli(TYPES.ACCOUNT, ACTIONS.DELETE, {
                config_root,
                name: account_invalid_schema.name,
            });
        });

        mocha.it(
            'Health all condition is success, all_account_details is false',
            async function () {
                Health.get_service_state.restore();
                Health.get_endpoint_response.restore();
                ...
                assert.strictEqual(
                    health_status.checks.accounts_status,
                    undefined,
                );
            },
        );
        ...
        mocha.it(
            'Health all condition is success, all_bucket_details is false',
            async function () {
                Health.get_service_state.restore();
                Health.get_endpoint_response.restore();
                Health.all_account_details = true;
                ...
                assert.strictEqual(health_status.status, 'OK');
                assert.strictEqual(
                    health_status.checks.buckets_status,
                    undefined,
                );
                assert.strictEqual(
                    health_status.checks.accounts_status.valid_accounts.length,
                ...
                assert.strictEqual(
                    health_status.checks.accounts_status.invalid_accounts
                        .length,
                    0,
                );
            },
        ...
        mocha.it(
            'Config root path without bucket and account folders',
            async function () {
                Health.get_service_state.restore();
                Health.get_endpoint_response.restore();
                Health.all_account_details = true;
        ...
        mocha.it('Account with inaccessible path - uid gid', async function () {
            ...
            assert.strictEqual(
                health_status.checks.accounts_status.invalid_accounts[0].code,
                'ACCESS_DENIED',
            );
            assert.strictEqual(
                health_status.checks.accounts_status.invalid_accounts[0].name,
                account_inaccessible_options.name,
            );
            await exec_manage_cli(TYPES.ACCOUNT, ACTIONS.DELETE, {
                config_root,
                name: account_inaccessible_options.name,
            });
        });
        ...
        mocha.it('Account with inaccessible path - dn', async function () {
            await exec_manage_cli(TYPES.ACCOUNT, ACTIONS.ADD, {
                config_root,
                ...account_inaccessible_dn_options,
            });
            Health.get_service_state.restore();
            Health.get_endpoint_response.restore();
            Health.all_account_details = true;
            ...
            assert.strictEqual(
                health_status.checks.accounts_status.invalid_accounts[0].name,
                account_inaccessible_dn_options.name,
            );
            await exec_manage_cli(TYPES.ACCOUNT, ACTIONS.DELETE, {
                config_root,
                name: account_inaccessible_dn_options.name,
            });
        });
        ...
        mocha.it('Account with invalid dn', async function () {
            ...
            const found =
                health_status.checks.accounts_status.invalid_accounts.filter(
                    account => account.name === invalid_account_dn_options.name,
                );
        ...
        mocha.it(
            'Account with new_buckets_path missing and allow_bucket_creation false, valid account',
            async function () {
                const account_valid = {
                    name: 'account_valid',
                    uid: 999,
        ...
        mocha.it(
            'Account with new_buckets_path missing and allow_bucket_creation true, invalid account',
            async function () {
                const account_invalid = {
                    name: 'account_invalid',
                    uid: 999,
                    gid: 999,
