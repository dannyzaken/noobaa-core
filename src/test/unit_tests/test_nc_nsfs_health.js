...
mocha.describe('nsfs nc health', function () {
  ...
  mocha.before(async () => {
    await P.all(
      _.map([CONFIG_SUBDIRS.ACCOUNTS, CONFIG_SUBDIRS.BUCKETS], async dir =>
  ...
  mocha.describe('health check', async function () {
    ...
    const account_inaccessible_dn = {
      name: 'account_inaccessible_dn',
      nsfs_account_config: {
        distinguished_name: 'inaccessible_dn',
        new_buckets_path: bucket_storage_path,
      },
    };
    const invalid_account_dn = {
      name: 'invalid_account_dn',
      nsfs_account_config: {
        distinguished_name: 'invalid_account_dn',
        new_buckets_path: bucket_storage_path,
      },
    };
    const fs_users = {
      other_user: {
        distinguished_name:
          account_inaccessible_dn.nsfs_account_config.distinguished_name,
        uid: 1312,
        gid: 1312,
      },
    };
    ...
    mocha.after(async () => {
      ...
      fs_utils.file_delete(
        path.join(config_root, CONFIG_SUBDIRS.BUCKETS, bucket1.name + '.json'),
      );
      fs_utils.file_delete(
        path.join(
          config_root,
    ...
    mocha.it('Health all condition is success', async function () {
      ...
      get_service_state
        .onFirstCall()
        .returns(Promise.resolve({ service_status: 'active', pid: 100 }))
        .onSecondCall()
        .returns(Promise.resolve({ service_status: 'active', pid: 200 }));
      const get_endpoint_response = sinon.stub(Health, 'get_endpoint_response');
      get_endpoint_response.onFirstCall().returns(
        Promise.resolve({
          response: { response_code: 'RUNNING', total_fork_count: 0 },
        }),
      );
    ...
    mocha.it('NooBaa service is inactive', async function () {
      ...
      get_service_state
        .onFirstCall()
        .returns(Promise.resolve({ service_status: 'inactive', pid: 0 }))
        .onSecondCall()
        .returns(Promise.resolve({ service_status: 'active', pid: 200 }));
      const get_endpoint_response = sinon.stub(Health, 'get_endpoint_response');
      get_endpoint_response.onFirstCall().returns(
    ...
    mocha.it(
      ...
      async function () {
        ...
        get_service_state
          .onFirstCall()
          .returns(Promise.resolve({ service_status: 'active', pid: 1000 }))
          .onSecondCall()
    ...
    mocha.it('NSFS invalid bucket schema json', async function () {
      ...
      assert.strictEqual(
        health_status.checks.buckets_status.invalid_buckets[0].name,
        'bucket_invalid_schema.json',
      );
      await fs_utils.file_delete(
        path.join(
          config_root,
          CONFIG_SUBDIRS.BUCKETS,
          bucket_invalid_schema.name + '.json',
        ),
      );
    });
    ...
    mocha.it('NSFS invalid account schema json', async function () {
      ...
      assert.strictEqual(
        health_status.checks.accounts_status.invalid_accounts[0].name,
        'account_invalid_schema.json',
      );
      await fs_utils.file_delete(
        path.join(
          config_root,
          CONFIG_SUBDIRS.ACCOUNTS,
          account_invalid_schema.name + '.json',
        ),
      );
    });
    ...
    mocha.it('Account with inaccessible path - uid gid', async function () {
      ...
      await fs_utils.file_delete(
        path.join(
          config_root,
          CONFIG_SUBDIRS.ACCOUNTS,
          account_inaccessible.name + '.json',
        ),
      );
    });
    ...
    mocha.it('Account with inaccessible path - dn', async function () {
      ...
      await fs_utils.file_delete(
        path.join(
          config_root,
          CONFIG_SUBDIRS.ACCOUNTS,
          account_inaccessible_dn.name + '.json',
        ),
      );
    });
    ...
    mocha.it('Account with invalid dn', async function () {
      ...
      const found =
        health_status.checks.accounts_status.invalid_accounts.filter(
          account => account.name === invalid_account_dn.name,
        );
    ...
    mocha.it(
      ...
      async function () {
        const account_valid = {
          name: 'account_valid',
          nsfs_account_config: { uid: 999, gid: 999 },
