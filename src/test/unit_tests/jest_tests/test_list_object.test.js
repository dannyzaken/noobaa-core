    ...
    describe('Telldir and Seekdir implementation', () => {
        ...
        beforeAll(async () => {
            ...
            for (let i = 0; i < total_files; i++) {
                create_temp_file(list_dir_root, `test_${i}.json`, {
                    test: test,
                });
            }
        });
        ...
        it('telldir returns bigint', async () => {
            const dir_handle = await nb_native().fs.opendir(
                DEFAULT_FS_CONFIG,
                list_dir_root,
            );
        ...
        it('seekdir expects bigint', async () => {
            ...
            const tell_dir = await dir_handle.telldir(DEFAULT_FS_CONFIG);
            expect(() =>
                dir_handle.seekdir(DEFAULT_FS_CONFIG, Number(tell_dir)),
            ).toThrow();
            expect(() =>
                dir_handle.seekdir(DEFAULT_FS_CONFIG, 2n ** (32n ** 32n)),
            ).toThrow();
            expect(() =>
                dir_handle.seekdir(DEFAULT_FS_CONFIG, -(2n ** (32n ** 32n))),
            ).toThrow();
            // valid scenario
            expect(
                await dir_handle.seekdir(DEFAULT_FS_CONFIG, big_int),
            ).toBeUndefined();
        });
        ...
        it('list dir files - telldir and seekdir.', async () => {
            ...
            let dir_entry;
            let dir_handle = await nb_native().fs.opendir(
                DEFAULT_FS_CONFIG,
                list_dir_root,
            );
