...
} = require('../../../server/bg_services/log_replication_scanner.js');
...
describe('Azure blob log parsing tests', () => {
    ...
    it('Test Azure blob log parsing for Write and Delete actions', async () => {
            ...
            tables: [
                {
                    rows: [
                        ...
                        [
                            new Date(2),
                            'Delete',
                            'Final Revision - including fixes (2) - FINAL.txt',
                        ],
                        [
                            new Date(3),
                            'Write',
                            'noobaa_blocks/646bdc5e46ce2a0028749d7e/blocks_tree/other.blocks/_test_store_perf',
                        ],
        ...
        const action_dictionary = {
            'mt/folder1/nmt.txt': 'delete',
            'Final Revision - including fixes (2) - FINAL.txt': 'delete',
            'noobaa_blocks/646bdc5e46ce2a0028749d7e/blocks_tree/other.blocks/_test_store_perf':
                'copy',
        };
        ...
        const candidates = log_parser.create_candidates(logs);
        expect(Object.keys(candidates).length).toBe(
            Object.keys(action_dictionary).length,
        );
    });
});
...
describe('AWS S3 server logs parsing/processing tests', () => {
    ...
    it('Test AWS S3 server log parsing and processing', async () => {
        ...
        for (const key in candidates) {
            if (candidates[key][0]) {
                expect(candidates[key][0].action).toEqual(
        ...
        const { copy_keys, delete_keys } = await log_scanner.process_candidates(
            src_bucket,
            dst_bucket,
            candidates,
        );
