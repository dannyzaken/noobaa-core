...
const MASTER_KEYS_JSON_PATH = path.join(
...
describe('NC master key manager tests - file store type', () => {
    ...
    describe('NC master key manager tests - happy path', () => {
        it('init nc_mkm first time', async () => {
            ...
            compare_master_keys_json_and_active_root_key(
                active_master_key,
                master_keys,
            );
        ...
        it('init nc_mkm second time - master_keys.json exists', async () => {
            ...
            expect(JSON.stringify(initial_master_key)).toEqual(
                JSON.stringify(active_master_key),
            );
            expect(JSON.stringify(initial_master_keys_by_id)).toEqual(
                JSON.stringify(nc_mkm_instance.master_keys_by_id),
            );
        });
        ...
        it('encrypt & decrypt value using nc_mkm', async () => {
            ...
            const encrypted_secret_key =
                await nc_mkm_instance.encrypt(unwrapped_secret_key);
            const decrypted_secret_key = await nc_mkm_instance.decrypt(
                encrypted_secret_key,
                nc_mkm_instance.active_master_key.id,
            );
        ...
        it('encrypt_access_keys of account (with extra properties)', async () => {
            ...
            const account_after_encryption =
                await nc_mkm_instance.encrypt_access_keys(account);
            const account_string = JSON.stringify(account_after_encryption);
            nsfs_schema_utils.validate_account_schema(
            ...
            expect(account_after_encryption.creation_date).toBe(
                account.creation_date,
            );
            expect(account_after_encryption.allow_bucket_creation).toBe(
                account.allow_bucket_creation,
            );
            expect(account_after_encryption.master_key_id).toBeDefined();
            expect(account_after_encryption.nsfs_account_config.uid).toBe(
                account.nsfs_account_config.uid,
            );
            expect(account_after_encryption.nsfs_account_config.gid).toBe(
                account.nsfs_account_config.gid,
            );
            expect(account_after_encryption.access_keys.length).toBe(
                account.access_keys.length,
            );
            for (
                let i = 0;
                i < account_after_encryption.access_keys.length;
                i++
            ) {
                expect(account_after_encryption.access_keys[i].access_key).toBe(
                    account.access_keys[i].access_key,
                );
                expect(
                    account_after_encryption.access_keys[i]
                        .encrypted_secret_key,
                ).toBeDefined(); // instead of secret_key
                expect(
                    account_after_encryption.access_keys[i].creation_date,
                ).toBe(account.access_keys[i].creation_date);
                expect(
                    account_after_encryption.access_keys[i].deactivated,
                ).toBe(account.access_keys[i].deactivated);
            }
        });
        ...
        it('decrypt_access_keys of account (with extra properties)', async () => {
            ...
            const decrypted_access_keys =
                await nc_mkm_instance.decrypt_access_keys(
                    account_after_encryption,
                );
            for (let i = 0; i < decrypted_access_keys.length; i++) {
                expect(decrypted_access_keys[i].access_key).toBe(
                    account.access_keys[i].access_key,
                );
                expect(decrypted_access_keys[i].secret_key).toBeDefined(); // instead of encrypted_secret_key
                expect(decrypted_access_keys[i].creation_date).toBe(
                    account.access_keys[i].creation_date,
                );
                expect(decrypted_access_keys[i].deactivated).toBe(
                    account.access_keys[i].deactivated,
                );
            }
        });
    });
    ...
    it('should fail - init nc_mkm - invalid master_keys.json - invalid active_master_key value', async () => {
        ...
        await fs.promises.writeFile(
            MASTER_KEYS_JSON_PATH,
            JSON.stringify({ active_master_key: 'blabla' }),
        );
        const new_nc_mkm_instance = nc_mkm.get_instance();
        try {
            await new_nc_mkm_instance.init();
...
function validate_mkm_instance(nc_mkm_instance) {
    ...
    expect(nc_mkm_instance.active_master_key).toEqual(
 ...
 */
function compare_master_keys_json_and_active_root_key(
    active_master_key,
    master_keys,
) {
    const active_master_key_id = master_keys.active_master_key;
    const active_master_key_obj =
        master_keys.master_keys_by_id[active_master_key_id];
    expect(active_master_key_id.toString()).toEqual(
        active_master_key.id.toString(),
    );
    const buffered_cipher_iv = Buffer.from(
        active_master_key_obj.cipher_iv,
        'base64',
    );
    const buffered_cipher_key = Buffer.from(
        active_master_key_obj.cipher_key,
        'base64',
    );
