...
const {
    ManageCLIError,
} = require('../../../manage_nsfs/manage_nsfs_cli_errors');
const {
    ManageCLIResponse,
...
describe('noobaa cli - upgrade', () => {
    afterEach(
        async () => await fs_utils.file_delete(config_fs.system_json_path),
    );
    ...
    it('upgrade invalid action - should fail', async () => {
        ...
        expect(parsed_res.error.code).toBe(
            ManageCLIError.InvalidUpgradeAction.code,
        );
        expect(parsed_res.error.message).toBe(
            ManageCLIError.InvalidUpgradeAction.message,
        );
    });
    ...
    it('upgrade start - should fail with not implemented', async () => {
        ...
        expect(parsed_res.error.message).toBe(
            ManageCLIError.UpgradeFailed.message,
        );
    });
    ...
    it('upgrade status - should fail - there is no ongoing upgrade', async () => {
        const res = await exec_manage_cli(
            TYPES.UPGRADE,
            UPGRADE_ACTIONS.STATUS,
            { config_root },
            true,
        );
        const parsed_res = JSON.parse(res);
        expect(parsed_res.response.code).toBe(
            ManageCLIResponse.UpgradeStatus.code,
        );
        expect(parsed_res.response.reply.message).toEqual(
    ...
    it('upgrade status', async () => {
        await fs_utils.replace_file(
            config_fs.system_json_path,
            JSON.stringify(expected_system_json),
        );
        const res = await exec_manage_cli(
            TYPES.UPGRADE,
            UPGRADE_ACTIONS.STATUS,
        ...
        expect(parsed_res.response.reply).toEqual(
            expected_system_json.config_directory.in_progress_upgrade,
        );
    });
    ...
    it('upgrade history - should fail - there is no config directory upgrade history', async () => {
        const res = await exec_manage_cli(
            TYPES.UPGRADE,
            UPGRADE_ACTIONS.HISTORY,
            { config_root },
            true,
        );
        const parsed_res = JSON.parse(res);
        expect(parsed_res.response.code).toBe(
            ManageCLIResponse.UpgradeHistory.code,
        );
        expect(parsed_res.response.reply.message).toEqual(
            'Config directory upgrade history is empty',
        );
    });
    ...
    it('upgrade history', async () => {
        ...
        expect(parsed_res.response.reply).toEqual(
            expected_system_json.config_directory.upgrade_history,
        );
    });
});
