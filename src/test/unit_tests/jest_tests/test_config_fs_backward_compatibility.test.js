...
const tmp_fs_path = path.join(
    TMP_PATH,
    'test_config_fs_backward_compatibility',
);
...
describe('ConfigFS Backwards Compatibility', () => {
    ...
    it('get_account_by_name() - {config_dir}/accounts_by_name/{account_name} is missing', async () => {
        ...
        const res = await config_fs.get_account_by_name(
            old_account_data.name,
            silent_options,
        );
    ...
    it('get_account_by_name() - {config_dir}/accounts_by_name/{account_name} exist', async () => {
        ...
        await write_manual_config_file(
    ...
    it('is_account_exists() - {config_dir}/accounts_by_name/{account_name} is missing', async () => {
        const old_account_data = accounts['1'].old;
        await write_manual_old_account_config_file(config_fs, old_account_data);
        const res = await config_fs.is_account_exists_by_name(
            old_account_data.name,
            undefined,
        );
    ...
    it('list_accounts() - only old exists', async () => {
        await write_manual_old_account_config_file(
            config_fs,
            accounts['1'].old,
        );
        await write_manual_old_account_config_file(
...
async function clean_accounts() {
    for (const account of Object.values(accounts)) {
        if (account.old) {
            const old_account_path = config_fs._get_old_account_path_by_name(
                account.old.name,
            );
            await fs_utils.file_delete(old_account_path);
        }
        if (account.new) {
            const new_account_path = config_fs.get_account_or_user_path_by_name(
                account.old.name,
            );
            const new_account_id_path = config_fs.get_identity_path_by_id(
                account.old._id,
            );
            const new_account_id_dir_path =
                config_fs.get_identity_dir_path_by_id(account.old._id);
