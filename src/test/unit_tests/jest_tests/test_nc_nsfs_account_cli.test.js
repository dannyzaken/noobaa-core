...
describe('manage nsfs cli account flow', () => {
  describe('cli create account', () => {
    ...
    beforeEach(async () => {
      await P.all(
        _.map(
          [CONFIG_SUBDIRS.ACCOUNTS, CONFIG_SUBDIRS.ACCESS_KEYS],
    ...
    it('cli create account with access_keys', async () => {
      const action = ACTIONS.ADD;
      const { type, access_key, secret_key, name, new_buckets_path, uid, gid } =
        defaults;
      const account_options = {
        config_root,
        access_key,
    ...
    it('cli create account - check for the symlink relative path, not absolute path', async () => {
      ...
      expect(real_path).toContain(
        '../accounts/' + account_options.name + '.json',
      );
    });
  });
  ...
  describe('cli update account', () => {
    ...
    it('cli regenerate account access_keys', async () => {
      ...
      const account_details = await read_config_file(
        config_root,
        CONFIG_SUBDIRS.ACCOUNTS,
        name,
      );
      const action = ACTIONS.UPDATE;
      await exec_manage_cli(type, action, account_options);
      let new_account_details = await read_config_file(
        config_root,
        CONFIG_SUBDIRS.ACCOUNTS,
        name,
      );
    ...
    it('cli regenerate account access_keys with value "false"', async () => {
      ...
      const action = ACTIONS.UPDATE;
      await exec_manage_cli(type, action, account_options);
      const new_account_details = await read_config_file(
        config_root,
        CONFIG_SUBDIRS.ACCOUNTS,
        name,
      );
    ...
    it('cli account update name undefined (and back to original name)', async function () {
      ...
      await exec_manage_cli(type, action, account_options);
      new_account_details = await read_config_file(
        config_root,
        CONFIG_SUBDIRS.ACCOUNTS,
        new_name,
      );
  ...
  describe('cli list account', () => {
    ...
    it('cli list', async () => {
      ...
      const res = await exec_manage_cli(type, action, account_options);
      expect(JSON.parse(res).response.reply.map(item => item.name)).toEqual(
    ...
    it('cli list filter by UID', async () => {
      ...
      const res = await exec_manage_cli(type, action, account_options);
      expect(JSON.parse(res).response.reply.map(item => item.name)).toEqual(
        expect.arrayContaining(['account3', 'account1']),
      );
    });
    ...
    it('cli list filter by GID', async () => {
      ...
      const res = await exec_manage_cli(type, action, account_options);
      expect(JSON.parse(res).response.reply.map(item => item.name)).toEqual(
        expect.arrayContaining(['account1']),
      );
    });
    ...
    it('cli list filter by UID and GID (account 3)', async () => {
      ...
      const res = await exec_manage_cli(type, action, account_options);
      expect(JSON.parse(res).response.reply.map(item => item.name)).toEqual(
        expect.arrayContaining(['account3']),
      );
    });
    ...
    it('cli list filter by user (account 4)', async () => {
      ...
      expect(JSON.parse(res).response.reply.map(item => item.name)).toEqual(
        expect.arrayContaining(['account4']),
      );
    });
  ...
  describe('cli delete account', () => {
    ...
    it('cli delete account (account that does not own any bucket)', async () => {
      ...
      const symlink_config_path = path.join(
        config_root,
        CONFIG_SUBDIRS.ACCESS_KEYS,
        defaults.access_key + '.symlink',
      );
  ...
  describe('cli create account using from_file', () => {
    ...
    it('cli create account using from_file with required options (uid, gid)', async () => {
      ...
      const path_to_option_json_file_name = await create_json_account_options(
