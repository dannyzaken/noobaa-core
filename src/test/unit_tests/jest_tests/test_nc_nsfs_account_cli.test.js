...
describe('manage nsfs cli account flow', () => {
    describe('cli create account', () => {
        ...
        it('cli create account without access_keys', async () => {
            ...
            const account_symlink = await config_fs.get_account_by_access_key(
                access_key,
                config_fs_account_options,
            );
        ...
        it('cli create account with access_key and without secret_key', async () => {
            const action = ACTIONS.ADD;
            const { type, access_key, name, new_buckets_path, uid, gid } =
                defaults;
            const account_options = {
                config_root,
                access_key,
            ...
            expect(JSON.parse(res.stdout).error.message).toBe(
                ManageCLIError.MissingAccountSecretKeyFlag.message,
            );
        });

        it('cli create account without access_key and with secret_key', async () => {
            const action = ACTIONS.ADD;
            const { type, secret_key, name, new_buckets_path, uid, gid } =
                defaults;
            const account_options = {
                config_root,
                secret_key,
            ...
            const res = await exec_manage_cli(type, action, account_options);
            expect(JSON.parse(res.stdout).error.message).toBe(
                ManageCLIError.MissingAccountAccessKeyFlag.message,
            );
        });
        ...
        it('should fail - cli update account invalid access_key - invalid size', async () => {
            ...
            const res = await exec_manage_cli(type, action, account_options);
            expect(JSON.parse(res.stdout).error.message).toBe(
                ManageCLIError.InvalidAccountAccessKeyFlag.message,
            );
        });
        ...
        it('should fail - cli update account invalid access_key - contains "+" ', async () => {
            ...
            const account_options = {
                config_root,
                access_key: 'abc+abc+abc+abc+abc+',
                secret_key,
                name,
                new_buckets_path,
                uid,
        ...
        it('should fail - cli update account invalid secret_key - invalid size', async () => {
            ...
            const res = await exec_manage_cli(type, action, account_options);
            expect(JSON.parse(res.stdout).error.message).toBe(
                ManageCLIError.InvalidAccountSecretKeyFlag.message,
            );
        });
        ...
        it('should fail - cli update account invalid secret_key - contains @', async () => {
            ...
            const account_options = {
                config_root,
                access_key,
                secret_key: 'abcaabcabcabc@abcabcabc@abcabcabc@abcabc',
                name,
                new_buckets_path,
                uid,
                gid,
        ...
        it('should fail - cli create account invalid option type (new_buckets_path as number)', async () => {
            ...
            const account_options = {
                config_root,
                name,
                new_buckets_path: new_buckets_path_invalid,
                uid,
                gid,
            };
            const res = await exec_manage_cli(type, action, account_options);
        ...
        it('should fail - cli create account invalid option type (new_buckets_path as string)', async () => {
            ...
            const res = await exec_manage_cli(type, action, account_options);
            expect(JSON.parse(res.stdout).error.message).toBe(
                ManageCLIError.InvalidAccountNewBucketsPath.message,
            );
        });
        ...
        it('cli account add - name is a number', async function () {
            ...
            const res = await exec_manage_cli(TYPES.ACCOUNT, ACTIONS.ADD, {
                config_root,
                ...options,
            });
            const account = JSON.parse(res).response.reply;
            assert_account(account, options, false);
            await exec_manage_cli(TYPES.ACCOUNT, ACTIONS.DELETE, {
                config_root,
                name: account_name,
            });
        });
        ...
        it('should fail - cli account add - use explicit flag allow_bucket_creation invalid string value', async function () {
            ...
            expect(JSON.parse(res.stdout).error.code).toBe(
                ManageCLIError.InvalidBooleanValue.code,
            );
        });
        ...
        it('cli create account - check for the symlink relative path, not absolute path', async () => {
            ...
            assert_account(account_symlink, account_options);
            const real_path = fs.readlinkSync(
                config_fs.get_account_or_user_path_by_access_key(access_key),
            );
            expect(real_path).toContain(
                config_fs.get_account_relative_path_by_id(account._id),
            );
        });
        ...
        it(
            'cli account add - use flag iam_operate_on_root_account (true) ' +
                'with allow_bucket_creation (true)',
            async function () {
                const action = ACTIONS.ADD;
                const { type, name, new_buckets_path, uid, gid } = defaults;
                const iam_operate_on_root_account = 'true';
                ...
                expect(JSON.parse(res).response.code).toEqual(
                    ManageCLIResponse.AccountCreated.code,
                );
            },
        );
        ...
        it('should fail - cli account add invalid flags combination (gid and user)', async function () {
            ...
            const res = await exec_manage_cli(type, action, account_options);
            expect(JSON.parse(res.stdout).error.code).toBe(
                ManageCLIError.InvalidFlagsCombination.code,
            );
        });
        ...
        it('should fail - cli account add invalid account name(anonymous)', async function () {
            ...
            const res = await exec_manage_cli(type, action, account_options);
            expect(JSON.parse(res.stdout).error.code).toBe(
                ManageCLIError.InvalidAccountName.code,
            );
        });
        ...
        it('should fail - cli account add - without identifier', async function () {
            ...
            const res = await exec_manage_cli(type, action, account_options);
            expect(JSON.parse(res.stdout).error.code).toBe(
                ManageCLIError.MissingAccountNameFlag.code,
            );
        });
        ...
        it('should fail - cli create account invalid option (new_name)', async () => {
            ...
            const account_options = {
                ...
                gid,
                new_name: 'lala',
            }; // new_name invalid option in add
            const action = ACTIONS.ADD;
            const res = await exec_manage_cli(type, action, account_options);
            expect(JSON.parse(res.stdout).error.code).toBe(
                ManageCLIError.InvalidArgument.code,
            );
        });
        ...
        it('should fail - cli create account invalid option (new_access_key)', async () => {
            ...
            const account_options = {
                ...
                uid,
                gid,
                new_access_key: 'GIGiFAnjaaE7OKD5N7lB',
            }; // new_access_key invalid option
            const action = ACTIONS.ADD;
            const res = await exec_manage_cli(type, action, account_options);
            expect(JSON.parse(res.stdout).error.code).toBe(
                ManageCLIError.InvalidArgument.code,
        ...
        it('should fail - cli create account invalid name (from internal list)', async () => {
            ...
            const account_options = {
                ...
                uid,
                gid,
            }; // invalid name (we don't allow '.' as name)
            const action = ACTIONS.ADD;
            const res = await exec_manage_cli(type, action, account_options);
            expect(JSON.parse(res.stdout).error.code).toBe(
                ManageCLIError.InvalidAccountName.code,
    ...
    describe('cli update account', () => {
        describe('cli update account (has uid and gid)', () => {
            ...
            it('cli regenerate account access_keys', async () => {
                ...
                const account_details = await config_fs.get_account_by_name(
                    name,
                    config_fs_account_options,
                );
                const action = ACTIONS.UPDATE;
                await exec_manage_cli(type, action, account_options);
                let new_account_details = await config_fs.get_account_by_name(
                    name,
                    config_fs_account_options,
                );
                expect(account_details.access_keys[0].access_key).not.toBe(
                    new_account_details.access_keys[0].access_key,
                );
                const new_access_key =
                    new_account_details.access_keys[0].access_key;
                const account_symlink =
                    await config_fs.get_account_by_access_key(
                        new_access_key,
                        config_fs_account_options,
                    );
                ...
                new_account_details = {
                    ...new_account_details,
                    ...new_account_details.nsfs_account_config,
                };
            ...
            it('cli regenerate account access_keys with value "false"', async () => {
                ...
                const action = ACTIONS.UPDATE;
                await exec_manage_cli(type, action, account_options);
                const new_account_details = await config_fs.get_account_by_name(
                    name,
                    config_fs_account_options,
                );
                expect(account_details.access_keys[0].access_key).toBe(
                    new_account_details.access_keys[0].access_key,
                );
            });
            ...
            it('cli account update name undefined (and back to original name)', async function () {
                ...
                await exec_manage_cli(type, action, account_options);
                new_account_details = await config_fs.get_account_by_name(
                    new_name,
                    config_fs_account_options,
                );
                expect(new_account_details.name).toBe(new_name);
            ...
            it('cli account update access key, secret_key & new_name by name', async function () {
                ...
                expect(account_details.access_keys[0].secret_key).not.toBe(
                    new_account_details.access_keys[0].secret_key,
                );
                expect(new_account_details.access_keys[0].access_key).toBe(
                    access_key,
                );
                const account_symlink =
                    await config_fs.get_account_by_access_key(
            ...
            it('should fail - cli update account invalid option type (user as boolean)', async () => {
                ...
                const command = create_command(type, action, account_options);
                const flag = 'user'; // we will add user flag without value
                const res = await exec_manage_cli_add_empty_option(
                    command,
                    flag,
                );
                expect(JSON.parse(res.stdout).error.message).toBe(
            ...
            it('cli update account unset new_buckets_path', async () => {
                ...
                expect(
                    new_account_details.nsfs_account_config.new_buckets_path,
                ).toBeUndefined();
                ...
                expect(
                    new_account_details.nsfs_account_config.new_buckets_path,
                ).toBe(defaults.new_buckets_path);
            ...
            it('cli account update account by name, access_key and secret_key - check for the symlink relative path, not absolute path', async function () {
                ...
                assert_account(account_symlink, new_account_details);
                const real_path = fs.readlinkSync(
                    config_fs.get_account_or_user_path_by_access_key(
                        access_key,
                    ),
                );
                expect(real_path).toContain(
                    config_fs.get_account_relative_path_by_id(
                        new_account_details._id,
                    ),
                );
            });
            ...
            it('cli update account set flag iam_operate_on_root_account', async function () {
                ...
                expect(new_account_details.iam_operate_on_root_account).toBe(
                    true,
                );
            ...
            it('should fail - cli update account iam_operate_on_root_account true when account owns IAM accounts', async function () {
                ...
                const config_data = await config_fs.get_account_by_name(
                    account_name,
                    config_fs_account_options,
                );
            ...
            it('should fail - cli update account iam_operate_on_root_account true when requester is IAM user', async function () {
                ...
                expect(JSON.parse(res.stdout).error.code).toBe(
                    ManageCLIError
                        .AccountCannotCreateRootAccountsRequesterIAMUser.code,
                );
            });
            ...
            it('should fail - cli update account without a property to update', async () => {
                ...
                expect(JSON.parse(res.stdout).error.message).toBe(
                    ManageCLIError.MissingUpdateProperty.message,
                );
            });
            ...
            it('cli update account that has uid and gid with distinguished name', async () => {
                ...
                expect(
                    account_details.nsfs_account_config.distinguished_name,
                ).toBe(distinguished_name);
            });
            ...
            it('should fail - cli update account with invalid new_name (anonymous)', async () => {
                ...
                expect(JSON.parse(res.stdout).error.message).toBe(
                    ManageCLIError.InvalidAccountName.message,
                );
            });
            ...
            it('cli update iam account with regenerate (only object access keys in index 0 changes)', async function () {
                ...
                config_data.access_keys[0].creation_date =
                    '2024-07-10T11:26:34.569Z';
                ...
                expect(
                    JSON.parse(res).response.reply.access_keys[0].creation_date,
                ).toBeUndefined();
                expect(
                    JSON.parse(res).response.reply.access_keys[0].deactivated,
                ).toBeUndefined();
            });
            ...
            it('cli update iam account with access_key and secret_key flag (only object access keys in index 0 changes)', async function () {
                ...
                expect(
                    JSON.parse(res).response.reply.access_keys[0].access_key,
                ).toBe(access_key);
                expect(
                    JSON.parse(res).response.reply.access_keys[0].secret_key,
                ).toBe(secret_key);
            });
            ...
            it('should fail - cli update account invalid new_name (more than max length)', async () => {
                ...
                const account_options = {
                    config_root,
                    name,
                    new_name: 'A'.repeat(100),
                }; // invalid new_name (too long)
                const res = await exec_manage_cli(
                    type,
                    ACTIONS.UPDATE,
                    account_options,
    ...
    describe('cli list account', () => {
        ...
        beforeAll(async () => {
            ...
            for (const account_defaults of Object.values(defaults)) {
                ...
                await fs_utils.create_fresh_path(
                    account_options.new_buckets_path,
                );
        ...
        it('cli list', async () => {
            ...
            expect(
                JSON.parse(res).response.reply.map(item => item.name),
            ).toEqual(
                expect.arrayContaining(['account3', 'account2', 'account1']),
            );
        });
        ...
        it('cli list wide', async () => {
            ...
            expect(
                JSON.parse(res).response.reply.map(
                    item => item.nsfs_account_config.uid,
                ),
            ).toEqual(expect.arrayContaining([999, 888]));
            expect(
                JSON.parse(res).response.reply.map(
                    item => item.nsfs_account_config.new_buckets_path,
                ),
            ).toEqual(
                expect.arrayContaining([
                    `${root_path}new_buckets_path_user1/`,
                    `${root_path}new_buckets_path_user2/`,
                    `${root_path}new_buckets_path_user3/`,
                ]),
            );
        });
        ...
        it('should fail - cli account list invalid flags combination (show_secrets without wide)', async () => {
            ...
            const res = await exec_manage_cli(type, action, account_options);
            expect(JSON.parse(res.stdout).error.message).toBe(
                ManageCLIError.InvalidFlagsCombination.message,
            );
        });
        ...
        it('cli list wide and show_secrets', async () => {
            ...
            for (const item of res_arr) {
                ...
                expect(
                    item.access_keys[0].encrypted_secret_key,
                ).toBeUndefined();
            }
        });
    });
    ...
    describe('cli delete account', () => {
        ...
        beforeEach(async () => {
            ...
            await exec_manage_cli(type, action, account_options);
            const is_account_exists =
                await config_fs.is_account_exists_by_name(name);
        ...
        it('cli delete account (account that does not own any bucket)', async () => {
            ...
            const account_by_name_exists =
                await config_fs.is_account_exists_by_name(name);
            expect(account_by_name_exists).toBe(false);
            const account_by_access_key_exists =
                await config_fs.is_account_exists_by_access_key(
                    defaults.access_key,
                );
        ...
        it('cli delete account (account has 2 access keys objects)', async () => {
            ...
            const access_key1_exists =
                await config_fs.is_account_exists_by_access_key(
                    defaults.access_key,
                );
            expect(access_key1_exists).toBe(false);
            const access_key2_exists =
                await config_fs.is_account_exists_by_access_key(access_key2);
        ...
        it('should fail - cli delete account, account owns a bucket', async () => {
            ...
            const is_bucket_exists =
                await config_fs.is_bucket_exists(bucket_name);
            ...
            const res = await exec_manage_cli(type, action, account_options);
            expect(JSON.parse(res.stdout).error.code).toBe(
                ManageCLIError.AccountDeleteForbiddenHasBuckets.code,
            );
            const is_account_exists =
                await config_fs.is_account_exists_by_name(name);
            expect(is_account_exists).toBe(true);
        ...
        it('should fail - cli account delete - root account has IAM accounts', async function () {
            ...
            expect(JSON.parse(res.stdout).error.code).toBe(
                ManageCLIError.AccountDeleteForbiddenHasIAMAccounts.code,
            );
        });
    });
    ...
    describe('cli status account', () => {
        ...
        it('cli account status without name and access_key', async function () {
            ...
            expect(JSON.parse(res.stdout).error.code).toBe(
                ManageCLIError.MissingIdentifier.code,
            );
        });
    ...
    describe('cli create account using from_file', () => {
        ...
        const path_to_json_account_options_dir = path.join(
            tmp_fs_path,
            'options',
        );
        const defaults = {
        ...
        it('cli create account using from_file with required options (uid, gid)', async () => {
            ...
            const path_to_option_json_file_name =
                await create_json_account_options(
                    path_to_json_account_options_dir,
                    account_options,
                );
            const command_flags = {
                config_root,
                from_file: path_to_option_json_file_name,
            };
        ...
        it('cli create account using from_file with access_key and secret_key', async () => {
            const action = ACTIONS.ADD;
            const { name, new_buckets_path, uid, gid, access_key, secret_key } =
                defaults;
            const account_options = {
                name,
                new_buckets_path,
        ...
        it('should fail - cli create account using from_file with invalid access_key and secret_key', async () => {
            ...
            expect(JSON.parse(res.stdout).error.code).toBe(
                ManageCLIError.InvalidAccountAccessKeyFlag.code,
            );
        });
        ...
        it('should fail - cli create account using from_file with invalid option (creation_date) in the file', async () => {
            ...
            const account_options = {
                ...
                gid,
                creation_date: new Date().toISOString(),
            }; // creation_date invalid option (user cannot set it)
            // write the json_file_options
            const path_to_option_json_file_name =
                await create_json_account_options(
                    path_to_json_account_options_dir,
        ...
        it('should fail - cli create account using from_file with from_file inside the JSON file', async () => {
            ...
            const account_options = {
                ...
                gid,
                from_file: 'blabla',
            }; //from_file inside options JSON file
            // write the json_file_options
            const path_to_option_json_file_name =
                await create_json_account_options(
                    path_to_json_account_options_dir,
        ...
        it('should fail - cli create account using from_file with invalid option type (in the file)', async () => {
            ...
            const account_options = {
                ...
                uid,
                gid: 'lala',
            }; // gid should be number (not string)
            // write the json_file_options
            const path_to_option_json_file_name =
                await create_json_account_options(
                    path_to_json_account_options_dir,
        ...
        it('should fail - cli create account using from_file with invalid path', async () => {
            ...
            const res = await exec_manage_cli(type, action, command_flags);
            expect(JSON.parse(res.stdout).error.code).toBe(
                ManageCLIError.InvalidFilePath.code,
            );
        });
        ...
        it('should fail - cli create account using from_file with invalid JSON file', async () => {
            ...
            const path_to_option_json_file_name = path.join(
                path_to_json_account_options_dir,
                option_json_file_name,
            );
            ...
            const res = await exec_manage_cli(type, action, command_flags);
            expect(JSON.parse(res.stdout).error.code).toBe(
                ManageCLIError.InvalidJSONFile.code,
            );
        });
...
describe('cli account flow distinguished_name - permissions', function () {
    ...
    const new_buckets_path = path.join(
        tmp_fs_path,
        'new_buckets_path_user_dn_test/',
    );
    const accounts = {
        root: {
            cli_options: {
    ...
    beforeAll(async () => {
        ...
        const res = await exec_manage_cli(
            type,
            ACTIONS.ADD,
            accounts.root.cli_options,
        );
        assert_account(
            JSON.parse(res).response.reply,
            accounts.root.cli_options,
            false,
        );
    }, timeout);
    ...
    afterAll(async () => {
        await exec_manage_cli(type, ACTIONS.DELETE, {
            name: accounts.accessible_user.cli_options.name,
            config_root,
        });
        await exec_manage_cli(type, ACTIONS.DELETE, {
    ...
    it(
        'cli account create - should fail - user does not exist',
        async function () {
            const action = ACTIONS.ADD;
            const res = await exec_manage_cli(
                type,
                action,
                accounts.non_existing_dn.cli_options,
            );
            expect(JSON.parse(res.stdout).error.code).toBe(
                ManageCLIError.InvalidAccountDistinguishedName.code,
            );
        },
        timeout,
    );
    ...
    it(
        'cli account update distinguished_name - should fail - user does not exist',
        async function () {
            const action = ACTIONS.UPDATE;
    ...
    it(
        'cli account create - should fail - account cant access new_bucket_path',
        async function () {
            const action = ACTIONS.ADD;
            const res = await exec_manage_cli(
                type,
                action,
                accounts.inaccessible_user.cli_options,
            );
            expect(JSON.parse(res.stdout).error.code).toBe(
                ManageCLIError.InaccessibleAccountNewBucketsPath.code,
            );
        },
        timeout,
    );
    ...
    it(
        'cli account update distinguished_name - should fail - not owner - account cant access new_bucket_path',
        async function () {
            const action = ACTIONS.UPDATE;
            const update_options = {
                config_root,
    ...
    it(
        'cli account create - valid and accessible distinguished name',
        async function () {
            await fs.promises.chown(
                new_buckets_path,
                accounts.accessible_user.fs_options.uid,
                accounts.accessible_user.fs_options.gid,
            );
            const action = ACTIONS.ADD;
            const res = await exec_manage_cli(
                type,
    ...
    it(
        'cli account update root - other valid and accessible distinguished_name',
        async function () {
            const action = ACTIONS.UPDATE;
            const update_options = {
                config_root,
            ...
            assert_account(
                JSON.parse(res).response.reply,
                { ...accounts.root.cli_options, ...update_options },
                false,
            );
        },
        timeout,
    ...
    it(
        'cli account update - should fail - no permissions to new_buckets_path',
        async function () {
            ...
            await set_path_permissions_and_owner(
                new_buckets_path,
                accounts.accessible_user.fs_options,
                0o077,
            );
            const action = ACTIONS.UPDATE;
            const update_options = {
    ...
    it(
        'cli account update - should fail - no write permissions of new_buckets_path',
        async function () {
    ...
    it(
        'cli account create - account cant access new_bucket_path - NC_DISABLE_ACCESS_CHECK = true',
        async function () {
            let action = ACTIONS.ADD;
            ...
            await fs.promises.writeFile(
                ...
                JSON.stringify({ NC_DISABLE_ACCESS_CHECK: true }),
            );
            const res = await exec_manage_cli(
                type,
                action,
            ...
            action = ACTIONS.DELETE;
            const delete_inaccessible_options = _.omit(
                accounts.inaccessible_user.cli_options,
                ['new_buckets_path', 'user'],
            );
...
function assert_account(account, account_options, verify_access_keys) {
    if (verify_access_keys) {
        expect(account.access_keys[0].access_key).toEqual(
            account_options.access_key,
        );
        expect(account.access_keys[0].secret_key).toEqual(
            account_options.secret_key,
        );
    }
    ...
    expect(account.nsfs_account_config.new_buckets_path).toEqual(
        account_options.new_buckets_path,
    );
}
 ...
 * @param {object} account_options
 */
async function create_json_account_options(
    path_to_json_account_options_dir,
    account_options,
) {
    const option_json_file_name = `${account_options.name}_options.json`;
