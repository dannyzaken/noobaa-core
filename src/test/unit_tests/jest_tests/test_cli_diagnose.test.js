    ...
    describe('gather-logs flow', async => {
        it('gather-logs - should fail with not implemented', async () => {
            const res = await exec_manage_cli(
                TYPES.DIAGNOSE,
                DIAGNOSE_ACTIONS.GATHER_LOGS,
                {},
                true,
            );
            expect(JSON.parse(res.stdout).error.message).toBe(
                ManageCLIError.NotImplemented.message,
            );
        });
    });
    ...
    describe('metrics flow', () => {
        it('diagnose metrics - should fail', async () => {
            ...
            try {
                ...
                const res = await exec_manage_cli(
                    TYPES.DIAGNOSE,
                    DIAGNOSE_ACTIONS.METRICS,
                    {},
                    true,
                );
                const parsed_res = JSON.parse(res);
                expect(parsed_res.response.code).toBe(
                    ManageCLIResponse.MetricsStatus.code,
                );
                expect(parsed_res.response.reply).toMatchObject(
                    metrics_obj_mock,
                );
            } finally {
        ...
        it('diagnose metrics - should fail, metrics server is down', async () => {
            ...
            expect(parsed_res.error.code).toBe(
                ManageCLIError.MetricsStatusFailed.code,
            );
            expect(parsed_res.error.message).toBe(
                ManageCLIError.MetricsStatusFailed.message,
            );
        });
    });
});
