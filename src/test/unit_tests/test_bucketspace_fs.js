...
const new_buckets_path_user1 = path.join(
    tmp_fs_path,
    'new_buckets_path_user1',
    '/',
);
const new_buckets_path_user2 = path.join(
    tmp_fs_path,
    'new_buckets_path_user2',
    '/',
);
const new_buckets_path_user4 = path.join(
    tmp_fs_path,
    'new_buckets_path_user4',
    '/',
);
...
function make_dummy_object_sdk() {
    return {
        ...
        read_bucket_sdk_namespace_info(name) {
            dummy_ns.write_resource.path = path.join(
                new_buckets_path,
                name.toString(),
            );
        ...
        async read_bucket_sdk_policy_info(name) {
            const bucket_info = await bucketspace_fs.read_bucket_sdk_info({
                name,
            });
...
const account_iam_user1 = {
    ...
    allow_bucket_creation:
        dummy_object_sdk.requesting_account.allow_bucket_creation,
    access_keys: [
        {
...
mocha.describe('bucketspace_fs', function () {
    ...
    mocha.before(async () => {
        ...
        for (let account of [
            ...
            const account_path =
                bucketspace_fs.config_fs.get_identity_path_by_id(account._id);
            const account_name_path =
                bucketspace_fs.config_fs.get_account_path_by_name(account.name);
            const account_access_path =
                bucketspace_fs.config_fs.get_account_or_user_path_by_access_key(
    ...
    mocha.describe('read_account_by_access_key', function () {
        mocha.it('read account by valid access key', async function () {
            const access_key =
                account_user1.access_keys[0].access_key.toString();
            const res = await bucketspace_fs.read_account_by_access_key({
                access_key,
            });
            assert.strictEqual(res.email.unwrap(), account_user1.email);
            assert.strictEqual(
                res.access_keys[0].access_key.unwrap(),
                account_user1.access_keys[0].access_key,
            );
        });

        mocha.it(
            'check uid/gid from distinguished name (root)',
            async function () {
                const access_key =
                    account_user2.access_keys[0].access_key.toString();
                const res = await bucketspace_fs.read_account_by_access_key({
                    access_key,
                });
                ...
                assert.strictEqual(
                    res.access_keys[0].access_key.unwrap(),
                    account_user2.access_keys[0].access_key,
                );
                const distinguished_name =
                    res.nsfs_account_config.distinguished_name.unwrap();
                assert.strictEqual(distinguished_name, 'root');
                const res2 = await get_user_by_distinguished_name({
                    distinguished_name,
                });
        ...
        mocha.it(
            'check uid/gid from distinguished name (none root)',
            async function () {
                const access_key =
                    account_user3.access_keys[0].access_key.toString();
                const res = await bucketspace_fs.read_account_by_access_key({
                    access_key,
                });
                ...
                assert.strictEqual(
                    res.access_keys[0].access_key.unwrap(),
                    account_user3.access_keys[0].access_key,
                );
                const distinguished_name =
                    res.nsfs_account_config.distinguished_name.unwrap();
    ...
    mocha.describe('create_bucket', function () {
        mocha.it(
            'create bucket and validate bucket folder and schema config',
            async function () {
        ...
        mocha.it(
            'validate bucket access with default context',
            async function () {
                try {
                    const param = { name: test_bucket };
                    const invalid_objects = await nb_native().fs.readdir(
                        DEFAULT_FS_CONFIG,
                        path.join(new_buckets_path, param.name),
                    );
        ...
        mocha.it(
            'validate bucket access with account specific context',
            async function () {
                const param = { name: test_bucket };
                await await nb_native().fs.writeFile(
                    ACCOUNT_FS_CONFIG,
                    path.join(new_buckets_path, param.name, 'dummy_data.json'),
        ...
        mocha.it(
            'validate bucket access with user not allowed to create bucket',
            async function () {
                try {
                    ...
                    assert.fail(
                        'should have failed with UNAUTHORIZED bucket creation',
                    );
                } catch (err) {
        ...
        mocha.it(
            'should fail - create bucket by iam account',
            async function () {
                // currently we do not allow IAM accounts to create buckets
                try {
                    const param = { name: test_bucket_iam_account };
                    const dummy_object_sdk_for_iam_account =
                        make_dummy_object_sdk_for_account(
                            dummy_object_sdk,
                            account_iam_user1,
                        );
                    await bucketspace_fs.create_bucket(
                        param,
                        dummy_object_sdk_for_iam_account,
                    );
                    assert.fail(
                        'should have failed with UNAUTHORIZED bucket creation',
                    );
        ...
        mocha.after(async function () {
            ...
            await fs_utils.folder_delete(
                `${new_buckets_path}/${test_bucket_iam_account}`,
            );
    ...
    mocha.describe('read_bucket_sdk_info', function () {
        ...
        mocha.it(
            'read bucket that was created - check non existing deprecated properties',
            async function () {
                const param = { name: test_bucket };
    ...
    mocha.describe('list_buckets', async function () {
        ...
        mocha.it(
            'list buckets - only for bucket with config',
            async function () {
                await fs_utils.create_path(
                    `${new_buckets_path}/${test_bucket_invalid}`,
                );
                const objects =
                    await bucketspace_fs.list_buckets(dummy_object_sdk);
                assert.equal(objects.buckets.length, 1);
            },
        );
        mocha.it('list buckets - iam accounts', async function () {
            ...
            const res = await bucketspace_fs.list_buckets(
                dummy_object_sdk_for_iam_account,
            );
            ...
            // account_iam_user2 can list the created bucket (the implicit policy - same root account)
            const dummy_object_sdk_for_iam_account2 =
                make_dummy_object_sdk_for_account(
                    dummy_object_sdk,
                    account_iam_user2,
                );
            const res2 = await bucketspace_fs.list_buckets(
                dummy_object_sdk_for_iam_account2,
            );
        ...
        mocha.it(
            'list buckets - different account with bucket policy (principal by name)',
            async function () {
        ...
        mocha.it(
            'list buckets - different account with bucket policy (principal by id)',
            async function () {
                // another user created a bucket
                // with bucket policy account_user3 can list it
                const policy = generate_s3_policy(
        ...
        mocha.it('list buckets - validate creation_date', async function () {
            ...
            const bucket_data = await read_file(
                process_fs_context,
                bucket_config_path,
            );
            assert.equal(
                objects.buckets[0].creation_date,
                bucket_data.creation_date,
            );
        });
    });
    mocha.describe('delete_bucket', function () {
        mocha.before(async function () {
            ...
            await fs_utils.file_must_exist(
                path.join(new_buckets_path, test_bucket),
            );
        });
        ...
        mocha.it('delete_bucket for non empty buckets', async function () {
            ...
            const bucket_file_path = path.join(
                new_buckets_path,
                param.name,
                'dummy.txt',
            );
        ...
        mocha.it(
            'delete_bucket for should_create_underlying_storage false',
            async function () {
                ...
                const bucket_temp_dir_path = path.join(
                    new_buckets_path,
                    param.name,
                    get_bucket_tmpdir_name(bucket._id),
                );
                ...
                try {
                    ...
                    assert.strictEqual(err.code, 'ENOENT');
                    const path_for_err_msg = path.join(
                        TMP_PATH,
                        'test_bucketspace_fs/config_root/buckets/buckettempdir.json',
                    );
                    assert.equal(
                        err.message,
                        `ENOENT: no such file or directory, stat '${path_for_err_msg}'`,
                    );
                }
                await fs.promises.stat(path.join(new_buckets_path, param.name));
            },
        ...
        mocha.it(
            'delete buckets - iam accounts (another IAM account deletes the bucket)',
            async function () {
                ...
                const dummy_object_sdk_for_account_iam_user1 =
                    make_dummy_object_sdk_for_account(
                        dummy_object_sdk,
                        account_iam_user1,
                    );
                const res = await bucketspace_fs.list_buckets(
                    dummy_object_sdk_for_account_iam_user1,
                );
                assert.ok(res.buckets.length > 0);
                assert.ok(
                    res.buckets.some(
                        bucket =>
                            bucket.name.unwrap() === test_bucket_iam_account,
                    ),
                );

                const param = { name: test_bucket_iam_account };
                // account_iam_user2 can delete the created bucket (the implicit policy - same root account)
                const dummy_object_sdk_for_account_iam_user2 =
                    make_dummy_object_sdk_for_account(
                        dummy_object_sdk,
                        account_iam_user2,
                    );
                await bucketspace_fs.delete_bucket(
                    param,
                    dummy_object_sdk_for_account_iam_user2,
                );
            },
        );

        mocha.it(
            'delete_bucket after the ULS was deleted (should_create_underlying_storage true)',
            async function () {
        ...
        mocha.it(
            'delete_bucket after the ULS was deleted (should_create_underlying_storage false)',
            async function () {
                const param = { name: test_bucket2 };
                await create_bucket(param.name);
        ...
        mocha.it('delete_bucket with delete marker', async function () {
            ...
            await bucketspace_fs.set_bucket_versioning(
                { name: param.name, versioning: 'ENABLED' },
                versioning_sdk,
            );
            ...
            const versioned_path = path.join(
                version_dir,
                'dummy_mtime-crkfjum9883k-ino-guu7',
            );
            await create_versioned_object(
                versioned_path,
                Buffer.from(JSON.stringify('data')),
            ...
            const delete_marker_path = path.join(
                version_dir,
                'dummy_mtime-crkfjx1hui2o-ino-guu9',
            );
            const delete_marker_obj = await create_versioned_object(
                delete_marker_path,
                Buffer.from(JSON.stringify('data')),
            ...
            delete_marker_obj.replacexattr(
                DEFAULT_FS_CONFIG,
                xattr_delete_marker,
            );
    ...
    mocha.describe('bucket encryption operations', function () {
        mocha.it('put_bucket_encryption ', async function () {
            ...
            const output_encryption =
                await bucketspace_fs.get_bucket_encryption(param);
    ...
    mocha.describe('bucket policy operations', function () {
        mocha.it('put_bucket_policy ', async function () {
            ...
            const param = { name: test_bucket, policy: policy };
            await bucketspace_fs.put_bucket_policy(param);
            const bucket_policy_res = await bucketspace_fs.get_bucket_policy(
                param,
                dummy_object_sdk,
            );
        ...
        mocha.it('delete_bucket_policy ', async function () {
            ...
            const delete_res = await bucketspace_fs.get_bucket_policy(
                param,
                dummy_object_sdk,
            );
        ...
        mocha.it(
            'put_bucket_policy other account object (principal as name)',
            async function () {
                const policy = {
                    Version: '2012-10-17',
                    Statement: [
                ...
                const param = { name: test_bucket, policy: policy };
                await bucketspace_fs.put_bucket_policy(param);
                const bucket_policy = await bucketspace_fs.get_bucket_policy(
                    param,
                    dummy_object_sdk,
                );
                assert.deepEqual(bucket_policy.policy, policy);
                const info_res =
                    await bucketspace_fs.read_bucket_sdk_info(param);
                assert.deepEqual(info_res.s3_policy, policy);
            },
        );

        mocha.it(
            'put_bucket_policy other account object (principal as id)',
            async function () {
                const policy = {
                    Version: '2012-10-17',
                    Statement: [
                ...
                const param = { name: test_bucket, policy: policy };
                await bucketspace_fs.put_bucket_policy(param);
                const bucket_policy_res =
                    await bucketspace_fs.get_bucket_policy(
                        param,
                        dummy_object_sdk,
                    );
                assert_bucket_policies(bucket_policy_res.policy, policy);
        ...
        mocha.it(
            'put_bucket_policy other account object - account does not exist',
            async function () {
                const policy = {
                    Version: '2012-10-17',
                    Statement: [
                ...
                try {
                    await bucketspace_fs.put_bucket_policy(param);
                    assert.fail(
                        'should have failed with invalid principal in policy',
                    );
                } catch (err) {
    ...
    mocha.describe('bucket lifecycle operations', function () {
        mocha.it('set_bucket_lifecycle_configuration_rules', async function () {
            ...
            await bucketspace_fs.set_bucket_lifecycle_configuration_rules(
                param,
            );
            const output_lifecycle =
                await bucketspace_fs.get_bucket_lifecycle_configuration_rules(
                    param,
                );
...
async function create_versioned_object(
    object_path,
    data,
    version_id,
    return_fd,
