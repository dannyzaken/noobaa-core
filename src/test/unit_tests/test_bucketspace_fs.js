...
mocha.describe('bucketspace_fs', function () {
  ...
  mocha.before(async () => {
    await P.all(
      _.map(
        ...
        async dir => await fs_utils.create_fresh_path(`${config_root}/${dir}`),
      ),
    );
    await fs_utils.create_fresh_path(new_buckets_path);
    for (let account of [account_user1, account_user2, account_user3]) {
      account = await nc_mkm.encrypt_access_keys(account);
      const account_path = get_config_file_path(
        CONFIG_SUBDIRS.ACCOUNTS,
        account.name,
      );
      const account_access_path = get_access_key_symlink_path(
        CONFIG_SUBDIRS.ACCESS_KEYS,
  ...
  mocha.describe('read_account_by_access_key', function () {
    ...
    mocha.it('read account by invalid access key', async function () {
      try {
        const access_key =
          account_user1.access_keys[0].access_key.toString() + 'invalid';
  ...
  mocha.describe('bucket policy operations', function () {
    mocha.it('put_bucket_policy ', async function () {
      ...
      const policy_res = await bucketspace_fs.get_bucket_policy(
        param,
        dummy_object_sdk,
      );
