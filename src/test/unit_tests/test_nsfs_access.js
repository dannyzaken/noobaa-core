...
mocha.describe('new tests check', async function () {
    ...
    mocha.it('ROOT readdir - sucsses', async function () {
        const root_entries = await nb_native().fs.readdir(
            ROOT_FS_CONFIG,
            full_path_root,
        );
    ...
    mocha.it('NON ROOT 1 readdir - failure', async function () {
        try {
            const non_root_entries = await nb_native().fs.readdir(
                NON_ROOT1_FS_CONFIG,
                full_path_root,
            );
    ...
    mocha.it('NON ROOT 1 mkdir - failure', async function () {
        try {
            const mkdir_res = await nb_native().fs.mkdir(
                NON_ROOT1_FS_CONFIG,
                full_path_non_root,
                0o770,
            );
            assert.fail(
                `non root has access to mkdir under root dir ${mkdir_res}`,
            );
        } catch (err) {
            assert.equal(err.code, 'EACCES');
        }
    ...
    mocha.it(
        'ROOT readdir - dir created by non dir - success',
        async function () {
            try {
                const root_entries = await nb_native().fs.readdir(
                    ROOT_FS_CONFIG,
                ...
                assert.fail(
                    `root has access to a folder that should not exist - ${root_entries}`,
                );
            } catch (err) {
    ...
    mocha.it('NON ROOT 1 readdir - success', async function () {
        try {
            ...
            assert.fail(
                `non root 1 has access to a folder created by root with 770 perm - ${p} - ${non_root_entries}`,
            );
        } catch (err) {
            assert.equal(err.code, 'EACCES');
        }
    ...
    mocha.it('NON ROOT 2 readdir - failure', async function () {
        try {
            ...
            assert.fail(
                `non root 2 has access to a folder created by root with 770 perm - ${p} ${non_root_entries}`,
            );
        } catch (err) {
            assert.equal(err.code, 'EACCES');
        }
