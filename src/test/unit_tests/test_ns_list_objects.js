...
function test_ns_list_objects(ns, object_sdk, bucket) {
    const files_without_folders_to_upload = make_keys(
        264,
        i => `file_without_folder${i}`,
    );
    ...
    const files_in_inner_folders_to_upload_post = make_keys(
        264,
        i => `folder1/inner_folder/file${i}`,
    );
    const files_in_inner_folders_to_upload_pre = make_keys(
        264,
        i => `folder1/ainner_folder/file${i}`,
    );
    ...
    mocha.describe('basic tests', function () {
        ...
        mocha.it('delimiter=/ prefix=folder1/', async function () {
            ...
            assert.deepStrictEqual(
                r.objects.map(it => it.key),
                ['folder1/', ...files_in_folders_to_upload],
            );
        });
        ...
        mocha.it('prefix=file_without_folder0', async function () {
            ...
            assert.deepStrictEqual(
                r.objects.map(it => it.key),
                files_without_folders_to_upload.slice(0, 1),
            );
        });
    ...
    mocha.describe('list objects - dirs', function () {
        ...
        mocha.it('key_marker=folder229/', async function () {
            ...
            assert.deepStrictEqual(r.is_truncated, false);
            assert.deepStrictEqual(r.common_prefixes, []);
            const fd = folders_to_upload.filter(
                folder => folder > 'folder229/',
            );
            assert.deepStrictEqual(
                r.objects.map(it => it.key),
        ...
        mocha.it('key_marker=folder1/file57', async function () {
            ...
            assert.deepStrictEqual(r.common_prefixes, []);
            const fd = folders_to_upload.filter(folder => folder > 'folder1/');
            const fd1 = files_in_folders_to_upload.filter(
                folder => folder > 'folder1/file57',
            );
            assert.deepStrictEqual(
                r.objects.map(it => it.key),
                [
        ...
        mocha.it('key_marker=folder1/inner_folder/file40', async function () {
            ...
            assert.deepStrictEqual(r.is_truncated, false);
            assert.deepStrictEqual(r.common_prefixes, []);
            const fd1 = files_in_inner_folders_to_upload_post.filter(
                file => file > 'folder1/inner_folder/file40',
            );
            const fd = folders_to_upload.filter(folder => folder > 'folder1/');
            assert.deepStrictEqual(
                r.objects.map(it => it.key),
                [...fd1, ...fd, ...files_in_utf_diff_delimiter],
            );
        });
        ...
        mocha.it('key_marker=folder1/inner_folder/', async function () {
            ...
            assert.deepStrictEqual(r.common_prefixes, []);
            const fd1 = files_in_inner_folders_to_upload_post.filter(
                file => file > 'folder1/inner_folder/',
            );
            const fd = folders_to_upload.filter(
                folder => folder > 'folder1/inner_folder/',
            );
            assert.deepStrictEqual(
                r.objects.map(it => it.key),
                [...fd1, ...fd, ...files_in_utf_diff_delimiter],
        ...
        mocha.it('key_marker=folder1/ainner_folder/', async function () {
            ...
            assert.deepStrictEqual(r.common_prefixes, []);
            const fd = folders_to_upload.filter(
                folder => folder > 'folder1/ainner_folder/',
            );
            const fd2 = files_in_inner_folders_to_upload_pre.filter(
                file => file > 'folder1/ainner_folder/file50',
            );
            assert.deepStrictEqual(
                r.objects.map(it => it.key),
                [
        ...
        mocha.it(
            'key_marker=folder1/inner_folder/file40 delimiter=/',
            async function () {
                const r = await ns.list_objects(
                    {
                        bucket,
    ...
    async function truncated_listing(
        params,
        use_upload_id_marker,
        upload_mode,
    ) {
        ...
        while (res.is_truncated) {
            ...
            if (params.delimiter) {
                ...
                if (use_upload_id_marker) {
                    params.upload_id_marker = r.upload_id_marker;
                }
            } else {
                const last = r.objects[r.objects.length - 1];
                params.key_marker = last.key;
                if (use_upload_id_marker) {
                    params.upload_id_marker = last.upload_id;
                }
            }
        }
