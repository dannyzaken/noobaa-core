...
class MockNamespace {
  ...
  async read_object_stream(params, object_sdk) {
    ...
    if (_.isUndefined(params.start) && _.isUndefined(params.end)) {
      return new MockReaderStream({ type: this.type, source_buf: obj.buf })
        .reader;
    } else {
...
mocha.describe('namespace caching: upload scenarios', () => {
  ...
  mocha.it('hub upload failure: object not cached', async () => {
    ...
    try {
      ...
      assert(
        _.isUndefined(
          recorder.get_event('cache', bucket, key, EVENT_CREATE_OBJ_MD),
        ),
      );
    }
  });
  ...
  mocha.it('range read case success', async () => {
    ...
    await P.wait_until(
      () => {
        const cache_obj = recorder.get_obj('cache', obj.bucket, obj.key);
        return (
          cache_obj.num_parts === 2 && cache_obj.upload_size === block_size * 3
        );
      },
      2000,
      100,
    ...
    async () => {
      ...
      await P.wait_until(
        () => {
          const cache_obj = recorder.get_obj('cache', obj.bucket, obj.key);
          return (
            cache_obj.num_parts === 1 && cache_obj.upload_size === block_size
          );
        },
        2000,
        100,
