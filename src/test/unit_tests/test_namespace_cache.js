...
function random_object(size) {
    ...
    console.log(
        `creating object for test: key: bucket: ${bucket} key: ${key} size: ${size}`,
    );
...
function validate_metric(bucket, ns_cache, metric_name, expect_value) {
    console.log(
        metric_utils.get_metric(
            ns_cache.stats.prom_metrics_report,
            metric_name,
        ),
    );
    assert(
        metric_utils.get_metric(ns_cache.stats.prom_metrics_report, metric_name)
            .hashMap[`bucket_name:${bucket},`].value === expect_value,
    );
}
...
class Recorder {
    ...
    record_event(type, bucket, key, event_name) {
        this._events[type][`${bucket}.${key}.${event_name}`] =
            new Date().getTime();
    }
    ...
    add_obj(type, bucket, key, obj) {
        this._events[type][`${bucket}.${key}.${EVENT_CREATE_OBJ_MD}`] =
            new Date().getTime();
        const _obj = _.clone(obj);
        _obj.cache_last_valid_time =
            obj.create_time ? obj.create_time : new Date().getTime();
    ...
    update_obj(type, bucket, key, obj) {
        this._events[type][`${bucket}.${key}.${EVENT_UPDATE_OBJ_MD}`] =
            new Date().getTime();
        const _obj = _.clone(obj);
        if (obj.parts) {
            _obj.num_parts = obj.parts.length;
            _obj.upload_size = _.reduce(
                obj.parts,
    ...
    delete_obj(type, bucket, key) {
        this._events[type][`${bucket}.${key}.${EVENT_DELETE_OBJ}`] =
            new Date().getTime();
...
class MockReaderStream {
    constructor({ type, source_buf }) {
        this.type = type;
        this.reader = buffer_utils.buffer_to_read_stream(
            Buffer.from(_.defaultTo(source_buf, 'sample data')),
        );
...
function mock_cache_create_object_upload(recorder, params) {
    console.log(
        `mock_cache_create_object_upload called for bucket:${params.bucket} key:${params.key} start:${params.start} end:${params.end}`,
    );
...
function mock_upload_object_range(recorder, params, object_sdk) {
    console.log(
        `mock_upload_object_range called for bucket:${params.bucket} key:${params.key} start:${params.start} end:${params.end}`,
    );
...
class MockNamespace {
    ...
    read_object_md(params, object_sdk) {
        console.log(
            `${this.type} mock: read_object_md called: bucket ${params.bucket} key ${params.key}`,
        );
        if (this._trigger_err === 'md') {
            const err = new Error(
                `${this.type}: md error triggered: bucket ${params.bucket} key ${params.key}`,
            );
    ...
    async read_object_stream(params, object_sdk) {
        console.log(
            `${this.type} mock: read_object_stream called: bucket ${params.bucket} key ${params.key}`,
            params.missing_part_getter,
        );
        if (this._trigger_err === 'read') {
            const err = new Error(
                `${this.type}: read error triggered: bucket ${params.bucket} key ${params.key}`,
            );
        ...
        this._recorder.record_event(
            this.type,
            params.bucket,
            params.key,
            EVENT_READ_OBJECT_STREAM,
        );
        const obj = this._recorder.get_obj(
            this.type,
            params.bucket,
            params.key,
        );
        if (_.isUndefined(params.start) && _.isUndefined(params.end)) {
            return new MockReaderStream({
                type: this.type,
                source_buf: obj.buf,
            }).reader;
        } else {
            ...
            if (this.type === 'hub') {
                return new MockReaderStream({
                    type: this.type,
                    source_buf: obj.buf.slice(start, end),
                }).reader;
            }
            ...
            if (obj.parts) {
                for (const part of obj.parts) {
                    if (part.start <= start && part.end >= end) {
                        return new MockReaderStream({
                            type: this.type,
                            source_buf: crypto.pseudoRandomBytes(end - start),
                        }).reader;
                    }
                }
            }
            const buf = await params.missing_part_getter(start, end);
            return new MockReaderStream({ type: this.type, source_buf: buf })
                .reader;
        }
    }
    ...
    async upload_object(params, object_sdk) {
        return new Promise((resolve, reject) => {
            ...
            params.source_stream.on('data', data => {
                console.log(
                    `${this.type} mock: got data in upload_object: bucket ${params.bucket} key ${params.key}`,
                    data,
                );
            ...
            params.source_stream.on('error', err => {
                console.log(
                    `${this.type} mock: got err in upload_object: bucket ${params.bucket} key ${params.key}`,
                    err,
                );
                this._write_err = err;
                reject(err);
            ...
            params.source_stream.on('close', () => {
                console.log(
                    `${this.type} mock: got close in upload_object: bucket ${params.bucket} key ${params.key}`,
                );
            });
            params.source_stream.on('end', async () => {
                console.log(
                    `${this.type} mock: got end in upload_object: bucket ${params.bucket} key ${params.key}`,
                );
                if (this._trigger_err === 'write') {
                    setTimeout(() => {
                        console.log(
                            `${this.type} mock: trigger error in upload_object: bucket ${params.bucket} key ${params.key}`,
                        );
                        const err = new Error(
                            `${this.type}: write error triggered: bucket ${params.bucket} key ${params.key}`,
                        );
                ...
                if (this._write_err) {
                    ...
                    console.log(
                        `${this.type} mock: write err bucket ${params.bucket} key ${params.key}`,
                    );
                ...
                if (this._slow_write) {
                    console.log(
                        `${this.type} mock: bucket ${params.bucket} key ${params.key} slowing down write......`,
                    );
                    await P.delay(100);
                }
            ...
            params.source_stream.on('finish', async () => {
                console.log(
                    `${this.type} mock: got finish in upload_object: bucket ${params.bucket} key ${params.key}`,
                    recv_buf,
                );
                this._recorder.record_event(
                    this.type,
    ...
    async delete_object(params, object_sdk) {
        console.log(
            `${this.type} mock: delete_object called: bucket ${params.bucket} key ${params.key}`,
        );
        if (this._trigger_err === 'delete') {
            console.log(
                `${this.type} mock: delete_object err triggered: bucket ${params.bucket} key ${params.key}`,
            );
            const err = new Error(
                `${this.type}: delete error triggered: bucket ${params.bucket} key ${params.key}`,
            );
    ...
    async delete_multiple_objects(params, object_sdk) {
        console.log(
            `${this.type} mock: delete_multiple_objects called: bucket ${params.bucket} key ${params.key}`,
        );
        if (this._trigger_err === 'multi-deletes') {
            console.log(
                `${this.type} mock: delete_multiple_objects err triggered: bucket ${params.bucket} keys ${params.objects}`,
            );
            const err = new Error(
                `${this.type}: delete_multiple_objects error triggered: bucket ${params.bucket} keys ${params.objects}`,
            );
    ...
    /** @returns {any} */
    not_implemented() {
        throw new Error('MockNamespace not implemented');
    }
    ...
    async set_triggers_for_bucket(params, object_sdk) {
        return this.not_implemented();
    }
    async get_triggers_for_bucket(params, object_sdk) {
        return this.not_implemented();
    }
    async _dispatch_multiple_delete_triggers(params, object_sdk) {
        return this.not_implemented();
    }
...
async function create_namespace_cache_and_read_obj({
    ...
    const hub = new MockNamespace({
        type: 'hub',
        recorder,
        slow_write: true,
        trigger_err: trigger_hub_err,
    });
    const cache = new MockNamespace({
        type: 'cache',
        recorder,
        trigger_err: trigger_cache_err,
    });
    ...
    if (params.start || params.end) {
        expect_etag = crypto
            .createHash('md5')
            .update(obj.buf.slice(start, end))
            .digest('hex');
    }
    assert(read_etag === expect_etag);

    const hub_obj_create_time = recorder.get_event(
        'hub',
        obj.bucket,
        obj.key,
        EVENT_CREATE_OBJ_MD,
...
mocha.describe('namespace caching: upload scenarios', () => {
    ...
    mocha.it('cache object during upload', async () => {
        ...
        const ns_cache = new NamespaceCache({
            namespace_hub: new MockNamespace({
                type: 'hub',
                recorder,
                slow_write: true,
            }),
        ...
        const params = {
            ...
            key,
            size,
            source_stream: new MockReaderStream({
                type: 's3_client',
                source_buf: buf,
            }).reader,
        };
        ...
        assert(!_.isUndefined(hub_obj_create_time));
        await P.wait_until(() => {
            const cache_obj_create_time = recorder.get_event(
                'cache',
                bucket,
                key,
                EVENT_CREATE_OBJ_MD,
    ...
    mocha.it('hub upload failure: object not cached', async () => {
        const ns_cache = new NamespaceCache({
            ...
            namespace_nb: new MockNamespace({
                type: 'cache',
                recorder,
                trigger_err: 'write',
            }),
        ...
        try {
            ...
            assert(
                _.isUndefined(
                    recorder.get_event('hub', bucket, key, EVENT_CREATE_OBJ_MD),
                ),
            );
            assert(
                _.isUndefined(
    ...
    mocha.it(
        'cache upload failure: return hub status and object not cached',
        async () => {
            const cache = new MockNamespace({
                type: 'cache',
                recorder,
...
mocha.describe('namespace caching: read scenarios and fresh objects', () => {
    ...
    mocha.it(
        'fresh object still cached if precondition (e.g. if-etag) is set by s3 client',
        async () => {
            const hub = new MockNamespace({
                type: 'hub',
                recorder,
...
mocha.describe(
    'namespace caching: read scenarios that object is cached',
    () => {
        let recorder;
        const ttl_ms = 100;
        const object_sdk = {
        ...
        mocha.it('update cache_last_valid_time', async () => {
            ...
            params.object_md = await ns_cache.read_object_md(
                params,
                object_sdk,
            );
            ...
            await P.wait_until(
                () => {
                    ...
                    return (
                        _obj.cache_last_valid_time > old_cache_last_valid_time
                    );
                },
                2000,
                100,
        ...
        mocha.it(
            'cache_last_valid_time updated after etag mismatch',
            async () => {
                const hub = new MockNamespace({
                    type: 'hub',
                    recorder,
                ...
                const old_cache_last_valid_time =
                    cache_obj_md.cache_last_valid_time;
                await P.delay(ttl_ms + 100);
                _.set(
                    object_sdk,
                ...
                const stream = await ns_cache.read_object_stream(
                    params,
                    object_sdk,
                );
                const read_buf = await buffer_utils.read_stream_join(stream);
                ...
                await P.wait_until(
                    () => {
                        const _obj = cache.get_obj(
                            cache_obj_md.bucket,
                            cache_obj_md.key,
                        );
                        return (
                            _obj.cache_last_valid_time >
                                old_cache_last_valid_time &&
                            _obj.etag === hub_obj_md.etag
                        );
                    },
                    2000,
                    100,
        ...
        mocha.it('read from hub if read from cache fails', async () => {
            ...
            const read_obj_stream_time = recorder.get_event(
                'hub',
                obj.bucket,
                obj.key,
                EVENT_READ_OBJECT_STREAM,
            ...
            assert(
                read_obj_stream_time > params.object_md.cache_last_valid_time,
            );
        });
        ...
        mocha.it('cache object that has inline read performed', async () => {
            ...
            const read_etag = crypto
                .createHash('md5')
                .update(params.object_md.first_range_data)
                .digest('hex');
            assert(read_etag === obj.etag);
...
mocha.describe(
    'namespace caching: proxy get request with partNumber query to hub',
    () => {
        let recorder;
        const ttl_ms = 2000;
        const object_sdk = {
...
mocha.describe('namespace caching: range read scenarios', () => {
    ...
    mocha.before(() => {
        ...
        _.set(
            ...
            _.curry(mock_cache_create_object_upload)(recorder),
        );
        _.set(object_sdk, 'object_io.upload_object_range', params =>
            mock_upload_object_range(recorder, params, object_sdk),
        );
    });
    ...
    mocha.it('range read case success', async () => {
        ...
        await P.wait_until(
            () => {
                const cache_obj = recorder.get_obj(
                    'cache',
                    obj.bucket,
                    obj.key,
                );
                return (
                    !_.isUndefined(cache_obj) &&
                    cache_obj.num_parts === 1 &&
                    cache_obj.upload_size === block_size
                );
            },
            2000,
            100,
        ...
        await P.wait_until(
            () => {
                ...
                return (
                    cache_obj.num_parts === 2 &&
                    cache_obj.upload_size === block_size * 3
                );
            },
            2000,
            100,
    ...
    mocha.it(
        'range read case: part cached if precondition if-match set by s3 client matches object md',
        async () => {
            const hub = new MockNamespace({
                type: 'hub',
                recorder,
    ...
    mocha.it(
        'range read case: part not cached if precondition if-match set by s3 client does not match object md',
        async () => {
            const hub = new MockNamespace({
                type: 'hub',
                recorder,
    ...
    mocha.it(
        'range read case: part not cached if other precondition than if-match is set by s3 client',
        async () => {
            const hub = new MockNamespace({
                type: 'hub',
                recorder,
        ...
        await P.wait_until(
            () => {
                try {
                    ...
                    return (
                        !_.isUndefined(cache_obj) && cache_obj.num_parts === 0
                    );
                } catch (err) {
                    return false;
                }
    ...
    mocha.it(
        'range read falls back to hub read if read from cache fails',
        async () => {
            const size = block_size * 2;
            let start = block_size - 2;
            // end is exclusive
            let end = start + 100;
            const { ns_cache, obj, cache } =
                await create_namespace_cache_and_read_obj({
                    recorder,
                    size,
                    ttl_ms,
            ...
            const read_etag = crypto
                ...
                .update(read_buf)
                .digest('hex');
            const expect_etag = crypto
                .createHash('md5')
                .update(obj.buf.slice(start, end))
                .digest('hex');
            assert(read_etag === expect_etag);

            const read_hub_time = recorder.get_event(
                'hub',
                obj.bucket,
                obj.key,
                EVENT_READ_OBJECT_STREAM,
            ...
            console.log(
                `CHECK_VALUE: read_hub_time ${read_hub_time} err_trigger_time ${err_trigger_time}`,
            );
...
mocha.describe('namespace caching: delete scenario', () => {
    ...
    mocha.it(
        'cache obj is still deleted if hub delete failure (should cache obj be kept?)',
        async () => {
            const { ns_cache, obj } = await create_namespace_cache_and_read_obj(
                {
                    recorder,
