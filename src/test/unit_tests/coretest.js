...
function setup(options = {}) {
  ...
  if (incomplete_rpc_coverage) {
    assert(
      incomplete_rpc_coverage === 'show' || incomplete_rpc_coverage === 'fail',
...
async function overwrite_system_address(system_name) {
  ...
  await P.wait_until(() => system_server.is_initialized(), 2 * 60 * 1000, 1000);
  ...
  await P.wait_until(
    async () => {
      ...
      return pools_to_create.every(
        pool => pool.host_count === (optimal_hosts_by_Pool[pool.name] || 0),
      );
    },
    5 * 60 * 1000,
    2500,
