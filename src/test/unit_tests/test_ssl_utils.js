...
mocha.describe('ssl_utils', function () {
  ...
  mocha.describe('nb_native().x509()', function () {
    ...
    mocha.it(
      'should generate unauthorized https cert due to expired date',
      async function () {
        try {
          await test_https_with_cert(x509({ dns: 'localhost', days: -1 }));
    ...
    mocha.it(
      'should generate unauthorized https cert due to DNS mismatch',
      async function () {
        try {
  ...
  mocha.describe('generate_ssl_certificate', function () {
    ...
    mocha.it('should detect invalid cert', function () {
      ...
      assert.throws(() => x509_verify(x), /^Error: X509 PEM decode failed$/);
    });
    ...
    mocha.it('should detect mismatch key cert', function () {
      ...
      assert.throws(() => x509_verify(x), /^Error: X509 verify failed$/);
    });
  });
  ...
  function check_cert(x) {
    ...
    assert.strictEqual(
      key_lines[key_lines.length - 1],
      '-----END PRIVATE KEY-----',
    );
    assert.strictEqual(cert_lines[0], '-----BEGIN CERTIFICATE-----');
    assert.strictEqual(
      cert_lines[cert_lines.length - 1],
      '-----END CERTIFICATE-----',
    );
  ...
  async function test_https_with_cert(
    ssl_cert,
