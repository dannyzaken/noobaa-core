...
mocha.describe('kmeans working', function () {
  mocha.it('handle noise in vector', () => {
    ...
    const noise_cluster = _.find(result, cluster =>
      _.isEmpty(_.difference(cluster.centroid, expected_response[1].centroid)),
    );
    const regular_cluster = _.find(result, cluster =>
      _.isEmpty(_.difference(cluster.centroid, expected_response[0].centroid)),
    );
    ...
    assert(
      _.isEmpty(
        _.difference(noise_cluster.clusterInd, expected_response[1].clusterInd),
      ),
