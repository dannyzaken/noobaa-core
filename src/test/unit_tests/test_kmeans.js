...
mocha.describe('kmeans errors', function () {
    ...
    mocha.it('should throw an error if K larger than number of vectors', () => {
        ...
        assert(
            was_thrown,
            'should throw an error if K larger than number of vectors',
        );
    });
    ...
    mocha.it("should throw an error if distance isn't a function", () => {
        ...
        assert(
            was_thrown,
            "should throw an error if distance isn't a function",
        );
    });

    mocha.it(
        "should throw an error if distance isn't a function with 2 parameters",
        () => {
            let was_thrown = false;
            try {
...
mocha.describe('kmeans working', function () {
    mocha.it('handle noise in vector', () => {
        const result = kmeans.run([[26], [4], [1992], [9999]], { k: 2 });
        ...
        const noise_cluster = _.find(result, cluster =>
            _.isEmpty(
                _.difference(cluster.centroid, expected_response[1].centroid),
            ),
        );
        const regular_cluster = _.find(result, cluster =>
            _.isEmpty(
                _.difference(cluster.centroid, expected_response[0].centroid),
            ),
        );
        ...
        assert(
            noise_cluster,
            'KMEANS did not create a noise cluster with expected centroid',
        );
        assert(
            regular_cluster,
            'KMEANS did not create a regular cluster with expected centroid',
        );
        assert(
            _.isEmpty(
                _.difference(
                    noise_cluster.clusterInd,
                    expected_response[1].clusterInd,
                ),
            ),
        ...
        assert(
            _.isEmpty(
                _.difference(
                    regular_cluster.clusterInd,
                    expected_response[0].clusterInd,
                ),
            ),
        ...
        assert(
            _.isEmpty(
                _.difference(
                    _.flatten(noise_cluster.cluster),
                    _.flatten(expected_response[1].cluster),
                ),
            ),
            'KMEANS did not create a noise cluster with expected vectors',
        );
        assert(
            _.isEmpty(
                _.difference(
                    _.flatten(regular_cluster.cluster),
                    _.flatten(expected_response[0].cluster),
                ),
            ),
            'KMEANS did not create a regular cluster with expected vectors',
        );
