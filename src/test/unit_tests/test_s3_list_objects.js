...
mocha.describe('s3_list_objects', function () {
    ...
    mocha.it('issue use case', function () {
        ...
        const issue_files_folders_to_upload = [
            '20220323/99/test.txt',
            '20220323/990/test.txt',
        ];
        const expected_files_uploaded = ['20220323/99/', '20220323/990/'];

        return run_case(
            issue_files_folders_to_upload,
            async function (server_upload_response) {
                ...
                assert.strictEqual(
                    _.difference(
                        expected_files_uploaded,
                        list_reply.common_prefixes,
                    ).length,
                    0,
                    'prefixes: ' + list_reply.common_prefixes,
                );
                assert.strictEqual(
                    _.map(list_reply.objects, obj => obj.key).length,
                    0,
    ...
    mocha.it('general use case', function () {
        ...
        return run_case(
            ...
            async function (server_upload_response) {
                ...
                assert.strictEqual(
                    _.difference(folders_to_upload, list_reply.common_prefixes)
                        .length,
                    0,
                    'prefixes: ' + list_reply.common_prefixes,
                );
                assert.strictEqual(
                    list_reply.objects.length,
                    0,
                    'objects: ' + list_reply.objects,
                );
                assert(
                ...
                assert.strictEqual(
                    _.difference(files_without_folders_to_upload, objects)
                        .length,
                    0,
                    'objects: ' + objects,
                );
                assert(
                    is_sorted_array(objects),
                    'objcets not sorted' + objects,
                );
                assert(
                    is_sorted_array(list_reply.common_prefixes),
                    'prefixes not sorted',
                ...
                assert.strictEqual(
                    list_reply.common_prefixes.length,
                    0,
                    'prefixes: ' + list_reply.common_prefixes,
                );
                assert.strictEqual(
                    _.difference(files_in_folders_to_upload, objects).length,
                    0,
                    'objects: ' + objects,
                );
                assert(
    ...
    mocha.it('general use case 2', function () {
        ...
        return run_case(
            ...
            async function (server_upload_response) {
                ...
                // Also we check that the response is not truncated
                assert.strictEqual(
                    _.difference(['תיקיה#'], list_reply.common_prefixes).length,
                    0,
                    'prefixes: ' + list_reply.common_prefixes,
                );
                assert.strictEqual(
                ...
                assert.strictEqual(
                    files_without_folders_to_upload[0],
                    objects[0],
                    'objects:' + objects,
                );
                assert(
    ...
    mocha.it('max keys test', function () {
        ...
        return run_case(
            ...
            async function (server_upload_response) {
                ...
                assert.strictEqual(
                    _.difference(max_keys_objects, objects).length,
                    0,
                    'objects:' + list_reply.objects,
                );
                assert(
                    is_sorted_array(list_reply.objects),
                    'objcets not sorted' + list_reply.objects,
                );
                assert(
                    is_sorted_array(list_reply.common_prefixes),
                    'prefixes not sorted',
    ...
    mocha.it('multipart uploads id-marker', function () {
        ...
        return run_case(
            files_in_multipart_folders_to_upload,
            async function (server_upload_response) {
                // Uploading zero size objects from the key arrays that were provided
                ...
                assert.strictEqual(
                    _.difference(
                        server_upload_response.map(obj => obj.key),
                        objects,
                    ).length,
                    0,
                    'objects:' + objects,
                ...
                assert.strictEqual(
                    _.difference([server_upload_response[0].key], objects)
                        .length,
                    0,
                    'objects:' + objects,
                );
                ...
                assert.strictEqual(
                    String(server_upload_response[0].obj_id),
                    String(list_reply.next_upload_id_marker),
                );
                assert(list_reply.is_truncated, 'not truncated');
    ...
    mocha.it('multipart uploads id-marker breakage test', function () {
        ...
        return run_case(
            ...
            async function (server_upload_response) {
                ...
                assert.strictEqual(
                    _.difference(['multipart2/'], list_reply.common_prefixes)
                        .length,
                    0,
                    'prefixes: ' + list_reply.common_prefixes,
                );
                assert.strictEqual(
                    _.difference(
                        _.concat(same_multipart_file1, same_multipart_file2),
                        objects,
                    ).length,
                    0,
                    'objects:' + objects,
                ...
                assert.strictEqual(
                    list_reply.common_prefixes.length + objects.length,
                    21,
                    util.inspect(list_reply),
                );
    ...
    mocha.it(
        'prefix with delimiter infinite infinite loop listing',
        function () {
            const self = this; // eslint-disable-line no-invalid-this
            self.timeout(10 * 60 * 1000);
            ...
            return run_case(
                ...
                async function (server_upload_response) {
                    ...
                    const list_reply = await truncated_listing(
                        ...
                        /* use_upload_id_marker = */ false,
                        /* upload_mode = */ false,
                    );
                    const objects = _.map(list_reply.objects, obj => obj.key);
                    assert.strictEqual(
                        _.difference(['d/d/'], list_reply.common_prefixes)
                            .length,
                        0,
                        'prefixes: ' + list_reply.common_prefixes,
                    );
                    assert.strictEqual(
                        _.difference(['d/f'], objects).length,
                        0,
                        'objects:' + objects,
                    );
                    assert.strictEqual(
...
function is_sorted_array(arr) {
    return _.every(arr, (value, index, array) => {
        ...
        const equal_key =
            index && String(array[index - 1].key) === String(value.key);
        return equal_key ?
...
async function run_case(array_of_names, case_func, only_initiate) {
    const response =
        only_initiate ?
            await initiate_upload_multiple_files(array_of_names)
        :   await upload_multiple_files(array_of_names);
    const response_array = response;
    await case_func(response);
    await clean_up_after_case(
        only_initiate ? response_array : array_of_names,
        only_initiate,
    );
}
...
async function truncated_listing(params, use_upload_id_marker, upload_mode) {
    ...
    while (listObjectsResponse.is_truncated) {
        ...
        if (res_list.objects.length) {
            listObjectsResponse.objects = _.concat(
                listObjectsResponse.objects,
                res_list.objects,
            );
        }
        if (res_list.common_prefixes.length) {
            listObjectsResponse.common_prefixes = _.concat(
                listObjectsResponse.common_prefixes,
                res_list.common_prefixes,
            );
        }
