...
mocha.describe('cli logging flow', async function () {
    ...
    mocha.before(async () => {
        ...
        await exec_manage_cli(TYPES.BUCKET, ACTIONS.ADD, {
            name: 'logbucket',
            path: bucket_path,
            owner: 'logbucketowner',
        });
        const data =
            '{"noobaa_bucket_logging":"true","op":"GET","bucket_owner":"admin@noobaa.io",' +
        ...
        await nb_native().fs.writeFile(
            DEFAULT_FS_CONFIG,
            path.join(pers_log_path + '/', 'bucket_logging.log'),
            Buffer.from(data + '\n'),
        );
    });

    mocha.it('cli run logging', async function () {
        const res = await exec_manage_cli(TYPES.LOGGING, '', {}, false, {
            GUARANTEED_LOGS_PATH: pers_log_path,
        });
        const entries = await nb_native().fs.readdir(
            DEFAULT_FS_CONFIG,
            bucket_path,
        );
        const log_objects = entries.filter(
        ...
        assert.equal(
            ...
            ManageCLIResponse.LoggingExported.code,
        );
    });
});
