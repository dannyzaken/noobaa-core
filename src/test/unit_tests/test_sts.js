...
const errors = {
    ...
    invalid_schema_params: {
        ...
        message:
            'INVALID_SCHEMA_PARAMS CLIENT account_api#/methods/create_account',
    },
    malformed_policy: {
...
mocha.describe('STS tests', function () {
    ...
    mocha.before(async function () {
        ...
        admin_keys = (await rpc_client.account.read_account({ email: EMAIL }))
            .access_keys;
    ...
    mocha.it(
        'user a assume role of admin - should be rejected',
        async function () {
    ...
    mocha.it(
        'admin assume role of user b - should be allowed',
        async function () {
            const params = {
            ...
            validate_assume_role_response(
                json,
                `arn:aws:sts::${user_b_key}:assumed-role/${role_b}/${params.RoleSessionName}`,
    ...
    mocha.it(
        'admin assume non existing role of user b - should be rejected',
        async function () {
            await assert_throws_async(
                sts_admin
                    .assumeRole({
    ...
    mocha.it(
        'admin assume non existing role of non existing user - should be rejected',
        async function () {
            await assert_throws_async(
                sts_admin
                    .assumeRole({
    ...
    mocha.it(
        'anonymous user a assume role of user b - should be rejected',
        async function () {
            await assert_throws_async(
                anon_sts
                    .assumeRole({
    ...
    mocha.it(
        'user c assume role of user b - should be allowed',
        async function () {
            const params = {
                RoleArn: `arn:aws:sts::${user_b_key}:role/${role_b}`,
                RoleSessionName: 'just_a_dummy_session_name',
            ...
            const temp_creds = validate_assume_role_response(
                json,
                `arn:aws:sts::${user_b_key}:assumed-role/${role_b}/${params.RoleSessionName}`,
                `${user_b_key}:${params.RoleSessionName}`,
                user_b_key,
            ...
            const list_objects_res = await s3
                .listObjects({ Bucket: 'first.bucket' })
                .promise();
    ...
    mocha.it(
        'user a assume role of user b - should be rejected',
        async function () {
            await assert_throws_async(
                sts
                    .assumeRole({
    ...
    mocha.it(
        'update assume role policy of user b to allow user a',
        async function () {
            const policy = {
                version: '2012-10-17',
                statement: [
    ...
    mocha.it(
        'user a assume role of user b - should be allowed',
        async function () {
            const params = {
                RoleArn: `arn:aws:sts::${user_b_key}:role/${role_b}`,
                RoleSessionName: 'just_a_dummy_session_name',
    ...
    mocha.it(
        'update assume role policy of user b to allow user a sts:*',
        async function () {
            const policy = {
                version: '2012-10-17',
                statement: [
    ...
    mocha.it(
        'user a assume role of user b - should be rejected sts:*',
        async function () {
            await assert_throws_async(
                sts
                    .assumeRole({
    ...
    mocha.it(
        'user c assume role of user b - should be allowed sts:*',
        async function () {
            const params = {
                RoleArn: `arn:aws:sts::${user_b_key}:role/${role_b}`,
                RoleSessionName: 'just_a_dummy_session_name',
    ...
    mocha.it(
        'user c assume role of user b - should be rejected *',
        async function () {
            await assert_throws_async(
                sts_c
                    .assumeRole({
...
function validate_assume_role_response(
    json,
    expected_arn,
    expected_role_id,
    assumed_access_key,
    ...
    assert.ok(
        json &&
            json.AssumeRoleResponse &&
            json.AssumeRoleResponse.AssumeRoleResult,
    );
    ...
    assert.ok(
        credentials &&
            credentials.AccessKeyId[0] &&
            credentials.SecretAccessKey[0],
    );
    assert.equal(
        credentials.Expiration[0],
        config.STS_DEFAULT_SESSION_TOKEN_EXPIRY_MS,
    );
    if (config.STS_DEFAULT_SESSION_TOKEN_EXPIRY_MS !== 0) {
        verify_session_token(
            credentials.SessionToken[0],
            credentials.AccessKeyId[0],
            credentials.SecretAccessKey[0],
            assumed_access_key,
        );
...
async function assert_throws_async(promise, expected_code, expected_message) {
    try {
        ...
        dbg.log0(
            'assert_throws_async err.message',
            err.message,
            expected_message,
            err.message !== expected_message,
        );
        dbg.log0(
            'assert_throws_async err.code',
            err.code,
            expected_code,
        ...
        dbg.log0(
            ...
            err.rpc_code,
            expected_code,
            err.rpc_code !== expected_code,
        );
        const code_or_rpc_code = err.code || err.rpc_code;
        if (
            err.message !== expected_message ||
            code_or_rpc_code !== expected_code
        ) {
            throw err;
        }
    }
...
function verify_session_token(
    ...
    assert.equal(
        assumed_role_access_key,
        session_token_json.assumed_role_access_key,
    );
}
...
mocha.describe('Session token tests', function () {
    ...
    mocha.it(
        'user b assume role of user a - default expiry - list s3 - should be allowed',
        async function () {
            ...
            const json = await assume_role_and_parse_xml(
                accounts[1].sts,
                params,
            );
            const result_obj = validate_assume_role_response(
                json,
                `arn:aws:sts::${user_a_key}:assumed-role/${role_alice}/${params.RoleSessionName}`,
                `${user_a_key}:${params.RoleSessionName}`,
                user_a_key,
            );
            ...
            const buckets1 = await temp_s3_with_session_token
                .listBuckets()
                .promise();
    ...
    mocha.it(
        'user b assume role of user a - default expiry - list s3 without session token - should be rejected',
        async function () {
            const user_a_key = accounts[0].access_keys[0].access_key.unwrap();
            const params = {
                RoleArn: `arn:aws:sts::${user_a_key}:role/${role_alice}`,
            ...
            await assert_throws_async(
                temp_s3.listBuckets().promise(),
                errors.invalid_access_key.code,
                errors.invalid_access_key.message,
            );
        },
    );

    mocha.it(
        'user b, user c assume role of user a - default expiry - user b list s3 with session token of user c- should be rejected',
        async function () {
            const user_a_key = accounts[0].access_keys[0].access_key.unwrap();
            const params = {
                RoleArn: `arn:aws:sts::${user_a_key}:role/${role_alice}`,
            ...
            const json1 = await assume_role_and_parse_xml(
                accounts[1].sts,
                params,
            );
            const result_obj1 = validate_assume_role_response(
                json1,
                `arn:aws:sts::${user_a_key}:assumed-role/${role_alice}/${params.RoleSessionName}`,
                `${user_a_key}:${params.RoleSessionName}`,
                user_a_key,
            ...
            const json2 = await assume_role_and_parse_xml(
                accounts[2].sts,
                params,
            );
            const result_obj2 = validate_assume_role_response(
                json2,
                `arn:aws:sts::${user_a_key}:assumed-role/${role_alice}/${params.RoleSessionName}`,
                `${user_a_key}:${params.RoleSessionName}`,
                user_a_key,
            ...
            await assert_throws_async(
                temp_s3.listBuckets().promise(),
                errors.signature_doesnt_match.code,
                errors.signature_doesnt_match.message,
            );
        },
    );

    mocha.it(
        'user b assume role of user a - default expiry - list s3 with permanent creds and temp session token- should be allowed',
        async function () {
            const user_a_key = accounts[0].access_keys[0].access_key.unwrap();
            const user_a_secret =
                accounts[0].access_keys[0].secret_key.unwrap();
            const params = {
                RoleArn: `arn:aws:sts::${user_a_key}:role/${role_alice}`,
                RoleSessionName: 'just_a_dummy_session_name',
            };
            ...
            await assert_throws_async(
                temp_s3_with_session_token.listBuckets().promise(),
                errors.signature_doesnt_match.code,
                errors.signature_doesnt_match.message,
            );
        },
    ...
    mocha.it(
        'user b assume role of user a - default expiry - list s3 with faulty temp session token- should be allowed',
        async function () {
            const user_a_key = accounts[0].access_keys[0].access_key.unwrap();
            const params = {
                RoleArn: `arn:aws:sts::${user_a_key}:role/${role_alice}`,
            ...
            await assert_throws_async(
                temp_s3_with_session_token.listBuckets().promise(),
                errors.invalid_token_s3.code,
                errors.invalid_token_s3.message,
            );
        },
    );

    mocha.it(
        'user b assume role of user a - default expiry - assume role sts with permanent creds and temp session token- should be allowed',
        async function () {
            const user_a_key = accounts[0].access_keys[0].access_key.unwrap();
            const user_a_secret =
                accounts[0].access_keys[0].secret_key.unwrap();
            ...
            await assert_throws_async(
                temp_sts_with_session_token.assumeRole(params).promise(),
                errors.access_denied.code,
                errors.access_denied.message,
            );
        },
    ...
    mocha.it(
        'user b assume role of user a - default expiry - assume role sts faulty temp session token- should be allowed',
        async function () {
            const user_a_key = accounts[0].access_keys[0].access_key.unwrap();
            const params = {
                RoleArn: `arn:aws:sts::${user_a_key}:role/${role_alice}`,
    ...
    mocha.it(
        'user b assume role of user a - expiry 0 - list s3 - should be rejected',
        async function () {
            ...
            await assert_throws_async(
                temp_s3_with_session_token.listBuckets().promise(),
                errors.expired_token_s3.code,
                errors.expired_token_s3.message,
            );
        },
    );

    mocha.it(
        'user b assume role of user a - expiry 0 - assume role sts - should be rejected',
        async function () {
            config.STS_DEFAULT_SESSION_TOKEN_EXPIRY_MS = 0;
            ...
            await assert_throws_async(
                temp_sts_with_session_token.assumeRole(params).promise(),
                errors.expired_token.code,
                errors.expired_token.message,
            );
        },
    );
});
...
mocha.describe('Assume role policy tests', function () {
    ...
    mocha.it(
        'create account with role policy - missing role_config',
        async function () {
            const empty_role_config = {};
            const email = 'assume_email1';
            await assert_throws_async(
                rpc_client.account.create_account({
                    ...account_defaults,
                    email,
                    name: email,
                    role_config: empty_role_config,
                }),
                errors.invalid_schema_params.code,
    ...
    mocha.it(
        'create account with role policy - missing assume role policy',
        async function () {
    ...
    mocha.it(
        'create account with role policy- invalid principal',
        async function () {
            ...
            await assert_throws_async(
                ...
                errors.malformed_policy.rpc_code,
                errors.malformed_policy.message_principal,
            );
        },
    );

    mocha.it(
        'create account with role policy- invalid effect',
        async function () {
    ...
    mocha.it(
        'create account with role policy - invalid action',
        async function () {
