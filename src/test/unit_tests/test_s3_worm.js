...
async function assert_throws_async(
  ...
  try {
    ...
    if (err.message !== expected_message || err.code !== expected_code) {
      throw err;
    }
  }
}
mocha.describe('s3 worm', function () {
  ...
  mocha.describe('buckets creation', function () {
    mocha.it('create bucket BKT & enable lock', async function () {
      const res = await s3_owner
        .createBucket({ Bucket: BKT, ObjectLockEnabledForBucket: true })
        .promise();
      assert.equal(res.Location, `/${BKT}`);
    });
    mocha.it('create bucket BKT1 & enable lock', async function () {
      const res = await s3_owner
        .createBucket({ Bucket: BKT1, ObjectLockEnabledForBucket: true })
        .promise();
    ...
    mocha.it(
      'should fail suspend versioning on locked bucket',
      async function () {
  ...
  mocha.describe(
    'object OBJ1 creation before having lock config on BKT',
    function () {
      mocha.it('create object OBJ1 in BKT', async function () {
        const put_object_res = await s3_owner
          .putObject({
            Bucket: BKT,
            Key: OBJ1,
            Body: file_body,
  ...
  mocha.describe('bucket object lock configuration on BKT', function () {
    mocha.it(
      'should fail put lock configuration with boch days and years',
      async function () {
        await assert_throws_async(
          s3_owner
            .putObjectLockConfiguration({
              ...
              ObjectLockConfiguration: {
                ...
                Rule: {
                  DefaultRetention: { Years: 1, Days: 15, Mode: 'COMPLIANCE' },
                },
              },
            })
            .promise(),
          'MalformedXML',
          malformedXML_message,
        );
      },
    );
    mocha.it(
      'should fail put lock configuration with 0 years',
      async function () {
        await assert_throws_async(
          s3_owner
            .putObjectLockConfiguration({
    ...
    mocha.it('should get empty lock configuration', async function () {
      const conf = await s3_owner
        .getObjectLockConfiguration({ Bucket: BKT })
        .promise();
      assert.equal(conf.ObjectLockEnabled, 'Enabled');
      assert.strictEqual(conf.ObjectLockConfiguration.Rule, undefined);
    });
    mocha.it(
      'should fail put lock configuration with disabled lock',
      async function () {
        await assert_throws_async(
          s3_owner
            .putObjectLockConfiguration({
    ...
    mocha.it('should get non empty lock configuration', async function () {
      ...
      assert.strictEqual(
        conf.ObjectLockConfiguration.Rule.DefaultRetention.Days,
        15,
      );
    });
  });
  mocha.describe('OBJ1 checks', function () {
    mocha.it(
      'should head object without default values of the BKT conf',
      async function () {
        const conf = await s3_owner
          .headObject({ Bucket: BKT, Key: OBJ1 })
          .promise();
        assert.ok(
          !conf.ObjectLockLegalHoldStatus &&
            !conf.ObjectLockRetainUntilDate &&
            !conf.ObjectLockMode,
        );
      },
    );
    ...
    mocha.it('should get object with legal hold', async function () {
      const conf = await s3_owner
        .getObjectLegalHold({ Bucket: BKT, Key: OBJ1, VersionId: version_id1 })
        .promise();
    ...
    mocha.it('should get retention 1', async function () {
      const conf = await s3_owner
        .getObjectRetention({ Bucket: BKT, Key: OBJ1, VersionId: version_id1 })
        .promise();
    ...
    mocha.it(
      'should get legal hold (check if legal hold overridden)',
      async function () {
        const conf = await s3_owner
          .getObjectLegalHold({
            Bucket: BKT,
    ...
    mocha.it(
      'IT25. should get object with retention (compliance mode)',
      async function () {
        const conf = await s3_owner
          .getObjectRetention({
            Bucket: BKT,
    ...
    mocha.it(
      'put object with retention mode and without date',
      async function () {
        const params = {
          Bucket: BKT,
          Key: OBJ1,
        ...
        await assert_throws_async(
          s3_owner.putObject(params).promise(),
          'InvalidArgument',
  ...
  mocha.describe(
    'put object OBJ3 after bucket configuration exist',
    function () {
      ...
      mocha.it('should head object', async function () {
        const conf = await s3_owner
          .headObject({ Bucket: BKT, Key: OBJ3 })
          .promise();
      ...
      mocha.it(
        'should fail put retention (compliance mode)',
        async function () {
          await assert_throws_async(
            s3_owner
              .putObjectRetention({
  ...
  mocha.describe('put object', function () {
    ...
    mocha.it('should get retention', async function () {
      const conf = await s3_owner
        .getObjectRetention({ Bucket: BKT, Key: OBJ4, VersionId: version_id4 })
        .promise();
      assert.ok(conf.Retention.RetainUntilDate);
      assert.equal(conf.Retention.Mode, 'GOVERNANCE');
    });
    mocha.it('should get LegalHold', async function () {
      const conf = await s3_owner
        .getObjectLegalHold({ Bucket: BKT, Key: OBJ4, VersionId: version_id4 })
        .promise();
      assert.equal(conf.LegalHold.Status, 'OFF');
    });
    mocha.it(
      'should fail delete object with retention (governanceBypass=false)',
      async function () {
        await assert_throws_async(
          s3_owner
            .deleteObject({
    ...
    mocha.it(
      'should delete object with retention (governanceBypass=trues)',
      async function () {
        const deleted = await s3_owner
          .deleteObject({
            Bucket: BKT,
            Key: OBJ4,
            BypassGovernanceRetention: true,
  ...
  mocha.describe('overwrite versions and check', function () {
    ...
    mocha.it('should head object', async function () {
      //get object version2 = latest
      const conf = await s3_owner
        .headObject({ Bucket: BKT1, Key: OBJ2 })
        .promise();
      assert.ok(
        !conf.ObjectLockLegalHoldStatus &&
          !conf.ObjectLockRetainUntilDate &&
    ...
    mocha.it(
      'IT29. should not overwrite object with retention',
      async function () {
        const put_object_res = await s3_owner
          .putObject({
            // now version5 is latest
            Bucket: BKT1,
            Key: OBJ2,
            Body: file_body2,
            ContentType: 'text/plain',
    ...
    mocha.it(
      'should put retention (compliance mode) version_id',
      async function () {
        await s3_owner
          .putObjectRetention({
            Bucket: BKT1,
