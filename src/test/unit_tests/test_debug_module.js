...
function file_content_verify(flag, expected) {
  return P.delay(1000).then(function () {
    ...
    if (flag === 'text') {
      // Verify Log requests content
      assert(content.indexOf(expected) !== -1);
    } else if (flag === 'no_text') {
      // Verify Log request DOES NOT appear
      assert(content.indexOf(expected) === -1);
    }
  });
}
...
mocha.describe('debug_module', function () {
  ...
  mocha.it('should parse __filename', function () {
    ...
    const filename =
      __filename.indexOf('noobaa-util') >= 0 ?
        __filename
  ...
  mocha.it('should parse file names with stems and prefix', function () {
    const dbg = new DebugModule(
      '/web/noise/noobaa-core/src/blabla.asd/lll.asd',
    );
    assert.strictEqual(dbg._name, 'core.blabla.asd.lll');
  });
  ...
  mocha.it('should parse windows style paths', function () {
    const dbg = new DebugModule(
      'C:\\Program Files\\NooBaa\\src\\agent\\agent_cli.js',
    );
    assert.strictEqual(dbg._name, 'core.agent.agent_cli');
  });

  mocha.it(
    'should set level for windows style module and propogate',
    function () {
      const dbg = new DebugModule(
        'C:\\Program Files\\NooBaa\\src\\agent\\agent_cli.js',
      );
  ...
  mocha.it('should log when level is appropriate', function () {
    ...
    return os_utils.exec(rotation_command).then(function () {
      ...
      return file_content_verify(
        'text',
        'test_debug_module: log0 should appear in the log',
      );
    });
  });
  ...
  mocha.it('should NOT log when level is lower', function () {
    ...
    return file_content_verify(
      'no_text',
      'test_debug_module: log2 should not appear in the log',
    );
  });
  ...
  mocha.it('should log after changing level of module', function () {
    ...
    return file_content_verify(
      'text',
      'core.blabla.asd.lll:: test_debug_module: log4 should appear after level change',
    );
  });
  ...
  mocha.it('should log backtrace if asked to', function () {
    ...
    return file_content_verify(
      'text',
      'core.blabla.asd.lll:: test_debug_module: log0 should appear with backtrace     at',
    );
  });
  ...
  mocha.it('setting a higher module should affect sub module', function () {
    ...
    dbg.log2(
      'test_debug_module: log2 setting a higher level module level should affect current',
    );
    dbg.set_module_level(0, 'core');
    return file_content_verify(
      'text',
      'core.blabla.asd.lll:: test_debug_module: log2 setting a higher level module level should affect current',
    );
  });

  mocha.it(
    'formatted string should be logged correctly (string substitutions)',
    function () {
      const dbg = new DebugModule(
        '/web/noise/noobaa-core/src/blabla.asd/lll.asd',
      );
      ...
      const d2 = 2;
      dbg.log0(
        '%s string substitutions (%d) %s be logged as %s, with two (%d) numbers',
        s1,
        d1,
        s2,
        s3,
      ...
      return file_content_verify(
        'text',
        ' this string substitutions (3) should be logged as expected, with two (2) numbers',
      );
    },
  );
    ...
    return _.reduce(
      ...
      function (promise, l) {
        return promise.then(function () {
          ...
          return file_content_verify(
            'text',
            'CONSOLE:: console - ' + l + ' - should be captured',
          );
        });
      },
      P.resolve(),
  ...
  mocha.it('fake browser verify logging and console wrapping', function () {
    ...
    return file_content_verify(
      'text',
      'core.blabla.asd.lll:: test_debug_module: browser should appear in the log',
    );
  });
});
