  ...
  mocha.it('works', function () {
    ...
    return (
      P.resolve()
        ...
        .then(() =>
          rpc_client.node.read_node({
            name: nodes[0].name,
          }),
        )
        .then(res =>
          rpc_client.node.get_test_nodes({
            count: 10,
        ...
        .then(() =>
          rpc_client.node.set_debug_node({
            node: {
              name: nodes[0].name,
            },
        ...
        .then(() =>
          rpc_client.node.decommission_node({
            name: nodes[0].name,
          }),
        )
        .then(() =>
          rpc_client.node.recommission_node({
            name: nodes[0].name,
          }),
        )
        .then(() =>
          rpc_client.node.collect_agent_diagnostics({
            name: nodes[0].name,
          }),
        )
        .then(() => {
        ...
        // }))
        .then(() =>
          rpc_client.node.test_node_network({
            source: nodes[0].rpc_address,
