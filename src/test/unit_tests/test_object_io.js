...
coretest.describe_mapper_test_case(
  ...
  ({
    ...
    if (data_placement !== 'SPREAD' || num_pools !== 1 || total_blocks > 10) {
      return;
    }
    ...
    async function multipart_upload_and_verify(part_size, num_parts) {
      ...
      await Promise.all(
        _.times(num_parts, i =>
          upload_multipart(i, generator.update(Buffer.alloc(part_size))),
        ),
      );
    ...
    async function verify_read_mappings(key, size) {
      ...
      const { chunks } = await rpc_client.object.read_object_mapping_admin({
        bucket,
        key,
      });
      let pos = 0;
