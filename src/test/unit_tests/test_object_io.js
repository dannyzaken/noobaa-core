...
coretest.describe_mapper_test_case(
    ...
    ({
        ...
        let big_loops = 0;
        if (
            (replicas === 3 && data_frags === 1 && parity_frags === 0) ||
        ...
        mocha.it('upload_and_verify', async function () {
            ...
            for (let i = 0; i < medium_loops; ++i) {
                await upload_and_verify(4015);
            }
        ...
        mocha.it('multipart_upload_and_verify', async function () {
            this.timeout(600000); // eslint-disable-line no-invalid-this
            for (let i = 0; i < small_loops; ++i) {
                await multipart_upload_and_verify(45, 7);
            }
            for (let i = 0; i < medium_loops; ++i) {
                await multipart_upload_and_verify(3245, 5);
            }
            for (let i = 0; i < big_loops; ++i) {
                await multipart_upload_and_verify(7924, 3);
            }
        });
         ...
         */
        mocha.it(
            'range_part_upload_and_verify_range_in_cache',
            async function () {
                this.timeout(600000); // eslint-disable-line no-invalid-this
         ...
         */
        mocha.it(
            'range_part_upload_and_verify_partial_range_in_cache_start',
            async function () {
                this.timeout(600000); // eslint-disable-line no-invalid-this
                await range_parts_upload_and_verify({
                    cached_parts: [
         ...
         */
        mocha.it(
            'range_part_upload_and_verify_partial_range_in_cache_middle_start',
            async function () {
                this.timeout(600000); // eslint-disable-line no-invalid-this
                await range_parts_upload_and_verify({
                    cached_parts: [
         ...
         */
        mocha.it(
            'range_part_upload_and_verify_partial_range_in_cache_middle_end',
            async function () {
                this.timeout(600000); // eslint-disable-line no-invalid-this
                await range_parts_upload_and_verify({
                    cached_parts: [
         ...
         */
        mocha.it(
            'range_part_upload_and_verify_partial_range_in_cache_end',
            async function () {
                this.timeout(600000); // eslint-disable-line no-invalid-this
                await range_parts_upload_and_verify({
                    cached_parts: [
         ...
         */
        mocha.it(
            'range_part_upload_and_verify_partial_range_covering_cache_part',
            async function () {
                this.timeout(600000); // eslint-disable-line no-invalid-this
                await range_parts_upload_and_verify({
                    cached_parts: [
         ...
         */
        mocha.it(
            'range_part_upload_and_verify_range_span_multiple_cached_parts_1',
            async function () {
                this.timeout(600000); // eslint-disable-line no-invalid-this
                await range_parts_upload_and_verify({
                    cached_parts: [
         ...
         */
        mocha.it(
            'range_part_upload_and_verify_range_span_multiple_cached_parts_2',
            async function () {
                this.timeout(600000); // eslint-disable-line no-invalid-this
                await range_parts_upload_and_verify({
                    cached_parts: [
         ...
         */
        mocha.it(
            'range_part_upload_and_verify_range_span_multiple_cached_parts_3',
            async function () {
                this.timeout(600000); // eslint-disable-line no-invalid-this
                await range_parts_upload_and_verify({
                    cached_parts: [
         ...
         */
        mocha.it(
            'range_part_upload_and_verify_range_not_in_cached_parts_at_start',
            async function () {
                this.timeout(600000); // eslint-disable-line no-invalid-this
                await range_parts_upload_and_verify({
                    cached_parts: [
         ...
         */
        mocha.it(
            'range_part_upload_and_verify_range_not_in_cached_parts_middle',
            async function () {
                this.timeout(600000); // eslint-disable-line no-invalid-this
                await range_parts_upload_and_verify({
                    cached_parts: [
         ...
         */
        mocha.it(
            'range_part_upload_and_verify_range_not_in_cached_parts_at_end',
            async function () {
                this.timeout(600000); // eslint-disable-line no-invalid-this
                await range_parts_upload_and_verify({
                    cached_parts: [
        ...
        mocha.it(
            'range_part_upload_and_verify_0_start_range_no_cached_part',
            async function () {
                this.timeout(600000); // eslint-disable-line no-invalid-this
                await range_parts_upload_and_verify({
                    cached_parts: [],
        ...
        mocha.it(
            'range_part_upload_and_verify_range_middle_no_cached_part',
            async function () {
                this.timeout(600000); // eslint-disable-line no-invalid-this
                await range_parts_upload_and_verify({
                    cached_parts: [],
        ...
        async function multipart_upload_and_verify(part_size, num_parts) {
            ...
            const mp_list_before = await rpc_client.object.list_multiparts({
                obj_id,
                bucket,
                key,
            });
            ...
            const get_part_slice = i =>
                data.slice(i * part_size, (i + 1) * part_size);
            ...
            await Promise.all(
                _.times(num_parts, i =>
                    upload_multipart(
                        i,
                        generator.update(Buffer.alloc(part_size)),
                        4,
                        'fail',
                    ).catch(err => ({ err })),
                ),
            ...
            const multiparts = await Promise.all(
                _.times(num_parts, i => upload_multipart(i, get_part_slice(i))),
            );
            ...
            const mp_list_after = await rpc_client.object.list_multiparts({
                obj_id,
                bucket,
                key,
            });
            ...
            await rpc_client.object.complete_object_upload({
                ...
                bucket,
                key,
                multiparts: multiparts.map((mp, i) => ({
                    num: i + 1,
                    etag: mp.etag,
                })),
            });
            ...
            await rpc_client.object.delete_object({ bucket, key });
            coretest.log(
                'multipart_upload_and_verify: OK',
                part_size,
                num_parts,
            );
        }
        ...
        async function verify_read_mappings(key, size) {
            ...
            for (const chunk of chunks) {
                ...
                const blocks_per_node = _.groupBy(
                    part_blocks,
                    block => block.adminfo.node_name,
                );
        ...
        async function verify_range_read_mappings(key, cached_parts) {
            ...
            try {
                ...
                const tot_cached_size = _.sumBy(
                    cached_parts,
                    p => p.data.length,
                );
                const tot_chunk_size = _.sumBy(
                    chunks,
                    p => p.parts[0].end - p.parts[0].start,
                );
        ...
        async function verify_read_range_data(
            key,
            obj_id,
            cached_parts,
            read_range,
            ...
            const { buffer, generated_missing_parts } = create_missing_parts(
                read_range.start,
                read_range.end,
                cached_parts,
            );
            coretest.log('verify_read_range_data: generated missing parts:', {
                key,
                generated_missing_parts,
            });

            async function missing_part_getter(
                missing_part_start,
                missing_part_end,
            ) {
                const part = generated_missing_parts[num_missing_parts_pulled];
                console.log('missing_part_getter invoked:', {
                    key,
                    num_missing_parts_pulled,
                    part,
                    missing_part_start,
                    missing_part_end,
                });
            ...
            assert.strictEqual(
                num_missing_parts_pulled,
                generated_missing_parts.length,
            );
            ...
            for (let i = 0; i < buffer.length; i++) {
                assert.strictEqual(
                    buffer[i],
                    read_buf[i],
                    `verify_read_range_data: mismatch data at pos ${i}`,
                );
            }
        }
        ...
        function create_missing_parts(start, end, cached_parts) {
            console.log('create_missing_parts: start', {
                start,
                end,
                cached_parts: _.omit(cached_parts, 'data'),
            });
            ...
            for (const part of cached_parts) {
                ...
                console.log('create_missing_parts:', {
                    part: _.omit(part, 'data'),
                    part_range,
                    pos,
                });
                if (!part_range) {
                ...
                if (pos < part_range.start) {
                    const data = generator.update(
                        Buffer.alloc(part_range.start - pos),
                    );
                    buffers.push(data);
                    generated_missing_parts.push({
                        start: pos,
                        end: part_range.start,
                        data,
                    });
                }
            ...
            return new stream.Readable({
                read() {
                    if (pos < data.length) {
                        ...
                        this.emit(
                            'error',
                            new Error('TEST_OBJECT_IO FAIL ON FINISH'),
                        );
                    } else {
                        this.push(null);
                    }
