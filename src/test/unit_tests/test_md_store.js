...
mocha.describe('md_store', function () {
    ...
    mocha.describe('objects', function () {
        mocha.it('insert/update/find_object()', async function () {
            ...
            await md_store.update_object_by_id(
                info._id,
                { size: 777 },
                { upload_size: 1 },
                { num_parts: 88 },
            ...
            await md_store.update_object_by_id(info._id, {
                deleted: new Date(),
            });
            obj = await md_store.find_object_latest(bucket_id, info.key);
            assert_equal(obj, null);

            const res = await md_store.populate_objects(
                { obj: info._id },
                'obj',
                { key: 1, num_parts: 1 },
            );
        ...
        mocha.it(
            'insert_object() detects missing key (bad schema)',
            async function () {
                const info = {
                    _id: md_store.make_md_id(),
                    system: system_id,
                ...
                assert(
                    err_message.startsWith('INVALID_SCHEMA_DB'),
                    err_message,
                );
            },
        );
        ...
        mocha.it('aggregate_objects_by_create_dates()', async function () {
            ...
            return md_store.aggregate_objects_by_create_dates(
                from_time,
                till_time,
            );
        });
        ...
        mocha.it('aggregate_objects_by_delete_dates()', async function () {
            const till_time = Date.now();
            const from_time = till_time - 24 * 3600 * 1000;
            return md_store.aggregate_objects_by_delete_dates(
                from_time,
                till_time,
            );
        });
    ...
    mocha.describe('chunks', function () {
        ...
        mocha.it('update_chunk_by_id()', async function () {
            await md_store.update_chunk_by_id(chunks[0]._id, {
                bucket: bucket_id,
            });
        ...
        mocha.it('find_chunks_by_ids()', async function () {
            ...
            if (config.DB_TYPE === 'mongodb') {
                res.forEach(chunk => {
                    ...
                    if (chunk.cipher_key) {
                        chunk.cipher_key = chunk.cipher_key.buffer;
                    }
                    if (chunk.cipher_iv) {
                        chunk.cipher_iv = chunk.cipher_iv.buffer;
                    }
                    if (chunk.cipher_auth_tag) {
                        chunk.cipher_auth_tag = chunk.cipher_auth_tag.buffer;
                    }
                });
            }
    ...
    mocha.describe('blocks', function () {
        ...
        mocha.it('delete_blocks_by_ids()', async function () {
            return md_store.delete_blocks_by_ids(
                blocks.map(block => block._id),
            );
        });
    });
