...
function assert_date(date, from, expected, tz = 'LOCAL') {
  ...
  if (tz === 'UTC') {
    ...
    assert(
      date.getHours() === that_if_not_this(expected.hour, from.getHours()),
    );
    assert(
      date.getMinutes() === that_if_not_this(expected.min, from.getMinutes()),
    );
    assert(
      date.getSeconds() === that_if_not_this(expected.sec, from.getSeconds()),
    );
  }
}
...
mocha.describe('nsfs_glacier', async () => {
  ...
  mocha.describe('nsfs_glacier_tapecloud', async () => {
    ...
    mocha.it('restore-object should successfully restore', async () => {
      ...
      assert(
        expected_expiry.getTime() >= md.restore_status.expiry_time.getTime(),
      );
