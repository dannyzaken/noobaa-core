...
const {
    ReplicationScanner,
} = require('../../server/bg_services/replication_scanner');
...
mocha.describe('replication configuration validity tests', function () {
    ...
    mocha.after('delete buckets', async function () {
        ...
        await rpc_client.bucket.delete_bucket_replication({
            name: first_bucket,
        });
    });

    mocha.it(
        '_put_replication - same dst bucket & no prefixes - should fail',
        async function () {
            await _put_replication(
                bucket1,
                [
                    {
                        rule_id: 'rule-1',
                        destination_bucket: first_bucket,
                        sync_versions: false,
                    },
                    {
                        rule_id: 'rule-2',
    ...
    mocha.it(
        '_put_replication - same dst bucket & 1 prefix - should fail ',
        async function () {
            await _put_replication(
                bucket1,
                [
    ...
    mocha.it(
        '_put_replication - 2 replication rules same rule_id - should fail',
        async function () {
            await _put_replication(
                bucket1,
                [
    ...
    mocha.it(
        '_put_replication - same dst bucket & prefix is a prefix of prefix 1 - should fail',
        async function () {
            await _put_replication(
                bucket2,
                [
    ...
    mocha.it(
        '_put_replication - same dst bucket & prefix is a prefix of prefix 2 - should fail',
        async function () {
            await _put_replication(
                bucket2,
                [
    ...
    mocha.it(
        '_put_replication - bidirectional replication for matching prefix - should fail',
        async function () {
            await _put_replication(
                bucket1,
                [
    ...
    mocha.it(
        '_put_replication - bidirectional replication for matching prefix subset 1 - should fail',
        async function () {
            await _put_replication(
                bucket1,
                [
    ...
    mocha.it(
        '_put_replication - bidirectional replication for matching prefix subset 2 - should fail',
        async function () {
            await _put_replication(
                bucket1,
                [
    ...
    mocha.it(
        '_put_replication - bidirectional replication for no prefix - should fail',
        async function () {
            await _put_replication(
                bucket1,
                [
    ...
    mocha.it('_put_replication 3 - valid', async function () {
        ...
        const res = await rpc_client.bucket.get_bucket_replication({
            name: bucket2,
        });

        assert.ok(
            res.rules[0].rule_id === 'rule-1' &&
                res.rules[0].destination_bucket.unwrap() === bucket1 &&
                res.rules[0].filter.prefix === '',
        );
        assert.ok(
            res.rules[1].rule_id === 'rule-2' &&
                res.rules[1].destination_bucket.unwrap() === first_bucket &&
                res.rules[1].filter.prefix === 'ab',
        );
    });
    ...
    mocha.it('_put_replication 4 - valid', async function () {
        ...
        assert.ok(
            res.rules[0].rule_id === 'rule-1' &&
                res.rules[0].destination_bucket.unwrap() === first_bucket &&
                res.rules[0].filter.prefix === '',
        );
    });
    ...
    mocha.it(
        '_put_replication - same rule_id - should fail ',
        async function () {
            await _put_replication(
                bucket1,
                [
    ...
    mocha.it('delete bucket replication ', async function () {
        ...
        try {
            ...
            assert.fail(
                "found a replication policy on the bucket but it shouldn't",
            );
        } catch (err) {
...
mocha.describe('replication collection tests', function () {
    ...
    const replication_store =
        require('../../server/system_services/replication_store').instance();
    ...
    mocha.it('_put_replication on both buckets 1', async function () {
        await P.all(
            _.map(buckets, async bucket_name => {
                ...
                assert.ok(
                    res.rules[0].rule_id === 'rule-1' &&
                        res.rules[0].destination_bucket.unwrap() ===
                            first_bucket,
                );
            }),
        );
    ...
    mocha.it('delete bucket replication from bucket1', async function () {
        ...
        const repl_buck2 = await rpc_client.bucket.get_bucket_replication({
            name: bucket2,
        });
        assert.ok(
            repl_buck2.rules[0].rule_id === 'rule-1' &&
                repl_buck2.rules[0].destination_bucket.unwrap() ===
                    first_bucket,
        );
        replication_policy_id = (
            await rpc_client.bucket.read_bucket({ name: bucket2 })
        ).replication_policy_id;
        try {
            const replication = await replication_store.get_replication_by_id(
                replication_policy_id,
            );
    ...
    mocha.it(
        'delete bucket replication from bucket2 - check replication still exists in db',
        async function () {
            await rpc_client.bucket.delete_bucket_replication({
                name: bucket2,
            });
            try {
                await rpc_client.bucket.get_bucket_replication({
                    name: bucket2,
                });
                assert.fail(
                    `get_bucket_replication succeeded but it should fail`,
                );
            } catch (err) {
                assert.ok(err.rpc_code === 'NO_REPLICATION_ON_BUCKET');
            }
    ...
    mocha.it('update bucket replication of bucket1', async function () {
        ...
        const repl_buck1 = await rpc_client.bucket.get_bucket_replication({
            name: bucket1,
        });
        const repl_buck2 = await rpc_client.bucket.get_bucket_replication({
            name: bucket2,
        ...
        const repl1 = (await rpc_client.bucket.read_bucket({ name: bucket1 }))
            .replication_policy_id;
        const repl2 = (await rpc_client.bucket.read_bucket({ name: bucket2 }))
            .replication_policy_id;
    ...
    mocha.it(
        'delete bucket3 - check that replication was delete from db ',
        async function () {
            const replication_id = (
                await rpc_client.bucket.read_bucket({ name: bucket3 })
            ).replication_policy_id;

            await rpc_client.bucket.delete_bucket({ name: bucket3 });
            const ans = await db_client
                .collection('replicationconfigs')
                .findOne({ _id: db_client.parse_object_id(replication_id) });
...
mocha.describe('replication configuration bg worker tests', function () {
    ...
    mocha.before('init scanner & populate buckets', async function () {
        ...
        const admin_account = await rpc_client.account.read_account({
            email: EMAIL,
        });
        ...
        cloud_utils.set_noobaa_s3_connection = () => {
            console.log(
                'setting connection to coretest endpoint and access key',
            );
            return s3_owner;
        };
        // init scanner
    ...
    mocha.it(
        'run replication scanner and wait - no replication - nothing to upload',
        async function () {
            const res1 = await scanner.run_batch();
            console.log(
                'waiting for replication objects no objects to upload',
                res1,
            );
    ...
    mocha.it('run replication scanner and delete bucket', async function () {
        await _put_replication(
            ...
            [
                {
                    rule_id: 'rule-1',
                    destination_bucket: bucket_to_delete,
                    sync_versions: false,
                    filter: { prefix: bucket1 },
                },
            ],
            false,
        );
    ...
    mocha.it(
        'run replication scanner and wait - prefix - prefixed objects should be uploaded',
        async function () {
            await _put_replication(
                bucket1,
                [
                    {
                        rule_id: 'rule-1',
                        destination_bucket: bucket_for_replications,
                        sync_versions: false,
                        filter: { prefix: 'pref' },
                    },
                ],
            ...
            console.log(
                'waiting for replication objects - one rule one prefix',
                res1,
            );
            let contents = await _list_objects_and_wait(
                s3_owner,
                bucket_for_replications,
                5,
            ); //Check that the 5 objects were replicated
            console.log('contents', contents);

            // delete object from dst
            await s3_owner
                .deleteObject({
                    Bucket: bucket_for_replications,
                    Key: contents[0].Key,
                })
                .promise();
            ...
            contents = await _list_objects_and_wait(
                s3_owner,
                bucket_for_replications,
                5,
            ); //Check that the delete object was replicate again
            const key1 = contents[0].Key;
            // override object in dst
            const dst_obj1 = await s3_owner
                .getObject({ Bucket: bucket_for_replications, Key: key1 })
                .promise();
            await s3_owner
                .putObject({
                    Bucket: bucket_for_replications,
            ...
            console.log(
                ...
                dst_obj1,
                dst_obj2,
                dst_obj1.Body.toString(),
                dst_obj2.Body.toString(),
            );
            ...
            assert.notDeepStrictEqual(
                dst_obj1.Body.toString(),
                dst_obj2.Body.toString(),
            ); // new dst object was updated and
            ...
            const dst_obj3 = await s3_owner
                .getObject({ Bucket: bucket_for_replications, Key: key1 })
                .promise();
            const src_obj = await s3_owner
                .getObject({ Bucket: bucket1, Key: key1 })
                .promise();
            ...
            console.log(
                ...
                src_obj,
                dst_obj3,
                src_obj.Body.toString(),
                dst_obj3.Body.toString(),
            );
            assert.notDeepStrictEqual(
                src_obj.Body.toString(),
                dst_obj3.Body.toString(),
            ); // object in src !== object in dst
            ...
            const obj_src_before_override = await s3_owner
                .getObject({ Bucket: bucket1, Key: key1 })
                .promise();
            await s3_owner
                .putObject({ Bucket: bucket1, Key: key1, Body: 'lalalala1' })
                .promise();

            // sync again - should replicate since src bucket is last modified
            res1 = await scanner.run_batch();
            console.log(
                'waiting for replication objects - one rule one prefix 1',
                res1,
            );
            const obj_dst_after_repl = await s3_owner
                .getObject({ Bucket: bucket_for_replications, Key: key1 })
                .promise();
            const obj_src_after_override = await s3_owner
                .getObject({ Bucket: bucket1, Key: key1 })
                .promise();

            console.log(
                'objs override src obj_src_before_override:',
                obj_src_before_override,
                obj_src_after_override,
                obj_dst_after_repl,
                obj_src_before_override.Body.toString(),
                obj_dst_after_repl.Body.toString(),
                obj_src_after_override.Body.toString(),
            );
            ...
            const obj_src_before_override_md = await s3_owner
                .getObject({ Bucket: bucket1, Key: key1 })
                .promise();
            await s3_owner
                .putObject({
                ...
                .promise();
            const obj_dst_before_repl_md = await s3_owner
                .getObject({ Bucket: bucket_for_replications, Key: key1 })
                .promise();
            ...
            res1 = await scanner.run_batch();
            console.log(
                'waiting for replication objects - one rule one prefix 2',
                res1,
            );
            const obj_dst_after_repl_md = await s3_owner
                .getObject({ Bucket: bucket_for_replications, Key: key1 })
                .promise();
            const obj_src_after_override_md = await s3_owner
                .getObject({ Bucket: bucket1, Key: key1 })
                .promise();

            console.log(
                'objs override src obj_src_before_override1:',
                obj_src_before_override_md,
                obj_src_after_override_md,
                obj_dst_after_repl_md,
                obj_src_before_override_md.Body.toString(),
                obj_dst_after_repl_md.Body.toString(),
                obj_src_after_override_md.Body.toString(),
            );
            ...
            assert.deepStrictEqual(obj_src_after_override_md.Metadata, {
                key1: 'val1',
            });
            assert.deepStrictEqual(
                obj_src_after_override_md.Body.toString(),
            ...
            assert.notDeepStrictEqual(
                obj_dst_before_repl_md.Metadata,
                obj_dst_after_repl_md.Metadata,
            );
            assert.deepStrictEqual(
                obj_src_after_override_md.Metadata,
                obj_dst_after_repl_md.Metadata,
            );
        },
    );
    ...
    mocha.it(
        'run replication scanner and wait - no prefix - all objects should be uploaded',
        async function () {
            const contents = await _list_objects_and_wait(
                s3_owner,
                bucket_for_replications,
                5,
            );
            for (const content of contents) {
                ...
                await s3_owner
                    .deleteObject({ Bucket: bucket_for_replications, Key: key })
                    .promise();
            }
            await _put_replication(
                bucket1,
            ...
            const res1 = await scanner.run_batch();
            console.log(
                'waiting for replication objects - one rule no prefix',
                res1,
            );
    ...
    mocha.it(
        'run replication scanner and wait - 2 prefixes - all objects should be uploaded',
        async function () {
            await _put_replication(
                bucket1,
                [
            ...
            console.log(
                'waiting for replication objects original bucket ',
                res,
            );
            let res1 = await scanner.run_batch();
            console.log(
                'waiting for replication objects - 2 rules 1 prefix1 ',
                res1,
            );
            await _list_objects_and_wait(s3_owner, bucket2, 5);
            res1 = await scanner.run_batch();
            console.log(
                'waiting for replication objects - 2 rules 1 prefix2 ',
                res1,
            );
    ...
    mocha.it(
        'run replication scanner and wait - 2 buckets - all objects should be uploaded',
        async function () {
            await _put_replication(
                bucket1,
                [
...
async function put_object(s3_owner, bucket_name, key, optional_body) {
    const res = await s3_owner
        .putObject({
            Bucket: bucket_name,
            Key: key,
            Body: optional_body || create_random_body(),
        })
        .promise();
...
async function _list_objects_and_wait(
    s3_owner,
    bucket,
    expected_num_of_objects,
) {
    let res;
    for (let retries = 1; retries <= 3; retries++) {
        try {
            ...
            console.log(
                `list_objects contents: expected: ${expected_num_of_objects} actual: ${res.length} ${res}`,
            );
            return res;
        } catch (e) {
            console.log(
                `waiting for replications of bucket: ${bucket}, response: ${res} `,
            ); //num of retries: ${retries}`);
...
mocha.describe('Replication pagination test', function () {
    ...
    mocha.before('init scanner & populate buckets', async function () {
        ...
        await _put_replication(
            ...
            [
                {
                    rule_id: '11obj-replication-rule',
    ...
    // Pagination test
    mocha.it(
        'Testing bucket replication pagination with 11 objects',
        async function () {
            // Copy the first 6
