...
mocha.describe('namespace_fs', function () {
    ...
    mocha.before(async () => {
        await P.all(
            _.map([src_bkt, upload_bkt, mpu_bkt], async buck =>
    ...
    mocha.it(
        'read_object_md succeed on directory head - not directory object - should fail',
        async function () {
            try {
                const res = await ns_src.read_object_md(
                    {
    ...
    mocha.describe('upload_object', function () {
        ...
        mocha.it('upload, read, delete of a small object', async function () {
            ...
            if (
                ...
                ns_tmp.force_md5_etag ||
                dummy_object_sdk.requesting_account.force_md5_etag
            ) {
                xattr[XATTR_MD5_KEY] = upload_res.etag;
            }
    ...
    mocha.describe('multipart upload', function () {
        ...
        mocha.it(
            'upload, read, delete a small multipart object',
            async function () {
                ...
                for (let i = 0; i < num_parts; ++i) {
                    ...
                    const part_res = await ns_tmp.upload_multipart(
                        {
                            ...
                            num: i + 1,
                            source_stream:
                                buffer_utils.buffer_to_read_stream(data_part),
                        },
                        dummy_object_sdk,
                    );
                    ...
                    console.log(
                        'list_multiparts response',
                        inspect(list_parts_res),
                    );
                }
                ...
                console.log(
                    'complete_object_upload response',
                    inspect(complete_res),
                );
                if (
                    config.NSFS_CALCULATE_MD5 ||
                    ns_tmp.force_md5_etag ||
                    ...
                    xattr[XATTR_MD5_KEY] = complete_res.etag;
                }
    ...
    mocha.describe('list multipart upload', function () {
        ...
        mocha.after(async function () {
            await ns_tmp.abort_object_upload(
                { bucket: mpu_bkt, obj_id: create_res1.obj_id },
                dummy_object_sdk,
            );
            await ns_tmp.abort_object_upload(
                { bucket: mpu_bkt, obj_id: create_res2.obj_id },
                dummy_object_sdk,
            );
            await ns_tmp.abort_object_upload(
                { bucket: mpu_bkt, obj_id: create_res3.obj_id },
                dummy_object_sdk,
            );
        });

        mocha.it(
            'multipartUploadList without prefix or delimiter',
            async function () {
                const res = await ns_tmp.list_uploads(
                    { bucket: mpu_bkt },
                    dummy_object_sdk,
                );
        ...
        mocha.it(
            'multipartUploadList with prefix and delimiter',
            async function () {
                const res = await ns_tmp.list_uploads(
                    { bucket: mpu_bkt, prefix, delimiter },
                    dummy_object_sdk,
                );
    ...
    mocha.describe('delete_object', function () {
        ...
        mocha.before(async function () {
            ...
            console.log(
                'upload_object with path (before) response',
                inspect(upload_res),
            );
        });
        ...
        mocha.it('do not delete the path', async function () {
            ...
            let entries;
            try {
                entries = await nb_native().fs.readdir(
                    DEFAULT_FS_CONFIG,
                    ns_tmp_bucket_path + dir_1,
                );
            } catch (e) {
                assert.ifError(e);
            }
        ...
        mocha.after(async function () {
            ...
            let entries_after;
            try {
                entries_before = await nb_native().fs.readdir(
                    DEFAULT_FS_CONFIG,
                    ns_tmp_bucket_path,
                );
                ...
                entries_after = await nb_native().fs.readdir(
                    DEFAULT_FS_CONFIG,
                    ns_tmp_bucket_path,
                );
            } catch (e) {
    ...
    mocha.describe('key with trailing /', function () {
        ...
        mocha.before(async function () {
            ...
            console.log(
                'upload_object with trailing / response',
                inspect(upload_res),
            );
        });

        mocha.it(
            `delete the path - stop when not empty and key with trailing /`,
            async function () {
                const upload_res = await ns_tmp.upload_object(
                    {
                        bucket: upload_bkt,
                ...
                console.log(
                    'upload_object with trailing / (key 2) response',
                    inspect(upload_res),
                );
                ...
                console.log(
                    'delete_object with trailing / response',
                    inspect(delete_res),
                );
            },
        );
        ...
        mocha.after(async function () {
            ...
            console.log(
                'delete_object with trailing / (key 2) response',
                inspect(delete_res),
            );
        });
    });
    ...
    mocha.describe('restore_object', function () {
        ...
        mocha.describe('GLACIER storage class not supported', function () {
            ...
            mocha.it('fails to issue restore-object', async function () {
                try {
                    ...
                    assert.strictEqual(
                        err.code,
                        S3Error.InvalidStorageClass.code,
                    );
                    assert.strictEqual(
                        err.message,
                        S3Error.InvalidStorageClass.message,
                    );
                    assert.strictEqual(
                        err.http_code,
                        S3Error.InvalidStorageClass.http_code,
                    );
                }
            });
        ...
        mocha.describe('GLACIER storage class supported', function () {
            ...
            mocha.it(
                'fails when the uploaded object is not in GLACIER storage class',
                async function () {
                    try {
                        await ns_tmp.restore_object(
                            {
                        ...
                        assert.strictEqual(
                            err.code,
                            S3Error.InvalidObjectStorageClass.code,
                        );
                        assert.strictEqual(
                            err.message,
                            S3Error.InvalidObjectStorageClass.message,
                        );
                        assert.strictEqual(
                            err.http_code,
                            S3Error.InvalidObjectStorageClass.http_code,
                        );
                    }
                },
            );
            ...
            mocha.it(
                'succeeds when object is in GLACIER storage class',
                async function () {
                    ...
                    assert.strictEqual(
                        restore_objectmd.storage_class,
                        s3_utils.STORAGE_CLASS_GLACIER,
                    );
                    assert.strictEqual(
                        restore_objectmd.restore_status.ongoing,
                        true,
                    );
                    assert.strictEqual(
                        restore_objectmd.restore_status.expiry_time,
                        undefined,
                    );

                    // Don't leak the xattrs to the user
                    assert.strictEqual(
                        restore_objectmd.xattr['user.noobaa.restore.request'],
                        undefined,
                    );
                    assert.strictEqual(
                        restore_objectmd.xattr['user.noobaa.restore.expiry'],
                        undefined,
                    );

                    const xattr = await get_xattr(
                        path.join(ns_tmp_bucket_path, temp_restore_key),
                    );
                    assert.strictEqual(
                        xattr['user.noobaa.restore.request'],
                        '1',
                    );
                    assert.strictEqual(
                        xattr['user.noobaa.restore.expiry'],
                        undefined,
                    );
                    ...
                    try {
                        ...
                        assert.strictEqual(
                            err.code,
                            S3Error.RestoreAlreadyInProgress.code,
                        );
                        assert.strictEqual(
                            err.message,
                            S3Error.RestoreAlreadyInProgress.message,
                        );
                        assert.strictEqual(
                            err.http_code,
                            S3Error.RestoreAlreadyInProgress.http_code,
                        );
                    }
                },
            );
            ...
            mocha.after(async function () {
                ...
                ns_tmp._is_storage_class_supported =
                    NamespaceFS.prototype._is_storage_class_supported;
            });
        });
    });
});

const add_user_prefix = user_xattr =>
    _.mapKeys(user_xattr, (val, key) => 'user.' + key);
...
mocha.describe('namespace_fs folders tests', function () {
    ...
    mocha.describe('folders xattr', function () {
        ...
        mocha.before(async function () {
            ...
            const full_xattr = await get_xattr(
                ns_tmp_bucket_path + '/' + upload_key_1,
            );
            ...
            const full_xattr2 = await get_xattr(
                ns_tmp_bucket_path + '/' + upload_key_2,
            );
            ...
            await ns_tmp.upload_object(
                {
                    ...
                    xattr: md,
                    source_stream:
                        buffer_utils.buffer_to_read_stream(undefined),
                    size: obj_sizes_map[upload_key_3],
                },
                dummy_object_sdk,
            );
            const full_xattr3 = await get_xattr(
                ns_tmp_bucket_path + '/' + upload_key_3,
            );
            ...
            const full_xattr4 = await get_xattr(
                ns_tmp_bucket_path + '/' + upload_key_4_full,
            );
            ...
            await P.all(
                Object.keys(mpu_keys_and_size_map).map(async key => {
                    const mpu_upload_id1 = await ns_tmp.create_object_upload(
                        {
                            bucket: upload_bkt,
                            key: key,
                            xattr:
                                mpu_keys_and_size_map[key] > 0 ? md : undefined,
                        },
                        dummy_object_sdk,
                    );
                    ...
                    const put_part_res = await ns_tmp.upload_multipart(
                        {
                            ...
                            key: key,
                            num: 1,
                            source_stream: buffer_utils.buffer_to_read_stream(
                                mpu_keys_and_size_map[key] > 0 ?
                                    data
                                :   undefined,
                            ),
        ...
        mocha.it(
            `read folder object md - key doesn't end with /`,
            async function () {
                try {
        ...
        mocha.it(
            `override object & read folder object md full md`,
            async function () {
                const read_res = buffer_utils.write_stream();
                await ns_tmp.read_object_stream(
                    {
                ...
                if (Object.keys(not_user_xattr).length) {
                    await set_xattr(
                        DEFAULT_FS_CONFIG,
                        ns_tmp_bucket_path + '/' + upload_key_2,
                        not_user_xattr,
                ...
                assert.ok(
                    Object.keys(md1).every(
                        md_cur => get_md_res.xattr[md_cur] !== undefined,
                    ),
                );
                const full_xattr2 = await get_xattr(
                    ns_tmp_bucket_path + '/' + upload_key_2,
                ...
                assert.deepEqual(full_xattr2, {
                    ...user_md1_and_dir_content_xattr,
                    [XATTR_DIR_CONTENT]: new_size,
                    ...not_user_xattr,
                });
        ...
        mocha.it(
            `.folder of dir object of content of size > 0 - should exist`,
            async function () {
                const p = path.join(
                    ns_tmp_bucket_path,
                    upload_key_1,
        ...
        mocha.it(
            `read folder object - 0 content - should return empty file`,
            async function () {
                const read_res = buffer_utils.write_stream();
                await ns_tmp.read_object_stream(
                    {
        ...
        mocha.it(
            `.folder of dir object of content of size 0 - should not exist`,
            async function () {
                const p = path.join(
                    ns_tmp_bucket_path,
                    upload_key_3,
        ...
        mocha.it(
            `read folder object > 0 content - should return size 100`,
            async function () {
                ...
                assert.equal(
                    res.objects.find(obj => obj.key === upload_key_1).size,
                    100,
                );
            },
        );
        ...
        mocha.it(
            `read folder object = 0 content - should return size 0`,
            async function () {
                const res = await ns_tmp.list_objects(
                    {
                        bucket: upload_bkt,
                ...
                assert.equal(
                    res.objects.find(obj => obj.key === upload_key_3).size,
                    0,
                );
            },
        );
        ...
        mocha.it(
            `read folder object > 0 content - should return data`,
            async function () {
                const read_res = buffer_utils.write_stream();
                await ns_tmp.read_object_stream(
                    {
        ...
        mocha.it(
            `.folder of non directory object - should not exist`,
            async function () {
                const p = path.join(
                    ns_tmp_bucket_path,
                    a,
        ...
        mocha.it(
            `get folder object md - should fail, not a directory object`,
            async function () {
                try {
                    await ns_tmp.read_object_md(
                        {
                    ...
                    assert.fail(
                        'get object should have failed with no such key',
                    );
                } catch (err) {
                    assert.equal(err.code, 'ENOENT');
                }
        ...
        mocha.it(`put /a/b/c folder object md exists`, async function () {
            ...
            assert.ok(
                full_xattr2[XATTR_DIR_CONTENT] !== undefined &&
                    full_xattr2[XATTR_DIR_CONTENT] === '0',
            );
            ...
            await fs_utils.file_must_not_exist(
                path.join(dir_path, config.NSFS_FOLDER_OBJECT_NAME),
            );
        });
        ...
        mocha.it(`list objects md with delimiter & prefix`, async function () {
            ...
            assert.deepEqual(ls_obj_res.common_prefixes, [
                upload_key_2 + upload_key_4,
            ]);
            assert.deepEqual(
                ls_obj_res.objects.map(obj => obj.key),
                [upload_key_2],
            );
        });
        ...
        mocha.it(
            `list objects md key marker 1 - dir content`,
            async function () {
                const ls_obj_res = await ns_tmp.list_objects(
                    { bucket: upload_bkt, key_marker: dir_1 },
                    dummy_object_sdk,
                );
                console.log(
                    'list objects md key marker 1',
                    inspect(ls_obj_res),
                );
                assert.deepEqual(
                    ls_obj_res.objects.map(obj => obj.key),
        ...
        mocha.it(`list objects md key marker 2`, async function () {
            const ls_obj_res = await ns_tmp.list_objects(
                { bucket: upload_bkt, key_marker: 'a/b/c' },
                dummy_object_sdk,
            );
        ...
        mocha.it(`list objects md prefix 1`, async function () {
            const ls_obj_res = await ns_tmp.list_objects(
                { bucket: upload_bkt, prefix: dir_1, delimiter: '/' },
                dummy_object_sdk,
            );
        ...
        mocha.it(`list objects md prefix 2`, async function () {
            const ls_obj_res = await ns_tmp.list_objects(
                { bucket: upload_bkt, prefix: 'a/b/c', delimiter: '/' },
                dummy_object_sdk,
            );
        ...
        mocha.it(`list objects md prefix 3 - not an object`, async function () {
            const ls_obj_res = await ns_tmp.list_objects(
                { bucket: upload_bkt, prefix: 'a/b/', delimiter: '/' },
                dummy_object_sdk,
            );
        ...
        mocha.it(
            'delete inner directory object /my-dir when exists /my-dir/my-dir2',
            async function () {
                ...
                await fs_utils.file_must_not_exist(
                    path.join(p1, config.NSFS_FOLDER_OBJECT_NAME),
                );
                const p2 = path.join(
                    ns_tmp_bucket_path,
                    upload_key_2,
                ...
                await fs_utils.file_must_exist(
                    path.join(p2, config.NSFS_FOLDER_OBJECT_NAME),
                );
        ...
        mocha.it(
            'delete object content 0 - no .folder file',
            async function () {
                ...
                await ns_tmp.delete_object(
                    { bucket: upload_bkt, key: upload_key_3 },
                    dummy_object_sdk,
                );
                await fs_utils.file_must_not_exist(
                    path.join(p1, config.NSFS_FOLDER_OBJECT_NAME),
        ...
        mocha.it('delete multiple objects /my-dir/my-dir2', async function () {
            ...
            await ns_tmp.delete_multiple_objects(
                {
                    bucket: upload_bkt,
                    objects: [upload_key_2 + upload_key_4, upload_key_2].map(
                        key => ({ key }),
                    ),
                },
                dummy_object_sdk,
...
mocha.describe('nsfs_symlinks_validations', function () {
    ...
    mocha.before(async () => {
        ...
        await P.all(
            _.map(expected_dirs, async dir =>
                fs_utils.create_fresh_path(`${bucket_full_path}/${dir}`),
            ),
        );
        await P.all(
            _.map(expected_files, async file =>
                create_file(`${bucket_full_path}/${file}`),
            ),
        );
        await P.all(
            _.map(expected_links, async link =>
    ...
    mocha.describe('without_symlinks', function () {
        mocha.it('without_symlinks:list iner dir', async function () {
            ...
            assert.strictEqual(
                res.objects.length,
                2,
                'amount of files is not as expected',
            );
        });

        mocha.it(
            'without_symlinks:list iner dir without delimiter',
            async function () {
                const res = await list_objects(
                    ns,
                    bucket,
    ...
    mocha.describe('by_symlinks', function () {
        ...
        mocha.it(
            'by_symlinks:list src dir without delimiter',
            async function () {
                const res = await list_objects(
                    ns,
                    bucket,
        ...
        mocha.it(
            'by_symlinks:list iner dir without delimiter',
            async function () {
                const res = await list_objects(
                    ns,
                    bucket,
        ...
        mocha.it('by_symlinks:read_object_md', async function () {
            try {
                ...
                assert.strictEqual(
                    err.code,
                    'EACCES',
                    'read_object_md should return access denied',
                );
            }
        });
        ...
        mocha.it('by_symlinks:read_object_stream', async function () {
            try {
                ...
                assert.strictEqual(
                    err.code,
                    'EACCES',
                    'read_object_stream should return access denied',
                );
            }
        });
        ...
        mocha.it('by_symlinks:upload_object', async function () {
            ...
            try {
                ...
                assert.strictEqual(
                    err.code,
                    'EACCES',
                    'upload_object should return access denied',
                );
            }
        });
        ...
        mocha.it('by_symlinks:delete_object', async function () {
            try {
                ...
                assert.strictEqual(
                    err.code,
                    'EACCES',
                    'delete_object should return access denied',
                );
            }
        });
    });
...
mocha.describe('namespace_fs copy object', function () {
    ...
    mocha.before(async () => {
        await P.all(
            _.map([src_bkt, upload_bkt], async buck =>
                fs_utils.create_fresh_path(`${tmp_fs_path}/${buck}`),
            ),
        );
    });
    ...
    mocha.describe('upload_object (copy)', function () {
        ...
        mocha.it(
            'copy, read of a small object copy - link flow',
            async function () {
                const params = {
                    bucket: upload_bkt,
                    key: copy_key_1,
                ...
                console.log(
                    'read_object_stream (copy) response',
                    inspect(read_res),
                );
                const read_data = read_res.join();
                assert.strictEqual(Buffer.compare(read_data, data), 0);
                ...
                console.log(
                    'delete_object (copy) response',
                    inspect(delete_copy_res),
                );
            },
        );

        mocha.it(
            'copy, read of the small object twice to the same file name',
            async function () {
                const params = {
                    bucket: upload_bkt,
                    key: copy_key_1,
                ...
                console.log(
                    'upload_object: copy twice (1) to the same file name response',
                    inspect(copy_res),
                );

                copy_res = await ns_tmp.upload_object(params, dummy_object_sdk);
                console.log(
                    'upload_object: copy twice (2) to the same file name response',
                    inspect(copy_res),
                );
                ...
                console.log(
                    'read_object_stream: copy twice to the same file name response',
                    inspect(read_res),
                );
                const read_data = read_res.join();
                assert.strictEqual(Buffer.compare(read_data, data), 0);
                ...
                console.log(
                    'delete_object: copy twice to the same file name response',
                    inspect(delete_copy_res),
                );
            },
        );
        ...
        mocha.it(
            `copy directory object to same dir - size 100`,
            async function () {
        ...
        mocha.it(
            `override directory object - size 100 -> size 0`,
            async function () {
                ...
                res = await ns_tmp.upload_object(
                    {
                        ...
                        key,
                        source_stream:
                            buffer_utils.buffer_to_read_stream(empty_data),
                        size: 0,
                    },
                    dummy_object_sdk,
                );
                ...
                assert.deepStrictEqual(
                    0,
                    Object.keys(read_md_res.xattr).length,
                );
                assert.deepStrictEqual(xattr, {
                    ...add_user_prefix(read_md_res.xattr),
                    [XATTR_DIR_CONTENT]: `${read_md_res.size}`,
        ...
        mocha.it(
            `copy directory object to same dir - size 0`,
            async function () {
        ...
        mocha.it(
            `copy object to a directory dir - size 100`,
            async function () {
        ...
        mocha.it(`copy object to a directory dir - size 0`, async function () {
            ...
            assert.deepStrictEqual(xattr, {
                ...add_user_prefix(copy_read_md_res.xattr),
                [XATTR_DIR_CONTENT]: `${copy_read_md_res.size}`,
            });
...
async function delete_object(ns, bucket, file_key, dummy_object_sdk) {
    ...
    console.log(
        'delete_object do not delete the path response',
        inspect(delete_copy_res),
    );
