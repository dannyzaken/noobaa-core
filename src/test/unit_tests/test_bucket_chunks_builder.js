...
const {
  BucketChunksBuilder,
} = require('../../server/bg_services/bucket_chunks_builder');
...
mocha.describe('bucket_chunks_builder', function () {
  mocha.describe('run_batch', function () {
    mocha.it(
      'should return empty chunk_ids and success on no\\empty buckets',
      async function () {
    ...
    mocha.it(
      'should return empty chunk_ids and success on empty iteration result',
      async function () {
        ...
        assert.strictEqual(
          iterate_chunks_func.callCount,
          1,
          'iterate_chunks_func should be called only once',
        );
        assert.strictEqual(
          build_chunks_func.callCount,
          0,
          'build_chunks_func should not be called',
        );
    ...
    mocha.it(
      'should pass (start_marker, end_marker, buckets) in this order',
      async function () {
        const iterate_chunks_func = sinon.stub();
        iterate_chunks_func.returns({ marker: null, chunk_ids: [] });
        const build_chunks_func = sinon.spy();
        ...
        assert(
          iterate_chunks_func.calledWithExactly('start', 'end', ['bucket']),
    ...
    mocha.it(
      'should not modify marker on empty iteration result if no end_marker',
      async function () {
        const iterate_chunks_func = sinon.stub();
        iterate_chunks_func
          .onFirstCall()
        ...
        assert.strictEqual(
          build_chunks_func.callCount,
          1,
          'build_chunks_func should be called once',
        );
        ...
        assert.strictEqual(
          iterate_chunks_func.callCount,
          2,
          'iterate_chunks_func should be called twice',
        );
        assert.strictEqual(
          build_chunks_func.callCount,
          1,
    ...
    mocha.it(
      'should not return done if end_marker is not sent',
      async function () {
        const iterate_chunks_func = sinon.stub();
        iterate_chunks_func.returns({ marker: null, chunk_ids: [] });
        const build_chunks_func = sinon.spy();
    ...
    mocha.it(
      'should call build_chunks_func with chunk_ids from iterate',
      async function () {
        const iterate_chunks_func = sinon.stub();
        iterate_chunks_func.returns({
          marker: 'chunk4',
        ...
        await builder.run_batch(['bucket1', 'bucket2']);
        assert(
          build_chunks_func.calledWithExactly([
            'chunk1',
            'chunk2',
            'chunk3',
            'chunk4',
    ...
    mocha.it(
      'should return successful chunk_ids on build_chunks_func success',
      async function () {
        const iterate_chunks_func = sinon.stub();
        iterate_chunks_func.returns({
          marker: 'chunk4',
    ...
    mocha.it(
      'should return chunk_ids with successful:false on build_chunks_func error',
      async function () {
        const iterate_chunks_func = sinon.stub();
        iterate_chunks_func.returns({
          marker: 'chunk4',
    ...
    mocha.it(
      'should return empty chunk_ids and successful:false iterate_chunks_func error',
      async function () {
        const iterate_chunks_func = sinon.stub();
