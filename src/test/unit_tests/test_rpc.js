...
mocha.describe('RPC', function () {
        ...
        param6: [
            ...
            },
            {
                subarray: ['m', 'a', 'r'].map(ch => ({
                    stam: Buffer.alloc(ch.charCodeAt(0) * 10, ch).toString(
                        'base64',
                    ),
                })),
            },
    ...
    function make_server() {
        const server = {
            ...
            common(req) {
                ...
                assert.deepStrictEqual(
                    PARAMS,
                    _.omit(req.rpc_params, RPC_BUFFERS),
                );
    ...
    mocha.describe('register_service', function () {
        mocha.it(
            'should work on empty server with allow_missing_methods',
            function () {
    ...
    mocha.describe('attachments', function () {
        mocha.it(
            'should send/receive attachments in params/reply',
            function () {
                const server = make_server();
                ...
                const params_attachments = {
                    buffer1: Buffer.alloc(
                        1024,
                        'testing params attachments!\n',
                    ),
                    another_buffer: Buffer.alloc(
                        1024,
                        'I am just another buffer!\n',
                    ),
                };
                const reply_attachments = {
                    buffer2: Buffer.alloc(1024, 'testing reply attachments!\n'),
                    different_buffer: Buffer.alloc(
                        1024,
                        'I am just a different buffer!\n',
                    ),
                };
                ...
                server.put = req => {
                    ...
                    const received_params_attachments =
                        req.rpc_params[RPC_BUFFERS];
                    log(
                        'received_params_attachments',
                        received_params_attachments,
                    );
                    assert(
                        _.isEqual(
                ...
                rpc.register_service(test_api, server);
                return client.test.put(params).then(reply => {
                    assert(received);
                    const received_reply_attachments = reply[RPC_BUFFERS];
                    log(
                        'received_reply_attachments',
                        received_reply_attachments,
                    );
                    assert(
                        _.isEqual(
    ...
    function n2n_tester(n2n_config) {
        return function () {
            ...
            const n2n_agent = rpc.register_n2n_agent(params =>
                rpc.accept_n2n_signal(params),
            );
