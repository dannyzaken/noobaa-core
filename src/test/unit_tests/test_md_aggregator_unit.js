...
function make_test_system_store(last_update, md_store) {
    ...
    const system_store = {
        ...
        find_system_by_id(id) {
            return _.find(
                this.data.systems,
                system => String(system._id) === String(id),
            );
        },
        find_bucket_by_id(id) {
            return _.find(
                this.data.buckets,
                bucket => String(bucket._id) === String(id),
            );
        },
        find_pool_by_id(id) {
            return _.find(
                this.data.pools,
                pool => String(pool._id) === String(id),
            );
        },
...
mocha.describe('md_aggregator', function () {
    ...
    mocha.describe('calculations', function () {
        ...
        mocha.it('should calculate pool delta', function () {
            ...
            const bucket_update = md_aggregator.calculate_new_bucket({
                ...
                existing_blocks_aggregate: _.cloneDeep(
                    existing_blocks_aggregate,
                ),
            ...
            _.each(bucket_update.pools, function (pool, id) {
                ...
                assert(
                    pool.blocks_size === added_block_size - deleted_block_size,
                );
            });
        });
        ...
        mocha.it('should update storage stats', function () {
            ...
            const expected_storage_stats = {
                chunks_capacity:
                    storage_stats.chunks_capacity +
                    added_chunk['123'].compress_size -
                    deleted_chunk['123'].compress_size,
                blocks_size:
                    storage_stats.blocks_size +
                    added_block.buckets['123'].size -
                    deleted_block.buckets['123'].size,
                objects_size:
                    storage_stats.objects_size +
                    added_object['123'].size -
                    deleted_object['123'].size,
                objects_count:
                    storage_stats.objects_count +
                    added_object['123'].count -
                    deleted_object['123'].count,
                stats_by_content_type: [],
                pools: {
                    890: {
                        blocks_size:
                            storage_stats.pools['890'].blocks_size +
                            added_block.buckets['123'].pools['890'].size -
                            deleted_block.buckets['123'].pools['890'].size,
                    },
                    789: {
                        blocks_size:
                            storage_stats.pools['789'].blocks_size +
                            added_block.buckets['123'].pools['789'].size -
                            deleted_block.buckets['123'].pools['789'].size,
                    },
                    900: {
    ...
    mocha.describe('aggregation', function () {
        ...
        mocha.it('should aggregate basic', function () {
            ...
            const block_id1 = md_store.make_md_id_from_time(
                last_update + sub_cycle(),
            );
            coretest.log(
                'block 1 addtion date',
                block_id1.getTimestamp().getTime(),
            );
            const system_id = system_store.data.systems[0]._id;

            return P.resolve()
                .then(() =>
                    md_store.insert_blocks([
                        {
                            _id: block_id1,
                            system: system_id,
                ...
                .then(() =>
                    md_store.insert_chunks([
                        {
                            _id: md_store.make_md_id_from_time(
                                last_update + sub_cycle(),
                            ),
                            system: system_id,
                            bucket: system_store.data.buckets[0]._id,
                ...
                .then(() =>
                    md_aggregator.run_md_aggregator(
                        md_store,
                        system_store,
                        target_now,
                        0,
                ...
                .then(() => {
                    ...
                    assert.strictEqual(
                        changes.update.buckets.length,
                        system_store.data.buckets.length,
                    );
                    assert.strictEqual(
                        changes.update.pools.length,
                        system_store.data.pools.length,
                    );
                    assert.strictEqual(
                        changes.update.buckets[0].storage_stats.blocks_size,
                        12,
                    );
                    assert.strictEqual(
                        changes.update.pools[0].storage_stats.blocks_size,
                        12,
                    );
                    changes.update.buckets.forEach(item => {
                        assert.strictEqual(
                            item.storage_stats.last_update,
                            last_update + CYCLE,
                        );
                    });
        ...
        mocha.it('should aggregate deletions', function () {
            ...
            const block_id2 = md_store.make_md_id_from_time(
                last_update + sub_cycle(),
            );
            ...
            coretest.log(
                'block 2 addtion date',
                block_id2.getTimestamp().getTime(),
            );
            const system_id = system_store.data.systems[0]._id;
            ...
            return P.resolve()
                ...
                .then(() =>
                    md_store.update_blocks_by_ids([block_id2], {
                ...
                .then(() => {
                    ...
                    assert.strictEqual(
                        changes0.update.buckets.length,
                        system_store.data.buckets.length,
                    );
                    assert.strictEqual(
                        changes0.update.pools.length,
                        system_store.data.pools.length,
                    );
                    assert.strictEqual(
                        changes0.update.buckets[0].storage_stats.blocks_size,
                        470,
                    );
                    assert.strictEqual(
                        changes0.update.pools[0].storage_stats.blocks_size,
                        470,
                    );
                    ...
                    assert.strictEqual(
                        changes1.update.buckets[0].storage_stats.blocks_size,
                        120,
                    );
                    assert.strictEqual(
                        changes1.update.pools[0].storage_stats.blocks_size,
                        120,
                    );
            ...
            return P.resolve()
                .then(() => {
                    ...
                    for (let i = 0; i < 1024; ++i) {
                        // 1 PB
                        const block_id = md_store.make_md_id_from_time(
                            last_update + sub_cycle(),
                        );
                        blocks.push(
                            make_block(
                ...
                .then(() =>
                    md_store.update_blocks_by_ids(blocks_to_delete, {
                        deleted: new Date(last_update + CYCLE + sub_cycle()),
                    }),
                )
                .then(() =>
        ...
        mocha.it('should aggregate multiple ranges', function () {
            ...
            return P.resolve()
                ...
                .then(() => {
                    assert.strictEqual(
                        system_store.changes_list.length,
                        num_ranges * 2,
                    );
                    system_store.changes_list.forEach((changes, i) => {
                        if (!changes.update.systems) {
                            ...
                            assert.strictEqual(
                                changes.update.buckets[0].storage_stats
                                    .blocks_size,
                                666,
                            );
                            assert.strictEqual(
                                changes.update.pools[0].storage_stats
                                    .blocks_size,
                                666,
                            );
                            changes.update.buckets.forEach(item => {
            ...
            return P.resolve()
                ...
                .then(() => {
                    ...
                    changes0.update.buckets.forEach(item => {
                        ...
                        assert.strictEqual(
                            item.storage_stats.chunks_capacity,
                            0,
                        );
