...
mocha.describe('namespace_fs - versioning', function () {
    ...
    mocha.before(async () =>
        fs_utils.create_fresh_path(ns_tmp_bucket_path, 0o770),
    );
    ...
    const dummy_object_sdk_no_nsfs_permissions = make_dummy_object_sdk(
        true,
        5,
        5,
    );
    ...
    mocha.it(
        'set bucket versioning - Enabled - should fail - no nsfs config',
        async function () {
            try {
                await ns_tmp.set_bucket_versioning(
                    'ENABLED',
    ...
    mocha.it(
        'safe move posix - Enabled - should fail, retry, success',
        async function () {
            const file_key = 'file1.txt';
            const from_path = path.join(ns_tmp_bucket_path, file_key);
            const to_path = path.join(
                ns_tmp_bucket_path,
                file_key + '_mtime-1-ino-2',
            );
            const fake_mtime_ino = { mtimeNsBigint: BigInt(0), ino: 0 };
            try {
                const bucket_tmp_dir_path =
                    ns_tmp.get_bucket_tmpdir_full_path();
