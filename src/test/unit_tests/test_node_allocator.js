...
mocha.describe('node_allocator', function () {
  ...
  mocha.it('kmeans divided to groups', function () {
    ...
    return P.delay(10000)
      ...
      .then(pool =>
        Promise.all([
          rpc_client.node.allocate_nodes({
      ...
      .then(res => {
        ...
        assert(
          allocation.latency_groups.length ===
            config.NODE_ALLOCATOR_NUM_CLUSTERS,
          'KMEANS did not divide to correct K number of groups',
        );
        assert(
          _.every(allocation.latency_groups, group => group.nodes.length),
          'KMEANS groups should have nodes',
        );
        const total_nodes_list = _.reduce(
          allocation.latency_groups,
          (sum, group) => sum + group.nodes.length,
          0,
        );
        assert(
          total_nodes_list === pool_nodes.total_count,
          `KMEANS groups should have all nodes, ${total_nodes_list} !== ${pool_nodes.total_count}`,
        );
      });
  });
});
