...
async function migrate_collection(col_name) {
    ...
    await client.query(
        `CREATE TABLE IF NOT EXISTS ${col_name} (_id char(24) PRIMARY KEY, data jsonb )`,
    );
    let done = false;
    let marker;
    let total = 0;
    while (!done) {
        ...
        if (docs.length > 0) {
            ...
            total += docs.length;
            console.log(
                `DZDZ: migrated ${total} documents to table ${col_name}`,
            );
    ...
    console.log(
        `DZDZ: completed migration of ${total} documents to table ${col_name}`,
    );
}
