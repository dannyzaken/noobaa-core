...
mocha.describe('ChunkSplitter', function () {
    ...
    mocha.it.skip(
        'splits almost the same when pushing bytes at the start',
        async function () {
            ...
            const res = await P.map(bufs, data =>
                split_buffer({ avg_chunk, delta_chunk, data }),
            );
            ...
            for (let i = 1; i < res.length; ++i) {
                ...
                for (let j = 0; j <= points2.length - points.length; ++j) {
                    if (
                        _.isEqual(points, points2.slice(j, j + points.length))
                    ) {
                        found = true;
                        break;
                    }
                ...
                assert(
                    found,
                    `points ${points.join(',')} not found in ${points2.join(',')}`,
                );
            }
        },
    );
