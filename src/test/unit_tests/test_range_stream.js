...
mocha.describe('range_stream', function () {
  mocha.it('range in the second buffer of two buffers', async function () {
    ...
    assert.deepStrictEqual(bufs, [Buffer.from([19, 20])]);
  });
  ...
  mocha.it('range in the first buffer', async function () {
    ...
    assert.deepStrictEqual(bufs, [Buffer.from([12, 13, 14, 15])]);
  });
  ...
  mocha.it('range has one byte', async function () {
    ...
    assert.deepStrictEqual(bufs, [Buffer.from([11])]);
  });
  ...
  mocha.it('range is entire buffer', async function () {
    ...
    assert.deepStrictEqual(bufs, [Buffer.from([17, 18, 19, 20, 21])]);
  });
  ...
  mocha.it(
    'range starts from the first buffer and is larger than buffers',
    async function () {
  ...
  mocha.it(
    'range starts from the second buffer and is larger than buffers',
    async function () {
      ...
      assert.deepStrictEqual(bufs, [Buffer.from([19, 20, 21])]);
    },
  );
});
