 ...
 */
async function create_object({ bucket, key, size = 1000 }) {
...
mocha.describe('map_builder', function () {
    coretest.describe_mapper_test_case(
        ...
        ({
            ...
            mocha.it('evict object', async function () {
                ...
                const afterObject =
                    await MDStore.instance().find_object_by_id(object_id);
                const chunks = await MDStore.instance().find_chunks_by_ids(
                    obj.chunk_ids,
                );
                const parts =
                    await MDStore.instance().find_all_parts_of_object(
                        afterObject,
                    );
                ...
                assert.notStrictEqual(
                    afterObject.deleted,
                    undefined,
                    'Object should be marked deleted',
                );
                assert.equal(
                    chunks.filter(chunk => !chunk.deleted).length,
                    0,
            ...
            mocha.it('evict object partial', async function () {
                ...
                const chunks_to_delete = obj.chunk_ids.slice(
                    obj.chunk_ids.length - 1,
                );
            ...
            mocha.it(
                'builds missing frags with another rebuild failure in the batch',
                async function () {
                    this.timeout(600000); // eslint-disable-line no-invalid-this
            ...
            mocha.it('does nothing for good object', async function () {
                ...
                const blocks_before = get_all_chunks_blocks(obj_before.chunks)
                    .map(block => String(block._id))
                    .sort();
                const blocks_after = get_all_chunks_blocks(obj_after.chunks)
                    .map(block => String(block._id))
                    .sort();
                assert.deepStrictEqual(blocks_after, blocks_before);
                assert(
                    blocks_before.length >= obj_before.chunks.length,
                    'blocks_before.length >= obj_before.chunks.length',
                );
            });
            ...
            async function delete_blocks_keep_minimal_frags(obj) {
                ...
                _.forEach(chunks, chunk => {
                    ...
                    const frags_to_delete = new Set(
                        _.sampleSize(chunk.frags, parity_frags),
                    );
            ...
            async function delete_blocks_keep_minimal_replicas(obj) {
                ...
                _.forEach(chunks, chunk => {
                    coretest.log(
                        'Keeping minimal replicas in chunk',
                        chunk._id,
                    );
            ...
            async function delete_blocks(blocks) {
                coretest.log(
                    'Deleting blocks',
                    blocks.map(block =>
                        _.pick(block, '_id', 'size', 'frag.id'),
                    ),
                );
                return Promise.all([
                    map_deleter.delete_blocks_from_nodes(blocks),
                    MDStore.instance().update_blocks_by_ids(
                        _.map(blocks, '_id'),
                        { deleted: new Date() },
                    ),
                ]);
            ...
            async function assert_fully_built(obj) {
                ...
                _.forEach(chunks, chunk => {
                    chunk.frags.forEach(frag => {
                        ...
                        const deletions = frag.blocks.filter(
                            block =>
                                block.is_deletion || block.is_future_deletion,
                        );
                        assert.strictEqual(deletions.length, 0);
                    });
                });

                const read_data = await object_io.read_entire_object({
                    client: rpc_client,
                    object_md: obj,
                });
    ...
    mocha.it('does block movement across storage class', async function () {
        ...
        /** @type {nb.Bucket} */
        let loaded_bucket = system_store.data.buckets.find(
            bucket => bucket.name.unwrap() === test_bucket_name,
        );
        const current_tier = loaded_bucket.tiering.tiers.find(
            tier => tier.order === 0,
        ).tier;
        ...
        await system_store.load();
        loaded_bucket = system_store.data.buckets.find(
            bucket => bucket.name.unwrap() === test_bucket_name,
        );
        const target_tier = loaded_bucket.tiering.tiers.find(
            tier => tier.order === 1,
        ).tier;

        const obj = await create_object({
            bucket: test_bucket_name,
            key: 'test-map-builder-storage-class-key',
            size: 128,
        });
        ...
        const builder = new MapBuilder(
            ...
            target_tier,
            false,
            obj.chunk_ids.map(() => current_tier),
        );
        await builder.run();

        const after_chunks = await MDStore.instance().find_chunks_by_ids(
            obj.chunk_ids,
        );

        assert.strictEqual(
            after_chunks.length,
            obj.chunk_ids.length,
            'All chunks should be found',
        );
        assert.strictEqual(
            after_chunks.filter(
                chunk => chunk.tier.toString() === target_tier._id.toString(),
            ).length,
            obj.chunk_ids.length,
            'All chunks should be in target tier',
        );
        assert.strictEqual(
            after_chunks.filter(
                chunk => chunk.tier.toString() === current_tier._id.toString(),
            ).length,
            0,
            'No chunks should be in current tier',
        );
    });
});
