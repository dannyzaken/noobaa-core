  ...
  coretest.describe_mapper_test_case(
    ...
    ({
      ...
      mocha.it('evict object', async function () {
        ...
        const parts =
          await MDStore.instance().find_all_parts_of_object(afterObject);
      ...
      mocha.it('evict object partial', async function () {
        ...
        const beforeObject =
          await MDStore.instance().find_object_by_id(object_id);
        const beforeParts =
          await MDStore.instance().find_all_parts_of_object(beforeObject);
      ...
      async function delete_blocks(blocks) {
        coretest.log(
          'Deleting blocks',
          blocks.map(block => _.pick(block, '_id', 'size', 'frag.id')),
        );
        return Promise.all([
          map_deleter.delete_blocks_from_nodes(blocks),
          MDStore.instance().update_blocks_by_ids(_.map(blocks, '_id'), {
            deleted: new Date(),
          }),
        ]);
      }
        ...
        _.forEach(chunks, chunk => {
          chunk.frags.forEach(frag => {
            ...
            const deletions = frag.blocks.filter(
              block => block.is_deletion || block.is_future_deletion,
            );
