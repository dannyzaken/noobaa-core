...
mocha.describe('AWS STS tests', function () {
    ...
    mocha.before('Creating STS stub', function () {
        ...
        STSStub = sinon.stub(AWS, 'STS').callsFake(() => stsFake);
    });
    ...
    mocha.it('should generate aws sts creds', async function () {
        ...
        const json = await cloud_utils.generate_aws_sts_creds(
            params,
            roleSessionName,
        );
        sinon.assert.calledOnce(STSStub);
        sinon.assert.calledWith(
            stsFake.assumeRoleWithWebIdentity,
            ...expectedParams,
        );
    ...
    mocha.it('should generate an STS S3 client', async function () {
        ...
        const s3 = await cloud_utils.createSTSS3Client(
            params,
            additionalParams,
        );
        ...
        assert.equal(
            s3.config.credentials.secretAccessKey,
            fakeSecretAccessKey,
        );
