...
mocha.describe('namespace_fs mpu optimization tests', function () {
    ...
    mocha.it(
        'MPU | MIX | no part size parameter in upload part ',
        async function () {
            this.timeout(20000); // eslint-disable-line no-invalid-this
    ...
    mocha.it(
        'MPU | TAIL | no part size parameter in upload part ',
        async function () {
            this.timeout(20000); // eslint-disable-line no-invalid-this
    ...
    mocha.it(
        'MPU | COMMON SIZE | single part | no part size parameter in upload part',
        async function () {
            this.timeout(20000); // eslint-disable-line no-invalid-this
    ...
    mocha.it(
        'MPU | COMMON SIZE | no part size parameter in upload part',
        async function () {
            this.timeout(2000000); // eslint-disable-line no-invalid-this
    ...
    mocha.it(
        'MPU | TAIL -> COMMON SIZE | override TAIL to COMMON SIZE',
        async function () {
            this.timeout(20000); // eslint-disable-line no-invalid-this
            ns_tmp.force_md5_etag = true;
    ...
    mocha.it(
        'MPU | TAIL -> TAIL | override TAIL to NEW UNIQUE SIZE',
        async function () {
            this.timeout(20000); // eslint-disable-line no-invalid-this
            ns_tmp.force_md5_etag = true;
            ...
            const parts_properties = {
                ...
                tail: {
                    size: tail_part_size,
                    pass_size: true,
                    override: tail_override_part_size,
                },
            };
            await mpu_test_flow(
                ns_tmp,
    ...
    mocha.it(
        'MPU | TAIL -> TAIL | override TAIL to tail size, different data',
        async function () {
            this.timeout(20000); // eslint-disable-line no-invalid-this
            ns_tmp.force_md5_etag = true;
    ...
    mocha.it(
        'MPU | COMMON SIZE -> TAIL | override last part of common size to be different size (TAIL)',
        async function () {
            this.timeout(2000000); // eslint-disable-line no-invalid-this
            ...
            const parts_properties = {
                common_size: {
                    size: part_size,
                    pass_size: true,
                    override: { part_num: 3, new_size: 15 },
                },
            };
            await mpu_test_flow(
                ns_tmp,
    ...
    mocha.it(
        'MPU | COMMON SIZE -> COMMON SIZE | override last part of common size to be common size, different data',
        async function () {
            this.timeout(2000000); // eslint-disable-line no-invalid-this
            ...
            const parts_properties = {
                common_size: {
                    size: part_size,
                    pass_size: true,
                    override: { part_num: 3, new_size: 10 },
                },
            };
            await mpu_test_flow(
                ns_tmp,
        ...
        async function () {
            ...
            const parts_properties = {
                common_size: {
                    size: part_size,
                    pass_size: true,
                    override: { part_num: 1, new_size: 10 },
                },
            };
            await mpu_test_flow(
                ns_tmp,
    ...
    mocha.it(
        'MPU | COMMON SIZE -> COMMON SIZE | override mid part of common size to be common size, different data',
        async function () {
            this.timeout(2000000); // eslint-disable-line no-invalid-this
            ...
            const parts_properties = {
                common_size: {
                    size: part_size,
                    pass_size: true,
                    override: { part_num: 2, new_size: 10 },
                },
            };
            await mpu_test_flow(
                ns_tmp,
    ...
    mocha.it(
        'MPU | COMMON SIZE -> MIX | override first part to be different size',
        async function () {
            this.timeout(2000000); // eslint-disable-line no-invalid-this
            ...
            const parts_properties = {
                common_size: {
                    size: part_size,
                    pass_size: true,
                    override: { part_num: 1, new_size: 15 },
                },
            };
            await mpu_test_flow(
                ns_tmp,
    ...
    mocha.it(
        'MPU | COMMON SIZE -> MIX | override mid part to be different size',
        async function () {
            this.timeout(2000000); // eslint-disable-line no-invalid-this
            ...
            const parts_properties = {
                common_size: {
                    size: part_size,
                    pass_size: true,
                    override: { part_num: 2, new_size: 15 },
                },
            };
            await mpu_test_flow(
                ns_tmp,
    ...
    mocha.it(
        'MPU | MIX | mid is different size - large size',
        async function () {
            this.timeout(200000); // eslint-disable-line no-invalid-this
...
async function mpu_test_flow(
    ...
    const multiparts = await upload_parts(
        ns_tmp,
        num_parts,
        src_file_path,
        mpu_bkt,
    ...
    if (parts_sizes_obj.tail) {
        ...
        let tail_multipart_res = await upload_single_part(
            ns_tmp,
            tail_part_num,
            src_file_path,
            mpu_bkt,
        ...
        if (to_be_override_size) {
            tail_multipart_res = await upload_single_part(
                ns_tmp,
                tail_part_num,
                src_file_path,
                mpu_bkt,
    ...
    const complete_res = await complete_mpu(
        ns_tmp,
        mpu_bkt,
        mpu_key,
        upload_id,
...
async function upload_single_part(
    namespace_fs_obj,
    part_num,
    original_file,
    mpu_bkt,
...
async function upload_parts(
    ...
    for (let i = 0; i < parts_num; ++i) {
        ...
        let part_res = await upload_single_part(
            namespace_fs_obj,
            part_num,
            original_file,
            mpu_bkt,
        ...
        if (override_part) {
            ...
            part_res = await upload_single_part(
                namespace_fs_obj,
                part_num,
                original_file,
                mpu_bkt,
...
async function complete_mpu(
    ...
    console.log(
        `test_namespace_fs_mpu: complete_mpu - ${upload_id} took ${took} ms`,
    );
    return complete_res;
}
