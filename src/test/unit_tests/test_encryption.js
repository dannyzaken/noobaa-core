    ...
    mocha.it('Update connection with Azure log credentials', async function () {
      ...
      await P.all(
        _.map(accounts.slice(10), async cur_account => {
          ...
          assert.notStrictEqual(
            db_account.sync_credentials_cache[0].azure_log_access_keys
              .azure_client_secret,
...
mocha.describe('Rotation tests', function () {
  ...
  mocha.it('disable account master key test', async function () {
    this.timeout(600000); // eslint-disable-line no-invalid-this
    await system_store.load();
    const original_secrets = await get_account_secrets_from_system_store_and_db(
      EMAIL,
      's3_creds',
    );
    await rpc_client.system.disable_master_key({
    ...
    await P.all(
      _.map(system_store.data.namespace_resources, async ns_resource => {
        ...
        const ns_resources_secrets =
          await get_ns_resources_secrets_from_system_store_and_db(ns_resource);
        if (
          is_connection_is_account_s3_creds(
            ns_resource,
            'ns',
  ...
  mocha.it('disable system master key test', async function () {
    ...
    await P.all(
      _.map(system_store.data.accounts, async account => {
        ...
        const acc_secrets = await get_account_secrets_from_system_store_and_db(
  ...
  mocha.it('rotate system master key test', async function () {
    ...
    await P.all(
      _.map(system_store.data.accounts, async account => {
        ...
        const s3_creds = await get_account_secrets_from_system_store_and_db(
          account.email.unwrap(),
          's3_creds',
        );
        let cloud_creds;
        if (
          ...
          cloud_creds = await get_account_secrets_from_system_store_and_db(
            account.email.unwrap(),
            'cloud_creds',
          );
        }
  ...
  mocha.it('test moving from ENV to files', async function () {
    ...
    const old_res_master_key =
      system_store.master_key_manager.resolved_master_keys_by_id[master_key_id];
  ...
  await P.all(
    _.map(db_chunks, async db_chunk => {
      const chunk_api = api_chunks.find(
        api_chunk => api_chunk._id.toString() === db_chunk._id.toString(),
      );
...
function is_pool_of_account(
  ...
  if (pool_or_ns === 'ns') {
    ans =
      pool_or_ns_obj.connection && pool_or_ns_obj.account._id === account_id;
  }
  return ans;
}
