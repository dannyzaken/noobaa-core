...
mocha.describe('Encryption tests', function () {
    ...
    mocha.describe('Check master keys in system', async function () {
        mocha.it('load system store', async function () {
            ...
            const coretest_account = account_by_name(
        ...
        mocha.it('System master key test', async function () {
            this.timeout(600000); // eslint-disable-line no-invalid-this
            const db_system = await db_client
                .collection('systems')
                .findOne({ name: SYSTEM });
            const root_key_id =
                system_store.master_key_manager.get_root_key_id();
            await compare_master_keys({
                db_master_key_id: db_system.master_key_id,
                father_master_key_id: root_key_id,
            });
        });

        mocha.it('corestest account master key test', async function () {
            this.timeout(600000); // eslint-disable-line no-invalid-this
            const db_coretest_account = await db_client
                .collection('accounts')
                .findOne({ email: EMAIL });
            const db_system = await db_client
                .collection('systems')
        ...
        mocha.it('Coretest acount access keys test', async function () {
            ...
            const db_coretest_account = await db_client
                .collection('accounts')
                .findOne({ email: EMAIL });
            const system_store_account = account_by_name(
                system_store.data.accounts,
                EMAIL,
            );
            ...
            const secrets = {
                ...
                system_store_secret:
                    system_store_account.access_keys[0].secret_key,
                encrypt_and_compare_secret:
                    system_store_account.access_keys[0].secret_key,
            };
            compare_secrets(secrets, system_store_account.master_key_id._id);
            await check_master_key_in_db(
                system_store_account.master_key_id._id,
            );
        });
        ...
        mocha.it('create buckets succefully', async function () {
            ...
            for (i = 0; i < 5; i++) {
                ...
                const db_bucket = await db_client
                    .collection('buckets')
                    .findOne({ name: `bucket${i}` });
                const db_system = await db_client
                    .collection('systems')
                    .findOne({ name: SYSTEM });
        ...
        mocha.it(
            'upload objects succefully and compare chunks cipher keys',
            async function () {
                this.timeout(600000); // eslint-disable-line no-invalid-this
                await P.all(
                    _.map(buckets.slice(0, 2), async cur_bucket => {
        ...
        mocha.it(
            'multipart upload objects succefully and compare chunks cipher keys',
            async function () {
                this.timeout(600000); // eslint-disable-line no-invalid-this
                await P.all(
                    _.map(buckets.slice(3, 4), async cur_bucket => {
                        const bucket_name = cur_bucket.bucket_name;
        ...
        mocha.it('delete buckets succefully', async function () {
            this.timeout(600000); // eslint-disable-line no-invalid-this
            await P.all(
                _.map(buckets, async cur_bucket => {
        ...
        mocha.it(
            'create accounts and compare acount access keys succefully',
            async function () {
                this.timeout(600000); // eslint-disable-line no-invalid-this
                const db_system = await db_client
                    .collection('systems')
                ...
                for (i = 0; i < 20; i++) {
                    ...
                    accounts.push({
                        email: `email${i}`,
                        create_account_result: response_account,
                        index: i,
                    });
                    const db_account = await db_client
                        .collection('accounts')
                        .findOne({ email: `email${i}` });
                    const system_store_account = account_by_name(
                        system_store.data.accounts,
                        `email${i}`,
                    );
                    ...
                    const secrets = {
                        ...
                        system_store_secret:
                            system_store_account.access_keys[0].secret_key,
                        encrypt_and_compare_secret:
                            response_account.access_keys[0].secret_key,
                    };
                    compare_secrets(
                        secrets,
                        system_store_account.master_key_id._id,
        ...
        mocha.it('create external connections succefully', async function () {
            ...
            await P.all(
                _.map(accounts, async cur_account => {
            ...
            await P.all(
                _.map(accounts, async cur_account => {
                    const db_account = await db_client
                        .collection('accounts')
                        .findOne({ email: cur_account.email });
                    const system_store_account = account_by_name(
                        system_store.data.accounts,
                        cur_account.email,
                    );
                    ...
                    const secrets = {
                        db_secret:
                            db_account.sync_credentials_cache[0].secret_key,
                        system_store_secret:
                            system_store_account.sync_credentials_cache[0]
                                .secret_key,
        ...
        mocha.it('create namespace resources succefully', async function () {
            this.timeout(600000); // eslint-disable-line no-invalid-this
            await P.all(
                _.map(accounts.slice(10), async cur_account => {
                    ...
                    namespace_resources.push({
                        ...
                        auth_token: cur_account.create_account_result.token,
                        access_key:
                            cur_account.create_account_result.access_keys[0].access_key.unwrap(),
                        secret_key:
                            cur_account.create_account_result.access_keys[0].secret_key.unwrap(),
                        target_bucket,
                    });
                }),
            );
            ...
            await P.all(
                _.map(accounts.slice(10), async cur_account => {
                    ...
                    const db_ns_resource = await db_client
                        .collection('namespace_resources')
                        .findOne({ name: namespace_resource_name });
                    const system_store_ns_resource = pool_by_name(
                        system_store.data.namespace_resources,
                        namespace_resource_name,
                    ); // system store data supposed to be decrypted
                    // check s3 creds key in db is encrypted
                    const secrets = {
                        db_secret: db_ns_resource.connection.secret_key,
                        system_store_secret:
                            system_store_ns_resource.connection.secret_key,
                        encrypt_and_compare_secret: wrapped_coretest_secret_key,
                    };
                    compare_secrets(
                        secrets,
                    ...
                    assert.strictEqual(
                        db_ns_resource.connection.secret_key,
                        db_account.sync_credentials_cache[0].secret_key,
                    );
                    assert.strictEqual(
                        system_store_ns_resource.connection.secret_key.unwrap(),
        ...
        mocha.it('regenerate creds coretest 1', async function () {
            ...
            assert.notStrictEqual(
                coretest_secret_key,
                system_store_account.access_keys[0].secret_key.unwrap(),
            );
        ...
        mocha.it(
            'update connections succefully - accounts + namespace resources',
            async function () {
                this.timeout(600000); // eslint-disable-line no-invalid-this
                await P.all(
                    _.map(accounts, async cur_account => {
        ...
        mocha.it(
            'Update connection with Azure log credentials',
            async function () {
                        ...
                        await rpc_client.account.update_external_connection(
                            {
                                ...
                                azure_log_access_keys: {
                                    ...
                                    azure_logs_analytics_workspace_id:
                                        new_azure_logs_analytics_workspace_id,
                                },
                            },
                            {
                                auth_token:
                ...
                await P.all(
                    _.map(accounts.slice(10), async cur_account => {
                        ...
                        assert.notStrictEqual(
                            db_account.sync_credentials_cache[0]
                                .azure_log_access_keys.azure_client_secret,
        ...
        mocha.it('create cloud pools succefully', async function () {
            this.timeout(600000); // eslint-disable-line no-invalid-this
            await P.all(
                _.map(accounts.slice(0, 5), async cur_account => {
            ...
            await P.all(
                _.map(accounts.slice(0, 5), async cur_account => {
                    ...
                    const db_pool = await db_client
                        .collection('pools')
                        .findOne({ name: pool_name });
                    const system_store_pool = pool_by_name(
                    ...
                    const secrets = {
                        db_secret:
                            db_pool.cloud_pool_info.access_keys.secret_key,
                        system_store_secret:
                            system_store_pool.cloud_pool_info.access_keys
                                .secret_key,
                        encrypt_and_compare_secret: wrapped_coretest_secret_key,
                    };
                    await check_master_key_in_db(
                    ...
                    assert.strictEqual(
                        system_store_pool.cloud_pool_info.access_keys.secret_key.unwrap(),
                        system_store_account.sync_credentials_cache[0].secret_key.unwrap(),
                    );
                }),
            );
        ...
        mocha.it('regenerate creds coretest 2', async function () {
            ...
            const db_account = await db_client
                .collection('accounts')
                .findOne({ email: EMAIL }); // db data - supposed to be encrypted
            const system_store_account = account_by_name(
                system_store.data.accounts,
                EMAIL,
            ); // system store data supposed to be decrypted
        ...
        mocha.it(
            'update connections succefully - accounts + pools',
            async function () {
                this.timeout(600000); // eslint-disable-line no-invalid-this
                await P.all(
                    _.map(accounts, async cur_account => {
                ...
                await P.all(
                    _.map(accounts.slice(0, 5), async cur_account => {
                        const db_account = await db_client
                            .collection('accounts')
                            .findOne({ email: cur_account.email }); // db data - supposed to be encrypted
                        const system_store_account = account_by_name(
                            system_store.data.accounts,
                            cur_account.email,
                        ); // system store data supposed to be decrypted
                        ...
                        const db_pool = await db_client
                            .collection('pools')
                            .findOne({ name: pool_name }); // db data - supposed to be encrypted
                        const system_store_pool = pool_by_name(
                            system_store.data.pools,
                            pool_name,
                        ); // system store data supposed to be decrypted
        ...
        mocha.it(
            'regenerate creds for all accounts (non coretest) succefully',
            async function () {
                this.timeout(600000); // eslint-disable-line no-invalid-this
                await P.all(
                    _.map(accounts, async cur_account => {
                        await rpc_client.account.generate_account_keys({
        ...
        mocha.it(
            'create and delete external connections succefully',
            async function () {
                this.timeout(600000); // eslint-disable-line no-invalid-this
...
mocha.describe('Rotation tests', function () {
    ...
    mocha.it('disable bucket master key test', async function () {
        const bucket = bucket_by_name(
            system_store.data.buckets,
            buckets[0].bucket_name,
        );
        const db_chunks_before_dis = await db_client
            .collection('datachunks')
            .find({ bucket: bucket._id, deleted: null });
        await rpc_client.system.disable_master_key({
            entity: buckets[0].bucket_name,
            entity_type: 'BUCKET',
        });
        await build_chunks_of_bucket(
            rpc_client,
            buckets[0].bucket_name,
            SYSTEM,
        );
        await compare_chunks_disabled(
            rpc_client,
            buckets[0].bucket_name,
            `key-${buckets[0].bucket_name}`,
            db_chunks_before_dis,
        );
    });
    mocha.it(
        'upload chunks to bucket - when bucket master key is disabled test',
        async function () {
            await put_object(
                buckets[0].bucket_name,
                'key-decrypted-chunks-object',
                s3,
            );
            await build_chunks_of_bucket(
                rpc_client,
    ...
    mocha.it(
        'upload chunks to bucket - when bucket master key is enabled test',
        async function () {
            await build_chunks_of_bucket(
                rpc_client,
                buckets[0].bucket_name,
    ...
    mocha.it('disable account master key test', async function () {
        ...
        const original_secrets =
            await get_account_secrets_from_system_store_and_db(
                EMAIL,
                's3_creds',
            );
        await rpc_client.system.disable_master_key({
        ...
        const secrets = await get_account_secrets_from_system_store_and_db(
            EMAIL,
            's3_creds',
        );
        await compare_secrets_disabled(
            secrets,
            system_store_account.master_key_id._id,
            original_secrets.system_store_secret,
        );
        await P.all(
            _.map(system_store.data.pools, async pool => {
                if (!pool.cloud_pool_info) return;
                if (pool.cloud_pool_info.target_bucket.startsWith(BKT)) return;
                const pools_secrets =
                    await get_pools_secrets_from_system_store_and_db(pool);
                if (
                    is_connection_is_account_s3_creds(
                        pool,
                        'pool',
                        system_store_account._id,
                        system_store_account.access_keys[0],
                    )
                ) {
                    // the pools secrets are still encrypted because they belong to different account
                    compare_secrets(
                        pools_secrets.secrets,
                        pools_secrets.owner_account_master_key_id,
                    );
                }
            }),
        );
        await P.all(
            _.map(system_store.data.namespace_resources, async ns_resource => {
                if (
                    !ns_resource.connection ||
                    ns_resource.connection.target_bucket.startsWith(BKT)
                ) {
                    return;
                }
                const ns_resources_secrets =
                    await get_ns_resources_secrets_from_system_store_and_db(
                        ns_resource,
                    );
                if (
                    is_connection_is_account_s3_creds(
                    ...
                    compare_secrets(
                        ns_resources_secrets.secrets,
                        ns_resources_secrets.owner_account_master_key_id,
                    );
                }
            }),
        );
    ...
    mocha.it(
        'disable account master key test - account has pool',
        async function () {
            this.timeout(600000); // eslint-disable-line no-invalid-this
            await system_store.load();
            const original_secrets =
            ...
            await P.all(
                _.map(system_store.data.pools, async pool => {
                    if (
                        ...
                        pool.cloud_pool_info.target_bucket.startsWith(BKT)
                    ) {
                        return;
                    }
                    const pools_secrets =
    ...
    mocha.it(
        'disable account master key test - account has ns resource',
        async function () {
            this.timeout(600000); // eslint-disable-line no-invalid-this
            await system_store.load();
            const original_secrets =
    ...
    mocha.it('disable system master key test', async function () {
        ...
        await system_store.load();
        const system_store_system = system_by_name(
            system_store.data.systems,
            SYSTEM,
        );
        await is_master_key_disabled(
            system_store_system.master_key_id._id,
            true,
        );
        ...
        await P.all(
            _.map(system_store.data.accounts, async account => {
                if (
                    !account.access_keys &&
                    account.email.unwrap() === 'support@noobaa.com'
                ) {
                    return;
                }
                const acc_secrets =
        ...
        await P.all(
            _.map(system_store.data.pools, async pool => {
                if (pool.cloud_pool_info) {
                    if (pool.cloud_pool_info.target_bucket.startsWith(BKT)) {
                        return;
                    }
                    const pools_secrets =
                        await get_pools_secrets_from_system_store_and_db(pool);
        ...
        await P.all(
            _.map(buckets, async bucket => {
                const sys_store_bucket = bucket_by_name(
                    system_store.data.buckets,
                    bucket.bucket_name,
                );
                await is_master_key_disabled(
                    sys_store_bucket.master_key_id._id,
                    true,
                );
                await build_chunks_of_bucket(
                    rpc_client,
    ...
    mocha.it(
        'create bucket after disable system master key test',
        async function () {
            this.timeout(600000); // eslint-disable-line no-invalid-this
    ...
    mocha.it(
        'create account after disable system master key test',
        async function () {
            this.timeout(600000); // eslint-disable-line no-invalid-this
            const new_account_params = {
                has_login: false,
            ...
            const response_account =
                await rpc_client.account.create_account(new_account_params);
            const acc_secrets =
                await get_account_secrets_from_system_store_and_db(
                    'account-after-disable-ststem',
                    's3_creds',
                );
            assert.strictEqual(
                response_account.access_keys[0].secret_key.unwrap(),
                acc_secrets.system_store_secret.unwrap(),
            );
            const account = account_by_name(
                system_store.data.accounts,
                'account-after-disable-ststem',
            );
            await compare_secrets_disabled(
        ...
        await P.all(
            _.map(system_store.data.accounts, async account => {
                ...
                if (
                    account.sync_credentials_cache &&
                    account
                        .sync_credentials_cache[0] /*account.email === 'coretest@noobaa.com'*/
                ) {
                    const cloud_secrets =
                        await get_account_secrets_from_system_store_and_db(
                            account.email.unwrap(),
    ...
    mocha.it('rotate bucket master key test', async function () {
        this.timeout(600000); // eslint-disable-line no-invalid-this
        let system_store_bucket = bucket_by_name(
            system_store.data.buckets,
            buckets[0].bucket_name,
        );
        const old_master_key_id = system_store_bucket.master_key_id;
        await rpc_client.system.rotate_master_key({
            entity: buckets[0].bucket_name,
            entity_type: 'BUCKET',
        });
        await system_store.load();
        ...
        system_store_bucket = bucket_by_name(
            system_store.data.buckets,
            buckets[0].bucket_name,
        );
        assert.notStrictEqual(
            old_master_key_id._id.toString(),
            system_store_bucket.master_key_id._id.toString(),
        );
    });
    ...
    mocha.it('rotate account master key test', async function () {
        this.timeout(600000); // eslint-disable-line no-invalid-this
        let system_store_account = account_by_name(
            system_store.data.accounts,
            EMAIL,
        );
        ...
        await system_store.load();
        system_store_account = account_by_name(
            system_store.data.accounts,
            EMAIL,
        );
        assert.notStrictEqual(
    ...
    mocha.it('rotate system master key test', async function () {
        ...
        await P.all(
            _.map(system_store.data.accounts, async account => {
                ...
                if (
                    account.sync_credentials_cache &&
                    account.sync_credentials_cache[0]
                ) {
                    cloud_creds =
                        await get_account_secrets_from_system_store_and_db(
                            account.email.unwrap(),
                ...
                old_accounts.push({
                    ...
                    s3_creds,
                    cloud_creds,
                    master_key: account.master_key_id,
                });
            }),
        );
        await P.all(
            _.map(buckets, async bucket => {
                ...
                old_buckets.push({
                    ...
                    master_key: sys_store_bucket.master_key_id,
                });
            }),
        );
        await P.all(
            _.map(system_store.data.pools, async pool => {
                if (pool.cloud_pool_info) {
                    ...
                    const sys_account = account_by_id(
                        system_store.data.accounts,
                        pool.cloud_pool_info.access_keys.account_id._id,
                    );
                    old_pools.push({
                        pool_name: pool.pool_name,
                        master_key: sys_account.master_key_id,
                    });
                }
            }),
        );
        let system_store_system = system_by_name(
            system_store.data.systems,
            SYSTEM,
        );
        ...
        assert.notStrictEqual(
            old_system_master_key_id.toString(),
            system_store_system.master_key_id._id.toString(),
        );
        await P.all(
            _.map(old_accounts, async account => {
                if (account.email.unwrap() === 'coretest@noobaa.com') {
                    ...
                    assert.strictEqual(
                        account.s3_creds.system_store_secret.unwrap(),
                        acc_secrets.system_store_secret.unwrap(),
                    );
                    assert.strictEqual(
                        account.s3_creds.db_secret,
                        acc_secrets.db_secret,
                    );
                    assert.notStrictEqual(
                        account.s3_creds.db_secret,
                        account.s3_creds.system_store_secret.unwrap(),
                    ...
                    const sys_account = account_by_name(
                        system_store.data.accounts,
                        EMAIL,
                    );
                    assert.notStrictEqual(
                        account.master_key.cipher_key.toString('base64'),
                ...
                if (
                    ...
                    assert.strictEqual(
                        account.cloud_secrets.system_store_secret.unwrap(),
                        cloud_secrets.system_store_secret.unwrap(),
                    );
                    assert.strictEqual(
                        account.cloud_secrets.db_secret,
                        cloud_secrets.db_secret,
                    );
                    assert.strictEqual(
                        account.cloud_secrets.db_secret,
                        account.cloud_secrets.system_store_secret.unwrap(),
                    ...
                    await compare_secrets_disabled(
                        cloud_secrets,
                        account.master_key._id,
                    );
                }
            }),
        );
        await P.all(
            _.map(old_buckets, async bucket => {
                const sys_store_bucket = bucket_by_name(
                    system_store.data.buckets,
                    bucket.bucket_name,
                );
                const is_disabled =
                    bucket.bucket_name !== buckets[0].bucket_name;
                await is_master_key_disabled(
                    sys_store_bucket.master_key_id._id,
                    is_disabled,
                );
                assert.notStrictEqual(
                    bucket.master_key.cipher_key.toString('base64'),
                    sys_store_bucket.master_key_id.cipher_key.toString(
                        'base64',
                    ),
                );
            }),
    ...
    mocha.it('test moving from ENV to files', async function () {
        this.timeout(600000); // eslint-disable-line no-invalid-this
        // collect old data
        const old_system_store_system = system_by_name(
            system_store.data.systems,
            SYSTEM,
        );
        const master_key_id = old_system_store_system.master_key_id._id;
        process.env.NOOBAA_ROOT_SECRET = await fs.promises.readFile(
        ...
        assert.strictEqual(
            old_cipher_key.toString(),
            new_cipher_key.toString(),
        );
        // decrypted keys be equal - if root secret was updated correctly
        assert.strictEqual(
            old_res_master_key.cipher_key.toString(),
            new_res_master_key.cipher_key.toString(),
        );
    });
    ...
    mocha.it(
        'rotate root key test twice - validate no change',
        async function () {
            this.timeout(600000); // eslint-disable-line no-invalid-this
            await system_store.load();
            // collect old data
            ...
            const master_key_id = old_system_store_system.master_key_id._id;
            const old_cipher_key =
                old_system_store_system.master_key_id.cipher_key;
            const old_res_master_key =
                system_store.master_key_manager.resolved_master_keys_by_id[
                    master_key_id
                ];
            ...
            const new_cipher_key =
                new_system_store_system.master_key_id.cipher_key;
            const new_res_master_key =
                system_store.master_key_manager.resolved_master_keys_by_id[
                    master_key_id
                ];
    ...
    mocha.it(
        'rotate root key test twice - validate key rotation as expected',
        async function () {
            this.timeout(600000); // eslint-disable-line no-invalid-this
            await system_store.load();
            // collect old data
...
async function multipart_upload(bucket, key, s3_conf) {
    let res = await s3_conf.createMultipartUpload({
        Bucket: bucket,
        Key: key,
    ...
    res = await s3_conf.uploadPart({
        Bucket: bucket,
        Key: key,
        UploadId: upload_id,
        PartNumber: 1,
        Body: 'TEST OF MULTIPART UPLOAD',
    });
...
async function check_master_key_in_db(master_key_id) {
    const db_account = await db_client
        ...
        .findOne({ _id: db_client.parse_object_id(master_key_id) });
    assert.ok(db_account);
}

function compare_secrets(secrets, master_key_id) {
    const { db_secret, system_store_secret, encrypt_and_compare_secret } =
        secrets;
    // 1. compare system store secret and original/response secret - should be equal
    if (encrypt_and_compare_secret) {
        assert.strictEqual(
            system_store_secret.unwrap(),
            encrypt_and_compare_secret.unwrap(),
        );
    }
...
async function compare_master_keys(master_keys) {
    ...
    const db_system_master_key = await db_client
        .collection('master_keys')
        .findOne({ _id: db_master_key_id });
    const resolved_master_key =
        system_store.master_key_manager.resolved_master_keys_by_id[
            db_master_key_id
        ];
    const encrypted_secret_key =
        system_store.master_key_manager.encrypt_buffer_with_master_key_id(
            Buffer.from(resolved_master_key.cipher_key, 'base64'),
            father_master_key_id,
        );

    // check cipher key in db is encrypted by the father master key id
    assert.strictEqual(
        encrypted_secret_key.toString('base64'),
        db_system_master_key.cipher_key.toString('base64'),
    );
}
...
async function compare_chunks(bucket_name, key, rpc_client) {
    const db_bucket = await db_client
        .collection('buckets')
        .findOne({ name: bucket_name });
    const db_chunks = await db_client
        .collection('datachunks')
        .find({ bucket: db_bucket._id, deleted: null });
    const api_chunks = (
        await rpc_client.object.read_object_mapping_admin({
            bucket: bucket_name,
            key,
        })
    ).chunks;
    await P.all(
        _.map(db_chunks, async db_chunk => {
            const chunk_api = api_chunks.find(
                api_chunk =>
                    api_chunk._id.toString() === db_chunk._id.toString(),
            );
            await check_master_key_in_db(db_chunk.master_key_id);
            assert.strictEqual(
                db_chunk.master_key_id.toString(),
                db_bucket.master_key_id.toString(),
            );
            assert.strictEqual(
                db_chunk.master_key_id.toString(),
                chunk_api.master_key_id.toString(),
            );
            const encrypted_secret_key =
                system_store.master_key_manager.encrypt_buffer_with_master_key_id(
                    Buffer.from(chunk_api.cipher_key_b64, 'base64'),
                    chunk_api.master_key_id,
                );
            // check cipher key in db is encrypted by the father master key id
            assert.strictEqual(
                encrypted_secret_key.toString('base64'),
                db_chunk.cipher_key.toString('base64'),
            );
        }),
    );
}
...
async function namespace_cache_tests(
    rpc_client,
    namespace_resources,
    sys_name,
    namespace_buckets,
    ...
    const account_s3 = configure_s3(
        namespace_resources[0].access_key,
        namespace_resources[0].secret_key,
    );
    await P.all(
        _.map(namespace_buckets, async cur_bucket => {
            const bucket_name = cur_bucket.bucket_name;
...
async function populate_system(rpc_client) {
    ...
    // create pools
    await P.all(
        _.map(accounts.slice(0, 2), async cur_account => {
            const pool_name = `${cur_account.email}-cloud-pool`;
            await rpc_client.pool.create_cloud_pool(
                {
                    name: pool_name,
    ...
    await P.all(
        _.map(accounts.slice(2, 4), async cur_account => {
            const namespace_resource_name = `${cur_account.email}-namespace-resource`;
            await rpc_client.pool.create_namespace_resource(
                {
                    name: namespace_resource_name,
...
async function create_delete_external_connections(rpc_client) {
    ...
    // create accounts
    const response_account1 = await rpc_client.account.create_account({
        ...new_account_params,
        email: `delete_email`,
        name: `delete_name`,
    });
    assert.ok(
        response_account1 &&
            ...
            response_account1.access_keys &&
            response_account1.access_keys.length === 1,
    );
    for (let i = 0; i < 5; i++) {
        const response_account2 = await rpc_client.account.create_account({
            ...new_account_params,
            email: `delete_email${i}`,
            name: `delete_name${i}`,
        });
        assert.ok(
            response_account2 &&
                response_account2.token &&
                response_account2.access_keys &&
                response_account2.access_keys.length === 1,
        );
    ...
    const db_account1 = await db_client
        .collection('accounts')
        .findOne({ email: `delete_email` });
    ...
    for (let i = 0; i < 5; i++) {
        const secrets1 = {
            ...
            system_store_secret:
                system_store_account.sync_credentials_cache[i].secret_key,
        };
    ...
    await rpc_client.account.delete_external_connection(
        { connection_name: external_connection.name },
        { auth_token: response_account1.token },
    );
    // compare secrets after deletion
    const db_account2 = await db_client
        .collection('accounts')
        .findOne({ email: `delete_email` });
    ...
    for (let i = 0; i < 4; i++) {
        const secrets3 = {
            ...
            system_store_secret:
                system_store_account.sync_credentials_cache[0].secret_key,
        };
        compare_secrets(secrets3, system_store_account.master_key_id._id);
    }
}

async function compare_chunks_disabled(
    rpc_client,
    bucket_name,
    key,
    db_chunks_before_dis,
    ...
    const db_bucket = await db_client
        .collection('buckets')
        .findOne({ name: bucket_name });
    const db_master_key = await db_client
        .collection('master_keys')
        .findOne({ _id: db_bucket.master_key_id });
    ...
    await P.all(
        _.map(db_chunks, async db_chunk => {
            ...
            assert.notStrictEqual(
                db_chunk.master_key_id,
                db_bucket.master_key_id.toString(),
            );
            assert.strictEqual(db_chunk.master_key_id, chunk_api.master_key_id);
            assert.strictEqual(
                chunk_api.cipher_key_b64,
                db_chunk.cipher_key.toString('base64'),
            );
            if (db_chunks_before_dis) {
                assert.strictEqual(
                    chunk_api._id.toString(),
                    db_chunks_before_dis[0]._id.toString(),
                );
                assert.notStrictEqual(
                    chunk_api.cipher_key_b64,
                    db_chunks_before_dis[0].cipher_key.toString('base64'),
                );
            }
        }),
    );
...
async function compare_secrets_disabled(
    secrets,
    master_key_id,
    original_system_store_secret,
) {
    const db_master_key = await db_client
        .collection('master_keys')
        .findOne({ _id: master_key_id });
    const { db_secret, system_store_secret } = secrets;
    assert.ok(db_master_key.disabled === true);
    assert.strictEqual(system_store_secret.unwrap(), db_secret);
    if (original_system_store_secret) {
        assert.strictEqual(original_system_store_secret.unwrap(), db_secret);
    }
    return true;
}
...
async function build_chunks_of_bucket(rpc_client, bucket_name, system_name) {
    ...
    const db_system = await db_client
        .collection('systems')
        .findOne({ name: system_name });

    await rpc_client.scrubber.build_chunks(
        { chunk_ids },
        {
...
async function get_account_secrets_from_system_store_and_db(
    email_address,
    type,
) {
    const db_account = await db_client
        .collection('accounts')
        .findOne({ email: email_address });
    const system_store_account = account_by_name(
        system_store.data.accounts,
        email_address,
    );
...
async function get_pools_secrets_from_system_store_and_db(system_store_pool) {
    const sys_account = account_by_id(
        system_store.data.accounts,
        system_store_pool.cloud_pool_info.access_keys.account_id._id,
    );
    const db_pool = await db_client
        .collection('pools')
        .findOne({ _id: system_store_pool._id });
    return {
        secrets: {
            db_secret: db_pool.cloud_pool_info.access_keys.secret_key,
            system_store_secret:
                system_store_pool.cloud_pool_info.access_keys.secret_key,
        },
        owner_account_master_key_id: sys_account.master_key_id._id,
    };
}

async function get_ns_resources_secrets_from_system_store_and_db(
    system_store_ns_resources,
) {
    const sys_account = account_by_id(
        system_store.data.accounts,
        system_store_ns_resources.account._id,
    );
    const db_ns_resource = await db_client
        .collection('namespace_resources')
        .findOne({ _id: system_store_ns_resources._id });
    const ans = {
        secrets: {
            db_secret: db_ns_resource.connection.secret_key,
            system_store_secret:
                system_store_ns_resources.connection.secret_key,
        },
        owner_account_master_key_id: sys_account.master_key_id._id,
    };
    return ans;
...
function is_connection_is_account_s3_creds(
    pool_or_ns_obj,
    pool_or_ns,
    account_id,
    account_s3_creds,
    ...
    if (pool_or_ns === 'pool') {
        ans =
            pool_or_ns_obj.cloud_pool_info &&
            //pool_or_ns_obj.cloud_pool_info.access_keys.account_id._id === account_id &&
            pool_or_ns_obj.cloud_pool_info.access_keys.access_key.unwrap() ===
                account_s3_creds.access_key.unwrap();
    }
    if (pool_or_ns === 'ns') {
        ans =
            pool_or_ns_obj.connection &&
            //pool_or_ns_obj.account._id === account_id &&
            pool_or_ns_obj.connection.access_key.unwrap() ===
                account_s3_creds.access_key.unwrap();
    }
    return ans;
}

function is_pool_of_account(
    pool_or_ns_obj,
    pool_or_ns,
    account_id,
    account_s3_creds,
    ...
    if (pool_or_ns === 'pool') {
        ans =
            pool_or_ns_obj.cloud_pool_info &&
            pool_or_ns_obj.cloud_pool_info.access_keys.account_id._id ===
                account_id;
    }
    if (pool_or_ns === 'ns') {
        ans =
