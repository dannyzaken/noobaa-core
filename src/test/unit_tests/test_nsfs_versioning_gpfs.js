...
mocha.describe('namespace_fs gpfs- versioning', async function () {
    ...
    mocha.before(async function () {
        ...
        if (!fs_root_stat) {
            console.log(
                `gpfs_root_path - ${gpfs_root_path} doesn't exist. Skipping test`,
            );
            this.skip(); // eslint-disable-line no-invalid-this
        }
    ...
    mocha.it(
        ...
        async function () {
            const put_res = await put_object(
                dummy_object_sdk,
                ns_obj,
                gpfs_bucket,
                key,
            );
            const head_res = await head_object(
                dummy_object_sdk,
                ns_obj,
                gpfs_bucket,
                key,
            ...
            const latest_version_stat = await stat_and_get_all(
                gpfs_bucket_path,
                key,
            );

            const latest_version_id =
                get_version_id_by_xattr(latest_version_stat);
    ...
    mocha.it(
        'put object - versioning disabled - to be enabled',
        async function () {
            const second_latest_version = await head_object(
                dummy_object_sdk,
                ns_obj,
                gpfs_bucket,
                key,
            ...
            const second_latest_version_id_by_stat =
                await find_max_version_past(gpfs_root_path, key, '');
            assert.equal(
                second_latest_version.VersionId,
                second_latest_version_id_by_stat,
            );
        },
    );
    ...
    mocha.it(
        ...
        async function () {
            ...
            // find second latest version and compare with the latest before the put
            const second_latest_version_id_by_stat =
                await find_max_version_past(gpfs_bucket_path, key, '');
            assert.equal(
                second_latest_version.version_id,
                second_latest_version_id_by_stat,
            );
    ...
    mocha.it(
        'delete object version id - versioning disabled',
        async function () {
            const second_latest_version_id = await find_max_version_past(
                gpfs_bucket_path,
                key,
                '',
            );
            const latest_version = await head_object(
                dummy_object_sdk,
                ns_obj,
                gpfs_bucket,
                key,
            );
            const delete_res = await delete_object(
                dummy_object_sdk,
                ns_obj,
                gpfs_bucket,
                key,
    ...
    mocha.it(
        'delete object - create delete marker - versioning disabled - to be enabled',
        async function () {
            const latest_version = await head_object(
                dummy_object_sdk,
                ns_obj,
            ...
            // latest version check
            const delete_marker_version_id = await find_max_version_past(
                gpfs_bucket_path,
                key,
                '',
            );
            assert.equal(
                delete_res.created_version_id,
                delete_marker_version_id,
            );
            assert.equal(delete_res.created_delete_marker, true);

            await fs_utils.file_must_not_exist(
                path.join(gpfs_bucket_path, key),
            );

            // .versions/ exist
            await fs_utils.file_must_exist(versions_path);
            // find second latest version and compare with the latest before the put
            const new_second_latest_version_id_by_stat =
                await find_max_version_past(gpfs_bucket_path, key, '', [
                    delete_marker_version_id,
                ]);
            assert.equal(
                latest_version.version_id,
                new_second_latest_version_id_by_stat,
            );
        },
    );

    mocha.it(
        'delete object version id in .versions/ - versioning disabled',
        async function () {
            try {
                await head_object(dummy_object_sdk, ns_obj, gpfs_bucket, key);
                assert.fail('head should have failed');
            ...
            const not_exist = await version_file_must_not_exists(
                gpfs_bucket_path,
                key,
                '',
                second_put_ver_id,
    ...
    mocha.it(
        'delete object version id - delete delete marker - versioning disabled',
        async function () {
            const latest_dm_version_id = await find_max_version_past(
                gpfs_bucket_path,
                key,
                '',
            );
