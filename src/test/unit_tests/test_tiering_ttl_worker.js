...
const {
    TieringTTLWorker,
} = require('../../server/bg_services/tier_ttl_worker');
...
async function upload_object({ bucket, key, size = 1024 }) {
...
mocha.describe('tiering_ttl_worker', function () {
    ...
    const ttl_worker = new TieringTTLWorker({
        name: 'test_tiering_ttl_worker',
        client: rpc_client,
    });
    ...
    mocha.it('should work', async function () {
        ...
        /** @type {nb.Bucket} */
        const bucket = await rpc_client.bucket.create_bucket({
            name: BUCKET_NAME,
        });
        ...
        obj.chunks.forEach(chunk => {
            assert.strictEqual(
                String(chunk.tier.name.unwrap()),
                String(tiers_by_order[1]),
            );
        });
