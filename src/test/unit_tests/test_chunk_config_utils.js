...
mocha.describe('chunk_config_utils', function () {
    const chunk_configs = [
        {
            _id: 'default',
            chunk_coder_config:
                chunk_config_utils.new_chunk_code_config_defaults(),
        },
        {
            _id: 'ec_4_2',
            chunk_coder_config:
    ...
    mocha.it('uses default', function () {
        [undefined, null, {}].forEach(ccc => {
            const cc = chunk_config_utils.resolve_chunk_config(
                ccc,
                account,
                system,
            );
    ...
    mocha.it('creates new with defaults when no match', function () {
        ...
        assert.strictEqual(
            cc.chunk_coder_config.data_frags,
            config.CHUNK_CODER_EC_DATA_FRAGS,
        );
