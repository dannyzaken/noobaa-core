...
mocha.describe('s3_ops', function () {
    ...
    mocha.describe('bucket-ops', function () {
        ...
        mocha.it(
            'should list buckets after no buckets left',
            async function () {
                const res = await s3.listBuckets({});
                assert(!res.Buckets.find(bucket => bucket.Name === BKT1));
            },
        ...
        mocha.it(
            'OBC account should not be allowed to delete a bucket',
            async function () {
                ...
                const obc_s3_client = new S3({
                    ...
                    credentials: {
                        accessKeyId:
                            obc_account.access_keys[0].access_key.unwrap(),
                        secretAccessKey:
                            obc_account.access_keys[0].secret_key.unwrap(),
                    },
                    forcePathStyle: true,
                    region: config.DEFAULT_REGION,
                    requestHandler: new NodeHttpHandler({
                ...
                try {
                    ...
                    assert.fail(
                        'expected AccessDenied error. bucket deletion should not be allowed for obc account',
                    );
                } catch (err) {
                ...
                try {
                    ...
                    assert.fail(
                        'expected bucket deletion to be successful for regular accounts',
                    );
                }
                // cleanup
    ...
    async function test_object_ops(
        bucket_name,
        bucket_type,
        caching,
        remote_endpoint_options,
    ) {
        const is_azure_namespace = is_namespace_blob_bucket(
            bucket_type,
            remote_endpoint_options && remote_endpoint_options.endpoint_type,
        );
        const is_azure_mock = is_namespace_blob_mock(
            bucket_type,
            remote_endpoint_options && remote_endpoint_options.endpoint_type,
        );
        ...
        mocha.before(async function () {
            ...
            if (bucket_type === 'regular') {
                ...
                if (process.env.NEWAWSPROJKEY && process.env.NEWAWSPROJSECRET) {
                    source_namespace_bucket =
                        caching ? SOURCE_BUCKET + '-caching' : SOURCE_BUCKET;
                    ...
                    const trgt_nsr = {
                        resource: NAMESPACE_RESOURCE_OTHER_PLATFORM,
                    };
                ...
                source_namespace_bucket =
                    caching ? SOURCE_BUCKET + '-caching' : SOURCE_BUCKET;
                target_namespace_bucket =
                    caching ? TARGET_BUCKET + '-caching' : TARGET_BUCKET;

                const ENDPOINT =
                    USE_REMOTE_ENDPOINT ?
                        process.env.ENDPOINT
                    :   coretest.get_https_address();
                const ENDPOINT_TYPE =
                    USE_REMOTE_ENDPOINT ?
                        process.env.ENDPOINT_TYPE
                    :   'S3_COMPATIBLE';
                const AWS_ACCESS_KEY_ID =
                    USE_REMOTE_ENDPOINT ?
                        process.env.AWS_ACCESS_KEY_ID
                    :   s3_client_params.credentials.accessKeyId;
                const AWS_SECRET_ACCESS_KEY =
                    USE_REMOTE_ENDPOINT ?
                        process.env.AWS_SECRET_ACCESS_KEY
                    :   s3_client_params.credentials.secretAccessKey;
                coretest.log(
                    'before creating azure target bucket: ',
                    source_namespace_bucket,
                    target_namespace_bucket,
                );
                if (is_azure_mock) {
                    coretest.log(
                        'creating azure target bucket: ',
                        source_namespace_bucket,
                        target_namespace_bucket,
                    );
                    ...
                    const blob_service =
                        azure_storage.BlobServiceClient.fromConnectionString(
                            azure_mock_connection_string,
                        );
                    await blob_service.createContainer(TARGET_BUCKET);
                    await blob_service.createContainer(SOURCE_BUCKET);
                }
                coretest.log(
                    'after creating azure target bucket: Using endpoint: ',
                    ENDPOINT,
                );
                const ns_remote_conn = remote_endpoint_options && {
                    ...remote_endpoint_options,
        ...
        mocha.it('should tag text file', async function () {
            ...
            if (notSupported) {
                ...
                assert.strictEqual(
                    res.TagSet.length,
                    params.Tagging.TagSet.length,
                    'Should be 1',
                );
                assert.strictEqual(
                    res.TagSet[0].Key,
                    params.Tagging.TagSet[0].Key,
                    'Should be s3ops',
                );
                assert.strictEqual(
                    res.TagSet[0].Value,
                    params.Tagging.TagSet[0].Value,
                    'Should be set_file_attribute',
                );
            }
            try {
        ...
        mocha.it('should version head text-file', async function () {
            try {
                ...
                if (caching) {
                    params_req.vesionId = 'rasWWGpgk9E4s0LyTJgusGeRQKLVIAFf';
                }
                await s3.headObject(params_req);
                assert.notStrictEqual(
                    caching,
                    undefined,
                    'Versioning not supported when caching is enabled',
                );
            } catch (err) {
        ...
        mocha.it(
            'should get text-file with size > inline range',
            async function () {
                this.timeout(100000);
        ...
        mocha.it(
            'should head text-file with size > inline range',
            async function () {
                this.timeout(100000);
                const ORIG_INLINE_MAX_SIZE = config.INLINE_MAX_SIZE;
                // Change the inline max size so the objects get cached.
        ...
        mocha.it(
            'list-objects should return proper object owner and id',
            async function () {
                if (is_azure_mock) this.skip();
                this.timeout(120000);
                await s3.copyObject({
                ...
                assert.strictEqual(
                    res.Contents[0].Owner.DisplayName,
                    'coretest@noobaa.com',
                );
                ...
                assert.strictEqual(
                    res.Contents[1].Owner.DisplayName,
                    'coretest@noobaa.com',
                );
                ...
                assert.strictEqual(
                    res.Contents[2].Owner.DisplayName,
                    'coretest@noobaa.com',
                );
        ...
        mocha.it(
            'list-object-versions should return proper object owner and id',
            async function () {
                if (is_azure_mock) this.skip();
                this.timeout(120000);
                await s3.copyObject({
                ...
                assert.strictEqual(
                    res.Versions[0].Owner.DisplayName,
                    'coretest@noobaa.com',
                );
                ...
                assert.strictEqual(
                    res.Versions[1].Owner.DisplayName,
                    'coretest@noobaa.com',
                );
                ...
                assert.strictEqual(
                    res.Versions[2].Owner.DisplayName,
                    'coretest@noobaa.com',
                );
        ...
        mocha.it(
            'should copy text-file tagging different platform',
            async function () {
                if (is_azure_mock) this.skip();
        ...
        mocha.it('should copy text-file multi-part', async function () {
            ...
            const UploadId = _.result(
                _.find(res6.Uploads, function (obj) {
        ...
        mocha.it('should list objects with text-file', async function () {
            ...
            if (!is_azure_mock) {
                await s3.getObject({ Bucket: bucket_name, Key: text_file2 });
            }
            // text_file3 was created using copy - Azurite does not support it
            if (!is_azure_mock) {
                await s3.getObject({ Bucket: bucket_name, Key: text_file3 });
            }
        ...
        mocha.it(
            'should delete non existing object without failing',
            async function () {
                this.timeout(60000);
        ...
        mocha.it(
            'should delete multiple non existing objects without failing',
            async function () {
                this.timeout(60000);
        ...
        mocha.it('should delete text-file', async function () {
            ...
            if (!is_azure_mock) {
                await s3.deleteObject({ Bucket: bucket_name, Key: text_file2 });
            }
            // text_file3 was created using copy - Azurite does not support it
            if (!is_azure_mock) {
                await s3.deleteObject({ Bucket: bucket_name, Key: text_file3 });
            }
            if (is_other_platform_bucket_created) {
        ...
        mocha.it(
            'should list objects after no objects left',
            async function () {
                this.timeout(100000);
                const res = await s3.listObjects({ Bucket: bucket_name });
        ...
        mocha.it(
            'Should create DeleteObjects as the number of the keys (different keys)',
            async function () {
                this.timeout(100000);
                await s3.putObject({
                    Bucket: bucket_name,
        ...
        mocha.it(
            'Should create one DeleteObjects (same key)',
            async function () {
                this.timeout(100000);
                await s3.putObject({
                    Bucket: bucket_name,
        ...
        mocha.it(
            'Providing an empty object version ID should return an error',
            async function () {
                try {
                    await s3.getObject({
                        Bucket: bucket_name,
                    ...
                    assert.fail(
                        "getObject with empty VersionId passed when it should've failed",
                    );
                } catch (err) {
        ...
        mocha.it(
            'Providing an empty object version ID marker should return an error',
            async function () {
                try {
                    ...
                    assert.fail(
                        "listObjectVersions with empty VersionId passed when it should've failed",
                    );
                } catch (err) {
                    assert.strictEqual(err.Code, 'InvalidArgument');
                }
        ...
        mocha.it(
            'should copy object (with copy source: /bucket/key)',
            async function () {
                if (is_azure_mock) this.skip();
                this.timeout(120000);
                const key = 'HappyFace';
        ...
        mocha.it(
            'should copy object (with copy source: %2bucket%2key)',
            async function () {
                if (is_azure_mock) this.skip();
                this.timeout(120000);
                ...
                // replace the '/' with %2
                const copy_source_escaped = encodeURIComponent(
                    `/${bucket_name}/${key}`,
                );
                // 1. put
            ...
            if (bucket_type === 'regular') {
                ...
                if (is_other_platform_bucket_created) {
                    ...
                    await rpc_client.account.delete_external_connection({
                        connection_name: CONNECTION_NAME_OTHER_PLATFORM,
                    });
                }
            } else {
                if (!USE_REMOTE_ENDPOINT) {
