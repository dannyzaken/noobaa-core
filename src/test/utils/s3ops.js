...
class S3OPS {
  ...
  async get_file_ranges_check_md5(bucket, file_name, parts, options) {
    ...
    try {
      while (start_byte < file_size) {
        ...
        finish_byte =
          start_byte + jump > file_size ? file_size : start_byte + jump;
      }
    ...
    return P.ninvoke(this.s3, ops, params)
      .then(res => {
        ...
        if (list.length === 0) {
          ...
          list.forEach(file => {
            listFiles.push(
              _.omitBy(_.pick(file, ['Key', 'VersionId']), !_.isUndefined),
            );
            if (!suppress_logs) {
  ...
  async get_list_multipart_uploads_filters(
    ...
    try {
      ...
      if (list.length === 0) {
        console.warn(
          'No objects in bucket with filters: ' + JSON.stringify(params),
        );
      } else {
