...
class S3OPS {
    constructor({
        ...
        if (sig_ver === 'v4' && !use_https) {
            throw new Error(
                'You cannot do this: SigV4 requires https to disable body signing and send streams...',
            );
        }
        const rest_endpoint =
            use_https ? `https://${ip}:${ssl_port}` : `http://${ip}:${port}`;
    ...
    log_error() {
        const args = Array.from(arguments).map(arg =>
            _.isError(arg) && this.suppress_long_errors ? arg.message : arg,
        );
        console.error(...args);
    }
    ...
    validate_multiplier(multiplier) {
        if (multiplier % 1024 !== 0) {
            throw new Error(`multiplier must be in multiples of 1024`);
        }
    }
    ...
    async put_file_with_md5(bucket, file_name, data_size, multiplier) {
        try {
            ...
            console.log(
                `>>> UPLOAD - About to upload object... name: ${file_name}, size: ${actual_size}, bucket: ${bucket}`,
            );
            ...
            verify_md5_map.set(
                `${this.system_verify_name}/${bucket}/${file_name}`,
                md5_hex,
            );
            return md5_hex;
        } catch (err) {
    ...
    async server_side_copy_file_with_md5(
        source_bucket,
        source,
        destination_bucket,
        destination,
        ...
        try {
            ...
            console.log(
                '>>> SS COPY - About to copy object... from: ' +
                    psource +
                    ' to: ' +
                    destination,
            );
            ...
            const file_md5 = verify_md5_map.get(
                `${this.system_verify_name}/${source_bucket}/${source}`,
            );
            verify_md5_map.set(
                `${this.system_verify_name}/${destination_bucket}/${destination}`,
                file_md5,
            );
            console.log(
                'SS Copy object took',
            ...
            this.log_error(
                `SS Copy failed from ${source} to ${destination}!`,
                err,
            );
            throw err;
        }
    ...
    async client_side_copy_file_with_md5(
        bucket,
        source,
        destination,
        versionid,
        ...
        const psource = source + (versionid ? ' v' + versionid : '');
        console.log(
            `>>> CS COPY - About to copy object... from: ${psource} to: ${destination}`,
        );
        ...
        await Promise.all([
            ...
            this._multipart_upload_internal(
                bucket,
                destination,
                pass_through,
                part_size,
    ...
    async upload_file_with_md5(
        ...
        return this._multipart_upload_internal(
            bucket,
            file_name,
            input_stream,
            part_size,
    ...
    async get_file_check_md5(bucket, file_name, options) {
        try {
            ...
            console.log(
                '>>> DOWNLOAD - About to download object...' + file_name,
            );
            const start_ts = Date.now();
            ...
            if (md5_hex === file_md5) {
                console.log(
                    `file ${file_name} MD5 is: ${file_md5} on upload and download, size: ${file_size}`,
                );
            } else {
                this.log_error(
                    `uploaded ${file_name} MD5: ${file_md5} and downloaded MD5: ${
                        md5_hex
                    } - they are different, size: ${file_size}`,
                );
    ...
    async get_file_ranges_check_md5(bucket, file_name, parts, options) {
        ...
        console.log(
            `>>> READ_RANGE - About to read object... ${file_name}, md5: ${file_md5}, parts: ${parts}`,
        );
        try {
            while (start_byte < file_size) {
                const data = await this.get_object_range(
                    bucket,
                    file_name,
                    start_byte,
                    finish_byte,
            ...
            if (md5_digest === file_md5) {
                ...
                this
                    .log_error(`uploaded ${file_name} MD5: ${file_md5} and downloaded MD5:
    ...
    async get_a_random_file(bucket, prefix) {
        try {
            ...
            this.log_error(
                `get_a_random_file:: listObjects - Bucket: ${bucket}, Prefix: ${prefix} failed!`,
                err,
            );
            throw err;
        }
    ...
    async get_a_random_version_file(bucket, prefix) {
        try {
            const { Versions: list } = await this.s3
                .listObjectVersions({
                    Bucket: bucket,
                    Prefix: prefix,
                })
            ...
            this.log_error(
                `get_a_random_file:: listObjectVersions - Bucket: ${bucket}, Prefix: ${prefix} failed!`,
                err,
            );
            throw err;
        }
    ...
    get_list_files(
        bucket,
        prefix,
        param = { suppress_logs: false, maxKeys: 1000, version: false },
    ) {
        ...
        return P.ninvoke(this.s3, ops, params)
            ...
            .catch(err => {
                this.log_error(
                    `get_list_files:: ${ops} ${params} failed!`,
                    err,
                );
                throw err;
            });
    ...
    async get_list_multipart_uploads(bucket) {
        const listFiles = [];
        try {
            const { Uploads: list } = await this.s3
            ...
            this.log_error(
                `get_list_multipart_uploads:: listMultipartUploads - Bucket: ${bucket} failed!`,
                err,
            );
            throw err;
        }
    ...
    async get_list_multipart_uploads_filters(
        bucket,
        delimiter,
        key_marker,
        max_uploads,
        ...
        const listFiles = [];
        try {
            const listMultipartUploads = await this.s3
            ...
            if (list.length === 0) {
                console.warn(
                    'No objects in bucket with filters: ' +
            ...
            this.log_error(
                `get_list_multipart_uploads:: listMultipartUploads ${params} failed!`,
                err,
            );
            throw err;
        }
    ...
    async get_list_prefixes(bucket) {
        const listPrefixes = [];
        try {
            const { CommonPrefixes: list } = await this.s3
                .listObjects({
                    Bucket: bucket,
                    Delimiter: '/',
                })
            ...
            this.log_error(
                `get_list_prefixes:: listObjects - Bucket: ${bucket},  Delimiter: "/" failed!`,
                err,
            );
            throw err;
        }
    ...
    async get_file_number(bucket, prefix) {
        try {
            ...
            this.log_error(
                `get_file_number:: listObjects - Bucket: ${bucket}, Prefix: ${prefix} failed!`,
                err,
            );
            throw err;
        }
    ...
    async delete_multiple_files(bucket, files) {
        ...
        const start_ts = Date.now();
        console.log(
            `>>> MULTI DELETE - About to delete multiple objects... ${files.length}`,
        );
        try {
    ...
    async delete_all_objects_in_bucket(bucket, is_versioning) {
        ...
        while (run_list) {
            const list = await this.get_list_files(bucket, '', {
                maxKeys: 1000,
                version: is_versioning,
                suppress_logs: true,
            });
            ...
            await this.delete_multiple_files(
                bucket,
                _.map(list, item => {
    ...
    async create_bucket(bucket_name, print_error = true) {
        ...
        try {
            ...
            if (print_error) {
                this.log_error(
                    `creating bucket ${bucket_name} is failed!`,
                    err.message,
                );
            }
            throw err;
    ...
    async get_object_uploadId(bucket, object_name) {
        ...
        if (list.length === 0) {
            ...
            const dataObject = list.find(
                content => content.Key === object_name,
            );
            if (!dataObject) throw new Error(`Object key wasn't found`);
            const uploadObjectId = dataObject.UploadId;
            console.log(
                `Object ${dataObject.Key} UploadId is: ${uploadObjectId}`,
            );
        ...
        try {
            ...
            const obj = await this.s3
                ...
                .on('build', req => {
                    ...
                    console.log(
                        `Added query params ${query_params} to path ${req_path}`,
                    );
            ...
            this.log_error(
                `get_object:: getObject ${JSON.stringify(params)} failed!`,
                err,
            );
            throw err;
        }
    ...
    async get_object_range(
        ...
        try {
            console.log(
                `Reading object ${key} ${versionid ? versionid : ''} from ${start_byte} to ${finish_byte} from ${bucket}`,
            );
            const obj = await this.s3.getObject(params).promise();
            return obj.Body;
        } catch (err) {
            this.log_error(
                `get_object_range:: getObject ${JSON.stringify(params)} failed!`,
                err,
            );
            throw err;
        }
    ...
    async get_object_size_etag(bucket, file_name) {
        ...
        try {
            ...
            this.log_error(
                `get object size and etag for ${file_name} in ${bucket} failed!`,
                err,
            );
            throw err;
        }
    ...
    async abort_multipart_upload(bucket, file_name, uploadId) {
        ...
        try {
            await this.s3
                ...
                .promise();
            console.log(
                `Upload ${uploadId} of filename: ${file_name} was aborted successfully`,
            );
        } catch (err) {
            this.log_error(
                `Abort failed ${file_name}, UploadId: ${uploadId}!`,
                err,
            );
            throw err;
        }
    ...
    async create_multipart_upload(bucket, file_name) {
        try {
            const multipartUpload = await this.s3
                ...
                .promise();
            console.log(
                `Initiated multipart upload filename: ${file_name} uploadId ${multipartUpload.UploadId}`,
            );
            return multipartUpload.UploadId;
        } catch (err) {
            this.log_error(
                `Initiating multipart upload failed ${file_name}`,
                err,
            );
            throw err;
        }
     ...
     * Internal Utils
     */
    async _multipart_upload_internal(
        bucket,
        file_name,
        data,
        part_size,
        ...
        try {
            ...
            console.log(
                `>>> MultiPart UPLOAD - About to multipart upload object... ${
                    file_name
                }`,
