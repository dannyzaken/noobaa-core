...
class NamespaceContext {
    ...
    _get_size_from_file_name(file_name) {
        ...
        if (tokens.length < 2) {
            return {
                size: 1,
                data_multiplier: unit_mapping.KB.data_multiplier,
            };
        } else {
            const { data_multiplier } = _.defaultTo(
                unit_mapping[_.last(tokens)],
                unit_mapping.KB,
            );
    ...
    async get_object_s3_md(
        ...
        try {
            const ret = await s3ops_arg.get_object(
                bucket,
                file_name,
                get_from_cache ? { get_from_cache: true } : undefined,
                preconditions,
            );
            return {
            ...
            console.log(
                `Failed to get data from ${file_name} in ${bucket}: ${err}`,
            );
            throw err;
        }
    }
    ...
    async get_via_cloud(type, file_name, preconditions) {
        ...
        return this.get_object_s3_md(
            this._ns_mapping[type].s3ops,
            cloud_bucket,
            file_name,
            false,
            preconditions,
    ...
    async get_object_expect_not_found(
        s3ops_arg,
        bucket,
        file_name,
        get_from_cache,
        ...
        try {
            ...
            throw new Error(
                `Expect ${file_name} not found in ${bucket}, but found with size: ${ret.ContentLength}`,
            );
        } catch (err) {
    ...
    async get_via_noobaa_expect_not_found(type, file_name, get_from_cache) {
        const noobaa_bucket = this._ns_mapping[type].gateway;
        await this.get_object_expect_not_found(
            this._noobaa_s3ops,
            noobaa_bucket,
            file_name,
            get_from_cache,
    ...
    async delete_from_cloud(type, file_name) {
        try {
            const cloud_bucket = this._ns_mapping[type].bucket2;
            await this._ns_mapping[type].s3ops.delete_file(
                cloud_bucket,
                file_name,
            );
        } catch (err) {
    ...
    async delete_files_from_noobaa(type, file_names) {
        const noobaa_bucket = this._ns_mapping[type].gateway;
        await this._noobaa_s3ops.delete_multiple_files(
            noobaa_bucket,
    ...
    async expect_not_found_in_cache(type, file_name) {
        const noobaa_bucket = this._ns_mapping[type].gateway;
        console.log(
            `expecting cache md not found for ${file_name} in ${noobaa_bucket}`,
        );
        await this.get_object_expect_not_found(
            this._noobaa_s3ops,
            noobaa_bucket,
    ...
    async validate_cache_noobaa_md({ type, file_name, validation_params }) {
        const noobaa_bucket = this._ns_mapping[type].gateway;
        console.log(
            `validating noobaa cache md for ${file_name} in ${noobaa_bucket}`,
        );
        const md =
            (
                !_.isUndefined(validation_params.num_parts) ||
                !_.isUndefined(validation_params.upload_size)
            ) ?
                await this._obj_functions.getObjectMDPartsInfo({
                    bucket: noobaa_bucket,
                    key: file_name,
                })
            :   await this._obj_functions.getObjectMD({
                    bucket: noobaa_bucket,
                    key: file_name,
                });
        const {
            ...
            if (cache_last_valid_time_range.end) {
                if (
                    !_.inRange(
                        ...
                        cache_last_valid_time_range.start,
                        cache_last_valid_time_range.end,
                    )
                ) {
                    ...
                    const err = new Error(
                        `Unexpected cache_last_valid_time in object md ${file_name} in bucket ${noobaa_bucket}: ${msg}`,
                    );
                    err.name = 'UnexpectedValue';
                    throw err;
                }
            } else if (
                md.cache_last_valid_time <= cache_last_valid_time_range.start
            ) {
        ...
        for (const [k, v] of Object.entries({
            ...
            if (!_.isUndefined(v)) {
                console.log(
                    `validating ${k}: expect ${v} and have ${md[k]} in noobaa md`,
                );
                if (v !== md[k]) {
                    const err = new Error(
                        `Unexpected ${k} expect ${v} and have ${md[k]} in object md ${file_name} in bucket ${noobaa_bucket}`,
                    );
                    err.name = 'UnexpectedValue';
                    throw err;
                }
        ...
        console.log(
            `validation passed: noobaa cache md for ${file_name} in ${noobaa_bucket}`,
        );
        return md;
    }

    async validate_md5_between_hub_and_cache({
        type,
        force_cache_read,
        file_name,
        expect_same,
        ...
        console.log(
            `comparing md5 between noobaa cache bucket and ${type} bucket for ${file_name}`,
        );
        const ns = this._ns_mapping[type];
        const cloud_bucket = this._ns_mapping[type].bucket2;
        const noobaa_bucket = this._ns_mapping[type].gateway;
        const cloud_md = await this.get_object_s3_md(
            ns.s3ops,
            cloud_bucket,
            file_name,
            false,
        ...
        const noobaa_md = await this.get_object_s3_md(
            this._noobaa_s3ops,
            noobaa_bucket,
            file_name,
            force_cache_read,
        ...
        console.log(
            `noobaa cache bucket (${noobaa_bucket}) has md5 ${
                noobaa_md5
            } and hub bucket ${cloud_bucket} has md5 ${cloud_md5} for ${file_name}`,
        );
        console.log(
            `${file_name} size is ${cloud_md.size} on ${type} and ${noobaa_md.size} on noobaa`,
        );

        if (expect_same && cloud_md5 !== noobaa_md5) {
            throw new Error(
                `Expect md5 ${noobaa_md5} in noobaa cache bucket (${noobaa_bucket}) is the same as md5 ${
                    cloud_md5
                } in hub bucket ${cloud_bucket} for ${file_name}`,
            );
        } else if (!expect_same && cloud_md5 === noobaa_md5) {
            throw new Error(
                `Expect md5 ${noobaa_md5} in noobaa cache bucket (${noobaa_bucket}) is different than md5 ${
                    cloud_md5
                } in hub bucket ${cloud_bucket} for ${file_name}`,
            );
        }
        ...
        if (
            expect_same &&
            cloud_md.last_modified_time.getTime() !==
                noobaa_md.last_modified_time.getTime()
        ) {
        ...
        console.log(
            `validation passed: noobaa cache bucket ${noobaa_bucket} and ${type} bucket have same md5 for ${file_name}`,
        );
        return { cloud_md, noobaa_md };
    }

    // end is inclusive
    async get_range_md5_size(
        s3ops_arg,
        bucket,
        file_name,
        start,
        ...
        console.log(
            `${BLUE}reading range ${start}-${end} from ${file_name} on bucket ${bucket}${NC}`,
        );
        try {
            const ret_body = await s3ops_arg.get_object_range(
                bucket,
                file_name,
                start,
                end,
            ...
            console.log(
                `Failed to get range data from ${file_name} in ${bucket}: ${err}`,
            );
            throw err;
        }
    }

    async get_range_md5_size_via_noobaa(
        type,
        file_name,
        start,
        end,
        ...
        const noobaa_bucket = this._ns_mapping[type].gateway;
        return this.get_range_md5_size(
            this._noobaa_s3ops,
            noobaa_bucket,
            file_name,
            start,
    ...
    async validate_md5_range_read_between_hub_and_cache({
        type,
        file_name,
        start,
        end,
        ...
        console.log(
            `comparing noobaa cache bucket to ${type} bucket for range ${start}-${end} in ${file_name}`,
        );
        const ns = this._ns_mapping[type];
        const cloud_bucket = this._ns_mapping[type].bucket2;
        const noobaa_bucket = this._ns_mapping[type].gateway;
        const cloud_md = await this.get_range_md5_size(
            ns.s3ops,
            cloud_bucket,
            file_name,
            start,
        ...
        const cloud_md5 = cloud_md.md5;
        const noobaa_md = await this.get_range_md5_size(
            this._noobaa_s3ops,
            noobaa_bucket,
            file_name,
            start,
        ...
        const noobaa_md5 = noobaa_md.md5;
        console.log(
            `noobaa cache bucket (${noobaa_bucket}) contains the md5 ${
                noobaa_md5
            } and hub bucket ${cloud_bucket} has md5 ${cloud_md5} for range ${start}-${end} in ${file_name}`,
        );
        console.log(
            `${file_name}: read range size is ${cloud_md.size} on ${type} and ${noobaa_md.size} on noobaa`,
        );
        ...
        if (expect_same) {
            if (cloud_md5 !== noobaa_md5) {
                throw new Error(
                    `Expect md5 ${noobaa_md5} in noobaa cache bucket (${noobaa_bucket}) is the same as md5 ${
                        cloud_md5
                    } in hub bucket ${cloud_bucket} for range ${start}-${end} in ${file_name}`,
                );
            }
            if (expect_read_size !== noobaa_md.size) {
                throw new Error(
                    `Expect range read size ${expect_read_size} on ${file_name} from noobaa cache bucket (${noobaa_bucket}) but got ${
                        noobaa_md.size
                    } for read range ${start}-${end}`,
                );
            }
        } else if (!expect_same && cloud_md5 === noobaa_md5) {
            throw new Error(
        ...
        console.log(
            `validation passed: noobaa cache bucket ${noobaa_bucket} and ${type} bucket ${cloud_bucket} have same md5 for range ${start}-${end} in ${file_name}`,
        );
        return { cloud_md, noobaa_md };
    }
    ...
    async upload_via_noobaa_endpoint(type, file_name, bucket) {
        ...
        const { size, data_multiplier } =
            this._get_size_from_file_name(file_name);
        ...
        try {
            const md5 = await this._noobaa_s3ops.put_file_with_md5(
                bucket,
                file_name,
                size,
                data_multiplier,
    ...
    async upload_directly_to_cloud(type, file_name) {
        ...
        console.log(
            `uploading ${file_name} directly to ${type} bucket ${hub_bucket}`,
        );
        if (this._files_cloud[`files_${type}`]) {
            this._files_cloud[`files_${type}`].push(file_name);
        } else {
        ...
        try {
            const md5 = await this._ns_mapping[type].s3ops.put_file_with_md5(
                hub_bucket,
                file_name,
                size,
                data_multiplier,
            ...
            throw new Error(
                `Failed to upload directly into ${type} bucket ${hub_bucket}`,
            );
        }
    }

    async list_files_in_cloud(type) {
        const list_files_obj = await this._ns_mapping[
            type
        ].s3ops.get_list_files(this._ns_mapping[type].bucket2);
    ...
    async check_via_cloud(type, file_name) {
        ...
        if (list_files.includes(file_name)) {
            console.log(
                `${file_name} was uploaded via noobaa and found via ${type}`,
            );
        } else {
            throw new Error(
                `${file_name} was uploaded via noobaa and was not found via ${type}`,
            );
        }
        return true;
    }

    async validate_range_read({
        type,
        file_name,
        cloud_obj_md,
        start,
        ...
        if (
            !_.isUndefined(cache_last_valid_time_range) ||
            !_.isUndefined(expect_num_parts) ||
        ...
        console.log(
            `validation passed: range read ${start}-${end} in ${file_name}`,
        );
        return mds;
    }
    ...
    async delay(delay_ms) {
        ...
        console.log(
            `${BLUE}waiting for ttl to expire in ${_delay_ms} ms......${NC}`,
        );
    ...
    async run_test_case(test_desc, cloud_type, test_case_fn) {
        ...
        try {
            ...
            console.log(
                `!!! ${RED}test case (${test_name}) failed${NC}: ${err}`,
            );
