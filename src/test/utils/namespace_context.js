...
class NamespaceContext {
  ...
  async validate_cache_noobaa_md({ type, file_name, validation_params }) {
    ...
    const { cache_last_valid_time_range, num_parts, size, etag, upload_size } =
      validation_params;
  ...
  async list_files_in_cloud(type) {
    const list_files_obj = await this._ns_mapping[type].s3ops.get_list_files(
      this._ns_mapping[type].bucket2,
    );
