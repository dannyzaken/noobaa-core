...
const inspect_readable_state = readable =>
    util.inspect(readable._readableState, {
...
async function test_pipe(use_pipeline) {
    ...
    try {
        if (use_pipeline) {
            await pipeline_async(source, decoder, target);
        } else {
    ...
    while (!source.destroyed && n_read < N_READ_MAX) {
        if (source.readableEnded) {
            console.error(
                'main: source ended but not destroyed',
                inspect_readable_state(source),
            );
        } else {
...
async function main() {
    ...
    console.log(
        '***    NOTICE HOW THE ENTIRE SOURCE IS READ IN CASE 2 ...  ***',
    );
    console.log(
        '***                                                        ***',
    );
