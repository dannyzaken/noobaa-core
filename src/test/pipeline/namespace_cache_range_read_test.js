...
async function test_case_range_read_initial_read_size_not_across_blocks({
    ...
    // read range :                       <-------------------------------------->
    console.log(
        `double check cached range is returned after ${file_name} is deleted from hub bucket and before TTL expires`,
    );
    await ns_context.delete_from_cloud(type, file_name);
    await ns_context.get_via_cloud_expect_not_found(type, file_name);
    const noobaa_md_again = await ns_context.get_range_md5_size_via_noobaa(
        type,
        file_name,
        start,
        end,
    ...
    if (!_.isEqual(noobaa_md, noobaa_md_again)) {
        throw new Error(
            `Unexpected range read results: expected ${JSON.stringify(noobaa_md)} but got ${JSON.stringify(noobaa_md_again)}`,
        );
    }
}
test_case_range_read_initial_read_size_not_across_blocks.desc =
    'range read: initial read size is < block_size and not across block boundary';
register_test_scenarios(
    test_case_range_read_initial_read_size_not_across_blocks,
);
...
test_case_range_read_range_variations.desc =
    'range read: range variations - range across block boundary and others';
register_test_scenarios(test_case_range_read_range_variations);

async function test_case_range_read_from_entire_object_to_partial({
    type,
    ns_context,
}) {
    // Make file big enough for holding multiple blocks
...
test_case_range_read_from_entire_object_to_partial.desc =
    'range read: from entire object to partial object';
register_test_scenarios(test_case_range_read_from_entire_object_to_partial);

async function test_case_range_read_from_partial_to_entire_object({
    type,
    ns_context,
}) {
    // Make file big enough for holding multiple blocks
...
test_case_range_read_from_partial_to_entire_object.desc =
    'range read: from partial object to entire object';
...
test_case_range_read_small_file.desc =
    'range read: small file is cached entirely';
register_test_scenarios(test_case_range_read_small_file);

async function test_case_range_read_if_match_etag_failure({
    type,
    ns_context,
}) {
    // Make file big enough for holding multiple blocks
...
test_case_range_read_if_match_etag_failure.desc =
    'range read: if-match etag failure during range read on hub';
register_test_scenarios(test_case_range_read_if_match_etag_failure);

async function test_case_range_read_precondition_set_in_s3_request({
    type,
    ns_context,
}) {
    // Make file big enough for holding multiple blocks
    ...
    try {
        await ns_context.get_range_md5_size_via_noobaa(
            type,
            file_name,
            start,
            end,
            { IfMatch: 'etag not matched' },
        );
        assert(false);
    } catch (err) {
...
test_case_range_read_precondition_set_in_s3_request.desc =
    'range read: s3 request has precondtion set';
...
async function run_namespace_cache_tests_range_read({ type, ns_context }) {
    for (const test_fn of test_funcs) {
        await ns_context.run_test_case(test_fn.desc, type, async () => {
