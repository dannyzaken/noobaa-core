...
const {
    mk_test_name,
    NamespaceContext,
} = require('../utils/namespace_context');
...
const caching_tests = [ns_cache_test_normal, ns_cache_test_range_read];
    ...
    NB_ACCESS_KEY_ID ?
        {
            ...
            use_https: false,
            sig_ver: 'v2',
            access_key: NB_ACCESS_KEY_ID,
            secret_key: NB_SECRET_ACCESS_KEY,
        }
...
async function create_account_resources(type) {
    let connection_name;
    try {
        connection_name = await cf.getConnection(
            connections_mapping[type].endpoint,
        );
        if (connection_name) {
            console.log(
                `connection ${connections_mapping[type].endpoint} exists under the name ${connection_name}`,
            );
        } else {
            //create connection
    ...
    try {
        ...
        report.success(
            mk_test_name('create namespace bucket with caching enabled', type),
        );
    } catch (e) {
    ...
    for (const type of clouds) {
        ...
        if (!skip_clean_conn) {
            try {
                ...
                report.success(
                    mk_test_name('delete external connection', type),
                );
            } catch (err) {
...
async function delete_namespace_bucket(bucket, type) {
    ...
    try {
        ...
        report.success(
            mk_test_name('delete namespace bucket with caching enabled', type),
        );
    } catch (err) {
        report.fail(
            mk_test_name('delete namespace bucket with caching enabled', type),
        ...
        throw new Error(
            `Failed to delete namespace bucket ${bucket} with error ${err}`,
        );
    }
}
