...
async function run_namespace_cache_tests_non_range_read({ type, ns_context }) {
    ...
    const min_file_size_kb =
        config.NAMESPACE_CACHING.DEFAULT_BLOCK_SIZE / 1024 + 1;
    ...
    await ns_context.run_test_case(
        'object cached during upload to namespace bucket',
        type,
        async () => {
    ...
    await ns_context.run_test_case(
        ...
        async () => {
            ...
            const upload_md = await ns_context.upload_via_noobaa_endpoint(
                type,
                file_name,
            );
            const upload_md5 = upload_md.md5;

            const read_prom = ns_context
                .get_via_noobaa(type, file_name)
                .then(ret => {
                    assert(ret.md5 === upload_md5);
                    return ret;
                });

            const delete_prom = P.delay(50).then(() =>
                ns_context.delete_from_noobaa(type, file_name),
            );

            return Promise.all([read_prom, delete_prom]).then(ret => {
                console.log(
                    'success in deleting cached object while read is onging',
                    ret,
                );
            });
        },
    ...
    await ns_context.run_test_case(
        ...
        async () => {
            // Upload a large file to namespace cache bucket
            const file_name = large_file_name;
            const first_upload_md = await ns_context.upload_via_noobaa_endpoint(
                type,
                file_name,
            );
            ...
            const new_upload_prom = P.delay(50).then(() =>
                ns_context.upload_via_noobaa_endpoint(type, file_name),
            );

            return Promise.all([read_prom, new_upload_prom]).then(ret => {
                console.log(
                    'success in reading cached object while overwrite is onging',
                );
    ...
    await ns_context.run_test_case(
        ...
        async () => {
            ...
            for (const file_name of file_names) {
                await ns_context.get_via_noobaa_expect_not_found(
                    type,
                    file_name,
                );
                await ns_context.get_via_cloud_expect_not_found(
                    type,
                    file_name,
                );
            }
    ...
    await ns_context.run_test_case(
        ...
        async () => {
            ...
            const { md5 } = await ns_context.upload_directly_to_cloud(
                type,
                file_name,
            );
            let obj_md = await ns_context.get_via_noobaa(type, file_name, {
                IfMatch: md5,
            });
            ...
            obj_md = await ns_context.get_via_noobaa(type, file_name, {
                IfNoneMatch: 'none-match-etag-value',
            });
            assert(obj_md.etag === md5);
    ...
    await ns_context.run_test_case(
        ...
        async () => {
            ...
            const md_cache = await ns_context.upload_via_noobaa_endpoint(
                type,
                file_name,
            );
            ...
            await P.delay(2000);
            const md_upload_hub = await ns_context.upload_directly_to_cloud(
                type,
                file_name,
            );
