...
const cases = ['fail upload over quota', 'upload over pool capacity'];
...
async function _upload_files(bucket_name, dataset_size, multiplier) {
    ...
    console.log(
        `${YELLOW}Writing ${number_of_files} files with total size: ${dataset_size + number_of_files} MB${NC}`,
    );
...
async function _wait_no_available_space(bucket_name) {
    ...
    while (Date.now() - base_time < 360 * 1000) {
        try {
            is_no_available =
                await bucket_functions.checkAvailableSpace(bucket_name);
...
async function _test_failed_upload(bucket_name, dataset_size, multiplier) {
    ...
    const time_stamp = Math.floor(Date.now() / 1000);
    console.log(
        `Trying to upload ${dataset_size} MB after we have reached the quota`,
    );
...
async function _check_file_in_pool(file_name, pool, bucket_name) {
    ...
    while (keep_run) {
        try {
            console.log(
                `Checking file ${file_name} is available and contains exactly in pool ${pool}`,
            );
            const { chunks } = await client.object.read_object_mapping_admin({
                bucket: bucket_name,
                key: file_name,
            });
            chunks_accessible = chunks.filter(
                chunk => chunk.is_accessible === true,
            );
            const chunks_accessible_length = chunks_accessible.length;
            const parts_in_pool = chunks.filter(chunk =>
                chunk.frags[0].blocks[0].adminfo.pool_name.includes(pool),
            ).length;
            const number_of_chunks = chunks.length;
            if (chunks_accessible_length === number_of_chunks) {
                console.log(
                    `Available chunks: ${chunks_accessible_length}/${number_of_chunks} for ${file_name}`,
                );
            } else {
                throw new Error(
                    `Chunks for file ${file_name} should all be in ${
                        pool
                    }, Expected ${number_of_chunks}, received ${chunks_accessible_length}`,
                );
            }
            if (parts_in_pool === number_of_chunks) {
                console.log(
                    `All of the ${number_of_chunks} chunks are in ${pool}`,
                );
            } else {
                throw new Error(
                    `Expected ${number_of_chunks} parts in ${pool} for file ${file_name}, received ${parts_in_pool}`,
                );
            }
            keep_run = false;
        } catch (e) {
