...
async function act_and_log(action_type) {
  ...
  if (action_type === 'RANDOM') {
    chosen =
      RANDOM_SELECTION[Math.floor(Math.random() * RANDOM_SELECTION.length)];
  } else {
...
function get_random_file(skip_version_check) {
  ...
  if (should_use_versioning() && !skip_version_check) {
    ...
    return s3ops.get_a_random_file(TEST_CFG.bucket, DATASET_NAME).then(res => ({
      filename: res.Key,
      extra: {
        size: res.Size,
      },
        ...
        .then(() => {
          ...
          return P.pwhile(
            () =>
              ...
              (Date.now() - start) / (60 * 1000) < TEST_CFG.aging_timeout,
            () => {
              console.log(
              ...
              if (TEST_STATE.current_size > TEST_CFG.dataset_size) {
                ...
                action_type =
                  Math.round(Math.random()) ? 'DELETE' : 'MULTI_DELETE';
              } else {
                action_type = 'RANDOM';
              }
              return P.resolve().then(() => act_and_log(action_type));
            },
          );
        })
        .then(async () => {
