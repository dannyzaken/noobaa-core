...
async function compereMD5betweenCloudAndNooBaa(
    type,
    bucket,
    noobaa_bucket,
    file_name,
    ...
    let cloudProperties;
    if (type === 'AWS') {
        cloudProperties = await getPropertiesFromS3Bucket(
            s3opsAWS,
            bucket,
            file_name,
        );
    } else if (type === 'AZURE') {
        cloudProperties = await blobops.getPropertyBlob(
            bucket,
            file_name,
            throwError,
        );
    ...
    const noobaaProperties = await getPropertiesFromS3Bucket(
        s3ops,
        noobaa_bucket,
        file_name,
    );
    ...
    if (cloudMD5 === noobaaMD5) {
        console.log(
            `Noobaa bucket (${noobaa_bucket}) contains the md5 ${
                noobaaMD5
            } and the cloud md5 is: ${JSON.stringify(cloudProperties)} for file ${file_name}`,
        );
    } else {
        console.warn(
            `file: ${file_name} size is ${cloudProperties.size} on ${
                type
            } and ${noobaaProperties.size} on noobaa`,
        );
        throw new Error(
            `Noobaa bucket (${noobaa_bucket}) contains the md5 ${
                noobaaMD5
            } instead of ${cloudProperties} for file ${file_name}`,
        );
    }
}
...
async function uploadDataSetToCloud(type, bucket) {
    for (const size of dataSet) {
        ...
        if (type === 'AWS') {
            await s3opsAWS.put_file_with_md5(
                bucket,
                file_name,
                size.data_size,
                data_multiplier,
            ...
            await blobops.uploadRandomFileDirectlyToAzure(
                bucket,
                file_name,
                actual_size,
                throwError,
...
async function upload_directly_to_cloud(type) {
    ...
    try {
        if (type === 'AWS') {
            ...
            await blobops.uploadRandomFileDirectlyToAzure(
                namespace_mapping.AZURE.bucket2,
                file_name,
                actual_size,
                throwError,
            );
...
async function isFilesAvailableInNooBaaBucket(gateway, files, type) {
    console.log(
        `Checking uploaded files ${files} in noobaa s3 server bucket ${gateway}`,
    );
    ...
    for (const file of files) {
        if (keys.includes(file)) {
            ...
            throw new Error(
                `Server is not contains uploaded file ${file} in bucket ${gateway}`,
            );
        }
    }
    if (!report_fail) {
...
async function _create_resource(type) {
    ...
    try {
        ...
        await cf.createNamespaceResource(
            connections_mapping[type].name,
            namespace_mapping[type].namespace,
            namespace_mapping[type].bucket2,
        );
...
async function _upload_via_cloud_check_via_noobaa(type) {
    try {
        ...
        await bucket_functions.createNamespaceBucket(
            namespace_mapping[type].gateway,
            namespace_mapping[type].namespace,
        );
    ...
    await isFilesAvailableInNooBaaBucket(
        namespace_mapping[type].gateway,
        files_cloud[`files_${type}`],
        type,
    );
    for (const file of files_cloud[`files_${type}`]) {
        try {
            await compereMD5betweenCloudAndNooBaa(
                type,
                namespace_mapping[type].bucket2,
                namespace_mapping[type].gateway,
                file,
...
async function list_files_in_cloud(type) {
    let list_files = [];
    if (type === 'AWS') {
        const list_files_obj = await s3opsAWS.get_list_files(
            namespace_mapping[type].bucket2,
        );
        list_files = list_files_obj.map(file => file.Key);
    } else if (type === 'AZURE') {
        list_files = await blobops.getListFilesAzure(
            namespace_mapping[type].bucket2,
            throwError,
        );
    }
...
async function _upload_via_noobaa_check_via_cloud({ type, file_name, bucket }) {
    ...
    const uploaded_file_name = await upload_via_noobaa({
        type,
        file_name,
        bucket,
    });
...
async function update_read_write_and_check(
    clouds,
    name,
    read_resources,
    write_resource,
    ...
    try {
        await bucket_functions.updateNamesapceBucket(
            name,
            write_resource,
            read_resources,
        );
    ...
    console.error(
        `${RED}TODO: REMOVE THIS DELAY, IT IS TEMP OVERRIDE FOR BUG #4831${NC}`,
    );
    const uploaded_file_name = await upload_via_noobaa({
        type: run_on_clouds[0],
        bucket: name,
    });
    ...
    for (let cycle = 0; cycle < run_on_clouds.length; cycle++) {
        ...
        try {
            should_fail = await check_via_cloud(
                run_on_clouds[cycle],
                uploaded_file_name,
            );
        } catch (e) {
        ...
        if (should_fail) {
            throw new Error(
                `Upload succeed To the read only cloud (${run_on_clouds[cycle]}) while it shouldn't`,
            );
        }
    }
}
...
async function add_and_remove_resources(clouds) {
    ...
    await _upload_via_noobaa_check_via_cloud({
        type: 'AWS',
        bucket: noobaa_bucket_name,
    });
    read_resources.push(namespace_mapping.AZURE.namespace);
    for (let cycle = 0; cycle < run_on_clouds.length; cycle++) {
        await update_read_write_and_check(
            run_on_clouds,
            noobaa_bucket_name,
            read_resources,
            namespace_mapping[run_on_clouds[0]].namespace,
        );
        for (
            let read_only_clouds = 1;
            read_only_clouds < run_on_clouds.length;
            read_only_clouds++
        ) {
            await list_cloud_files_read_via_noobaa(
                run_on_clouds[read_only_clouds],
                noobaa_bucket_name,
            );
        }
        run_on_clouds.push(run_on_clouds.shift());
        console.log(
            `cycle: ${cycle} number of clouds: ${run_on_clouds.length}`,
        );
    }
    ...
    if (keys) {
        for (const file of keys) {
            try {
                ...
                console.error(
                    `${RED}TODO: REMOVE THIS TRY CATCH, IT IS TEMP OVERRIDE FOR BUG #4832${NC}`,
                );
            }
        }
    }
...
async function main(clouds) {
    try {
        ...
        for (const type of clouds) {
            ...
            await _upload_via_noobaa_check_via_cloud({
                type,
                file: 'file_azure_15KB',
            });
            await _clean_namespace_bucket(
                namespace_mapping[type].gateway,
                type,
            );
        }
