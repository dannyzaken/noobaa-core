...
async function test_case_large_file_not_cached({ type, ns_context }) {
  ...
  const cloud_obj_md = await ns_context.get_size_etag_via_cloud(
    type,
    file_name,
  );
...
test_case_large_file_not_cached.desc =
  'large file not cached in upload, normal and range read';
