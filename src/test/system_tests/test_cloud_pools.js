...
function list_all_s3_objects(bucket_name) {
    ...
    return P.pwhile(
        ...
        console.log(
            'list_all_s3_objects - current cloud objects:',
            listObjectsResponse.objects,
        );
...
function verify_object_parts_on_cloud_nodes(
    replicas_in_tier,
    bucket_name,
    object_key,
    cloud_pool,
        ...
        function () {
            ...
            return client.object
                ...
                .then(function (obj_mapping_arg) {
                    ...
                    _.forEach(obj_mapping_arg.parts, part => {
                        ...
                        _.forEach(part.chunk.frags, frag =>
                            _.forEach(frag.blocks, block => {
                                if (
                                    String(cloud_pool) ===
                                    String(block.adminfo.pool_name)
                                ) {
                                    blocks_by_cloud_pool_name.blocks.push(
                                        block,
                                    );
                                }
                            }),
                    ...
                    if (
                        blocks_correct &&
                        blocks_by_cloud_pool_name.blocks.length ===
                            obj_mapping_arg.parts.length
                    ) {
                        console.log(
                            'verify_object_parts_on_cloud_nodes blocks found:',
                            util.inspect(blocks_by_cloud_pool_name, {
                                depth: null,
                            }),
                        );
                        ...
                        if (diff > abort_timeout_sec * 1000) {
                            throw new Error(
                                'aborted verify_object_parts_on_cloud_nodes after ' +
                                    abort_timeout_sec +
                                    ' seconds',
                            );
                        }
        ...
        authenticate()
            ...
            .then(function (source_bucket) {
                ...
                return client.tier
                    ...
                    .then(function (tier) {
                        ...
                        const new_pools = tier.attached_pools.concat(
                            TEST_CTX.cloud_pool_name,
                        );
                    ...
                    .then(function () {
                        return client.tier
                            ...
                            .then(function (tier) {
                                replicas_in_tier =
                                    tier.chunk_coder_config.replicas;
                                files_bucket_tier = tier;
                            });
                    });
            })
            .then(function () {
                return P.map_one_by_one(file_names, function (fname) {
                    return basic_server_ops
                        .generate_random_file(file_sizes[0])
                        .then(function (fl) {
                            return basic_server_ops.upload_file(
                                TEST_CTX.source_ip,
                                fl,
                                TEST_CTX.source_bucket,
                                fname,
                            );
                        });
                });
            ...
            .then(() =>
                verify_object_parts_on_cloud_nodes(
                    replicas_in_tier,
                    TEST_CTX.source_bucket,
                    file_names[0],
                    TEST_CTX.cloud_pool_name,
            ...
            .then(() =>
                ...
                client.host
                    .list_hosts({
                        query: {
                            pools: [TEST_CTX.cloud_pool_name],
                            include_all: true,
                        },
                    })
                    .then(list_reply => {
                        const node_name = _.get(
                            list_reply,
                            'hosts.0.storage_nodes_info.nodes.0.name',
                        );
                        ...
                        console.log(
                            `TEST_CTX.cloud_pool_id = ${TEST_CTX.cloud_pool_id} `,
                        );
                    }),
            )
            .then(() =>
                put_object(
                    s3,
                    TEST_CTX.target_bucket,
                    `noobaa_blocks/${TEST_CTX.cloud_pool_id}/blocks_tree/j3n14.blocks/m4g1c4l5l0th`,
                ),
            )
            .then(function (block_ids) {
                return test_utils
                    .blocks_exist_on_cloud(
                        true,
                        TEST_CTX.cloud_pool_id,
                        TEST_CTX.target_bucket,
                        _.map(block_ids.blocks, block => block.block_md.id),
                        s3,
                    )
                    .then(() => block_ids);
            })
            .then(function (block_ids) {
                return (
                    P.ninvoke(
                        ...
                        .then(() =>
                            test_utils.blocks_exist_on_cloud(
                                false,
                                TEST_CTX.cloud_pool_id,
                                TEST_CTX.target_bucket,
                                _.map(
                                    block_ids.blocks,
                                    block => block.block_md.id,
                                ),
                                s3,
                            ),
                        )
                        .catch(err => {
                            console.error(err);
                            throw new Error(
                                'deleteObject::Blocks still on cloud',
                            );
                        })
                );
            })
            ...
            .then(function (block_ids) {
                const new_pools = _.filter(
                    files_bucket_tier.attached_pools,
                    pool => String(pool) !== TEST_CTX.cloud_pool_name,
                );
                // This is used in order to make sure that the blocks will be deleted from the cloud
                return client.tier
                    .update_tier({
                    ...
                    .then(() =>
                        test_utils.blocks_exist_on_cloud(
                            ...
                            TEST_CTX.target_bucket,
                            _.map(
                                block_ids.second_blocks.blocks,
                                block => block.block_md.id,
                            ),
                            s3,
                        ),
                    ...
                    .catch(err => {
                        console.error(err);
                        throw new Error(
                            'Remove Cloud Resource Policy::Blocks still on cloud',
                        );
                    })
                    .then(() =>
                        P.ninvoke(s3, 'headObject', {
                            Bucket: TEST_CTX.target_bucket,
