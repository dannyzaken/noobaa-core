...
async function ceph_test_setup() {
  ...
  try {
    ...
    console.log(
      'Failed to create account or tenant, assuming they were already created and continuing. ',
      err.message,
    );
  }
  system = await client.system.read_system();
  const ceph_account = system.accounts.find(
    account => account.email.unwrap() === CEPH_TEST.new_account_params.email,
  );

  const ceph_account_tenant = system.accounts.find(
    account =>
  ...
  await os_utils.exec(
    `echo access_key = ${access_key.unwrap()} >> ${CEPH_TEST.test_dir}${CEPH_TEST.ceph_config}`,
  );
  await os_utils.exec(
    `echo secret_key = ${secret_key.unwrap()} >> ${CEPH_TEST.test_dir}${CEPH_TEST.ceph_config}`,
  );

  const { access_key: access_key_tenant, secret_key: secret_key_tenant } =
    ceph_account_tenant.access_keys[0];
  if (os_utils.IS_MAC) {
    await os_utils.exec(
      `sed -i "" "s|tenant_access_key|${access_key_tenant.unwrap()}|g" ${CEPH_TEST.test_dir}${CEPH_TEST.ceph_config}`,
    );
    await os_utils.exec(
      `sed -i "" "s|tenant_secret_key|${secret_key_tenant.unwrap()}|g" ${CEPH_TEST.test_dir}${CEPH_TEST.ceph_config}`,
    );
  } else {
    await os_utils.exec(
      `sed -i -e 's:tenant_access_key:${access_key_tenant.unwrap()}:g' ${CEPH_TEST.test_dir}${CEPH_TEST.ceph_config}`,
    );
    await os_utils.exec(
      `sed -i -e 's:tenant_secret_key:${secret_key_tenant.unwrap()}:g' ${CEPH_TEST.test_dir}${CEPH_TEST.ceph_config}`,
    );
    await os_utils.exec(
      `sed -i -e 's:s3_access_key:${access_key.unwrap()}:g' ${CEPH_TEST.test_dir}${CEPH_TEST.ceph_config}`,
    );
    await os_utils.exec(
      `sed -i -e 's:s3_secret_key:${secret_key.unwrap()}:g' ${CEPH_TEST.test_dir}${CEPH_TEST.ceph_config}`,
    );
  }
}
