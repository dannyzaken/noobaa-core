...
async function ceph_test_setup() {
  ...
  await os_utils.exec(
    `echo access_key = ${access_key} >> ${CEPH_TEST.test_dir}${CEPH_TEST.ceph_config}`,
  );
  await os_utils.exec(
    `echo secret_key = ${secret_key} >> ${CEPH_TEST.test_dir}${CEPH_TEST.ceph_config}`,
  );
  ...
  if (os_utils.IS_MAC) {
    await os_utils.exec(
      `sed -i "" "s|tenant_access_key|"${access_key_tenant}"|g" ${CEPH_TEST.test_dir}${CEPH_TEST.ceph_config}`,
    );
    await os_utils.exec(
      `sed -i "" "s|tenant_secret_key|${secret_key_tenant}|g" ${CEPH_TEST.test_dir}${CEPH_TEST.ceph_config}`,
    );
  } else {
    await os_utils.exec(
      `sed -i -e 's:tenant_access_key:${access_key_tenant}:g' ${CEPH_TEST.test_dir}${CEPH_TEST.ceph_config}`,
    );
    await os_utils.exec(
      `sed -i -e 's:tenant_secret_key:${secret_key_tenant}:g' ${CEPH_TEST.test_dir}${CEPH_TEST.ceph_config}`,
    );
    await os_utils.exec(
      `sed -i -e 's:s3_access_key:${access_key}:g' ${CEPH_TEST.test_dir}${CEPH_TEST.ceph_config}`,
    );
    await os_utils.exec(
      `sed -i -e 's:s3_secret_key:${secret_key}:g' ${CEPH_TEST.test_dir}${CEPH_TEST.ceph_config}`,
    );
  }
}
...
async function get_access_keys(path) {
  ...
  const secret_key = await nc_mkm.decrypt(
    encrypted_secret_key,
    data_json.master_key_id,
  );
