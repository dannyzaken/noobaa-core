...
async function setup() {
  ...
  await Promise.all(
    full_access_user.allowed_buckets.permission_list
      .map(bucket =>
        test_utils.generate_s3_policy(full_access_user.email, bucket, ['s3:*']),
      )
      .map(generated =>
        client.bucket.put_bucket_policy({
...
async function test_bucket_write_denied() {
  ...
  try {
    ...
    throw new Error(
      'expecting upload to fail with statusCode 403- AccessDenied',
    );
  } catch (err) {
    assert(
      err.statusCode === 403,
...
async function test_bucket_read_denied() {
  ...
  try {
    ...
    assert(
      err.statusCode === 403,
      'expecting read to fail with statusCode 403- AccessDenied',
    );
  }
}
...
async function test_create_bucket_add_creator_permissions() {
  ...
  const bucket = await client.bucket.read_bucket({
    rpc_params: { name: unique_bucket_name },
  });
  assert(
    bucket.owner_account.email.unwrap() === full_access_user.email,
    'expecting full_access_user to have permissions to access ' +
      unique_bucket_name,
  );
}
...
async function test_delete_bucket_deletes_permissions() {
  ...
  try {
    await client.bucket.read_bucket({
      rpc_params: { name: unique_bucket_name },
    });
    ...
    assert(
      !error,
      'expecting full_access_user to not have permissions to access ' +
        unique_bucket_name,
    );
  }
}
...
async function test_no_s3_access() {
  ...
  assert(
    ...
    'expecting an empty bucket list for no_access_user',
  );
}
...
async function test_ip_restrictions() {
  ...
  assert(
    data.Buckets.length !== 0,
    'expecting none empty bucket list for none-restricted IP',
  );
