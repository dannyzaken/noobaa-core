...
function control_services(command, services) {
    return os_utils
        .exec(`supervisorctl ${command} ${(services || []).join(' ')}`, {
            ignore_rc: false,
            return_stdout: true,
        })
        .then(res => {
...
async function prepare_buckets_with_objects() {
    ...
    for (let cycle = 0; cycle < CYCLES_TO_TEST; ++cycle) {
        ...
        await Promise.all(
            buckets_used.map(async bucket_obj => {
                const fkey = await upload_file_to_bucket(
                    bucket_obj.bucket_name,
                );
                const bucket_f = buckets_used.find(
                    b =>
                        String(b.bucket_name) ===
                        String(bucket_obj.bucket_name),
                );
...
function calculate_expected_storage_stats_for_buckets(
    buckets_array,
    storage_read_by_bucket,
) {
        ...
        return P.map_one_by_one(bucket.file_names, function (file_name) {
            return client.object
                ...
                .then(res => {
                    ...
                    current_bucket_storage.chunks_capacity += _.sum(
                        _.map(res.chunks, chunk => chunk.compress_size || 0),
                    );
                });
        }).then(() => {
            if (
                current_bucket_storage.chunks_capacity !==
                    storage_read_by_bucket[bucket.bucket_name]
                        .chunks_capacity ||
...
function wait_for_s3_and_web(max_seconds_to_wait) {
    return P.all([
        wait_for_server_to_start(
            max_seconds_to_wait,
            String(process.env.ENDPOINT_PORT || 80),
        ),
        wait_for_server_to_start(
            max_seconds_to_wait,
...
function wait_for_mongodb_to_start(max_seconds_to_wait) {
    ...
    return P.pwhile(
        ...
        function () {
            return os_utils
                .exec('supervisorctl status mongo_wrapper', {
                    ignore_rc: false,
                    return_stdout: true,
                })
                .then(function (res) {
                ...
                .catch(function (err) {
                    ...
                    if (wait_counter >= MAX_RETRIES) {
                        console.error(
                            'Too many retries after restart mongodb',
                            err,
                        );
...
function wait_for_server_to_start(max_seconds_to_wait, port) {
    ...
    return P.pwhile(
        ...
        function () {
            return http_utils
                .http_get('http://localhost:' + port)
                .then(function () {
                    console.log(
                        'server started after ' + wait_counter + ' seconds',
                    );
                    isNotListening = false;
                })
                .catch(function (err) {
                    ...
                    if (wait_counter >= MAX_RETRIES) {
                        console.error(
                            'Too many retries after restart server',
                            err,
                        );
                        throw new Error('Too many retries');
                    }
