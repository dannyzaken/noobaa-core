...
async function run({ dbg, system_store, system_server }) {
  try {
    ...
    for (const bucket of system_store.data.buckets) {
      ...
      const new_rules = bucket.lifecycle_configuration_rules.map(
        lifecycle_configuration_rules => ({
          ...
          abort_incomplete_multipart_upload:
            lifecycle_configuration_rules.abort_incomplete_multipart_upload,
          transition: lifecycle_configuration_rules.transition,
          noncurrent_version_expiration:
            lifecycle_configuration_rules.noncurrent_version_expiration,
          noncurrent_version_transition:
            lifecycle_configuration_rules.noncurrent_version_transition,
        }),
      );
    ...
    if (buckets.length > 0) {
      dbg.log0(
        `Replacing lifecycle rules to the new API structure for buckets: ${buckets.map(bucket => util.inspect(bucket)).join(', ')}`,
      );
