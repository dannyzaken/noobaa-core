...
async function run({ dbg, system_store }) {
    try {
        const operator_account = system_store.data.accounts.find(
            account => account.email.unwrap() === config.OPERATOR_ACCOUNT_EMAIL,
        );
        const operator_account_id = operator_account._id;
        dbg.log0(
            'Assigning account ID ',
            operator_account_id,
            ' as owner_id to all pools and resources',
        );
...
module.exports = {
    run,
    description:
        'Assign an owner_id to all resources and pools based on the ID of the system owner',
};
