...
async function run({ dbg, system_store, system_server }) {
    try {
            ...
            .map(
                s =>
                    s.namespace && {
                        ...
                        $set: {
                            namespace: {
                                read_resources: s.namespace.read_resources.map(
                                    nsr => ({ resource: nsr._id }),
                                ),
                                write_resource: {
                                    resource: s.namespace.write_resource._id,
                                },
        ...
        if (namespace_buckets.length > 0) {
            dbg.log0(
                `adding new namespace_bucket structure for these namespace buckets: ${namespace_buckets.map(b => util.inspect(b)).join(', ')}`,
            );
            await system_store.make_changes({
                update: { buckets: namespace_buckets },
            });
        } else {
