...
async function run({ dbg, system_store }) {
    try {
        ...
        if (systems.length > 0) {
            dbg.log0(
                `removing default_chunk_config to these systems: ${systems.map(b => b._id).join(', ')}`,
            );
            await system_store.make_changes({ update: { systems } });
        } else {
            dbg.log0(
                'there are no systems that need default_chunk_config removal',
            );
        }
    } catch (err) {
        dbg.error(
            'got error while removing default_chunk_config for systems:',
            err,
        );
        throw err;
    }
