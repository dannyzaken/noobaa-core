    ...
    switch (this._state) {
      case STATE_INIT:
        ...
        this._connect_timeout = setTimeout(
          () =>
            this.emit_error(
              new RpcError('RPC_CONNECT_TIMEOUT', 'RPC CONNECT TIMEOUT'),
            ),
      ...
      default:
        throw new Error(
          'RPC CONN ON WEIRD STATE ' + this._state + ' ' + this.connid,
        );
    }
  }
  ...
  close(err) {
    ...
    if (this.connecting_defer) {
      this.connecting_defer.reject(
        err ||
          new RpcError('RPC_CONN_CLOSED', 'RPC CONN CLOSED ' + this.connid),
      );
      this.connecting_defer = null;
    }
    this._close();
