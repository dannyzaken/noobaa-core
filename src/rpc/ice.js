...
const RAND_ICE_CHAR_POOL_64 = `abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRTTUVWXYZ0123456789+/`;
...
function Ice(connid, n2n_config, signal_target) {
  ...
  _.each(os.networkInterfaces(), function (interfaces, name) {
    _.each(interfaces, function (n) {
      ...
      if (n.address === ip_module.address() && self.config.public_ips.length) {
  ...
  const activate_packet = stun.new_packet(
    ...
    [
      ...
      },
      {
        ...
        value:
          this.remote_credentials.ufrag + ':' + this.local_credentials.ufrag,
      },
      {
