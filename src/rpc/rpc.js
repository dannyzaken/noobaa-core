...
class RPC extends EventEmitter {
    ...
    try {
      ...
      await P.timeout(
        ...
        () =>
          new RpcError(
            'RPC_REQUEST_TIMEOUT',
            'RPC REQUEST TIMEOUT while sending request ' + req.base_info,
          ),
      );
      ...
      const reply = await P.timeout(
        ...
        () =>
          new RpcError(
            'RPC_REQUEST_TIMEOUT',
            'RPC REQUEST TIMEOUT while waiting for response ' + req.base_info,
          ),
      );
  ...
  async _on_request(conn, msg) {
    ...
    try {
      ...
      if (err instanceof RpcError) {
        ...
        req.error = new RpcError(err.rpc_code || 'INTERNAL', err.message, {
          retryable: true,
        });
        req.error.stack = ''; // don't print this stack
      }
  ...
  _get_connection(addr_url, srv) {
    ...
    if (!conn) {
      if (this.n2n_proxy && !this.n2n_agent && addr_url.protocol === 'n2n:') {
  ...
  _accept_new_connection(conn) {
    if (this._disconnected_state) {
      conn.close(
        new RpcError(
          'RPC_DISCONNECTED_STATE',
          'RPC IN DISCONNECTED STATE - rejecting connection ' + conn.connid,
        ),
      );
      return;
    }
        ...
        if (conn._ping_reqid_set && conn._ping_reqid_set.has(msg.body.reqid)) {
          ...
          dbg.warn(
            `RPC PINGPONG MISMATCH pings ${Array.from(
              conn._ping_reqid_set,
            ).join(',')} pong ${msg.body.reqid} connid ${conn.connid}`,
          );
        }
        break;
      }
