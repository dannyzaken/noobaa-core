...
if (argv.help || (!argv.server && !argv.client && !argv.fcall)) {
    ...
    if (argv.client || argv.server) {
        process.stdout.write(
            'fcall mode is mutually exclusive with server, client modes\n',
        );
        print_usage_and_exit(1);
    } else if (argv.n2n || argv.addr) {
        process.stdout.write(
            'fcall mode does not support the n2n or the addr flags\n',
        );
        print_usage_and_exit(1);
    }
}
...
argv.addr.protocol =
    (argv.proto && argv.proto + ':') || argv.addr.protocol || 'ws:';
...
function print_usage_and_exit(exit_code) {
    ...
    process.stdout.write(
        '    node ' +
            script_name +
            ' --server --addr tcp://server:5656 [--n2n] \n',
    );
    process.stdout.write(
        '    node ' +
            script_name +
            ' --client --addr tcp://server:5656 [--n2n] \n',
    );
    ...
    try {
        ...
        if (argv.n2n) {
            ...
            const n2n_agent = rpc.register_n2n_agent((...args) =>
                client.rpcbench.n2n_signal(...args),
            );
            n2n_agent.set_any_rpc_address();
        } else {
            target_addresses = [
                url.format(argv.fcall ? 'fcall://fcall' : argv.addr),
            ];
        }
...
function report() {
    ...
    dbg.log0(
        ...
        ' |||  Read ',
        (v_rbytes / MB).toFixed(3),
        'MB  (~' + (v_rbytes_start / MB).toFixed(3) + ')',
        ' |||  Write ',
        (v_wbytes / MB).toFixed(3),
