...
class HostedAgents {
    ...
    reload() {
        ...
        const agents_to_start = system_store.data.pools.filter(
            pool =>
                !_.isUndefined(pool.cloud_pool_info) ||
                !_.isUndefined(pool.mongo_pool_info),
        );
        dbg.log0(
            `will start agents for these pools: ${util.inspect(agents_to_start)}`,
        );
    ...
    stop() {
        ...
        _.each(this._started_agents, (agent, node_name) =>
            this._stop_agent(node_name),
        );
    }
    ...
    _monitor_stats() {
        P.pwhile(
            ...
            () => {
                ...
                dbg.log0(
                    `hosted_agent_stats_titles - process: cpu_usage_user, cpu_usage_sys, mem_usage_rss`,
                );
                dbg.log0(
                    `hosted_agent_stats_values - process: ${cpu_usage.user}, ${cpu_usage.system}, ${mem_usage.rss}`,
                );
                for (const agent of Object.keys(this._started_agents)) {
                    const agent_stats =
                        this._started_agents[agent].agent.sample_stats();
                    if (agent_stats) {
                        const agent_stats_keys = Object.keys(agent_stats);
                        dbg.log0(
                            `hosted_agent_stats_titles - ${agent}: ` +
                                agent_stats_keys.join(', '),
                        );
                        dbg.log0(
                            `hosted_agent_stats_values - ${agent}: ` +
                                agent_stats_keys
    ...
    async _start_pool_agent(pool) {
        ...
        if (config.DB_TYPE !== 'mongodb' && pool.resource_type === 'INTERNAL') {
            return;
        }
        ...
        if (this._started_agents[node_name]) {
            dbg.warn(
                `agent for ${pool.name}, id: ${pool._id} already started. skipping _start_pool_agent`,
            );
            return;
        }
        ...
        const pool_property_path =
            pool.resource_type === 'INTERNAL' ?
                'mongo_pool_info.agent_info.mongo_path'
            :   'cloud_pool_info.agent_info.cloud_path';
        const pool_path = _.get(
            pool,
            pool_property_path,
            `noobaa_blocks/${pool_id}`,
        );
        const pool_path_property =
            pool.resource_type === 'INTERNAL' ? 'mongo_path' : 'cloud_path';
        const pool_info_property =
            pool.resource_type === 'INTERNAL' ? 'mongo_info' : 'cloud_info';
        ...
        const { token_wrapper, create_node_token_wrapper } =
            _get_pool_token_wrapper(pool);
        const info =
            pool.resource_type === 'INTERNAL' ?
                pool.mongo_pool_info
            :   pool.cloud_pool_info;
        if (!info.agent_info || !info.agent_info.create_node_token) {
            const existing_token =
                info.agent_info ? info.agent_info.node_token : null;
            const pool_agent_path =
                pool.resource_type === 'INTERNAL' ?
                    'mongo_pool_info'
                :   'cloud_pool_info';
            const update = {
            ...
            if (!info.agent_info || !info.agent_info[pool_path_property]) {
                update.pools[0][`${pool_agent_path}.agent_info`][
                    pool_path_property
                ] = pool_path;
            }
        ...
        if (pool.cloud_pool_info && pool.cloud_pool_info.storage_limit) {
            agent_params.storage_limit = pool.cloud_pool_info.storage_limit;
        }
        if (pool.cloud_pool_info && pool.cloud_pool_info.aws_sts_arn) {
            agent_params.aws_sts_arn = pool.cloud_pool_info.aws_sts_arn;
        }
    ...
    start_local_agent(params) {
        ...
        const token_wrapper = {
            ...
            write: token =>
                fs_utils.replace_file(path.join(storage_path, 'token'), token),
        };
        ...
        return fs_utils
            .create_path(storage_path, fs_utils.PRIVATE_DIR_PERMISSIONS)
    ...
    async _stop_agent(node_name, should_clean) {
        ...
        if (agent) {
            ...
            try {
                ...
                dbg.error(
                    'failed on agent cleanup. continue with agent deletion',
                    err,
                );
            }
        }
    ...
    update_agents_credentials({ pool_ids, access_keys }) {
        for (const pid of pool_ids) {
            ...
            if (!this._started_agents[node_name]) {
                dbg.error(
                    `update_agents_credentials agent ${node_name} does not exist`,
                );
                continue;
            }
            const agent = this._started_agents[node_name].agent;
    ...
    update_agents_storage_limit({ pool_ids, storage_limit }) {
        for (const pid of pool_ids) {
            ...
            if (!this._started_agents[node_name]) {
                dbg.error(
                    `update_storage_limit agent ${node_name} does not exist`,
                );
                continue;
            }
            const agent = this._started_agents[node_name].agent;
...
function create_pool_agent(req) {
    return HostedAgents.instance()._start_pool_agent(
        req.system.pools_by_name[req.params.pool_name],
    );
}
...
async function update_credentials(req) {
    ...
    try {
        await HostedAgents.instance().update_agents_credentials({
            pool_ids,
...
function remove_pool_agent(req) {
    ...
    if (!node_name) {
        ...
        if (!pool) {
            throw new Error(
                'could not find pool by name ' + req.params.pool_name,
            );
        }
        node_name = 'noobaa-internal-agent-' + pool._id;
    }
    return HostedAgents.instance()._stop_agent(node_name, true);
}
...
function _get_pool_token_wrapper(token_pool) {
    const read_token = token_key => {
        ...
        const pool_agent_info = _.get(
            pool_and_path.pool,
            pool_and_path.pool_property_path,
        );
        if (!pool_agent_info) {
            throw new Error(`Pool ${token_pool.name} was not initialised`);
        }
    ...
    const write_token = (new_token, token_key) => {
        ...
        const pool_and_path = _get_pool_and_path_for_token(token_pool);
        db_update[`${pool_and_path.pool_property_path}.${token_key}`] =
            new_token;
...
function _get_pool_and_path_for_token(token_pool) {
    ...
    if (!pool) {
        throw new Error(
            `Pool ${token_pool.name}, ${token_pool._id} does not exist`,
        );
    }
    const pool_property_path =
        pool.resource_type === 'INTERNAL' ?
            'mongo_pool_info.agent_info'
...
exports.create_agent = req =>
    HostedAgents.instance().start_local_agent(req.params);
exports.remove_agent = req =>
    HostedAgents.instance()._stop_agent(req.params.name, true);
