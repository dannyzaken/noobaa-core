...
async function validate_input_types(type, action, argv) {
    ...
    if (action === ACTIONS.UPDATE) {
        validate_min_flags_for_update(type, input_options_with_data);
    }
    ...
    if (
        ...
        validate_flags_value_combination(
            type,
            action,
            input_options_with_data_from_file,
        );
...
function validate_type_and_action(type, action) {
    ...
    if (type === TYPES.ACCOUNT || type === TYPES.BUCKET) {
        ...
        if (!Object.values(DIAGNOSE_ACTIONS).includes(action)) {
            throw_cli_error(ManageCLIError.InvalidDiagnoseAction);
        }
    } else if (type === TYPES.UPGRADE) {
        if (!Object.values(UPGRADE_ACTIONS).includes(action)) {
...
function validate_options_type_by_value(input_options_with_data) {
    for (const [option, value] of Object.entries(input_options_with_data)) {
        ...
        if (type_of_value !== type_of_option) {
            ...
            if (
                BOOLEAN_STRING_OPTIONS.has(option) &&
...
function validate_min_flags_for_update(type, input_options_with_data) {
    ...
    const config_and_identifier_options = [
        'config_root',
        'config_root_backend',
        'name',
    ];
    ...
    if (input_options_with_data.name === undefined) {
        if (type === TYPES.ACCOUNT && !input_options_with_data.anonymous) {
    ...
    const flags_for_update = input_options.filter(
        option => !config_and_identifier_options.includes(option),
    );
    if (
        flags_for_update.length === 0 ||
        (flags_for_update.length === 1 &&
            input_options_with_data.regenerate === 'false')
    ) {
...
function validate_flags_combination(type, action, input_options) {
    ...
    if (type === TYPES.ACCOUNT) {
        if (action === ACTIONS.ADD || action === ACTIONS.UPDATE) {
            if (
                input_options_set.has('user') &&
                (input_options_set.has('uid') || input_options_set.has('gid'))
            ) {
                const detail =
                    'Please use --uid and --gid flags (or only --user flag)';
                throw_cli_error(ManageCLIError.InvalidFlagsCombination, detail);
            }
        }
        if (action === ACTIONS.LIST) {
            if (
                input_options_set.has('show_secrets') &&
                !input_options_set.has('wide')
            ) {
                const detail =
                    'Please use --show_secrets and --wide flags together (or only --wide flag)';
                throw_cli_error(ManageCLIError.InvalidFlagsCombination, detail);
            }
        }
    }
 ...
 * @param {object} input_options_with_data
 */
function validate_flags_value_combination(
    type,
    action,
    ...
    if (type === TYPES.ACCOUNT) {
        if (action === ACTIONS.ADD || action === ACTIONS.UPDATE) {
            if (
                input_options_with_data.name === ANONYMOUS ||
                input_options_with_data.new_name === ANONYMOUS
            ) {
...
function validate_account_name(type, action, input_options_with_data) {
    ...
    try {
        if (action === ACTIONS.ADD) {
            ...
            input_options_with_data.new_name !== undefined
        ) {
...
function validate_bucket_identifier(action, input_options) {
    if (
        ...
        if (input_options.name === undefined) {
            throw_cli_error(ManageCLIError.MissingBucketNameFlag);
        }
    }
...
async function check_new_name_exists(type, config_fs, action, data) {
    ...
    if (type === TYPES.BUCKET) {
        ...
        if (exists) {
            throw_cli_error(ManageCLIError.BucketAlreadyExists, new_name, {
                bucket: new_name,
            });
        }
    } else if (type === TYPES.ACCOUNT) {
        ...
        if (exists) {
            throw_cli_error(ManageCLIError.AccountNameAlreadyExists, new_name, {
                account: new_name,
            });
        }
    }
...
async function check_new_access_key_exists(config_fs, action, data) {
    ...
    const exists =
        await config_fs.is_account_exists_by_access_key(new_access_key);
    if (exists) {
        throw_cli_error(
            ManageCLIError.AccountAccessKeyAlreadyExists,
            new_access_key,
            { account: new_access_key },
        );
    }
}
...
async function validate_bucket_args(config_fs, data, action) {
    if (action === ACTIONS.ADD || action === ACTIONS.UPDATE) {
        ...
        if (action === ACTIONS.UPDATE && data.new_name !== undefined) {
            native_fs_utils.validate_bucket_creation({ name: data.new_name });
        }
        if (action === ACTIONS.ADD && data.owner_account === undefined) {
        ...
        const owner_account_data = await get_bucket_owner_account(
            config_fs,
            undefined,
            data.owner_account,
        );
        const account_fs_context = await native_fs_utils.get_fs_context(
            owner_account_data.nsfs_account_config,
        ...
        if (!config.NC_DISABLE_ACCESS_CHECK) {
            ...
            if (!accessible) {
                throw_cli_error(
                    ManageCLIError.InaccessibleStoragePath,
                    data.path,
                );
            }
        }
        if (action === ACTIONS.ADD) {
            if (!owner_account_data.allow_bucket_creation) {
                const detail_msg =
                    `${owner_account_data.name} account not allowed to create new buckets. ` +
                    `Please make sure to have a valid new_buckets_path and enable the flag allow_bucket_creation`;
                throw_cli_error(
                    ManageCLIError.BucketCreationNotAllowed,
                    detail_msg,
                );
            }
        }
        if (owner_account_data.owner) {
            ...
            throw_cli_error(
                ManageCLIError.BucketSetForbiddenBucketOwnerIsIAMAccount,
                detail_msg,
        ...
        if (data.s3_policy) {
            try {
                await bucket_policy_utils.validate_s3_policy(
                    ...
                    data.name,
                    async principal =>
                        config_fs.is_account_exists_by_principal(principal, {
                            silent_if_missing: true,
                        }),
                );
            } catch (err) {
                dbg.error(
                    'validate_bucket_args invalid bucket policy err:',
                    err,
                );
...
function validate_account_identifier(action, input_options) {
    ...
    if (action === ACTIONS.STATUS) {
        ...
        if (
            input_options.access_key === undefined &&
            input_options.name === undefined
        ) {
 ...
 */
async function validate_account_args(
    config_fs,
    data,
    action,
    ...
    if (action === ACTIONS.ADD || action === ACTIONS.UPDATE) {
        ...
        if (
            data.nsfs_account_config.distinguished_name === undefined &&
            (data.nsfs_account_config.uid === undefined ||
                data.nsfs_account_config.gid === undefined)
        ) {
            throw_cli_error(
...
async function validate_account_resources_before_deletion(config_fs, data) {
    ...
    if (data.owner === undefined) {
        await check_if_root_account_does_not_have_IAM_users(
            config_fs,
            data,
            ACTIONS.DELETE,
        );
...
async function validate_account_not_owns_buckets(config_fs, account_data) {
    ...
    await P.map_with_concurrency(10, bucket_names, async bucket_name => {
        const data = await config_fs.get_bucket_by_name(bucket_name, {
            silent_if_missing: true,
        });
...
async function check_if_root_account_does_not_have_IAM_users(
    ...
    await P.map_with_concurrency(10, account_names, async account_name => {
        ...
        const is_root_account_owns_user = check_root_account_owns_user(
            account_to_check,
            account_data,
        );
        if (is_root_account_owns_user) {
            ...
            if (action === ACTIONS.DELETE) {
                throw_cli_error(
                    ManageCLIError.AccountDeleteForbiddenHasIAMAccounts,
                    detail_msg,
                );
            }
            // else it is called with action ACTIONS.UPDATE
            throw_cli_error(
                ManageCLIError.AccountCannotBeRootAccountsManager,
                detail_msg,
            );
        }
        return account_data;
...
async function validate_root_accounts_manager_update(config_fs, account) {
    if (account.owner) {
        throw_cli_error(
            ManageCLIError.AccountCannotCreateRootAccountsRequesterIAMUser,
        );
    }
    await check_if_root_account_does_not_have_IAM_users(
        config_fs,
...
function validate_whitelist_ips(ips_to_validate) {
    for (const ip_to_validate of ips_to_validate) {
        if (net.isIP(ip_to_validate) === 0) {
            ...
            throw_cli_error(
                ManageCLIError.InvalidWhiteListIPFormat,
                detail_msg,
            );
        }
    }
...
exports.validate_root_accounts_manager_update =
    validate_root_accounts_manager_update;
