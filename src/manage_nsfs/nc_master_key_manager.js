...
class NCMasterKeysManager {
    ...
    static get_instance() {
        NCMasterKeysManager._instance =
            NCMasterKeysManager._instance || new NCMasterKeysManager();
    ...
    async init() {
        ...
        throw new RpcError(
            INVALID_MASTER_KEY,
            `Invalid Master keys store type - ${store_type} - ${TYPE_EXEC}`,
        );
    }
    ...
    _set_keys(master_keys) {
        if (!master_keys.active_master_key || !master_keys.master_keys_by_id) {
            throw new RpcError(INVALID_MASTER_KEY, 'Invalid master_keys.json');
        }
        for (const [master_key_id, master_key] of Object.entries(
            master_keys.master_keys_by_id,
        )) {
            try {
                this.master_keys_by_id[master_key.id] =
                    get_buffered_master_key(master_key);
                if (master_key_id === master_keys.active_master_key) {
                    this.active_master_key =
                        this.master_keys_by_id[master_key.id];
                }
            } catch (err) {
        ...
        if (!this.active_master_key) {
            throw new RpcError(
                INVALID_MASTER_KEY,
                "Invalid master_keys.json, couldn't find active master key in master_keys_by_id",
            );
        }
    ...
    async _create_master_key() {
        ...
        throw new RpcError(
            INVALID_MASTER_KEY,
            `Invalid Master keys store type - ${store_type}`,
        );
    }
    ...
    async _init_from_file() {
        ...
        for (let retries = 0; retries < 3; ) {
            try {
                ...
                const master_keys = await native_fs_utils.read_file(
                    fs_context,
                    master_keys_path,
                );
                ...
                if (err.code === 'ENOENT') {
                    dbg.warn(
                        "init_from_file: couldn't find master keys file",
                        master_keys_path,
                    );
                    await this._create_master_key();
                } else if (err.rpc_code === INVALID_MASTER_KEY) {
                    dbg.error(
                        'init_from_file: master keys file is invalid',
                        master_keys_path,
                    );
                    throw err;
                } else {
                    dbg.error(
                        "init_from_file: couldn't load master keys file",
                        master_keys_path,
                        err,
                    );
                    retries += 1;
    ...
    _get_master_keys_json(new_master_key) {
        ...
        const master_keys_by_id = {
            ...this.master_keys_by_id,
            [active_master_key_id]: new_master_key,
        };
    ...
    async _create_master_keys_file(new_master_key) {
        ...
        try {
            ...
            dbg.log0(
                '_create_master_keys_file created file successfuly',
                master_keys_path,
            );
        } catch (err) {
    ...
    async _create_master_keys_exec(master_key) {
        ...
        try {
            const put_master_keys_res = await os_util.spawn(
                config.NC_MASTER_KEYS_PUT_EXECUTABLE,
                [EXEC_KEY_SUFFIX, init_version],
                {
                    shell: '/bin/sh',
                    input: master_keys_json,
                    stdio: [],
                    return_stdout: true,
                },
            );
            ...
            if (status === EXEC_STATUS_OK) {
                ...
                dbg.warn(
                    `_create_master_keys_exec: master keys file already exist status=${status}`,
                );
            } else {
                throw new Error(
                    `_create_master_keys_exec: failed with error=${status}`,
                );
            }
        } catch (err) {
    ...
    async _init_from_exec() {
        ...
        for (let retries = 0; retries < config.MASTER_KEYS_EXEC_MAX_RETRIES; ) {
            try {
                if (
                    this.last_init_time &&
                    new Date().getTime() - this.last_init_time >
                        config.NC_MASTER_KEYS_MANAGER_REFRESH_THRESHOLD
                ) {
                    dbg.log1(
                        '_init_from_exec: cache is updated nothing to do, skipping...',
                    );
                    return;
                }
                dbg.log1(
                    '_init_from_exec: calling config.NC_MASTER_KEYS_GET_EXECUTABLE script',
                );
                const get_master_keys_res = await os_util.exec(command, {
                    return_stdout: true,
                });
                const { status, version, data } =
                    JSON.parse(get_master_keys_res);
                if (status === EXEC_STATUS_OK) {
                    dbg.log0(
                        `init_from_exec: get master keys response status=${status}, version=${version}`,
                    );
                    this._set_keys(data);
                    this.last_init_time = new Date().getTime();
                    dbg.log1(
                        '_init_from_exec: updating this.last_init_time',
                        this.last_init_time,
                    );
                    return;
                } else if (status === EXEC_STATUS_NOT_FOUND) {
                    dbg.warn(
                        `init_from_exec: get master keys failed with status=${status}, creating a new master key`,
                    );
                    await this._create_master_key();
                } else {
                    throw new RpcError(
                        INVALID_MASTER_KEY,
                        `init_from_exec: get master keys failed with status=${status}`,
                    );
                }
            } catch (err) {
                dbg.error(
                    `init_from_exec: get master keys failed with error=${err} retries=${retries} max_retries=${config.MASTER_KEYS_EXEC_MAX_RETRIES}`,
                );
                retries += 1;
                await P.delay(1000);
            }
    ...
    encryptSync(secret_key, master_key_id = this.active_master_key?.id) {
        ...
        const { cipher_key, cipher_iv, encryption_type } =
            this.master_keys_by_id[master_key_id];
    ...
    decryptSync(secret_key, master_key_id) {
        ...
        const decrypted_secret_key = decipher
            .update(Buffer.from(secret_key, 'base64'))
            .toString();
    ...
    async encrypt_access_keys(account) {
        ...
        const encrypted_access_keys = await P.all(
            _.map(account.access_keys, async access_keys => {
                const encrypted_access_keys_object = { ...access_keys };
                if (encrypted_access_keys_object.secret_key) {
                    encrypted_access_keys_object.encrypted_secret_key =
                        await this.encrypt(
    ...
    async decrypt_access_keys(account) {
        const decrypted_access_keys = await P.all(
            _.map(account.access_keys, async access_keys => {
                const decrypted_access_keys_object = { ...access_keys };
                if (decrypted_access_keys_object.encrypted_secret_key) {
                    decrypted_access_keys_object.secret_key =
                        await this.decrypt(
                            access_keys.encrypted_secret_key,
    ...
    _validate_master_key_manager(master_key_id) {
        if (!this.master_keys_by_id) {
            throw new RpcError(
                INVALID_MASTER_KEY,
                'master_keys_by_id object is undefined',
            );
        }
        if (!master_key_id) {
            throw new RpcError(
                INVALID_MASTER_KEY,
                'Master key id is undefined',
            );
        }
        if (!this.master_keys_by_id[master_key_id]) {
            throw new RpcError(
                INVALID_MASTER_KEY,
...
function get_buffered_master_key(master_key) {
    try {
        ...
        throw new RpcError(
            INVALID_MASTER_KEY,
            'Could not convert master key strings to buffers',
        );
    }
}
...
function get_master_keys_file_path() {
    const default_master_keys_path = path.join(
        config.NSFS_NC_CONF_DIR,
        'master_keys.json',
    );
    const master_keys_path =
        config.NC_MASTER_KEYS_FILE_LOCATION || default_master_keys_path;
