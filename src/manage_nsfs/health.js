...
const health_errors = {
    NOOBAA_SERVICE_FAILED: {
        ...
        error_message:
            'NooBaa service is not started properly, Please verify the service with status command.',
    },
    NOOBAA_ENDPOINT_FAILED: {
        error_code: 'NOOBAA_ENDPOINT_FAILED',
        error_message:
            'S3 endpoint process is not running. Restart the endpoint process.',
    },
    NOOBAA_ENDPOINT_FORK_MISSING: {
        error_code: 'NOOBAA_ENDPOINT_FORK_MISSING',
        error_message:
            'One or more endpoint fork is not started properly. Verify the total and missing fork count in response.',
    },
    STORAGE_NOT_EXIST: {
        error_code: 'STORAGE_NOT_EXIST',
        error_message:
            'Storage path mentioned in schema pointing to the invalid directory.',
    },
    INVALID_CONFIG: {
        error_code: 'INVALID_CONFIG',
        error_message:
            'Schema JSON is not valid, Please check the JSON format.',
    },
    ACCESS_DENIED: {
        error_code: 'ACCESS_DENIED',
        error_message:
            'Account do no have access to storage path mentioned in schema.',
    },
    MISSING_CONFIG: {
...
const fork_response_code = {
    ...
    MISSING_FORKS: {
        ...
        response_message:
            'Number of running forks is less than the expected fork count.',
    },
    NOT_RUNNING: {
...
class NSFSHealth {
    ...
    async nc_nsfs_health() {
        let endpoint_state;
        let memory;
        const noobaa_service_state =
            await this.get_service_state(NOOBAA_SERVICE);
        ...
        const error_code = await this.get_error_code(
            service_status,
            pid,
            response_code,
        );
        if (this.all_bucket_details) {
            bucket_details = await this.get_bucket_status();
        }
        if (this.all_account_details) {
            account_details = await this.get_account_status();
        }
        const health = {
            ...
            checks: {
                ...
                accounts_status: {
                    invalid_accounts:
                        account_details === undefined ? undefined : (
                            account_details.invalid_storages
                        ),
                    valid_accounts:
                        account_details === undefined ? undefined : (
                            account_details.valid_storages
                        ),
                    error_type: health_errors_tyes.PERSISTENT,
                },
                buckets_status: {
                    invalid_buckets:
                        bucket_details === undefined ? undefined : (
                            bucket_details.invalid_storages
                        ),
                    valid_buckets:
                        bucket_details === undefined ? undefined : (
                            bucket_details.valid_storages
                        ),
                    error_type: health_errors_tyes.PERSISTENT,
                },
            },
    ...
    async get_endpoint_response() {
        ...
        try {
            await P.retry({
                ...
                func: async () => {
                    ...
                    if (
                        ...
                        throw new Error(
                            'Noobaa endpoint is not running, all the retries failed',
                        );
                    }
                },
            });
            ...
            console.log(
                'Error while pinging endpoint host :' +
                    HOSTNAME +
                    ', port ' +
                    this.https_port,
                err,
    ...
    async get_service_state(service_name) {
        ...
        let pid;
        try {
            service_status = await os_util.exec(
            ...
            dbg.warn(
                'could not receive service active state',
                service_name,
                err,
            );
        ...
        if (
            ['inactive', 'missing service status info'].includes(service_status)
        ) {
            service_health.error_type = health_errors_tyes.PERSISTENT;
            const service_error_name =
                _.upperCase(_.camelCase(service_name)) + '_SERVICE_FAILED';
    ...
    async get_endpoint_fork_response() {
        ...
        let response;
        try {
            const fork_count_response =
                await this.make_endpoint_health_request(url_path);
            ...
            if (total_fork_count > 0) {
                ...
                await P.retry({
                    ...
                    delay_ms: 1,
                    func: async () => {
                        const fork_id_response =
                            await this.make_endpoint_health_request(url_path);
                        if (
                            fork_id_response.worker_id &&
                            !worker_ids.includes(fork_id_response.worker_id)
                        ) {
    ...
    async get_service_memory_usage() {
        let memory_status;
        try {
            memory_status = await os_util.exec(
                'systemctl status ' + NOOBAA_SERVICE + ' | grep Memory ',
                {
                    ignore_rc: false,
                    return_stdout: true,
                    trim_stdout: true,
    ...
    async get_bucket_status() {
        const bucket_details = await this.get_storage_status(
            TYPES.BUCKET,
            this.all_bucket_details,
        );
        return bucket_details;
    }

    async get_account_status() {
        const account_details = await this.get_storage_status(
            TYPES.ACCOUNT,
            this.all_account_details,
        );
    ...
    async get_storage_status(type, all_details) {
        ...
        if (type === TYPES.BUCKET) {
            ...
            config_root_type_exists =
                await this.config_fs.validate_config_dir_exists(
                    config_dir_path,
                );
        ...
        if (!config_root_type_exists) {
            dbg.log1(
                `Config directory type - ${type} is missing, ${config_dir_path}`,
            );
            return {
        ...
        for (const config_file of config_files) {
            ...
            const { config_data = undefined, err_obj = undefined } =
                await this.get_config_file_data_or_error_object(
                    type,
                    config_file,
                );
            ...
            if (type === TYPES.ACCOUNT) {
                const config_file_path =
                    this.config_fs.get_account_path_by_name(config_file.name);
                res = await is_new_buckets_path_valid(
                    config_file_path,
                    config_data,
                    storage_path,
                );
            } else if (type === TYPES.BUCKET) {
                res = await is_bucket_storage_path_exists(
                    this.config_fs.fs_context,
                    config_data,
                    storage_path,
                );
            }
    ...
    async get_config_file_data_or_error_object(type, config_file_name) {
        ...
        try {
            ...
            if (err.code === 'ENOENT') {
                dbg.log1(
                    `Error: Config file path should be a valid path`,
                    config_file_path,
                    err,
                );
                ...
                dbg.log1(
                    'Error: while accessing the config file: ',
                    config_file_path,
                    err,
                );
                err_code = health_errors.INVALID_CONFIG.error_code;
            }
            err_obj = get_invalid_object(
                config_file_name,
                config_file_path,
                undefined,
                err_code,
...
async function get_health_status(argv, config_fs) {
    try {
        ...
        const all_account_details = get_boolean_or_string_value(
            argv.all_account_details,
        );
        const all_bucket_details = get_boolean_or_string_value(
            argv.all_bucket_details,
        );

        if (deployment_type === 'nc') {
            const health = new NSFSHealth({
                https_port,
                all_account_details,
                all_bucket_details,
                config_fs,
            ...
            write_stdout_response(
                ManageCLIResponse.HealthStatus,
                health_status,
            );
        } else {
 ...
 */
async function is_new_buckets_path_valid(
    config_file_path,
    config_data,
    new_buckets_path,
) {
    ...
    if (!new_buckets_path) {
        if (config_data.allow_bucket_creation) {
            res_obj = get_invalid_object(
                config_data.name,
                undefined,
                new_buckets_path,
                health_errors.STORAGE_NOT_EXIST.error_code,
            );
        } else {
            res_obj = get_valid_object(
                config_data.name,
                undefined,
                new_buckets_path,
            );
    ...
    // 2
    try {
        account_fs_context = await native_fs_utils.get_fs_context(
            config_data.nsfs_account_config,
        );
    } catch (err) {
        dbg.log1(
            `Error: Could not get account fs context`,
            config_data.nsfs_account_config,
            err,
        );
    ...
    try {
        ...
        if (err.code === 'ENOENT') {
            dbg.log1(
                `Error: Storage path should be a valid dir path`,
                new_buckets_path,
            );
            ...
            dbg.log1(
                'Error:  Storage path should be accessible to account: ',
                new_buckets_path,
            );
 ...
 * @returns {Promise<object>}
 */
async function is_bucket_storage_path_exists(
    fs_context,
    config_data,
    storage_path,
) {
