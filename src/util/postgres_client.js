...
class PostgresTable {
  ...
  async find(query, options = {}) {
    ...
    sql_query.where =
      !_.isEmpty(query) &&
      mongo_to_pg('data', encoded_query, calculateOptionsAndArrayFields(query));
  ...
  async _reduceFinalizeFuncStats(rows, scope) {
    ...
    if (response_times.length > scope.max_samples) {
      reduced.completed_response_times = Array.from(
        ...
        () => response_times[Math.floor(Math.random() * response_times.length)],
      );
    }
  ...
  async distinct(property, query, options) {
    ...
    sql_query.order_by =
      options.sort &&
      convert_sort(options.sort).replace("data->'bucket'", "data->>'bucket'");
    sql_query.limit = options.limit;
    sql_query.offset = options.skip;
