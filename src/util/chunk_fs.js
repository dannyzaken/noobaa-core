...
class ChunkFS extends stream.Transform {
  ...
  constructor({
    ...
    this.iov_max =
      platform_iov_max ?
        Math.min(platform_iov_max, config.NSFS_DEFAULT_IOV_MAX)
      : config.NSFS_DEFAULT_IOV_MAX;
    this.bucket = bucket;
  ...
  async _transform(chunk, encoding, callback) {
    try {
      ...
      while (chunk && chunk.length) {
        ...
        const buf =
          available_size < chunk.length ?
            chunk.slice(0, available_size)
          : chunk;
        ...
        if (
          this.q_buffers.length === this.iov_max ||
          this.q_size >= config.NSFS_BUF_SIZE_L
        ) {
          await this._flush_buffers();
        }
        chunk =
          available_size < chunk.length ? chunk.slice(available_size) : null;
      }
      return callback();
    } catch (error) {
      console.error(
        'ChunkFS _transform failed',
        this.q_size,
        this._total_num_buffers,
        error,
      );
  ...
  async _flush_buffers(callback) {
    try {
      if (this.q_buffers.length) {
        ...
        this.q_size = 0;
        dbg.log1(
          `Chunk_fs._flush_buffers: writing ${buffers_to_write.length} buffers, total size is ${size_to_write}`,
        );
        await this.target_file.writev(
          this.fs_context,
          buffers_to_write,
          this.offset,
        );
      ...
      if (callback) {
        if (this.MD5Async) {
          this.digest = (await this.MD5Async.digest()).toString('hex');
        }
        return callback();
      }
    } catch (error) {
      console.error(
        'ChunkFS _flush_buffers failed',
        this.q_size,
        this._total_num_buffers,
        error,
      );
