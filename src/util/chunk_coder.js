...
class ChunkCoder extends stream.Transform {
    ...
    _transform(chunk, encoding, callback) {
        this.stream_sem
            .surround(() =>
                ChunkCoder.global_sem.surround(() => {
                    chunk.chunk_coder_config =
                        chunk.chunk_coder_config || this.chunk_coder_config;
                    if (this.cipher_key_b64) {
                        chunk.cipher_key_b64 = this.cipher_key_b64;
                    }
                    const chunk_promise = P.fromCallback(cb =>
                        nb_native().chunk_coder(this.coder, chunk, cb),
                    );
                    // TODO: Need to remove the cipher_key in case of SSE-C
                    this.stream_promise = Promise.all([
                        chunk_promise,
                        this.stream_promise,
