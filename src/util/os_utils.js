...
function get_distro() {
  ...
  return P.fromCallback(callback => os_detailed_info(callback)).then(distro => {
    let res = '';
...
async function read_kubernetes_agent_drives() {
  ...
  const size_by_bd = await get_block_device_sizes();
  return _.compact(volumes.map(vol => linux_volume_to_drive(vol, size_by_bd)));
}
    ...
    .then(rules => {
      const filtered_rules = rules.filter(rule => {
        if (
          ...
          dest_ips.every(
            dest_ip => !ip_module.cidrSubnet(rule.dst).contains(dest_ip),
          ) ||
  ...
  const list = _.flatMap(services, service_info => {
    ...
    return _.flatMap(spec.ports, port_info => {
      const routes_to_port = service_routes.filter(
        route_info => route_info.spec.port.targetPort === port_info.name,
      );
