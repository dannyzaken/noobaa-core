...
class ChunkEraser extends stream.Transform {
  ...
  _transform(chunk, encoding, callback) {
    ...
    if (chunk.pos !== this.pos) {
      return callback(
        new Error(`ChunkEraser: chunk pos ${chunk.pos} !== ${this.pos}`),
      );
    }
    this.pos += chunk.size;

    const data_frags =
      (chunk.chunk_coder_config && chunk.chunk_coder_config.data_frags) || 0;
    const parity_frags =
      (chunk.chunk_coder_config && chunk.chunk_coder_config.parity_frags) || 0;
    const erasures =
    ...
    if (this.verbose) {
      console.log(`ChunkEraser: erased ${erasures} frags in chunk`, chunk);
    }
