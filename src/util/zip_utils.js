...
function unzip_from_buffer(zip_buffer) {
    return P.fromCallback(cb =>
        yauzl.fromBuffer(zip_buffer, UNZIP_OPTIONS, cb),
    );
}
    ...
    return new Promise((resolve, reject) =>
        zipfile
            ...
            .on('entry', entry =>
                P.resolve()
                    .then(() =>
                        P.fromCallback(cb => zipfile.openReadStream(entry, cb)),
                    )
...
function unzip_to_mem(zipfile) {
    ...
    return unzip_to_callback(zipfile, (entry, stream) =>
        buffer_utils.read_stream_join(stream).then(buffer => {
            files.push({
