...
function _authenticate_header_v4(req) {
    ...
    if (!v4) {
        dbg.warn(
            'Could not match AWS V4 Authorization:',
    ...
    if (!xamzdate.startsWith(credentials[1])) {
        throw new Error(
            'Mismatching date in Authorization Credential and x-amz-date',
        );
    }
    ...
    return {
        ...
        signature: v4[3],
        string_to_sign: _string_to_sign_v4(
            req,
            signed_headers,
            xamzdate,
            region,
...
function _string_to_sign_v4(req, signed_headers, xamzdate, region, service) {
    ...
    const signed_headers_set =
        signed_headers ? new Set(signed_headers.split(';')) : null;

    v4.isSignableHeader = key =>
        !signed_headers_set || signed_headers_set.has(key.toLowerCase());
...
function _check_expiry_query_v4(request_date, expires_seconds) {
    const now = Date.now();
    const expires =
        new Date(request_date).getTime() + Number(expires_seconds) * 1000;
    if (now > expires) {
...
function _authenticate_header_s3(req) {
    ...
    if (!s3) {
        dbg.warn(
            'Could not match AWS S3 Authorization:',
            req.headers.authorization,
        );
        throw new S3Error(S3Error.InvalidArgument);
    }
...
function _string_to_sign_s3(req) {
    ...
    aws_request.headers['presigned-expires'] =
        req.query.Expires || req.headers.date;
...
function _aws_request(req, region, service) {
    ...
    const pathname =
        service === 's3' ?
            u.pathname
                .split('/')
                .map(c => AWS.util.uriEscape(decodeURIComponent(c)))
                .join('/')
    ...
    const equals_handling =
        v2_signature ?
            query_to_string.replace(/=$/, '').replace(/=&/g, '&')
        :   query_to_string;
    ...
    for (let i = 0; i < req.rawHeaders.length; i += 2) {
        ...
        const sdk_key =
            ...
            key.split('-').map(_.capitalize).join('-');
    ...
    const aws_request = {
        ...
        path:
            v2_signature ?
                u.pathname + (search_string ? '?' + search_string : '')
            :   req.originalUrl,
...
function check_request_expiry(req) {
    if (req.query['X-Amz-Date'] && req.query['X-Amz-Expires']) {
        _check_expiry_query_v4(
            req.query['X-Amz-Date'],
            req.query['X-Amz-Expires'],
        );
...
function authorize_client_request(req) {
    ...
    req.credentials = {
        accessKeyId: req.access_key,
        secretAccessKey: req.secret_key,
    };
    req.pathname = () => `/${req.bucket}/${req.key}`;
    req.search = () =>
        req.query ? '?' + AWS.util.queryParamsToString(req.query) : '';
    req.path = `${req.pathname()}${req.search()}`;
    req.headers = req.headers || {};
    req.headers.Host = req.host;
    req.headers['Content-Length'] = req.body.length;
    req.headers['User-Agent'] = 'Smith';
    req.headers['X-Amz-Date'] = new Date()
        .toISOString()
        .replace(/-|:|\.\d{3}/g, '');
    req.headers['X-Amz-Content-Sha256'] = crypto
        .createHash('sha256')
        .update(req.body)
        .digest('hex');
    const v4 = new AWS.Signers.V4(req, req.service, 'signatureCache');
    req.headers.Authorization = v4.authorization(
        req.credentials,
...
function authorize_request_account_by_token(token, requesting_account) {
    if (!requesting_account) {
        _throw_error_authorize_request_account_by_token('requesting_account');
    }
    ...
    const access_key_obj = _.find(
        requesting_account.access_keys,
    ...
    if (!access_key_obj) {
        ...
        dbg.error(
            `authorize_request_account_by_token: could not find access_key_id ${access_key_id_to_find}`,
        );
        throw new RpcError(
    ...
    if (access_key_obj.deactivated) {
        // if it is undefined then it is Active by default
        dbg.error(
            `authorize_request_account_by_token: access key id ${access_key_id_to_find} ` +
                `status is Inactive`,
        );
        throw new RpcError(
            'DEACTIVATED_ACCESS_KEY_ID',
            `Account with access_key deactivated`,
        );
    }
    ...
    const signature_secret =
        token.temp_secret_key || access_key_obj.secret_key.unwrap();
...
function _throw_error_authorize_request_account_by_token(component) {
    ...
    throw new RpcError(
        'INTERNAL_ERROR',
        `Needs ${component} for authorize request account`,
    );
}
