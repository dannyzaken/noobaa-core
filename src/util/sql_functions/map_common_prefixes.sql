  ...
  WHILE FOUND AND total_count < max_keys
    LOOP
      ...
      IF pos > 0 THEN
          ...
          value := null;
          IF keys_map->cut_prefix IS NULL THEN
              keys_map := keys_map || jsonb_build_object(cut_prefix, 1);
              total_count := total_count + 1;
          ELSE
              keys_map := keys_map || jsonb_build_object(cut_prefix, keys_map->cut_prefix + 1);
          END IF;
          -- a common prefix is found, so we can skip the rest of the keys with the same prefix
          -- use as the next marker the common prefix "incremented" by 1 (inc the last char)
          next_marker := substring(cut_prefix from 1 for length(cut_prefix) - 1) || chr(ascii(delimiter) + 1);
