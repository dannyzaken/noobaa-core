...
async function _make_path_dirs(file_path, fs_context) {
    ...
    if (last_dir_pos > 0) {
        return _create_path(file_path.slice(0, last_dir_pos), fs_context);
    }
}

async function _create_path(
    dir,
    fs_context,
    dir_permissions = config.BASE_MODE_DIR,
) {
    ...
    for (const item of dir.split(path.sep)) {
        ...
        try {
            await nb_native().fs.mkdir(
                fs_context,
                dir_path,
                get_umasked_mode(dir_permissions),
            );
        } catch (err) {
    ...
    if (config.NSFS_TRIGGER_FSYNC) {
        await nb_native().fs.fsync(fs_context, dir_path);
    }
}
...
async function open_file(
    ...
    if ((open_mode === 'wt' || open_mode === 'w') && dir_path !== bucket_path) {
        dbg.log1(
            `NamespaceFS._open_file: mode=${open_mode} creating dirs`,
            open_path,
            bucket_path,
        );
    ...
    return nb_native().fs.open(
        fs_context,
        actual_open_path,
        open_mode,
        get_umasked_mode(file_permissions),
    );
}
...
async function copy_bytes(
    ...
    dbg.log1(
        `Native_fs_utils.copy_bytes size=${size} read_offset=${read_offset} write_offset=${write_offset}`,
    );
    ...
    try {
        ...
        for (;;) {
            ...
            const { buffer, callback } = await multi_buffers_pool
                .get_buffers_pool(total_bytes_left)
                .get_buffer();
            buffer_pool_cleanup = callback;
            const bytesRead = await src_file.read(
                fs_context,
                buffer,
                0,
                buffer.length,
            ...
            if (total_bytes_left < bytesRead) {
                data = data.slice(0, total_bytes_left);
            }
...
async function finally_close_files(fs_context, list_of_files = []) {
    await P.map_with_concurrency(5, list_of_files, async file => {
        try {
            ...
            dbg.warn(
                'Native_fs_utils.finally_close_files file close error',
                err,
            );
        }
    });
...
async function safe_unlink(
    ...
    if (_is_gpfs(fs_context)) {
        ...
        if (dir_file) {
            ...
            dbg.error(
                `safe_unlink: dir_file is ${dir_file}, cannot use it to call safe_unlink_gpfs`,
            );
            throw new Error(
                `dir_file is ${dir_file}, need a value to safe unlink GPFS`,
            );
        }
    } else {
        await safe_unlink_posix(
            fs_context,
            src_path,
            src_ver_info,
            tmp_dir_path,
...
// 1. safe_link
// 2. safe_unlink
async function safe_move_posix(
    fs_context,
    src_path,
    dst_path,
    src_ver_info,
...
// safe_link_posix links src_path to dst_path while verifing dst_path has the expected ino and mtimeNsBigint values
async function safe_link_posix(
    fs_context,
    src_path,
    dst_path,
    src_version_info,
    ...
    dbg.log1(
        'Namespace_fs.safe_link_posix:',
        src_path,
        dst_path,
        src_version_info,
    );
    await nb_native().fs.safe_link(
        fs_context,
        src_path,
        dst_path,
        src_version_info.mtimeNsBigint,
        src_version_info.ino,
    );
}
...
async function safe_unlink_posix(
    fs_context,
    to_delete_path,
    to_delete_version_info,
    tmp_dir_path,
) {
    dbg.log1(
        'Namespace_fs.safe_unlink_posix:',
        to_delete_path,
        to_delete_version_info,
        tmp_dir_path,
    );
    try {
        const unique_temp_path = await _generate_unique_path(
            fs_context,
            tmp_dir_path,
        );
        const { mtimeNsBigint, ino } = to_delete_version_info;
        await nb_native().fs.safe_unlink(
            fs_context,
            to_delete_path,
            unique_temp_path,
            mtimeNsBigint,
        ...
        if (err.code === 'ENOENT') {
            dbg.warn(
                'Namespace_fs.safe_unlink_posix unlink: file already deleted, ignoring..',
            );
            return;
        }
        throw err;
...
async function safe_link_gpfs(fs_context, dst_path, src_file, dst_file) {
    dbg.log1(
        'Namespace_fs.safe_link_gpfs source_file:',
        src_file,
        src_file.fd,
        dst_file,
        dst_file && dst_file.fd,
    );
...
async function safe_unlink_gpfs(
    fs_context,
    to_delete_path,
    to_delete_file,
    dir_file,
    ...
    dbg.log1(
        'Namespace_fs._delete_version_id unlink:',
        dir_file,
        dir_file.fd,
        to_delete_path,
        to_delete_file,
        to_delete_file && to_delete_file.fd,
    );
    try {
        await dir_file.unlinkfileat(
            fs_context,
            path.basename(to_delete_path),
            to_delete_file && to_delete_file.fd,
        );
    } catch (err) {
        if (err.code === 'ENOENT') {
            dbg.warn(
                'Namespace_fs.safe_unlink_gpfs unlink: file already deleted, ignoring..',
            );
            return;
        }
        if (err.code === gpfs_link_unlink_retry_err) {
            err.code = gpfs_unlink_retry_catch;
        }
        throw err;
    }
}
...
function should_retry_link_unlink(is_gpfs, err) {
    return is_gpfs ?
            [gpfs_link_unlink_retry_err, gpfs_unlink_retry_catch].includes(
                err.code,
            )
        :   [posix_link_retry_err, posix_unlink_retry_err].includes(
                err.message,
            );
}
...
async function create_config_file(
    ...
    try {
        ...
        if (is_gpfs) {
            gpfs_dst_file = await open_file(
                fs_context,
                schema_dir,
                config_path,
                'w*',
                config.BASE_MODE_CONFIG_FILE,
            );
        }
        ...
        let open_path =
            is_gpfs ? config_path : (
                await _generate_unique_path(fs_context, tmp_dir_path)
            );
        upload_tmp_file = await open_file(
            fs_context,
            schema_dir,
            open_path,
            open_mode,
        ...
        if (is_gpfs) {
            gpfs_options = {
                dst_file: gpfs_dst_file,
                dir_file: upload_tmp_file,
            };
        ...
        dbg.log1(
            'create_config_file:: moving from:',
            open_path,
            'to:',
            config_path,
            'is_gpfs=',
...
async function delete_config_file(fs_context, schema_dir, config_path) {
    ...
    try {
        ...
        let gpfs_options;
        if (is_gpfs) {
            gpfs_dir_file = await open_file(
                fs_context,
                path.dirname(schema_dir),
                schema_dir,
                'r',
            );
            gpfs_src_file = await open_file(
                fs_context,
                schema_dir,
                config_path,
                'r',
        ...
        dbg.log1(
            'native_fs_utils: delete_config_file config_path:',
            config_path,
            'is_gpfs:',
            is_gpfs,
        );
        ...
        // moving tmp file to config path atomically
        dbg.log1(
            'native_fs_utils: delete_config_file unlinking:',
            config_path,
            'is_gpfs=',
            is_gpfs,
        );
        ...
        try {
            ...
            if (err.code === 'ENOENT') {
                dbg.warn(
                    `delete_config_file: config file already deleted ${config_path}`,
                );
                return;
            }
            throw err;
 ...
 * @param {string} config_data
 */
async function update_config_file(
    fs_context,
    schema_dir,
    config_path,
    config_data,
    ...
    try {
        ...
        dbg.log1(
            'native_fs_utils: update_config_file config_path:',
            config_path,
            'config_data:',
            config_data,
        );
        ...
        // moving tmp file to config path atomically
        dbg.log1(
            'native_fs_utils: update_config_file moving from:',
            open_path,
            'to:',
            config_path,
            'is_gpfs=',
        ...
        for (;;) {
            try {
                const src_stat =
                    is_gpfs ? undefined : (
                        await nb_native().fs.stat(fs_context, open_path)
                    );
                await safe_move(
                    fs_context,
                    open_path,
                ...
                dbg.log1(
                    'native_fs_utils: update_config_file done',
                    config_path,
                );
                break;
            } catch (err) {
                retries -= 1;
                if (retries <= 0 || !should_retry_link_unlink(is_gpfs, err)) {
                    throw err;
                }
                dbg.warn(
                    `native_fs_utils.update_config_file: Retrying failed move to dest retries=${retries}` +
                        ` source_path=${open_path} dest_path=${config_path}`,
                    err,
                );
                if (is_gpfs) {
...
async function get_user_by_distinguished_name({ distinguished_name }) {
    try {
        ...
        const user = await nb_native().fs.getpwname(
            context,
            distinguished_name,
        );
        return user;
        ...
        dbg.error(
            'native_fs_utils.get_user_by_distinguished_name: failed with error',
            err,
            distinguished_name,
        );
        ...
        throw new RpcError(
            'NO_SUCH_USER',
            'User with distinguished_name not found',
            err,
        );
    }
}
 ...
 */
function get_process_fs_context(
    backend = '',
    warn_threshold_ms = config.NSFS_WARN_THRESHOLD_MS,
    report_fs_stats = undefined,
) {
    return {
        uid: process.getuid(),
        gid: process.getgid(),
...
async function get_fs_context(nsfs_account_config, fs_backend) {
    ...
    if (nsfs_account_config.distinguished_name) {
        account_ids_by_dn = await get_user_by_distinguished_name({
            distinguished_name: nsfs_account_config.distinguished_name,
        });
    }
    return {
        uid:
            (account_ids_by_dn && account_ids_by_dn.uid) ??
            nsfs_account_config.uid,
        gid:
            (account_ids_by_dn && account_ids_by_dn.gid) ??
            nsfs_account_config.gid,
        warn_threshold_ms: config.NSFS_WARN_THRESHOLD_MS,
        backend: fs_backend,
    };
}
...
async function folder_delete(dir, fs_context, is_temp, silent_if_missing) {
    ...
    const results = await Promise.all(
        entries.map(entry => {
            ...
            const task =
                isDirectory(entry) ?
                    folder_delete(fullPath, fs_context)
