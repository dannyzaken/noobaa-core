...
const projectedServiceAccountToken =
...
function find_cloud_connection(account, conn_name) {
    const conn = (account.sync_credentials_cache || []).filter(
        sync_conn => sync_conn.name === conn_name,
    )[0];
    ...
    if (!conn) {
        ...
        throw new RpcError(
            'INVALID_CONNECTION',
            'Connection dosn\'t exists: "' + conn_name + '"',
        );
    }
...
async function createSTSS3Client(params, additionalParams) {
    const creds = await generate_aws_sts_creds(
        params,
        additionalParams.RoleSessionName,
    );
    return new AWS.S3({
        credentials: creds,
...
async function generate_aws_sts_creds(params, roleSessionName) {
    ...
    const creds = await sts
        .assumeRoleWithWebIdentity({
            ...
            WebIdentityToken: (
                await fs.promises.readFile(projectedServiceAccountToken)
            ).toString(),
    ...
    if (_.isEmpty(creds.Credentials)) {
        dbg.error(
            `AWS STS empty creds ${params.RoleArn}, RolesessionName: ${params.RoleSessionName},Projected service Account Token Path : ${projectedServiceAccountToken}`,
        );
        throw new RpcError(
            'AWS_STS_ERROR',
            'Empty AWS STS creds retrieved for Role "' + params.RoleArn + '"',
        );
    }
...
function get_signed_url(params) {
    const s3 = new AWS.S3({
        ...
        s3DisableBodySigning: disable_s3_compatible_bodysigning(
            params.endpoint,
        ),
        region: 'eu-central-1',
        httpOptions: {
...
function is_aws_endpoint(endpoint) {
    ...
    const is_aws =
        endpoint_url.host && endpoint_url.host.endsWith('amazonaws.com');
    return is_aws;
}

function disable_s3_compatible_bodysigning(endpoint) {
    const endpoint_url = url.parse(endpoint);
    const disable_sign = Boolean(
        !is_aws_endpoint(endpoint) && endpoint_url.protocol === 'http:',
    );
    return disable_sign;
}
...
function get_used_cloud_targets(
    endpoint_type,
    bucket_list,
    pool_list,
    namespace_resources,
) {
    const cloud_resource_targets =
        pool_list ?
            pool_list
                .filter(
                    pool =>
                ...
                .map(pool_with_cloud_resource => ({
                    ...
                    endpoint_type:
                        pool_with_cloud_resource.cloud_pool_info.endpoint_type,
                    source_name: pool_with_cloud_resource.name,
                    target_name:
                        pool_with_cloud_resource.cloud_pool_info.target_bucket,
                    usage_type: 'CLOUD_RESOURCE',
                }))
        :   [];
    const namespace_resource_targets =
        namespace_resources ?
            namespace_resources
                .filter(
                    ns =>
                        ns.connection &&
                        _.includes(endpoint_type, ns.connection.endpoint_type),
                )
                .map(ns_rec =>
                    _.omitBy(
                        {
...
function set_noobaa_s3_connection(sys) {
    const system_address = _.filter(sys.system_address, {
        api: 's3',
        kind: 'INTERNAL',
    });
    ...
    const access_key =
        sys.owner &&
        sys.owner.access_keys &&
        sys.owner.access_keys[0].access_key.unwrap();
    const secret_key =
        sys.owner &&
        sys.owner.access_keys &&
        sys.owner.access_keys[0].secret_key.unwrap();
    if (!endpoint || !access_key || !secret_key) {
        dbg.error(
            'set_noobaa_s3_connection: temporary error: invalid noobaa s3 connection details',
        );
        return;
    }
...
function generate_access_keys() {
    return {
        access_key: new SensitiveString(
            string_utils.crypto_random_string(
                20,
                string_utils.ALPHA_NUMERIC_CHARSET,
            ),
        ),
