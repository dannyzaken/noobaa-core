...
function start_workers(metrics_port, count = 0) {
    ...
    if (cluster.isPrimary && count > 0) {
        ...
        cluster.on('exit', (worker, code, signal) => {
            ...
            new NoobaaEvent(NoobaaEvent.FORK_EXIT).create_event(
                undefined,
                {
                    id: worker.id,
                    pid: worker.process.pid,
            ...
            while (
                exit_events.length &&
                now - exit_events[0] >
                    config.NSFS_EXIT_EVENTS_TIME_FRAME_MIN * 60 * 1000
            ) {
                exit_events.shift();
            }
            exit_events.push(now); // adding the new exit event that just happened
            if (
                exit_events.length > config.NSFS_MAX_EXIT_EVENTS_PER_TIME_FRAME
            ) {
            ...
            console.warn(
                `${exit_events.length} exit events in the last ${config.NSFS_EXIT_EVENTS_TIME_FRAME_MIN} minutes,` +
                    ` max allowed are: ${config.NSFS_MAX_EXIT_EVENTS_PER_TIME_FRAME}`,
            );
            const new_worker = cluster.fork();
            console.warn('WORKER re-started', {
                id: new_worker.id,
                pid: new_worker.process.pid,
            });
        });
