...
class MongoClient extends EventEmitter {
          ...
          col.db_indexes.map(async index => {
            ...
            try {
              const res = await db
                ...
                .createIndex(
                  index.fields,
                  _.extend({ background: true }, index.options),
                );
  ...
  define_collection(col) {
    ...
    if (this.mongo_client) {
      this._init_collection(this.mongo_client.db(), mongo_collection).catch(
        _.noop,
      ); // TODO what is best to do when init_collection fails here?
    }
  ...
  collection(col_name) {
    if (!this.mongo_client) {
      throw new Error(`mongo_client not connected (collection ${col_name})`);
    }
  ...
  initiate_replica_set(set, members, is_config_set) {
    ...
    if (!is_config_set) {
      ...
      return P.resolve(this.mongo_client.db().admin().command(command)).catch(
        err => {
          dbg.error(
      ...
      await P.timeout(
        ...
        (async () => {
          ...
          while (db_is_down && !waiting_exhausted) {
            try {
              const stats =
                this.mongo_client && (await this.mongo_client.db().stats());
