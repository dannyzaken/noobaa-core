...
class NamespaceCache {
  ...
  async _range_read_hub_check(params, object_sdk) {
    ...
    if (block_info) {
      ...
      if (params.object_md.size <= params.bucket_free_space_bytes) {
        const part_size =
          (block_info.end_block_idx - block_info.start_block_idx + 1) *
          block_size;
        ...
        if (
          ...
          const { total_cached_size } = await this._get_cached_range_parts_info(
            params,
            object_sdk,
          );
      ...
      if (object_info_hub.etag === cache_etag) {
        ...
        _global_cache_uploader.submit_background(
          ...
          async () => {
            ...
            const upload_res = await this.namespace_nb.upload_object(
              upload_params,
              object_sdk,
            );
  ...
  async _read_object_stream(params, object_sdk) {
    ...
    params.bucket_free_space_bytes = await this._get_bucket_free_space_bytes(
      params,
      object_sdk,
    );
    ...
    try {
      const start_time = process.hrtime.bigint();
      const cache_read_stream = await this.namespace_nb.read_object_stream(
        params,
        object_sdk,
      );
    ...
    if (hub_read_size <= params.bucket_free_space_bytes) {
      ...
      if (hub_read_range) {
        if (
          ...
          _global_cache_uploader.submit_background(hub_read_size, async () => {
            const start_time = process.hrtime.bigint();
  ...
  async upload_object(params, object_sdk) {
    ...
    if (params.size > bucket_free_space_bytes) {
      ...
      if (cache_ok) {
        ...
        dbg.log0('NamespaceCache.upload_object: error in cache upload', {
          params: _.omit(params, 'source_stream'),
          hub_res,
          cache_res,
        });
    ...
    const head_res =
      ...
      (await P.map(params.objects, async obj => {
        ...
        try {
          obj_md = _.defaults(
            { key: obj.key },
            await this.namespace_hub.read_object_md(request, object_sdk),
          );
        } catch (err) {
