...
class ConfigFS {
    ...
    constructor(config_root, config_root_backend, fs_context) {
        ...
        this.old_accounts_dir_path = path.join(
            config_root,
            CONFIG_SUBDIRS.ACCOUNTS,
        );
        this.accounts_by_name_dir_path = path.join(
            config_root,
            CONFIG_SUBDIRS.ACCOUNTS_BY_NAME,
        );
        this.identities_dir_path = path.join(
            config_root,
            CONFIG_SUBDIRS.IDENTITIES,
        );
        this.access_keys_dir_path = path.join(
            config_root,
        ...
        this.fs_context =
            fs_context ||
            native_fs_utils.get_process_fs_context(this.config_root_backend);
    }
    ...
    add_config_file_suffix(config_file_name, suffix) {
        if (!config_file_name) {
            dbg.warn(`Config file name is missing - ${config_file_name}`);
        }
    ...
    async create_config_dirs_if_missing() {
        ...
        for (const dir_path of pre_req_dirs) {
            try {
                const dir_exists =
                    await this.validate_config_dir_exists(dir_path);
                if (dir_exists) {
                    dbg.log1(
                        'create_config_dirs_if_missing: config dir exists:',
                        dir_path,
                    );
                } else {
                    ...
                    dbg.log1(
                        'create_config_dirs_if_missing: config dir was created:',
                        dir_path,
                    );
                }
            } catch (err) {
                dbg.log1(
                    'create_config_dirs_if_missing: could not create prerequisite path',
                    dir_path,
                );
            }
        }
    ...
    async get_config_json() {
        const config_json_data = await this.get_config_data(
        ...
        try {
            ...
            if (decrypt_secret_key) {
                config_data.access_keys =
                    await nc_mkm.decrypt_access_keys(config_data);
            }
            return config_data;
        } catch (err) {
            dbg.warn(
                'get_identity_config_data: with config_file_path',
                config_file_path,
                'got an error',
                err,
            );
    ...
    async get_config_data(config_file_path, options = {}) {
        try {
            ...
            dbg.warn(
                'get_config_data: with config_file_path',
                config_file_path,
                'got an error',
                err,
            );
    ...
    async get_identity_by_id(id, type, options = {}) {
        ...
        let identity = await this.get_identity_config_data(identity_path, {
            ...options,
    ...
    async search_accounts_by_id(id, options = {}) {
        ...
        for (const account_name of old_account_names) {
            const account_data = await this.get_account_by_name(account_name, {
                ...options,
                silent_if_missing: true,
            });
    ...
    async get_identities_by_id(ids, options = {}) {
        ...
        for (const id of ids) {
            ...
            const id_data = await this.get_identity_config_data(id_path, {
                ...options,
                silent_if_missing: true,
            });
            if (!id_data) continue;
    ...
    async is_account_exists_by_name(account_name, owner_account_id) {
        const path_to_check = this.get_account_or_user_path_by_name(
            account_name,
            owner_account_id,
        );
        let account_exists = await native_fs_utils.is_path_exists(
            this.fs_context,
            path_to_check,
        );
        ...
        if (
            ...
            const old_path_to_check =
                this._get_old_account_path_by_name(account_name);
            account_exists = await native_fs_utils.is_path_exists(
                this.fs_context,
                old_path_to_check,
            );
        }
    ...
    async is_identity_exists(id, type, options) {
        ...
        let identity = await native_fs_utils.is_path_exists(
            this.fs_context,
            path_to_check,
        );
    ...
    async is_account_exists_by_access_key(access_key) {
        const path_to_check =
            this.get_account_or_user_path_by_access_key(access_key);
    ...
    async get_account_by_access_key(access_key, options = {}) {
        const account_path =
            this.get_account_or_user_path_by_access_key(access_key);
        const account = await this.get_identity_config_data(
            account_path,
            options,
        );
        return account;
    ...
    async get_account_by_name(account_name, options = {}) {
        ...
        let account = await this.get_identity_config_data(account_path, {
            ...options,
            silent_if_missing: true,
        });
        if (!account) {
            const old_account_path =
                this._get_old_account_path_by_name(account_name);
            account = await this.get_identity_config_data(old_account_path, {
                ...options,
                silent_if_missing: true,
            });
        }
        if (!account && !options.silent_if_missing) {
            const err = new Error(
                `Could not find account by name ${account_name}`,
            );
            err.code = 'ENOENT';
            throw err;
        }
     ...
     * @returns {Promise<Boolean>}
     */
    async is_account_exists_by_principal(
        principal,
        options = { silent_if_missing: true },
    ) {
        if (principal === undefined) return undefined;

        const principal_as_string =
            principal instanceof SensitiveString ?
                principal.unwrap()
            :   principal;
        ...
        const principal_by_id = await this.is_identity_exists(
            principal_as_string,
            undefined,
            options,
        );
        dbg.log2(
            'is_account_exists_by_principal: principal_by_id',
            principal_by_id,
        );
        if (principal_by_id) return true;
        const principal_by_name = await this.is_account_exists_by_name(
            principal_as_string,
            undefined,
        );
        dbg.log2(
            'is_account_exists_by_principal: principal_by_name',
            principal_by_name,
        );
     ...
     */
    async list_accounts() {
        const new_entries = await nb_native().fs.readdir(
            this.fs_context,
            this.accounts_by_name_dir_path,
        );
        const new_accounts_names = this._get_config_entries_names(
            new_entries,
            SYMLINK_SUFFIX,
        );
        const old_accounts_names = await this.list_old_accounts();
        return this.unify_old_and_new_accounts(
            new_accounts_names,
            old_accounts_names,
        );
    }
    ...
    async list_old_accounts() {
        const old_accounts_dir_exists = await native_fs_utils.is_path_exists(
            this.fs_context,
            this.old_accounts_dir_path,
        );
        if (!old_accounts_dir_exists) return [];

        const old_entries = await nb_native().fs.readdir(
            this.fs_context,
            this.old_accounts_dir_path,
        );
    ...
    async create_account_config_file(account_data) {
        ...
        const { parsed_account_data, string_account_data } =
            await this._prepare_for_account_schema(account_data);
    ...
    async update_account_config_file(account_new_data, options = {}) {
        ...
        const { parsed_account_data, string_account_data } =
            await this._prepare_for_account_schema(account_new_data);
        const account_path = this.get_identity_path_by_id(_id);
        const account_dir_path = this.get_identity_dir_path_by_id(_id);
        await native_fs_utils.update_config_file(
            this.fs_context,
        ...
        if (options.old_name) {
            await this.link_account_name_index(_id, name, owner);
            await this.unlink_account_name_index(
                options.old_name,
                account_path,
            );
        }
        await this.link_access_keys_index(_id, options.new_access_keys_to_link);
        await this.unlink_access_keys_indexes(
            options.access_keys_to_delete,
            account_path,
        );
        return parsed_account_data;
    }
    ...
    async _prepare_for_account_schema(account_data) {
        const encrypted_account =
            await nc_mkm.encrypt_access_keys(account_data);
    ...
    async link_account_name_index(account_id, account_name, owner_id) {
        const account_name_path = this.get_account_or_user_path_by_name(
            account_name,
            owner_id,
        );
        const account_id_relative_path =
    ...
    async unlink_account_name_index(account_name, account_id_config_path) {
        ...
        const should_unlink = await this._is_symlink_pointing_to_identity(
            account_name_path,
            account_id_config_path,
        );
        if (should_unlink) {
            try {
                ...
                if (err.code === 'ENOENT') {
                    dbg.warn(
                        `config_fs.unlink_account_name_index: account name already unlinked ${account_name} ${account_id_config_path}`,
                    );
                    return;
                }
                throw err;
    ...
    async link_access_keys_index(account_id, access_keys_to_link = []) {
        ...
        const account_config_relative_path =
            this.get_account_relative_path_by_id(account_id);
        for (const access_keys of access_keys_to_link) {
            const new_access_key_path =
                this.get_account_or_user_path_by_access_key(
    ...
    async unlink_access_key_index(access_key, account_id_config_path) {
        ...
        if (should_unlink) {
            try {
                ...
                if (err.code === 'ENOENT') {
                    dbg.warn(
                        `config_fs.unlink_access_key_index: account access_key already unlinked ${access_key} ${account_id_config_path}`,
                    );
                    return;
                }
                throw err;
    ...
    async _is_symlink_pointing_to_identity(symlink_path, identity_path) {
        ...
        return (
            ...
            (os_utils.IS_MAC &&
                full_path === path.join('/private/', identity_path))
        );
    }
     ...
     */
    async list_buckets() {
        const bucket_entries = await nb_native().fs.readdir(
            this.fs_context,
            this.buckets_dir_path,
        );
        const bucket_names = this._get_config_entries_names(
            bucket_entries,
            JSON_SUFFIX,
        );
        return bucket_names;
    ...
    async create_bucket_config_file(bucket_data) {
        const { parsed_bucket_data, string_bucket_data } =
            this._prepare_for_bucket_schema(bucket_data);
    ...
    _prepare_for_bucket_schema(bucket_data) {
        const bucket_data_api_props_omitted = _.omitBy(
            bucket_data,
            _.isUndefined,
        );
        const string_bucket_data = JSON.stringify(
            bucket_data_api_props_omitted,
        );
    ...
    async update_bucket_config_file(bucket_data) {
        const { parsed_bucket_data, string_bucket_data } =
            this._prepare_for_bucket_schema(bucket_data);
        const bucket_config_path = this.get_bucket_path_by_name(
            bucket_data.name,
        );
        await native_fs_utils.update_config_file(
            this.fs_context,
    ...
    async get_system_config_file(options) {
        const system_data = await this.get_config_data(
            this.system_json_path,
            options,
        );
        return system_data;
    }
