...
class ObjectIO {
  ...
  async upload_object_range(params) {
    ...
    const obj_upload =
      await params.client.object.get_upload_object_range_info(upload_params);
  ...
  async upload_object(params) {
    ...
    try {
      ...
      const complete_result =
        await params.client.object.complete_object_upload(complete_params);
      if (params.copy_source) {
    ...
    try {
      ...
      const mc = new MapClient({
        ...
        report_error: (block_md, action, err) =>
          this._report_error_on_object_upload(params, block_md, action, err),
      });
      await mc.run();
      if (mc.had_errors) throw new Error('Upload map errors');
      if (params.upload_chunks_hook) {
        params.upload_chunks_hook(params.range.end - params.range.start);
      }
      return callback();
    } catch (err) {
              ...
              if (buffers && buffers.length) {
                for (const buffer of buffers) {
                  if (buffer.data) {
                    ...
                    // calls using promise.
                    const missing_buf = await params.missing_part_getter(
                      buffer.start,
                      buffer.end,
                    );
