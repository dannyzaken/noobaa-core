...
class NamespaceS3 {
    ...
    async _prepare_sts_client() {
        if (this.s3_params.aws_sts_arn) {
            ...
            this.s3 = await cloud_utils.createSTSS3Client(
                this.s3_params,
                additionalParams,
            );
        }
    ...
    async list_objects(params, object_sdk) {
        ...
        return {
            objects: _.map(res.Contents, obj =>
                this._get_s3_object_info(obj, params.bucket),
            ),
    ...
    async list_object_versions(params, object_sdk) {
        dbg.log0(
            'NamespaceS3.list_object_versions:',
            this.bucket,
            inspect(params),
        );
        await this._prepare_sts_client();
        ...
        return {
            objects: _.concat(
                _.map(res.Versions, obj =>
                    this._get_s3_object_info(obj, params.bucket),
                ),
                _.map(res.DeleteMarkers, obj =>
                    this._get_s3_object_info(
                        _.assign(obj, { DeleteMarker: true }),
                        params.bucket,
                    ),
                ),
            ),
    ...
    _set_md_conditions(params, request) {
        if (params.md_conditions) {
            ...
            if (params.md_conditions.if_modified_since) {
                request.IfModifiedSince = new Date(
                    params.md_conditions.if_modified_since,
                );
            }
            if (params.md_conditions.if_unmodified_since) {
                request.IfUnmodifiedSince = new Date(
                    params.md_conditions.if_unmodified_since,
                );
            }
        }
    }
    ...
    async read_object_md(params, object_sdk) {
        try {
            ...
            const can_use_get_inline =
                !params.part_number && !request.Key.endsWith('/');
            ...
            try {
                ...
                res = await this.s3
                    .headObject({ ...request, Range: undefined })
                    .promise();
            }
            dbg.log0(
                'NamespaceS3.read_object_md:',
            ...
            return this._get_s3_object_info(
                res,
                params.bucket,
                params.part_number,
            );
    ...
    async read_object_stream(params, object_sdk) {
        dbg.log0(
            'NamespaceS3.read_object_stream:',
            this.bucket,
        ...
        return new Promise((resolve, reject) => {
            ...
            const request = {
                ...
                Range:
                    params.end ?
                        `bytes=${params.start}-${params.end - 1}`
                    :   undefined,
                PartNumber: params.part_number,
            };
            this._set_md_conditions(params, request);
    ...
    async upload_object(params, object_sdk) {
        ...
        const Tagging =
            params.tagging &&
            params.tagging.map(tag => tag.key + '=' + tag.value).join('&');
        if (params.copy_source) {
            const { copy_source, copy_source_range } =
                s3_utils.format_copy_source(params.copy_source);
            if (copy_source_range) {
                // note that CopySourceRange is only supported by s3.uploadPartCopy()
                throw new Error(
                    'NamespaceS3.upload_object: CopySourceRange not supported by s3.copyObject()',
                );
            }
    ...
    async create_object_upload(params, object_sdk) {
        dbg.log0(
            'NamespaceS3.create_object_upload:',
            this.bucket,
            inspect(params),
        );
        await this._prepare_sts_client();
    ...
    async complete_object_upload(params, object_sdk) {
        dbg.log0(
            'NamespaceS3.complete_object_upload:',
            this.bucket,
            inspect(params),
        );
        await this._prepare_sts_client();
    ...
    async abort_object_upload(params, object_sdk) {
        dbg.log0(
            'NamespaceS3.abort_object_upload:',
            this.bucket,
            inspect(params),
        );
        await this._prepare_sts_client();
    ...
    async put_object_tagging(params, object_sdk) {
        dbg.log0(
            'NamespaceS3.put_object_tagging:',
            this.bucket,
            inspect(params),
        );
        await this._prepare_sts_client();
    ...
    async delete_object_tagging(params, object_sdk) {
        dbg.log0(
            'NamespaceS3.delete_object_tagging:',
            this.bucket,
            inspect(params),
        );
        await this._prepare_sts_client();
    ...
    async get_object_tagging(params, object_sdk) {
        dbg.log0(
            'NamespaceS3.get_object_tagging:',
            this.bucket,
            inspect(params),
        );
        await this._prepare_sts_client();
    ...
    async delete_multiple_objects(params, object_sdk) {
        dbg.log0(
            'NamespaceS3.delete_multiple_objects:',
            this.bucket,
            inspect(params),
        );
        await this._prepare_sts_client();
        ...
        return _.map(params.objects, obj => {
            const deleted = _.find(
                res.Deleted,
                del_rec =>
                    del_rec.VersionId === obj.version_id &&
                    del_rec.Key === obj.key,
            );
            if (deleted) {
                ...
                const error = _.find(
                    res.Errors,
                    err_rec =>
                        err_rec.VersionId === obj.version_id &&
                        err_rec.Key === obj.key,
                );
                return {
                    err_code: error.Code,
    ...
    _get_s3_object_info(res, bucket, part_number) {
        ...
        const ranges =
            res.ContentRange ? Number(res.ContentRange.split('/')[1]) : 0;
        const size = ranges || res.ContentLength || res.Size || 0;
        const last_modified_time =
            res.LastModified ? res.LastModified.getTime() : Date.now();
        return {
    ...
    _translate_error_code(params, err) {
        if (err.code === 'NoSuchKey') err.rpc_code = 'NO_SUCH_OBJECT';
            ...
            if (err.code === 'PreconditionFailed') {
                ...
                else if (md_conditions.if_unmodified_since) {
                    err.rpc_code = 'IF_UNMODIFIED_SINCE';
                }
            } else if (err.code === 'NotModified') {
                if (md_conditions.if_modified_since) {
                    err.rpc_code = 'IF_MODIFIED_SINCE';
                } else if (md_conditions.if_none_match_etag) {
                    err.rpc_code = 'IF_NONE_MATCH_ETAG';
                }
            }
        }
    }
    ...
    _assign_encryption_to_request(params, request) {
        if (params.copy_source && params.copy_source.encryption) {
            ...
            request.CopySourceSSECustomerKey = Buffer.from(
                key_b64,
                'base64',
            ).toString('ascii');
        }
        ...
        if (params.encryption) {
            ...
            if (key_b64) {
                ...
                // This will generate an error of non correct key, this is why we decode and send as ascii string
                // Also the key_md5_b64 will be populated by the SDK.
                request.SSECustomerKey = Buffer.from(
                    key_b64,
                    'base64',
                ).toString('ascii');
            } else {
