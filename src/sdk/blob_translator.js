...
async function get_uncommitted_list(params, object_sdk) {
  try {
    const uncommitted_path = get_uncommitted_blocks_path(
      params.bucket,
      params.key,
    );
    const obj_list = [];
...
async function write_committed_list(block_list, bucket, key, object_sdk) {
  ...
  const committed_blocks = block_list.map(block =>
    _.omit(block, 'copy_source'),
  );
...
function process_block_list({
  ...
  committed_list.forEach(item => {
    blocks_by_id[item.block_id] = Object.assign(item, {
      block_type: 'committed',
    });
  });
  uncommitted_list.forEach(item => {
    blocks_by_id[item.block_id] = Object.assign(item, {
      block_type: 'uncommitted',
    });
  });

  return block_list.map(block => {
    const resolved_block = blocks_by_id[block.block_id];
    if (block.type !== resolved_block.block_type && block.type !== 'latest') {
...
async function remove_block_lists(
  object_sdk,
  { bucket, key, uncommitted_list, remove_uncommitted, remove_committed },
) {
  const uncommitted_path = get_uncommitted_blocks_path(bucket, key);
  const objects =
    remove_uncommitted ?
      uncommitted_list.map(block => ({
        key: `${uncommitted_path}/${block.block_id}`,
      }))
    : [];
  if (remove_committed) {
    const committed_path = get_committed_blocks_path(bucket, key);
...
async function commit_blob_block_list(params, object_sdk) {
  ...
  const multiparts = final_block_list.map(block => ({
    num: block.num,
    etag: block.etag,
  }));
...
async function get_blob_block_lists(params, object_sdk) {
  const [committed_list, uncommitted_list] = await P.all([
    params.requested_lists.committed ?
      get_committed_list({ bucket: params.bucket, key: params.key }, object_sdk)
    : P.resolve(),
    params.requested_lists.uncommitted ?
      get_uncommitted_list(
  ...
  const res = _.omitBy(
    {
      committed:
        committed_list ?
          committed_list.map(block => _.pick(block, 'block_id', 'size'))
        : undefined,
      uncommitted:
        uncommitted_list ?
          uncommitted_list.map(block => _.pick(block, 'block_id', 'size'))
        : undefined,
    },
    _.isUndefined,
  );
...
async function delete_blocks(object_sdk, { bucket, keys } = {}) {
  ...
  const block_lists = await P.map(keys, async key => {
    ...
    const uncommitted_path = get_uncommitted_blocks_path(bucket, key);
    const committed_path = get_committed_blocks_path(bucket, key);
    const delete_objects = lists.uncommitted.map(block => ({
      key: `${uncommitted_path}/${block.block_id}`,
    }));
  ...
  await object_sdk.delete_multiple_objects({
    bucket,
    objects: all_objects,
    skip_blocks_delete: true,
  });
}
