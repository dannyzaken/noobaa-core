...
class ObjectSDK {
  ...
  constructor({
    ...
    this.stats = stats;
    this.bucketspace =
      bucketspace || new BucketSpaceNB({ rpc_client, internal_rpc_client });
    ...
    try {
      ...
      if (error.rpc_code) {
        ...
        if (error.rpc_code === 'NO_SUCH_USER') {
          throw new RpcError(
            'UNAUTHORIZED',
            `Distingushed name associated with access_key not found`,
          );
        }
      } else {
        throw error;
  ...
  async authorize_request_account(req) {
    ...
    if (token) {
      const signature_secret =
        token.temp_secret_key ||
        this.requesting_account?.access_keys?.[0]?.secret_key?.unwrap();
      if (signature_secret) {
  ...
  async fix_copy_source_params(params, target_ns) {
    ...
    if (
      actual_target_ns.is_server_side_copy(actual_source_ns, source_md, params)
    ) {
