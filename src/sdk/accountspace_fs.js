...
const {
    create_arn,
    get_action_message_title,
    check_iam_path_was_set,
} = require('../endpoint/iam/iam_utils');
const {
    IAM_ACTIONS,
    MAX_NUMBER_OF_ACCESS_KEYS,
    IAM_DEFAULT_PATH,
...
class AccountSpaceFS {
    ...
    async create_user(params, account_sdk) {
        ...
        try {
            ...
            this._check_if_requesting_account_is_root_account(
                action,
                requesting_account,
            ...
            const created_account =
                await this._copy_data_from_requesting_account_to_account_config(
                    action,
                    requesting_account,
                    params,
                );
    ...
    async get_user(params, account_sdk) {
        ...
        try {
            const requesting_account = account_sdk.requesting_account;
            const { requester } = this._check_root_account_or_user(
                requesting_account,
                params.username,
            );
            ...
            const account_to_get =
                await this.config_fs.get_account_by_name(username);
            this._check_if_requested_account_is_root_account_or_IAM_user(
                action,
                requesting_account,
                account_to_get,
            );
            this._check_if_requested_is_owned_by_root_account(
                action,
                requesting_account,
                account_to_get,
            );
    ...
    async update_user(params, account_sdk) {
        ...
        try {
            ...
            await this._check_if_account_config_file_exists(
                action,
                params.username,
            );
            const requested_account = await this.config_fs.get_account_by_name(
                params.username,
            ...
            const is_username_update =
                params.new_username !== undefined &&
                params.new_username !== params.username;
            if (params.new_iam_path !== undefined) {
                requested_account.iam_path = params.new_iam_path;
            }
            if (is_username_update) {
                dbg.log1(
                    `AccountSpaceFS.${action} username was updated, is_username_update`,
                    is_username_update,
                );
                await this._update_account_config_new_username(
                    action,
                    params,
                    requested_account,
                );
    ...
    async delete_user(params, account_sdk) {
        ...
        try {
            ...
            const account_to_delete = await this.config_fs.get_account_by_name(
                params.username,
                { show_secrets: true },
            );
            this._check_if_requested_account_is_root_account_or_IAM_user(
            ...
            await this._check_if_user_does_not_have_resources_before_deletion(
                action,
                account_to_delete,
            );
    ...
    async list_users(params, account_sdk) {
        ...
        try {
            ...
            let members = await this._list_config_files_for_users(
                requesting_account,
    ...
    async create_access_key(params, account_sdk) {
        ...
        try {
            const requesting_account = account_sdk.requesting_account;
            const requester =
                this._check_if_requesting_account_is_root_account_or_user_om_himself(
                    action,
                    requesting_account,
                    params.username,
                );
            ...
            const { generated_access_key, generated_secret_key } =
                this._generate_access_key();
            ...
            return {
                ...
                status: this._get_access_key_status(
                    created_access_key_obj.deactivated,
                ),
                secret_key: generated_secret_key,
            };
        } catch (err) {
            ...
            throw native_fs_utils.translate_error_codes(
                err,
                native_fs_utils.entity_enum.ACCESS_KEY,
            );
        }
    }
    ...
    async get_access_key_last_used(params, account_sdk) {
        ...
        try {
            ...
            await this._check_if_account_exists_by_access_key_symlink(
                action,
                access_key_id,
            );
            const requested_account =
                await this.config_fs.get_account_by_access_key(access_key_id, {
                    show_secrets: true,
                });
            this._check_if_requested_account_same_root_account_as_requesting_account(
                action,
                requesting_account,
                requested_account,
            );
    ...
    async update_access_key(params, account_sdk) {
        ...
        try {
            ...
            const requested_account =
                await this.config_fs.get_account_by_access_key(
                    params.access_key,
                    { show_secrets: true, decrypt_secret_key: true },
                );
            this._check_username_match_to_requested_account(
                action,
                username,
                requested_account,
            );
            this._check_access_key_belongs_to_account(
                action,
                requested_account,
                access_key_id,
            );
            ...
            const access_key_obj = _.find(
                requested_account.access_keys,
                access_key => access_key.access_key === access_key_id,
            );
            if (
                this._get_access_key_status(access_key_obj.deactivated) ===
                params.status
            ) {
                ...
                dbg.log1(
                    `AccountSpaceFS.${action} status was not change, not updating the account config file`,
                );
                return;
            }
            access_key_obj.deactivated = this._check_access_key_is_deactivated(
                params.status,
            );
            await this.config_fs.update_account_config_file(requested_account);
            this._clean_account_cache(requested_account);
    ...
    async delete_access_key(params, account_sdk) {
        ...
        try {
            ...
            requested_account.access_keys =
                requested_account.access_keys.filter(
                    access_key_obj =>
    ...
    async list_access_keys(params, account_sdk) {
        ...
        try {
            ...
            let members =
                this._list_access_keys_from_account(requested_account);
            members = members.sort((a, b) =>
    ...
    _new_user_defaults(requesting_account, params, master_key_id) {
        const distinguished_name =
            requesting_account.nsfs_account_config.distinguished_name;
        const user_defaults = {
            _id: generate_id(),
            ...
            nsfs_account_config: {
                ...
                uid:
                    distinguished_name ? undefined : (
                        requesting_account.nsfs_account_config.uid
                    ),
                gid:
                    distinguished_name ? undefined : (
                        requesting_account.nsfs_account_config.gid
                    ),
                new_buckets_path:
                    requesting_account.nsfs_account_config.new_buckets_path,
        ...
        if (requesting_account.iam_operate_on_root_account) {
            ...
            if (
                !user_defaults.allow_bucket_creation &&
                user_defaults.nsfs_account_config.new_buckets_path
            ) {
    ...
    _check_root_account(account) {
        if (account.owner === undefined || account.owner === account._id) {
            return true;
        }
        return false;
    }
    ...
    _throw_access_denied_error(action, requesting_account, details, entity) {
        ...
        const arn_for_requesting_account = create_arn(
            requesting_account._id,
            requesting_account.name.unwrap(),
            requesting_account.path,
        );
        const basic_message =
            `User: ${arn_for_requesting_account} is not authorized to perform:` +
        ...
        if (entity === native_fs_utils.entity_enum.USER) {
            ...
            // native_fs_utils.entity_enum.ACCESS_KEY
            message_with_details =
                basic_message + `access key ${details.access_key}`;
        }
    ...
    // TODO: move to IamError class with a template
    _throw_error_perform_action_on_another_root_account(
        action,
        requesting_account,
        requested_account,
    ) {
        const username =
            requested_account.name instanceof SensitiveString ?
                requested_account.name.unwrap()
            :   requested_account.name;
        ...
        dbg.error(
            `AccountSpaceFS.${action} root account of requested account is different than requesting root account`,
            requesting_account,
            requested_account,
        );
    ...
    _throw_error_delete_conflict(action, account_to_delete, resource_name) {
        dbg.error(
            `AccountSpaceFS.${action} requested account ` +
    ...
    _throw_error_perform_action_from_root_accounts_manager_on_iam_user(
        action,
        requesting_account,
        requested_account,
    ) {
        dbg.error(
            `AccountSpaceFS.${action} root accounts manager cannot perform actions on IAM users`,
            requesting_account,
            requested_account,
        );
    ...
    _throw_error_no_such_entity_access_key(action, access_key_id) {
        dbg.error(
            `AccountSpaceFS.${action} access key does not exist`,
            access_key_id,
        );
    ...
    async _list_config_files_for_users(requesting_account, iam_path_prefix) {
        ...
        const config_files_list = await P.map_with_concurrency(
            10,
            account_names,
            async account_name => {
                const account_data = await this.config_fs.get_account_by_name(
                    account_name,
                    { silent_if_missing: true },
                );
                if (!account_data) return undefined;
                const is_root_account_owns_user =
                    this._check_root_account_owns_user(
                        requesting_account,
                        account_data,
                    );
                if (
                    (!requesting_account.iam_operate_on_root_account &&
                        is_root_account_owns_user) ||
                    (requesting_account.iam_operate_on_root_account &&
                        this._check_root_account(account_data))
                ) {
                    if (should_filter_by_prefix) {
                        if (account_data.iam_path === undefined) {
                            return undefined;
                        }
                        if (
                            !account_data.iam_path.startsWith(iam_path_prefix)
                        ) {
                            return undefined;
                        }
                    }
    ...
    _check_if_requesting_account_is_root_account(
        action,
        requesting_account,
        user_details = {},
    ) {
        ...
        dbg.log1(
            `AccountSpaceFS.${action} requesting_account ID: ${requesting_account._id}` +
                `name: ${requesting_account.name.unwrap()}`,
            'is_root_account',
            is_root_account,
        );
        if (!is_root_account) {
            dbg.error(
                `AccountSpaceFS.${action} requesting account is not a root account`,
                requesting_account,
            );
            this._throw_access_denied_error(
                action,
                requesting_account,
                user_details,
                native_fs_utils.entity_enum.USER,
    ...
    _check_if_requested_account_is_root_account_or_IAM_user(
        ...
        const is_requested_account_root_account =
            this._check_root_account(requested_account);
        dbg.log1(
            `AccountSpaceFS.${action} requested_account ID: ${requested_account._id} name: ${requested_account.name}`,
            'is_requested_account_root_account',
            is_requested_account_root_account,
        );
        ...
        if (
            ...
            this._throw_error_perform_action_on_another_root_account(
                action,
                requesting_account,
                requested_account,
            );
        ...
        if (
            ...
            this._throw_error_perform_action_from_root_accounts_manager_on_iam_user(
                action,
                requesting_account,
                requested_account,
            );
    ...
    async _check_username_already_exists(action, username) {
        const name_exists =
            await this.config_fs.is_account_exists_by_name(username);
        if (name_exists) {
            dbg.error(
                `AccountSpaceFS.${action} username already exists`,
                username,
            );
    ...
    async _copy_data_from_requesting_account_to_account_config(
        action,
        requesting_account,
        params,
    ) {
        const master_key_id = await nc_mkm.get_active_master_key_id();
        const created_account = this._new_user_defaults(
            requesting_account,
            params,
            master_key_id,
        );
    ...
    async _check_if_account_config_file_exists(action, username) {
        const is_user_account_exists =
            await this.config_fs.is_account_exists_by_name(username);
        if (!is_user_account_exists) {
            dbg.error(
                `AccountSpaceFS.${action} username does not exist`,
                username,
            );
            const message_with_details = `The user with name ${username} cannot be found.`;
            const { code, http_code, type } = IamError.NoSuchEntity;
    ...
    _check_if_requested_is_owned_by_root_account(
        action,
        requesting_account,
        requested_account,
    ) {
        if (requesting_account.iam_operate_on_root_account) return;
        const is_user_account_to_get_owned_by_root_user =
            this._check_root_account_owns_user(
                requesting_account,
                requested_account,
            );
        if (!is_user_account_to_get_owned_by_root_user) {
            dbg.error(
                `AccountSpaceFS.${action} requested account is not owned by root account`,
                requested_account,
            );
    ...
    async _check_if_user_does_not_have_resources_before_deletion(
        action,
        account_to_delete,
    ) {
        const is_account_to_delete_root_account =
            this._check_root_account(account_to_delete);
        if (is_account_to_delete_root_account) {
            await this._check_if_root_account_does_not_have_buckets_before_deletion(
                action,
                account_to_delete,
            );
            await this._check_if_root_account_does_not_have_IAM_users_before_deletion(
                action,
                account_to_delete,
            );
        }
        this._check_if_user_does_not_have_access_keys_before_deletion(
            action,
            account_to_delete,
        );
    }

    async _check_if_root_account_does_not_have_buckets_before_deletion(
        action,
        account_to_delete,
    ) {
        ...
        await P.map_with_concurrency(10, bucket_names, async bucket_name => {
            const bucket_data = await this.config_fs.get_bucket_by_name(
                bucket_name,
                { silent_if_missing: true },
            );
            if (
                bucket_data &&
                bucket_data.owner_account === account_to_delete._id
            ) {
                this._throw_error_delete_conflict(
                    action,
                    account_to_delete,
                    resource_name,
                );
    ...
    async _check_if_root_account_does_not_have_IAM_users_before_deletion(
        action,
        account_to_delete,
    ) {
        ...
        await P.map_with_concurrency(10, account_names, async account_name => {
            ...
            if (
                (!account_to_delete.iam_operate_on_root_account &&
                    is_root_account_owns_user) ||
                (account_to_delete.iam_operate_on_root_account &&
                    this._check_root_account(account_data))
            ) {
    ...
    _check_if_user_does_not_have_access_keys_before_deletion(
        ...
        const is_access_keys_removed =
            account_to_delete.access_keys.length === 0;
    ...
    async _update_account_config_new_username(
        action,
        params,
        requested_account,
    ) {
    ...
    _check_root_account_or_user(requesting_account, username) {
        ...
        let requester = {};
        const requesting_account_name =
            requesting_account.name instanceof SensitiveString ?
                requesting_account.name.unwrap()
            :   requesting_account.name;
    ...
    _generate_access_key() {
        ...
        ({
            access_key: generated_access_key,
            secret_key: generated_secret_key,
        } = cloud_utils.generate_access_keys());
    ...
    _get_access_key_status(deactivated) {
        ...
        const status =
            deactivated ?
                ACCESS_KEY_STATUS_ENUM.INACTIVE
            :   ACCESS_KEY_STATUS_ENUM.ACTIVE;
        return status;
    }
    ...
    _check_if_requesting_account_is_root_account_or_user_om_himself(
        action,
        requesting_account,
        username,
    ) {
        const { is_root_account_or_user_on_itself, requester } =
            this._check_root_account_or_user(requesting_account, username);
        dbg.log1(
            `AccountSpaceFS.${action} requesting_account ID: ${requesting_account._id}, name: ${requesting_account.name.unwrap()}`,
            'is_root_account_or_user_on_itself',
            is_root_account_or_user_on_itself,
        );
        if (!is_root_account_or_user_on_itself) {
            dbg.error(
                `AccountSpaceFS.${action} requesting account is neither a root account ` +
    ...
    async _check_if_account_exists_by_access_key_symlink(
        action,
        access_key_id,
    ) {
        const is_user_account_exists =
            await this.config_fs.is_account_exists_by_access_key(access_key_id);
        if (!is_user_account_exists) {
            this._throw_error_no_such_entity_access_key(action, access_key_id);
        }
    }

    _check_username_match_to_requested_account(
        action,
        username,
        requested_account,
    ) {
        if (username !== requested_account.name) {
            dbg.error(
                `AccountSpaceFS.${action} user with name ${username} cannot be found`,
            );
            const message_with_details = `The user with name ${username} cannot be found.`;
            const { code, http_code, type } = IamError.NoSuchEntity;
            throw new IamError({
    ...
    _check_if_requested_account_same_root_account_as_requesting_account(
        action,
        requesting_account,
        requested_account,
    ) {
        if (requesting_account.iam_operate_on_root_account) return;
        const root_account_id_requesting_account =
            requesting_account.owner || requesting_account._id; // if it is root account then there is no owner
        const root_account_id_requested =
            requested_account.owner || requested_account._id;
    ...
    _check_access_key_belongs_to_account(
        action,
        requested_account,
        access_key_id,
    ) {
        const is_access_key_belongs_to_account =
            this._check_specific_access_key_exists(
                requested_account.access_keys,
                access_key_id,
            );
