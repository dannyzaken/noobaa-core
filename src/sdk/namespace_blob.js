...
class NamespaceBlob {
  ...
  constructor({
    ...
    this.blob =
      azure_storage.BlobServiceClient.fromConnectionString(connection_string);
  ...
  async list_objects(param, object_sdk) {
    ...
    dbg.log0('NamespaceBlob.list_objects:', this.container, inspect(params));
    const max_keys = params.limit || 1000;
    let iterator;
    if (params.delimiter) {
      iterator = await this.container_client
        .listBlobsByHierarchy(params.delimiter, { prefix: params.prefix })
        .byPage({
  ...
  async list_uploads(params, object_sdk) {
    dbg.log0('NamespaceBlob.list_uploads:', this.container, inspect(params));
    // TODO list uploads
    return {
      objects: [],
      common_prefixes: [],
  ...
  async read_object_md(params, object_sdk) {
    try {
      ...
      object_sdk.rpc_client.pool.update_issues_report({
        ...
        error_code:
          err.code || (err.details && err.details.errorCode) || 'InternalError',
        time: Date.now(),
      });
      throw err;
    }
  ...
  async get_blob_block_lists(params) {
    ...
    if (list_blocks_res.UncommittedBlocks) {
      response.uncommitted = list_blocks_res.CommittedBlocks.map(block => ({
        block_id: block.Name,
        size: block.Size,
      }));
    }
  ...
  async create_object_upload(params, object_sdk) {
    ...
    if (params.xattr && Object.keys(params.xattr).length > 0) {
      try {
        this._check_valid_xattr(params);
        upload =
          await object_sdk.rpc_client.object.create_object_upload(params);
        dbg.log0(
  ...
  _check_valid_tagging(params) {
    for (const tag of params.tagging) {
      ...
      if (invalid_azure_tag_regex.test(tag.key)) {
        const err = new Error(
          'InvalidTags: tag key ' + tag.key + ' has invalid charecters',
        );
        err.rpc_code = 'INVALID_REQUEST';
        throw err;
      }
      ...
      if (invalid_azure_tag_regex.test(tag.value)) {
        const err = new Error(
          'InvalidTags: tag value ' + tag.value + ' has invalid charecters',
        );
        err.rpc_code = 'INVALID_REQUEST';
        throw err;
      }
  ...
  async upload_multipart(params, object_sdk) {
    ...
    if (params.copy_source) {
      const start =
        params.copy_source.ranges ? params.copy_source.ranges[0].start : 0;
      const end =
  ...
  async list_multiparts(params, object_sdk) {
    dbg.log0('NamespaceBlob.list_multiparts:', this.container, inspect(params));
    const blob_client = this._get_blob_client(params.key);
  ...
  _translate_error_code(err) {
    if (
      (err.code || (err.details && err.details.errorCode)) === 'BlobNotFound'
    ) {
      err.rpc_code = 'NO_SUCH_OBJECT';
    }
    if (
      (err.code || (err.details && err.details.errorCode)) === 'InvalidMetadata'
    ) {
      err.rpc_code = 'INVALID_REQUEST';
    }
  }
