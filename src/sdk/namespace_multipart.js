...
class NamespaceMultipart {
  ...
  list_objects(params, object_sdk) {
    return this._ns_map(ns => ns.list_objects(params, object_sdk)).then(res =>
      this._handle_list(res, params),
    );
  }

  list_uploads(params, object_sdk) {
    return this._ns_map(ns => ns.list_uploads(params, object_sdk)).then(res =>
      this._handle_list(res, params),
    );
  }

  list_object_versions(params, object_sdk) {
    return this._ns_map(ns => ns.list_object_versions(params, object_sdk)).then(
  ...
  _get_failed_responses(reply_array, except_reasons) {
    return reply_array
      ...
      .filter(
        error =>
          !_.includes(except_reasons || [], error.rpc_code || 'UNKNOWN_ERR'),
      );
  }
