...
class NamespaceNB {
    ...
    async list_objects(params, object_sdk) {
        if (this.target_bucket) {
            params = _.defaults({ bucket: this.target_bucket }, params);
        }
        const object_info =
            await object_sdk.rpc_client.object.list_objects(params);
        return object_info;
    }
    ...
    async complete_object_upload(params, object_sdk) {
        ...
        const reply =
            await object_sdk.rpc_client.object.complete_object_upload(params);
        if (load_for_trigger) {
            const obj = {
                bucket: params.bucket,
                key: params.key,
    ...
    abort_object_upload(params, object_sdk) {
        ...
        if (!upload_id || !object_id_regex.test(upload_id)) {
    ...
    async delete_multiple_objects(params, object_sdk) {
        ...
        const deleted_res =
            await object_sdk.rpc_client.object.delete_multiple_objects(params);
        if (load_for_trigger) {
    ...
    _dispatch_multiple_delete_triggers(params) {
        ...
        if (head_res.length !== deleted_res.length) {
