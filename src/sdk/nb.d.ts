...
type ReplicationLogCandidates = Record<
  string,
  { action: ReplicationLogAction; time: Date }
>;
...
interface ObjectMD {
  ...
  lock_settings: {
    retention: { mode: string; retain_until_date: Date };
    legal_hold: { status: string };
  };
}
...
interface Native {
  ...
  syslog(
    level: number,
    message: string,
    facility?: 'LOG_LOCAL0' | 'LOG_LOCAL1' | 'LOG_LOCAL2',
  );
...
interface NativeFile {
  ...
  stat(
    fs_context: NativeFSContext,
    options?: { skip_user_xattr?: boolean; xattr_get_keys?: string[] },
  ): Promise<NativeFSStats>;
  read(
    fs_context: NativeFSContext,
    buffer: Buffer,
  ...
  flock(
    fs_context: NativeFSContext,
    operation: 'EXCLUSIVE' | 'SHARED' | 'UNLOCK',
  ): Promise<void>;
  fcntllock(
    fs_context: NativeFSContext,
    operation: 'EXCLUSIVE' | 'SHARED' | 'UNLOCK',
