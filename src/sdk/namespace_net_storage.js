...
const CONFIG_FIELDS = ['hostname', 'keyName', 'key', 'cpCode', 'ssl'];
...
class NamespaceNetStorage {
    ...
    list_object_versions(params, object_sdk) {
        dbg.log0(
            'NamespaceNetStorage.list_object_versions:',
    ...
    list_uploads(params, object_sdk) {
        dbg.log0(
            'NamespaceNetStorage.list_uploads:',
            this.ns_config.cpCode,
            inspect(params),
        );
        // TODO list uploads
    ...
    list_objects(params, object_sdk) {
        dbg.log0(
            'NamespaceNetStorage.list_objects:',
            this.ns_config.cpCode,
            inspect(params),
        );
    ...
    read_object_md(params, object_sdk) {
        return P.fromCallback(callback =>
            ...
            .then(res =>
                this._get_net_storage_md(res.body.stat.file[0], params.bucket),
            )
            .catch(err => {
                this._translate_error_code(err);
                throw err;
    ...
    upload_object(params, object_sdk) {
        dbg.log0(
            'NamespaceNetStorage.upload_object:',
            this.ns_config.cpCode,
        ...
        if (params.copy_source) {
            throw new Error(
                'NamespaceNetStorage.upload_object: copy object not yet supported',
            );
        }

        return P.fromCallback(callback =>
            this.net_storage.upload(
                {
    ...
    delete_object(params, object_sdk) {
        dbg.log0(
            'NamespaceNetStorage.delete_object:',
            this.ns_config.cpCode,
            inspect(params),
        );
        return P.fromCallback(callback =>
            this.net_storage.delete(
                {
                    path: `${this.ns_config.cpCode}/${params.key}`,
                },
                callback,
    ...
    _parse_net_storage_dir(res, bucket) {
        ...
        return {
            objects: _.map(
                ...
                obj => this._get_net_storage_md(obj, bucket),
            ),
