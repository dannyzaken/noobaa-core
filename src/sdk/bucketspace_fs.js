...
class BucketSpaceFS extends BucketSpaceSimpleFS {
  ...
  async read_account_by_access_key({ access_key }) {
    try {
      ...
      if (account.nsfs_account_config.distinguished_name) {
        account.nsfs_account_config.distinguished_name = new SensitiveString(
  ...
  async read_bucket_sdk_info({ name }) {
    try {
      ...
      dbg.log0(
        'BucketSpaceFS.read_bucket_sdk_info: bucket_config_path',
        bucket_config_path,
      );
      ...
      if (bucket.s3_policy) {
        for (const [
          ...
          if (statement_principal.AWS) {
            const sensitive_arr = _.flatten([statement_principal.AWS]).map(
              principal => new SensitiveString(principal),
            );
            if (statement.Principal) {
              bucket.s3_policy.Statement[s_index].Principal.AWS = sensitive_arr;
            }
            if (statement.NotPrincipal) {
              bucket.s3_policy.Statement[s_index].NotPrincipal.AWS =
                sensitive_arr;
            }
          } else {
  ...
  async list_buckets(object_sdk) {
    ...
    const buckets = await P.map_with_concurrency(10, entries, async entry => {
      ...
      const bucket_policy_accessible = await this.has_bucket_action_permission(
        bucket,
        account,
        's3:ListBucket',
      );
  ...
  async create_bucket(params, sdk) {
    return bucket_semaphore.surround_key(String(params.name), async () => {
      ...
      try {
        ...
        dbg.log2(
          'create_bucket: bucket properties before validate_bucket_schema',
          bucket_to_validate,
        );
        ...
        new NoobaaEvent(NoobaaEvent.BUCKET_CREATION_FAILED).create_event(
          name,
          { bucket_name: name },
          err,
        );
      ...
      try {
        ...
        new NoobaaEvent(NoobaaEvent.BUCKET_DIR_CREATION_FAILED).create_event(
          name,
  ...
  new_bucket_defaults(
    ...
    return {
      ...
      versioning:
        config.NSFS_VERSIONING_ENABLED && lock_enabled ? 'ENABLED' : 'DISABLED',
  ...
  async delete_bucket(params, object_sdk) {
    return bucket_semaphore.surround_key(String(params.name), async () => {
      ...
      try {
        const namespace_bucket_config =
          await object_sdk.read_bucket_sdk_namespace_info(params.name);
        dbg.log1(
        ...
        if (
          namespace_bucket_config &&
          namespace_bucket_config.should_create_underlying_storage
        ) {
          ...
          await ns.delete_uls(
            {
              ...
              full_path: path.join(
                this.fs_root,
                namespace_bucket_config.write_resource.path,
              ), // includes write_resource.path + bucket name (s3 flow)
            },
            object_sdk,
          );
        } else if (namespace_bucket_config) {
          // S3 Delete for NSFS Manage buckets
          const list = await ns.list_objects(
            { ...params, limit: 1 },
            object_sdk,
          );
          ...
          const bucket = await object_sdk.read_bucket_sdk_config_info(
            params.name,
          );
        ...
        new NoobaaEvent(NoobaaEvent.BUCKET_DELETE_FAILED).create_event(
          params.name,
          { bucket_name: params.name, bucket_path: bucket_path },
          err,
        );
  ...
  async set_bucket_versioning(params, object_sdk) {
    try {
      ...
      dbg.log2(
        'set_bucket_versioning: bucket properties before validate_bucket_schema',
        bucket,
      );
      nsfs_schema_utils.validate_bucket_schema(bucket);
  ...
  async put_bucket_encryption(params) {
    try {
      ...
      dbg.log2(
        'put_bucket_encryption: bucket properties before validate_bucket_schema',
        bucket_to_validate,
      );
      nsfs_schema_utils.validate_bucket_schema(bucket_to_validate);
      const update_bucket = JSON.stringify(bucket);
  ...
  async delete_bucket_encryption(params) {
    try {
      ...
      dbg.log2(
        'delete_bucket_encryption: bucket properties before validate_bucket_schema',
        bucket,
      );
  ...
  async put_bucket_website(params) {
    try {
      ...
      const bucket_to_validate = _.omitBy(bucket, _.isUndefined);
      dbg.log2(
        'put_bucket_website: bucket properties before validate_bucket_schema',
        bucket_to_validate,
      );
      nsfs_schema_utils.validate_bucket_schema(bucket_to_validate);
      const update_bucket = JSON.stringify(bucket);
  ...
  async delete_bucket_website(params) {
    try {
      ...
      dbg.log2(
        'delete_bucket_website: bucket properties before validate_bucket_schema',
        bucket,
      );
      // on the safe side validate before changing configuration
      nsfs_schema_utils.validate_bucket_schema(bucket);
  ...
  async put_bucket_policy(params) {
    try {
      ...
      const bucket_to_validate = _.omitBy(bucket, _.isUndefined);
      dbg.log2(
        'put_bucket_policy: bucket properties before validate_bucket_schema',
        bucket_to_validate,
      );
      nsfs_schema_utils.validate_bucket_schema(bucket_to_validate);
      await bucket_policy_utils.validate_s3_policy(
        bucket.s3_policy,
        bucket.name,
        async principal => this._get_account_by_name(principal),
      );
      const update_bucket = JSON.stringify(bucket);
      await nb_native().fs.writeFile(
        this.fs_context,
  ...
  async delete_bucket_policy(params) {
    try {
      ...
      dbg.log2(
        'delete_bucket_policy: bucket properties before validate_bucket_schema',
        bucket,
      );
      // on the safe side validate before changing configuration
      nsfs_schema_utils.validate_bucket_schema(bucket);
  ...
  async _has_access_to_nsfs_dir(ns, object_sdk) {
    ...
    if (
      !account ||
      !account.nsfs_account_config ||
      _.isUndefined(account.nsfs_account_config.uid) ||
      _.isUndefined(account.nsfs_account_config.gid)
    ) {
      return false;
    }
    try {
      ...
      const fs_context = prepare_fs_context(
        object_sdk,
      ...
      if (
        err.code === 'ENOENT' ||
        err.code === 'EACCES' ||
        (err.code === 'EPERM' && err.message === 'Operation not permitted')
      ) {
        return false;
      }
      throw err;
