...
class BucketSpaceFS extends BucketSpaceSimpleFS {
    ...
    async read_account_by_access_key({ access_key }) {
        try {
            ...
            const account =
                access_key === anonymous_access_key ?
                    await this.config_fs.get_account_by_name(
            ...
            dbg.error(
                'BucketSpaceFS.read_account_by_access_key: failed with error',
                err,
            );
            if (err.code === 'ENOENT') {
                throw new RpcError(
            ...
            new NoobaaEvent(NoobaaEvent.ACCOUNT_NOT_FOUND).create_event(
                access_key,
    ...
    async read_bucket_sdk_info({ name }) {
        try {
            ...
            if (!is_valid) {
                dbg.warn(
                    'BucketSpaceFS: one or more bucket config check is failed for bucket : ',
                    name,
                );
            }
            ...
            bucket.namespace = {
                ...
                should_create_underlying_storage:
                    bucket.should_create_underlying_storage,
            };
            ...
            if (bucket.s3_policy) {
                for (const [
                    s_index,
                    statement,
                ] of bucket.s3_policy.Statement.entries()) {
                    const statement_principal =
                        statement.Principal || statement.NotPrincipal;
                    if (statement_principal.AWS) {
                        const sensitive_arr = _.flatten([
                            statement_principal.AWS,
                        ]).map(principal => new SensitiveString(principal));
                        if (statement.Principal) {
                            bucket.s3_policy.Statement[s_index].Principal.AWS =
                                sensitive_arr;
                        }
                        if (statement.NotPrincipal) {
                            bucket.s3_policy.Statement[
                                s_index
                            ].NotPrincipal.AWS = sensitive_arr;
                        }
                    } else {
                        const sensitive_principal = new SensitiveString(
                            statement_principal,
                        );
                        if (statement.Principal) {
                            bucket.s3_policy.Statement[s_index].Principal =
                                sensitive_principal;
                        }
                        if (statement.NotPrincipal) {
                            bucket.s3_policy.Statement[s_index].NotPrincipal =
                                sensitive_principal;
                        }
                    }
                }
            ...
            if (err.rpc_code === 'INVALID_SCHEMA') {
                err.rpc_code = 'INVALID_BUCKET_STATE';
            }
            new NoobaaEvent(NoobaaEvent[rpc_error.rpc_code]).create_event(
                name,
                { bucket_name: name },
                err,
            );
    ...
    async list_buckets(object_sdk) {
        ...
        try {
            ...
            if (err.code === 'ENOENT') {
                dbg.error(
                    'BucketSpaceFS: root dir not found',
                    err,
                    this.config_fs.buckets_dir_path,
                );
        ...
        const buckets = await P.map_with_concurrency(
            10,
            bucket_names,
            async bucket_name => {
                let bucket;
                try {
                    bucket =
                        await object_sdk.read_bucket_sdk_config_info(
                            bucket_name,
                        );
                } catch (err) {
                    dbg.warn(
                        'list_buckets: read_bucket_sdk_config_info of bucket',
                        bucket_name,
                        'got an error',
                        err,
                    );
                ...
                if (!bucket) return;
                const bucket_policy_accessible =
                    await this.has_bucket_action_permission(
                        bucket,
                        account,
                        's3:ListBucket',
                    );
                dbg.log2(
                    `list_buckets: bucket_name ${bucket_name} bucket_policy_accessible`,
                    bucket_policy_accessible,
                );
                if (!bucket_policy_accessible) return;
                const fs_accessible = await this.validate_fs_bucket_access(
                    bucket,
                    object_sdk,
                );
    ...
    async create_bucket(params, sdk) {
        return bucket_semaphore.surround_key(String(params.name), async () => {
            if (!sdk.requesting_account.allow_bucket_creation) {
                throw new RpcError(
                    'UNAUTHORIZED',
                    'Not allowed to create new buckets',
                );
            }
            // currently we do not allow IAM account to create a bucket (temporary)
            if (sdk.requesting_account.owner !== undefined) {
                dbg.warn(
                    'create_bucket: account is IAM account (currently not allowed to create buckets)',
                );
                throw new RpcError(
                    'UNAUTHORIZED',
                    'Not allowed to create new buckets',
            ...
            if (
                !sdk.requesting_account.nsfs_account_config ||
            ...
            const bucket_config_path =
                this.config_fs.get_bucket_path_by_name(name);
            ...
            const bucket = this.new_bucket_defaults(
            ...
            try {
                ...
                new NoobaaEvent(
                    NoobaaEvent.BUCKET_CREATION_FAILED,
                ).create_event(name, { bucket_name: name }, err);
                throw translate_error_codes(err, entity_enum.BUCKET);
            }
            ...
            try {
                ...
                dbg.error(
                    'BucketSpaceFS: create_bucket could not create underlying directory - nsfs, deleting bucket',
                    err,
                );
                new NoobaaEvent(
                    NoobaaEvent.BUCKET_DIR_CREATION_FAILED,
                ).create_event(
                    name,
                    { bucket: name, path: bucket_storage_path },
                    err,
                );
    ...
    new_bucket_defaults(
        account,
        { name, tag, lock_enabled, force_md5_etag },
        create_uls,
        bucket_storage_path,
    ) {
        return {
            ...
            creator: account._id,
            versioning:
                config.NSFS_VERSIONING_ENABLED && lock_enabled ?
                    'ENABLED'
                :   'DISABLED',
    ...
    async delete_bucket(params, object_sdk) {
        ...
        return bucket_semaphore.surround_key(String(name), async () => {
            ...
            try {
                const { ns, bucket } =
                    await object_sdk.read_bucket_full_info(name);
                const namespace_bucket_config = bucket && bucket.namespace;
                dbg.log1(
                    'BucketSpaceFS.delete_bucket: namespace_bucket_config',
                    namespace_bucket_config,
                );
                if (!namespace_bucket_config) {
                    throw new RpcError(
                        'INTERNAL_ERROR',
                ...
                if (namespace_bucket_config.should_create_underlying_storage) {
                    ...
                    dbg.log1(
                        'BucketSpaceFS.delete_bucket: deleting uls',
                        namespace_bucket_config.write_resource.path,
                    );
                    const bucket_storage_path =
                        namespace_bucket_config.write_resource.path; // includes write_resource.path + bucket name (s3 flow)
                    try {
                        await ns.delete_uls(
                            { name, full_path: bucket_storage_path },
                            object_sdk,
                        );
                    } catch (err) {
                        dbg.warn(
                            ...
                            name,
                            'with bucket_storage_path',
                            bucket_storage_path,
                            'got an error while trying to delete_uls',
                            err,
                        );
                    ...
                    try {
                        if (ns._is_versioning_disabled()) {
                            list = await ns.list_objects(
                                { ...params, bucket: name, limit: 1 },
                                object_sdk,
                            );
                        } else {
                            list = await ns.list_object_versions(
                                { ...params, bucket: name, limit: 1 },
                                object_sdk,
                            );
                        }
                        ...
                        dbg.warn(
                            'delete_bucket: bucket name',
                            name,
                            'got an error while trying to list_objects',
                            err,
                        );
                        // in case the ULS was deleted - we will continue
                        if (err.rpc_code !== 'NO_SUCH_BUCKET') throw err;
                    ...
                    const bucket_tmpdir_path = get_bucket_tmpdir_full_path(
                        namespace_bucket_config.write_resource.path,
                        bucket._id,
                    );
                    await folder_delete(
                ...
                dbg.log1(
                    `BucketSpaceFS: delete_bucket: deleting config file ${bucket_config_path}`,
                );
                await this.config_fs.delete_bucket_config_file(name);
                new NoobaaEvent(NoobaaEvent.BUCKET_DELETE).create_event(name, {
                    bucket_name: name,
                });
            } catch (err) {
                dbg.error(
                    'BucketSpaceFS: delete_bucket: bucket name',
                    name,
                    'error',
                    err,
                );
                new NoobaaEvent(NoobaaEvent.BUCKET_DELETE_FAILED).create_event(
                    name,
                    { bucket_name: name, bucket_path: bucket_config_path },
                    err,
                );
                throw translate_error_codes(err, entity_enum.BUCKET);
            }
    ...
    async get_bucket_lifecycle_configuration_rules(params) {
        try {
            const { name } = params;
            dbg.log0(
                'BucketSpaceFS.get_bucket_lifecycle_configuration_rules: Bucket name',
                name,
            );
            const bucket = await this.config_fs.get_bucket_by_name(name);
    ...
    async set_bucket_lifecycle_configuration_rules(params) {
        try {
            ...
            dbg.log0(
                'BucketSpaceFS.set_bucket_lifecycle_configuration_rules: Bucket name, rules',
                name,
                rules,
            );
            const bucket = await this.config_fs.get_bucket_by_name(name);
    ...
    async delete_bucket_lifecycle(params) {
        try {
            const { name } = params;
            dbg.log0(
                'BucketSpaceFS.delete_bucket_lifecycle: Bucket name',
                name,
            );
            const bucket = await this.config_fs.get_bucket_by_name(name);
    ...
    async set_bucket_versioning(params, object_sdk) {
        try {
            ...
            dbg.log0(
                'BucketSpaceFS.set_bucket_versioning: Bucket name, versioning',
                name,
                versioning,
            );
            const bucket = await this.config_fs.get_bucket_by_name(name);
    ...
    async put_bucket_tagging(params) {
        try {
            ...
            dbg.log0(
                'BucketSpaceFS.put_bucket_tagging: Bucket name, tagging',
                name,
                tagging,
            );
            const bucket = await this.config_fs.get_bucket_by_name(name);
    ...
    async put_bucket_logging(params) {
        try {
            ...
            dbg.log0(
                'BucketSpaceFS.put_bucket_logging: Bucket name, logging',
                name,
                logging,
            );
            const bucket = await this.config_fs.get_bucket_by_name(name);
            ...
            let target_bucket;
            try {
                target_bucket = await this.config_fs.get_bucket_by_name(
                    logging.log_bucket,
                );
            } catch (err) {
                dbg.error(
                    'ERROR with reading TARGET BUCKET data',
                    logging.log_bucket,
                    err,
                );
                if (err.code === 'ENOENT') {
            ...
            if (target_bucket.owner_account !== bucket.owner_account) {
                dbg.error(
                    'TARGET BUCKET NOT OWNED BY USER',
                    target_bucket,
                    bucket,
                );
                throw new RpcError(
    ...
    async put_bucket_encryption(params) {
        try {
            ...
            dbg.log0(
                'BucketSpaceFS.put_bucket_encryption: Bucket name, encryption',
                name,
                encryption,
            );
            const bucket = await this.config_fs.get_bucket_by_name(name);
    ...
    async delete_bucket_encryption(params) {
        try {
            const { name } = params;
            dbg.log0(
                'BucketSpaceFS.delete_bucket_encryption: Bucket name',
                name,
            );
            const bucket = await this.config_fs.get_bucket_by_name(name);
    ...
    async put_bucket_website(params) {
        try {
            ...
            dbg.log0(
                'BucketSpaceFS.put_bucket_website: Bucket name, website',
                name,
                website,
            );
            const bucket = await this.config_fs.get_bucket_by_name(name);
    ...
    async put_bucket_policy(params) {
        try {
            ...
            dbg.log0(
                'BucketSpaceFS.put_bucket_policy: Bucket name, policy',
                name,
                policy,
            );
            const bucket = await this.config_fs.get_bucket_by_name(name);
            ...
            nsfs_schema_utils.validate_bucket_schema(
                _.omitBy(bucket, _.isUndefined),
            );
            await bucket_policy_utils.validate_s3_policy(
                bucket.s3_policy,
                bucket.name,
                async principal =>
                    this.config_fs.is_account_exists_by_principal(principal, {
                        silent_if_missing: true,
                    }),
            );
            await this.config_fs.update_bucket_config_file(bucket);
    ...
    async get_bucket_policy(params, object_sdk) {
        try {
            ...
            const bucket_policy_info =
                await object_sdk.read_bucket_sdk_policy_info(name);
            dbg.log0(
                'BucketSpaceFS.get_bucket_policy: policy',
                bucket_policy_info,
            );
    ...
    async has_bucket_action_permission(
        bucket,
        account,
        action,
        bucket_path = '',
        ...
        dbg.log1(
            ...
            account.name.unwrap(),
            account._id,
            bucket.bucket_owner.unwrap(),
        );

        const is_system_owner =
            Boolean(bucket.system_owner) &&
            bucket.system_owner.unwrap() === account.name.unwrap();

        // If the system owner account wants to access the bucket, allow it
        if (is_system_owner) return true;
        const is_owner =
            bucket.owner_account && bucket.owner_account.id === account._id;
        ...
        if (!bucket_policy) {
            ...
            const is_iam_and_same_root_account_owner =
    ...
    async validate_fs_bucket_access(bucket, object_sdk) {
        dbg.log1(
            'bucketspace_fs.validate_fs_bucket_access:',
            bucket.name.unwrap(),
        );
    ...
    async _has_access_to_nsfs_dir(ns, object_sdk) {
        ...
        if (
            !account ||
            !account.nsfs_account_config ||
            account.nsfs_account_config.uid === undefined ||
            account.nsfs_account_config.gid === undefined
        ) {
            return false;
        }
        try {
            dbg.log1(
                '_has_access_to_nsfs_dir: checking access:',
                ns.write_resource,
                account.nsfs_account_config.uid,
                account.nsfs_account_config.gid,
            );
            const path_to_check = path.join(
                ns.write_resource.resource.fs_root_path,
                ns.write_resource.path || '',
            );
            const fs_context = this.prepare_fs_context(
                object_sdk,
                ns.write_resource.resource.fs_backend,
            );
