...
const XATTR_METADATA_IGNORE_LIST = [XATTR_STORAGE_CLASS_KEY];
        ...
        if (_is_version_null_version(old_version.name)) {
            try {
                const stat = await nb_native().fs.stat(
                    fs_context,
                    path.join(version_path, old_version.name),
                );
                ...
                dbg.error(
                    `_rename_null_version of ${old_version.name} got error:`,
                    err,
                );
 ...
 * @returns
 */
async function is_directory_or_symlink_to_directory(
    stat,
    fs_context,
    entry_path,
) {
    try {
        ...
        if (!r && is_symbolic_link(stat)) {
            const targetStat = await nb_native().fs.stat(
                fs_context,
                entry_path,
            );
            if (!targetStat) {
                throw new Error(
                    'is_directory_or_symlink_to_directory: targetStat is empty',
                );
            }
...
function to_xattr(fs_xattr) {
    const xattr = _.mapKeys(fs_xattr, (val, key) => {
        ...
        if (
            key.startsWith(XATTR_USER_PREFIX) &&
            !key.startsWith(XATTR_NOOBAA_INTERNAL_PREFIX)
        ) {
...
const versions_dir_cache = new LRUCache({
    ...
    load: async ({ dir_path, fs_context }) => {
        ...
        let usage = config.NSFS_DIR_CACHE_MIN_DIR_SIZE;
        if (
            stat.size + ver_dir_stat_size <=
            config.NSFS_DIR_CACHE_MAX_DIR_SIZE
        ) {
            const latest_versions = await nb_native().fs.readdir(
                fs_context,
                dir_path,
            );
            if (is_version_path_exists) {
                const old_versions = await nb_native().fs.readdir(
                    fs_context,
                    version_path,
                );
                ...
                const { renamed_null_versions_set, old_versions_after_rename } =
                    await _rename_null_version(
                        old_versions,
                        fs_context,
                        version_path,
                    );
                const entries = latest_versions.concat(
                ...
                if (renamed_null_versions_set.size > 0) {
                    for (const ent of sorted_entries) {
                        if (renamed_null_versions_set.has(ent.name)) {
                            ...
                            const version_name_with_null =
                                file_name + NULL_VERSION_SUFFIX;
    ...
    validate: async ({ stat, ver_dir_stat }, { dir_path, fs_context }) => {
        const new_stat = await nb_native().fs.stat(fs_context, dir_path);
        const versions_dir_path = path.normalize(
            path.join(dir_path, '/', HIDDEN_VERSIONS_PATH),
        );
        let new_versions_stat;
        try {
            new_versions_stat = await nb_native().fs.stat(
                fs_context,
                versions_dir_path,
            );
        } catch (err) {
            if (err.code === 'ENOENT') {
                dbg.log0(
                    'NamespaceFS: Version dir not found, ',
                    versions_dir_path,
                );
            } else {
                throw err;
...
class NamespaceFS {
    ...
    constructor({
        ...
        this.versioning =
            (config.NSFS_VERSIONING_ENABLED && versioning) ||
            versioning_status_enum.VER_DISABLED;
        this.stats = stats;
    ...
    prepare_fs_context(object_sdk) {
        ...
        if (!fs_context) {
            throw new RpcError(
                'UNAUTHORIZED',
                'nsfs_account_config is missing',
            );
        }
    ...
    get_bucket_tmpdir_full_path() {
        return native_fs_utils.get_bucket_tmpdir_full_path(
            this.bucket_path,
            this.bucket_id,
        );
    }
    ...
    run_update_issues_report(object_sdk, err) {
        if (!config.NSFS_UPDATE_ISSUES_REPORT_ENABLED) {
            dbg.log0(
                'update_issues_report disabled:',
                this.namespace_resource_id,
                err,
            );
            return;
        }
    ...
    _should_update_issues_report(params, file_path, err) {
        ...
        const no_such_key_condition =
            err.code === `ENOENT` && non_internal_path;
    ...
    async _list_objects(params, object_sdk, list_versions) {
        try {
            ...
            const limit = Math.min(
                1000,
                _.isUndefined(params.limit) ? 1000 : params.limit,
            );
            ...
            const process_dir = async dir_key => {
                ...
                const insert_entry_to_results_arr = async r => {
                    ...
                    if (
                        results.length &&
                        r.key < results[results.length - 1].key &&
                ...
                const process_entry = async ent => {
                    ...
                    if (
                        list_versions &&
                        _is_version_or_null_in_file_name(ent.name)
                    ) {
                        r = {
                ...
                try {
                    ...
                    if (['ENOENT', 'ENOTDIR'].includes(err.code)) {
                        dbg.log0(
                            'NamespaceFS: no keys for non existing dir',
                            dir_path,
                        );
                        return;
                    }
                ...
                const is_dir_content =
                    cached_dir.stat.xattr &&
                    cached_dir.stat.xattr[XATTR_DIR_CONTENT];
                if (
                    is_dir_content &&
                    dir_key > key_marker &&
                    (!delimiter || dir_key === prefix)
                ) {
                ...
                if (cached_dir.sorted_entries) {
                    ...
                    if (marker_index) {
                        ...
                        if (
                            marker_curr.startsWith(prev_dir_name) &&
                            dir_key !== prev_dir.name
                        ) {
                            if (!delimiter) {
                                const isDir =
                                    await is_directory_or_symlink_to_directory(
                                        prev_dir,
                                        fs_context,
                                ...
                                if (isDir) {
                                    await process_dir(
                                        path.join(dir_key, prev_dir_name, '/'),
                                    );
                                }
                            }
                        }
                ...
                try {
                    ...
                    dir_handle = await nb_native().fs.opendir(
                        fs_context,
                        dir_path,
                    ); //, { bufferSize: 128 });
                    for (;;) {
            ...
            await Promise.all(
                results.map(async r => {
                    ...
                    const use_lstat =
                        !(await this._is_path_in_bucket_boundaries(
                            fs_context,
                            entry_path,
                        ));
            ...
            for (const r of results) {
                ...
                if (r.common_prefix) {
                    ...
                    obj_info = this._get_object_info(
                        bucket,
                        r.key,
                        r.stat,
                        false,
                    ...
                    if (this._is_hidden_version_path(obj_info.key)) {
                        obj_info.key = path.normalize(
                            obj_info.key.replace(
                                HIDDEN_VERSIONS_PATH + '/',
                                '',
                            ),
                        );
                ...
                if (res.is_truncated) {
                    if (list_versions && _is_version_object(r.key)) {
                        const next_version_id_marker = r.key.substring(
                            r.key.lastIndexOf('/') + 1,
                        );
            ...
            throw native_fs_utils.translate_error_codes(
                err,
                native_fs_utils.entity_enum.OBJECT,
            );
        }
    }
        ...
        try {
            ...
            if (isDir) {
                if (
                    ...
                    const dir_content_path = await this._find_version_path(
                        fs_context,
                        params,
                    );
                    const dir_content_path_stat = await nb_native().fs.stat(
                        fs_context,
                        dir_content_path,
                    );
    ...
    async read_object_stream(params, object_sdk, res) {
        ...
        try {
            ...
            const is_dir_content = this._is_directory_content(
                file_path,
                params.key,
            );
            if (is_dir_content) {
                try {
                    ...
                    const dir_stat = await nb_native().fs.stat(
                        fs_context,
                        md_path,
                    );
                    if (dir_stat && dir_stat.xattr[XATTR_DIR_CONTENT] === '0') {
                        return null;
                    }
                } catch (err) {
                    //failed to get object
                    new NoobaaEvent(NoobaaEvent.OBJECT_GET_FAILED).create_event(
                        params.key,
                        {
                    ...
                    dbg.log0(
                        'NamespaceFS: read_object_stream couldnt find dir content xattr',
                        err,
                    );
                }
            }
            ...
            const end =
                isNaN(Number(params.end)) ? Infinity : Number(params.end);
            ...
            for (let pos = start; pos < end; ) {
                ...
                if (
                    config.NSFS_BUF_WARMUP_SPARSE_FILE_READS &&
                    is_sparse_file(stat)
                ) {
                    dbg.log0(
                        'NamespaceFS: read_object_stream - warmup sparse file',
                        {
                            file_path,
                            pos,
                            size: stat.size,
                ...
                const { buffer, callback } = await multi_buffer_pool
                    .get_buffers_pool(remain_size)
                    .get_buffer();
                ...
                log2_size_histogram[log2_size] =
                    (log2_size_histogram[log2_size] || 0) + 1;
            ...
            dbg.log0(
                'NamespaceFS: read_object_stream completed file',
                file_path,
                {
                    num_bytes,
                    num_buffers,
            ...
            dbg.log0(
                'NamespaceFS: read_object_stream error file',
                file_path,
                err,
            );
            //failed to get object
            new NoobaaEvent(NoobaaEvent.OBJECT_STREAM_GET_FAILED).create_event(
                params.key,
                { bucket_path: this.bucket_path, object_name: params.key },
                err,
            );
            throw native_fs_utils.translate_error_codes(
                err,
            ...
            try {
                if (file) {
                    dbg.log0(
                        'NamespaceFS: read_object_stream finally closing file',
                        file_path,
                    );
                    await file.close(fs_context);
                }
                ...
                dbg.warn(
                    'NamespaceFS: read_object_stream file close error',
                    err,
                );
            }
            try {
                ...
                //failed to get object
                new NoobaaEvent(NoobaaEvent.OBJECT_CLEANUP_FAILED).create_event(
                    params.key,
                    { bucket_path: this.bucket_path, object_name: params.key },
                    err,
                );
                dbg.warn(
                    'NamespaceFS: read_object_stream buffer pool cleanup error',
                    err,
                );
            }
        }
    ...
    async upload_object(params, object_sdk) {
        ...
        try {
            ...
            if (this.empty_dir_content_flow(file_path, params)) {
                const content_dir_info = await this._create_empty_dir_content(
                    fs_context,
                    params,
                    file_path,
                );
            ...
            await this._throw_if_storage_class_not_supported(
                params.storage_class,
            );

            upload_params = await this._start_upload(
                fs_context,
                object_sdk,
                file_path,
                params,
            ...
            if (
                !params.copy_source ||
                upload_params.copy_res === copy_status_enum.FALLBACK
            ) {
                ...
                const upload_res = await bp.sem.surround_count(
                    bp.buf_size,
                    async () => this._upload_stream(upload_params),
                );
            ...
            this.run_update_issues_report(object_sdk, err);
            //filed to put object
            new NoobaaEvent(NoobaaEvent.OBJECT_UPLOAD_FAILED).create_event(
                params.key,
                { bucket_path: this.bucket_path, object_name: params.key },
                err,
            );
            dbg.warn(
                'NamespaceFS: upload_object buffer pool cleanup error',
                err,
            );
            throw native_fs_utils.translate_error_codes(
                err,
            ...
            try {
                if (upload_params && upload_params.target_file) {
                    await upload_params.target_file.close(fs_context);
                }
            } catch (err) {
    ...
    async _start_upload(fs_context, object_sdk, file_path, params, open_mode) {
        const force_copy_fallback = await this._check_copy_storage_class(
            fs_context,
            params,
        );
        ...
        if (force_copy_fallback) {
            ...
            copy_res = await this._try_copy_file(
                fs_context,
                params,
                file_path,
                upload_path,
        ...
        if (copy_res) {
            if (copy_res === copy_status_enum.FALLBACK) {
                ...
                if (copy_res === copy_status_enum.SAME_INODE) {
                    open_path = file_path;
                }
            }
        }
        const target_file = await native_fs_utils.open_file(
            fs_context,
            this.bucket_path,
            open_path,
            open_mode,
    ...
    async _try_copy_file(fs_context, params, file_path, upload_path) {
        const source_file_path = await this._find_version_path(
            fs_context,
            params.copy_source,
        );
        await this._check_path_in_bucket_boundaries(
            fs_context,
            source_file_path,
        );
    ...
    async _check_copy_storage_class(fs_context, params) {
        if (params.copy_source) {
            const src_file_path = await this._find_version_path(
                fs_context,
                params.copy_source,
            );
            const stat = await nb_native().fs.stat(fs_context, src_file_path);
            const src_storage_class = s3_utils.parse_storage_class(
                stat.xattr[XATTR_STORAGE_CLASS_KEY],
            );
            const src_restore_status = GlacierBackend.get_restore_status(
                stat.xattr,
                new Date(),
                src_file_path,
            );
            ...
            if (src_storage_class === s3_utils.STORAGE_CLASS_GLACIER) {
                if (
                    src_restore_status?.ongoing ||
                    !src_restore_status?.expiry_time
                ) {
                    dbg.warn(
                        '_validate_upload: object is not restored yet',
                        src_restore_status,
                    );
        ...
        return (
            params.copy_source &&
            params.storage_class === s3_utils.STORAGE_CLASS_GLACIER
        );
    }
    ...
    async _finish_upload({
        ...
        const same_inode =
            params.copy_source && copy_res === copy_status_enum.SAME_INODE;
        const is_dir_content = this._is_directory_content(
            file_path,
            params.key,
        );
        ...
        this._verify_encryption(
            params.encryption,
            this._get_encryption_info(stat),
        );
        ...
        if (part_upload) {
            fs_xattr = await this._assign_part_props_to_fs_xattr(
                fs_context,
                params.size,
                digest,
                offset,
        ...
        if (
            !part_upload &&
            (this._is_versioning_enabled() || this._is_versioning_suspended())
        ) {
            const cur_ver_info = await this._get_version_info(
                fs_context,
                file_path,
            );
            fs_xattr = await this._assign_versions_to_fs_xattr(
                fs_context,
                cur_ver_info,
                stat,
                params.key,
        ...
        if (!same_inode && !part_upload) {
            ...
            if (config.NSFS_TRIGGER_FSYNC) {
                await nb_native().fs.fsync(fs_context, path.dirname(file_path));
            }
        }
        ...
        if (is_dir_content) {
            if (params.copy_source) {
                fs_xattr = await this._get_copy_source_xattr(
                    params,
                    fs_context,
                    fs_xattr,
                );
            }
            await this._assign_dir_content_to_xattr(
                fs_context,
                fs_xattr,
        ...
        const upload_info = this._get_upload_info(
            stat,
            fs_xattr && fs_xattr[XATTR_VERSION_ID],
        );
        return upload_info;
    }
    ...
    async _create_empty_dir_content(fs_context, params, file_path) {
        ...
        try {
            ...
            dbg.log0(
                `namespace_fs._create_empty_dir_content: dir object file ${config.NSFS_FOLDER_OBJECT_NAME} was already deleted`,
            );
        }
    ...
    async _get_copy_source_xattr(params, fs_context, fs_xattr) {
        ...
        const source_file_md_path = await this._find_version_path(
            fs_context,
            params.copy_source,
            is_source_dir,
        );
        const source_stat = await nb_native().fs.stat(
            fs_context,
            source_file_md_path,
        );
        ...
        for (;;) {
            try {
                ...
                if (this._is_versioning_disabled()) {
                    ...
                    await this._move_to_dest_version(
                        fs_context,
                        source_path,
                        dest_path,
                        target_file,
                ...
                if (
                    source_path &&
                    !(await this.check_access(fs_context, source_path))
                ) {
                    throw err;
                }
                dbg.warn(
    ...
    async _move_to_dest_version(
        ...
        dbg.log1(
            'Namespace_fs._move_to_dest_version:',
            new_ver_tmp_path,
            latest_ver_path,
            upload_file,
        );
        ...
        for (;;) {
            try {
                const new_ver_info =
                    !is_gpfs &&
                    (await this._get_version_info(
                        fs_context,
                        new_ver_tmp_path,
                    ));
                // get latest version_id if exists
                const latest_ver_info = await this._get_version_info(
                    fs_context,
                    latest_ver_path,
                );
                const versioned_path =
                    latest_ver_info &&
                    this._get_version_path(key, latest_ver_info.version_id_str);
                const versioned_info =
                    latest_ver_info &&
                    (await this._get_version_info(fs_context, versioned_path));

                gpfs_options =
                    is_gpfs ?
                        await this._open_files_gpfs(
                            fs_context,
                            new_ver_tmp_path,
                            latest_ver_path,
                            upload_file,
                ...
                if (this._is_versioning_suspended()) {
                    if (latest_ver_info?.version_id_str === NULL_VERSION_ID) {
                        dbg.log1(
                            'NamespaceFS._move_to_dest_version suspended: version ID of the latest version is null - the file will be unlinked',
                        );
                        ...
                        await this._delete_null_version_from_versions_directory(
                            key,
                            fs_context,
                        );
                    }
                ...
                if (
                    ...
                    (this._is_versioning_enabled() ||
                        (this._is_versioning_suspended() &&
                            latest_ver_info.version_id_str !== NULL_VERSION_ID))
                ) {
                    dbg.log1(
                        'NamespaceFS._move_to_dest_version version ID of the latest version is a unique ID - the file will be moved it to .versions/ directory',
                    );
                    await native_fs_utils._make_path_dirs(
                        versioned_path,
                        fs_context,
                    );
                    await native_fs_utils.safe_move(
                        fs_context,
                        latest_ver_path,
                        versioned_path,
                        latest_ver_info,
                ...
                try {
                    ...
                    if (
                        err.message !==
                            native_fs_utils.posix_unlink_retry_err &&
                        err.code !== native_fs_utils.gpfs_unlink_retry_catch
                    ) {
                        throw err;
                    }
                    dbg.warn(
                        'Namespace_fs._move_to_dest_version: unable to delete new version tmp file, ignoring...',
                    );
                }
                break;
            } catch (err) {
                ...
                if (
                    retries <= 0 ||
                    !native_fs_utils.should_retry_link_unlink(is_gpfs, err)
                ) {
                    throw err;
                }
                dbg.warn(
                    `NamespaceFS._move_to_dest_version retrying retries=${retries}` +
                        ` new_ver_tmp_path=${new_ver_tmp_path} latest_ver_path=${latest_ver_path}`,
                    err,
                );
            } finally {
                if (gpfs_options) {
                    await this._close_files_gpfs(
                        fs_context,
                        gpfs_options.move_to_dst,
                        open_mode,
                    );
                }
            }
    ...
    async _is_same_inode(fs_context, source_file_path, file_path) {
        try {
            ...
            if (
                file_path_inode === source_file_inode &&
                file_path_device === source_file_device
            ) {
    ...
    async _upload_stream({
        ...
        try {
            ...
            const chunk_fs = new ChunkFS({
                ...
                large_buf_size:
                    multi_buffer_pool.get_buffers_pool(undefined).buf_size,
            });
            chunk_fs.on('error', err1 =>
                dbg.error(
                    'namespace_fs._upload_stream: error occured on stream ChunkFS: ',
                    err1,
                ),
            );
            ...
            return {
                ...
                total_bytes: chunk_fs.total_bytes,
            };
        } catch (error) {
    ...
    async list_uploads(params, object_sdk) {
        ...
        const multipart_upload_dirs = await nb_native().fs.readdir(
            fs_context,
            mpu_root_path,
        );
        const common_prefixes_set = new Set();
        const multipart_uploads = await P.map(
            multipart_upload_dirs,
            async obj => {
                ...
                const { data: create_params_buffer } =
                    await nb_native().fs.readFile(fs_context, create_path);
                const create_params_parsed = JSON.parse(
                    create_params_buffer.toString(),
                );

                // dont add keys that dont start with the prefix
                if (
                    params.prefix &&
                    !create_params_parsed.key.startsWith(params.prefix)
                ) {
                    return undefined;
                }
                ...
                if (params.delimiter) {
                    ...
                    const delimiter_idx = create_params_parsed.key.indexOf(
                        params.delimiter,
                        start_idx,
                    );
                    if (delimiter_idx > 0) {
                        common_prefixes_set.add(
                            create_params_parsed.key.substring(
                                0,
                                delimiter_idx + 1,
                            ),
                        );
    ...
    async upload_multipart(params, object_sdk) {
        ...
        try {
            ...
            part_md_file = await native_fs_utils.open_file(
                fs_context,
                this.bucket_path,
                md_upload_path,
                md_open_mode,
            ...
            let md_upload_params = {
                ...
                params,
                object_sdk,
                upload_path: md_upload_path,
                open_mode: md_open_mode,
                target_file: part_md_file,
                file_path: md_upload_path,
            };
            ...
            dbg.log1(
                `NamespaceFS: upload_multipart, data path=${upload_path} offset=${offset} data_open_mode=${data_open_mode}`,
            );

            target_file = await native_fs_utils.open_file(
                fs_context,
                this.bucket_path,
                upload_path,
                data_open_mode,
            ...
            if (pre_known_size) {
                ...
                await native_fs_utils.copy_bytes(
                    multi_buffer_pool,
                    fs_context,
                    part_md_file,
                    target_file,
            ...
            await native_fs_utils.finally_close_files(fs_context, [
                target_file,
                part_md_file,
            ]);
        }
    ...
    async complete_object_upload(params, object_sdk) {
        ...
        try {
            const md5_enabled = this._is_force_md5_enabled(object_sdk);
            const MD5Async =
                md5_enabled ? new (nb_native().crypto.MD5Async)() : undefined;
            ...
            for (const { num, etag } of multiparts) {
                ...
                const md_part_stat = await nb_native().fs.stat(
                    fs_context,
                    md_part_path,
                );
                const part_size = Number(md_part_stat.xattr[XATTR_PART_SIZE]);
                const part_offset = Number(
                    md_part_stat.xattr[XATTR_PART_OFFSET],
                );
                ...
                const data_part_path = this._get_part_data_path({
                    ...params,
                    size: part_size,
                });
                if (part_size_to_fd_map.has(part_size)) {
                    ...
                    read_file = await native_fs_utils.open_file(
                        fs_context,
                        this.bucket_path,
                        data_part_path,
                        config.NSFS_OPEN_READ_MODE,
                ...
                if (part_size_to_fd_map.size === 1) {
                    ...
                    part_size_to_fd_map.size === 2 &&
                    should_copy_file_prefix
                ) {
                    // 2
                    if (num === multiparts.length) {
                        const prev_data_part_path = this._get_part_data_path({
                            ...params,
                            size: prev_part_size,
                        });
                        await nb_native().fs.link(
                        ...
                        const prev_read_file =
                            part_size_to_fd_map.get(prev_part_size);
                        if (!target_file) {
                            target_file = await native_fs_utils.open_file(
                                fs_context,
                                this.bucket_path,
            ...
            upload_params.params.storage_class =
                create_params_parsed.storage_class;
            upload_params.digest =
                MD5Async &&
            ...
            if (config.NSFS_REMOVE_PARTS_ON_COMPLETE) {
                await native_fs_utils.folder_delete(
                    params.mpu_path,
                    fs_context,
                );
            }
    ...
    async complete_object_upload_finally(
        buffer_pool_cleanup,
        read_file_arr,
        write_file,
        fs_context,
        ...
        try {
            ...
            dbg.warn(
                'NamespaceFS: complete_object_upload buffer pool cleanup error',
                err,
            );
        }
        ...
        try {
            ...
            dbg.warn(
                'NamespaceFS: complete_object_upload write file close error',
                err,
            );
        }
    }
    ...
    async delete_object(params, object_sdk) {
        try {
            ...
            if (this._is_versioning_disabled()) {
                ...
                res =
                    params.version_id ?
                        await this._delete_version_id(
                            fs_context,
                            file_path,
                            params,
                        )
                    :   await this._delete_latest_version(
                            fs_context,
                            file_path,
                            params,
                        );
            ...
            if (this._is_versioning_disabled()) {
                for (const { key, version } of params.objects) {
                    ...
                    try {
                        ...
                        dbg.log1(
                            'NamespaceFS: delete_multiple_objects',
                            file_path,
                        );
                ...
                for (const { key, version_id } of params.objects) {
                    if (versions_by_key_map[key]) {
                        versions_by_key_map[key].push(version_id);
                    } else versions_by_key_map[key] = [version_id];
                }
                dbg.log3(
                    'NamespaceFS: versions_by_key_map',
                    versions_by_key_map,
                );
    ...
    async _delete_single_object(fs_context, file_path, params) {
        ...
        if (this._is_directory_content(file_path, params.key)) {
            await this._clear_user_xattr(
                fs_context,
                this._get_file_md_path(params),
                XATTR_USER_PREFIX,
            );
        }
    }
    ...
    async set_bucket_versioning(versioning, object_sdk) {
        if (!config.NSFS_VERSIONING_ENABLED) {
            throw new RpcError('BAD_REQUEST', 'nsfs versioning is unsupported');
        }
        try {
            const fs_context = this.prepare_fs_context(object_sdk);
            ...
            throw native_fs_utils.translate_error_codes(
                err,
                native_fs_utils.entity_enum.BUCKET,
            );
        }
    }
    ...
    async get_object_tagging(params, object_sdk) {
        ...
        try {
            const fs_context = this.prepare_fs_context(object_sdk);
            dbg.log0(
                'NamespaceFS.get_object_tagging: param ',
                params,
                'file_path :',
                file_path,
            );
            ...
            const stat = await file.stat(fs_context);
            if (stat.xattr) {
                for (const [xattr_key, xattr_value] of Object.entries(
                    stat.xattr,
                )) {
                    if (xattr_key.includes(XATTR_NOOBAA_CUSTOM_PREFIX)) {
                        tag_set.push({
                            key: xattr_key.replace(
            ...
            dbg.error(
                `NamespaceFS.get_object_tagging: failed in dir ${file_path} with error: `,
                err,
            );
            throw native_fs_utils.translate_error_codes(
                err,
        ...
        dbg.log0(
            'NamespaceFS.get_object_tagging: return tagging ',
            tag_set,
            'file_path :',
            file_path,
        );
    ...
    async delete_object_tagging(params, object_sdk) {
        ...
        try {
            ...
            dbg.error(
                `NamespaceFS.delete_object_tagging: failed in dir ${file_path} with error: `,
                err,
            );
            throw native_fs_utils.translate_error_codes(
                err,
    ...
    async put_object_tagging(params, object_sdk) {
        ...
        const fs_context = this.prepare_fs_context(object_sdk);
        dbg.log0(
            'NamespaceFS.put_object_tagging: fs_xattr ',
            fs_xattr,
            'file_path :',
            file_path,
        );
        try {
            ...
            dbg.error(
                `NamespaceFS.put_object_tagging: failed in dir ${file_path} with error: `,
                err,
            );
            throw native_fs_utils.translate_error_codes(
                err,
    ...
    */
    ...
    async restore_object(params, object_sdk) {
        ...
        const fs_context = this.prepare_fs_context(object_sdk);
        const file_path = await this._find_version_path(
            fs_context,
            params,
            false,
        );
        ...
        try {
            ...
            if (
                ...
                GlacierBackend.RESTORE_STATUS_CAN_RESTORE
            ) {
            ...
            if (
                restore_status.state === GlacierBackend.RESTORE_STATUS_ONGOING
            ) {
                throw new S3Error(S3Error.RestoreAlreadyInProgress);
            }

            if (
                restore_status.state === GlacierBackend.RESTORE_STATUS_RESTORED
            ) {
            ...
            dbg.error(
                'namespace_fs.restore_object: failed with error: ',
                error,
                file_path,
            );
            throw native_fs_utils.translate_error_codes(
    ...
    _get_file_md_path({ key }) {
        ...
        return this._is_directory_content(p, key) ?
    ...
    async _assign_versions_to_fs_xattr(
        fs_context,
        prev_ver_info,
        new_ver_stat,
        key,
        ...
        if (!prev_ver_info) {
            prev_ver_info = await this.find_max_version_past(fs_context, key);
        }
        ...
        if (prev_ver_info) {
            fs_xattr[XATTR_PREV_VERSION_ID] = prev_ver_info.version_id_str;
        }
    ...
    async _assign_part_props_to_fs_xattr(
        fs_context,
        size,
        digest,
        offset,
    ...
    async set_fs_xattr_op(fs_context, file_path, set, clear) {
        ...
        try {
            ...
            dbg.error(
                'NamespaceFS.handle_fs_xattr_op: failed with error: ',
                error,
                file_path,
            );
            throw native_fs_utils.translate_error_codes(
    ...
    async _clear_user_xattr(fs_context, file_path, prefix) {
        try {
            ...
            if (err.code !== 'ENOENT') throw err;
            dbg.log0(
                `NamespaceFS._clear_user_xattr: dir ${file_path} was already deleted`,
            );
        }
    }
     ...
     */
    async _assign_dir_content_to_xattr(
        fs_context,
        fs_xattr,
        params,
        copy_xattr,
    ...
    _is_directory_content(file_path, key) {
        return (
            file_path &&
            file_path.endsWith(config.NSFS_FOLDER_OBJECT_NAME) &&
            key &&
            key.endsWith('/')
        );
    }
    ...
    _get_object_info(bucket, key, stat, isDir, is_latest = true) {
        ...
        const version_id =
            (this._is_versioning_enabled() ||
                this._is_versioning_suspended()) &&
            this._get_version_id_by_xattr(stat);
        const delete_marker = stat.xattr?.[XATTR_DELETE_MARKER] === 'true';
        const dir_content_type =
            stat.xattr?.[XATTR_DIR_CONTENT] &&
            ((Number(stat.xattr?.[XATTR_DIR_CONTENT]) > 0 &&
                'application/octet-stream') ||
                'application/x-directory');
        const content_type =
            stat.xattr?.[XATTR_CONTENT_TYPE] ||
            (isDir && dir_content_type) ||
            mime.getType(key) ||
            'application/octet-stream';
        const storage_class = s3_utils.parse_storage_class(
            stat.xattr?.[XATTR_STORAGE_CLASS_KEY],
        );
        ...
        return {
            ...
            delete_marker,
            storage_class,
            restore_status: GlacierBackend.get_restore_status(
                stat.xattr,
                new Date(),
                this._get_file_path({ key }),
            ),
    ...
    _verify_encryption(user_encryption, fs_encryption) {
        if (
            user_encryption &&
            user_encryption.algorithm === 'AES256' &&
            !fs_encryption
        ) {
            dbg.error(
    ...
    async _load_bucket(params, fs_context) {
        try {
            ...
            dbg.warn(
                '_load_bucket failed, on bucket_path',
                this.bucket_path,
                'got error',
                err,
            );
    ...
    _mpu_path(params) {
        return path.join(this._mpu_root_path(), params.obj_id);
    }
    ...
    async _delete_path_dirs(file_path, fs_context) {
        try {
            ...
            if (
                ...
                dbg.log0(
                    'NamespaceFS: _delete_object_empty_path skip on unexpected error',
                    err,
                );
            }
        }
    ...
    async create_uls(params, object_sdk) {
        ...
        try {
            await nb_native().fs.mkdir(
                fs_context,
                params.full_path,
                native_fs_utils.get_umasked_mode(0o777),
            );
        } catch (err) {
            dbg.error(
                'NamespaceFS: create_uls fs_context:',
    ...
    async delete_uls(params, object_sdk) {
        ...
        try {
            let list;
            if (this._is_versioning_disabled()) {
                list = await this.list_objects(
                    { ...params, bucket: params.name, limit: 1 },
                    object_sdk,
                );
            } else {
                list = await this.list_object_versions(
                    { ...params, bucket: params.name, limit: 1 },
                    object_sdk,
                );
            }
            ...
            if (list && list.objects && list.objects.length > 0) {
                throw new RpcError(
                    'NOT_EMPTY',
                    'underlying directory has files in it',
                );
            }
    ...
    async _is_path_in_bucket_boundaries(fs_context, entry_path) {
        dbg.log1(
            'check_bucket_boundaries: fs_context',
            fs_context,
            'file_path',
            entry_path,
            'this.bucket_path',
        ...
        try {
            ...
            const full_path = await nb_native().fs.realpath(
                fs_context,
                entry_path,
            );
            ...
            if (err.code === 'ENOTDIR') {
                dbg.warn(
                    '_is_path_in_bucket_boundaries: the path',
                    entry_path,
                    'is not a directory',
                );
                return true;
            ...
            if (err.code === 'ENOENT') {
                return this._is_path_in_bucket_boundaries(
                    fs_context,
                    path.dirname(entry_path),
                );
            }
            ...
            throw error_utils.new_error_code(
                'INTERNAL_ERROR',
                'check_bucket_boundaries error ' +
                    err.code +
                    ' ' +
                    entry_path +
                    ' ' +
    ...
    async _check_path_in_bucket_boundaries(fs_context, entry_path) {
        ...
        if (
            !(await this._is_path_in_bucket_boundaries(fs_context, entry_path))
        ) {
            throw error_utils.new_error_code(
                'EACCES',
                'Entry ' + entry_path + ' is not in bucket boundaries',
            );
        }
    }
    ...
    _extract_version_info_from_xattr(version_id_str) {
        ...
        if (arr.length < 2) {
            throw new Error(
                'Invalid version_id_string, cannot extract version info',
            );
        }
        return {
    ...
    _get_version_path(key, version_id) {
        const key_version =
            path.basename(key) + (version_id ? '_' + version_id : '');
    ...
    async _get_version_info(fs_context, version_path) {
        try {
            ...
            const ver_info_by_xattr =
                this._extract_version_info_from_xattr(version_id_str);
            return {
            ...
            if (err.code !== 'ENOENT') throw err;
            dbg.warn(
                `NamespaceFS._get_version_info version of ${version_path} doesn't exist`,
                err,
            );
        }
    ...
    async _find_version_path(fs_context, { key, version_id }, return_md_path) {
        ...
        if (cur_ver_info && cur_ver_info.version_id_str === version_id) {
            return cur_ver_path;
        }
    ...
    _throw_if_delete_marker(stat) {
        if (
            this.versioning === versioning_status_enum.VER_ENABLED ||
            this.versioning === versioning_status_enum.VER_SUSPENDED
        ) {
    ...
    _throw_if_wrong_version_format(version_id) {
        ...
        if (
            !version_format.test(v_parts[1]) ||
            !version_format.test(v_parts[3])
        ) {
            throw new RpcError('BAD_REQUEST', 'Bad Request');
        }
    }
    ...
    async _delete_single_object_versioned(fs_context, key, version_id) {
        ...
        for (;;) {
            ...
            let gpfs_options;
            try {
                file_path = await this._find_version_path(fs_context, {
                    key,
                    version_id,
                });
                await this._check_path_in_bucket_boundaries(
                ...
                const version_info = await this._get_version_info(
                    fs_context,
                    file_path,
                );
                ...
                dbg.warn(
                    `NamespaceFS._delete_single_object_versioned: retrying retries=${retries} file_path=${file_path}`,
                    err,
                );
            } finally {
                if (gpfs_options) {
    ...
    async _delete_objects_versioned(fs_context, key, versions) {
        ...
        for (const version_id of versions) {
            try {
                if (version_id) {
                    const del_ver_info =
                        await this._delete_single_object_versioned(
                            fs_context,
                            key,
                            version_id,
                        );
                    ...
                    if (del_ver_info.latest) {
                        ...
                        deleted_delete_marker =
                            deleted_delete_marker || del_ver_info.delete_marker;
                    }
                    res.push({
                        deleted_delete_marker: del_ver_info.delete_marker,
                    });
                } else {
                    const version_res = await this._delete_latest_version(
                        fs_context,
                        latest_version_path,
                        { key, version_id },
                    );
        ...
        if (
            (latest_ver_info || deleted_delete_marker) &&
            !delete_marker_created
        ) {
            await this._promote_version_to_latest(
                fs_context,
                { key },
                latest_ver_info,
                latest_version_path,
    ...
    async _delete_version_id(fs_context, file_path, params) {
        ...
        const del_obj_version_info = await this._delete_single_object_versioned(
            fs_context,
            params.key,
            params.version_id,
        );
        ...
        if (del_obj_version_info.latest || del_obj_version_info.delete_marker) {
            const latest_version_path = this._get_file_path({
                key: params.key,
            });
            await this._promote_version_to_latest(
                fs_context,
                params,
                del_obj_version_info,
                latest_version_path,
            );
        }
        await this._delete_path_dirs(file_path, fs_context);
    ...
    async _promote_version_to_latest(
        fs_context,
        params,
        ...
        dbg.log1(
            'Namespace_fs._promote_version_to_latest',
            params,
            deleted_version_info,
            latest_ver_path,
        );
        const deleted_latest =
            deleted_version_info &&
            deleted_version_info.path === latest_ver_path;
        const prev_version_id =
            deleted_latest && deleted_version_info.prev_version_id;

        let retries = config.NSFS_RENAME_RETRIES;
        for (;;) {
            try {
                const latest_version_info = await this._get_version_info(
                    fs_context,
                    latest_ver_path,
                );
                if (latest_version_info) return;
                const max_past_ver_info =
                    (prev_version_id &&
                        (await this.get_prev_version_info(
                            fs_context,
                            params.key,
                            prev_version_id,
                        ))) ||
                ...
                if (!max_past_ver_info || max_past_ver_info.delete_marker) {
                    return;
                }
                ...
                if (
                    ...
                    deleted_version_info.mtimeNsBigint <
                        max_past_ver_info.mtimeNsBigint
                ) {
                    return;
                }
                dbg.log1(
                    'Namespace_fs._promote_version_to_latest ',
                    max_past_ver_info.path,
                    latest_ver_path,
                    max_past_ver_info,
                    latest_version_info,
                );
                ...
                if (retries <= 0) throw err;
                if (
                    !native_fs_utils._is_gpfs(fs_context) &&
                    err.code === 'EEXIST'
                ) {
                    dbg.warn(
                        'Namespace_fs._delete_version_id: latest version exist - skipping',
                    );
                    return;
                }
                if (err.code !== 'ENOENT') throw err;
                dbg.warn(
                    `NamespaceFS: _promote_version_to_latest failed retries=${retries}`,
                    err,
                );
            }
        }
    ...
    async _delete_latest_version(fs_context, latest_ver_path, params) {
        dbg.log0(
            'Namespace_fs._delete_latest_version:',
            latest_ver_path,
            params,
        );
        ...
        for (;;) {
            try {
                // get latest version_id if exists
                latest_ver_info = await this._get_version_info(
                    fs_context,
                    latest_ver_path,
                );
                const versioned_path =
                    latest_ver_info &&
                    this._get_version_path(
                        params.key,
                        latest_ver_info.version_id_str,
                    );
                const versioned_info =
                    latest_ver_info &&
                    (await this._get_version_info(fs_context, versioned_path));
                ...
                if (latest_ver_info) {
                    const suspended_and_latest_is_not_null =
                        this._is_versioning_suspended() &&
                        latest_ver_info.version_id_str !== NULL_VERSION_ID;
                    const bucket_tmp_dir_path =
                        this.get_bucket_tmpdir_full_path();
                    if (
                        this._is_versioning_enabled() ||
                        suspended_and_latest_is_not_null
                    ) {
                        await native_fs_utils._make_path_dirs(
                            versioned_path,
                            fs_context,
                        ...
                        dbg.log1(
                            'NamespaceFS._delete_latest_version: suspended mode version ID of the latest version is null - file will be unlinked',
                        );
                        await native_fs_utils.safe_unlink(
                            fs_context,
                            latest_ver_path,
                ...
                dbg.warn(
                    `NamespaceFS._delete_latest_version: Retrying retries=${retries} latest_ver_path=${latest_ver_path}`,
                    err,
                );
            } finally {
                if (gpfs_options) {
        ...
        const created_version_id = await this._create_delete_marker(
            fs_context,
            params,
            latest_ver_info,
        );
    ...
    async _delete_null_version_from_versions_directory(key, fs_context) {
        const is_gpfs = native_fs_utils._is_gpfs(fs_context);
        let retries = config.NSFS_RENAME_RETRIES;
        const null_versioned_path = this._get_version_path(
            key,
            NULL_VERSION_ID,
        );
        await this._check_path_in_bucket_boundaries(
        ...
        for (;;) {
            try {
                const null_versioned_path_info = await this._get_version_info(
                    fs_context,
                    null_versioned_path,
                );
                dbg.log1(
                    'Namespace_fs._delete_null_version_from_versions_directory:',
                    null_versioned_path,
                    null_versioned_path_info,
                );
                ...
                dbg.warn(
                    `NamespaceFS._delete_null_version_from_versions_directory Retrying retries=${retries} null_versioned_path=${null_versioned_path}`,
                    err,
                );
            }
        }
    ...
    async _create_delete_marker(fs_context, params, deleted_version_info) {
        ...
        for (;;) {
            try {
                ...
                const file_path = this._get_version_path(
                    params.key,
                ...
                if (fs_xattr) {
                    await upload_params.target_file.replacexattr(
                        fs_context,
                        fs_xattr,
                    );
                }
                ...
                if (err.code === 'EEXIST') {
                    dbg.warn(
                        `NamespaceFS: _create_delete_marker already exists, success`,
                        err,
                    );
                    return delete_marker_version_id;
                }
                dbg.warn(
                    `NamespaceFS: _create_delete_marker failed retries=${retries}`,
                    err,
                );
            } finally {
                if (upload_params) {
    ...
    async get_prev_version_info(fs_context, key, prev_version_id) {
        ...
        const prev_path_info = await this._get_version_info(
            fs_context,
            prev_path,
        );
    ...
    async find_max_version_past(fs_context, key) {
        const versions_dir = path.normalize(
            path.join(
                this.bucket_path,
                path.dirname(key),
                HIDDEN_VERSIONS_PATH,
        ...
        try {
            ...
            const arr = await P.map_with_concurrency(
                10,
                versions,
                async entry => {
                    const index =
                    ...
                    if (index < 0 || entry.name.slice(0, index) !== key) {
                        return undefined;
                    }
                    const { mtimeNsBigint } =
                        this._extract_version_info_from_xattr(
                            entry.name.slice(key.length + 1),
                        ) ||
                        (await this._get_version_info(
                            fs_context,
            ...
            const max_entry_info = arr.reduce(
                (acc, cur) =>
                    cur && cur.mtimeNsBigint > acc.mtimeNsBigint ? cur : acc,
                { mtimeNsBigint: BigInt(0), name: undefined },
            );
            return (
                max_entry_info.mtimeNsBigint > BigInt(0) &&
                this._get_version_info(
                    fs_context,
                    path.join(versions_dir, max_entry_info.name),
                )
            );
        } catch (err) {
            dbg.warn(
                'namespace_fs.find_max_version_past: .versions/ folder could not be found',
                err,
            );
        }
    }
    ...
    async _open_files_gpfs(
        ...
        try {
            ...
            versioned_file =
                versioned_info &&
                (await native_fs_utils.open_file(
                    fs_context,
                    this.bucket_path,
                    versioned_info.path,
                    'r',
            ...
            if (delete_version) {
                src_file = await native_fs_utils.open_file(
                    fs_context,
                    this.bucket_path,
                    src_path,
                    'r',
                );
                dir_file = await native_fs_utils.open_file(
                    fs_context,
                    this.bucket_path,
            ...
            if (dst_ver_info) {
                dbg.log1(
                    'NamespaceFS._open_files_gpfs dst version exist - opening dst version file...',
                );
                dst_file = await native_fs_utils.open_file(
                    fs_context,
                    this.bucket_path,
                    dst_path,
                    'r',
            ...
            await this._close_files_gpfs(
                fs_context,
                { src_file, dst_file, dir_file, versioned_file },
                open_mode,
                delete_version,
            );
            throw err;
    ...
    async _close_files_gpfs(
        ...
        const {
            ...
            dst_file = undefined,
            dir_file,
            versioned_file = undefined,
        } = files_to_close;
        try {
            if (src_file && (delete_version || open_mode === 'wt')) {
                await src_file.close(fs_context);
            }
        } catch (err) {
        ...
        try {
            if (dir_file && (delete_version || open_mode !== 'wt')) {
        ...
        try {
            ...
            dbg.warn(
                'NamespaceFS: _close_files_gpfs versioned_file error',
                err,
            );
        }
    }
    ...
    async _throw_if_low_space(fs_context, size_hint = 0) {
        ...
        const MB = 1024 ** 2;
        const { statfs } = await nsfs_bucket_statfs_cache.get_with_cache({
            bucket_path: this.bucket_path,
            fs_context,
        });
        const block_size_mb = statfs.bsize / MB;
        const free_space_mb =
            Math.floor(statfs.bfree * block_size_mb) -
            Math.floor(size_hint / MB);
