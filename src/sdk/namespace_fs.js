...
const dir_cache = new LRUCache({
  ...
  validate: async ({ stat }, { dir_path, fs_context }) => {
    ...
    return (
      new_stat.ino === stat.ino && new_stat.mtimeNsBigint === stat.mtimeNsBigint
    );
  },
...
const versions_dir_cache = new LRUCache({
  ...
  validate: async ({ stat, ver_dir_stat }, { dir_path, fs_context }) => {
    ...
    if (ver_dir_stat) {
      ...
      const new_versions_stat = await nb_native().fs.stat(
        fs_context,
        versions_dir_path,
      );
      return (
...
class NamespaceFS {
    ...
    try {
      ...
      const process_dir = async dir_key => {
        ...
        const process_entry = async ent => {
          ...
          if (
            list_versions &&
            _is_version_object_including_null_version(ent.name)
          ) {
            r = {
        ...
        const is_dir_content =
          cached_dir.stat.xattr && cached_dir.stat.xattr[XATTR_DIR_CONTENT];
        if (
          is_dir_content &&
          dir_key > key_marker &&
        ...
        if (cached_dir.sorted_entries) {
          ...
          if (list_versions && marker_curr && !marker_curr.includes('/')) {
            ...
            marker_index =
              _.findIndex(sorted_entries, { name: start_marker }) + 1;
          } else {
        ...
        if (r.common_prefix) {
          ...
          if (this._is_hidden_version_path(obj_info.key)) {
            obj_info.key = path.normalize(
              obj_info.key.replace(HIDDEN_VERSIONS_PATH + '/', ''),
            );
  ...
  async upload_object(params, object_sdk) {
    ...
    try {
      ...
      if (
        ...
        const upload_res = await bp.sem.surround_count(bp.buf_size, async () =>
          this._upload_stream(upload_params),
        );
  ...
  async _finish_upload({
    ...
    let stat = await target_file.stat({
      ...fs_context,
      disable_ctime_check: part_upload,
    });
        ...
        if (md5_hex !== digest) {
          throw new Error(
            '_upload_stream mismatch etag: ' +
              util.inspect({ key, bucket, upload_id, md5_hex, digest }),
          );
        }
      }
    ...
    stat = await nb_native().fs.stat(
      { ...fs_context, disable_ctime_check: part_upload },
      file_path,
    );
  ...
  async _move_to_dest_version(
    ...
    for (;;) {
      try {
        const new_ver_info =
          !is_gpfs &&
          (await this._get_version_info(fs_context, new_ver_tmp_path));
        ...
        gpfs_options =
          ...
          : undefined;
        const bucket_tmp_dir_path = path.join(
          this.bucket_path,
          this.get_bucket_tmpdir(),
        );
        dbg.log1(
        ...
        try {
          ...
          if (
            err.message !== native_fs_utils.posix_unlink_retry_err &&
      ...
      if (params.delimiter) {
        ...
        if (delimiter_idx > 0) {
          common_prefixes_set.add(
            create_params_parsed.key.substring(0, delimiter_idx + 1),
          );
  ...
  async upload_multipart(params, object_sdk) {
    ...
    try {
      ...
      if (!pre_known_size) {
        ...
        upload_res = await bp.sem.surround_count(bp.buf_size, async () =>
          this._upload_stream(md_upload_params),
        );
      ...
      if (pre_known_size) {
        const bp = multi_buffer_pool.get_buffers_pool(params.size);
        upload_res = await bp.sem.surround_count(bp.buf_size, async () =>
          this._upload_stream(data_upload_params),
        );
      } else {
    ...
    try {
      ...
      for (const { num, etag } of multiparts) {
        ...
        if (etag !== this._get_etag(md_part_stat)) {
          throw new Error(
            'mismatch part etag: ' +
              util.inspect({ num, etag, md_part_path, md_part_stat, params }),
          );
        }
        ...
        } else if (part_size_to_fd_map.size === 2 && should_copy_file_prefix) {
          // 2
          if (num === multiparts.length) {
  ...
  _translate_object_error_codes(err) {
    ...
    if (err.code === 'IO_STREAM_ITEM_TIMEOUT') {
      err.rpc_code = 'IO_STREAM_ITEM_TIMEOUT';
    }
  ...
  _extract_version_info_from_xattr(version_id_str) {
    ...
    if (arr.length < 2) {
      throw new Error('Invalid version_id_string, cannot extract version info');
    }
    return {
  ...
  async _promote_version_to_latest(
    ...
    const deleted_latest =
      deleted_version_info && deleted_version_info.path === latest_ver_path;
    ...
    for (;;) {
      try {
        ...
        if (
          ...
          deleted_version_info.mtimeNsBigint < max_past_ver_info.mtimeNsBigint
        ) {
          return;
        }
        dbg.log1(
  ...
  async _delete_latest_version(fs_context, latest_ver_path, params) {
    ...
    for (;;) {
      try {
        ...
        const versioned_path =
          latest_ver_info &&
          this._get_version_path(params.key, latest_ver_info.version_id_str);
        const versioned_info =
          latest_ver_info &&
          (await this._get_version_info(fs_context, versioned_path));
  ...
  async _create_delete_marker(fs_context, params, deleted_version_info) {
    ...
    for (;;) {
      try {
        ...
        if (fs_xattr) {
          await upload_params.target_file.replacexattr(fs_context, fs_xattr);
        }
  ...
  async find_max_version_past(fs_context, key) {
    const versions_dir = path.normalize(
      path.join(this.bucket_path, path.dirname(key), HIDDEN_VERSIONS_PATH),
    );
    try {
  ...
  async _throw_if_low_space(fs_context, size_hint = 0) {
    ...
    const free_space_mb =
      Math.floor(statfs.bfree * block_size_mb) - Math.floor(size_hint / MB);
