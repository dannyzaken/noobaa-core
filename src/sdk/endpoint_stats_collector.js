...
class EndpointStatsCollector {
    constructor() {
        ...
        this.update_fs_stats = fs_worker_stats =>
            this._update_fs_stats(fs_worker_stats);
    ...
    static instance() {
        if (!EndpointStatsCollector._instance) {
            EndpointStatsCollector._instance = new EndpointStatsCollector();
        }
    ...
    async _process_endpoint_stats(data) {
        const bucket_counters = Object.entries(
            data.bucket_counters ?? {},
        ).flatMap(([bucket_name, bkt]) =>
            Object.entries(bkt).map(([content_type, io_stats]) => {
                dbg.log0(
                    `bucket stats - ${bucket_name} ${content_type} :`,
                    io_stats,
                );
                return {
                    bucket_name,
        ...
        const namespace_stats = Object.entries(data.namespace_stats ?? {}).map(
            ([namespace_resource_id, io_stats]) => {
                dbg.log0(
                    `namespace stats - ${namespace_resource_id} :`,
                    io_stats,
                );
                return {
    ...
    update_bucket_read_counters({ bucket_name, key, content_type }) {
        ...
        this.endpoint_stats_collector.update({
            bucket_counters: {
                [bucket_name]: { [content_type]: { read_count: 1 } },
            },
        });
    }
    ...
    update_bucket_write_counters({ bucket_name, key, content_type }) {
        ...
        this.endpoint_stats_collector.update({
            bucket_counters: {
                [bucket_name]: { [content_type]: { write_count: 1 } },
            },
        });
    }
    ...
    update_semaphore_state(report, type, report_sample_sizes) {
        ...
        if (
            this.semaphore_reports[type].length >
            report_sample_sizes[report_sample_sizes.length - 1]
        ) {
        ...
        for (const sample_size of report_sample_sizes) {
            ...
            const average_interval =
                (sample_size * config.SEMAPHORE_MONITOR_DELAY) / (60 * 1000);
            average_intervals.push(average_interval);
            const average_values = this.semaphore_reports[type].slice(
                this.semaphore_reports[type].length - sample_size,
            );
            this.update_semaphore_reports(
                average_values,
    ...
    update_nsfs_read_stats({
        ...
        this.update_namespace_read_stats({
            namespace_resource_id,
            bucket_name,
            size,
            count,
    ...
    update_nsfs_write_stats({
        ...
        this.update_namespace_write_stats({
            namespace_resource_id,
            bucket_name,
            size,
            count,
    ...
    update_cache_stats({
        ...
        if (read_count) {
            this.prom_metrics_report.inc(
                range_op ? 'cache_range_read_count' : 'cache_object_read_count',
                { bucket_name },
                read_count,
            );
        }
        ...
        if (hit_count) {
            this.prom_metrics_report.inc(
                range_op ?
                    'cache_range_read_hit_count'
                :   'cache_object_read_hit_count',
                { bucket_name },
                hit_count,
            );
        }
    ...
    update_cache_latency_stats({
        bucket_name,
        cache_read_latency = 0,
        cache_write_latency = 0,
    }) {
        if (cache_read_latency) {
            this.prom_metrics_report.observe(
                'cache_read_latency',
                { bucket_name },
                cache_read_latency,
            );
