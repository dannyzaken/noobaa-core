...
class BucketSpaceNB {
    ...
    async read_account_by_access_key({ access_key }) {
        return this.internal_rpc_client.account.read_account_by_access_key({
            access_key,
        });
    }
    ...
    async list_buckets(object_sdk) {
        ...
        const has_access_buckets = (
            await P.all(
                _.map(buckets, async bucket => {
                    const ns = await object_sdk.read_bucket_sdk_namespace_info(
                        bucket.name.unwrap(),
                    );
                    const has_access_to_bucket =
                        object_sdk.is_nsfs_bucket(ns) ?
                            await this._has_access_to_nsfs_dir(ns, object_sdk)
                        :   object_sdk.has_non_nsfs_bucket_access(
    ...
    async create_bucket(params, object_sdk) {
        ...
        try {
            if (
                resp.namespace &&
                resp.namespace.should_create_underlying_storage
            ) {
                const ns = await object_sdk._get_bucket_namespace(
                    params.name.unwrap(),
                );
                const namespace_bucket_config =
                    await object_sdk.read_bucket_sdk_namespace_info(
                        params.name.unwrap(),
                    );
                await ns.create_uls(
                    {
                        name: params.name,
                        full_path: path.join(
                            namespace_bucket_config.write_resource.resource
                                .fs_root_path,
            ...
            dbg.log0(
                'could not create underlying directory - nsfs, deleting bucket',
                err,
            );
            await this.rpc_client.bucket.delete_bucket({
    ...
    async get_bucket_lifecycle_configuration_rules(params) {
        return this.rpc_client.bucket.get_bucket_lifecycle_configuration_rules(
            params,
        );
    }

    async set_bucket_lifecycle_configuration_rules(params) {
        return this.rpc_client.bucket.set_bucket_lifecycle_configuration_rules(
            params,
        );
    }
