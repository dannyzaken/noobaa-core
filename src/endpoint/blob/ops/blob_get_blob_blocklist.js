...
async function get_blob_blocklist(req, res) {
    ...
    if (block_lists.committed) {
        response.BlockList.CommittedBlocks = format_block_list(
            block_lists.committed,
        );
    }
    if (block_lists.uncommitted) {
        response.BlockList.UncommittedBlocks = format_block_list(
            block_lists.uncommitted,
        );
    }
