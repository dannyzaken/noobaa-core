...
function parse_filter(filter) {
    ...
    if (
        ...
        current_rule_filter.object_size_greater_than = parseInt(
            filter.ObjectSizeGreaterThan[0],
            10,
        );
    }
    if (filter.ObjectSizeLessThan && filter.ObjectSizeLessThan.length === 1) {
        current_rule_filter.object_size_less_than = parseInt(
            filter.ObjectSizeLessThan[0],
            10,
        );
    }
    if (
        current_rule_filter.object_size_greater_than !== undefined &&
        current_rule_filter.object_size_less_than !== undefined &&
        current_rule_filter.object_size_greater_than >=
            current_rule_filter.object_size_less_than
    ) {
        dbg.error(
            'Invalid size range: filter',
            filter,
            'size range: object_size_greater_than',
            current_rule_filter.object_size_greater_than,
    ...
    if (filter.And && filter.And.length === 1) {
        ...
        if (filter.And[0].Prefix && filter.And[0].Prefix.length === 1) {
            current_rule_filter.prefix = filter.And[0].Prefix[0];
        }
        current_rule_filter.tags = _.map(filter.And[0].Tag, tag => ({
            key: tag.Key[0],
            value: tag.Value[0],
        }));
        if (
            filter.And[0].ObjectSizeGreaterThan &&
            filter.And[0].ObjectSizeGreaterThan.length === 1
        ) {
            current_rule_filter.object_size_greater_than = parseInt(
                filter.And[0].ObjectSizeGreaterThan[0],
                10,
            );
        }
        if (
            filter.And[0].ObjectSizeLessThan &&
            filter.And[0].ObjectSizeLessThan.length === 1
        ) {
            current_rule_filter.object_size_less_than = parseInt(
                filter.And[0].ObjectSizeLessThan[0],
                10,
            );
        }
    }
...
function parse_expiration(expiration) {
    ...
    if (expiration.Days && expiration.Days.length === 1) {
        ...
        if (output_expiration.days < 1) {
            dbg.error(
                'Minimum value for expiration days is 1, actual',
                expiration.Days,
                'converted',
                output_expiration.days,
            );
            throw new S3Error(S3Error.InvalidArgument);
        }
        ...
        dbg.error(
            'ExpiredObjectDeleteMarker is not implemented, expiration:',
            expiration,
        );
...
async function put_bucket_lifecycle(req) {
    const lifecycle_rules = _.map(
        req.body.LifecycleConfiguration.Rule,
        rule => {
            const current_rule = {
                filter: {},
            };
            ...
            if (rule.Prefix) {
                dbg.error(
                    'Rule should not have prefix, it should be filter.prefix',
                    rule,
                );
                throw new S3Error(S3Error.InvalidArgument);
            }
            ...
            if (rule.AbortIncompleteMultipartUpload) {
                dbg.error(
                    'AbortIncompleteMultipartUpload is not implemented, rule:',
                    rule,
                );
                throw new S3Error(S3Error.NotImplemented);
            }
            ...
            if (rule.NoncurrentVersionExpiration) {
                dbg.error(
                    'NoncurrentVersionExpiration is not implemented, rule:',
                    rule,
                );
                throw new S3Error(S3Error.NotImplemented);
            }
            if (rule.NoncurrentVersionTransition) {
                dbg.error(
                    'NoncurrentVersionTransition is not implemented, rule:',
                    rule,
                );
                throw new S3Error(S3Error.NotImplemented);
            }
