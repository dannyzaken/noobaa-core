...
async function put_object(req, res) {
  ...
  const lock_settings =
    config.WORM_ENABLED ? s3_utils.parse_lock_header(req) : undefined;
  // Copy request sends empty content and not relevant to the object data
  const { size, md5_b64, sha256_b64 } =
    copy_source ?
      {}
    : {
        ...
        sha256_b64:
          req.content_sha256_buf && req.content_sha256_buf.toString('base64'),
      };
  ...
  const source_stream =
    req.chunked_content ? s3_utils.decode_chunked_upload(req) : req;
  const reply = await req.object_sdk.upload_object({
    bucket: req.params.bucket,
    key: req.params.key,
    content_type:
      req.headers['content-type'] ||
      (copy_source ? undefined : (
        mime.getType(req.params.key) || 'application/octet-stream'
      )),
    ...
    source_md_conditions: http_utils.get_md_conditions(
      req,
      'x-amz-copy-source-',
    ),
    ...
    lock_settings,
    storage_class,
    azure_invalid_md_header:
      req.headers['azure-metadata-handling'] || undefined,
  });
