...
async function get_bucket(req) {
    ...
    const optional_object_attributes =
        req.headers['x-amz-optional-object-attributes'];
    const restore_status_requested =
        optional_object_attributes === 'RestoreStatus';
    ...
    const params = {
        ...
        key_marker:
            list_type === '2' ?
                cont_tok_to_key_marker(cont_tok) || start_after
            :   req.query.marker,
    };
    ...
    const default_object_owner = await s3_utils.get_default_object_owner(
        req.params.bucket,
        req.object_sdk,
    );
        ...
        ListBucketResult: [
            {
                ...
                ...(list_type === '2' ?
                    {
                        ...
                        KeyCount:
                            reply.objects.length + reply.common_prefixes.length,
                ...
                :   {
                        ...
                        NextMarker:
                            req.query.delimiter ? reply.next_marker : undefined,
                    }),
            },
            _.map(reply.objects, obj => ({
                Contents: {
                    ...
                    LastModified: s3_utils.format_s3_xml_date(
                        obj.last_modified_time || obj.create_time,
                    ),
                    ETag: `"${obj.etag}"`,
                    Size: obj.size,
                    Owner:
                        (!list_type || req.query['fetch-owner']) &&
                        (s3_utils.get_object_owner(obj) ||
                            default_object_owner),
                    StorageClass: s3_utils.parse_storage_class(
                        obj.storage_class,
                    ),
                    RestoreStatus: get_object_restore_status(
                        obj,
                        restore_status_requested,
                    ),
                },
            })),
...
function get_object_restore_status(obj, restore_status_requested) {
    ...
    if (!obj.restore_status.ongoing && obj.restore_status.expiry_time) {
        restore_status.RestoreExpiryDate = new Date(
            obj.restore_status.expiry_time,
        ).toUTCString();
    }
