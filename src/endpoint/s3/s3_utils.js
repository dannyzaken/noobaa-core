...
function decode_chunked_upload(source_stream) {
  ...
  stream_utils
    ...
    .catch(err =>
      console.warn('decode_chunked_upload: pipeline error', err.stack || err),
    );
...
function set_encryption_response_headers(req, res, encryption = {}) {
  ...
  encryption_headers.forEach(
    header => req.headers[header] && res.setHeader(header, req.headers[header]),
  );
  ...
  if (kms_key_id) {
    res.setHeader('x-amz-server-side-encryption-aws-kms-key-id', kms_key_id);
  }
}
...
function parse_body_object_lock_conf_xml(req) {
  ...
  if (
    ...
    (!retention.Days && !retention.Years) ||
    (retention.Mode[0] !== 'GOVERNANCE' && retention.Mode[0] !== 'COMPLIANCE')
  ) {
    throw new S3Error(S3Error.MalformedXML);
  }
          ...
          return _.omitBy(
            {
              condition: rule.Condition && {
                ...
                http_error_code_returned_equals:
                  rule.Condition[0].HttpErrorCodeReturnedEquals[0],
              },
              redirect: {
                protocol:
