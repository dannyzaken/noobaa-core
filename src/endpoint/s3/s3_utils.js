...
function decode_chunked_upload(source_stream) {
    ...
    stream_utils
        .pipeline([source_stream, decoder], true)
        .catch(err =>
            console.warn(
                'decode_chunked_upload: pipeline error',
                err.stack || err,
            ),
        );

    decoder.on('error', err1 =>
        dbg.error(
            's3_utils: error occured on stream ChunkedContentDecoder: ',
            err1,
        ),
    );
    return decoder;
...
function set_response_xattr(res, xattr) {
    ...
    for (const key of keys) {
        ...
        try {
            ...
            dbg.warn(
                `s3_utils.set_response_xattr set_header failed, skipping... res.req.url=${res.req?.url} xattr key=${key} xattr value=${val}`,
            );
        }
    }
}
...
function parse_sse_c(req, copy_source) {
    const algorithm =
        req.headers[
            `x-amz-${copy_source ? 'copy-source-' : ''}server-side-encryption-customer-algorithm`
        ];
    const key_b64 =
        req.headers[
            `x-amz-${copy_source ? 'copy-source-' : ''}server-side-encryption-customer-key`
        ];
    const key_md5_b64 =
        req.headers[
            `x-amz-${copy_source ? 'copy-source-' : ''}server-side-encryption-customer-key-md5`
        ];
    ...
    if (algorithm !== 'AES256') {
        throw new S3Error(S3Error.InvalidEncryptionAlgorithmError);
    }

    if (!key_b64 || !base64_regex.test(key_b64)) {
        throw new S3Error(S3Error.InvalidDigest);
    }

    if (!key_md5_b64 || !base64_regex.test(key_md5_b64)) {
        throw new S3Error(S3Error.InvalidDigest);
    }

    const key = Buffer.from(key_b64, 'base64');
    const calculated_key = crypto
        .createHash('md5')
        .update(key)
        .digest('base64');
    if (key_md5_b64 !== calculated_key) {
        throw new S3Error(S3Error.InvalidDigest);
    }
...
function parse_sse(req) {
    ...
    const kms_key_id =
        req.headers['x-amz-server-side-encryption-aws-kms-key-id'];
    const context_b64 =
        algorithm === 'aws:kms' ?
            req.headers['x-amz-server-side-encryption-context']
        :   undefined;
    ...
    if (algorithm === 'aws:kms' && !kms_key_id) {
        throw new S3Error(S3Error.InvalidArgument);
    }
...
function set_encryption_response_headers(req, res, encryption = {}) {
    ...
    encryption_headers.forEach(
        header =>
            req.headers[header] && res.setHeader(header, req.headers[header]),
    );
...
function parse_copy_source(req) {
    ...
    const ranges = http_utils.parse_http_ranges(
...
function set_response_object_md(res, object_md) {
    ...
    if (object_md.last_modified_time) {
        res.setHeader(
            ...
            time_utils.format_http_header_date(
                new Date(object_md.last_modified_time),
            ),
        );
    } else {
        res.setHeader(
    ...
    if (object_md.content_encoding) {
        res.setHeader('Content-Encoding', object_md.content_encoding);
    }
    res.setHeader(
        'Content-Length',
        object_md.content_length === undefined ?
            object_md.size
        :   object_md.content_length,
    );
    ...
    if (config.WORM_ENABLED && object_md.lock_settings) {
        if (object_md.lock_settings.legal_hold) {
            res.setHeader(
                ...
                object_md.lock_settings.legal_hold.status,
            );
        }
        if (object_md.lock_settings.retention) {
            res.setHeader(
                'x-amz-object-lock-mode',
                object_md.lock_settings.retention.mode,
            );
            res.setHeader(
                'x-amz-object-lock-retain-until-date',
                object_md.lock_settings.retention.retain_until_date,
            );
        }
    }
    if (object_md.version_id) {
        res.setHeader('x-amz-version-id', object_md.version_id);
    }
    set_response_xattr(res, object_md.xattr);
    if (object_md.tag_count) {
        res.setHeader('x-amz-tagging-count', object_md.tag_count);
    }
    if (object_md.num_multiparts) {
        res.setHeader('x-amz-mp-parts-count', object_md.num_multiparts);
    }
    if (object_md.content_range) {
        res.setHeader('Content-Range', object_md.content_range);
    }
    ...
    if (object_md.restore_status) {
        const restore = [
            `ongoing-request="${object_md.restore_status.ongoing}"`,
        ];
        if (
 ...
 */
function set_response_supported_storage_classes(
    res,
    supported_storage_classes = [],
) {
    res.setHeader(
        X_NOOBAA_AVAILABLE_STORAGE_CLASSES,
        supported_storage_classes,
    );
}
...
function parse_storage_class(storage_class) {
    ...
    if (storage_class === STORAGE_CLASS_GLACIER_IR) {
        return STORAGE_CLASS_GLACIER_IR;
    }
...
function parse_body_tagging_xml(req) {
    ...
    const tag_set_map = _.map(tag_obj.Tag, tag => {
        ...
        if (
            !_.isEmpty(_.difference(tag_keys, tag_key_names)) ||
            tag_keys.length !== tag_key_names.length
        ) {
            throw new S3Error(S3Error.InvalidTag);
        }
        return {
    ...
    if (tag_set_map.length === 0 || tag_set_map.length > 10) {
        throw new S3Error(S3Error.InvalidTag);
    }
...
function parse_lock_header(req) {
    ...
    const retain_until_date =
        ...
        new Date(req.headers['x-amz-object-lock-retain-until-date']);
    ...
    if (status) {
        if (!_is_valid_legal_hold(status)) {
            throw new S3Error(S3Error.InvalidArgument);
        }
...
function is_copy_tagging_directive(req) {
    ...
    if (!allowed_values.includes(tagging_directive)) {
        throw new S3Error(S3Error.InvalidArgument);
    }
...
function parse_body_encryption_xml(req) {
    ...
    const kms_key_id =
        sse_default.KMSMasterKeyID && sse_default.KMSMasterKeyID[0];
    if (
...
function parse_body_object_lock_conf_xml(req) {
    ...
    if (
        (retention.Days && retention.Years) ||
        (!retention.Days && !retention.Years) ||
        (retention.Mode[0] !== 'GOVERNANCE' &&
            retention.Mode[0] !== 'COMPLIANCE')
    ) {
        throw new S3Error(S3Error.MalformedXML);
    }
...
function parse_body_website_xml(req) {
    ...
    if (website_configuration.RedirectAllRequestsTo) {
        ...
        if (website_configuration.RoutingRules) {
            if (!website_configuration.RoutingRules[0].RoutingRule) {
                throw new S3Error(S3Error.InvalidArgument);
            }
            const routing_rules =
                website_configuration.RoutingRules[0].RoutingRule.map(rule => {
                    if (!rule.Redirect) {
                        throw new S3Error(S3Error.InvalidArgument);
                    }
                    return _.omitBy(
                        {
                            condition: rule.Condition && {
                                ...
                                key_prefix_equals:
                                    rule.Condition[0].KeyPrefixEquals[0],
                            ...
                            redirect: {
                                protocol:
                                    rule.Redirect[0].Protocol[0] &&
                                    rule.Redirect[0].Protocol[0].toUpperCase(),
...
function parse_to_camel_case(obj_lock, root_key) {
    ...
    const reply =
        root_key ? { root_key: rename_keys(obj_lock) } : rename_keys(obj_lock);
    return reply;
}
...
function parse_website_to_body(website) {
    ...
    if (reply.WebsiteConfiguration.RoutingRules) {
        reply.WebsiteConfiguration.RoutingRules = _.map(
...
function get_http_response_date(res) {
    ...
    if (!r.httpResponse.headers.date) {
        throw new Error('date not found in response header');
    }
...
function get_response_field_encoder(req) {
    ...
    if (typeof encoding_type === 'undefined' || encoding_type === null) {
        return response_field_encoder_none;
    }
    if (encoding_type.toLowerCase() === 'url') {
        return response_field_encoder_url;
    }
 ...
 */
function parse_version_id(
    version_id,
    empty_err = S3Error.InvalidArgumentEmptyVersionId,
) {
    if (version_id === '') {
...
function parse_restore_request_days(req) {
    ...
    if (days > config.S3_RESTORE_REQUEST_MAX_DAYS) {
        ...
        dbg.log0(
            `Restore request days ${days} is above max ${config.S3_RESTORE_REQUEST_MAX_DAYS} - truncating`,
        );
...
exports.set_response_supported_storage_classes =
    set_response_supported_storage_classes;
