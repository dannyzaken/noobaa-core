...
async function has_bucket_policy_permission(
  ...
  if (
    await _is_statements_fit(deny_statements, account, method, arn_path, req)
  ) {
    return 'DENY';
  }
  ...
  if (
    await _is_statements_fit(allow_statements, account, method, arn_path, req)
  ) {
    return 'ALLOW';
  }
...
async function validate_s3_policy(policy, bucket_name, get_account_handler) {
  ...
  for (const statement of policy.Statement) {
    ...
    if (statement_principal.AWS) {
      ...
      throw new RpcError('MALFORMED_POLICY', 'Invalid principal in policy', {
