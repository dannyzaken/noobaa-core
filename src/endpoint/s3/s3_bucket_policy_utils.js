...
const OP_NAME_TO_ACTION = Object.freeze({
    ...
    delete_object_tagging: {
        regular: 's3:DeleteObjectTagging',
        versioned: 's3:DeleteObjectVersionTagging',
    },
    delete_object_uploadId: { regular: 's3:AbortMultipartUpload' },
    delete_object: {
        regular: 's3:DeleteObject',
        versioned: 's3:DeleteObjectVersion',
    },
    ...
    get_object_tagging: {
        regular: 's3:GetObjectTagging',
        versioned: 's3:GetObjectVersionTagging',
    },
    ...
    put_object_tagging: {
        regular: 's3:PutObjectTagging',
        versioned: 's3:PutObjectVersionTagging',
    },
...
const predicate_map = {
    StringEquals: (request_value, policy_value) =>
        request_value === policy_value,
    StringNotEquals: (request_value, policy_value) =>
        request_value !== policy_value,
    StringEqualsIgnoreCase: (request_value, policy_value) =>
        request_value.toLowerCase() === policy_value.toLowerCase(),
    StringNotEqualsIgnoreCase: (request_value, policy_value) =>
        request_value.toLowerCase() !== policy_value.toLowerCase(),
    StringLike: function (request_value, policy_value) {
        const value_regex = RegExp(
            `^${policy_value.replace(qm_regex, '.?').replace(ar_regex, '.*')}$`,
        );
    ...
    Null: function (request_value, policy_value) {
        return policy_value === 'true' ?
                request_value === null
            :   request_value !== null;
    },
};
...
async function has_bucket_policy_permission(
    policy,
    account,
    method,
    arn_path,
...
function _is_action_fit(method, statement) {
    ...
    for (const action of _.flatten([statement_action])) {
        dbg.log1(
            'bucket_policy: ',
            statement.Action ? 'Action' : 'NotAction',
            ' fit?',
            action,
            method,
        );
...
function _is_principal_fit(account, statement) {
    ...
    statement_principal =
        statement_principal.AWS ? statement_principal.AWS : statement_principal;
    for (const principal of _.flatten([statement_principal])) {
        dbg.log1(
            'bucket_policy: ',
            statement.Principal ? 'Principal' : 'NotPrincipal',
            ' fit?',
            principal,
            account,
        );
...
function _is_resource_fit(arn_path, statement) {
    ...
    for (const resource of _.flatten([statement_resource])) {
        ...
        const resource_regex = RegExp(
            `^${resource.replace(qm_regex, '.?').replace(ar_regex, '.*')}$`,
        );
        dbg.log1(
            'bucket_policy: ',
            statement.Resource ? 'Resource' : 'NotResource',
            ' fit?',
            resource_regex,
            arn_path,
        );
...
async function _is_statements_fit(statements, account, method, arn_path, req) {
    for (const statement of statements) {
        ...
        if (action_fit && principal_fit && resource_fit && condition_fit) {
            return true;
        }
    }
    return false;
...
async function _is_condition_fit(policy_statement, req, method) {
    ...
    for (const [condition, condition_statements] of Object.entries(
        policy_statement.Condition,
    )) {
        const predicate = predicate_map[condition];
        for (const [condition_key, value] of Object.entries(
            ...
            if (
                (await condition_fit_functions[condition_key](
                    req,
                    predicate,
                    value,
                )) === false
...
async function validate_s3_policy(policy, bucket_name, get_account_handler) {
    const all_op_names = _.compact(
        _.flatMap(OP_NAME_TO_ACTION, action => [
            action.regular,
            action.versioned,
        ]),
    );
    for (const statement of policy.Statement) {
        ...
        if (statement_principal.AWS) {
            ...
            typeof statement_principal === 'string' ?
                statement_principal !== '*'
            :   statement_principal.unwrap() !== '*'
        ) {
            throw new RpcError(
                'MALFORMED_POLICY',
                'Invalid principal in policy',
                { detail: statement.Principal },
            );
        }
        for (const resource of _.flatten([
        ...
        if (statement.Condition) {
            for (const condition of Object.values(statement.Condition)) {
                for (const condition_key of Object.keys(condition)) {
                    ...
                    const key_parts = condition_key.split('/');
                    if (
                        !SUPPORTED_BUCKET_POLICY_CONDITIONS.includes(
                            key_parts[0],
                        )
                    ) {
                        throw new RpcError(
                            'MALFORMED_POLICY',
                            'Policy has invalid condition key or unsupported condition key',
