...
async function main(options = {}) {
  try {
    ...
    const http_port =
      options.http_port || Number(process.env.ENDPOINT_PORT) || 6001;
    const https_port =
      options.https_port || Number(process.env.ENDPOINT_SSL_PORT) || 6443;
...
function get_rpc_router(env) {
  ...
  return {
    default:
      env.MGMT_ADDR || addr_utils.format_base_address(hostname, ports.mgmt),
