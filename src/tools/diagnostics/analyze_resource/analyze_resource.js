...
const get_cloud_vendor =
    require('./analyze_resource_cloud_vendor').get_cloud_vendor;
...
const connection_basic_details = {
    ...
    signature_version:
        process.env.RESOURCE_TYPE === 'aws-s3' ?
            'v4'
        :   process.env.S3_SIGNATURE_VERSION,
};
...
async function main() {
    try {
        ...
        const cloud_vendor = get_cloud_vendor(
            resource_type,
            connection_basic_details,
        );
...
function check_supported_resource_type(resource_type) {
    const SUPPORTED_TYPES_OF_RESOURCES = [
        'aws-s3',
        's3-compatible',
        'ibm-cos',
        'google-cloud-storage',
        'azure-blob',
    ]; //'pv-pool' and 'nsfs' are not cloud vendor and we will not check them
...
async function test_resource(cloud_vendor, bucket) {
    ...
    console.info(
        `We will have several tests to check the status of the resource and permissions of the bucket ${bucket}`,
    );
    let head_object_executed;
    let head_skip_reason;
    const key_to_head = await list_objects_and_get_one_key(
        cloud_vendor,
        bucket,
    );
    if (key_to_head) {
        ...
        head_skip_reason =
            'head object after list objects was not executed since there was no returned key';
    }
    const key_to_write = `test_resource_${new Date().getTime()}_(can_be_deleted).txt`;
    const is_write_object = await write_object(
        cloud_vendor,
        bucket,
        key_to_write,
    );
    ...
    if (!head_object_executed) {
        ...
        console.warn(
            `Test head object in bucket ${bucket} skipped, since ${head_skip_reason}`,
        );
    }

    // in case we wrote an object we want to delete it
    await delete_object_after_write(
        cloud_vendor,
        bucket,
        key_to_write,
        is_write_object,
...
function check_arguments(cloud_vendor, bucket) {
    ...
    if (!cloud_vendor) {
        console.warn(
            `${test}: connection details of the cloud vendor were not provided.`,
        );
        throw new Error(
            `Connection details of the cloud vendor were not provided. Please provide connection details`,
        );
    }
    if (!bucket) {
...
async function head_object(cloud_vendor, bucket, key) {
    ...
    try {
        ...
        testing_status.fail.push(test);
        console.error(
            `Test ${test} in bucket ${bucket} on key ${key} failed, error: `,
            err,
        );
    }
}
...
async function delete_object_after_write(
    cloud_vendor,
    bucket,
    key,
    is_write_object,
    ...
    if (is_write_object) {
        ...
        console.warn(
            `Test ${test} in bucket ${bucket} skipped, since object ${key} was not written`,
        );
    }
}
...
async function delete_object(cloud_vendor, bucket, key) {
    ...
    try {
        ...
        testing_status.fail.push(test);
        console.error(
            `Test ${test} in bucket ${bucket} failed, object ${key} was written and not deleted, error: `,
            err,
        );
    }
}
...
function print_test_summary() {
    ...
    console.info(
        `${prefix_of_tests_result} ${resource_name}: ${testing_status.pass.length} passed, ${testing_status.fail.length} failed, and ${testing_status.skip.length} skipped.`,
    );
    if (testing_status.pass.length > 0) {
        console.info(`Passed Tests: ${testing_status.pass}`);
    }
    if (testing_status.fail.length > 0) {
        console.info(`Failed Tests: ${testing_status.fail}`);
    }
    if (testing_status.skip.length > 0) {
        console.info(`Skipped Tests: ${testing_status.skip}`);
    }
}
