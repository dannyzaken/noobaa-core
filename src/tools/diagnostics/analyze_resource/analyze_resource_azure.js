...
class AnalyzeAzure extends CloudVendor {
  constructor(account_name, account_key, endpoint) {
    ...
    const account_name_sensitive =
      account_name instanceof SensitiveString ? account_name : (
        new SensitiveString(account_name)
      );
    const account_key_sensitive =
      account_key instanceof SensitiveString ? account_key : (
        new SensitiveString(account_key)
      );
    ...
    this.azure_blob =
      AZURE.BlobServiceClient.fromConnectionString(connectionString);
  }
  ...
  async head_object(container, blob) {
    const container_client = this.azure_blob.getContainerClient(container);
    const blob_client = container_client
      .getBlobClient(blob)
      .getBlockBlobClient();
  ...
  async write_object(container, blob) {
    ...
    dbg.log0(
      `Write of ${blob} to URL: ${block_blob_client.url} requestId: ${upload_Blob_response.requestId}`,
    );
  }
