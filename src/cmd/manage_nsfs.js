...
const {
    ...
    set_debug_level,
    is_name_update,
    is_access_key_update,
} = require('../manage_nsfs/manage_nsfs_cli_utils');
...
async function main(argv = minimist(process.argv.slice(2))) {
    try {
        if (process.getuid() !== 0 || process.getgid() !== 0) {
            throw new Error(
                'Root permissions required for Manage NSFS execution.',
            );
        }
        const type = argv._[0] || '';
        ...
        const user_input_from_file =
            await manage_nsfs_validations.validate_input_types(
                type,
                action,
                argv,
            );
        ...
        if (type === TYPES.ACCOUNT) {
            ...
            await noobaa_cli_diagnose.manage_diagnose_operations(
                action,
                user_input,
                config_fs,
            );
        } else if (type === TYPES.UPGRADE) {
            await noobaa_cli_upgrade.manage_upgrade_operations(
                action,
                config_fs,
            );
        } else {
...
async function fetch_bucket_data(action, user_input) {
    let data = {
        ...
        _id: undefined,
        name:
            user_input.name === undefined ? undefined : String(user_input.name),
        ...
        new_name:
            user_input.new_name === undefined ?
                undefined
            :   String(user_input.new_name),
        fs_backend:
            user_input.fs_backend === undefined ?
                config.NSFS_NC_STORAGE_BACKEND
            :   String(user_input.fs_backend),
        force_md5_etag:
            (
                user_input.force_md5_etag === undefined ||
                user_input.force_md5_etag === ''
            ) ?
                user_input.force_md5_etag
            :   get_boolean_or_string_value(user_input.force_md5_etag),
    };
    ...
    if (user_input.bucket_policy !== undefined) {
        if (typeof user_input.bucket_policy === 'string') {
            ...
            if (user_input.bucket_policy === '') {
                ...
                data.s3_policy = JSON.parse(
                    user_input.bucket_policy.toString(),
                );
            }
        } else {
            // it is object type
    ...
    if (
        (action === ACTIONS.UPDATE && user_input.owner) ||
        action === ACTIONS.ADD
    ) {
        const account = await get_bucket_owner_account(
            config_fs,
            String(user_input.owner),
        );
    ...
    data.force_md5_etag =
        data.force_md5_etag === '' ? undefined : data.force_md5_etag;
...
async function merge_new_and_existing_config_data(user_input_bucket_data) {
    ...
    try {
        existing_bucket_data = await config_fs.get_bucket_by_name(
            user_input_bucket_data.name,
        );
    } catch (err) {
        throw_cli_error(
...
async function update_bucket(data) {
    ...
    if (name_update) {
        parsed_bucket_data = await config_fs.create_bucket_config_file({
            ...data,
            name: new_name,
        });
    ...
    await set_bucker_owner(parsed_bucket_data);
    return {
        code: ManageCLIResponse.BucketUpdated,
        detail: parsed_bucket_data,
    };
}
...
async function delete_bucket(data, force) {
    try {
        ...
        const bucket_temp_dir_path =
            native_fs_utils.get_bucket_tmpdir_full_path(data.path, data._id);
        ...
        try {
            ...
            dbg.warn(
                `delete_bucket: bucket name ${data.name},` +
                    `got an error on readdir with path: ${data.path}`,
                err,
            );
        ...
        if (entries) {
            ...
            if (object_entries.length > 0 && !force) {
                throw_cli_error(
                    ManageCLIError.BucketDeleteForbiddenHasObjects,
                    data.name,
                );
            }
        }
        ...
        return {
            code: ManageCLIResponse.BucketDeleted,
            detail: '',
            event_arg: { bucket: data.name },
        };
    } catch (err) {
...
async function bucket_management(action, user_input) {
    const data =
        action === ACTIONS.LIST ?
            undefined
        :   await fetch_bucket_data(action, user_input);
...
async function fetch_account_data(action, user_input) {
    const { access_keys = [], new_access_key = undefined } =
        user_input.anonymous ? {} : get_access_keys(action, user_input);
    let data = {
        // added undefined values to keep the order the properties when printing the data object
        _id: undefined,
        name:
            user_input.name === undefined ? undefined : String(user_input.name),
        email:
            user_input.name === undefined ? undefined : String(user_input.name), // temp, keep the email internally
        creation_date:
            action === ACTIONS.ADD ? new Date().toISOString() : undefined,
        new_name:
            user_input.new_name === undefined ?
        ...
        iam_operate_on_root_account:
            user_input.iam_operate_on_root_account === undefined ?
                undefined
            :   get_boolean_or_string_value(
    ...
    if (action === ACTIONS.UPDATE || action === ACTIONS.DELETE) {
        ...
        data = await fetch_existing_account_data(
            action,
            data,
            decrypt_secret_key,
        );
    ...
    data.nsfs_account_config.new_buckets_path =
        data.nsfs_account_config.new_buckets_path || undefined;
...
async function fetch_existing_account_data(action, target, decrypt_secret_key) {
    ...
    try {
        ...
        dbg.warn(
            `fetch_existing_account_data: account with name ${target.name} got an error:`,
            err,
        );
        if (err.code === 'ENOENT') {
    ...
    if (action === ACTIONS.UPDATE) {
        ...
        const dn_update =
            target.nsfs_account_config.distinguished_name !== undefined;
        const user_fs_permissions_change =
            uid_update || gid_update || dn_update;
...
async function add_account(data) {
    ...
    return {
        code: ManageCLIResponse.AccountCreated,
        detail: data,
        event_arg: { account: data.name },
    };
}
...
async function update_account(data) {
    ...
    await config_fs.update_account_config_file(data, {
        ...
        access_keys_to_delete: update_access_key && [
...
async function delete_account(data) {
    ...
    return {
        code: ManageCLIResponse.AccountDeleted,
        detail: '',
        event_arg: { account: data.name },
    };
}
...
async function account_management(action, user_input) {
    ...
    const is_flag_iam_operate_on_root_account = get_boolean_or_string_value(
        user_input.iam_operate_on_root_account,
    );
    ...
    const data =
        action === ACTIONS.LIST ?
            undefined
        :   await fetch_account_data(action, user_input);
    await manage_nsfs_validations.validate_account_args(
        config_fs,
        data,
        action,
        is_flag_iam_operate_on_root_account,
    );
            ...
            if (wide || should_filter) {
                ...
                if (type === TYPES.BUCKET) {
                    ...
                    if (!data.bucket_owner) {
                        ...
                        bucket_owners_map[data.owner_account] =
                            data.bucket_owner;
                    }
                    return data;
                }
...
function get_access_keys(action, user_input) {
    ...
    if (
        ...
        manage_nsfs_validations._validate_access_keys(
...
async function set_bucker_owner(bucket_data) {
    const account_data = await config_fs.get_identity_by_id(
        bucket_data.owner_account,
        TYPES.ACCOUNT,
        { silent_if_missing: true },
    );
