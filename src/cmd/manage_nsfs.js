...
async function check_and_create_config_dirs() {
  ...
  for (const dir_path of pre_req_dirs) {
    try {
      ...
      const dir_exists = await native_fs_utils.is_path_exists(
        fs_context,
        dir_path,
      );
      if (dir_exists) {
        dbg.log1(
          'nsfs.check_and_create_config_dirs: config dir exists:',
          dir_path,
        );
      } else {
        await native_fs_utils._create_path(
          dir_path,
          fs_context,
          config.BASE_MODE_CONFIG_DIR,
        );
        dbg.log1(
          'nsfs.check_and_create_config_dirs: config dir was created:',
          dir_path,
        );
      }
    } catch (err) {
      dbg.log1(
        'nsfs.check_and_create_config_dirs: could not create pre requisite path',
        dir_path,
      );
    }
  }
...
async function main(argv = minimist(process.argv.slice(2))) {
  try {
    ...
    config_root =
      argv.config_root ? String(argv.config_root) : config.NSFS_NC_CONF_DIR;
    ...
    config_root_backend =
      argv.config_root_backend ?
        String(argv.config_root_backend)
    ...
    const manage_err =
      (err instanceof ManageCLIError && err) ||
      new ManageCLIError({
...
async function fetch_bucket_data(action, user_input) {
  let data = {
    ...
    wide:
      _.isUndefined(user_input.wide) ? undefined : (
        get_boolean_or_string_value(user_input.wide)
      ),
    ...
    creation_date:
      action === ACTIONS.ADD ? new Date().toISOString() : undefined,
    path: user_input.path,
    should_create_underlying_storage:
      action === ACTIONS.ADD ? false : undefined,
    new_name:
      _.isUndefined(user_input.new_name) ? undefined : (
        String(user_input.new_name)
      ),
    fs_backend:
      _.isUndefined(user_input.fs_backend) ?
...
async function fetch_existing_bucket_data(target) {
  let source;
  try {
    const bucket_config_path = get_config_file_path(
      buckets_dir_path,
      target.name,
    );
...
async function add_bucket(data) {
  ...
  const exists = await native_fs_utils.is_path_exists(
    fs_context,
    bucket_conf_path,
  );
  if (exists) {
    throw_cli_error(ManageCLIError.BucketAlreadyExists, data.name, {
      bucket: data.name,
    });
  }
...
async function get_bucket_owner_account(bucket_owner) {
  const account_config_path = get_config_file_path(
    accounts_dir_path,
    bucket_owner,
  );
  try {
    ...
    if (err.code === 'ENOENT') {
      ...
      throw_cli_error(
        ManageCLIError.BucketSetForbiddenNoBucketOwner,
        detail_msg,
...
async function get_bucket_status(data) {
  ...
  try {
    ...
    const err_code =
      err.code === 'EACCES' ?
        ManageCLIError.AccessDenied
      : ManageCLIError.NoSuchBucket;
...
async function update_bucket(data) {
  ...
  if (!update_name) {
    ...
    await native_fs_utils.update_config_file(
      fs_context,
      buckets_dir_path,
      bucket_config_path,
      data,
  ...
  const cur_bucket_config_path = get_config_file_path(
    buckets_dir_path,
    cur_name,
  );
  const new_bucket_config_path = get_config_file_path(
    buckets_dir_path,
    data.name,
  );
...
async function delete_bucket(data, force) {
  ...
  const fs_context_config_root_backend =
    native_fs_utils.get_process_fs_context(config_root_backend);
  const fs_context_fs_backend = native_fs_utils.get_process_fs_context(
    data.fs_backend,
  );
  ...
  try {
    ...
    const entries = await nb_native().fs.readdir(
      fs_context_fs_backend,
      data.path,
    );
    const object_entries = entries.filter(
      element => !element.name.endsWith(temp_dir_name),
    );
    if (object_entries.length === 0 || force) {
      await native_fs_utils.folder_delete(
        bucket_temp_dir_path,
        fs_context_fs_backend,
        true,
      );
      await native_fs_utils.delete_config_file(
        fs_context_config_root_backend,
        buckets_dir_path,
        bucket_config_path,
      );
    ...
    if (err.code === 'ENOENT') {
      throw_cli_error(ManageCLIError.NoSuchBucket, data.name);
    }
    throw err;
  }
}
...
async function manage_bucket_operations(action, data, user_input) {
  if (action === ACTIONS.ADD) {
    ...
    const buckets = await list_config_files(
      TYPES.BUCKET,
      buckets_dir_path,
      data.wide,
      undefined,
...
async function account_management(action, user_input) {
  ...
  if ([ACTIONS.ADD, ACTIONS.UPDATE].includes(action) || show_secrets) {
    await nc_mkm.init();
  }
...
function set_access_keys(access_key, secret_key, generate) {
  ...
  if (generate) {
    ({ access_key: generated_access_key, secret_key: generated_secret_key } =
...
async function fetch_account_data(action, user_input) {
  ...
  data.access_keys[0].access_key =
    _.isUndefined(data.access_keys[0].access_key) ? undefined : (
      new SensitiveString(String(data.access_keys[0].access_key))
    );
  // secret_key as SensitiveString
  data.access_keys[0].secret_key =
    _.isUndefined(data.access_keys[0].secret_key) ? undefined : (
  ...
  if (_.isUndefined(user_input.allow_bucket_creation)) {
    data.allow_bucket_creation = !_.isUndefined(
    ...
    data.allow_bucket_creation =
      user_input.allow_bucket_creation.toLowerCase() === 'true';
  }
  // fs_backend deletion specified with empty string '' (but it is not part of the schema)
  data.nsfs_account_config.fs_backend =
    data.nsfs_account_config.fs_backend || undefined;
...
async function fetch_existing_account_data(target, decrypt_secret_key) {
  ...
  try {
    const account_path =
      ...
      : get_symlink_config_file_path(
          access_keys_dir_path,
          target.access_keys[0].access_key,
        );
    ...
    if (err.code === 'ENOENT') {
      if (_.isUndefined(target.name)) {
        throw_cli_error(
          ManageCLIError.NoSuchAccountAccessKey,
          target.access_keys[0].access_key,
        );
      } else {
...
async function add_account(data) {
  ...
  const account_config_relative_path = get_config_file_path(
    acounts_dir_relative_path,
    data.name,
  );
  const account_config_access_key_path = get_symlink_config_file_path(
    access_keys_dir_path,
    access_key,
  );
  ...
  const access_key_exists = await native_fs_utils.is_path_exists(
    fs_context,
    account_config_access_key_path,
    true,
  );
  ...
  if (name_exists || access_key_exists) {
    const err_code =
      name_exists ?
        ManageCLIError.AccountNameAlreadyExists
...
async function update_account(data) {
  ...
  const update_access_key =
    data.new_access_key &&
    cur_access_key &&
    data.new_access_key !== cur_access_key;
  ...
  const cur_account_config_path = get_config_file_path(
    accounts_dir_path,
    cur_name,
  );
  const new_account_config_path = get_config_file_path(
    accounts_dir_path,
    data.name,
  );
  const new_account_relative_config_path = get_config_file_path(
    acounts_dir_relative_path,
    data.name,
  );
  const cur_access_key_config_path = get_symlink_config_file_path(
    access_keys_dir_path,
    cur_access_key,
  );
  const new_access_key_config_path = get_symlink_config_file_path(
    access_keys_dir_path,
    data.access_keys[0].access_key,
  );
  const name_exists =
    update_name &&
    (await native_fs_utils.is_path_exists(fs_context, new_account_config_path));
  const access_key_exists =
    update_access_key &&
    (await native_fs_utils.is_path_exists(
      fs_context,
      new_access_key_config_path,
      true,
    ));
  if (name_exists || access_key_exists) {
    const err_code =
  ...
  if (update_name) {
    await native_fs_utils.create_config_file(
      fs_context,
      accounts_dir_path,
      new_account_config_path,
      encrypted_data,
    );
    await native_fs_utils.delete_config_file(
      fs_context,
      accounts_dir_path,
      cur_account_config_path,
    );
  ...
  await nb_native().fs.symlink(
    fs_context,
    new_account_relative_config_path,
    new_access_key_config_path,
  );
  write_stdout_response(ManageCLIResponse.AccountUpdated, data);
}
...
async function delete_account(data) {
  ...
  const access_key_config_path = get_symlink_config_file_path(
    access_keys_dir_path,
    data.access_keys[0].access_key,
  );
  ...
  await P.map_with_concurrency(10, entries, async entry => {
    if (entry.name.endsWith('.json')) {
      ...
      if (data.bucket_owner === account_name) {
        ...
        throw_cli_error(
          ManageCLIError.AccountDeleteForbiddenHasBuckets,
          detail_msg,
        );
      }
      return data;
...
async function manage_account_operations(
  action,
  data,
  show_secrets,
  user_input,
  ...
  if (action === ACTIONS.ADD) {
    ...
    const accounts = await list_config_files(
      TYPES.ACCOUNT,
      accounts_dir_path,
      data.wide,
      show_secrets,
...
function filter_list_item(type, entity, filters) {
  return type === TYPES.ACCOUNT ?
      filter_account(entity, filters)
    : filter_bucket(entity, filters);
}
  ...
  for (const [key, val] of Object.entries(filters)) {
    if (key === 'uid' || key === 'gid') {
      if (
        ...
        account.nsfs_account_config[key] !== val
      ) {
        return false;
      }
    } else if (key === 'user') {
      if (
        account.nsfs_account_config &&
        account.nsfs_account_config.distinguished_name !== val
      ) {
        return false;
      }
    ...
    } else if (account[key] !== val) {
      // We will never reach here if we will not add an appropriate field to the filter
      return false;
    }
  }
  return true;
 ...
 */
async function list_config_files(
  type,
  config_path,
  wide,
  show_secrets,
  ...
  let config_files_list = await P.map_with_concurrency(
    10,
    entries,
    async entry => {
      if (entry.name.endsWith('.json')) {
        if (wide || should_filter) {
          ...
          const data = await get_config_data(
            full_path,
            show_secrets || should_filter,
          );
          ...
          if (data.access_keys && show_secrets) {
            data.access_keys = await nc_mkm.decrypt_access_keys(data);
          }
          if (should_filter && !filter_list_item(type, data, filters)) {
            return undefined;
          }
          // remove secrets on !show_secrets && should filter
          return wide ?
              _.omit(data, show_secrets ? [] : ['access_keys'])
            : { name: entry.name.slice(0, entry.name.indexOf('.json')) };
        } else {
...
async function get_config_data(config_file_path, show_secrets = false) {
  ...
  const config_data = _.omit(
    JSON.parse(data.toString()),
    show_secrets ? [] : ['access_keys'],
  );
  return config_data;
}
...
function get_access_keys(action, user_input) {
  ...
  if (action === ACTIONS.ADD || action === ACTIONS.STATUS) {
    ...
    access_keys = set_access_keys(
      user_input.access_key,
...
async function validate_bucket_args(data, action) {
  if (action === ACTIONS.DELETE || action === ACTIONS.STATUS) {
    ...
    // action === ACTIONS.ADD || action === ACTIONS.UPDATE
    if (_.isUndefined(data.name)) {
      throw_cli_error(ManageCLIError.MissingBucketNameFlag);
    }
    try {
    ...
    if (
      data.fs_backend !== undefined &&
      !['GPFS', 'CEPH_FS', 'NFSv4'].includes(data.fs_backend)
    ) {
    ...
    const account_fs_context = await native_fs_utils.get_fs_context(
      account.nsfs_account_config,
      data.fs_backend,
    );
    const accessible = await native_fs_utils.is_dir_rw_accessible(
      account_fs_context,
      data.path,
    );
    if (!accessible) {
    ...
    if (action === ACTIONS.ADD) {
      if (!account.allow_bucket_creation) {
        const detail_msg =
          `${data.bucket_owner} account not allowed to create new buckets. ` +
    ...
    if (data.s3_policy) {
      try {
        await bucket_policy_utils.validate_s3_policy(
          data.s3_policy,
          data.name,
          async principal => {
            const account_config_path = get_config_file_path(
...
async function validate_account_args(data, action) {
  if (action === ACTIONS.STATUS || action === ACTIONS.DELETE) {
    ...
    if (
      data.nsfs_account_config.gid &&
      data.nsfs_account_config.uid === undefined
    ) {
      throw_cli_error(
        ManageCLIError.MissingAccountNSFSConfigUID,
        data.nsfs_account_config,
      );
    }
    if (
      data.nsfs_account_config.uid &&
    ...
    if (
      _.isUndefined(data.nsfs_account_config.distinguished_name) &&
      (data.nsfs_account_config.uid === undefined ||
        data.nsfs_account_config.gid === undefined)
    ) {
      throw_cli_error(
        ManageCLIError.InvalidAccountNSFSConfig,
        data.nsfs_account_config,
      );
    }
    if (
      !_.isUndefined(data.nsfs_account_config.fs_backend) &&
      !['GPFS', 'CEPH_FS', 'NFSv4'].includes(
        data.nsfs_account_config.fs_backend,
      )
    ) {
      throw_cli_error(ManageCLIError.InvalidFSBackend);
    ...
    if (!accessible) {
      throw_cli_error(
        ManageCLIError.InaccessibleAccountNewBucketsPath,
        data.nsfs_account_config.new_buckets_path,
      );
    }
  }
...
async function validate_input_types(type, action, argv) {
  ...
  if (
    (type === TYPES.ACCOUNT || type === TYPES.BUCKET) &&
    action === ACTIONS.ADD &&
    path_to_json_options
  ) {
    const input_options_with_data_from_file =
      await get_options_from_file(path_to_json_options);
    const input_options_from_file = Object.keys(
      input_options_with_data_from_file,
    );
...
function validate_type_and_action(type, action) {
  ...
  if (type === TYPES.ACCOUNT || type === TYPES.BUCKET) {
    ...
    if (!Object.values(GLACIER_ACTIONS).includes(action)) {
      throw_cli_error(ManageCLIError.InvalidAction);
    }
  }
}
 ...
 */
function validate_no_extra_options(
  type,
  action,
  input_options,
  is_options_from_file,
) {
  let valid_options; // for performance, we use Set as data structure
  const from_file_condition =
    (type === TYPES.ACCOUNT || type === TYPES.BUCKET) &&
    action === ACTIONS.ADD &&
    input_options.includes(FROM_FILE);
  ...
  const invalid_input_options = input_options.filter(
    element => !valid_options.has(element),
  );
  if (invalid_input_options.length > 0) {
    const type_and_action =
      type === TYPES.IP_WHITELIST ? type : `${type} ${action}`;
    const invalid_option_msg =
      invalid_input_options.length === 1 ?
    ...
    if (from_file_condition) {
      details += ` (when using ${FROM_FILE} flag only partial list of flags are supported)`;
    }
    throw_cli_error(ManageCLIError.InvalidArgument, details);
  }
}
...
function validate_boolean_string_value(value) {
  if (value && typeof value === 'string') {
    const check_allowed_boolean_value = BOOLEAN_STRING_VALUES.includes(
      value.toLowerCase(),
    );
...
async function whitelist_ips_management(args) {
  ...
  try {
    ...
    dbg.error(
      'manage_nsfs.whitelist_ips_management: Error while updation config.json,  path ' +
        config_path,
      err,
    );
