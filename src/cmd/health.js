...
class NSFSHealth {
  ...
  async nc_nsfs_health() {
    ...
    let account_details;
    const response_code =
      endpoint_state ? endpoint_state.response.response_code : 'NOT_RUNNING';
    ...
    if (this.all_bucket_details) {
      bucket_details = await this.get_bucket_status(this.config_root);
    }
    if (this.all_account_details) {
      account_details = await this.get_account_status(this.config_root);
    }
    const health = {
  ...
  async get_service_state(service_name) {
    const service_status = await os_util.exec(
  ...
  async get_service_memory_usage() {
    const memory_status = await os_util.exec(
  ...
  async get_storage_status(config_root, type, all_details) {
    ...
    try {
      ...
      dbg.log1(
        `Config root path missing ${type} folder in ${config_root_type_path}`,
      );
      return {
    ...
    const entries = await nb_native().fs.readdir(
      fs_context,
      config_root_type_path,
    );
    const config_files = entries.filter(
      entree =>
        !native_fs_utils.isDirectory(entree) && entree.name.endsWith('.json'),
    );
  ...
  get_config_path(config_root, type) {
    return path.join(
      config_root,
      type === TYPES.BUCKET ? '/buckets' : '/accounts',
    );
  }
}
 ...
 */
async function get_config_file_data(
  fs_context,
  config_file_path,
  config_file_name,
) {
